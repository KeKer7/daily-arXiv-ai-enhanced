...
TL;DR: 提出一个AI框架，通过同时处理多切片病理图像来预测前列腺癌根治术后的生化复发，在大型数据集上验证了其优于临床基准的预测性能。


<details>
  <summary>Details</summary>
Motivation: 前列腺癌是全球男性高发恶性肿瘤，但根治术后生化复发的精确预测仍然困难，主要挑战在于肿瘤在前列腺腺体内的多灶性分布。

Method: 开发了一个新颖的AI框架，同时处理一系列多切片病理图像，以捕捉整个前列腺腺体的全面肿瘤景观。使用23,451张切片（来自789名患者）的大规模数据集，并整合了补丁和切片子采样策略以降低计算成本。

Result: 该框架在1年和2年生化复发预测中表现出强大的预测性能，显著优于已建立的临床基准。AI衍生的风险评分在多变量Cox比例风险分析中被验证为最强的独立预后因素，超越了术前PSA和Gleason评分等传统临床标志物。外部验证证实了AI的泛化能力。

Conclusion: 研究结果突出了所提出的基于AI的多切片分析作为前列腺癌术后管理可扩展工具的临床可行性和预后价值。
...
TL;DR: 研究通过领域感知激活相似性分析VLM解码器层的结构化剪枝，发现数学推理任务中不同剪枝预算下性能呈现三阶段模式，并提出有效的领域感知剪枝策略。


<details>
  <summary>Details</summary>
Motivation: Transformer视觉语言模型存在深度冗余，但移除特定解码器层的影响尚不清楚，特别是在需要感知与多步推理紧密耦合的数学领域。需要理解深度如何贡献于领域特定行为。

Method: 通过领域感知激活相似性分析，测量每个层对数学与非数学输入的表示转换强度，建立数学感知、非数学感知和混合排名标准，识别在目标领域内输入输出激活变化最小的层。

Result: 在两个最先进的VLM和广泛的数学与通用多模态基准测试中，发现一致的三阶段结构：低剪枝预算时性能对移除哪些层高度敏感；中等预算时方法收敛；高预算时结构连续性占主导。领域感知排名在排名敏感阶段实现最强稳定性。

Conclusion: 研究揭示了深度如何贡献于VLM的领域特定行为，提供了一种实用、可解释的方法来减少模型深度而不牺牲基本数学或通用视觉语言能力。
...
TL;DR: Mix-and-Match Pruning：一种全局引导的层间稀疏化框架，通过结合敏感度分数和架构规则生成多样化高质量剪枝配置，在边缘设备上实现高效压缩。


<details>
  <summary>Details</summary>
Motivation: 在边缘设备上部署深度神经网络需要强压缩同时最小化精度损失。不同层和架构对剪枝的响应不同，单一策略方法效果不佳，需要更灵活的剪枝方案。

Method: 提出Mix-and-Match剪枝框架：1）使用敏感度分数（幅度、梯度或其组合）评估层重要性；2）基于架构感知的稀疏度范围（如保留归一化层，更积极剪枝分类器）；3）系统采样生成每个敏感度信号的十种策略，避免重复剪枝运行。

Result: 在CNN和Vision Transformer上的实验显示Pareto最优结果：Mix-and-Match将Swin-Tiny的精度下降相对标准单准则剪枝减少了40%，提供部署就绪的精度-稀疏度权衡。

Conclusion: 协调现有剪枝信号比引入新准则更能实现可靠高效的压缩模型，Mix-and-Match框架通过架构感知的层间稀疏化策略有效解决了不同层对剪枝响应差异的问题。
...
TL;DR: STAC提出了一种时空感知的缓存压缩框架，用于流式3D重建中的大因果Transformer，通过保留长期信息token和压缩空间冗余token，在减少10倍内存消耗的同时保持重建质量。


<details>
  <summary>Details</summary>
Motivation: 流式3D重建需要长期时间一致性和高效内存使用。现有的因果VGGT Transformer使用KV缓存机制，但缓存随流长度线性增长，造成内存瓶颈。在有限内存预算下，早期缓存逐出会显著降低重建质量和时间一致性。

Method: STAC包含三个关键组件：1) 工作时间token缓存机制，使用衰减累积注意力分数保留长期信息token；2) 长期空间token缓存方案，将空间冗余token压缩为体素对齐表示以高效存储；3) 基于块的多帧优化策略，联合处理连续帧以提高时间一致性和GPU效率。

Result: STAC在保持最先进重建质量的同时，将内存消耗减少近10倍，推理速度提升4倍，显著提高了流式设置中实时3D重建的可扩展性。

Conclusion: STAC通过利用因果Transformer中注意力的时空稀疏性，提出了一种高效的缓存压缩框架，解决了流式3D重建中的内存瓶颈问题，实现了高质量、高效率的实时重建。
...
TL;DR: 提出了两种面向边缘部署的高效视觉异常检测方法：PatchCore-Lite和Padim-Lite，显著减少内存占用和推理时间，适合工业实时检测需求。


<details>
  <summary>Details</summary>
Motivation: 工业质量控制的视觉异常检测系统需要满足严格的实时性和隐私要求，必须从云端处理转向本地边缘部署。但边缘设备内存和计算资源有限，需要高效解决方案。

Method: 基于流行的PatchCore和PaDiM模型，提出两种优化方法：1) PatchCore-Lite：先对产品量化内存库进行粗搜索，然后在解码子集上进行精确搜索；2) Padim-Lite：使用对角协方差，将马氏距离转换为高效的元素级计算。

Result: 在MVTec AD和VisA基准测试中验证了方法的有效性：PatchCore-Lite实现总内存占用减少79%；PaDiM-Lite实现总内存减少77%，推理时间减少31%。

Conclusion: 视觉异常检测可以有效部署在边缘设备上，实现实时、隐私保护且成本效益高的工业检测，为边缘环境提供了实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 遥感图像常受雾、霾和薄云等大气退化影响，遮蔽地表反射信息并阻碍下游应用。目前缺乏对该领域的系统性调查和基准评估，需要提供可靠的经验参考和物理一致性分析。

Method: 将现有方法分为三个阶段演进：基于手工物理先验的方法、数据驱动的深度恢复方法、以及混合物理智能生成方法。在5个公开数据集上使用12个指标进行大规模定量实验，包括PSNR、SSIM、CIEDE、LPIPS、FID等。还进行了专门的物理辐射一致性实验。

Result: 实验表明，基于Transformer和扩散模型的方法平均将SSIM提高了12%~18%，感知误差降低了20%~35%。混合物理引导设计实现了更高的辐射稳定性，具有明确传输或大气光约束的模型将颜色偏差降低了27%。

Conclusion: 总结了开放挑战：动态大气建模、多模态融合、轻量级部署、数据稀缺和联合退化问题。为未来可信、可控、高效的去雾系统发展提出了研究方向。所有资源已在GitHub公开。
...
TL;DR: 提出基于视觉-触觉融合的透明碎片轮廓估计框架，解决透明碎片轮廓估计难题，包括数据集构建、抓取位置检测、触觉信息融合和轮廓匹配算法


<details>
  <summary>Details</summary>
Motivation: 透明碎片轮廓估计在精密光学仪器修复、文物复原等领域至关重要，但透明碎片因严格的光学特性、不规则形状和边缘而面临更大挑战，需要新的解决方案

Method: 1) 构建TransFrag27K透明碎片数据集和可扩展合成数据生成管道；2) 提出TransFragNet视觉抓取位置检测网络；3) 使用带Gelsight Mini传感器的两指夹持器获取触觉信息；4) 提出视觉-触觉融合材料分类器；5) 基于多维相似度度量的轮廓匹配和重组算法

Result: 提出的视觉-触觉融合框架在真实世界验证中表现出强大性能，实验结果表明该框架的有效性，提供了可复现的评估基准

Conclusion: 该研究为透明碎片轮廓估计和重组提供了完整的解决方案，通过视觉-触觉融合方法有效解决了透明碎片轮廓估计的挑战，数据集和代码已开源
...
TL;DR: 提出基于教师的知识保留方法，用于高光谱图像增量分类，通过增量类别样本缓解模型遗忘，无需依赖旧类别样本，并引入基于掩码的部分类别知识蒸馏算法提升精度。
...
Result: 对比实验和消融实验表明，所提方法具有鲁棒的性能表现，能够有效缓解灾难性遗忘问题，提升整体分类精度。

Conclusion: 该方法成功解决了高光谱图像增量分类中的灾难性遗忘问题，通过不依赖旧类别样本的知识保留策略和优化的知识蒸馏技术，实现了更好的分类性能。
...
TL;DR: 本文提出InjectFlow方法，通过注入正交语义来缓解Flow Matching模型中的数据集偏差问题，无需重新训练即可修复75%的生成失败提示。
...
Method: 首先在FM框架中形式化"偏差流形"概念，识别出条件期望平滑机制导致推理时的轨迹锁定问题。然后提出InjectFlow方法，在初始速度场计算时注入正交语义，无需改变随机种子，防止潜在表示向多数模式漂移。

Result: 在GenEval数据集上，InjectFlow成功修复了标准流匹配模型无法正确生成的75%提示。该方法在保持高生成质量的同时，有效防止了向多数模式的潜在漂移。

Conclusion: 理论分析和算法为构建更公平、更鲁棒的视觉基础模型提供了即用型解决方案，解决了FM模型中的数据集偏差问题。
...
TL;DR: DiffMark：一种用于扩散模型的即插即用水印方法，支持单次多比特检测、每图像密钥灵活性和跨模型可迁移性，无需完整DDIM反演或模型微调。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型水印方法存在两大问题：基于采样的方法需要昂贵的N步DDIM反演进行零比特检测；基于微调的方法虽然能快速提取多比特水印，但需要针对每个模型架构重新训练。需要一种更高效、灵活的水印方案。

Method: DiffMark通过在冻结扩散模型的每个去噪步骤中注入持久学习扰动δ，水印信号在最终去噪潜在空间z0中累积。使用潜在一致性模型作为可微分训练桥梁，将梯度步骤从50步DDIM减少到4步LCM，实现单次检测。编码器学习将运行时密钥映射到唯一扰动。

Result: DiffMark实现单次检测仅需16.4毫秒，比基于采样的方法快45倍。支持真正的每图像密钥灵活性，无需模型微调即可迁移到未见过的扩散架构。在对抗失真、再生和攻击时保持竞争性的水印鲁棒性。

Conclusion: DiffMark提供了一种高效、灵活且可迁移的扩散模型水印解决方案，解决了现有方法的局限性，在保持鲁棒性的同时显著提升了检测速度和部署灵活性。
...
TL;DR: 该论文指出当前地理空间数据融合主要采用"主从"范式，缺乏对称互利的融合方法，提出需要建立更有效的多源数据交互方案来发挥"全球-本地循环"的潜力。


<details>
  <summary>Details</summary>
Motivation: 当前地理空间数据融合面临挑战：虽然存在海量多尺度、多来源的地理空间数据，但现有融合方法大多采用"主从"范式，即一种数据辅助处理另一种"主要"数据，缺乏相互受益的对称融合。这种范式限制了不同尺度、不同社区数据源之间的桥梁和反馈，未能充分利用"全球-本地循环"的潜力。

Method: 通过案例研究建立最相关的交互方案，讨论未充分探索的研究方向，这些方向可以利用跨多个范围和社区的可利用数据。

Result: 论文提出了需要建立更有效的多源数据交互方案，特别是对称利用多个数据源的方法，以更好地服务于通用或专题应用。

Conclusion: 需要打破当前地理空间数据融合中的"主从"范式，建立对称互利的融合方法，促进不同尺度、不同社区数据源之间的桥梁和反馈，充分发挥"全球-本地循环"的潜力，这对通用和专题应用都具有重要意义。
...
TL;DR: EARTalking：一种用于交互式音频驱动说话人生成的端到端GPT风格自回归模型，通过帧级流式生成和创新的注意力机制实现高效、灵活的控制


<details>
  <summary>Details</summary>
Motivation: 现有AR方法依赖中间面部表示限制了表现力和真实感，而基于扩散的方法采用逐片段生成，缺乏细粒度控制且存在延迟。需要一种能够支持交互式控制、高效生成的方法

Method: 提出EARTalking：1）帧级流式生成范式；2）Sink Frame Window Attention机制支持变长视频生成和身份一致性；3）Frame Condition In-Context方案流式注入多样化控制信号

Result: 实验表明EARTalking优于现有自回归方法，性能可与基于扩散的方法相媲美，实现了高效的流式生成和交互控制

Conclusion: 证明了上下文流式自回归控制的可行性，为灵活高效的生成开辟了可扩展的方向，代码将开源以确保可复现性
...
TL;DR: GraphiContact：一个联合3D人体网格重建与顶点级人-场景接触预测的框架，通过Transformer编码器迁移人体先验，并引入SIMU训练策略提升鲁棒性。
...
Method: 提出GraphiContact框架，从两个预训练Transformer编码器迁移互补人体先验，在重建网格上预测顶点级人-场景接触。引入SIMU训练策略，通过令牌级自适应路由模拟遮挡和噪声观测，同时保持测试时高效单分支推理。

Result: 在五个基准数据集上的实验表明，GraphiContact在接触预测和3D人体重建方面均取得一致提升。

Conclusion: GraphiContact有效解决了单目图像中3D人体重建与顶点级接触预测的联合任务，通过迁移人体先验和SIMU训练策略提升了鲁棒性，为交互系统提供了综合解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型经常基于语言先验而非视觉证据产生幻觉响应，这在临床应用中存在风险。现有方法通常需要训练或使用固定权重的对比方法，缺乏对每个token的自适应控制。

Method: 提出视觉基础分数引导解码(VGS-Decoding)：1) 引入视觉基础分数(VGS)，通过比较原始图像和失真图像的token分布差异来衡量每个token的视觉依赖性；2) 在解码过程中，根据VGS重新加权概率，放大视觉基础token，抑制幻觉token；3) 无需训练，仅在推理时应用。

Result: 在MIMIC-Diff-VQA和VQA-RAD数据集上，对LLaVA-Med、CheXagent和MedGemma模型进行实验，结果显示：整体性能提升高达+9.12%，开放问答召回率提升+8.98%，仅增加2倍推理开销，无需额外训练。

Conclusion: VGS-Decoding是一种实用且有效的训练免费方法，能够自适应地减少医学视觉语言模型的幻觉问题，适合临床部署，代码将公开以促进可重复性。
...
TL;DR: 论文提出基于工作负载的框架，用于决定哪些AI任务适合在轨道数据中心执行，通过语义抽象大幅减少数据传输需求，支持早期太空计算应用


<details>
  <summary>Details</summary>
Motivation: 随着发射成本下降和AI工作负载增长，太空计算变得可行。需要确定哪些任务适合在轨道执行，哪些适合地面云，以优化资源利用

Method: 提出工作负载中心框架和分阶段采用模型，通过轨道数据中心的成熟度评估任务分配。开发了在轨语义缩减原型，包括地球观测管道和多视角立体重建系统

Result: 西雅图和班加罗尔的Sentinel-2图像处理实现99.7-99.99%有效载荷缩减；立体重建从306MB减少到1.57MB（99.49%缩减）。证明语义抽象而非原始计算规模驱动早期工作负载适用性

Conclusion: 工作负载优先视角表明，语义抽象是早期太空计算的关键驱动因素。框架为太空计算部署提供实用指导，支持分阶段采用策略
...
TL;DR: 提出了一种基于节点中心框架的视频人体姿态估计方法，通过显式整合视觉、时间和结构推理，使用速度嵌入、注意力编码器和双分支时空图来提升姿态估计精度。


<details>
  <summary>Details</summary>
Motivation: 现有视频姿态估计方法存在运动模糊、遮挡和复杂时空动态等问题，传统基于热图或隐式时空特征聚合的方法限制了关节拓扑表达能力和跨帧一致性。

Method: 1) 视觉-时间速度关节嵌入：融合亚像素关节线索和帧间运动；2) 注意力驱动姿态查询编码器：将关节表示映射到姿态感知节点空间；3) 双分支解耦时空注意力图：在专门分支中建模时间传播和空间约束推理；4) 节点空间专家融合模块：自适应融合两个分支的互补输出。

Result: 在三个广泛使用的视频姿态基准测试上进行了大量实验，结果表明该方法优于现有最先进方法。

Conclusion: 显式节点中心推理为推进视频人体姿态估计提供了新视角，验证了该方法在解决运动模糊、遮挡和时空动态问题上的有效性。
...
TL;DR: DCG-Net：整合多模态对齐与结构化概念推理的端到端可解释框架，通过双交叉注意力模块和参数化概念图提升医学图像分析的性能和可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型在医学图像分析中性能优秀但缺乏可解释性，概念瓶颈模型虽部分解决此问题但忽略了概念间的上下文依赖关系，需要开发能同时考虑概念依赖和空间定位证据的可解释框架。

Method: 提出DCG-Net框架：1）双交叉注意力模块，用双向注意力替代余弦相似度匹配，实现视觉标记与规范化文本概念-值原型之间的空间定位证据归因；2）参数化概念图，用正点互信息先验初始化并通过稀疏控制的消息传递进行优化，建模符合临床领域知识的概念间依赖关系。

Result: 在白细胞形态和皮肤病变诊断实验中，DCG-Net实现了最先进的分类性能，同时产生临床可解释的诊断解释。

Conclusion: DCG-Net通过整合多模态对齐与结构化概念推理，在保持高性能的同时提供了临床可解释的诊断解释，解决了现有概念瓶颈模型忽略概念依赖关系的问题。
...
TL;DR: 提出一种无需提示的轻量级SAM适配方法，用于组织病理学细胞核分割，仅需微调4.1M参数，在多个数据集上达到SOTA性能
...
Conclusion: 该方法为组织病理学应用提供了一种有效且实用的细胞核分割框架，具有轻量化和强泛化优势
...
TL;DR: 提出一种尺度编码的神经表面表示方法，解决多视角法线积分中因拍摄距离不同导致的法线不一致问题，实现高保真表面重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法对每个像素只采样单条光线，未考虑像素覆盖的空间区域随相机内参和距离变化而变化。当目标物体在不同距离拍摄时，对应像素的法线可能不同，导致多视角法线不一致，重建表面高频细节模糊。

Method: 提出尺度编码的神经表面表示，将像素覆盖面积纳入神经表示。每个3D点关联空间尺度，通过混合网格编码计算法线。引入网格提取模块，基于训练观察为每个顶点分配最优局部尺度。

Result: 实验结果表明，该方法在不同距离观察的法线下能一致实现高保真表面重建，优于现有多视角法线积分方法。

Conclusion: 通过将像素覆盖面积纳入神经表示并引入尺度感知机制，有效解决了多视角法线不一致问题，实现了高质量的表面重建。
...
TL;DR: MV-BrainFM是一个多视角脑网络基础模型，通过整合解剖距离先验和跨视角一致性学习，实现任意图谱构建的脑网络的通用表示学习，在17个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有脑网络基础模型存在三个主要局限：1）图谱依赖性（atlas dependency），模型性能受限于特定脑区图谱；2）未能充分利用多视角网络信息；3）解剖先验知识整合不足。需要开发一个能够从任意图谱构建的脑网络中学习通用、可扩展表示的模型。

Method: 1）在Transformer建模中显式整合解剖距离信息，指导脑区间的交互；2）引入无监督跨视角一致性学习策略，将同一受试者不同图谱的表示对齐到共享潜在空间；3）采用统一的多视角预训练范式，支持同时从多个数据集和图谱学习，提高计算效率。

Result: 在超过20,000名受试者（来自17个fMRI数据集）上的实验表明，MV-BrainFM在单图谱和多图谱设置下均一致优于14个现有脑网络基础模型和任务特定基线。模型展现出强可扩展性，随着数据多样性增加而持续受益，同时在未见图谱配置上保持稳定性能。

Conclusion: MV-BrainFM通过整合解剖先验和跨视角对齐，成功构建了一个通用、可扩展的脑网络基础模型，能够从任意图谱构建的脑网络中学习鲁棒表示，为脑网络分析提供了更灵活、高效的解决方案。
...
TL;DR: 提出360°显著性图表示法，将场景的视觉、上下文、语义和几何信息编码为图节点、边和权重，用于增强室内视觉导航和场景定位


<details>
  <summary>Details</summary>
Motivation: 现有场景表示方法（RGB-D、LiDAR、关键点等）包含的隐式信息对场景索引、视觉导航等应用不够高效，且传统方法在室内环境中面临光照变化、遮挡、阴影等挑战

Method: 提出新颖的360°显著性图表示，将场景相关视觉、上下文、语义和几何信息显式编码为图的节点、边、边权重和角度位置，该表示对场景视角变化具有鲁棒性

Result: 实验结果表明，所提出的360°显著性图表示能有效增强场景定位和基于视觉的室内导航性能

Conclusion: 360°显著性图是一种丰富高效的场景表示方法，能解决室内环境挑战并提升视觉导航应用效果
...
Method: 提出Uni-Classifier (Uni-C)模块，利用视频扩散先验来指导上游模型的去噪过程，使其输出与下游模型要求对齐。该模块也可独立应用于单个生成模型以提升其输出质量。

Result: 在视频和3D生成任务上的大量实验表明，Uni-C在工作流级联和独立应用场景下都能持续提升生成质量，展现了其多功能性和强大的泛化能力。

Conclusion: Uni-C是一个简单有效的解决方案，能够解决多模型工作流中的分布不匹配问题，提升生成质量，并具有广泛的适用性。
...
TL;DR: 该论文提出了一种用于13类白细胞分类的多阶段微调方法，通过训练不同分类器头（线性、余弦、MLP）并构建头多样性集成模型，解决了白细胞分类中的类别不平衡、领域偏移和形态连续体混淆等挑战。


<details>
  <summary>Details</summary>
Motivation: 白细胞分类对白血病诊断至关重要，但自动化方法面临类别不平衡、领域偏移和形态连续体混淆（相邻成熟阶段特征重叠）等挑战。WBCBench 2026挑战赛（ISBI 2026）需要解决这些难题。

Method: 采用多阶段微调方法，基于DINOBloom-base模型训练三种不同的分类器头家族（线性、余弦、MLP）。根据各类别的特异性表现，构建头多样性集成模型：MLP头作为主要预测器，仅在四个预定义混淆对中，当另外两个头家族达成一致时才替换其预测。

Result: 不同分类器头在不同类别上表现最佳：余弦头在成熟粒细胞边界（BNE F1=0.470）表现最好，线性头在较不成熟粒细胞（MMY F1=0.585）表现最佳，MLP头在最不成熟粒细胞（PMY F1=0.733）表现最优。集成模型进一步提升了性能，同时发现所有模型持续误分类的病例很可能存在标注错误或固有形态模糊性。

Conclusion: 多阶段微调方法和头多样性集成策略有效解决了白细胞分类中的挑战，不同分类器头在不同类别上表现出特异性优势。该方法不仅提升了分类性能，还能识别潜在的标注错误和形态模糊病例，为临床诊断提供了更可靠的自动化工具。
...
TL;DR: 提出一种结合视觉基础模型嵌入和图神经网络的新型多示例学习方法，通过局部空间结构和跨补丁空间感知提升病理图像分类性能


<details>
  <summary>Details</summary>
Motivation: 传统多示例学习(MIL)方法将病理图像补丁视为可交换的，忽略了组织图像中丰富的空间和拓扑结构。现有方法缺乏对局部空间结构和跨补丁空间关系的有效建模。

Method: 1) 使用视觉基础模型嵌入捕捉每个补丁内的局部空间结构；2) 采用图神经网络结合新颖的拼图正则化实现跨补丁空间感知

Result: 在乳腺癌、头颈癌和结肠癌的基准数据集上，该方法显著优于最先进的基于注意力的MIL方法

Conclusion: 结合局部空间结构建模和跨补丁空间感知能有效提升计算病理学中全切片图像的分类性能，为组织图像分析提供了更全面的空间结构建模方法
...
TL;DR: 提出无需训练、无需相机标定的多视角人体网格重建框架，利用预训练单视角模型作为先验，通过测试时优化实现多视角一致性


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：几何方法需要繁琐的相机标定，而学习方法因缺乏多视角训练数据导致泛化能力差，难以适应真实世界任意相机配置

Method: 利用预训练单视角HMR模型作为强先验，无需多视角训练数据；首先从单视角预测构建鲁棒一致的多视角初始化，然后通过测试时优化在多视角一致性和解剖约束指导下进行细化

Result: 在标准基准测试中达到最先进性能，超越了需要显式多视角监督训练的多视角模型

Conclusion: 提出了一种无需标定、无需多视角训练数据的框架，利用单视角模型先验和测试时优化，实现了对任意相机配置的强泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 传统全微调在多任务学习中成本过高，现有参数高效微调方法使用固定秩且无法学习任务间空间关系，需要更优的适配器秩分配和跨任务关系建模方法。

Method: 提出FAAR方法：1) 性能驱动秩缩减(PDRS)为每个适配器位置和任务分配最优秩；2) 任务-频谱金字塔解码器(TS-PD)通过图像频谱分析注入输入特定上下文到空间偏置学习中，更好地反映跨任务关系。

Result: 在密集视觉任务基准测试中，FAAR在准确性和效率上优于其他PEFT方法，相比传统MTL微调减少多达9倍参数的同时提升整体性能。

Conclusion: FAAR通过自适应秩分配和频谱感知的空间关系建模，为多任务学习提供了高效且高性能的参数微调解决方案。
...
TL;DR: 提出了个性化流式视频理解新任务PSVU，构建首个基准PEARL-Bench，并提出无需训练的即插即用方法PEARL作为强基线。


<details>
  <summary>Details</summary>
Motivation: 人类认知新概念是连续流式过程，但现有多模态个性化方法局限于静态图像或离线视频，无法提供实时交互的个性化响应，限制了未来AI助手的发展。

Method: 1) 提出并正式定义PSVU新任务；2) 构建PEARL-Bench基准，包含132个独特视频和2,173个细粒度标注，支持帧级和视频级两种模式；3) 提出PEARL方法，即插即用、无需训练的策略。

Result: PEARL在8个离线和在线模型上达到最先进性能，在3种不同架构上带来一致的PSVU改进，证明其高效性和鲁棒性。

Conclusion: 该工作推动了视觉语言模型个性化研究，为流式个性化AI助手的发展提供了新方向。代码已开源。
...
TL;DR: 该论文提出了一个专注于新颖视角合成的SfM基准测试，通过两种简单策略展示了如何在保持高精度姿态估计的同时提高SfM效率。


<details>
  <summary>Details</summary>
Motivation: 当前SfM方法存在效率与精度之间的权衡：传统方法（如bundle adjustment）精度高但计算慢，而基于神经网络的前馈方法速度快但精度低。需要开发既高效又有效的SfM方法。

Method: 1. 使用更少的特征点来加速传统SfM方法；2. 使用前馈网络获取初始估计，然后用传统SfM技术进行精炼。基于现有数据集构建基准测试。

Result: 1. 仅使用较少特征点就能显著加速传统SfM方法，同时保持高姿态精度；2. 前馈网络+传统SfM精炼的组合实现了最佳效率-效果权衡。

Conclusion: 该研究为开发既高效又有效的SfM方法提供了基准测试和实用策略，展示了简单方法就能在效率与精度之间取得良好平衡。
...
TL;DR: 提出轻量级预处理和splatting流程，解决热成像在神经辐射场中的动态范围低和光度不稳定问题，实现热成像新视角合成SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 热成像相机在黑暗和恶劣条件下提供可靠可见性，但热图像在新视角合成中比可见光图像更难使用。主要困难来自两个特性：1) 热图像动态范围极低，削弱了外观线索并限制了优化梯度；2) 热数据存在快速帧间光度波动和缓慢辐射漂移，破坏了对应关系估计并在视角合成中产生高频漂浮伪影。

Method: 引入轻量级预处理和splatting流程，扩展可用动态范围并稳定每帧光度测量。该方法不需要任何数据集特定的调优。

Result: 在纯热成像新视角合成基准测试中实现了最先进的性能。

Conclusion: 通过解决热成像特有的动态范围和光度稳定性问题，提出的方法显著提升了热成像新视角合成的质量，无需RGB引导即可实现可靠性能。
...
TL;DR: 本文提出两种解决神经网络逆映射问题的方法：前向传递法（基于根查找和输入图像雅可比矩阵）和后向传递法（逐层反向传播并添加线性层零空间随机向量），能生成随机样式的输入图像获得完美分类分数，揭示网络脆弱性。


<details>
  <summary>Details</summary>
Motivation: 神经网络系统描述复杂的映射关系，这些映射通常难以理解。本文研究逆问题：确定哪些输入图像会被映射到特定的神经网络类别。期望这些图像包含与其对应类别分类相关的可识别特征。

Method: 提出两种通用方法：1) 前向传递法：基于根查找算法和输入图像雅可比矩阵的逆方法；2) 后向传递法：从顶层开始迭代地反转每个层，在反转过程中为每个线性层添加从零空间采样的随机向量。

Result: 在transformer架构和基于线性层的序列网络上验证了新方法。与先前方法不同，新方法能够生成随机样式的输入图像，在所有情况下都能获得近乎完美的分类分数，揭示了底层网络的脆弱性。

Conclusion: 所提出的方法为解决逆映射问题提供了更全面的输入图像空间覆盖，表明这些方法能有效生成欺骗神经网络分类器的对抗性输入。
...
TL;DR: CREG是一个无需训练的VLM可解释性框架，通过将多层对比梯度-激活归因投影到极坐标系中，生成方向性证据分布，用于解释视觉语言模型中的空间方向关系。


<details>
  <summary>Details</summary>
Motivation: 现有归因方法（如GradCAM和注意力展开）只能显示模型关注的位置，但不能揭示对象之间的方向关系。需要一种能够解释VLM如何编码方向性关系的框架。

Method: CREG框架将多层对比梯度-激活归因投影到以参考对象为中心的极坐标系中，生成方向性证据分布。提出了三个评估指标：方向对齐误差、边缘准确率和因果遮挡分数。

Result: 在Qwen2-VL-7B模型上，CREG在VSR和COCO-Pairs数据集上均优于标准归因基线。在COCO-Pairs上，预测目标CREG实现了55.5度的方向对齐误差和0.553的边缘准确率，比注意力展开方法分别改善了16.1度和0.120。

Conclusion: 对比性多层归因能够比标准显著性解释更忠实地揭示VLM空间推理中的方向性证据。CREG在更大规模模型上效果更好，表明它受益于更大规模模型中出现的更结构化空间表示。
...
TL;DR: 该研究首次系统分析了相机陷阱物种识别在时间维度上的挑战，提出了包含546个相机陷阱的流式评估基准，发现生物基础模型在站点适应、时间漂移等方面存在显著问题，并提出了改进策略。


<details>
  <summary>Details</summary>
Motivation: 现有计算机视觉研究主要关注跨域泛化，但忽视了生态实践中更关键的挑战：在固定站点随时间推移保持可靠识别能力，因为生态系统动态变化会导致背景和动物分布的显著时间漂移。

Method: 构建包含546个相机陷阱的现实基准，采用流式评估协议按时间顺序评估模型；研究模型在真实部署场景下的适应策略，包括使用过去数据更新模型并评估未来时间间隔的性能。

Result: 发现生物基础模型（如BioCLIP 2）在多数站点初始阶段表现不佳；在真实评估中，朴素适应策略甚至可能低于零样本性能；识别出类别严重不平衡和时间漂移是主要困难；有效整合模型更新和后处理技术可显著提升准确性。

Conclusion: 研究为生态从业者提供了可操作的部署指南，同时为计算机视觉和机器学习领域确立了新的研究方向，包括预测零样本模型在新站点的成功概率以及确定模型更新的必要性时机。
...
<details>
  <summary>Details</summary>
Motivation: 偏振信息对材料识别很重要，但传统测量需要多次偏振态调制，耗时且对某些识别任务不必要。现有方法使用偏振测量子集时，最优测量角度配置不明确。

Method: 提出端到端优化框架，联合学习材料分类器并确定控制入射和反射光状态的偏振元件旋转角度的最优组合。使用Mueller矩阵材料数据集。

Result: 方法在有限测量次数下实现了高精度的材料分类。

Conclusion: 通过联合优化分类器和测量配置，可以在减少测量次数的同时保持材料分类的高精度，为高效偏振材料识别提供了解决方案。
...
TL;DR: 该论文研究了CLIP模型理解否定词的能力不足问题，提出了基于多模态LLM的评估框架，发现了CLIP嵌入空间中存在否定方向，并通过表示工程实现无需微调的否定感知行为调控。


<details>
  <summary>Details</summary>
Motivation: CLIP等联合视觉-语言嵌入模型在理解文本查询中的否定词（如"no"）方面存在明显缺陷，现有研究主要通过数据驱动方法解决，但评估指标不可靠，无法准确反映模型是否真正理解了否定含义。

Method: 1) 识别现有检索评估指标的两个关键限制；2) 提出基于多模态LLM作为评判者的替代评估框架；3) 探索CLIP嵌入空间中是否存在否定方向；4) 通过表示工程进行测试时干预，无需微调即可调控CLIP的否定感知行为；5) 在非常见图像-文本样本上测试否定理解的泛化能力。

Result: 1) 发现了CLIP嵌入空间中确实存在与否定相关的方向；2) 通过表示工程可以有效地引导CLIP表现出否定感知行为；3) 在分布偏移下评估了否定理解的泛化性能。

Conclusion: 该研究为CLIP模型的否定理解问题提供了更可靠的评估框架，并展示了通过表示工程进行测试时干预的有效性，为改进多模态模型的语义理解能力提供了新思路。
...
TL;DR: 提出SGG混合引导方法，基于弱到强原则结合CFG和AG优势，提升扩散模型泛化能力


<details>
  <summary>Details</summary>
Motivation: 现有引导方法（如CFG和AG）的有效操作机制研究不足，导致选择困难；扩散模型中无模拟目标与迭代过程的不匹配导致梯度误差累积，影响结果质量和泛化能力

Method: 1. 通过合成比较从弱到强原则角度分析CFG和AG的有效机制；2. 提出SGG混合实例化方法，结合两者优势；3. 将W2S原则和SGG迁移到训练目标中，提升无引导扩散模型的泛化能力

Result: 1. 推理时，SGG在SD3和SD3.5上优于现有训练免费引导变体；2. 训练时，在Transformer架构上验证了有效迁移和性能提升，在条件和无条件设置中均有效

Conclusion: 基于弱到强原则提出的SGG方法有效结合了CFG和AG的优势，既能提升推理性能，又能迁移到训练目标中增强扩散模型的泛化能力
...
TL;DR: RayMap3R是一种免训练的流式动态场景重建框架，通过对比RayMap与图像预测识别动态区域，抑制其对内存更新的干扰，在动态场景重建中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有的流式前馈3D重建方法能够实时估计场景几何和相机位姿，但在动态物体存在时会产生伪影和漂移问题，缺乏显式的动态推理能力。
...
Conclusion: RayMap3R通过利用RayMap的静态场景偏差进行动态识别，结合双分支推理和稳定性增强技术，实现了高效准确的动态场景流式重建，无需额外训练。
...
TL;DR: GaussianPile：结合3D高斯泼溅与成像系统感知焦点模型，用于切片体数据的高效压缩与重建，实现16倍压缩和11倍加速


<details>
  <summary>Details</summary>
Motivation: 切片体成像应用广泛，需要既能高压缩又能保持内部结构以进行分析的表示方法。现有方法在压缩效率、重建质量和计算速度之间存在权衡。

Method: 提出三个关键创新：1) 切片感知堆积策略，定位各向异性3D高斯分布以建模跨切片贡献；2) 可微投影算子，编码成像系统的有限厚度点扩散函数；3) 紧凑编码和联合优化管道，同时重建和压缩高斯集合。基于CUDA的设计保持高斯原语的压缩和实时渲染效率。

Result: 在显微镜和超声数据集上验证，方法减少存储和重建成本，保持诊断保真度，支持快速2D可视化和3D体素化。仅需3分钟获得高质量结果，比NeRF方法快11倍，实现相对于体素网格的16倍压缩。

Conclusion: GaussianPile为切片体数据集的部署压缩和探索提供了实用路径，统一了3D高斯泼溅与成像系统感知焦点模型，在压缩效率、重建质量和计算速度方面取得显著改进。
...
TL;DR: ScaleEditor：开源多智能体框架，构建大规模高质量图像编辑数据集ScaleEdit-12M，提升统一多模态模型编辑能力


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑数据集依赖闭源模型标注（成本高）或固定合成流程（质量有限），需要开源、可扩展的高质量数据集构建方案

Method: 提出ScaleEditor框架：1）世界知识注入的源图像扩展；2）自适应多智能体编辑指令-图像合成；3）任务感知数据质量验证机制

Result: 构建ScaleEdit-12M（最大开源图像编辑数据集，涵盖23个任务族）；微调UniWorld-V1和Bagel模型，在ImgEdit/GEdit基准上提升10.4%/35.1%，在RISE/KRIS-Bench上提升150.0%/26.5%

Conclusion: 开源智能体管道可接近商业级数据质量，同时保持成本效益和可扩展性；框架和数据集将开源
...
TL;DR: 提出MCL-FIR框架，通过多头设计、对比学习和EMA蒸馏实现细粒度时尚图像检索的类增量学习，在动态场景中显著降低训练成本。


<details>
  <summary>Details</summary>
Motivation: 现有细粒度时尚图像检索方法假设静态设置，当新属性出现时需要完全重新训练，成本高昂且不适用于动态场景。虽然预训练模型支持零样本推理，但无监督时准确性下降，且尚无工作探索细粒度FIR的类增量学习。

Method: 提出MCL-FIR框架：1）多头设计以适应增量中的演化类别；2）将三元组输入重构为使用InfoNCE的双重输入，实现更简单有效的训练；3）采用EMA蒸馏进行高效知识迁移。

Result: 在四个数据集上的实验表明，MCL-FIR在可扩展性之外，实现了效率与准确性的良好平衡。在相似训练成本下显著优于CIL基线方法，与静态方法相比，仅使用约30%的训练成本就能达到相当的性能。

Conclusion: MCL-FIR为细粒度时尚图像检索提供了一种有效的类增量学习解决方案，在动态场景中实现了训练成本的大幅降低和性能的良好平衡。
...
TL;DR: IBCapsNet：基于信息瓶颈原则的胶囊网络，通过变分聚合机制替代迭代路由，在保持准确率的同时显著提升鲁棒性和效率。


<details>
  <summary>Details</summary>
Motivation: 传统胶囊网络存在两个关键限制：1）由于迭代动态路由导致高计算成本；2）在输入损坏时鲁棒性差。需要一种既能保持胶囊网络层次空间建模优势，又能解决这些限制的新架构。

Method: 基于信息瓶颈原则，采用一次性变分聚合机制：首先将初级胶囊压缩为全局上下文表示，然后通过类别特定的变分自编码器处理，通过KL散度正则化推断潜在胶囊。这种设计实现了高效推理并固有地过滤噪声。

Result: 在MNIST、Fashion-MNIST、SVHN和CIFAR-10上，IBCapsNet在干净数据准确率上与CapsNet相当（MNIST 99.41%，SVHN 92.01%），但在四种合成噪声下显著优于CapsNet：对钳位加性噪声平均提升+17.10%，对乘性噪声提升+14.54%。训练速度提升2.54倍，推理吞吐量提升3.64倍，模型参数减少4.66%。

Conclusion: IBCapsNet将信息论表示学习与胶囊网络相结合，为构建鲁棒、高效且可解释的深度模型提供了原则性路径，解决了传统胶囊网络的计算效率和鲁棒性限制。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散和流模型在真实图像超分辨率中效果显著，但多步采样导致推理速度慢、部署困难。单步蒸馏虽然降低成本，但往往牺牲恢复质量且无法进行多步优化。

Method: 使用MeanFlow作为学习目标，让学生模型能够近似PF-ODE任意状态间的平均速度，无需显式展开即可捕捉教师模型的动态。改进原始MeanFlow的CFG公式，采用教师CFG蒸馏策略，增强恢复能力并保留细节。

Result: 在合成和真实世界基准测试中，MFSR实现了高效、灵活且高质量的超分辨率，结果与多步教师模型相当甚至更好，同时计算成本大幅降低。

Conclusion: MFSR框架在保持高质量超分辨率的同时，提供了单步推理的高效率和可选多步优化的灵活性，解决了现有方法在速度和质量之间的权衡问题。
...
TL;DR: 该研究提出卫星到街景合成方法，通过VLM引导和损伤敏感MoE两种生成策略，从卫星图像合成灾后街景，以解决灾后地面数据获取困难的问题。


<details>
  <summary>Details</summary>
Motivation: 自然灾害后需要快速态势感知，但卫星观测缺乏地面视角，而地面数据（如街景图像）在时效性事件中难以获取，因此需要桥接这一数据鸿沟。

Method: 提出两种生成策略：1）VLM引导方法；2）损伤敏感MoE方法。建立结构感知评估框架，包含像素级质量评估、ResNet语义一致性验证和VLM-as-a-Judge感知对齐。

Result: 在300个灾害场景实验中，发现现实感-保真度权衡：扩散方法（如ControlNet）感知现实感高但会幻觉结构细节。ControlNet语义准确度最高（0.71），VLM增强和MoE模型纹理合理性好但语义清晰度不足。

Conclusion: 建立了可信跨视角合成的基准，强调视觉上真实的生成可能仍无法保留可靠灾害评估所需的关键结构信息。
...
TL;DR: 提出CogAlign框架解决MLLMs在胃肠内镜应用中的临床认知对齐和因果关联问题，通过层次化临床认知数据集SFT和反事实强化学习实现SoTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在医学图像分析中面临两个关键限制：1）通用模型推理与标准化临床认知路径之间的不对齐；2）视觉特征与诊断结果之间缺乏因果关联，阻碍了在胃肠内镜中的应用。

Method: 提出临床认知对齐框架CogAlign：1）构建层次化临床认知数据集，通过监督微调将专家从解剖定位、形态评估到微血管分析的层次化诊断逻辑内化到模型中；2）提出反事实驱动的强化学习策略，通过病灶掩蔽生成反事实正常样本，并以临床认知为中心进行奖励优化，强制模型基于因果病灶特征进行诊断。

Result: 在多个基准测试中实现了最先进的性能，显著提升了复杂临床场景下的诊断准确性。

Conclusion: CogAlign框架通过临床认知对齐和因果特征学习，有效解决了MLLMs在胃肠内镜应用中的关键限制，为医学图像分析提供了新思路。
...
TL;DR: EHETM利用事件相机的高时间分辨率特性，通过分析湍流事件中的极性交替模式和动态物体形成的"事件管"来高效解决湍流消除问题，在减少数据开销和系统延迟的同时实现高质量恢复。


<details>
  <summary>Details</summary>
Motivation: 传统多帧湍流消除方法面临精度与效率的权衡：更多帧数提高恢复质量但增加系统延迟和数据开销。事件相机具有微秒级时间分辨率和对动态变化的高效感知能力，为突破这一瓶颈提供了机会。

Method: 基于两个关键发现：1)湍流诱导事件表现出与图像梯度相关的极性交替模式，为场景恢复提供结构线索；2)动态物体形成时空连贯的"事件管"，与湍流事件的随机模式形成对比。设计了两个互补模块：极性加权梯度模块用于场景细化，事件管约束模块用于运动解耦。

Result: EHETM在包含动态物体的场景中优于现有方法，同时将数据开销减少约77.3%，系统延迟降低约89.5%。构建了两个真实世界的事件帧湍流数据集，涵盖大气和热湍流情况。

Conclusion: 事件相机为湍流消除提供了新思路，EHETM通过利用事件数据的独特特性实现了高质量、高效率的湍流恢复，特别适用于包含动态物体的复杂场景。
...
TL;DR: 本文系统研究了3D高斯泼溅中初始化点云密度与致密化策略的关系，发现当前致密化方法无法充分利用密集初始化，稀疏SfM初始化往往已足够。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅通常使用稀疏SfM点云初始化并依赖致密化阶段，但缺乏对初始化密度与致密化关系的系统研究，需要评估不同初始化类型对最终重建质量的影响。

Method: 提出新基准测试，系统研究不同类型初始化（密集激光扫描、密集多视角立体点云、密集单目深度估计、稀疏SfM点云）与不同致密化方案的组合效果。

Result: 当前致密化方法无法充分利用密集初始化的优势，通常无法显著超越基于稀疏SfM的初始化效果，表明现有致密化策略存在局限性。

Conclusion: 需要开发能更好利用密集初始化的致密化方法，提出的基准将为未来研究提供评估标准，推动3D高斯泼溅技术的进一步发展。
...
TL;DR: 提出跨模态模糊对齐网络，通过模糊逻辑量化token级可靠性实现细粒度对齐，并引入地面视图作为桥梁代理，缓解无人机图像与文本描述之间的语义差距，用于文本-无人机行人检索。


<details>
  <summary>Details</summary>
Motivation: 无人机捕获的图像由于视角和飞行高度变化大，视觉信息退化严重，与文本描述的语义对齐非常困难。现有地面视图文本-图像行人检索方法难以应对无人机图像的独特挑战。

Method: 1) 模糊令牌对齐模块：使用模糊隶属函数动态建模token级关联强度，抑制不可观察或噪声token的影响；2) 上下文感知动态对齐模块：引入地面视图作为桥梁代理，自适应结合直接对齐和代理辅助对齐；3) 构建AERI-PEDES大规模基准数据集，采用思维链分解文本生成过程。

Result: 在AERI-PEDES和TBAPR数据集上的实验证明了方法的优越性，能够有效缓解无人机图像与文本描述之间的语义差距，提升检索性能。

Conclusion: 提出的跨模态模糊对齐网络通过模糊逻辑量化token级可靠性和引入地面视图代理，显著提升了文本-无人机行人检索的性能，为解决无人机图像与文本描述之间的语义对齐挑战提供了有效方案。
...
TL;DR: Premier是一个新颖的偏好调制框架，通过可学习的用户嵌入表示用户偏好，结合偏好适配器和分散损失实现个性化图像生成，在历史数据有限时也能有效泛化。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成方法难以准确捕捉用户细微偏好，多模态大语言模型推断的提示或潜在代码通常无法忠实反映用户偏好，导致个性化效果不佳。

Method: 1) 将每个用户偏好表示为可学习的嵌入向量；2) 引入偏好适配器融合用户嵌入与文本提示；3) 使用融合后的偏好嵌入调制生成过程；4) 加入分散损失增强用户嵌入间的区分度；5) 新用户表示为训练期间学习到的现有偏好嵌入的线性组合。

Result: 在相同历史长度下，Premier优于先前方法，在偏好对齐、文本一致性、ViPer代理指标和专家评估方面表现更优。

Conclusion: Premier通过可学习的偏好嵌入和调制框架，实现了更准确、细粒度的个性化图像生成，在用户数据稀缺时也能有效泛化。
...
TL;DR: 提出弱监督多模态分割框架，通过置信度门控深度感知交叉注意力融合二维井壁图像与一维测井数据，实现无标注的井壁结构分割。


<details>
  <summary>Details</summary>
Motivation: 声学井壁图像提供高分辨率结构信息，但大规模解释困难，因为密集专家标注稀缺且地下信息本质上是多模态的。需要开发结合二维图像纹理与深度对齐一维测井数据的弱监督方法。

Method: 提出弱监督多模态分割框架，通过阈值引导伪标签学习进行细化。保留经典阈值和聚类工作流的无标注特性，同时加入去噪、置信感知伪监督和物理结构化融合。引入置信度门控深度感知交叉注意力（CG-DCA）融合策略。

Result: 阈值引导学习细化相比原始阈值、去噪阈值和潜在聚类基线提供最稳健的改进。多模态性能强烈依赖于融合策略：直接拼接增益有限，而深度感知交叉注意力、门控融合和置信感知调制显著提高与弱监督参考的一致性。CG-DCA模型始终优于基于阈值、仅图像和早期多模态基线。

Conclusion: 建立了一个实用、可扩展的无标注分割框架，表明当辅助测井数据被选择性和深度感知地整合时，多模态改进最大化。置信感知融合和结构化局部深度交互是关键优势来源。
...
<details>
  <summary>Details</summary>
Motivation: 现有多目标跟踪算法在低质量视频中性能显著下降，主要原因是无法有效处理低质量图像中的信息损失问题。现实场景中视频质量往往不佳，需要专门针对低质量视频的跟踪解决方案。

Method: 提出视觉语义蒸馏引导的多目标跟踪框架(VSD-MOT)：1) 引入CLIP图像编码器提取全局视觉语义信息；2) 采用师生学习框架进行知识蒸馏，避免直接集成影响效率；3) 设计双约束语义蒸馏方法(DCSD)让学生模型学习适合跟踪任务的语义提取能力；4) 提出动态语义权重调节模块(DSWR)，根据实时帧质量评估自适应分配融合权重。

Result: 大量实验证明该方法在现实世界低质量视频场景中具有有效性和优越性，同时在常规场景中也能保持良好的性能。

Conclusion: 通过视觉语义蒸馏方法有效解决了低质量视频中的多目标跟踪问题，提出的VSD-MOT框架能够在信息损失严重的低质量视频中保持稳定的跟踪性能，同时不牺牲常规场景的表现。
...
TL;DR: SATTC是一种无标签的测试时校准方法，通过几何专家和结构专家融合，改善跨被试EEG-图像检索中的相似性几何和排名稳定性。


<details>
  <summary>Details</summary>
Motivation: 跨被试EEG到图像检索面临被试偏移和嵌入空间中心性问题的挑战，这会扭曲相似性几何并破坏top-k排名稳定性，使得小k候选列表不可靠。

Method: SATTC直接在冻结的EEG和图像编码器的相似性矩阵上操作，包含几何专家（被试自适应白化和自适应CSLS）和结构专家（互最近邻、双向top-k排名和类别流行度），通过专家乘积规则融合。

Result: 在THINGS-EEG数据集上，SATTC在严格的留一被试协议下，显著提升了Top-1和Top-5准确率，减少了中心性和类别不平衡，产生了更可靠的小k候选列表。

Conclusion: SATTC作为一种编码器无关、无标签的测试时校准层，可有效改善跨被试神经解码的检索性能，具有很好的泛化能力。
...
TL;DR: 提出SADG框架，基于Mamba的上下文学习，通过结构感知序列化和层次化域感知建模，实现跨域多任务点云表示学习，并在测试时使用谱图对齐进行特征迁移。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer和Mamba架构在单任务/单域设置中表现良好，但在多任务域泛化(DG)中性能下降。Transformer有二次注意力成本且缺乏显式结构顺序，Mamba依赖坐标驱动的序列化对视角变化和缺失区域敏感，导致结构漂移和不稳定的序列建模。

Method: 1. 结构感知序列化(SAS)：使用基于质心的拓扑和测地曲率连续性生成变换不变的序列；2. 层次化域感知建模(HDM)：通过整合域内结构和融合域间关系来稳定跨域推理；3. 测试时谱图对齐(SGA)：在谱域中将目标特征向源原型转移，无需更新模型参数；4. 引入MP3DObject数据集用于多任务DG评估。

Result: 综合实验表明，该方法提高了结构保真度，在重建、去噪和配准等多个任务上持续优于最先进的方法。

Conclusion: 提出的SADG框架通过结构感知序列化、层次化域感知建模和测试时谱图对齐，有效解决了点云多任务域泛化中的结构漂移问题，在保持结构层次的同时实现了跨域鲁棒性。
...
TL;DR: CTCal通过利用小时间步中可靠的文本-图像对齐（交叉注意力图）来校准大时间步中的表示学习，为扩散模型提供显式监督，改善文本-图像对齐。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的文本到图像生成模型在实现精确的文本提示与生成图像对齐方面仍面临挑战，主要原因是传统扩散损失仅提供隐式监督，难以建模细粒度的文本-图像对应关系。

Method: 提出跨时间步自校准（CTCal）方法，利用小时间步（噪声较少）中形成的可靠文本-图像对齐（交叉注意力图）来校准大时间步（噪声较多）中的表示学习，提供显式监督。同时提出时间步感知自适应加权，实现CTCal与扩散损失的和谐集成。

Result: 在T2I-Compbench++和GenEval基准测试上的广泛实验证明了CTCal的有效性和泛化能力。该方法模型无关，可无缝集成到现有文本到图像扩散模型中，包括基于扩散的方法（如SD 2.1）和基于流的方法（如SD 3）。

Conclusion: CTCal通过利用小时间步中的可靠对齐信息来校准大时间步的学习，为扩散模型提供了显式监督，显著改善了文本-图像对齐效果，且具有良好的通用性和可集成性。
...
<details>
  <summary>Details</summary>
Motivation: 解决手术中纱布遗留在患者体内（Gossypiboma）的问题，避免严重并发症和医疗纠纷。传统人工计数方法耗时且分散护理资源，需要更高效的预防方案。

Method: 使用YOLOv5深度学习模型，通过实时视频监控识别"进"和"出"托盘中的纱布。训练集包含11,000张手术室图像，采用数据增强技术，集成人检测和纱布检测功能，支持手动计数调整。

Result: 系统准确率提高，帧率从8 FPS提升到15 FPS，能够实时监控纱布使用情况，确保手术结束时没有纱布遗留在患者体内。

Conclusion: AI辅助的纱布计数系统能有效预防Gossypiboma，提高手术安全性，减少医疗资源浪费，具有实际临床应用价值。
...
TL;DR: DiT-BlockSkip：一种内存高效的扩散Transformer微调框架，通过动态补丁采样和块跳过机制，显著降低内存使用，实现资源受限设备上的个性化图像生成


<details>
  <summary>Details</summary>
Motivation: 扩散Transformer（DiTs）在文本到图像生成方面表现出色，但微调需要大量计算复杂度和内存，限制了在资源受限设备上的实际部署。需要开发内存高效的微调方法以实现设备端应用。

Method: 提出DiT-BlockSkip框架：1）时间步感知的动态补丁采样，根据扩散时间步调整补丁大小并调整到固定低分辨率；2）块跳过机制，选择性微调关键transformer块，为跳过的块预计算残差特征；3）基于交叉注意力掩码的块选择策略识别个性化关键块。

Result: 方法在保持竞争力的个性化性能的同时，显著减少了内存使用，为大规模扩散Transformer在智能手机、物联网等设备端部署提供了可行性。

Conclusion: DiT-BlockSkip通过创新的动态补丁采样和块跳过机制，有效解决了扩散Transformer微调的内存瓶颈问题，为实现资源受限设备上的高质量个性化图像生成提供了实用解决方案。
...
TL;DR: PiLoT是一个统一的无人机自定位与目标地理定位框架，通过将实时视频流与地理参考3D地图直接配准，在GNSS拒止环境中实现鲁棒、准确、实时的定位，无需GNSS或激光测距仪等昂贵硬件。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖GNSS和视觉惯性里程计(VIO)进行自定位，以及激光测距仪等主动传感器进行目标定位，这些方法在GNSS拒止环境中容易失效，且硬件成本高、复杂度大。

Method: 1) 双线程引擎：将地图渲染与核心定位线程解耦，确保低延迟和无漂移精度；2) 大规模合成数据集：提供精确几何标注，用于训练轻量级网络实现从仿真到真实数据的零样本泛化；3) 联合神经引导随机梯度优化器(JNGO)：在剧烈运动下实现鲁棒收敛。

Result: 在公共和新收集的基准测试中，PiLoT优于现有最先进方法，同时在NVIDIA Jetson Orin平台上运行速度超过25 FPS。

Conclusion: PiLoT通过直接视频流与3D地图配准的新范式，解决了传统方法在GNSS拒止环境中的局限性，实现了低成本、高鲁棒性的无人机定位系统。
...
<details>
  <summary>Details</summary>
Motivation: 基于交叉熵损失的边缘检测模型通常产生较厚的边缘预测，与人类标注的单像素边缘不符。现有方法多专注于设计特殊损失函数或修改网络架构，但本文证明仅通过精心设计的训练和推理策略就能达到人类级别的边缘质量

Method: 提出掩码边缘预测模型(MEMO)：1) 构建大规模合成边缘数据集进行预训练；2) 在下游数据集上仅需1.2%额外参数的轻量模块进行微调；3) 训练时学习在不同掩码比例下预测边缘；4) 推理时利用厚边缘预测中的置信度梯度（中心高、边界低），提出渐进式预测策略，按置信度顺序逐步确定边缘预测

Result: 方法实现了视觉上吸引人、无需后处理、类似人类的边缘图，在清晰度感知评估中优于先前方法

Conclusion: 通过精心设计的训练和推理策略，仅使用交叉熵损失就能生成准确且清晰的边缘，无需专门设计损失函数或修改网络架构
...
TL;DR: ME-IQA：一种即插即用的测试时记忆增强重排序框架，通过构建记忆库、将VLM重构为概率比较器，并融合序数证据来解决推理诱导视觉语言模型在图像质量评估中的离散崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 推理诱导视觉语言模型（VLMs）通过文本推理推进了图像质量评估（IQA），但其标量评分通常缺乏敏感性，并会坍缩到少数几个值（即离散崩溃问题）。

Method: 1. 构建记忆库并使用推理摘要检索语义和感知对齐的邻居；2. 将VLM重构为概率比较器，获得成对偏好概率，并在Thurstone Case V模型下将序数证据与初始评分融合；3. 执行门控反射并巩固记忆以改进未来决策。

Result: 在多个IQA基准测试中，ME-IQA相比强大的推理诱导VLM基线、现有非推理IQA方法以及测试时缩放替代方案，均显示出持续的性能提升。

Conclusion: ME-IQA框架能够产生更密集、对失真更敏感的预测，有效缓解了离散崩溃问题，为推理诱导VLMs在图像质量评估中的应用提供了有效的增强方案。
...
TL;DR: Co-VLN框架让多个机器人在共享环境中导航时，通过交换感知记忆来扩展各自的观察范围，从而提升视觉语言导航性能。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言导航系统受限于部分可观测性，每个智能体只能积累自己访问过的位置信息。随着多个机器人在共享环境中共存，研究它们能否通过共享观察来相互受益。

Method: 提出Co-VLN框架，当独立导航的智能体识别到共同经过的位置时，交换结构化的感知记忆，在不增加探索成本的情况下扩展每个智能体的感知范围。该框架是模型无关的，在R2R基准上验证了两种代表性范式（基于学习的DUET和零样本的MapGPT）。

Result: 实验结果表明，视觉共享模型在两种范式上都带来了显著的性能提升，为未来协作具身导航研究奠定了坚实基础。

Conclusion: 通过系统研究智能体间观察共享的底层动态，证明了在共享环境中导航的智能体可以通过交换感知记忆相互受益，这为多机器人协作导航开辟了新的研究方向。
...
TL;DR: Clifford-M：一种轻量级多标签眼底诊断模型，通过稀疏几何交互替代频率分解，在双分辨率架构中实现高效跨尺度融合，参数量仅0.85M但性能优于更大模型。


<details>
  <summary>Details</summary>
Motivation: 多标签眼底诊断需要同时捕捉细粒度病变和大尺度视网膜结构。现有多尺度医学视觉模型通过显式频率分解处理此问题，但作者发现这些启发式方法在眼底诊断中效果有限：Octave卷积增加35%参数和2.23倍计算量但未提升准确率，固定小波变换性能更差。

Method: 提出Clifford-M轻量级骨干网络，用稀疏几何交互替代前馈扩展和频率分割模块。模型基于Clifford式滚动乘积，以线性复杂度联合捕捉对齐和结构变化，在紧凑双分辨率架构中实现高效跨尺度融合和自我精炼。

Result: 无预训练情况下，在ODIR-5K上达到平均AUC-ROC 0.8142和平均macro-F1 0.5481（仅0.85M参数），显著优于相同训练协议下更大的中尺度CNN基线。在RFMiD上无微调评估获得0.7425±0.0198 macro AUC和0.7610±0.0344 micro AUC，显示对跨数据集偏移的合理鲁棒性。
...
TL;DR: MLLMs在视觉表示上存在退化问题，作者提出预测正则化方法来解决这一问题，从而提升多模态理解性能。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在视觉语言任务上表现出色，但其语言驱动的训练对内部视觉基础能力的影响尚不清楚。作者发现MLLMs存在视觉表示退化问题，即中间层的视觉表示相比初始视觉特征在全局功能和补丁结构上都出现退化，这影响了模型的全面多模态理解能力。

Method: 提出预测正则化方法，强制退化的中间特征预测初始视觉特征，从而保持MLLM内部表示的内在视觉属性。这种方法旨在平衡跨模态推理和核心视觉能力。

Result: 大量实验证实，缓解视觉退化问题能有效提升视觉语言性能，强调了在MLLMs内部培养稳健视觉表示对于全面多模态理解的重要性。

Conclusion: 一个稳健的MLLM需要同时具备强大的跨模态推理能力和核心视觉能力。视觉退化是由单一文本生成目标驱动的视觉牺牲现象，通过预测正则化方法可以保持模型的视觉保真度，从而提升整体性能。
...
TL;DR: EDC：面向效率的差异条件光学-SAR语义分割框架，通过三流编码器、差异条件混合融合和云去除分支，在减少参数和加速推理的同时提升云遮挡下的分割精度。


<details>
  <summary>Details</summary>
Motivation: 云遮挡严重降低光学遥感图像的语义完整性，而结合SAR提供互补观测时，现有方法存在全局建模效率低、跨模态融合不可靠的问题。密集全局注意力会传播云诱导噪声，而提高鲁棒性通常需要增大模型容量，导致计算开销增加，形成效率-可靠性权衡困境。

Method: 提出EDC框架：1）使用带载波令牌的三流编码器实现紧凑的全局上下文建模，降低复杂度；2）引入差异条件混合融合机制，在全局聚合时选择性抑制不可靠区域；3）采用教师引导蒸馏的辅助云去除分支增强遮挡下的语义一致性。

Result: 在M3M-CR和WHU-OPT-SAR数据集上，mIoU分别提升0.56%和0.88%，同时参数减少46.7%，推理加速1.98倍，实现了精度和效率的双重提升。
...
TL;DR: 提出PlanaReLoc，首个使用平面基元和3D平面地图进行轻量级6-DoF相机重定位的方法，无需纹理地图、位姿先验或逐场景训练


<details>
  <summary>Details</summary>
Motivation: 传统基于结构的重定位方法依赖点对应关系，而平面基元作为投影几何基本实体，能提供区域化表示，包含结构和语义信息，更适合结构化环境中的跨模态对应

Method: 提出平面中心范式PlanaReLoc：使用深度匹配器在统一嵌入空间中将查询图像与地图中的平面基元关联，然后在鲁棒框架下求解和优化6-DoF位姿

Result: 在ScanNet和12Scenes数据集上进行了数百个场景的综合实验，证明平面基元在促进可靠的跨模态结构对应和实现有效相机重定位方面的优越性

Conclusion: 平面基元为结构化环境中的相机重定位提供了轻量级解决方案，无需真实纹理/彩色地图、位姿先验或逐场景训练，展现了平面表示的潜力
...
TL;DR: EruDiff通过知识分布匹配和负向强化学习，解决扩散模型处理隐含提示时的知识错位问题，显著提升科学和世界知识基准上的生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型在处理需要深层世界知识的隐含提示时存在严重不足，导致生成反事实图像。问题的根源在于知识结构的错位，隐含提示的组织混乱无序。

Method: 提出EruDiff框架：1) 扩散知识分布匹配(DK-DM)，将隐含提示的知识分布与明确锚点的知识分布对齐；2) 仅负向强化学习(NO-RL)策略，纠正明确提示渲染中的固有偏差。

Result: 在FLUX和Qwen-Image等主流扩散模型上，方法显著提升了科学知识基准(Science-T2I)和世界知识基准(WISE)的性能，证明了有效性和泛化能力。

Conclusion: EruDiff通过重构扩散模型中的知识结构，有效解决了隐含提示处理的知识错位问题，为提升扩散模型的深层知识理解能力提供了新途径。
...
TL;DR: MERIT：首个统一的多域RAW图像翻译框架，通过单个模型实现任意相机域间的RAW图像转换，解决了不同相机传感器RAW图像间的域偏移问题。


<details>
  <summary>Details</summary>
Motivation: 不同相机传感器捕获的RAW图像由于光谱响应、噪声特性和色调行为的差异存在显著的域偏移，这限制了它们在计算机视觉任务中的直接使用。现有方法需要为每个源-目标对训练专门的翻译器，无法扩展到涉及多种商业相机的真实场景。

Method: 提出MERIT统一框架，包含：1）传感器感知的噪声建模损失，显式对齐生成图像与目标域的信号相关噪声统计特性；2）条件多尺度大核注意力模块，增强生成器的上下文和传感器感知特征建模能力；3）构建MDRAW数据集，包含五种不同相机传感器的配对和非配对RAW图像。

Result: MERIT在质量上比先前模型提升5.56 dB，在可扩展性上减少80%的训练迭代次数，显著优于现有方法。

Conclusion: MERIT是首个统一的多域RAW图像翻译框架，通过传感器感知的噪声建模和条件注意力机制，实现了高质量、可扩展的跨相机域RAW图像转换，为实际应用提供了有效解决方案。
...
TL;DR: Dodgersort是一种高效的人工标注框架，通过CLIP分层预排序、神经排序头、概率集成、不确定性分解和信息理论对选择，在减少人工比较次数的同时提高排序可靠性。


<details>
  <summary>Details</summary>
Motivation: 成对比较标注比传统分类标注具有更高的评分者间信度，但穷举比较需要二次成本。需要一种方法在减少人工比较的同时提高排序的可靠性。

Method: 提出Dodgersort框架，包含：1) CLIP分层预排序；2) 神经排序头；3) 概率集成（Elo、BTL、GP）；4) 认知-随机不确定性分解；5) 信息理论对选择。通过跨领域消融实验验证神经适应和集成不确定性的重要性。

Result: 在医学影像、历史年代和美学等视觉排序任务中，Dodgersort实现了11-16%的标注减少，同时提高了评分者间信度。在FG-NET数据集上，相比基线方法，每个比较提取5-20倍的排序信息，实现了帕累托最优的准确率-效率权衡。

Conclusion: Dodgersort通过智能对选择和概率建模，显著提高了成对比较标注的效率，在减少人工标注成本的同时提升了排序质量，为高效标注提供了有效解决方案。
...
TL;DR: GOLDMARK是一个用于计算病理学的标准化基准测试框架，基于TCGA队列构建，包含33个肿瘤-生物标志物任务，旨在解决该领域缺乏标准化数据格式和可重复评估的问题。


<details>
  <summary>Details</summary>
Motivation: 计算病理学目前缺乏标准化的中间数据格式、来源追踪、检查点约定和可重复的评估指标，这些是临床级部署所必需的。虽然基于病理基础模型的多实例学习方法提高了预测性能，但标准化基准测试框架的缺失阻碍了方法的直接比较和可重复性。

Method: GOLDMARK框架基于精心策划的TCGA队列构建，包含临床可操作的OncoKB 1-3级生物标志物标签。该框架发布结构化中间表示，包括瓦片坐标图、来自规范病理基础模型的每张切片特征嵌入、质量控制元数据、预定义的患者级分割、训练的切片级模型和评估输出。模型在TCGA上训练，并在独立的MSKCC队列上进行互惠测试。

Result: 在33个肿瘤-生物标志物任务中，平均AUROC为0.689（TCGA）和0.630（MSKCC）。限制在8个最高性能任务时，平均AUROC分别为0.831和0.801。这些任务对应已建立的形态学-基因组关联（如LGG IDH1、COAD MSI/BRAF等），并显示出最稳定的跨站点性能。不同编码器之间的差异相对于任务特异性变异性较小。
...
<details>
  <summary>Details</summary>
Motivation: 传统手部视频缺乏接触力、运动信号等物理信息，且存在频繁遮挡问题，需要一种能保留物理交互动态的裸手视频生成方法

Method: 提出Glove2Hand框架：1) 使用新型3D高斯手模型确保时间渲染一致性；2) 基于扩散模型的手部修复器处理复杂手-物交互和非刚性变形；3) 创建HandSense数据集，包含同步触觉和IMU信号

Result: HandSense数据集显著提升了基于视频的接触估计和严重遮挡下的手部跟踪等下游裸手应用性能

Conclusion: Glove2Hand框架成功解决了传统手部视频的物理信息缺失和遮挡问题，为计算机视觉、机器人和AR/VR领域的手-物交互理解提供了有效解决方案
...
TL;DR: 提出轻量级集成方法用于白细胞分类，特别关注粒细胞、单核细胞和淋巴细胞生成过程，通过数据集扩展缓解类别不平衡，使用SwinV2-Tiny、DinoBloom-Small和ConvNeXT-V2-Tiny三种预训练模型的集成在挑战性数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 自动化白细胞分类可替代耗时耗力的病理学家人工检查，但罕见细胞类型分类面临染色、扫描差异和患者异质性等挑战，需要设计鲁棒算法。

Method: 采用轻量级集成方法，使用SwinV2-Tiny、DinoBloom-Small和ConvNeXT-V2-Tiny三种预训练模型，通过数据集扩展缓解类别不平衡，在分层3折交叉验证框架中训练每个架构的3个实例，对输入图像通过所有9个模型前向传播并通过logit平均进行聚合。

Result: 在具有挑战性的数据集上取得了优异性能，但模型在粒细胞生成中相似形态的髓细胞和淋巴细胞生成中的淋巴细胞之间存在混淆。

Conclusion: 提出的轻量级集成方法在白细胞分类任务中表现优秀，特别关注造血生物学过程，但相似细胞类型的区分仍是挑战，为白血病诊断的自动化细胞分类提供了有效解决方案。
...
TL;DR: FRoG是一个高效鲁棒的动态场景重建框架，通过时间嵌入融合加速渲染，使用深度误差引导采样增强对稀疏初始化的鲁棒性，并调制不透明度变化改善暗场景的颜色保真度。


<details>
  <summary>Details</summary>
Motivation: 现有的基于变形场的3D高斯泼溅动态场景重建方法存在渲染速度慢、对初始点云依赖性强、在暗场景中容易陷入局部最优等问题，需要更高效鲁棒的解决方案。

Method: 1) 集成每个高斯的嵌入与从粗到细的时间嵌入策略，通过早期融合时间嵌入加速渲染；2) 引入深度和误差引导的采样策略，在低偏差初始位置添加新3D高斯，减轻变形场的优化负担；3) 通过调制不透明度变化缓解暗场景中的局部最优问题。

Result: 综合实验验证表明，该方法在保持最先进视觉质量的同时实现了加速渲染速度，在动态场景重建中表现出色。

Conclusion: FRoG框架通过创新的时间嵌入融合、采样策略和不透明度调制，有效解决了现有动态3D高斯泼溅方法的局限性，实现了高效鲁棒的高质量动态场景重建。
...
<details>
  <summary>Details</summary>
Motivation: 迁移学习中，预训练权重可能饱和导致梯度消失（神经可塑性丧失），阻碍模型有效适应下游任务，尤其是在非典型数据集上。这个问题在持续学习中已被广泛研究，但在迁移学习领域相对较少探索。

Method: 提出一种有针对性的权重重新初始化策略，在微调前恢复神经可塑性。该方法计算开销极小，且与常见的迁移学习流程兼容。

Result: 实验表明，卷积神经网络（CNN）和视觉变换器（ViT）都从此方法中受益，在多个图像分类基准测试中获得了更高的测试准确率和更快的收敛速度。

Conclusion: 通过有针对性的权重重新初始化恢复神经可塑性，可以有效提升迁移学习性能，特别是当下游数据集具有非典型特征时。
...
TL;DR: TAFG-MAN是一个用于低剂量CT图像去噪的潜在扩散框架，通过时间步自适应频率门控设计，在紧凑潜在空间中实现高效高质量的去噪，平衡噪声抑制与细节保留。
...
Result: 实验表明TAFG-MAN在质量-效率权衡方面优于代表性基线方法。与没有TAFG的基础变体相比，进一步改善了细节保留和感知质量，同时保持基本相同的推理成本，消融实验证实了所提条件机制的有效性。

Conclusion: TAFG-MAN通过时间步自适应频率门控机制，在低剂量CT图像去噪中实现了更好的噪声抑制与细节保留平衡，为高效高质量的医学图像恢复提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频多模态理解方法主要关注全局理解，用户交互有限。需要让用户能够通过特定提示（如边界框）来精确控制视频内容的解释，生成符合用户意图的掩码和字幕。

Method: 提出SG-FSCFormer框架：1）提示引导的时间图变换器（Prompt-guided Temporal Graph Former），通过自适应提示适配器捕捉用户意图；2）细粒度掩码-语言解码器（Fine-grained Mask-linguistic Decoder），使用多实体对比损失协同预测高质量的字幕-掩码对，并提供掩码与对应字幕标记的细粒度对齐。

Result: 在两个基准数据集上的实验表明，SG-FSCFormer取得了显著性能，能有效捕捉用户意图并生成符合用户要求的精确多模态输出。

Conclusion: 提出的SegCaptioning任务和SG-FSCFormer框架成功实现了用户可控的视频多模态解释，通过用户提示生成精确的掩码和字幕，增强了用户对视频内容的理解。
...
<details>
  <summary>Details</summary>
Motivation: 当前神经元形态分析方法仅单独分析拓扑或图形结构，无法充分利用形态编码的电路功能、发育和疾病信息，需要一种统一这两种互补视角的方法。

Method: 采用多模态架构：视觉分支处理三通道持久性图像（未加权、持久性加权和半径加权拓扑密度），使用DINOv2-ViT-S；并行使用TreeLSTM编码器从骨架图捕获几何和径向属性。两者投影到共享嵌入空间，通过对称InfoNCE损失训练，并辅以保持拓扑语义的持久性空间变换。
...
TL;DR: 研究发现医学视觉语言模型的一致性评估存在根本缺陷，模型可通过依赖文本模式而非图像实现完美一致性，提出了结合一致性和图像依赖性的四象限安全分类法。


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉语言模型部署中，一致性（语义等价提示产生相同预测）被广泛用作可靠性的代理指标，但作者发现这一代理指标存在根本缺陷，因为模型可以通过依赖文本模式而非输入图像来实现完美一致性。

Method: 提出了四象限每样本安全分类法，联合评估一致性（在改写提示下的稳定预测）和图像依赖性（移除图像时预测的变化）。样本被分类为理想型、脆弱型、危险型和最差型。评估了五种医学VLM配置在两个胸部X光数据集（MIMIC-CXR、PadChest）上的表现。

Result: LoRA微调显著降低了翻转率，但将大多数样本转移到危险象限：LLaVA-Rad Base在PadChest上实现1.5%翻转率，而98.5%的样本属于危险型。危险样本表现出高准确率（高达99.6%）和低熵，使其对基于置信度的筛选不可见。观察到翻转率与危险分数之间存在负相关（r=-0.89）。

Conclusion: 一致性检查必须与文本基线配对使用，仅依赖一致性作为可靠性代理存在根本缺陷。部署评估应始终包含文本基线前向传递，以暴露虚假可靠性陷阱。
...
TL;DR: SkinCLIP-VL是一个资源高效的视觉语言模型框架，用于皮肤癌诊断，通过冻结CLIP编码器和轻量级Qwen2.5-VL结合，在参数减少43%的情况下，在ISIC和Derm7pt基准上超越13B参数基线4.3-6.2%的准确率。


<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在皮肤病学部署中的三重困境：高计算成本、极端数据稀缺性和深度学习黑盒性质，以建立可信赖的皮肤癌诊断系统。

Method: 采用"冻结感知、自适应推理"范式，将冻结的CLIP编码器与轻量级量化Qwen2.5-VL通过低秩适应（LoRA）集成。提出一致性感知焦点对齐（CFA）损失，在长尾分布下严格对齐视觉区域与临床语义，结合焦点重加权、语义对齐和校准。

Result: 在ISIC和Derm7pt基准测试中，SkinCLIP-VL以43%更少的参数超越13B参数基线4.3-6.2%的准确率。盲法专家评估和分布外测试证实，视觉基础推理显著增强了临床信任度，优于传统显著性图。

Conclusion: SkinCLIP-VL成功解决了皮肤病学中视觉语言模型部署的三大挑战，通过资源高效的设计和一致性感知对齐，实现了高性能且可信赖的皮肤癌诊断，为临床实践提供了可解释的视觉推理。
...
TL;DR: LPNSR提出了一种基于先验增强的高效扩散框架，通过优化中间噪声和初始上采样，在仅需4步推理的情况下实现高质量图像超分辨率。


<details>
  <summary>Details</summary>
Motivation: 现有残差移位扩散框架虽然实现了高效的4步推理，但在紧凑采样轨迹下存在严重性能下降。主要问题包括：中间步骤无约束随机高斯噪声的次优性导致误差累积和LR先验指导不足，以及朴素双三次上采样导致的初始化偏差。

Method: 1) 数学推导残差移位扩散范式的最优中间噪声闭式解析解；2) 设计LR引导的多输入感知噪声预测器替代随机高斯噪声，将LR结构先验嵌入反向过程；3) 使用高质量预上采样网络优化扩散起点，缓解初始偏差；4) 在紧凑4步轨迹下进行端到端优化。

Result: 在合成和真实世界数据集上的广泛实验表明，LPNSR在感知性能上达到最先进水平，且不依赖任何大规模文本到图像先验。

Conclusion: LPNSR通过优化中间噪声和初始上采样，有效解决了扩散超分辨率中推理效率与重建质量之间的权衡问题，在保持高效4步推理的同时显著提升重建质量。
...
Result: 在已见和未见环境中均优于最先进的无人机视觉语言导航基线，在未见Full分割上减少NE 4.03米，提高SR 1.27%。轨迹分析显示产生更好路径对齐、更平滑稳定的运动轨迹。

Conclusion: SpatialFly通过几何引导的空间表示有效解决了无人机视觉语言导航中的2D-3D表示不匹配问题，在复杂3D环境中实现了更好的导航性能。
...
TL;DR: 该论文提出了一个利用LLM生成临床合理对抗性提示的评估框架，用于测试医学视觉语言模型在超声影像分析中的对抗鲁棒性，揭示了现有模型的现实脆弱性。


<details>
  <summary>Details</summary>
Motivation: 超声影像分析高度依赖操作者经验，AI辅助方法有需求。虽然视觉语言模型在医学影像分析中表现出色，但其对抗鲁棒性存在严重问题。由于Med-VLMs通过自然语言指令操作，提示表述成为现实的攻击点，小的语言变化（如拼写错误、简写、模糊请求）就能显著改变模型输出，这对临床安全构成威胁。

Method: 提出一个可扩展的对抗性评估框架，利用大型语言模型生成临床合理的对抗性提示变体。通过"人性化"重写和最小编辑来模拟常规临床沟通，在超声多选题基准上系统评估SOTA Med-VLMs对这些攻击的脆弱性。研究攻击者LLM能力对攻击成功率的影响，分析攻击成功与模型置信度的关系，并识别跨模型的一致失败模式。

Result: 研究结果显示Med-VLMs存在现实的鲁棒性差距。系统评估揭示了模型对临床合理对抗性提示的脆弱性，攻击者LLM能力影响攻击成功率，攻击成功与模型置信度存在相关性，并识别了跨模型的一致失败模式。这些发现表明现有模型在临床转化前需要解决安全漏洞。

Conclusion: 该研究强调了Med-VLMs在对抗鲁棒性方面的现实脆弱性，这些漏洞必须解决才能实现安全的临床转化。提出的评估框架为系统测试医学AI系统的安全性提供了可扩展的方法，代码将在评审后公开发布。
...
TL;DR: 提出一个用于驾驶员监控的两阶段时序动作定位框架，结合VideoMAE特征提取和增强自掩码注意力检测器，通过空间金字塔池化模块提升多尺度时序特征捕捉能力。


<details>
  <summary>Details</summary>
Motivation: 从车内视频流中识别危险驾驶行为对提升道路安全至关重要，但现有时序动作定位技术难以平衡准确性与计算效率，特别是在运输安全检查点或车队管理等周期性检查场景中。

Method: 采用两阶段流程：1) VideoMAE-based特征提取（ViT-Giant或ViT-base变体）；2) 增强自掩码注意力检测器，结合空间金字塔池化快速模块捕捉多尺度时序特征。

Result: ViT-Giant骨干网在特征提取阶段达到88.09% Top-1准确率，ViT-base变体以显著更低的计算成本（101.85 vs 1584.06 GFLOPs/segment）达到82.55%准确率。在定位任务中，SPPF模块持续提升性能，ViT-Giant+SPPF达到92.67% mAP峰值。

Conclusion: 该框架在驾驶员监控场景中实现了准确性与计算效率的良好平衡，为运输安全检查点和车队管理系统提供了实用的解决方案，SPPF模块对多尺度时序特征捕捉具有重要价值。
...
TL;DR: SGAD-SLAM提出了一种新的3D高斯表示方法，通过像素对齐的高斯并允许沿光线调整位置，在保持系统可扩展性的同时提高渲染质量，同时使用高斯深度分布加速相机跟踪。


<details>
  <summary>Details</summary>
Motivation: 当前3D高斯溅射方法在RGBD SLAM中使用的3D高斯要么过于灵活，要么运动受限，导致收敛慢或渲染质量有限。需要一种既能保持系统可扩展性又能提高渲染质量的高斯表示方法。

Method: 采用像素对齐的高斯，但允许每个高斯沿其光线调整位置以最大化渲染质量，即使简化高斯以提高系统可扩展性。为加速跟踪，将每个像素周围的深度分布建模为高斯分布，并使用这些分布快速将每帧对齐到3D场景。

Result: 在广泛使用的基准测试中进行了评估，验证了设计有效性，在视图渲染、相机跟踪、运行时间和存储复杂度方面均优于最新方法。

Conclusion: SGAD-SLAM通过创新的像素对齐高斯表示和深度分布建模，在保持系统可扩展性的同时显著提高了3D高斯溅射在RGBD SLAM中的渲染质量和跟踪速度。
...
TL;DR: LRHPerception是一个实时单目感知系统，将物体跟踪预测、道路分割和深度估计集成到统一框架中，在单GPU上达到29FPS，比最快的地图方法快555%


<details>
  <summary>Details</summary>
Motivation: 当前基于摄像头的自动驾驶技术往往优先考虑效果而忽视计算效率，需要开发既能保持高性能又能实时运行的感知系统

Method: 结合端到端学习的计算效率和局部地图方法的丰富表示细节，将单目图像数据处理成包含RGB、道路分割和像素级深度估计的五通道张量，并增强物体检测和轨迹预测

Result: 在物体跟踪预测、道路分割和深度估计方面有显著改进，在单GPU上实现29FPS的实时处理，比最快的地图方法快555%

Conclusion: LRHPerception成功解决了自动驾驶感知系统中效率与性能的平衡问题，为实时单目感知提供了有效的解决方案
...
TL;DR: 2Xplat：基于双专家设计的姿态无前馈3D高斯泼溅框架，将几何估计与高斯生成分离，在少于5K训练迭代中超越现有方法


<details>
  <summary>Details</summary>
Motivation: 当前姿态无前馈3DGS方法采用统一单体架构，将相机姿态估计和3DGS表示合成耦合在单一网络中。这种"一体化"设计可能不是高保真3DGS生成的最优方案，因为它在共享表示中纠缠了几何推理和外观建模。

Method: 提出2Xplat框架，采用双专家设计：专门的几何专家首先预测相机姿态，然后显式传递给强大的外观专家，由外观专家合成3D高斯。这种模块化设计明确分离了几何估计和高斯生成。

Result: 在少于5K训练迭代中，该双专家流水线显著优于先前的姿态无前馈3DGS方法，性能与最先进的姿态已知方法相当。这些结果挑战了当前统一的范式。

Conclusion: 模块化设计原则在复杂3D几何估计和外观合成任务中具有潜在优势。简单的双专家设计被证明非常有效，尽管在先前工作中很少被探索。
...
TL;DR: NoOVD是一个用于开放词汇目标检测的新训练框架，通过自蒸馏机制和知识引导的FPN来解决训练与测试阶段的差距问题，显著提升新类别物体的检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇目标检测方法存在训练与测试阶段的显著差距。在训练时，RPN和RoI头经常将未标记的新类别物体误分类为背景，导致这些提案在RPN阶段被过早过滤或在RoI头被进一步误分类。测试时这些提案得分低，在后处理中被移除，导致召回率显著下降，最终削弱新类别检测性能。

Method: 提出NoOVD训练框架：1) K-FPN：利用预训练视觉语言模型的知识引导模型发现新类别物体，实现无需额外数据的知识蒸馏，避免将新物体强制对齐到背景；2) R-RPN：在推理时调整提案的置信度分数，提高新类别物体的召回率。

Result: 在OV-LVIS、OV-COCO和Objects365数据集上的跨数据集评估表明，该方法在多个指标上始终优于现有方法，实现了卓越的性能。

Conclusion: NoOVD通过创新的自蒸馏机制和知识引导的架构设计，有效解决了开放词汇目标检测中训练与测试阶段的差距问题，显著提升了新类别物体的检测性能。
...
TL;DR: 提出协作教师语义分割框架，通过多教师协作机制和交叉教师可靠性评估，在仅有2%标注数据下提升前视声纳图像分割性能5.08%


<details>
  <summary>Details</summary>
Motivation: 前视声纳图像受斑点噪声、低纹理对比度、声学阴影和几何畸变影响，传统师生框架在标注数据极少条件下难以获得满意性能

Method: 提出协作教师语义分割框架，包含一个通用教师和多个声纳特定教师的多教师协作机制，采用多教师交替指导策略，并设计交叉教师可靠性评估机制动态量化伪标签可靠性

Result: 在FLSMD数据集上，仅使用2%标注数据时，相比其他最先进方法，mIoU提升5.08%

Conclusion: 所提框架能有效学习通用语义表示同时捕捉声纳图像独特特征，通过可靠性评估机制减轻噪声伪标签负面影响，在有限标注数据下实现更全面鲁棒的特征建模
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在跨模态感知和推理方面取得显著进展，但一个基本问题仍未解决：视觉编码器应该微调还是冻结？现有方法在不同多模态任务中表现不一致，且存在视觉偏好冲突问题。

Method: 提出上下文感知视觉微调（CoVFT）框架，包含上下文向量提取（CVE）和上下文混合专家（CoMoE）模块，将多模态上下文显式融入视觉适应，分解冲突的优化信号。

Result: 在12个多模态基准测试中，CoVFT实现了最先进的性能，具有优越的稳定性。使用CoVFT微调的7B MLLM超越了其13B对应模型的平均性能。

Conclusion: CoVFT框架解决了视觉编码器微调中的不稳定问题，揭示了MLLMs中视觉编码器优化的巨大潜力，为多模态模型设计提供了新方向。
...
TL;DR: 提出TextCSP框架，通过层次化文本引导的软级联架构，利用放射学描述文本改进脑肿瘤分割，在TextBraTS数据集上取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 脑肿瘤分割中，全肿瘤、肿瘤核心和增强肿瘤三个子区域常存在模糊的视觉边界。现有多模态方法通常将报告压缩为单一全局文本嵌入，忽略了不同子区域的临床特征差异。

Method: 提出TextCSP框架：1) 文本调制的软级联解码器，按解剖包含层次从粗到细预测WT->TC->ET；2) 子区域感知提示调优，使用可学习软提示和LoRA适配的BioBERT编码器为每个子区域生成专门文本表示；3) 文本语义通道调制器，将文本表示转换为通道级细化信号，使解码器能强调与临床描述模式对齐的特征。

Result: 在TextBraTS数据集上，相比现有最优方法，在所有子区域上Dice指标提升1.7%，HD95指标提升6%。

Conclusion: TextCSP通过层次化文本引导的软级联架构，有效利用放射学描述文本的临床信息，显著提升了脑肿瘤分割性能，特别是在处理模糊边界和区分不同子区域方面表现出色。
...
TL;DR: 该论文发现潜在扩散模型中β-VAE编码器产生的潜在空间对采样扰动过于敏感，导致生成质量下降，提出通过方差扩展损失来构建更鲁棒的潜在空间。


<details>
  <summary>Details</summary>
Motivation: 现有潜在扩散模型主要关注重建精度和语义对齐，但忽略了潜在空间对采样扰动的鲁棒性这一关键因素。研究发现β-VAE编码器产生的过度紧凑潜在流形对扩散采样中的随机扰动高度敏感，导致视觉质量下降。

Method: 提出方差扩展损失来对抗方差坍缩，利用重建与方差扩展之间的对抗性相互作用实现自适应平衡，在保持重建精度的同时提高对随机采样的鲁棒性。

Result: 实验表明该方法在不同潜在扩散架构中一致提升生成质量，证实潜在空间鲁棒性是稳定且忠实扩散采样的关键缺失要素。

Conclusion: 潜在空间对采样扰动的鲁棒性是决定生成质量的关键因素，提出的方差扩展损失能有效构建鲁棒潜在空间，为潜在扩散模型提供更稳定的生成能力。
...
TL;DR: DGRNet通过多视角分歧不确定性估计和文本引导细化，解决了脑肿瘤分割中不确定性量化不足和放射学报告信息利用不充分的问题，在TextBraTS数据集上显著提升了分割精度。


<details>
  <summary>Details</summary>
Motivation: 当前脑肿瘤分割方法存在两个关键问题：1) 单模型预测缺乏可靠的不确定性量化，这对临床决策至关重要；2) 未充分利用放射学报告中的丰富信息来指导模糊区域的分割。

Method: 提出DGRNet框架，包含：1) 多视角分歧不确定性估计：在共享编码器-解码器上附加四个轻量级视角特定适配器，单次前向传播生成多样化预测；2) 文本引导细化：基于分歧图识别高不确定性区域，利用临床报告进行选择性细化；3) 多样性保持训练策略：结合成对相似性惩罚和梯度隔离防止视角塌缩。

Result: 在TextBraTS数据集上，DGRNet将Dice系数提升了2.4%，HD95指标提升了11%，同时提供了有意义的不确定性估计。

Conclusion: DGRNet通过多视角分歧估计和文本引导细化，有效解决了脑肿瘤分割中的不确定性量化和临床信息利用问题，为临床部署提供了更可靠的解决方案。
...
TL;DR: 提出RLAR梯度正则化方法，在甲状腺超声多任务学习中同时预测结节分割和TI-RADS风险分类，解决标注者差异导致的梯度竞争问题


<details>
  <summary>Details</summary>
Motivation: 甲状腺超声中放射科医生需要同时输出结节轮廓和TI-RADS风险等级，但不同医生的标注风格和风险分级存在差异，这种不一致的监督会降低标准学习流程的效果

Method: 提出临床指导的多任务框架，联合预测结节掩码和TI-RADS类别；使用TI-RADS对齐的放射组学目标指导分类嵌入；引入RLAR（表示级对抗梯度正则化），在潜在空间中使用任务特定的对抗方向作为几何探针，惩罚任务间对抗方向的过度角度对齐

Result: 在公共TI-RADS数据集上，带RLAR的临床指导多任务模型相比单任务训练和传统多任务基线，在保持分割质量的同时持续改善风险分层

Conclusion: RLAR方法通过显式控制多任务学习中的梯度竞争，有效解决了标注者差异带来的监督不一致问题，实现了更好的临床风险分层性能
...
TL;DR: 提出PATrack方法，通过渐进式适配器将RGB预训练模型有效迁移到多模态跟踪任务，包含模态依赖、模态纠缠和任务级三种适配器。


<details>
  <summary>Details</summary>
Motivation: 现有方法在将RGB预训练模型适配到多模态跟踪时，通常只使用参数高效的微调模块，但忽视了高级适配策略，未能有效调制单个特定模态、跨模态交互和预测头。

Method: 提出渐进式多模态跟踪适配(PATrack)，包含三种适配器：1) 模态依赖适配器通过分解高低频分量增强模态特定信息；2) 模态纠缠适配器通过跨模态注意力实现模态间交互；3) 任务级适配器专门适配预测头以适应融合信息。
...
<details>
  <summary>Details</summary>
Motivation: 当前参数高效微调（PEFT）方法主要局限于单任务适应，而多任务学习（MTL）需要单个模型高效解决多个任务。现有方法在多任务场景下的参数效率和性能平衡不足。

Method: 提出Free Sinewich框架：1）Sine-AWB层将低秩因子和卷积先验结合为单一核，通过正弦变换进行逐元素调制生成任务专用权重；2）轻量级Clock Net生成有界频率以稳定训练期间的调制；3）正弦调制增强低秩适配器的秩，频率分离解耦不同任务的权重。

Result: 在密集预测基准测试中，Free Sinewich实现了最先进的性能-效率平衡：相比单任务微调提升高达+5.39%，仅需6.53M可训练参数，提供了基于频率参数共享的紧凑可扩展范式。

Conclusion: Free Sinewich通过频率切换实现近零成本的权重调制，为多任务学习提供了参数高效且性能优越的解决方案，在保持紧凑性的同时实现了任务间的有效解耦。
...
Method: 引入一个受控诊断基准，在100个合成场景和6,000个关系查询中评估关系一致性。测量视角一致性、360°循环一致性和顺序变换下的关系稳定性。评估多种输入表示：视觉输入、文本边界框和结构化场景图。

Result: 尽管单视角准确率高，但最先进的MLLMs在反事实视角变化下表现出系统性退化，频繁违反循环一致性，关系稳定性迅速衰减。增加表示结构（如结构化场景图）能改善稳定性。

Conclusion: 单视角空间准确性高估了诱导空间表示的鲁棒性，表示结构在反事实空间推理中起着关键作用。需要更稳健的空间表示方法来应对视角变化。
...
TL;DR: 提出LiFR-Seg框架，利用异步事件数据实现任意时间点的语义分割，使低帧率相机达到接近高帧率系统的性能


<details>
  <summary>Details</summary>
Motivation: 传统低帧率相机在动态环境中存在感知间隙，无法捕捉快速运动场景的细节，需要新的方法来实现高帧率感知能力

Method: 提出LiFR-Seg框架，核心是：1) 基于事件驱动的运动场进行不确定性感知的特征扭曲；2) 时间记忆注意力模块确保动态场景中的连贯性

Result: 在DSEC数据集上达到73.82% mIoU，与拥有完整目标帧信息的高帧率上限系统性能差距仅0.09%，统计上无显著差异

Conclusion: 提出了一种新的高效范式，仅使用低帧率硬件就能实现鲁棒的高帧率感知，为动态环境语义分割提供了新解决方案
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在多焦点图像融合中表现优异，但散焦模糊会导致对称几何结构（如纹理和边缘）扭曲变形，从而在融合图像中产生意外伪影。因此，将旋转等变性嵌入扩散网络至关重要，这能使融合结果忠实保持输入图像中几何模式的原始方向和结构一致性。

Method: 提出ReDiffuse旋转等变扩散模型，精心构建基本扩散架构以实现端到端的旋转等变性，并提供严格的理论分析来评估其内在等变误差，证明嵌入等变结构的有效性。

Result: 在四个数据集（Lytro、MFFW、MFI-WHU和Road-MF）上全面评估，与各种MFIF方法相比，ReDiffuse在六个评估指标上实现了0.28-6.64%的性能提升，表现出竞争力。

Conclusion: ReDiffuse通过嵌入旋转等变性有效解决了多焦点图像融合中几何结构扭曲的问题，在保持结构一致性的同时提升了融合性能，代码已开源。
...
TL;DR: 提出MCM框架，通过多聚类内存组织改进实用测试时适应，解决传统单聚类内存与多模态测试流不匹配的问题


<details>
  <summary>Details</summary>
Motivation: 现有PTTA方法普遍使用单聚类内存存储样本，但实际测试流本质上是多模态的，单聚类设计与实际分布不匹配，需要更合理的记忆组织方式

Method: 提出多聚类内存框架：1) 基于像素级统计描述符的聚类分配；2) 相邻聚类合并控制内存使用；3) 均匀聚类检索确保平衡监督

Result: 在4个数据集12个配置上一致提升，ImageNet-C提升5.00%，DomainNet提升12.13%，分布复杂度越高提升越大

Conclusion: 内存组织是实用测试时适应的关键设计维度，多聚类内存能保持接近最优的分布平衡、熵和模式覆盖
...
TL;DR: MS-CustomNet：用于多主体定制的扩散模型框架，支持零样本集成多个用户提供的对象，并允许用户显式定义层次结构和空间关系，在保持个体身份的同时实现精确的多主体构图控制。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散式文本到图像生成方法在多主体场景定制方面存在局限，难以提供对构图结构和主体间空间关系的显式控制，导致用户无法精确指定多个不同主体之间的交互和布局。

Method: 提出MS-CustomNet框架，支持零样本集成多个用户提供的对象，允许用户显式定义层次化排列和空间布局。同时构建了基于COCO的MSI数据集，用于训练复杂的多主体构图模型。

Result: 在身份保持方面获得DINO-I分数0.61，在位置控制方面获得YOLO-L分数0.94，表明该方法能够生成高保真图像，并实现精确的用户导向多主体构图和空间控制。

Conclusion: MS-CustomNet通过提供细粒度的多主体构图控制，显著提升了扩散模型在多主体定制场景中的能力，实现了更好的身份保持和空间关系控制，为复杂场景生成提供了有效解决方案。
...
TL;DR: 提出RLVC框架，使用强化学习增强生成式零样本学习，通过视觉线索和结果奖励机制提升特征生成质量，在三个基准测试上实现4.7%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有生成式零样本学习方法存在两个主要问题：1）生成的特征通常是任务无关的，导致性能下降；2）仅从语义原型推断分布对于语义相似但视觉差异大的类别不够准确。需要一种能生成任务相关特征并更好处理视觉差异的方法。

Method: 提出RLVC框架：1）使用基于结果的强化学习奖励机制，鼓励生成任务相关特征；2）引入类别级视觉线索，将生成特征与视觉原型对齐并稳定RL训练；3）提出新颖的冷启动策略用于训练过程。

Result: 在三个主流零样本学习基准测试上进行全面实验和分析，RLVC实现了最先进的性能，获得了4.7%的性能提升。

Conclusion: RLVC框架通过强化学习自我进化机制、视觉线索对齐和冷启动策略，有效解决了生成式零样本学习中特征任务无关和视觉差异处理不足的问题，显著提升了零样本学习性能。
...
TL;DR: 提出无需训练的3D室内场景重建系统，从稀疏无位姿RGB图像实现重建、理解和渲染，结合点云重建、实例提升和扩散模型渲染三大创新。


<details>
  <summary>Details</summary>
Motivation: 传统辐射场方法需要密集视角和逐场景优化，限制了3D场景重建的效率和应用。本文旨在开发无需训练或位姿预处理的系统，从稀疏图像实现高质量3D重建和编辑。

Method: 系统包含三个核心模块：1) 基于扭曲异常去除的鲁棒点云重建；2) 扭曲引导的2D到3D实例提升，将2D分割掩码传播为一致的实例感知3D表示；3) 点云投影结合3D感知扩散模型的渲染方法。

Result: 实现了高保真3D重建和渲染，在稀疏输入条件下通过扩散模型补偿缺失几何并增强真实感。支持通过修改点云进行对象级场景编辑（如实例移除），无需重新训练即可合成一致的编辑视图。

Conclusion: 为无需场景特定优化的高效、可编辑3D内容生成开辟了新方向，展示了训练无关方法在3D重建和编辑方面的潜力。
...
TL;DR: GIDE提出了一种用于扩散大语言模型的无训练图像编辑框架，通过离散噪声反演机制在离散标记空间中实现高保真重建，支持多种编辑指令并保持背景不变。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在多模态生成方面表现出色，但缺乏精确的无训练图像编辑能力。离散标记化阻碍了传统噪声反演技术的应用，导致编辑时结构退化。

Method: GIDE框架包含三个阶段：1) 基础定位阶段，2) 离散噪声反演机制在离散标记空间中准确捕捉潜在噪声模式，3) 细化阶段。支持文本、点和框等多种编辑指令。

Result: 在GIDE-Bench（805个组合编辑场景）上，GIDE显著优于现有无训练方法，语义正确性提升51.83%，感知质量提升50.39%。在ImgEdit-Bench上也表现出色，与领先模型相当。

Conclusion: GIDE为扩散大语言模型提供了一个统一的无训练图像编辑框架，通过离散噪声反演解决了结构退化问题，在多种编辑任务中实现了高保真度和背景保持。
...
TL;DR: 提出DSCSNet网络，通过深度展开ADMM算法结合可学习参数，解决红外小目标混合斑点的分离问题，在稀疏保证和场景适应性间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 由于光学镜头焦距和探测器分辨率限制，远距离集群红外小目标常呈现为混合斑点。现有方法难以平衡模型驱动方法的严格稀疏保证与数据驱动方法的动态场景适应性。

Method: 提出动态稀疏压缩感知网络(DSCSNet)，将ADMM算法与可学习参数耦合。在ADMM辅助变量更新步骤中嵌入严格的ℓ₁范数稀疏约束替代传统ℓ₂范数平滑项；在重建阶段集成基于自注意力的动态阈值机制，自适应调整稀疏化强度。

Result: 在合成红外数据集CSIST-100K上的大量实验表明，DSCSNet在CSO-mAP和亚像素定位误差等关键指标上优于现有最先进方法。

Conclusion: DSCSNet保留了压缩感知的物理逻辑，实现了鲁棒的稀疏诱导和场景适应性，提高了复杂红外场景下的分离精度和泛化能力。
...
TL;DR: 提出了一种无提示、边界感知的实例分割框架，通过预测有符号距离函数来精确分离显微镜视频中的相邻细胞实例


<details>
  <summary>Details</summary>
Motivation: 显微镜视频中细胞实例分割对于研究细胞动力学至关重要，但分离接触或重叠的细胞实例仍然是一个持续挑战。现有的基础分割模型（如SAM）在没有大量提示的情况下，难以分离密集显微镜场景中的相邻细胞实例

Method: 提出了一种无提示、边界感知的实例分割框架，预测有符号距离函数而不是二值掩码，通过学习的sigmoid映射将SDF转换为概率图，使用统一的改进豪斯多夫距离损失函数结合区域和边界项指导训练

Result: 在公共和私有高通量显微镜数据集上的评估表明，与最近的SAM基方法和基础模型方法相比，该方法在边界精度和实例级性能方面都有所提升

Conclusion: 该方法通过预测有符号距离函数和统一的边界感知损失，有效解决了显微镜图像中相邻细胞实例的分离问题，提高了分割精度
...
<details>
  <summary>Details</summary>
Motivation: 现有绕过T2I模型安全过滤器的方法要么依赖代理损失优化而非真正的端到端目标，要么需要大规模且昂贵的RL训练生成器。这些局限性促使研究者开发更轻量、高效的绕过方法。

Method: JANUS将绕过攻击形式化为在T2I系统及其安全过滤器的黑盒端到端奖励下优化结构化提示分布。它用低维混合策略替代高容量生成器，该策略基于两个语义锚定的提示分布，实现高效探索同时保持目标语义。

Result: 在Stable Diffusion 3.5 Large Turbo上，将ASR-8从25.30%提升到43.15%，CLIP和NSFW分数也更高。该方法在开源和商业模型上都取得成功。

Conclusion: JANUS暴露了当前T2I安全管道的结构性弱点，需要更强的、分布感知的防御措施。论文包含可能令人反感的模型输出。
...
Method: Positional Seg-CFT方法将每个解剖结构细分为区域段，为每个区域独立推导测量值，从而实现对空间局部化和解剖一致的反事实生成的控制。

Result: 在冠状动脉CT血管造影实验中，Pos-Seg-CFT能够生成真实、区域特定的修改，为疾病进展建模提供更精细的空间控制。

Conclusion: Positional Seg-CFT方法通过区域细分和独立测量，克服了现有反事实图像生成方法的局限性，实现了空间局部化且解剖一致的反事实生成，为疾病建模提供了更精细的控制能力。
...
TL;DR: 提出GKP-GSA框架解决长尾分布中的尾部性能退化问题，通过分组知识保存和分组锐度感知模块，促进模型收敛到平坦且共享的最小值，无需额外数据或预训练模型。


<details>
  <summary>Details</summary>
Motivation: 长尾数据分布中性能权衡的长期挑战源于"尾部性能退化"现象：模型倾向于严重过拟合头部类别，同时快速遗忘尾部类别。从损失景观视角观察发现，不同类别在损失景观中具有不同的收敛点，且当模型陷入尖锐、非鲁棒的最小值时，这种分歧会加剧。

Method: 提出受持续学习启发的GKP-GSA框架：1) 分组知识保存模块：记忆组特定的收敛参数，促进收敛到共享解决方案，避免低效的每类参数保存；2) 分组锐度感知模块：通过显式处理损失景观的几何形状来寻找更平坦的最小值。框架无需外部训练样本或预训练模型。

Result: 在四个基准测试上进行广泛实验，相比最先进方法取得了显著的性能提升。代码已开源。

Conclusion: 通过损失景观视角解决长尾分布中的尾部性能退化问题，提出的GKP-GSA框架能够促进模型收敛到平坦且共享的最小值，显著提升长尾分类性能，具有广泛的适用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有道路提取方法通常只关注道路表面分割，缺乏对道路网络拓扑结构和层次等级的联合建模，难以生成语义一致的多等级道路地图

Method: 1) 构建SYSU-HiRoads数据集，包含密集道路掩码、矢量化中心线和三级层次标签；2) 开发RoadReasoner框架，通过频率敏感线索和多尺度上下文增强道路特征表示，利用几何描述符和几何感知文本提示，通过视觉语言模型进行层次推理

Result: 在SYSU-HiRoads和CHN6-CUG数据集上，RoadReasoner超越现有道路提取基线，生成准确且语义一致的道路层次地图，达到72.6% OA、64.2% F1分数和60.6% SegAcc

Conclusion: SYSU-HiRoads数据集和RoadReasoner框架为自动化交通基础设施制图、道路库存更新和基础设施管理应用提供了有效解决方案，数据集和代码将公开发布
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉计数研究主要集中在人群和交通分析，而具有非刚性形态、生长阶段和环境变化的植物细粒度分类感知计数研究不足，需要填补这一空白。

Method: 构建TPC-268数据集，包含实例级点标注、林奈分类标签（界->种）和器官类别，支持层次推理和物种感知评估。采用类无关计数（CAC）问题设置，提供分类一致、尺度感知的数据划分。

Result: 数据集包含10,000张图像、678,050个点标注，涵盖268个可计数植物类别、242个植物和真菌物种，覆盖从冠层遥感图像到组织显微镜的多尺度观测。基准测试了最先进的回归和检测类CAC方法。

Conclusion: TPC-268通过捕捉生物多样性、层次结构和多尺度特性，为推进细粒度类无关计数提供了生物学基础测试平台，填补了植物分类感知计数研究的空白。
...
TL;DR: 提出QMoP框架，通过三个分支自适应压缩视觉token，结合查询引导的路由器动态协调，显著降低多模态大语言模型的计算和内存开销。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型中视觉token数量远多于文本token，导致严重的计算和内存瓶颈。现有方法依赖固定启发式策略，缺乏跨场景的适应性。

Method: 提出Query Guided Mixture-of-Projector (QMoP)框架，包含三个协作分支：池化分支（粗粒度全局语义）、重采样分支（高层语义表示）、剪枝分支（细粒度token选择）。引入Query Guided Router (QGR)基于视觉输入和文本查询动态选择和加权各分支输出，采用MoE风格融合机制聚合结果。

Result: QMoP在性能上优于强基线方法，同时在内存、计算和推理时间方面带来显著节省。还开发了VTCBench基准来系统评估视觉token压缩的信息损失。

Conclusion: QMoP通过自适应协调多种压缩策略，有效解决了多模态大语言模型的视觉token压缩问题，在保持性能的同时大幅降低了计算开销。
...
TL;DR: DepthTCM是一个基于物理感知的端到端深度图压缩框架，通过多波长深度编码将高比特深度图无损转换为3通道图像，然后使用Transformer-CNN混合神经网络进行压缩，在保持高精度的同时显著降低比特率。


<details>
  <summary>Details</summary>
Motivation: 深度图压缩在计算机视觉和机器人应用中至关重要，但传统方法在保持高精度和低比特率方面存在挑战。需要一种既能有效压缩深度数据，又能保持物理准确性的解决方案。

Method: 1. 使用多波长深度编码将高比特深度图无损转换为3通道图像表示；2. 对MWD编码表示进行全局4比特量化以降低熵；3. 采用结合卷积和Transformer层的学习编解码器进行压缩。

Result: 在Middlebury 2014数据集上达到0.307 bpp，同时保持99.38%的精度；在ScanNet++ iPhone RGB-D子集上平均端到端推理时间：编码器41.48 ms，解码器47.45 ms；4比特量化相比8比特减少66%比特率，质量损失极小。

Conclusion: DepthTCM框架在深度图压缩方面表现出色，通过物理感知编码和Transformer-CNN混合架构实现了高精度、低比特率和实时性能的良好平衡，为实际应用提供了有效的解决方案。
...
TL;DR: 提出基于Vision Transformers和色彩映射特征增强的深度学习框架，用于多类别脑肿瘤MRI分类，在BRISC2025数据集上达到98.90%准确率，优于传统CNN模型。


<details>
  <summary>Details</summary>
Motivation: 脑肿瘤的准确分类对早期诊断和治疗规划至关重要。传统方法可能无法充分捕捉MRI图像中的复杂特征和长距离依赖关系，需要更先进的深度学习方法来提高分类性能。

Method: 提出基于Vision Transformers的深度学习框架，结合色彩映射特征表示增强。利用Transformer架构捕捉长距离依赖关系，通过色彩映射技术强调MRI扫描中的重要结构和强度变化特征。

Result: 在BRISC2025数据集（包含胶质瘤、脑膜瘤、垂体瘤和非肿瘤四类）上，模型达到98.90%分类准确率，AUC为99.97%，优于ResNet50、ResNet101和EfficientNetB2等基线CNN模型。
...
TL;DR: CornOrb是一个公开的多模态数据集，包含来自阿尔及利亚患者的1,454只眼睛的Orbscan角膜地形图图像和临床注释，用于AI驱动的圆锥角膜检测和分析研究。


<details>
  <summary>Details</summary>
Motivation: 为非洲地区提供首个大规模Orbscan角膜地形图数据集，支持基于多模态数据的圆锥角膜AI检测和分析研究，填补该地区相关数据资源的空白。

Method: 回顾性收集744名患者的1,454只眼睛数据，包括889只正常眼和565只圆锥角膜病例。为每只眼提供四种角膜地形图（轴向曲率、前表面高度、后表面高度、角膜厚度图）以及结构化表格数据（人口统计信息和关键临床参数）。所有数据经过完全匿名化处理，并预处理为标准化的PNG和CSV格式。

Result: 创建了包含1,454只眼睛的公开数据集，其中889只正常眼和565只圆锥角膜病例。数据集包含四种角膜地形图图像和结构化临床数据，已标准化处理并公开发布在Zenodo平台上。

Conclusion: CornOrb数据集为AI驱动的圆锥角膜检测和分析提供了首个来自非洲地区的大规模Orbscan多模态数据资源，有助于推动该领域的算法研究和临床应用。
...
TL;DR: FoB将基于SAM的少样本医学图像分割重构为提示定位任务，通过背景中心提示生成器约束SAM的过分割问题，在三个医学数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统少样本医学图像分割方法存在性能瓶颈，而SAM直接应用于医学图像时由于解剖边界模糊导致过分割问题，需要有效约束机制。

Method: 提出FoB框架，将分割任务重构为提示定位任务，通过背景中心提示生成器提供准确背景提示来约束SAM；建模丰富上下文信息捕捉前景-背景空间依赖关系，并利用医学图像中背景提示的结构模式作为约束逐步优化预测。

Result: 在三个不同的医学图像数据集上，FoB大幅超越其他基线方法，实现了少样本医学图像分割的最先进性能，并展现出强大的跨域泛化能力。

Conclusion: FoB通过将SAM-based少样本医学图像分割重构为提示定位任务，有效解决了SAM在医学图像中的过分割问题，为临床应用提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在推理任务上的改进主要依赖高质量标注数据或教师模型蒸馏，这两种方法成本高昂且难以扩展，需要一种可扩展的无监督训练方法

Method: 提出SelfJudge框架：1）对每个输入采样多个推理轨迹并建模组内结构；2）使用Actor的自一致性信号作为训练先验；3）引入有界Judge调制机制重新加权不同质量的轨迹；4）将调制分数建模为组级分布，将绝对分数转换为组内相对优势，实现更稳健的策略更新

Result: 在五个数学推理基准测试上，该方法持续提升了推理性能和泛化能力，为自进化多模态模型提供了可扩展路径

Conclusion: SelfJudge框架通过无监督的自进化训练，在不依赖人工标注或外部奖励模型的情况下，实现了多模态推理模型的稳定性能提升，为解决数据标注成本问题提供了有效方案
...
TL;DR: TIGON提出了一种结合文本和图像条件的3D生成方法，通过双分支架构和轻量级跨模态融合，解决了单模态方法在视角偏差和细节缺失方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成器大多依赖单一条件模态：图像条件模型虽然视觉保真度高，但在输入视角有限或模糊时存在视角偏差；文本条件模型提供广泛的语义指导，但缺乏低层次视觉细节。这限制了用户表达意图的能力，需要探索如何结合两种模态以实现更灵活和忠实的3D生成。

Method: 提出TIGON方法，采用简约的双分支基线架构，包含独立的图像条件分支和文本条件分支，以及轻量级的跨模态融合机制，实现文本和图像的联合推理。

Result: 实验表明，文本-图像条件方法持续优于单模态方法，简单的后期融合也能超越单一模态模型，揭示了跨模态互补性的强大潜力。

Conclusion: 文本-图像条件3D生成通过结合视觉范例和文本规范，提供了更灵活和忠实的生成能力，跨模态视觉-语言指导是未来3D生成研究的有前景方向。
...
Method: 提出Mv²ID多视角条件化框架，采用区域掩码训练策略防止捷径学习，通过鼓励模型聚合跨视角的互补身份特征来提取本质身份特征。设计参考解耦RoPE机制，为视频和条件token分配不同的位置编码以更好地建模它们的异构特性。构建大规模多样化面部角度变化数据集。

Result: 大量实验表明，该方法在保持运动自然性的同时显著提高了身份一致性，性能优于使用交叉配对数据训练的现有方法。

Conclusion: Mv²ID框架通过创新的训练策略和位置编码机制，有效平衡了身份一致性和运动自然性，为解决多视角参考视频生成中的关键挑战提供了有效解决方案。
...
TL;DR: F4Splat提出了一种用于前馈3D高斯泼溅的自适应高斯分配策略，通过密度分数引导分配，减少冗余高斯并控制总数量，实现紧凑高质量3D重建。


<details>
  <summary>Details</summary>
Motivation: 现有前馈3D高斯泼溅方法采用刚性像素到高斯或体素到高斯的均匀分配管道，导致跨视图的冗余高斯，且缺乏有效控制高斯总数同时保持重建保真度的机制。

Method: 提出F4Splat，采用密度分数引导的分配策略，根据空间复杂度和多视图重叠自适应分布高斯。模型预测每个区域的密度分数来估计所需高斯密度，并允许显式控制最终高斯预算而无需重新训练。

Result: 实验表明，相比先前未校准的前馈方法，F4Splat实现了更优的新视角合成性能，同时使用显著更少的高斯。

Conclusion: F4Splat通过自适应高斯分配策略解决了前馈3D高斯泼溅中的冗余问题，实现了紧凑且高质量的3D表示，同时提供对高斯预算的显式控制。
...
TL;DR: FedDP-STECAR：联邦差分隐私选择性调优与高效通信的视频动作识别框架，通过选择性调优和扰动任务相关层，在保护隐私的同时减少99%通信开销，在UCF-101数据集上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 联邦视频动作识别面临两个关键挑战：1) 模型暴露风险 - 客户端与服务器交换的梯度可能泄露私有运动模式；2) 通信开销 - 高维视频网络的完整模型同步导致显著的带宽和通信成本。

Method: 提出FedDP-STECAR框架，采用选择性调优策略：仅对任务相关的小部分层进行差分隐私下的微调和扰动，减少信息泄露面同时保持视频特征的时间连贯性。在聚合时仅传输调优层，大幅降低通信流量。

Result: 在UCF-101数据集上使用MViT-B-16x4 transformer进行实验：1) 严格隐私设置(ε=0.65)下，中心化设置中准确率提升70.2%；2) 联邦设置中训练速度加快48%，准确率达73.1%；3) 通信流量相比完整模型更新减少超过99%。

Conclusion: FedDP-STECAR通过选择性层调优和差分隐私扰动，有效解决了联邦视频动作识别中的隐私泄露和通信开销问题，实现了可扩展且隐私保护的视频动作识别系统。
...
TL;DR: TTA-CaP：一种基于缓存的测试时自适应方法，通过三个协调的缓存实现无梯度的VLM个性化，用于视频面部表情识别，在保持低计算开销的同时提升性能。


<details>
  <summary>Details</summary>
Motivation: 视频面部表情识别存在显著的个体差异，需要模型个性化。现有的测试时自适应方法通常依赖无监督参数优化，计算开销大，不适用于实际应用。同时，基于缓存的方法仅使用动态内存存储测试样本，容易因噪声伪标签积累错误。

Method: 提出TTA-CaP方法，包含三个协调的缓存：个性化源缓存（存储源域原型）、正目标缓存（积累可靠的特定主体样本）、负目标缓存（存储低置信度样本作为负样本）。通过基于时间稳定性、置信度和与个性化缓存一致性的三门机制控制缓存更新和替换，最后通过嵌入融合细化预测。

Result: 在BioVid、StressID和BAH三个视频FER数据集上的实验表明，TTA-CaP在主体特定和环境偏移下优于最先进的TTA方法，同时保持低计算和内存开销，适合实际部署。

Conclusion: TTA-CaP通过协调的缓存机制实现了高效、无梯度的视觉语言模型个性化，解决了现有TTA方法计算开销大和缓存方法容易积累噪声的问题，为视频面部表情识别的实际应用提供了可行的解决方案。
...
Method: 1. 在DCT域应用自适应卡尔曼滤波，将高频DCT系数视为带噪声信号进行递归降噪；2. 基于估计的信噪比动态调整噪声参数；3. 训练时加入时间平滑性和关节角度限制等物理约束

Result: 在Human3.6M和HumanEva-I数据集上达到最先进的准确率，有效减少抖动伪影，生成平滑且物理合理的运动
...
TL;DR: EmoTaG：基于预训练-适配范式的少样本情感感知3D说话人头合成框架，通过结构化FLAME参数空间和门控残差运动网络，提升表情丰富度、唇形同步和运动稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有少样本方法在表达性面部运动下存在几何不稳定性和音频-情感不匹配问题，需要更有效的情感感知运动建模。

Method: 1. 在结构化FLAME参数空间而非直接变形3D高斯中进行运动预测，引入显式几何先验；2. 提出门控残差运动网络（GRMN），从音频捕获情感韵律，补充头部姿态和上脸线索。

Result: 在情感表达丰富度、唇形同步、视觉真实感和运动稳定性方面达到最先进性能。

Conclusion: EmoTaG通过结构化运动建模和情感感知机制，有效解决了现有少样本方法在表达性面部运动下的局限性。
...
TL;DR: 提出Coarse-to-Fine Diffusion Models with Time Step Sequence Redistribution方法，通过粗到细去噪和时间步序列重分配，在保持性能的同时大幅减少扩散模型的计算开销。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能生成高质量图像，但多步去噪过程计算量大，难以部署在资源受限的边缘设备上。现有方法要么忽略了输入冗余，要么需要过长的搜索时间。

Method: 1. Coarse-to-Fine Denoising (C2F)：利用早期生成图像难以区分的特性，在粗特征生成阶段减少计算；2. Time Step Sequence Redistribution (TRD)：设计高效采样轨迹调整方法，搜索时间少于10分钟。

Result: 在CIFAR10和LSUN-Church数据集上，方法实现了接近无损的性能，同时计算量减少了80%到90%。

Conclusion: 提出的C2F和TRD方法能有效减少扩散模型的计算开销，同时保持生成质量，为边缘设备部署提供了可行方案。
...
TL;DR: 使用视频变换器识别呼吸窘迫，通过运动后恢复视频创建数据集，采用ViViT编码器结合Lie相对编码和运动引导掩码，实现F1分数0.81


<details>
  <summary>Details</summary>
Motivation: 通过视觉检查识别呼吸窘迫是一项救命的临床技能，临床医生能检测呼吸恶化的早期迹象，为早期干预创造宝贵窗口。研究评估视频变换器能否使AI系统从视频中识别呼吸窘迫迹象。

Method: 收集健康志愿者剧烈运动后恢复的视频，利用参与者呼吸状态的自然恢复创建标记数据集。将视频分割为短片段，早期片段对应更严重的呼吸短促，设计时间排序挑战来评估AI系统检测呼吸窘迫的能力。采用ViViT编码器增强Lie相对编码和运动引导掩码，结合基于嵌入的比较策略。

Result: 增强的ViViT编码器结合Lie相对编码和运动引导掩码，配合嵌入比较策略，在该任务上实现了0.81的F1分数。
...
TL;DR: FluidGaussian：一种结合流体模拟的3D重建方法，通过物理交互评估表面质量，提升视觉和物理保真度
...
Method: 提出FluidGaussian方法，将几何重建与流体-结构相互作用紧密结合，定义基于流体模拟的不确定性度量，并与主动学习结合，优先选择能同时提升视觉和物理保真度的视角

Result: 在NeRF Synthetic、Mip-NeRF 360和DrivAerNet++数据集上，FluidGaussian方法实现了最高+8.6%的视觉PSNR提升和-62.3%的流体模拟速度散度降低

Conclusion: 通过将物理交互（特别是流体模拟）融入3D重建过程，可以显著提升重建结果的物理合理性和视觉质量，为物理感知的3D重建提供了新思路
...
TL;DR: 本文提出Relax Forcing方法，通过结构化时间记忆机制改进自回归视频扩散模型的长时生成能力，将历史上下文分解为三个功能角色进行选择性利用，显著提升运动动态和时序一致性。


<details>
  <summary>Details</summary>
Motivation: 当前自回归视频扩散模型在扩展到分钟级长视频生成时面临渐进性时间退化问题。研究发现这并非由内存不足引起，而是时间记忆在推理过程中的利用方式不当。传统方法将时间记忆视为同质缓冲区，而实际分析表明历史上下文的时间位置显著影响运动动态。

Method: 提出Relax Forcing结构化时间记忆机制，将时间上下文分解为三个功能角色：Sink（全局稳定性）、Tail（短期连续性）和动态选择的History（结构性运动指导）。该方法选择性地仅纳入最相关的历史信息，而不是密集关注整个生成历史。

Result: 在VBench-Long基准测试中，Relax Forcing显著改善了运动动态和整体时序一致性，同时减少了注意力开销。实验表明增加内存并不能持续改善长时生成，而结构化时间记忆机制能有效缓解外推过程中的错误累积。

Conclusion: 结构化时间记忆对于可扩展的长视频生成至关重要，补充了现有的基于强制的训练策略。Relax Forcing通过更智能地利用历史上下文，在保持运动演化的同时缓解了错误累积问题。
...
TL;DR: HamVision是一个医学图像分析框架，利用阻尼谐振子作为结构化归纳偏置，通过相空间分解得到位置、动量和能量三种功能不同的表示，分别用于特征内容、边界纹理信息和显著性图，在分割和分类任务上都取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分析需要有效的特征表示，传统方法缺乏结构化归纳偏置。阻尼谐振子作为信号处理的基本构件，其相空间分解能自然产生功能不同的表示，为医学图像分析提供有物理意义的特征。

Method: 使用阻尼谐振子作为结构化归纳偏置，通过相空间分解得到位置q（特征内容）、动量p（空间梯度编码边界和纹理信息）、能量H（无参数显著性图）。对于分割任务，能量门控跳跃连接，动量在每个解码器层注入边界信息（HamSeg）；对于分类任务，三种表示全局池化并拼接成相空间特征向量（HamCls）。

Result: 在10个医学影像基准测试中，HamSeg在ISIC 2018（89.38%）、ISIC 2017（88.40%）、TN3K（87.05%）和ACDC（92.40%）上达到最先进的Dice分数，仅用8.57M参数；HamCls在BloodMNIST（98.85%）和PathMNIST（96.65%）上达到最先进准确率，在其他MedMNIST数据集上与MedMamba和MedViT竞争。诊断分析确认动量编码了内>边界>外梯度，能量图与分类判别区域相关。

Conclusion: HamVision展示了阻尼谐振子作为结构化归纳偏置在医学图像分析中的有效性，其相空间分解产生的功能表示能同时支持分割和分类任务，且这些属性完全从哈密顿动力学中涌现，无需监督学习。
...
<details>
  <summary>Details</summary>
Motivation: 传统相位解缠方法在处理地震形变等复杂场景时容易失败，特别是浅层形变源会产生地表断层和位移不连续；现有基于学习的方法受限于固定的小输入尺寸，而实际InSAR干涉图通常是大规模且空间异质的

Method: 基于扩散模型的相位解缠框架，能够处理大规模干涉图并解决断层相关的相位跳跃问题，利用扩散模型架构恢复物理一致的解缠相位场

Result: 在合成和真实数据集上的实验结果表明，该方法能有效处理近地表形变相关的不连续性，并能很好地扩展到大规模InSAR图像，为挑战性场景提供了实用的手动解缠替代方案

Conclusion: 提出的扩散模型框架为解决InSAR相位解缠中的复杂形变和不连续问题提供了有效方案，特别是在处理大规模数据和断层相关相位跳跃方面表现出优势
...
<details>
  <summary>Details</summary>
Motivation: 开放词汇全景分割面临两个耦合问题：掩码选择偏差（在封闭词汇上训练的对象性头会抑制未见类别的掩码）和视觉语言模型（如CLIP）的区域理解有限（这些模型是为全局图像分类而非局部分割优化的）。

Method: 提出OVRCOAT框架，包含两个组件：1) CLIP条件对象性调整（COAT），更新背景/前景概率，保留未见对象的高质量掩码；2) 开放词汇掩码到文本细化（OVR），增强CLIP的区域级对齐，以更低的记忆成本改进已见和未见类别的分类。

Result: 在ADE20K上实现了+5.5% PQ的显著提升，在Mapillary Vistas和Cityscapes上分别获得+7.1%和+3% PQ的明显增益，达到了新的最先进水平。

Conclusion: OVRCOAT通过联合改进对象性估计和掩码识别，有效解决了开放词汇全景分割中的关键问题，尽管方法简单但效果显著，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 传统隐私保护方法在现实开放集视频场景中失效，因为身份未知且无身份标签，需要解决隐私保护与表情识别性能的平衡问题

Method: 两阶段框架：1) 使用无身份标签的真实视频训练身份抑制网络，利用视频内和视频间知识先验去身份化；2) 去噪模块恢复表情相关信息；3) 提出基于伪造的验证方法评估隐私鲁棒性

Result: 在三个视频数据集上实验表明，该方法有效保护隐私，同时保持与身份监督基线相当的表情识别准确率

Conclusion: 提出的无身份标签两阶段框架成功解决了开放集视频表情识别中的隐私保护问题，实现了隐私与效用的有效分离
...
TL;DR: Beta-KD是一种基于贝叶斯框架的不确定性感知知识蒸馏方法，通过自适应调整学生对教师指导的依赖程度来提升视觉语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 在知识蒸馏中，确定从数据学习和从教师学习之间的最佳平衡具有挑战性，因为有些样本可能包含噪声，而其他样本可能受到教师不确定性的影响，因此需要自适应地平衡数据和教师的监督。

Method: 提出Beta加权知识蒸馏（Beta-KD），从统一的贝叶斯视角制定师生学习，将教师监督解释为学生激活的Gibbs先验，从而产生闭式的不确定性感知加权机制，支持任意的蒸馏目标及其组合。

Result: 在多模态VQA基准测试上的广泛实验表明，从大型教师VLM蒸馏学生视觉语言模型持续提升性能，Beta-KD优于现有的知识蒸馏方法。

Conclusion: Beta-KD提供了一个有效的、不确定性感知的知识蒸馏框架，能够自适应地调整学生对教师指导的依赖，在视觉语言任务中表现出优越性能。
...
TL;DR: 开发了一个能够从手绘图纸中解决理想化梁模型的程序，结合计算机视觉和统计学习技术进行结构元素检测与视觉解释


<details>
  <summary>Details</summary>
Motivation: 将计算机视觉技术应用于土木工程领域，特别是结构分析，解决从手绘图纸自动识别和解析结构模型的问题，探索计算机视觉在土木工程中的可靠应用

Method: 基于计算机视觉和统计学习技术，开发PhotoBeamSolver程序，通过视觉检测和解释结构元素，分析计算机视觉在结构分析中的集成挑战和限制

Result: 实现了能够从手绘图纸中解决理想化梁模型的程序，分析了计算机视觉在土木工程中应用的主要挑战、限制和可靠性要求

Conclusion: 计算机视觉在土木工程领域，特别是结构分析、基础设施检测和工程决策支持系统中具有应用潜力，但需要解决集成挑战和可靠性要求
...
<details>
  <summary>Details</summary>
Motivation: 在线单目3D重建面临稳定性-适应性困境：模型需要快速适应新视角，同时保持已积累的场景结构。现有方法使用均匀或基于注意力的更新机制，无法有效处理突然的视角变化，导致长序列中的轨迹漂移和几何不一致。
...
Conclusion: PAS3R通过姿态自适应更新机制有效解决了在线3D重建的稳定性-适应性困境，为长视频序列的鲁棒重建提供了新思路。
...
TL;DR: EpiMask：针对卫星图像的半稠密匹配网络，通过结合相机几何模型和注意力机制，相比地面训练模型提升30%匹配精度


<details>
  <summary>Details</summary>
Motivation: 现有深度学习图像匹配网络基于地面图像数据集训练，针对针孔相机几何优化，在卫星图像匹配中表现不佳，因为卫星图像是移动相机逐行扫描合成的

Method: 提出EpiMask网络：1) 引入块状仿射近似相机几何模型；2) 使用基于极线距离的注意力掩码限制交叉注意力到几何合理区域；3) 微调预训练图像编码器进行鲁棒特征提取

Result: 在SatDepth数据集上实验显示，相比重新训练的地面模型，匹配精度提升高达30%

Conclusion: EpiMask通过显式建模卫星相机几何和注意力机制，显著提升了卫星图像匹配性能，解决了现有地面训练模型在卫星图像上的局限性
...
TL;DR: ALADIN通过属性-语言蒸馏网络，将冻结CLIP教师的知识蒸馏到轻量级ReID学生模型中，引入细粒度属性-局部对齐和场景感知提示生成，提升行人重识别性能。
...
Method: 提出ALADIN框架：1) 场景感知提示生成器产生图像特定的软提示；2) 属性-局部蒸馏强制文本属性与局部视觉特征的一致性；3) 跨模态对比和关系蒸馏保持属性间结构关系；4) 利用多模态大语言模型生成结构化属性描述，并通过CLIP转换为局部注意力图。

Result: 在Market-1501、DukeMTMC-reID和MSMT17数据集上，ALADIN超越了基于CNN、Transformer和CLIP的方法，展现出更好的泛化能力和可解释性。

Conclusion: ALADIN通过细粒度属性-语言对齐和自适应提示生成，有效提升了行人重识别性能，同时保持了轻量级推理，为跨模态行人重识别提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 持续遗忘面临的主要挑战是：在顺序遗忘更新过程中，共享表示会被扭曲，导致视觉语言对与拒绝行为之间产生虚假关联，从而阻碍精确识别拒绝目标，造成不适当的拒绝。现有方法难以在保持模型通用能力的同时，实现精确的拒绝行为。

Method: 提出基于概念的持续遗忘框架：1) 通过概念调制器识别哪些视觉-语言概念组合表征每个遗忘类别；2) 通过拒绝专家混合（refusers）确定如何生成适当的拒绝响应，每个专家专门负责概念对齐的拒绝生成；3) 引入多模态概念驱动路由方案，在顺序任务中重用共享相似概念的拒绝专家，并为新概念调整未充分利用的专家。

Result: 在视觉语言基准测试上的广泛实验表明，所提出的框架优于现有方法，能够生成基于概念的拒绝响应，并在遗忘序列中保持模型的通用能力。

Conclusion: 通过将拒绝行为建立在细粒度概念分解上，并采用概念驱动的路由机制，该框架能够有效解决持续遗忘中的表示扭曲问题，实现精确的拒绝目标识别和适当的拒绝响应生成，同时保持模型的整体效用。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频推理分割方法依赖单向隐式的文本-轨迹对齐，在面对复杂视频动态时难以准确感知物体轨迹，需要更有效的轨迹感知机制。

Method: 提出双向文本-轨迹对齐：MLLM同时接受定位导向（文本到轨迹）和描述导向（轨迹到文本）的指令；引入帧级内容集成模块将轨迹级token适配到帧特定信息；采用统一掩码解码器将所有帧的分割统一到单一结构中。

Result: 在多个参考和推理视频分割数据集上的实验表明，TrajSeg在所有指标上均优于现有视频推理分割方法，表现出卓越的性能。

Conclusion: TrajSeg通过双向对齐机制和统一架构，有效解决了视频动态场景下的轨迹感知问题，为视频推理分割提供了简单、统一且可端到端训练的解决方案。
...
TL;DR: StreamingEval是一个统一的评估框架，用于在真实资源约束下评估Video-LLMs的流式视频理解能力，通过标准化协议量化效率、存储和准确性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有流式视频理解研究通常只关注孤立方面（如有限视觉上下文下的问答准确性或编码效率改进），而忽视了实际部署中的资源约束问题。需要弥合这一差距，为实际流式应用提供系统性评估。

Method: 采用固定容量记忆库来规范化可访问的历史视觉上下文，联合评估视觉编码效率、文本解码延迟和任务性能，通过标准化协议对主流离线模型和近期在线视频模型进行基准测试。

Result: 跨多个数据集的广泛实验显示，当前Video-LLMs与真实流式应用需求之间存在显著差距，为未来研究方向提供了系统性基础。

Conclusion: StreamingEval框架填补了流式视频理解评估的空白，通过统一标准量化效率-存储-准确性权衡，揭示了当前模型的局限性并指导未来研究。
...
TL;DR: 提出HIPSS方法，通过缩放平移特征的高效提示调优和软层次文本指导策略，解决少样本弱监督WSI分类中计算成本高和信息损失问题


<details>
  <summary>Details</summary>
Motivation: 解决现有FSWC方法中预训练视觉语言模型存在的两个主要问题：1）提示调优方法显著增加可训练参数和推理开销；2）丢弃与文本嵌入对齐度低的实例导致信息损失

Method: 1）提出参数高效的提示调优方法，通过在文本编码器中缩放和移位特征来降低计算成本；2）引入软层次文本指导策略的WSI表示学习方法，利用VLM预训练知识和WSI固有层次结构，避免硬实例过滤

Result: 在乳腺、肺和卵巢癌病理数据集上分别取得10.9%、7.8%和13.8%的性能提升，可训练参数分别减少18.1%、18.1%和5.8%，同时在弱监督肿瘤定位方面表现优异

Conclusion: HIPSS方法通过高效提示调优和软层次指导策略，在减少计算成本的同时充分利用WSI信息，显著提升了少样本弱监督WSI分类性能
...
TL;DR: BTP是一个零样本3D异常检测框架，通过点云-语言模型对齐实现无需目标类别训练数据的缺陷检测，在多个数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法将3D点云渲染为2D图像，利用预训练视觉语言模型进行异常检测，但会丢失几何细节且对局部异常敏感度有限。需要探索点云-语言模型在3D异常检测中的潜力。

Method: 提出BTP框架，通过多粒度补丁特征与文本表示对齐实现局部异常检测，结合几何描述符增强结构异常敏感性，并利用辅助点云数据进行联合表示学习。

Result: 在Real3D-AD和Anomaly-ShapeNet数据集上的广泛实验表明，BTP在零样本3D异常检测中实现了优越性能。

Conclusion: BTP证明了点云-语言模型在3D异常检测中的有效性，为工业检测提供了无需训练数据的可靠解决方案。
...
TL;DR: VIGIL是一个基于多模态大语言模型的深度伪造检测框架，采用"先规划后检查"的两步流程，通过分阶段注入证据和渐进式训练，显著提升检测的可解释性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于MLLM的深度伪造检测方法将证据生成和操纵定位结合为单一步骤，导致忠实观察与幻觉解释的界限模糊，产生不可靠的结论。需要更接近专家法医实践的可靠框架。

Method: VIGIL采用部分中心的结构化法医框架：1) 基于全局视觉线索规划需要检查的面部部位；2) 独立检查每个部位并获取法医证据。采用阶段门控注入机制确保部位选择不受外部信号偏见影响。提出渐进式三阶段训练范式，其中强化学习阶段使用部分感知奖励来强制解剖有效性和证据-结论一致性。

Result: 在OmniFake（包含5个层次的基准测试）和跨数据集评估中，VIGIL在所有泛化级别上始终优于专家检测器和并发的MLLM方法。模型仅在三个基础生成器上训练，但能逐步测试到野外社交媒体数据。

Conclusion: VIGIL通过结构化的"先规划后检查"流程和渐进式训练，显著提高了深度伪造检测的可解释性和泛化能力，为可靠的MLLM法医分析提供了新范式。
...
TL;DR: PEARL提出了一种无需训练的开集语义分割方法，通过Procrustes对齐和文本感知的拉普拉斯传播两步推理，在保持设计简洁的同时实现了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有无需训练的开集语义分割方法要么依赖繁重的后处理，要么孤立处理文本和视觉信息，未能充分利用跨模态几何关系。其他方法引入额外的视觉骨干或多模型流水线，增加了复杂性和延迟，同时损害了设计简洁性。

Method: PEARL采用对齐-传播两步推理：1) Procrustes对齐步骤在最后一个自注意力块内执行正交投影，通过稳定的极坐标迭代将键向量旋转到查询子空间；2) 文本感知的拉普拉斯传播通过置信度加权、文本引导的图求解在小网格上细化每个像素的logits，文本提供数据信任信号和邻居门控，图像梯度保留边界。

Result: PEARL在标准基准测试中，无论使用背景协议还是不使用背景协议，都实现了无需训练的开集语义分割的SOTA性能，无需额外数据或辅助骨干网络。

Conclusion: PEARL是一种完全无需训练、即插即用的方法，仅使用固定常数，通过小的每头投影和少量共轭梯度步骤添加最小延迟，在保持设计简洁的同时实现了卓越性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到视频扩散模型的概念擦除技术评估仅限于检查目标概念是否从生成帧中消失，将输出级抑制视为表征移除的证据，但缺乏对模型内部表征残留的量化评估。

Method: PROBE协议在模型参数冻结的情况下，通过去噪重建目标和新颖的潜在对齐约束优化轻量级伪令牌嵌入，量化已擦除概念的再激活潜力。

Result: 在三种T2V架构、三种概念类别和三种擦除策略上的系统实验表明，所有测试方法都留下可测量的残留能力，其鲁棒性与干预深度相关，并识别出时间性重现这一视频特定失败模式。

Conclusion: 当前擦除方法仅实现输出级抑制而非真正的表征移除，PROBE协议为可复现的安全审计提供了诊断工具，揭示了概念擦除的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有单图像3D生成方法在处理多样化语义类别和复杂结构时存在泛化能力不足的问题，通常采用整体建模或固定部件数量，容易导致过拟合、结构碎片化或缺失，以及面对新物体布局时的组合泛化能力有限。

Method: 提出部件到整体的3D生成世界模型，通过自适应槽门控机制从图像令牌推断软组合掩码，动态发现潜在结构槽；将提炼的槽与可学习的类别无关原型库对齐，实现跨类别形状共享；引入轻量级3D去噪器通过统一扩散目标重建几何和外观。

Result: 实验显示在跨类别迁移和部件数量外推方面取得一致提升，消融实验证实原型库在形状先验共享和槽门控在结构适应方面的互补优势。

Conclusion: 通过将单图像3D生成重新构想为在灵活3D潜在空间中学习自适应部件-整体层次结构，实现了更好的泛化能力和结构适应性，为真实世界视觉到图形模型提供了更可靠的解决方案。
...
TL;DR: 提出PALS方法，通过可学习的配对亲和力学习与评分，解决弱监督视频场景图生成中非交互对象噪声问题，结合关系感知匹配提升伪标签质量，在Action Genome数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 弱监督视频场景图生成方法依赖现成检测器生成对象提案，但这些检测器会无差别检测所有可见对象（包括非交互对象），导致关系模型被大量噪声配对淹没，而全监督检测器会隐式过滤非交互对象，两者存在根本差异。

Method: 1. 引入可学习的配对亲和力估计主体-对象对之间的交互可能性；2. PALS：将配对亲和力纳入推理时排名；3. PAM：通过配对亲和力调制将亲和力整合到上下文推理中；4. RAM：利用视觉-语言基础解决伪标签生成中的类别级模糊性，为亲和力学习提供更干净的监督。

Result: 在Action Genome数据集上的广泛实验表明，该方法在不同基线和骨干网络上均带来显著改进，实现了最先进的WS-VSGG性能。

Conclusion: 通过配对亲和力学习和关系感知匹配，有效解决了弱监督视频场景图生成中非交互对象噪声问题，显著提升了模型性能，为减少标注成本的同时保持高质量场景图解析提供了有效方案。
...
TL;DR: 提出基于多模态提示的无监督持续异常检测框架，通过多模态提示记忆库和缺陷语义引导的自适应融合机制，在MVTec AD和VisA数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有仅依赖视觉信息的UCAD方法在复杂场景中难以充分捕捉正常模式流形，限制了异常检测精度的进一步提升。

Method: 1) 提出持续多模态提示记忆库(CMPMB)，从视觉和文本域逐步提取和保留原型正常模式；2) 设计缺陷语义引导的自适应融合机制(DSG-AFM)，包含自适应归一化模块(ANM)和动态融合策略(DFS)。

Result: 在MVTec AD和VisA数据集上，在图像级AUROC和像素级AUPR指标上达到最先进的性能。

Conclusion: 多模态信息能够丰富正常模式的表示，提出的框架通过结合视觉和文本信息有效提升了无监督持续异常检测的准确性和对抗鲁棒性。
...
TL;DR: 提出FSCE框架，通过频率-空间浅层特征自适应增强模块和噪声鲁棒知识蒸馏，提升SAR目标识别在噪声条件下的稳定性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: SAR图像固有的相干斑点噪声会掩盖目标特征，导致识别精度下降和模型泛化能力有限，需要解决噪声环境下的目标识别问题。

Method: 提出目标感知的频率-空间增强框架(FSCE)，包含频率-空间浅层特征自适应增强模块(DSAF)，结合空间多尺度卷积和频域小波卷积处理浅层特征。采用师生学习范式结合在线知识蒸馏方法，引导学生网络更关注目标区域，增强对高噪声背景的鲁棒性。

Result: 在MSTAR、FUSARShip和OpenSARShip数据集上实验，DSAFNet-L在三个数据集上达到竞争性或优于多种方法；DSAFNet-M在保持可比精度的同时显著降低模型复杂度。结果表明FSCE框架具有强大的跨模型泛化能力。

Conclusion: 提出的FSCE框架通过注意力转移和噪声鲁棒表示学习的协同优化，显著提高了噪声条件下目标识别的稳定性，为SAR ATR提供了有效的解决方案。
...
TL;DR: CataractSAM-2是基于Segment Anything Model 2的领域适配扩展，用于白内障眼科手术视频的实时语义分割，同时提供了交互式标注框架以减少人工标注负担，并展示了在青光眼手术中的零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够精确进行白内障眼科手术视频实时语义分割的模型，以支持机器人辅助和计算机引导手术系统，同时解决医疗视频标注耗时费力的问题，促进眼科手术数据集的可扩展发展。

Method: 基于Meta的Segment Anything Model 2进行领域适配，开发CataractSAM-2模型；同时提出结合稀疏提示和基于视频的掩码传播的交互式标注框架，显著减少标注时间。

Result: CataractSAM-2能够高精度地实时分割白内障眼科手术视频，标注工具大幅减少了标注时间，模型在青光眼小梁切除术中表现出强大的零样本泛化能力，证明了跨手术程序的实用性。

Conclusion: CataractSAM-2为扩展眼前节眼科手术数据集和推进医疗机器人实时AI驱动解决方案奠定了基础，模型和标注工具已作为开源资源发布，具有广泛的手术应用潜力。
...
TL;DR: 该论文提出组合隐私风险分类法(CPRT)，将隐私视为组合属性而非二元属性，并开发了可解释的隐私严重性评分系统，同时创建了数据集和8B参数模型来评估和改进视觉语言模型的组合隐私评估能力。
...
Method: 引入组合隐私风险分类法(CPRT)，这是一个基于法规的框架，根据独立可识别性和组合伤害潜力组织视觉属性。CPRT定义了四个分级严重性级别，并配有可解释的评分函数来分配连续隐私严重性分数。构建了包含6.7K图像的分类法对齐数据集，并推导出真实组合风险分数。

Result: 评估前沿和开源视觉语言模型发现：前沿模型在有结构化指导时能很好地对齐组合严重性，但系统性地低估组合驱动风险；较小模型难以内化分级隐私推理。为解决这一差距，引入了可部署的8B监督微调(SFT)模型，在组合隐私评估上接近前沿模型性能。

Conclusion: 隐私评估需要从二元框架转向组合框架。CPRT提供了结构化方法来评估组合隐私风险，开发的8B SFT模型展示了在保持性能的同时实现高效隐私评估的可行性，为实际部署提供了解决方案。
...
TL;DR: 提出一种基于难度感知课程学习的半监督旋转回归方法，通过动态选择伪标签样本和结构化数据增强，在有限标注数据下提升3D旋转估计性能。


<details>
  <summary>Details</summary>
Motivation: 现有旋转回归模型需要大量标注数据或额外信息（如点云、CAD模型），而半监督方法FisherMatch使用固定的熵阈值过滤伪标签，无法有效区分可靠与不可靠的无标签样本，限制了在有限标注数据下的性能。

Method: 1) 提出难度感知课程学习框架，动态选择伪标签样本，从简单到复杂逐步学习；2) 引入多阶段和自适应课程策略，替代固定阈值过滤；3) 设计针对旋转估计的结构化数据增强策略，通过增强图像块组合保持几何完整性同时增加特征多样性。

Result: 在PASCAL3D+和ObjectNet3D数据集上的实验表明，该方法优于现有的监督和半监督基线方法，特别是在低数据量情况下，验证了课程学习框架和结构化增强策略的有效性。

Conclusion: 提出的难度感知课程学习框架和结构化数据增强策略能够有效提升半监督旋转回归性能，为有限标注数据下的3D旋转估计提供了更灵活的解决方案。
...
Method: SARe-Gen联合预测断裂表面token概率和碎片间接触图来定位接触区域并推断候选邻接关系，采用基于查询点的条件方案，从冻结的几何编码器中提取对齐的局部几何token。SARe-Refine通过几何一致性检查验证候选接触边，选择可靠子结构并在保持已验证部分固定的同时重新采样不确定区域。

Result: 在合成断裂、扫描真实物体的模拟断裂和真实物理断裂扫描三种设置下评估，SARe表现出最先进的性能，在具有挑战性的大规模重组中，随着碎片数量增加，性能下降更平缓且成功率更高。

Conclusion: SARe通过显式接触建模和推理时优化，解决了多碎片3D重组中的邻接关系不可靠问题，在碎片数量增加时表现出更稳定和一致的组装能力。
...
Method: 提出两个互补创新：1）渐进注入计划，用连续衰减函数（sigmoid、余弦或线性）替代硬二进制截止，实现从源特征保留到目标特征生成的平滑过渡；2）通道选择性潜在扰动，基于反转和随机潜在之间的分布差距估计每通道重要性，并相应应用差异化扰动强度。

Result: 在PIE-Bench基准测试（700张图像，10种编辑类型）上，AdaEdit相比强基线实现了LPIPS降低8.7%、SSIM提高2.6%、PSNR提高2.3%，同时保持竞争力的CLIP相似度。框架完全即插即用，兼容多种ODE求解器。

Conclusion: AdaEdit通过自适应注入策略有效解决了流匹配模型中基于反转的图像编辑的注入困境，在保持背景的同时更好地合成编辑内容，实现了训练自由的文本引导图像操作。
...
TL;DR: 4DGS360是一个无需扩散的框架，用于从单目视频重建360°动态物体，通过3D原生初始化解决现有方法过度拟合可见表面导致几何不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法严重依赖2D原生先验，导致初始点过度拟合每个训练视图中的可见表面，无法重建一致的360°几何形状。需要解决被遮挡区域的几何模糊性问题。

Method: 提出3D原生初始化方法，通过AnchorTAP3D 3D跟踪器利用置信度高的2D跟踪点作为锚点，生成增强的3D点轨迹，抑制漂移并提供可靠的初始化。结合优化过程实现连贯的360° 4D重建。

Result: 在iPhone360、iPhone和DAVIS数据集上实现了最先进的性能，iPhone360是新提出的基准测试，测试相机与训练视图最多相隔135°，能够提供现有数据集无法实现的360°评估。

Conclusion: 4DGS360通过先进的3D原生初始化有效解决了360°动态物体重建中的几何模糊性问题，在多个数据集上表现出色，为360°重建提供了新的解决方案。
...
TL;DR: 提出一种无需训练的AI生成图像检测方法，通过测量图像表示对结构化频率扰动的敏感性来检测细微操作，计算轻量且速度极快。


<details>
  <summary>Details</summary>
Motivation: 文本到图像模型的快速发展使AI生成图像越来越逼真，给准确检测生成内容带来重大挑战。基于训练的方法泛化能力有限，而无训练方法虽然鲁棒性更好，但难以捕捉真实与合成图像间的细微差异。
...
Conclusion: 该方法提供了一种高效、轻量且无需训练的AI生成图像检测解决方案，在准确性和计算效率方面均优于现有最先进方法。
...
TL;DR: 提出PGR-Net，一种结合数据驱动空间先验的ROI感知脑肿瘤MRI分割框架，通过分层Top-K ROI决策和WinGS-ROI模块提升小病灶分割性能


<details>
  <summary>Details</summary>
Motivation: 脑肿瘤MRI分割中，病灶仅占很小体积空间，存在严重空间稀疏性，现有分割网络常忽略临床观察到的肿瘤发生空间先验，导致在大量背景区域进行冗余特征计算

Method: 提出PGR-Net：1) 数据驱动的空间先验集捕捉肿瘤分布和尺度特征；2) 分层Top-K ROI决策机制逐步选择最置信的病灶候选区域；3) WinGS-ROI模块使用多窗口高斯模板和空间衰减函数生成中心增强的引导图；4) 采用窗口化RetNet骨干网络

Result: 在BraTS-2019/2023和MSD Task01数据集上，仅使用8.64M参数，在全肿瘤区域分别达到89.02%、91.82%和89.67%的Dice分数，优于现有方法

Conclusion: PGR-Net通过显式结合空间先验和ROI感知机制，有效解决了脑肿瘤分割中的空间稀疏性问题，在保持参数效率的同时实现了优异的性能
...
TL;DR: 提出TCEI框架，通过直觉系统和经验系统在测试时校准MOT模型，缓解分布偏移问题


<details>
  <summary>Details</summary>
Motivation: MOT任务在训练和测试数据之间存在外观、运动模式和类别分布偏移，导致在线推理时性能显著下降。现有TTA方法主要关注帧级适应，忽略了跨帧和视频的时间一致性和身份关联。

Method: 提出TCEI框架：直觉系统利用瞬时记忆回忆最近观察到的对象进行快速预测；经验系统利用先前测试视频积累的经验重新评估和校准这些直觉预测。利用在线测试中的置信和不确定对象分别作为历史先验和反思案例。

Result: 在多个基准数据集上一致实现优越性能，显著增强了模型在分布偏移下的适应性。

Conclusion: TCEI框架通过模拟人类决策过程，有效解决了MOT中的分布偏移问题，提高了测试时适应能力。
...
TL;DR: 首个完全稀疏的事件相机目标检测器SparseVoxelDet，仅处理活跃体素，相比密集方法实现858倍内存压缩和3670倍存储减少，在FRED基准上达到83.38% mAP@50。


<details>
  <summary>Details</summary>
Motivation: 现有事件相机检测器将稀疏事件流转换为密集张量，丢弃了神经形态传感的表示效率。需要开发能充分利用事件数据稀疏性的原生稀疏处理方法。

Method: 提出SparseVoxelDet，首个完全稀疏的事件相机目标检测器。使用3D稀疏卷积，仅在占用的体素位置进行特征提取、特征金字塔融合和检测头操作，整个流程不实例化任何密集特征张量。

Result: 在FRED基准(629,832标注帧)上达到83.38% mAP@50，每帧仅处理14,900个活跃体素(占T×H×W网格的0.23%)。相比密集YOLOv11基线(87.68% mAP@50)，实现858倍GPU内存压缩和3,670倍存储减少。将IoU阈值从0.50放宽到0.40时，mAP恢复至89.26%。

Conclusion: 原生稀疏处理是事件相机目标检测的可行范式，能充分利用神经形态传感器数据的结构稀疏性，无需神经形态计算硬件。该框架的表示成本由场景活动而非像素数量决定，这对高分辨率事件相机尤为重要。
...
TL;DR: FedCVU是一个联邦学习框架，专门解决多摄像头视频理解中的跨视图挑战，通过VS-Norm处理视图特定统计、CV-Align对齐跨视图表示、SLA减少通信开销。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在多摄像头视频理解中存在三个主要挑战：1) 异构视角和背景导致非独立同分布数据分布和过拟合；2) 局部分布偏差导致表示不对齐；3) 大型视频架构带来高昂通信开销。

Method: 提出FedCVU框架，包含三个组件：VS-Norm保留归一化参数处理视图特定统计；CV-Align是轻量级对比正则化模块，改善跨视图表示对齐；SLA是选择性层聚合策略，减少通信同时保持准确性。

Result: 在动作理解和行人重识别任务的跨视图协议下，FedCVU在未见视图上显著提升准确性，同时保持强健的已见视图性能，优于现有联邦学习基线方法，对领域异构性和通信约束具有鲁棒性。

Conclusion: FedCVU有效解决了联邦学习中跨视图视频理解的挑战，通过处理视图特定统计、对齐跨视图表示和优化通信效率，实现了更好的跨摄像头协同学习效果。
...
TL;DR: OmniFM是一个模态和任务无关的联邦学习框架，通过频域分析统一处理多种医学图像任务，解决了现有框架对特定模态和任务的依赖问题。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习框架紧密耦合于特定任务架构，对异构成像模态脆弱，限制了在现实医疗部署中的应用，因为不同机构的模态分布差异大且需要支持多种下游任务。

Method: 基于频域洞察：低频频谱成分具有跨模态一致性，编码模态不变解剖结构。OmniFM整合：1) 全局频谱知识检索注入全局频率先验；2) 嵌入级交叉注意力融合对齐表示；3) 前缀-后缀频谱提示联合调节全局和个性化线索；通过频谱-邻近对齐目标正则化稳定聚合。

Result: 在真实世界数据集上的实验表明，OmniFM在模态内和跨模态异质性下持续超越最先进的联邦学习基线，在微调和从头训练设置下均取得优越结果。

Conclusion: OmniFM提供了一个统一的联邦学习框架，能够处理多种医学图像分析任务和异构成像模态，为现实世界医疗部署提供了更灵活和鲁棒的解决方案。
...
TL;DR: 提出跨场景去雨自适应框架，无需目标域配对雨天图像，仅用无雨背景图，通过超像素生成、分辨率自适应融合和伪标签重合成机制，提升OOD泛化能力


<details>
  <summary>Details</summary>
Motivation: 当前深度学习方法在分布外场景泛化能力差，主要原因是合成训练数据与真实世界雨物理动态存在显著域差异，需要解决跨场景去雨的自适应问题

Method: 1) 超像素生成模块从源域提取稳定结构先验；2) 分辨率自适应融合策略通过纹理相似性对齐源结构与目标背景；3) 伪标签重合成机制使用多阶段噪声生成模拟真实雨纹

Result: 在OOD域获得32%到59%的PSNR显著提升，同时显著加速训练收敛，可作为即插即用模块集成到任意去雨架构

Conclusion: 提出的跨场景去雨自适应框架有效解决了分布外泛化问题，无需目标域配对数据，通过结构先验提取和伪数据合成实现了优异的性能提升
...
TL;DR: 论文提出VR-SDR框架和HumanOmni-Speaker基准，解决多模态大模型在复杂多人对话场景中"谁在何时说了什么"的识别难题，通过视觉Delta编码器捕获高频唇动和说话人轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有全模态大语言模型存在"能力幻觉"，依赖视觉偏见绕过真正的跨模态对齐，且采用低帧率采样破坏了唇动等高频动态信息，无法准确识别复杂多人对话中的说话人身份和时间信息。

Method: 提出视觉注册说话人分离与识别（VR-SDR）范式，消除视觉捷径；开发HumanOmni-Speaker模型，采用视觉Delta编码器，以25fps采样原始视频，将帧间运动残差压缩为每帧仅6个token，捕获细粒度唇动和说话人轨迹。

Result: HumanOmni-Speaker展现出强大的多模态协同能力，原生支持端到端唇读和高精度空间定位，无需侵入式裁剪，在广泛的说话人中心任务上实现卓越性能。

Conclusion: 通过VR-SDR框架和视觉Delta编码器，成功解决了全模态大模型在复杂多人对话场景中的身份绑定问题，实现了真正的端到端时空身份识别，为多模态对话理解提供了新范式。
...
TL;DR: RefracGS：通过神经高度场建模折射水面和3D高斯场表示水下场景，实现非平面折射表面的高质量新视角合成


<details>
  <summary>Details</summary>
Motivation: 传统NeRF和3DGS等方法假设光线直线传播，无法处理非平面折射表面（如水波）引起的严重光学畸变，导致合成质量下降

Method: 1) 将折射边界与目标物体解耦：折射表面用神经高度场建模，水下场景用3D高斯场表示；2) 提出折射感知的高斯光线追踪方法，基于斯涅尔定律计算非线性光线轨迹；3) 端到端联合优化两个表示

Result: 在合成和真实世界复杂水波场景中，RefracGS在视觉质量上优于现有折射方法，训练速度提升15倍，实时渲染达到200 FPS

Conclusion: RefracGS通过解耦折射表面和水下场景表示，结合折射感知的高斯光线追踪，成功解决了非平面折射表面的新视角合成问题，实现了高质量、高效率的渲染
...
<details>
  <summary>Details</summary>
Motivation: PPGLs是罕见神经内分泌肿瘤，15-25%会发生转移，5年生存率低至34%。现有GAPP评分系统存在三大局限：1) 临床医生工作量大，需要手动评估六个独立组件；2) 细胞密度和Ki-67等关键指标评估主观性强；3) 未纳入SDHB突变等重要的转移风险因素。现有智能诊断系统缺乏可追溯的推理过程，且未整合基因型信息。

Method: 提出PPGL-Swarm多智能体诊断系统：1) 将诊断分解为微任务，分配给专门智能体；2) 基因和表格智能体使用知识增强技术解释基因型和实验室结果；3) 训练中使用强化学习优化工具选择和任务分配；4) 生成综合报告，包括自动GAPP评分（量化细胞密度和Ki-67）、基因风险预警和多模态证据整合。

Result: 系统能够：1) 自动完成GAPP评分，减少临床医生工作量；2) 提供客观量化的细胞密度和Ki-67评估；3) 整合SDHB突变等基因风险因素；4) 生成包含综合证据的多模态报告；5) 提供可审计的推理轨迹。

Conclusion: PPGL-Swarm通过多智能体架构解决了现有PPGL诊断系统的局限性，提供了自动化、客观化、全面化的诊断解决方案，特别在整合基因风险因素和提供可追溯推理方面具有优势，有望改善临床实践。
...
TL;DR: MetaCompress：一种基于学习的提示无关视觉令牌压缩方法，针对多轮视觉问答场景，通过可学习的压缩映射统一剪枝和合并，在有限计算成本下实现最优效率-精度平衡。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在视觉理解方面表现出色，但过多的视觉令牌导致高推理成本。现有令牌缩减方法主要针对单轮视觉问答，而更实用的多轮视觉问答场景面临额外挑战：后续问题未知且可能涉及任意图像区域，现有方法效果不佳。

Method: 提出基于学习的提示无关方法MetaCompress，将令牌缩减形式化为可学习的压缩映射，统一剪枝和合并等现有格式为单一学习目标。引入数据高效的训练范式，能够在有限计算成本下学习最优压缩映射。

Result: 在多轮视觉问答基准测试和多个LVLM架构上的广泛实验表明，MetaCompress实现了优越的效率-精度权衡，同时在对话轮次间保持强大的泛化能力。

Conclusion: MetaCompress克服了启发式设计的局限性，为多轮视觉问答场景提供了一种高效且通用的视觉令牌压缩解决方案，在保持模型性能的同时显著降低推理成本。
...
TL;DR: 研究指出，在大型视觉语言模型中引入指向机制（预测物体坐标再计数）能提升零样本计数任务的泛化能力，坐标的空间信息是关键，但存在空间偏差。


<details>
  <summary>Details</summary>
Motivation: 尽管指向机制已被证明能提高LVLMs的准确性，但其具体作用机制和在认知任务中的相关性尚不清楚。同时，中间点预测的可靠性研究不足，限制了其作为视觉解释的用途。

Method: 研究采用两种方法微调最先进的LVLMs：1）直接计数（仅预测物体总数）；2）先指向再计数（先生成目标物体的坐标，再基于这些坐标进行计数）。在零样本视觉场景计数任务中评估这两种方法。

Result: 先指向再计数方法在分布外泛化方面表现更好，表明坐标帮助模型学习技能而非过拟合特定任务。预测点的F1分数超过89%，但性能在不同图像区域存在差异，揭示了空间偏差。机制分析表明计数性能提升源于坐标编码的空间信息。

Conclusion: 指向机制通过提供空间信息增强了LVLMs在计数任务中的泛化能力，但模型存在空间偏差，预测点作为视觉解释的可靠性需要进一步评估。
...
TL;DR: DaP-ICoT提出动态精确视觉思维的交叉模态思维链推理，解决现有ICoT方法中视觉信息静态插入和视觉表示不连贯的问题，显著提升推理效率和性能。


<details>
  <summary>Details</summary>
Motivation: 当前交叉模态思维链（ICoT）方法存在两大局限：1）静态视觉思维定位，在固定步骤静态插入视觉信息，导致推理效率低下且不灵活；2）破碎的视觉思维表示，视觉标记不连续且语义不连贯。需要更高效、灵活的视觉推理方法。

Method: 提出DaP-ICoT方法，包含两个关键组件：1）动态视觉思维集成，根据推理需求自适应引入视觉输入，减少冗余提高效率；2）精确视觉思维引导，确保视觉表示语义连贯且上下文对齐。

Result: 在多个基准测试和模型上，DaP-ICoT实现了最先进的性能。同时显著减少了插入图像数量，token消耗降低72.6%，实现了更高效的ICoT推理。
...
TL;DR: 本文提出SHARP方法，通过动态调整RoPE位置编码的缩放策略，结合专门针对遥感图像训练的RS-FLUX模型，显著提升遥感图像生成的超分辨率效果。


<details>
  <summary>Details</summary>
Motivation: 当前遥感图像生成落后于通用文本到图像生成，主要存在两个障碍：缺乏领域专用的DiT先验模型，以及在遥感应用所需的高分辨率下训练成本过高。现有训练免费的超分辨率方法在整个去噪过程中采用静态位置缩放规则，这对遥感图像特别有害，因为遥感图像的中高频能量更密集，编码了关键的精细结构信息。

Method: 1) 在超过10万张精选遥感图像上微调FLUX模型，构建领域专用先验模型RS-FLUX；2) 提出SHARP方法，将有理分数时间调度k_rs(t)引入RoPE位置编码，在早期布局形成阶段应用强位置提升，在细节恢复阶段逐步放松，使外推强度与扩散去噪的频率渐进特性对齐。

Result: 在六个正方形和矩形分辨率上的广泛实验表明，SHARP在CLIP Score、Aesthetic Score和HPSv2指标上始终优于所有训练免费的基线方法，在更激进的外推因子下优势更明显，且计算开销可忽略不计。

Conclusion: SHARP方法通过动态调整位置编码缩放策略，结合领域专用先验模型，有效解决了遥感图像生成中的分辨率提升问题，实现了高质量的多尺度生成，且无需额外训练成本。
...
TL;DR: DEBIR提出了一种动态预测曝光时间的突发图像恢复方法，通过BAENet根据预览图像、运动幅度和增益估计最佳曝光时间，然后使用恢复网络重建高质量图像。


<details>
  <summary>Details</summary>
Motivation: 突发图像恢复通常使用手动设计的曝光设置，但这些设置显著影响最终恢复性能，而寻找最佳曝光设置的问题一直被忽视。

Method: 提出DEBIR管道：1) BAENet根据预览图像、运动幅度和增益估计每个突发图像的最佳曝光时间；2) 使用这些最佳曝光时间捕获的突发图像通过恢复网络重建高质量图像；3) 引入可微分突发模拟器和三阶段训练策略。

Result: 实验表明该管道实现了最先进的恢复质量，并在真实相机系统上验证了其有效性，证明了实用性。

Conclusion: DEBIR通过动态预测曝光时间显著提升了突发图像恢复质量，为实际相机系统提供了实用的解决方案。
...
TL;DR: 提出首个基于图像条件强化学习的VO前端参数在线调优框架，通过轻量级CNN编码器直接映射视觉输入到特征检测和跟踪参数，实现3倍更长的特征跟踪和3倍更低的计算成本


<details>
  <summary>Details</summary>
Motivation: 资源受限的自主机器人依赖稀疏直接/半直接视觉里程计，但其性能严重依赖手工调优的超参数。这些参数在部署时通常固定，而最优值随场景特性（纹理密度、光照、运动模糊、传感器噪声）变化，导致实际环境中性能脆弱

Method: 将前端配置建模为序列决策问题，学习直接映射视觉输入到特征检测和跟踪参数的策略。使用轻量级纹理感知CNN编码器，训练时采用特权批评器。与仅依赖内部VO统计的先前RL方法不同，本方法观察图像内容并在跟踪退化前主动调整参数

Result: 在TartanAirV2和TUM RGB-D数据集上实验，实现了3倍更长的特征跟踪和3倍更低的计算成本，尽管完全在仿真中训练

Conclusion: 提出首个图像条件强化学习框架用于VO前端参数在线调优，有效将专家知识嵌入系统，通过观察图像内容主动适应场景变化，显著提升特征跟踪长度并降低计算成本
...
TL;DR: 论文提出"通用正态嵌入"假设，认为生成模型和视觉编码器共享一个高斯潜在空间，并通过NoiseZoo数据集验证了DDIM反向噪声和编码器表示在语义上的线性对齐关系。


<details>
  <summary>Details</summary>
Motivation: 生成模型和视觉编码器虽然在目标和数学原理上不同，但都表现出潜在空间的高斯特性。作者假设它们共享一个共同的潜在源——通用正态嵌入，并希望验证这一假设。

Method: 提出NoiseZoo数据集，包含每张图像的DDIM反向扩散噪声和对应的编码器表示（CLIP、DINO）。在CelebA数据集上，通过线性探针分析两个空间的语义对齐，并利用正交化方法减少虚假纠缠。

Result: 在两个空间中，线性探针都能产生强且对齐的属性预测，表明生成噪声沿线性方向编码有意义的语义。这些方向能够实现忠实、可控的图像编辑（如微笑、性别、年龄），无需架构修改。

Conclusion: 结果支持UNE假设，揭示了编码和生成之间共享的高斯潜在几何结构，为理解生成模型和视觉编码器的内在联系提供了实证依据。
...
TL;DR: 在工业机器人场景中，基于事件相机的多类别物体检测研究较少。本文在MTEvent数据集上对循环ReYOLOv8s进行基准测试，分析时间记忆效果，发现GEN1预训练效果最佳，达到0.329 mAP50。


<details>
  <summary>Details</summary>
Motivation: 事件相机具有高时间分辨率、高动态范围和低运动模糊等优点，适合工业机器人应用。然而，现有事件相机物体检测研究主要集中在户外驾驶场景或有限类别设置，缺乏针对工业环境多类别识别的研究。

Method: 在MTEvent数据集上对循环ReYOLOv8s进行基准测试，使用非循环YOLOv8s变体作为基线分析时间记忆效果。比较了从零开始训练、GEN1预训练和PEDRo预训练三种初始化策略。

Result: 最佳从零开始训练的循环模型达到0.285 mAP50，比非循环基线(0.260)相对提升9.6%。GEN1预训练微调效果最好，在clip length 21时达到0.329 mAP50，且性能随clip长度持续提升。PEDRo预训练效果较差(0.251)，甚至不如从零开始训练。主要失败模式包括类别不平衡和人-物交互。

Conclusion: 本文是对工业环境中循环事件相机检测的专注基准测试和分析研究。事件域预训练(GEN1)效果显著，而源域不匹配的预训练可能不如从零开始训练。循环结构在工业多类别识别中具有优势。
...
TL;DR: TIC-TALK是一个多模态数据集，包含90个专业单口喜剧特别节目的5400+时间对齐主题片段，整合了语言、手势和观众反应数据，用于研究喜剧表演动态。


<details>
  <summary>Details</summary>
Motivation: 传统喜剧研究主要关注语言内容，但现场表演同样依赖身体表现和观众反馈。需要多模态资源来全面分析喜剧表演的动态特征。

Method: 结合BERTopic进行60秒主题分割，Whisper-AT检测笑声，YOLOv8-cls分类镜头，YOLOv8s-pose提取骨骼关键点。保留原始17关节坐标，计算连续运动学信号（手臂展开、动能、躯干倾斜）。

Result: 动能与观众笑声率呈负相关(r=-0.75)，支持"笑点前的静止"模式；个人和身体内容比地缘政治主题引发更多笑声；特写镜头比例与笑声正相关(r=+0.28)。

Conclusion: TIC-TALK数据集为喜剧表演的多模态分析提供了新资源，揭示了身体动态、主题内容和镜头选择如何共同影响观众反应，为喜剧研究和表演分析开辟了新方向。
...
TL;DR: 提出基于预训练医学图像分词器和潜在变换器的主动采样框架，利用量化视觉标记的分布推导不确定性度量，通过标记熵指导主动采样，在MRI加速重建中超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统MRI数据采集速度慢，影响临床效率和患者体验。压缩感知MRI（CS-MRI）通过欠采样k空间数据重建图像来加速，但需要优化采样轨迹和高质量重建模型。

Method: 利用预训练医学图像分词器和潜在变换器，将解剖结构表示为量化视觉标记字典，获得潜在空间概率分布。基于标记熵推导不确定性度量，提出两种主动采样策略：1）潜在熵选择（LES），将补丁级标记熵投影到k空间域识别信息丰富的采样线；2）基于梯度的熵优化（GEO），通过总潜在熵损失的k空间梯度识别最大不确定性减少区域。

Result: 在fastMRI单线圈膝关节和脑部数据集上，以×8和×16加速进行评估。主动采样策略在感知度量和基于特征的距离方面优于最先进的基线方法。

Conclusion: 提出的主动采样框架通过利用预训练医学图像分词器的离散结构，实现了有效的k空间采样轨迹优化，显著提升了MRI加速重建的性能。
...
TL;DR: 提出一种基于多任务学习的无级联架构，联合整合音素和视位等中间表示，通过语义引导的局部对比损失实现特征对齐，在中文普通话视觉语音识别中取得优越性能。


<details>
  <summary>Details</summary>
Motivation: 中文普通话视觉语音识别(VSR)由于声调特性，传统序列到序列建模方法效果有限。现有中文VSR系统常采用级联架构引入拼音等中间表示，但这会导致错误累积和推理延迟增加。

Method: 提出基于多任务学习的无级联架构，联合整合音素和视位等多种中间表示。设计了语义引导的局部对比损失，在时间上对齐特征，实现推理时的按需激活。

Result: 在公开数据集上的实验表明，该方法实现了优越的识别性能，在推理效率和性能之间取得平衡，同时减轻了投影和重嵌入导致的错误累积。

Conclusion: 提出的无级联多任务学习架构有效解决了中文VSR中的错误累积和延迟问题，通过联合利用多种中间表示提升了识别性能，为声调语言的视觉语音识别提供了新思路。
...
TL;DR: 提出CGMD框架，通过临床相似性图在未配对的MRI和眼底图像数据集间传递高血压知识，提升眼底图像的高血压预测性能。


<details>
  <summary>Details</summary>
Motivation: 眼底图像成本低但高血压相关线索微弱，预测方差大；MRI提供更强的血管和小血管疾病标记但昂贵，两种模态数据集通常不配对，形成模态孤岛问题。

Method: CGMD框架：1) 利用共享结构化生物标记构建临床相似性kNN图连接两个队列；2) 训练MRI教师模型；3) 在图上传导教师表示，为眼底患者推算脑部信息表示目标；4) 训练眼底学生模型，结合高血压监督、目标蒸馏和关系蒸馏的联合目标。

Result: 在新收集的未配对MRI-眼底-生物标记数据集上，CGMD相比标准蒸馏和非图推算基线，持续改善基于眼底图像的高血压预测性能，消融实验确认临床基础图连接的重要性。

Conclusion: CGMD框架成功解决了未配对多模态数据下的知识迁移问题，通过临床相似性图桥接不同模态，有效提升了低成本眼底图像的高血压筛查性能。
...
TL;DR: ControlAugment (Ctrl-A) 是一种基于控制理论的自动数据增强算法，通过在线调整增强强度分布来优化图像视觉任务训练，无需手动设计增强策略。


<details>
  <summary>Details</summary>
Motivation: 传统数据增强方法需要手动设计增强策略和初始化增强强度，这在新任务中需要大量工程工作。作者希望开发一种能够自动适应训练过程、动态调整增强强度的算法。

Method: Ctrl-A 采用控制循环架构，基于相对操作响应曲线动态调整各个增强操作的强度分布。算法通过操作依赖的更新过程，能够抑制对模型性能有负面影响的增强风格。

Result: 在 CIFAR-10、CIFAR-100 和 SVHN-core 基准数据集上使用 WideResNet-28-10 架构的实验表明，Ctrl-A 与现有最先进的数据增强策略相比具有高度竞争力。

Conclusion: ControlAugment 提供了一种自动化的数据增强方法，通过控制理论原理实现增强强度的在线调整，减少了手动工程需求，在多个基准数据集上表现优异。
...
TL;DR: 该论文挑战红外与可见光图像融合(IVIF)中严格配对训练范式(SPTP)的必要性，提出非配对(UPTP)和任意配对(APTP)训练范式，显著降低数据收集成本并提升模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有IVIF方法依赖大量严格对齐的图像对进行训练，但数据对齐过程成本高、劳动密集。严格的配对设置限制了跨模态关系的丰富性，影响模型泛化性能。需要探索更灵活的训练范式来降低数据收集难度。

Method: 提出非配对(UPTP)和任意配对(APTP)训练范式，建立APTP的理论目标。开发了一个实用框架，即使在严重有限且未对齐的训练数据下也能显著丰富跨模态关系。设计了三个轻量级基线模型(CNN、Transformer、GAN)和创新的损失函数。

Result: 实验表明，APTP和UPTP在严重有限且内容不一致的红外与可见光数据集上训练模型，能达到与SPTP下100倍更大数据集相当的性能。这从根本上降低了数据收集成本，从数据角度增强了模型鲁棒性。

Conclusion: 该研究挑战了IVIF中严格配对训练范式的必要性，提出的非配对和任意配对训练范式为IVIF研究提供了可行的解决方案，显著降低了数据收集成本，同时提升了模型性能。
...
TL;DR: SteelDefectX是一个包含7,778张图像和25个缺陷类别的视觉-语言数据集，提供从粗到细的文本描述，旨在提升钢材表面缺陷检测的可解释性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前钢材表面缺陷检测方法主要依赖基于标签的图像分类模型，缺乏可解释性且泛化能力有限。需要更丰富的视觉-语言数据集来支持可解释、可泛化的缺陷检测研究。

Method: 构建SteelDefectX数据集，包含7,778张图像覆盖25个缺陷类别，提供两个层次的文本标注：粗粒度（类别信息、视觉属性、工业原因）和细粒度（形状、大小、深度、位置、对比度等样本特定属性）。建立包含四个任务的基准测试：视觉分类、视觉-语言分类、少样本/零样本识别、零样本迁移。

Result: 实验表明，从粗到细的文本标注显著提高了模型的可解释性、泛化能力和迁移能力。数据集为可解释、可泛化的钢材表面缺陷检测研究提供了有价值的资源。

Conclusion: SteelDefectX数据集通过丰富的视觉-语言标注，解决了当前钢材缺陷检测方法的局限性，为开发更可解释、更泛化的缺陷检测模型奠定了基础。数据集将公开可用。
...
TL;DR: 提出MDSVM-UNet框架，结合多方向蛇形卷积和残差视觉Mamba，用于CTA图像中冠状动脉分割，解决传统方法在长距离依赖建模和计算效率方面的限制。


<details>
  <summary>Details</summary>
Motivation: 冠状动脉CTA图像分割对心血管疾病诊断至关重要，但面临血管多分支、细长管状形态以及前景-背景类别不平衡的挑战。传统CNN方法难以捕捉远距离血管结构依赖，而ViT方法计算开销过大，不适合临床资源受限环境。

Method: 提出两阶段分割框架MDSVM-UNet：编码阶段使用MDSConv（多方向蛇形卷积），沿三个正交解剖平面学习自适应偏移，实现多视角特征融合；解码阶段使用RVM（残差视觉Mamba）上采样模块，利用选择性状态空间机制建模切片间长距离依赖，保持线性计算复杂度。采用渐进两阶段分割策略：第一阶段进行粗粒度全图像分割指导智能块提取，第二阶段进行细粒度块级分割恢复血管细节并抑制假阳性。

Result: 论文声称该方法能够有效捕捉冠状动脉的细长曲折几何特征，建模长距离依赖关系，同时保持线性计算复杂度，适合临床部署。

Conclusion: MDSVM-UNet通过结合MDSConv和RVM，解决了冠状动脉分割中的几何建模和长距离依赖问题，计算效率高，有望在临床资源受限环境中应用。
...
TL;DR: 提出基于视频扩散模型的MJO生成方法，通过低维条件控制生成可解释的MJO序列，弥合理论与观测之间的差距


<details>
  <summary>Details</summary>
Motivation: 生成式深度学习在模拟热带MJO方面很强大，但其与传统理论框架的关系仍不清楚，需要建立低维理论与高分辨率大气复杂性之间的桥梁

Method: 使用视频扩散模型，基于大气再分析数据训练，通过关键低维指标条件化生成长序列MJO，并设计理想化条件生成可解释的MJO变体

Result: 生成的MJO能捕捉关键特征（合成图、功率谱、多尺度结构等），尽管存在一些偏差；通过理想化条件生成可追踪的MJO变体，有助于解构物理过程

Conclusion: 该方法为弥合低维MJO理论与高分辨率大气复杂性提供了实用框架，有助于热带大气预测
...
<details>
  <summary>Details</summary>
Motivation: 视频生成在生成式AI中日益重要，但视频合成计算成本高昂，需要模型蒸馏来实现高效部署。目前缺乏专门针对视频扩散模型的蒸馏方法，现有方法通常直接采用图像蒸馏技术，导致过饱和、时间不一致和模式崩溃等伪影问题。

Method: 提出专门针对视频扩散模型的蒸馏框架，包含三个核心创新：1) 自适应回归损失，动态调整空间监督权重以防止分布偏移过大导致的伪影；2) 时间正则化损失，对抗时间崩溃，促进平滑且物理合理的采样轨迹；3) 推理时帧插值策略，在保持感知质量的同时减少采样开销。

Result: 在VBench和VBench2基准测试上的广泛实验和消融研究表明，该方法实现了稳定的少步视频合成，显著提高了感知保真度和运动真实感，在多个指标上持续优于现有蒸馏基线方法。

Conclusion: 提出的专门针对视频扩散模型的蒸馏框架有效解决了现有图像蒸馏方法直接应用于视频时的问题，通过自适应回归损失、时间正则化损失和推理时帧插值策略，实现了高效且高质量的少步视频合成。
...
TL;DR: 提出对抗性伪装方法，通过在面部区域添加优化图案来保护隐私，能有效降低多种人脸识别模型的性能
...
Result: 显著降低所有测试的先进人脸识别模型性能，在黑盒系统中具有高跨模型可迁移性，真实世界实验显示有前景的结果

Conclusion: 对抗性伪装是高效、易于复制的物理世界隐私保护方案，揭示了模型鲁棒性差异和攻击跨架构可迁移性
...
TL;DR: SAGE-GRPO提出了一种稳定的视频生成对齐方法，通过约束探索在预训练模型定义的数据流形附近，解决了现有GRPO方法因ODE-to-SDE转换引入过多噪声导致的不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 视频生成的GRPO方法（如FlowGRPO）相比语言模型和图像的对应方法可靠性较差。这是因为视频生成具有复杂的解空间，且ODE-to-SDE转换会注入过多噪声，降低生成质量并使奖励估计不可靠，从而破坏后训练对齐的稳定性。

Method: SAGE-GRPO在微观和宏观两个层面施加约束：微观层面推导精确的流形感知SDE，引入对数曲率校正和梯度范数均衡器；宏观层面使用双信任区域，包含周期性移动锚点和逐步约束，以限制长期漂移。

Result: 在HunyuanVideo1.5上使用VideoAlign作为奖励模型进行评估，SAGE-GRPO在VQ、MQ、TA和视觉指标（CLIPScore、PickScore）上均优于先前方法，在奖励最大化和整体视频质量方面表现出色。

Conclusion: 通过约束探索在预训练模型定义的数据流形附近，SAGE-GRPO能够稳定视频生成的对齐过程，提高生成质量和奖励估计的可靠性，为视频生成的后训练对齐提供了有效的解决方案。
...
TL;DR: 本文提出了首个红外领域的通用物理对抗攻击方法UPPA，使用参数化贝塞尔块建模扰动，通过粒子群优化实现全局优化，并将数字扰动转化为物理冷贴片，实现了高效、鲁棒的物理攻击。
...
Conclusion: UPPA是红外领域首个通用物理攻击方法，通过参数化建模和全局优化解决了现有方法的部署成本高和物理鲁棒性不足的问题，为红外视觉系统的安全性研究提供了新方向。
...
TL;DR: 将现代学习型立体匹配方法集成到卫星立体处理流程中，改进数字表面模型生成质量，但植被等复杂表面仍存在挑战


<details>
  <summary>Details</summary>
Motivation: 尽管学习型立体匹配方法在标准基准测试中表现优异，但由于视角几何和视差假设的差异，将其集成到实际卫星处理流程中仍面临挑战。本研究旨在将现代学习型方法整合到卫星立体处理流程中，提升数字表面模型生成质量。

Method: 将StereoAnywhere、MonSter、Foundation Stereo等现代学习型立体匹配方法集成到卫星立体处理流程(S2P)中，通过调整校正阶段来确保视差极性和范围的兼容性。同时发布了相应代码以实现可重复使用。

Result: 在卫星影像实验中，相比传统的基于代价体积的方法，在数字表面模型精度方面取得了一致性改进。定性结果显示几何细节更丰富、结构更清晰，但常用的平均绝对误差等指标存在饱和效应。所有评估模型在植被等复杂表面类型上的性能仍然有限。

Conclusion: 学习型立体匹配方法能够显著提升卫星数字表面模型的质量，但需要更好的评估策略来反映感知和结构保真度。植被等自然环境的复杂表面仍然是所有评估模型的开放挑战。
...
<details>
  <summary>Details</summary>
Motivation: 当前LoRA微调策略中，秩的选择通常基于社区共识而非个性化主题的复杂性，导致性能与内存消耗的权衡不佳。由于为每个LoRA组件选择合适秩的成本是组合性的，实践中通常为所有组件固定相同秩。

Method: 受变分方法启发，提出LoRA²方法，让每层的秩在微调过程中自由适应。通过对秩位置施加重要性排序，鼓励仅在严格需要时创建更高秩。

Result: 在29个主题上，LoRA²在DINO、CLIP-I和CLIP-T指标上实现了有竞争力的权衡，同时比高秩LoRA版本需要更少的内存和更低的秩。

Conclusion: LoRA²是首个克服LoRA秩选择挑战的方法，通过自适应秩调整实现了更好的性能-内存权衡，为个性化图像生成提供了更高效的微调策略。
...
TL;DR: CLEAR是一个无需掩码的视频字幕去除框架，通过上下文感知自适应学习实现端到端推理，在中文基准上优于掩码依赖方法，并具有跨语言的零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的方法在训练和推理阶段都需要显式的掩码序列，这限制了实际部署。需要一种无需掩码的端到端框架来解决这个问题。

Method: 采用两阶段设计：第一阶段通过自监督正交约束在双编码器上学习解耦的字幕表示；第二阶段使用基于LoRA的自适应和生成反馈进行动态上下文调整。仅需基础扩散模型0.77%的参数进行训练。

Result: 在中文字幕基准上，CLEAR比掩码依赖基线方法在PSNR上提升+6.77dB，VFID降低-74.7%。在六种语言（英语、韩语、法语、日语、俄语、德语）上表现出优异的零样本泛化能力。

Conclusion: CLEAR通过上下文感知自适应学习实现了无需掩码的端到端视频字幕去除，在性能和泛化能力上优于现有方法，具有实际部署的可行性。
...
TL;DR: SHAPE是一个用于医学图像分割的无监督域自适应框架，通过分层特征调制和超图合理性评估解决现有方法在全局解剖结构合理性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有无监督域自适应方法存在两个根本限制：1）语义不敏感的特征对齐导致分布保真度差；2）伪标签验证忽略全局解剖约束，无法防止形成全局不合理结构。需要解决医学分割模型在不同临床环境部署时的适应问题。

Method: 基于DINOv3构建，包含：1）分层特征调制模块生成高保真和类别感知的特征；2）超图合理性评估利用超图评估标准图无法捕获的全局解剖合理性；3）结构异常修剪通过跨视图稳定性清除剩余伪影。

Result: 在心脏和腹部跨模态基准测试中显著优于先前方法，达到最先进的平均Dice分数：心脏数据90.08%（MRI->CT）和78.51%（CT->MRI），腹部数据87.48%（MRI->CT）和86.89%（CT->MRI）。

Conclusion: SHAPE通过将自适应重新定义为全局解剖合理性，解决了现有UDA方法的根本限制，在医学图像分割的跨域适应中取得了最先进的性能。
...
TL;DR: 提出基于潜在表示的HSI生成框架，支持光谱级和空间-光谱级仿真，在重建精度和光谱保真度上优于传统方法


<details>
  <summary>Details</summary>
Motivation: 传统辐射传输模型计算成本高且通常只能生成光谱级输出，需要一种更高效、更灵活的合成高光谱图像生成方法

Method: 基于潜在表示的生成框架，学习高光谱数据的潜在生成表示，支持单步直接训练或VAE预训练与参数-潜在插值的两步策略

Result: 在PROSAIL模拟植被数据和Sentinel-3 OLCI影像上，方法在重建精度、光谱保真度和对真实空间变异性的鲁棒性方面优于经典回归仿真器

Conclusion: 仿真HSI在下游生物物理参数反演中保持性能，证明了仿真数据在遥感应用中的实际价值
...
TL;DR: GOLD方法通过识别和利用"黄金子空间"实现高效持续测试时适应，仅更新少量参数就能达到良好适应效果，在效率和泛化间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 现有持续测试时适应方法面临效率与泛化的权衡：更新更多参数能改善适应效果但严重降低在线推理效率。需要找到既能有效适应又只需最小特征更新的解决方案。

Method: 提出GOLD方法：1）证明黄金子空间存在且与预训练分类器的行空间重合；2）使用样本级平均梯度外积作为高效代理来估计分类器权重；3）通过轻量适配器将特征投影到黄金子空间，学习紧凑缩放向量，子空间通过AGOP动态更新。

Result: 在分类和分割基准测试（包括自动驾驶场景）中，GOLD在效率、稳定性和整体性能方面均表现优异，实现了高效适应。

Conclusion: 通过识别黄金子空间并利用AGOP进行高效维护，GOLD方法成功解决了持续测试时适应中的效率-泛化权衡问题，实现了既高效又有效的在线适应。
...
TL;DR: SatGeo-NeRF通过三种模型无关的正则化方法改进卫星图像的NeRF重建，减少几何伪影，在DFC2019基准上比现有方法提升11-14%的精度。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的NeRF模型在处理卫星图像时存在过拟合导致的几何伪影问题，需要改进几何正则化方法来提高重建精度。
...
TL;DR: 提出一种相机无关的3D高斯点云一次性剪枝方法，仅依赖属性导出的邻域描述符，通过Beta证据模型量化每个点的可靠性，实现高效剪枝同时保持重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯点云剪枝方法大多依赖相机参数、渲染图像或视点相关度量，这在相机无关的交换场景中成为障碍，因为点云直接作为点基表示（如.ply文件）共享。

Method: 提出混合描述符框架，直接从点云表示捕获结构和外观一致性；将剪枝建模为统计证据估计问题，引入Beta证据模型，通过概率置信分数量化每个点的可靠性。

Result: 在ISO/IEC MPEG通用测试条件定义的标准化测试序列上实验表明，该方法在保持重建质量的同时实现了显著的剪枝效果，为现有相机依赖剪枝策略提供了实用且可泛化的替代方案。

Conclusion: 该方法为3D高斯点云提供了一种相机无关、一次性、训练后剪枝的有效解决方案，特别适用于新兴的相机无关交换场景，具有实际应用价值和泛化能力。
...
TL;DR: ChronoCon：一种利用患者纵向扫描时间顺序进行对比学习的方法，无需专家标注即可学习疾病相关表征，在不可逆疾病领域显著减少标注需求


<details>
  <summary>Details</summary>
Motivation: 医学影像中的定量疾病严重程度评分成本高、耗时长且存在读者间差异，而临床档案包含大量纵向影像数据但缺乏专家标注。现有自监督方法通常忽略时间顺序信息。

Method: 提出ChronoCon对比学习方法，基于患者纵向扫描的时间顺序（而非标签）构建排序损失。假设不可逆疾病具有单调进展性，仅利用就诊时间顺序学习疾病相关表征，将Rank-N-Contrast从标签距离推广到时间排序。
...
TL;DR: 提出Gaussian Splatting Alignment (GSA)方法，用于对齐两个独立的3D高斯溅射模型（即使它们是同一类别中的不同对象），通过相似变换（旋转、平移、缩放）实现，无需已知真实尺度。


<details>
  <summary>Details</summary>
Motivation: 现有方法只能对齐同一对象的3DGS模型，且通常需要已知真实尺度作为输入。需要一种能够对齐同一类别中不同对象（如不同汽车）的3DGS模型的方法，并能自动估计尺度。

Method: 采用基于视点引导的球面图特征获取鲁棒对应关系，引入两阶段优化框架：1) 迭代特征引导的绝对定向求解器进行粗配准（对初始化鲁棒）；2) 受逆辐射场公式启发的多视图特征一致性细配准。

Result: 在相同对象情况下，GSA显著优于现有方法（即使其他方法已知真实尺度）。在不同对象的类别级配准中，GSA大幅超越现有方法，首次提供有效的解决方案。

Conclusion: GSA是首个有效的类别级3DGS配准方法，解锁了新的应用场景，粗配准阶段已达到SOTA性能，细配准进一步改进结果。
...
TL;DR: 提出了MultiBind基准测试，用于评估多主体图像生成中的跨主体属性误绑定问题，并设计了维度混淆评估协议来诊断具体失败模式。


<details>
  <summary>Details</summary>
Motivation: 当前多主体图像生成中存在跨主体属性误绑定的关键失败模式（如属性被保留、编辑或转移到错误的主体），而现有基准和指标主要关注整体保真度或单主体自相似性，难以诊断此类问题。

Method: 构建了基于真实多人照片的MultiBind基准，每个实例提供槽位有序的主体裁剪、掩码和边界框、规范化主体参考、修复的背景参考以及从结构化标注派生的密集实体索引提示。提出了维度混淆评估协议，将生成的主体与真实槽位匹配，并使用面部识别、外观、姿态和表情等专家模型测量槽位间相似度。

Result: 通过减去相应的真实相似度矩阵，该方法能分离自退化与真正的跨主体干扰，并揭示可解释的失败模式（如漂移、交换、主导和混合）。在现代多参考生成器上的实验表明，MultiBind能揭示传统重建指标遗漏的绑定失败。

Conclusion: MultiBind基准和评估协议为多主体图像生成中的跨主体属性绑定问题提供了有效的诊断工具，能够识别传统指标无法检测的具体失败模式，有助于改进多参考图像生成系统的性能。
...
TL;DR: FeatDistill：集成多专家ViT与特征蒸馏的AI生成图像检测框架，针对现实世界取证中的退化干扰、特征表示不足和泛化有限问题，在NTIRE挑战中表现优异。


<details>
  <summary>Details</summary>
Motivation: 深度伪造技术的快速迭代和广泛传播对信息安全构成严重挑战，需要鲁棒且泛化能力强的AI生成伪造图像检测方法。现实世界取证面临三个实际瓶颈：退化干扰、特征表示不足和泛化能力有限。

Method: 1. 构建四骨干Vision Transformer集成（CLIP和SigLIP变体）捕捉互补取证线索；2. 扩展训练集并引入综合退化建模，暴露检测器于多样质量变化和合成伪影；3. 采用两阶段训练：先用标准二分类目标优化，再通过密集特征级自蒸馏进行表示对齐；4. 推理时平均四个独立训练专家的概率输出。

Result: 在NTIRE挑战设置中的广泛评估表明，FeatDistill在多样"野外"条件下实现了强大的鲁棒性和泛化能力。尽管采用集成设计，框架仍保持高效，仅需约10GB峰值GPU内存。

Conclusion: FeatDistill为现实世界深度伪造图像检测提供了一个有效且实用的解决方案，通过特征蒸馏与多专家集成，有效解决了现实取证中的关键瓶颈问题。
...
TL;DR: GeoFlow提出了一种轻量高效的跨视角地理定位框架，通过概率映射和迭代优化打破精度与速度的权衡，实现实时29FPS的高效定位。


<details>
  <summary>Details</summary>
Motivation: 在GPS拒止环境中，精确快速的地理定位对自动驾驶安全导航至关重要。现有的细粒度跨视角地理定位方法面临精度与速度的权衡：高精度模型无法满足实时性要求，而快速模型精度不足。

Method: GeoFlow采用轻量级框架，学习从地面图像到卫星图像的直接概率映射，预测位置假设所需的位移校正（距离和方向）。配合新颖的推理算法——迭代优化采样（IRS），该方法通过迭代优化一组假设，使其从随机起点"流动"到稳健的收敛共识。

Result: 在KITTI和VIGOR数据集上的实验表明，GeoFlow实现了最先进的效率，以29FPS的实时速度运行，同时保持有竞争力的定位精度。该方法支持灵活的推理时间缩放，无需重新训练即可在性能与计算之间直接权衡。

Conclusion: GeoFlow打破了地理定位中精度与速度的传统权衡，为实用实时地理定位系统的开发开辟了新路径，在保持精度的同时实现了实时性能。
...
TL;DR: Group3D提出了一种多视角开放词汇3D检测框架，通过语义兼容性组将语义约束直接集成到实例构建过程中，解决了几何一致性导致的过合并和碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 现有多视角开放词汇3D检测方法通常将几何实例构建与语义标注解耦，仅依赖几何一致性进行实例合并。当几何证据不完整或视角依赖时，这种纯几何合并会导致不可逆的错误，包括不同对象的过合并或单个实例的碎片化。

Method: Group3D从多模态大语言模型获取场景自适应词汇表，并组织成语义兼容性组，编码合理的跨视角类别等价关系。这些组作为合并时的约束条件：只有当3D片段同时满足语义兼容性和几何一致性时才进行关联。该方法支持已知姿态和自由姿态设置，仅依赖RGB观测。

Result: 在ScanNet和ARKitScenes数据集上的实验表明，Group3D在多视角开放词汇3D检测中实现了最先进的性能，同时在零样本场景中表现出强大的泛化能力。

Conclusion: 通过将语义约束直接集成到实例构建过程中，Group3D有效缓解了几何驱动的过合并问题，同时吸收了多视角类别变异性，为开放词汇3D检测提供了更鲁棒的解决方案。
...
TL;DR: 提出统一的时空令牌压缩策略，通过全局选择机制在极低保留率下保持视频理解性能，仅保留约2%视觉令牌即可维持90.1%基线性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型的令牌压缩方法采用两阶段时空压缩策略，依赖阶段特定指标和时空可分离性假设。在极低保留率下，这种方法会导致不平衡分配和关键视觉证据丢失，影响问答准确性。

Method: 将令牌压缩重新定义为全局令牌保留池中的时空分配任务。提出统一选择机制，整合注意力权重和语义相似性来全局选择贡献高、冗余低的令牌。未选中的令牌通过聚类合并并重新填充。在LLM内部引入文本感知合并，基于查询相关性进行二次压缩。无需重新训练，可作为即插即用模块。

Result: 仅保留约2%的视觉令牌即可保持90.1%的基线性能，同时将FLOPs降低至约2.6%。该方法在不同骨干网络上具有普适性，减少了端到端推理延迟和内存消耗，在超低令牌保留条件下建立了视频理解的最新水平。

Conclusion: 提出的统一时空令牌压缩策略通过全局选择机制有效解决了极低保留率下的令牌分配问题，在保持性能的同时大幅降低计算成本，为视频大语言模型的高效部署提供了有效解决方案。
...
TL;DR: 提出了缅甸手写数字数据集(BHDD)，包含87,561张28x28像素的灰度图像，涵盖10个缅甸数字类别，用于缅甸文字识别研究。


<details>
  <summary>Details</summary>
Motivation: 缅甸文字的手写数字识别缺乏公开可用的数据集，现有研究主要关注拉丁字母数字，需要专门的数据集来支持缅甸文字处理技术的发展。

Method: 收集了150多名不同年龄和背景的参与者手写样本，构建了包含60,000个训练样本和27,561个测试样本的数据集，采用MNIST格式，并分析了类别分布、像素统计和形态变化。

Result: 数据集包含87,561张图像，简单基线模型（MLP、两层CNN和改进CNN）分别达到99.40%、99.75%和99.83%的测试准确率，识别了缅甸数字中因圆形形状而易混淆的数字对。

Conclusion: BHDD填补了缅甸手写数字数据集的空白，为缅甸文字识别研究提供了重要资源，数据集在CC BY-SA 4.0许可下公开可用。
...
TL;DR: GeoFusion-CAD：基于扩散和状态空间模型的参数化CAD序列生成框架，通过层次化树结构和C-Mamba块解决长序列生成问题，在DeepCAD-240基准上超越Transformer方法。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer方法在生成长CAD命令序列时面临二次注意力成本和有限上下文窗口问题，难以处理复杂几何拓扑依赖关系，限制了参数化CAD建模的可扩展性。

Method: 将CAD程序编码为层次化树结构，通过状态空间扩散过程联合建模几何和拓扑信息；使用轻量级C-Mamba块通过选择性状态转移建模长程结构依赖，实现跨扩展命令序列的连贯生成。

Result: 在DeepCAD-240基准测试中，GeoFusion-CAD在短序列和长序列（40-240）范围内均取得优异性能，保持高几何保真度和拓扑一致性，而Transformer模型在长序列上性能下降。

Conclusion: GeoFusion-CAD为长序列参数化CAD生成建立了新的SOTA，提供了可扩展的基础框架，为下一代CAD建模系统奠定基础。
...
TL;DR: daVinci-MagiHuman是一个开源的人体中心音频-视频生成基础模型，使用单流Transformer统一处理文本、视频和音频，支持多语言语音生成，在视觉质量和音频-视频同步方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 开发一个统一的人体中心音频-视频生成模型，避免多流或交叉注意力架构的复杂性，同时保持优化的便利性，专注于生成高质量的人体表现、自然语音-表情协调和精确的音频-视频同步。

Method: 采用单流Transformer架构，将文本、视频和音频统一处理为单一令牌序列，仅使用自注意力机制。结合模型蒸馏、潜在空间超分辨率和Turbo VAE解码器实现高效推理。

Result: 在自动评估中达到最高视觉质量和文本对齐，语音可懂度的词错误率最低（14.60%）。在人类评估中，对Ovi 1.1的胜率为80.0%，对LTX 2.3的胜率为60.9%。支持中文（普通话和粤语）、英语、日语、韩语、德语和法语的多语言语音生成。

Conclusion: daVinci-MagiHuman展示了单流Transformer在音频-视频生成任务中的有效性，特别是在人体中心场景中表现出色，并开源了完整的模型栈，为社区提供了强大的音频-视频生成工具。
...
TL;DR: LRC-WeatherNet是一个多传感器融合框架，集成激光雷达、雷达和摄像头数据，用于自动驾驶中的实时天气分类，在恶劣天气条件下显著优于单模态方法。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆在雨、雾、雪等恶劣天气下面临感知和导航挑战，因为LiDAR、RADAR和RGB摄像头等传感器性能会下降。虽然每种传感器都有独特优势（如雷达在低能见度下的鲁棒性，激光雷达在清晰条件下的精确性），但在环境遮挡下也各有局限性。

Method: 提出LRC-WeatherNet多传感器融合框架，采用早期融合（使用统一的鸟瞰图表示）和中层门控融合（融合模态特定的特征图），根据天气变化下各传感器可靠性的变化进行自适应融合。

Result: 在覆盖9种天气类型的MSU-4S数据集上评估，LRC-WeatherNet实现了优越的分类性能和计算效率，在恶劣条件下显著优于单模态基线方法。

Conclusion: 这是首个结合所有三种模态用于自动驾驶中鲁棒实时天气分类的工作，作者发布了训练模型和源代码。
...
TL;DR: STENet提出了一种基于超像素令牌增强的网络，通过引入超像素到跨模态交互中，解决RGB-D显著性目标检测中注意力机制二次复杂度和局部细节提取有限的问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于Transformer的RGB-D显著性目标检测方法面临两个主要挑战：注意力机制的二次计算复杂度，以及局部细节提取能力有限。需要一种既能捕获长距离依赖又能高效处理局部细节的方法。

Method: 提出STENet网络，采用双流编码器-解码器结构。核心是两个超像素驱动的跨模态交互模块：1）超像素注意力全局增强模块，建模全局像素到超像素关系而非像素到像素关系；2）超像素注意力局部细化模块，利用超像素内像素相似性筛选局部像素进行特征增强。还改进了超像素生成方法，扩展了每个超像素的邻域范围。

Result: 在七个RGB-D显著性目标检测数据集上的实验表明，STENet相比最先进方法取得了有竞争力的性能。

Conclusion: STENet通过引入超像素到跨模态交互中，有效解决了RGB-D显著性目标检测中的计算复杂度和局部细节问题，实现了全局和局部特征的全面增强。
...
TL;DR: SegMaFormer：轻量级混合架构，结合Mamba和Transformer模块，用于高效3D医学图像分割，在保持性能的同时大幅减少参数和计算量。


<details>
  <summary>Details</summary>
Motivation: Transformer和Mamba架构虽然能建模全局上下文，但现有Transformer模型计算复杂、参数多，对体积数据尤其不利，且医学图像标注数据有限，需要更高效的解决方案。

Method: 提出SegMaFormer，分层体积编码器中协同使用Mamba和Transformer模块：早期高分辨率阶段用Mamba层减少计算开销并捕获空间上下文；后期低分辨率阶段用自注意力机制精炼特征表示；加入广义旋转位置嵌入增强空间感知。

Result: 在三个公开基准（Synapse、BraTS、ACDC）上达到与更大模型相当的Dice系数，同时参数减少高达75倍，FLOPs大幅降低，实现了高效高性能的3D医学图像分割。

Conclusion: SegMaFormer通过巧妙结合Mamba和Transformer的优势，在保持分割性能的同时显著降低了计算复杂度和参数数量，为3D医学图像分割提供了高效实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统OCT扫描拼接方法（基于图像配准或仅平移扫描）在扫描曲面组织时存在限制，需要更精确的六自由度手眼标定方法

Method: 设计专用标记进行完整的六自由度手眼标定，不依赖图像配准，避免误差累积

Result: 标定结果具有高度可重复性，能够实现对大型曲面组织的稳定扫描，相比传统3D平移扫描有明显改进

Conclusion: 提出的六自由度手眼标定方法有效解决了曲面组织扫描拼接问题，避免了传统方法的误差累积限制
...
Method: 1) 基于FLUX.1-dev流匹配模型；2) 统一多模态融合(UMMF)：将多模态条件编码为统一序列指导高质量图像合成；3) 测试时缩放(TTS)：训练免费范式，通过奖励模型反馈动态引导去噪方向，实现可控计算开销下的性能提升

Result: 在多个标准基准测试中实现了最先进的性能表现
...
TL;DR: 提出基于3DGS的GTSR管道，使用表面和内部高斯建模半透明物体，结合菲涅尔项混合和迪士尼BSDF增强约束，实现高效重建和实时渲染。


<details>
  <summary>Details</summary>
Motivation: 现有方法重建半透明物体存在挑战：基于可微分路径追踪和神经隐式场的方法计算成本高，而基于3DGS的高效方法难以处理半透明物体，因为它们没有考虑半透明物体的光学特性。

Method: 提出GTSR管道：1) 使用两组高斯（表面高斯和内部高斯）分别建模表面和光线穿过半透明物体时的散射颜色；2) 引入菲涅尔项混合两组高斯来渲染半透明外观；3) 采用迪士尼BSDF模型和延迟渲染增强法线和深度约束，改善非轮廓区域的细节重建。

Result: 在NeuralTO Syn数据集上优于基线重建方法，同时展现出优秀的实时渲染性能。扩展数据集包含不同材质特性的新半透明物体，证明方法能适应不同半透明材料。

Conclusion: GTSR成功解决了基于3DGS的半透明物体重建问题，通过创新的高斯建模和渲染技术，在保持高效率的同时实现了高质量的重建结果。
...
TL;DR: DTVI：一种双阶段推理时防御框架，通过类别感知的序列级干预和视觉生成阶段的衰减机制，有效防御文本到图像生成中的不安全内容


<details>
  <summary>Details</summary>
Motivation: 现有推理时防御方法在文本嵌入空间进行类别无关的令牌级干预，无法捕捉分布在完整令牌序列中的恶意语义，且容易受到对抗性提示的攻击

Method: 提出双阶段防御框架：1）在文本嵌入空间进行类别感知的序列级干预，捕捉分布式恶意语义；2）在视觉生成阶段衰减剩余不安全影响

Result: 在性类别基准上平均防御成功率达94.43%，在七个不安全类别上达88.56%，同时在良性提示上保持合理的生成质量

Conclusion: DTVI通过序列级干预和双阶段防御机制，实现了对不安全文本到图像生成的有效且鲁棒的防御，同时保持了良性内容的生成质量
...
TL;DR: UNCHA提出了一种不确定性引导的双曲视觉语言模型对齐方法，通过建模部分到整体的语义代表性，提升对复杂多对象场景的理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有的双曲视觉语言模型虽然能更好地保持层次结构，但没有建模每个部分对整体具有不同语义代表性的问题，限制了在复杂多对象场景中的理解能力。

Method: UNCHA使用双曲不确定性建模部分到整体的语义代表性，将代表性程度融入对比目标中，并通过基于熵的正则化项校准不确定性，学习更准确的部分-整体排序。

Result: UNCHA在零样本分类、检索和多标签分类基准测试中取得了最先进的性能。

Conclusion: 通过不确定性引导的双曲对齐方法，UNCHA能够更好地捕捉图像中的组合结构，提升对复杂多对象场景的理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有艺术字体生成方法存在风格多样性有限和粗粒度控制的问题。本文探索元素驱动的字体生成，将元素作为基本视觉单元来提供参考风格，旨在解决高保真重建参考元素纹理和结构的挑战。

Method: 1. 提出FontCrafter框架，构建大规模ElementFont数据集；2. 采用视觉上下文生成策略，将元素图像作为视觉上下文，使用修复模型在像素级别将元素风格转移到字形区域；3. 设计轻量级上下文感知掩码适配器（CMA）注入形状信息；4. 引入无需训练的注意力重定向机制实现区域感知风格控制并抑制笔画幻觉；5. 应用边缘重绘使边界更自然。

Result: 大量实验表明，FontCrafter在零样本生成方面表现出色，特别是在保持结构和纹理保真度方面，同时支持风格混合等灵活控制。

Conclusion: FontCrafter通过元素驱动的方法成功解决了艺术字体生成中的风格多样性和控制精度问题，实现了高保真的元素风格转移和灵活的字体创作控制。
...
<details>
  <summary>Details</summary>
Motivation: 现有的预训练视觉编码器主要在2D图像数据上训练，缺乏对真实世界3D空间关系的理解，这限制了它们在许多下游应用中的有效性。

Method: 将2D图像中的密集3D空间信息转换为语言表达，通过大语言模型将空间知识注入视觉编码器，采用多轮思维链推理逐步构建层次化空间理解。

Result: 在DINOv3等先进视觉编码器上应用SpatialBoost，在需要3D感知和通用视觉能力的广泛基准上获得显著性能提升，如ADE20K上mIoU从55.9提升到59.7，达到最先进水平。

Conclusion: SpatialBoost通过语言介导的3D空间知识注入，有效增强了现有视觉编码器的空间感知能力，为提升视觉表示模型在真实世界应用中的性能提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态3D视觉语言模型在领域偏移下性能显著下降，而测试时适应方法中的缓存机制存在历史信息有限、预测融合启发式导致适应不稳定的问题。
...
Conclusion: BayesMM通过多模态贝叶斯分布学习框架，解决了现有测试时适应方法的信息损失和融合不稳定问题，实现了对演化测试数据的持续适应。
...
TL;DR: P-Flow是一个无需训练的视频生成框架，通过测试时提示优化来定制动态视觉效果，利用视觉语言模型进行语义和时间推理，迭代优化提示以在视频中实现所需的动态效果。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型在遵循文本提示方面已有显著进步，但动态视觉效果（如物体破碎或爆炸）的定制仍未被充分探索。现有方法主要关注主体或相机的低级运动控制，而动态视觉效果涉及更高级的语义，更适合通过文本提示控制。然而，人工制作准确描述这些效果的提示既困难又耗时，因为它们需要复杂的时间推理和迭代优化。

Method: P-Flow是一个无需训练、无需修改底层模型的框架。它利用视觉语言模型的语义和时间推理能力，执行测试时提示优化：基于参考视频的视觉效果与生成输出之间的差异，迭代优化文本提示。通过这种迭代细化，提示逐渐演化以更好地在新场景中诱导所需的动态效果。

Result: 实验表明，P-Flow在动态视觉效果定制方面实现了高保真度和多样性，在文本到视频和图像到视频生成任务上均优于其他模型。

Conclusion: P-Flow通过测试时提示优化有效解决了动态视觉效果定制问题，无需修改底层视频生成模型，为复杂视觉效果的定制提供了一种高效且灵活的方法。
...
TL;DR: NullSteer：一种零空间投影激活防御框架，通过线性变换在模型激活中构建拒绝方向，在良性子空间保持零扰动，同时在潜在有害方向上动态诱导拒绝，理论上实现安全增强而不损害模型通用能力。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在开放世界场景中部署时，容易受到视觉越狱攻击诱导生成有害内容，对模型安全和可信使用构成严重风险。现有的激活引导方法虽然有效，但存在过度拒绝问题，会降低模型在良性输入上的性能，且缺乏理论可解释性，鲁棒性和有效性有限。

Method: 提出NullSteer框架，通过线性变换在模型激活中构建拒绝方向。该方法在良性子空间保持零扰动，同时在潜在有害方向上动态诱导拒绝，理论上实现安全增强而不损害模型通用能力。

Result: 大量实验表明，NullSteer在各种越狱攻击下显著减少有害输出（在MiniGPT-4上平均ASR降低超过15%），同时在通用基准测试上保持与原始模型相当的性能。

Conclusion: NullSteer通过零空间投影激活防御框架，有效平衡了安全性和实用性，在增强模型安全性的同时不损害其通用能力，为解决视觉语言模型的安全风险提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 增强现实和机器人领域对关节物体重建的需求日益增长，但现有方法需要非平凡的轴对齐或覆盖不足，限制了实际应用。需要一种简单设置且高可扩展性的解决方案。
...
Conclusion: FreeArtGS通过自由移动场景下的关节物体重建新设置，结合部件分割、关节估计和端到端优化，实现了高可扩展性和实用性，为增强现实和机器人应用提供了有效的资产生成方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前具身智能代理存在能力碎片化问题：仅支持离线视频理解、缺乏长期多模态记忆机制、难以在流式输入下实现实时推理和主动交互。这些缺陷阻碍了它们在真实环境中持续感知、实时决策和执行行动的能力。

Method: 提出StreamingClaw框架，具备五大核心能力：1) 实时流式推理；2) 未来事件推理和在线交互目标演化下的主动交互；3) 多模态长期存储、分层演化和多代理共享内存高效检索；4) 感知-决策-行动闭环，提供流式工具和面向物理环境的动作中心技能；5) 兼容OpenClaw框架，利用开源社区资源。

Result: StreamingClaw将在线实时推理、多模态长期记忆和主动交互集成到统一框架中，通过将决策转化为可执行动作，实现对物理世界的直接控制，支持具身交互的实际部署。

Conclusion: StreamingClaw解决了当前具身智能代理的关键瓶颈，提供了一个支持实时流式视频理解、多模态记忆和主动交互的统一框架，为实现真实环境中的感知-决策-行动闭环提供了有效解决方案。
...
TL;DR: 提出一个两阶段框架，通过LLM引导的字幕匹配生成辅助短视频作为时序先验，然后使用多模态控制Mamba网络高效融合先验和长视频序列，提升视频时刻检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频时刻检索方法存在以下问题：1) 难以捕捉未修剪视频中的隐藏时序动态，导致长序列中定位不精确；2) 传统方法依赖自然语言查询或静态图像增强，忽略了运动序列；3) Transformer架构计算成本高；4) 现有方法未能有效整合字幕上下文和生成的时序先验。

Method: 提出两阶段框架：第一阶段使用LLM引导的字幕匹配从视频字幕中识别相关文本线索，与查询融合后通过文本到视频模型生成辅助短视频，作为捕捉隐含运动信息的时序先验。第二阶段通过多模态控制Mamba网络处理增强查询，扩展文本控制选择并加入视频引导门控，高效融合生成的先验和长序列同时过滤噪声。

Result: 在TVR基准测试中，相比最先进方法取得显著改进，包括降低计算开销和在长序列定位中获得更高的召回率。

Conclusion: 该框架对基础检索模型具有不可知性，广泛适用于多模态视频时刻检索，通过有效整合字幕上下文和生成的时序先验，解决了现有方法在捕捉时序动态和计算效率方面的局限性。
...
TL;DR: 提出PRISM-RM方法，使用隐式神经表示结合生物物理约束进行呼吸运动建模，实现无固定参考状态的连续可微运动表示，在插值场景表现良好，外推能力有待提升。


<details>
  <summary>Details</summary>
Motivation: 肺部及上腹部放疗中，呼吸运动导致显著治疗不确定性，需要运动管理技术。传统配对配准方法存在局限性，需要更连续、可推广的运动模型。

Method: 提出PRISM-RM方法：基于隐式神经表示(INR)的代理运动建模，引入生物物理约束确保生理合理性，提供无固定参考状态的轨迹感知时空连续可微运动表示。

Result: 轨迹感知方法在插值场景表现与初始INR方法相当，外推能力有所改进。相比序列配准方法，插值表现相当但外推场景表现不足。INR方法学特性使其在呼吸运动建模中具有潜力。

Conclusion: 隐式神经表示结合生物物理约束为呼吸运动建模提供新途径，具有连续可微、无固定参考状态等优势，性能持续改进中，展现出推动该领域发展的强大潜力。
...
TL;DR: DA-VAE通过扩展预训练VAE的潜在空间维度并保持原有结构，实现高压缩比图像生成，减少token数量，提升训练和推理效率。


<details>
  <summary>Details</summary>
Motivation: 减少token数量对潜在扩散模型的高效训练和推理至关重要，特别是在高分辨率下。现有高压缩图像tokenizer在仅用于重建时，高维潜在空间会失去有意义的语义结构，使扩散训练更困难。

Method: 提出Detail-Aligned VAE，采用显式潜在布局：前C个通道来自预训练VAE的基础分辨率，额外D个通道编码更高分辨率细节。通过细节对齐机制保持扩展潜在空间的结构，采用warm-start微调策略。

Result: 使用Stable Diffusion 3.5实现1024×1024图像生成仅需32×32 tokens（比原始模型少4倍），在5个H100天内完成。进一步实现2048×2048生成，速度提升6倍同时保持图像质量。在ImageNet上定量验证了方法和设计选择。

Conclusion: DA-VAE通过轻量级适配预训练扩散骨干，有效扩展VAE压缩比，保持潜在空间结构，实现高效高分辨率图像生成，为扩散模型的高效训练和推理提供了实用解决方案。
...
TL;DR: OpenEarth-Agent：首个面向开放环境地球观测的工具创建智能体框架，通过自适应工作流规划和工具创建来应对多源数据和异构任务，而非依赖预定义工具。


<details>
  <summary>Details</summary>
Motivation: 地球观测在感知动态地表变化中至关重要，但开放环境中部署自主EO面临多源数据多样性和异构任务的挑战。现有遥感智能体局限于封闭环境，依赖预定义工具，难以泛化到多样化数据和任务。

Method: 提出OpenEarth-Agent框架，采用自适应工作流规划和工具创建机制，而非调用预定义工具。通过多阶段工具和跨领域知识库的开放式集成，增强在整个EO流程中的鲁棒执行能力。

Result: 在OpenEarth-Bench基准测试（7个应用领域596个真实案例）上，OpenEarth-Agent成功掌握开放环境中的全流程EO。在跨基准测试中，仅用6个基础预训练模型的工具创建智能体性能与依赖104个专用工具的工具调用智能体相当，使用完整工具集时显著超越。创建的工具在某些情况下比人工设计的工具对数据异常更具鲁棒性。

Conclusion: OpenEarth-Agent通过工具创建而非工具调用的创新方法，有效解决了开放环境地球观测中的泛化问题，为自主EO系统在多样化场景中的部署提供了可行方案。
...
TL;DR: Bearing-UAV：纯视觉驱动的跨视角导航方法，联合预测无人机绝对位置和航向，无需GNSS，在野外实现准确、轻量、鲁棒的导航。


<details>
  <summary>Details</summary>
Motivation: 现有跨视角地理定位方法主要匹配无人机视图与机载地图瓦片，存在精度与存储开销的权衡，且忽视航向信息。此外，跨视角场景的巨大差异和重叠变化考虑不足，限制了实际应用。

Method: 提出Bearing-UAV方法，利用全局和局部结构特征，显式编码相对空间关系，从相邻特征联合预测无人机绝对位置和航向。同时创建Bearing-UAV-90k多城市基准数据集。

Result: 实验表明Bearing-UAV在不同地形下比之前的匹配/检索方法具有更低的位置误差，在跨视角变化、不对齐和特征稀疏条件下表现出鲁棒性。

Conclusion: Bearing-UAV为GNSS拒止环境提供了一种准确、轻量、鲁棒的纯视觉导航解决方案，通过联合位置和航向预测克服了现有方法的局限性。
...
TL;DR: ACPO提出了一种非对称约束偏好优化方法，解决DPO中的似然位移问题，特别针对多模态场景中的视觉锚点崩溃，有效减少幻觉并提升模型性能。


<details>
  <summary>Details</summary>
Motivation: DPO作为大视觉语言模型对齐的主流方法，存在似然位移问题——被选和被拒响应的概率都会崩溃。在多模态场景中，这种优化缺陷尤为严重：被选似然的侵蚀（称为视觉锚点崩溃）导致模型放弃视觉证据而依赖语言先验，引发显著幻觉。

Method: 提出非对称约束偏好优化（ACPO），这是一种模态无关的对齐机制，对偏好优化应用动态、目标导向的缩放。ACPO推导出复杂度感知的缩放系数，仅应用于被拒奖励项，非对称地抑制被拒项的梯度流，同时将被选分布保持为梯度稳定的参考。

Result: 在InternVL模型上的实验表明，ACPO有效逆转了标准DPO的被选奖励退化。通过阻止视觉锚点崩溃，ACPO在幻觉基准（HallusionBench、MM-IFEval）和通用排行榜（MMBench、MMStar、OCRBenchV2）上普遍优于基线方法，同时推动通用能力的同步提升。

Conclusion: ACPO通过打破梯度对称性，解决了DPO在多模态任务中的关键缺陷，有效抑制语言先验对视觉标记的压制，为多模态对齐提供了更稳健的优化框架。
...
TL;DR: VFLM是一个利用视觉反馈进行迭代优化的布局生成框架，通过强化学习和OCR准确度奖励机制，显著提升布局的可读性和美观性


<details>
  <summary>Details</summary>
Motivation: 现有基于代码的布局生成方法无法看到渲染后的视觉效果，难以保证可读性和美观性，视觉反馈成为关键因素

Method: 提出视觉反馈布局模型(VFLM)，采用自改进框架，通过强化学习和基于OCR准确度的视觉奖励模型，实现自适应反思生成和迭代优化

Result: 在多个基准测试中，VFLM持续优于先进的MLLM、现有布局模型和纯代码基线，验证了视觉反馈在设计导向MLLM中的关键作用

Conclusion: 视觉反馈是布局生成的关键因素，VFLM框架通过迭代优化和视觉奖励机制有效提升了布局质量，为设计导向的多模态大语言模型提供了新方向
...
TL;DR: 本文研究了不同骨干网络在自监督学习（SSL）作为辅助任务时对基于纹理的局部描述符特征建模的影响，用于高效人脸分析。通过MAE作为辅助目标重建纹理特征，结合主要任务进行人脸分析，发现骨干网络性能高度依赖下游任务，没有统一的骨干网络适用于所有人脸分析任务。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明，将主要任务与自监督辅助任务结合可以实现更鲁棒和更具判别性的表示学习。本文旨在研究不同骨干网络（从浅层到深层）在自监督学习作为辅助任务时，对基于纹理的局部描述符特征建模的影响，以提升人脸分析的效率和鲁棒性。

Method: 采用局部模式自监督辅助任务（L-SSAT）框架，使用掩码自编码器（MAE）作为辅助目标来重建纹理特征（如局部模式），同时执行主要任务。通过不同骨干网络（从浅层到深层）进行实验，在FaceForensics++、CelebA和AffectNet等数据集上进行综合比较分析。

Result: 实验表明，所提出方法的骨干网络性能高度依赖下游任务：在FaceForensics++上达到0.94的平均准确率，CelebA上0.87，AffectNet上0.88。对于人脸属性预测、情感分类和深度伪造检测等不同人脸分析任务，没有统一的骨干网络能够保证一致的特征表示质量和泛化能力。

Conclusion: 骨干网络在L-SSAT框架中的性能高度依赖于具体的人脸分析任务，没有通用的骨干网络适用于所有任务。不同任务需要选择不同的骨干网络架构来获得最佳性能，这强调了针对特定任务进行骨干网络选择的重要性。
...
TL;DR: PAM：一个统一的姿态-外观-运动引擎，用于可控的手-物交互视频生成，在多个基准测试中超越了现有方法，并能通过合成数据增强下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有手-物交互生成研究存在三个分散的领域：仅姿态合成、单图像生成（缺乏动态）、需要完整姿态序列和真实第一帧的视频生成。需要一个统一的框架将姿态、外观和运动整合在一起。

Method: 提出PAM（姿态-外观-运动引擎），一个统一的可控手-物交互视频生成框架。通过结合深度、分割和关键点等多条件输入，生成高分辨率视频。

Result: 在DexYCB数据集上获得FVD 29.13（优于InterDyn的38.83）和MPJPE 19.37mm（优于CosHand的30.05mm），生成480x720分辨率视频。在OAKINK2上，多条件模型将FVD从68.76提升到46.31。合成数据增强使仅用50%真实数据加合成数据就能达到100%真实数据的性能。

Conclusion: PAM提供了一个统一的姿态-外观-运动引擎，能够生成高质量的手-物交互视频，在多个基准测试中超越现有方法，并能有效增强下游任务的训练数据。
...
TL;DR: MAMMOTH是一个用于计算病理学中多实例学习（MIL）的参数高效多专家混合模块，通过为每个补丁的表型定制低秩变换来提升任务特定特征表示，显著提高各种MIL方法的性能。


<details>
  <summary>Details</summary>
Motivation: 在多实例学习（MIL）框架中，现有研究主要关注补丁特征提取和聚合方法的优化，而忽略了将通用特征转换为任务特定特征的关键线性层。作者假设这一层是性能瓶颈，通过为每个补丁的表型定制低秩变换可以显著提升表示能力。

Method: 提出MAMMOTH模块，这是一个参数高效的多头专家混合模块，可以轻松集成到任何MIL模型中。该模块为每个补丁的表型提供定制的低秩变换，将通用特征转换为任务特定特征，与现有聚合方法协同工作。

Result: 在8种MIL方法和19个分类任务上的实验表明，任务特定变换对性能的影响比聚合方法选择更大。配备MAMMOTH后，即使是简单的最大或平均池化方法也能获得比标准线性层更好的平均性能。在152个配置中，130个性能得到提升，平均性能提升+3.8%。

Conclusion: MAMMOTH模块通过为每个补丁的表型提供定制的低秩变换，有效解决了MIL框架中被忽视的性能瓶颈问题，显著提升了各种MIL方法的性能，且参数高效，易于集成。
...
TL;DR: 提出了Omni-WorldBench基准，专门评估4D世界模型的交互响应能力，包含Omni-WorldSuite提示套件和Omni-Metrics评估框架，测试了18个代表性模型并揭示了现有局限。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型评估要么只关注生成模型的视觉保真度和文本-视频对齐，要么依赖静态3D重建指标而忽略时间动态。世界建模的未来在于4D生成，需要评估交互响应能力——即交互动作如何驱动时空状态转换的能力，但现有基准缺乏对此的系统评估。
...
TL;DR: 提出SpatialReward奖励模型，专门评估生成图像中的空间布局准确性，通过多阶段流程提升文本到图像生成的空间一致性。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成的奖励模型主要关注语义对齐和视觉质量，但对细粒度空间关系关注不足，导致生成的图像整体合理但对象定位不准确。

Method: 采用多阶段流程：1) Prompt Decomposer分解提示中的实体、属性和空间元数据；2) 专家检测器提供对象位置和属性的视觉定位；3) 视觉语言模型通过思维链推理评估复杂空间关系。同时提出SpatRelBench基准测试。

Result: 在Stable Diffusion和FLUX上的实验表明，将SpatialReward纳入强化学习训练能持续提升空间一致性和整体生成质量，结果更符合人类判断。

Conclusion: 可验证的奖励模型在实现更准确和可控的文本到图像生成模型优化方面具有显著潜力。
...
TL;DR: 该研究对四种先进的深度学习模型在真实航空LiDAR数据上进行3D语义分割性能评估，发现所有模型总体准确率超过93%，KPConv在平均IoU上表现最佳，Point Transformer V3在车辆类别上表现突出。


<details>
  <summary>Details</summary>
Motivation: 现有3D语义分割模型主要针对室内或地面数据集，在真实航空采集条件下的性能尚未充分探索。虽然已有少数研究涉及类似场景，但在数据集设计、采集条件和模型选择上存在差异，需要填补这一研究空白。

Method: 在西班牙纳瓦拉地区通过实际飞行条件获取的大规模航空LiDAR数据集上进行实验基准测试，该数据集覆盖城市、乡村和工业等异质景观。比较了四种代表性深度学习模型：KPConv、RandLA-Net、Superpoint Transformer和Point Transformer V3，评估它们在五个常见航空测量语义类别（地面、植被、建筑物、车辆等）上的性能。
...
Conclusion: 该研究为航空LiDAR数据的3D语义分割提供了重要的基准测试，揭示了不同模型在真实航空条件下的性能特点。KPConv在整体性能上表现最优，而Point Transformer V3在特定类别上具有优势，为实际应用中的模型选择提供了指导。
...
TL;DR: 使用Point Transformer v2对多光谱LiDAR点云进行语义分割，实现河岸环境土地覆盖分类，包括沙、砾石、植被、水体等类别，在芬兰Oulanka河上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 河岸环境的准确土地覆盖制图对于河流管理、生态理解和地貌变化监测至关重要。传统方法可能无法充分利用多光谱LiDAR数据的几何和光谱信息。

Method: 采用Point Transformer v2架构处理3通道LiDAR点云数据，利用几何和光谱特征进行语义分割。通过消融实验分析不同特征的重要性，并探索多数据集训练以提高模型泛化能力。

Result: 完整特征配置获得0.950的平均交并比，显著优于仅使用几何特征的基线。强度和反射率特征是准确分类的关键。多数据集训练提高了模型在新河岸环境中的泛化性能。

Conclusion: 基于Transformer的架构在多光谱点云河岸环境分类中表现出巨大潜力，为监测沉积物输移和其他河流管理应用提供了新能力。
...
TL;DR: 提出EgoGroups数据集，用于评估社交群体检测模型在真实世界第一人称视角下的表现，涵盖65个国家不同人群密度和场景条件。


<details>
  <summary>Details</summary>
Motivation: 现有社交群体检测基准存在场景多样性不足、依赖第三人称视角（如监控视频）的问题，无法真实评估模型在不同文化背景和无约束环境下的群体形成与演化。

Method: 构建EgoGroups数据集，包含65个国家、不同人群密度（低/中/高）和四种天气/时间条件下的第一人称视角视频，提供密集的人员和社交群体标注，以及丰富的地理和场景元数据。

Result: 评估发现：1）在零样本设置下，VLM/LLM模型可以超越监督基线；2）人群密度和文化区域明显影响模型性能。

Conclusion: EgoGroups填补了社交群体检测在真实世界评估方面的空白，为研究社交动态提供了高质量的第一人称视角数据集，揭示了模型性能受环境和文化因素影响的重要发现。
...
<details>
  <summary>Details</summary>
Motivation: 传统监督式光流方法依赖昂贵的人工标注，限制了可扩展性；无监督和半监督方法基于亮度恒定和平滑性假设，在复杂真实场景中往往产生不可靠的监督信号，导致运动估计不准确。

Method: 1) 利用预训练深度估计网络生成伪光流作为条件输入；2) 训练下一帧生成模型产生高保真、像素对齐的后续帧；3) 提出不一致像素过滤策略，识别并移除生成帧中的不可靠像素以提升真实数据集上的微调性能。

Result: 在KITTI2012、KITTI2015和Sintel数据集上的广泛实验表明，FlowGen取得了与现有无监督和半监督方法相当或更优的结果，展示了其作为可扩展、免标注光流学习方案的潜力。

Conclusion: FlowGen框架通过合成大规模、完美对齐的帧-光流数据对，为光流学习提供了一种可扩展的免标注解决方案，在多个基准数据集上表现出色，有望推动光流估计领域的发展。
...
TL;DR: DUO-VSR：一种三阶段双流蒸馏框架，通过结合分布匹配和对抗监督实现一步视频超分辨率，在保持高质量的同时大幅降低采样成本


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的视频超分辨率方法虽然取得了显著的保真度，但存在采样成本过高的问题。直接应用分布匹配蒸馏到VSR会导致训练不稳定、监督不足和质量下降

Method: 1. 渐进引导蒸馏初始化：通过轨迹保持蒸馏稳定后续训练
2. 双流蒸馏：联合优化DMD流和RFS-GAN流，后者利用真实和伪造评分模型的判别特征提供对抗监督
3. 偏好引导精炼：根据感知质量偏好进一步对齐学生模型

Result: DUO-VSR在视觉质量和效率方面优于先前的一步VSR方法，实现了高质量的视频超分辨率同时大幅降低计算成本

Conclusion: DUO-VSR通过创新的三阶段双流蒸馏框架，成功解决了扩散模型在视频超分辨率中采样成本过高的问题，为高效高质量的视频增强提供了有效解决方案
...
TL;DR: GLD提出使用几何基础模型的几何一致特征空间作为多视角扩散的潜在空间，用于新颖视角合成，相比传统VAE潜在空间在图像质量和3D一致性方面表现更好，训练速度更快。


<details>
  <summary>Details</summary>
Motivation: 当前新颖视角合成方法通常在视角无关的VAE潜在空间中操作，缺乏几何一致性，而几何基础模型的特征空间具有跨视角几何对应关系，更适合NVS任务。

Method: 提出几何潜在扩散框架，将几何基础模型的几何一致特征空间重新用作多视角扩散的潜在空间，支持高保真RGB重建和跨视角几何对应。

Result: GLD在2D图像质量和3D一致性指标上优于VAE和RAE，训练速度比VAE潜在空间快4.4倍以上，即使没有大规模文本到图像预训练，也能与最先进方法竞争。

Conclusion: 几何基础模型的几何一致特征空间是新颖视角合成的理想潜在空间，GLD框架为多视角生成提供了高效且高质量的解决方案。
...
TL;DR: VLMs使用两种机制表示物体属性与空间关系：语言模型中间层在视觉标记上编码内容无关的空间关系，但主要空间信息来自视觉编码器，其全局表示直接影响预测。


<details>
  <summary>Details</summary>
Motivation: 研究多模态任务中视觉语言模型如何计算物体属性与空间关系，明确这些关联在模型中的计算位置和机制。

Method: 分析VLMs中两种并发机制：语言模型中间层在视觉标记上表示内容无关的空间关系，以及视觉编码器编码物体布局的全局空间表示。

Result: 发现空间关系的主要来源是视觉编码器，其表示全局分布在所有视觉标记中，增强这些表示能提升自然图像上的空间推理性能。

Conclusion: 阐明了VLMs中空间关联的计算方式，强调了视觉编码器在空间推理中的核心作用。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型和视觉语言模型在细粒度视觉编辑中存在空间理解和布局一致性问题，需要更结构化的推理方法来改善空间关系控制
...
Conclusion: 结构化推理框架通过显式的关系表示引导推理过程，显著提升了空间关系的可解释性和控制能力，在布局编辑任务中表现出优越的空间精度
...
TL;DR: DualCoT-VLA：一种并行推理的视觉语言思维链方法，通过视觉CoT和语言CoT分别处理空间细节和任务规划，将自回归推理转为单步推理，提升复杂任务性能并降低延迟。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在处理需要逻辑规划的多步骤任务和需要精细空间感知的精确操作时存在困难。现有的CoT方法存在两个关键限制：1）无法同时捕捉低级视觉细节和高级逻辑规划；2）自回归逐步解码导致高推理延迟和错误累积。

Method: 提出DualCoT-VLA方法：1）集成视觉CoT进行低级空间理解，语言CoT进行高级任务规划；2）引入并行CoT机制，使用两组可学习的查询令牌，将自回归推理转变为单步前向推理。

Result: 在LIBERO和RoboCasa GR1基准测试以及真实世界平台上实现了最先进的性能，证明了该方法在复杂多步骤任务中的有效性。

Conclusion: DualCoT-VLA通过并行视觉-语言思维链机制，解决了现有VLA模型在复杂任务中的局限性，实现了高效的多模态推理和低延迟执行。
...
<details>
  <summary>Details</summary>
Motivation: 现有潜在世界模型（如V-JEPA2）从短观察窗口进行密集预测，限制了时间上下文，偏向局部低层次外推，难以捕捉长时程语义；而视觉语言模型（VLMs）虽然提供强语义基础，但由于计算驱动的稀疏采样、语言输出瓶颈和数据不匹配，不适合作为独立的密集预测器

Method: 提出VLM引导的JEPA风格潜在世界建模框架，包含双时间通路：密集JEPA分支用于细粒度运动和交互线索，均匀采样的VLM思考者分支用于知识丰富的指导；引入分层金字塔表示提取模块，将多层VLM表示聚合为与潜在预测兼容的指导特征

Result: 在手部操作轨迹预测实验中，该方法优于纯VLM基线和JEPA预测器基线，并产生更稳健的长时程推演行为

Conclusion: 通过结合密集帧动态建模与VLM的长时程语义指导，提出的双通路框架能够有效提升世界模型的预测能力和下游任务实用性
...
TL;DR: UniMotion是首个统一处理人体运动、自然语言和RGB图像的理解与生成的框架，通过连续模态表示和多种对齐技术，在七项跨模态任务上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有统一模型仅处理有限模态子集（如运动-文本或静态姿态-图像），且主要依赖离散化表示，这会引入量化误差并破坏时间连续性。需要一种能同时处理运动、语言和图像的真正统一框架。

Method: 1. 将运动视为与RGB平等的连续模态；2. 提出跨模态对齐运动VAE（CMA-VAE）和对称双路径嵌入器；3. 提出双后验KL对齐（DPA）将视觉语义先验注入运动表示；4. 提出潜在重建对齐（LRA）解决冷启动问题，通过自监督预训练校准运动通路。

Result: 在涵盖三个模态之间任意到任意理解、生成和编辑的七项任务上实现了最先进的性能，尤其在跨模态组合任务上表现出显著优势。

Conclusion: UniMotion通过将运动作为连续模态处理，并采用创新的对齐和校准技术，成功构建了首个真正统一的人体运动、语言和图像理解与生成框架，为多模态AI系统提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有潜在扩散模型需要复杂的多阶段训练：先训练标记器，再在冻结的潜在空间中训练扩散模型。这种分离训练限制了潜在空间的协同优化。

Method: 提出UNITE架构，包含一个生成编码器，通过权重共享同时作为图像标记器和潜在生成器。核心洞见是将标记化（从完整图像推断潜在）和生成（从噪声加条件推断潜在）视为同一潜在推断问题的不同条件形式。采用单阶段训练，通过同一生成编码器的两次前向传播联合优化两个任务。

Result: 在图像和分子模态上，无需对抗损失或预训练编码器，UNITE达到接近SOTA性能：ImageNet 256×256上Base模型FID 2.12，Large模型FID 1.73。通过表示对齐和压缩分析验证了生成编码器的有效性。

Conclusion: 单阶段联合训练标记化和生成是可行的，共享参数能够塑造"共同潜在语言"，简化了潜在扩散模型的训练流程。
...
Method: 将视频分割成片段，构建视觉-时间亲和图，通过假设-验证-精炼循环估计查询相关性并传播到未观察片段

Result: 在主流MLLMs上取得显著提升，VideoMME-long基准上准确率最高提升7.5%

Conclusion: VideoDetective通过整合查询相关性和片段间关系，有效解决了长视频理解中的线索定位问题
...
Result: 在Cosmos-Predict2.5-2B模型上，PAI-Bench评估显示：实现2.3倍推理加速，同时保持99.4%的基线质量，显著优于现有的免训练缓存方法

Conclusion: WorldCache通过感知约束的动态缓存框架，有效解决了现有特征缓存方法在动态场景中的问题，在保持高质量的同时大幅提升推理速度，为扩散变换器的实际应用提供了高效解决方案
