...
TL;DR: 该论文提出了一种端到端的光学音乐识别框架，结合残差瓶颈卷积和双向门控循环单元，在Camera-PrIMuS和PrIMuS数据集上实现了高精度和高效率的音乐符号识别。


<details>
  <summary>Details</summary>
Motivation: 光学音乐识别(OMR)旨在将印刷或手写乐谱图像转换为可编辑的符号表示。传统方法通常需要复杂的预处理和符号分割步骤，而端到端方法可以简化流程并提高识别性能。

Method: 提出端到端OMR框架：1) 使用ResNet-v2风格的残差瓶颈块和多尺度空洞卷积的CNN提取特征，编码细粒度符号细节和全局五线谱结构；2) 将特征序列输入BiGRU网络建模音乐符号间的时序依赖；3) 使用连接时序分类损失进行端到端训练，无需显式对齐标注。

Result: 在Camera-PrIMuS数据集上：序列错误率7.52%，符号错误率0.45%，音高、类型和音符准确率分别为99.33%、99.60%、99.28%。在PrIMuS数据集上：序列错误率8.11%，符号错误率0.49%，音高、类型和音符准确率分别为99.27%、99.58%、99.21%。平均每轮训练时间1.74秒，计算效率高。

Conclusion: 该端到端OMR框架结合残差卷积和BiGRU序列建模，在保持高计算效率的同时实现了优异的识别性能，细粒度错误分析进一步验证了模型的有效性。
...
TL;DR: HalfV 是一个针对高分辨率多模态大语言模型的高效推理框架，通过解耦视觉冗余为通用内在冗余和架构依赖的二次饱和冗余，实现跨不同骨干网络的统一加速，在保持性能的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 高分辨率多模态大语言模型在推理时面临视觉令牌爆炸导致的计算成本过高问题。现有加速策略存在严重的"骨干网络依赖性"，在特定架构（如Vicuna、Mistral）上表现良好，但在其他架构（如Qwen）上会导致显著性能下降。

Method: 利用截断矩阵熵揭示通用的三阶段推理生命周期，将视觉冗余解耦为：1）通用内在视觉冗余（IVR）；2）架构依赖的二次饱和冗余（SSR）。HalfV框架首先通过统一剪枝策略缓解IVR，然后根据SSR的具体表现自适应处理。

Result: HalfV在不同骨干网络上实现了优越的效率-性能权衡。在Qwen25-VL上，以4.1倍FLOPs加速保留了96.8%的性能，显著优于现有最先进基线方法。

Conclusion: 通过解耦视觉冗余为通用和架构依赖两部分，HalfV框架解决了现有加速策略的骨干网络依赖问题，为高分辨率MLLMs提供了跨架构的高效推理解决方案。
...
TL;DR: 提出一种视频隐空间压缩方法，通过移除隐表示中的高频分量而非直接减少通道数，在保持压缩比的同时提升视频重建质量


<details>
  <summary>Details</summary>
Motivation: 视频变分自编码器通常需要大量隐通道来保证高质量重建，但过多的隐通道会阻碍隐扩散模型的收敛并降低生成性能，即使重建质量仍然很高

Method: 提出隐空间压缩方法，移除视频隐表示中的高频分量，而不是直接减少通道数，从而在保持压缩比的同时避免重建质量下降

Result: 实验结果表明，该方法在相同整体压缩比下，相比强基线实现了更优的视频重建质量

Conclusion: 通过移除隐表示中的高频分量而非直接减少通道数，可以在保持压缩效率的同时提升视频重建质量，解决隐扩散模型中隐通道过多的问题
...
TL;DR: 本文提出了一种基于立体视觉的无人机系统，用于检测和定位辐射松树枝以支持自主修剪。系统包含两个阶段：分支分割和深度估计，比较了多种深度学习和传统方法。


<details>
  <summary>Details</summary>
Motivation: 为林业自动化修剪提供低成本解决方案，通过无人机搭载立体视觉系统实现树枝的精确检测和定位，提高林业作业效率。

Method: 系统分为分支分割和深度估计两阶段。分割阶段比较YOLOv8、YOLOv9和Mask R-CNN变体；深度估计阶段比较传统方法（SGBM+WLS滤波）和多种深度学习模型（PSMNet、ACVNet等）。提出基于质心的三角测量算法配合MAD异常值剔除来计算分支距离。

Result: 在1-2米距离的定性评估中，基于深度学习的视差图比传统SGBM方法产生更一致的深度估计，证明了低成本立体视觉在林业自动化分支定位中的可行性。

Conclusion: 该研究展示了基于无人机的立体视觉系统在辐射松树枝检测和定位方面的潜力，深度学习方法的视差估计优于传统方法，为林业自动化修剪提供了可行的技术方案。
...
TL;DR: ETC是一个可扩展的概念擦除框架，能够同时擦除数千个概念，同时保持生成质量，通过t分布混合模型建模概念分布，并使用MoEraser模块实现精确擦除


<details>
  <summary>Details</summary>
Motivation: 大规模文本到图像扩散模型虽然视觉保真度高，但存在安全风险，可能生成不受欢迎的内容（如受版权保护的内容）。现有概念擦除方法在可扩展性、精确性和鲁棒性方面存在平衡问题，通常只能擦除几百个概念

Method: 1. 使用学生t分布混合模型(tMM)建模低秩概念分布；2. 通过仿射最优传输实现目标概念的精确擦除，同时通过锚定目标概念分布边界来保护其他概念；3. 训练基于混合专家(MoE)的MoEraser模块，移除目标嵌入同时保留锚定嵌入；4. 通过向文本嵌入投影器注入噪声并微调MoEraser进行恢复，实现对抗白盒攻击的鲁棒性

Result: 在超过2000个跨异构领域的概念和不同扩散模型上的广泛实验表明，该方法在大规模概念擦除方面实现了最先进的可扩展性和精确性

Conclusion: ETC框架成功解决了现有概念擦除方法在可扩展性、精确性和鲁棒性方面的限制，能够同时擦除数千个概念，为大规模文本到图像模型的安全应用提供了有效解决方案
...
TL;DR: 这篇论文调查了视觉机器人空间记忆效率问题，提出了α指标来衡量运行时内存与存储地图大小的比例，揭示了神经方法中内存使用差异巨大，并提出了标准化评估协议。


<details>
  <summary>Details</summary>
Motivation: 随着视觉机器人在更大环境中导航，其空间记忆会无限增长，最终耗尽计算资源，特别是在嵌入式平台（8-16GB共享内存，<30W）上，无法通过增加硬件来解决。需要解决空间记忆效率问题。

Method: 调查了88篇参考文献和52个系统（1989-2025），从占据栅格到神经隐式表示。引入了α = M_peak/M_map指标，即峰值运行时内存与存储地图大小的比例。提出了标准化评估协议，包括内存增长率、查询延迟、内存-完整性曲线和吞吐量下降。

Result: 独立分析显示α在神经方法中跨越两个数量级，从2.3（Point-SLAM）到215（NICE-SLAM）。通过帕累托前沿分析发现，没有单一范式在其评估体系中占主导地位：3DGS方法在Replica数据集上达到最佳绝对精度（90-254MB地图大小），而场景图以可预测成本提供语义抽象。

Conclusion: 论文提供了首个独立测量的α参考值和α感知的预算算法，使从业者能够在实施前评估目标硬件上的部署可行性。内存架构而非范式标签决定了部署可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的T2I扩散模型概念擦除方法存在严重问题：特征校正方法容易过度校正，令牌级干预缺乏语义粒度，且两种方法都容易导致语义漂移或表示崩溃，需要更精确可控的解决方案。

Method: 提出动态语义引导（DSS）框架，包含两个核心组件：1）敏感语义边界建模（SSBM）自动发现安全语义锚点；2）敏感语义引导（SSG）利用交叉注意力特征进行精确检测，并通过闭式解进行校正。

Result: DSS实现了91.0%的平均擦除率，显著优于现有SOTA方法（从18.6%到85.9%），同时对输出保真度影响最小。

Conclusion: DSS为T2I扩散模型提供了一种轻量级、无需训练、可解释且可控的概念擦除框架，有效解决了现有方法的局限性，在安全内容生成方面具有重要应用价值。
...
TL;DR: CLWM模型通过DINOv3特征作为生成目标解耦交互语义与视觉噪声，采用双状态测试时训练内存实现O(1)内存占用，使用推测异步推理减少50%延迟，结合EmbodiChain框架实现高效策略扩展，在双臂仿真和零样本sim-to-real迁移中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前生成式世界-动作模型在部署中存在三个主要瓶颈：冗余的像素级重建导致计算浪费，O(T)内存缩放限制了长时程任务，顺序推理延迟影响了实时性。这些瓶颈严重制约了机器人操作任务的实用化部署。
...
Conclusion: CLWM通过解耦语义表示、优化内存和延迟、以及高效的策略扩展框架，成功解决了生成式世界模型在机器人操作中的关键部署瓶颈，为实现鲁棒、高效的机器人控制系统提供了有效方案。
...
TL;DR: 提出Saccade Attention Network，通过学习人眼扫视机制实现稀疏注意力，将transformer计算量减少近80%


<details>
  <summary>Details</summary>
Motivation: 传统transformer的自注意力机制存在二次复杂度问题，限制了序列长度。人类视觉通过扫视机制聚焦关键特征，大大减少计算量，这启发了稀疏注意力的研究。

Method: 提出Saccade Attention Network，利用大型预训练模型学习注意力位置，预处理图像，通过仅关注关键特征来大幅减少输入序列长度。

Result: 实验结果表明，该方法能减少接近80%的计算量，同时保持相似的性能表现。

Conclusion: 通过学习人眼扫视机制的稀疏注意力，可以有效解决transformer的序列长度限制问题，显著降低计算成本。
...
TL;DR: PhyLAA-X：一种物理条件化的深度伪造检测器，通过注入光学流、镜面反射和rPPG等物理特征，在跨生成器、压缩和对抗攻击场景下显著提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测器在域内表现优异，但在跨生成器迁移、重度压缩和对抗扰动下性能下降。核心问题在于语义伪影学习与物理不变量的解耦，如光流不连续性、镜面反射不一致性和心脏调制反射等物理特征被忽视或仅作为后处理特征。

Method: 提出PhyLAA-X，在LAA-X基础上注入三种端到端可微的物理特征：光学流旋度、镜面反射偏度和空间上采样的rPPG功率谱。通过交叉注意力门控和共振一致性损失，强制网络学习语义不一致与物理违规共现的篡改边界。采用高效时空集成架构（EfficientNet-B4+BiLSTM等）和不确定性感知自适应加权。

Result: 在多个基准测试中取得SOTA：FaceForensics++ (c23) 97.2%准确率/0.992 AUC-ROC；Celeb-DF v2 94.9%/0.981；DFDC 90.8%/0.966。在跨生成器设置中比最强基线LAA-Net提升4.1-7.3%，在ε=0.02 PGD-10攻击下保持79.4%准确率。单骨干消融实验显示PhyLAA-X带来4.2%跨数据集AUC增益。

Conclusion: PhyLAA-X通过将物理不变性特征直接注入注意力机制，显著提升了深度伪造检测的鲁棒性，特别是在跨域和对抗攻击场景下。该方法为物理感知的伪造检测提供了新范式，相关代码和数据集已开源。
...
TL;DR: 本文提出两种无需重新训练的CLIP检索优化方法：基于匈牙利匹配的邻域重排序和查询条件局部引导，通过改善局部几何一致性提升检索性能。


<details>
  <summary>Details</summary>
Motivation: CLIP检索通常被视为共享嵌入空间中的点相似度问题，虽然实现了全局跨模态对齐，但局部几何不一致导致检索失败，如邻近项排序错误、系统性混淆（五边形vs六边形）和结果集分散。现有方法主要优化点相关度或微调，本文则从邻域对齐角度解决这一问题。

Method: 提出两种方法：1）基于匈牙利匹配的邻域级重排序，奖励结构一致性；2）查询条件局部引导，利用查询周围对比邻域导出的方向重塑检索。这两种方法在推理时无需重新训练。

Result: 这些技术在属性绑定和组合检索任务上提升了检索性能。重排序方法奖励对齐，局部引导控制邻域结构。

Conclusion: 检索质量和可控性关键依赖于局部结构，可以在推理时利用局部结构改进而不需要重新训练模型。
...
<details>
  <summary>Details</summary>
Motivation: 脑部图像分割对神经疾病诊断和医学图像计算至关重要，但现有深度学习方法在处理分割不确定性方面存在不足，需要更好的损失函数来优化模型。

Method: 提出一种结合分类交叉熵（CCE）损失函数和基于模糊逻辑的模糊熵的新型损失函数，通过模糊逻辑处理像素分类中的固有不确定性。

Result: 在IBSR和OASIS两个公开基准数据集上，使用U-Net和U-Net++架构评估，相比CCE损失函数，新损失函数在多个性能指标上表现更好，并能有效处理训练中的不确定性。

Conclusion: 该模糊逻辑增强的损失函数不仅能提高脑部图像分割性能，还能增强模型预测的可靠性，为医学图像分割中的不确定性处理提供了有效解决方案。
...
TL;DR: 提出轻量级Transformer架构，通过统一token化机制融合多种fNIRS表征，实现疼痛识别，在保持计算紧凑的同时获得竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 疼痛评估具有重要临床和社会意义，但现有方法复杂且计算成本高，需要一种轻量级、能融合多表征的自动化评估方法。

Method: 采用轻量级Transformer架构，通过统一token化机制融合原始波形和功率谱密度两种fNIRS表征，使用结构化分割方案控制局部聚合和全局交互粒度。

Result: 在AI4Pain数据集上评估，模型在保持计算紧凑的同时获得竞争性疼痛识别性能，适合GPU和CPU硬件上的实时推理。

Conclusion: 提出的轻量级Transformer架构能有效融合多fNIRS表征进行疼痛识别，在性能和计算效率间取得良好平衡，适合临床应用。
...
TL;DR: LayerCache：基于层感知的Transformer缓存框架，通过分层缓存决策显著提升Flow Matching模型的推理速度，同时保持生成质量


<details>
  <summary>Details</summary>
Motivation: 现有Flow Matching模型使用大型Transformer进行迭代去噪，推理成本高昂。现有缓存方法将Transformer视为整体，无法利用不同层组的速度动态异质性：浅层稳定适合缓存，深层变化大需要完整计算。

Method: 提出LayerCache框架：1）将Transformer划分为层组；2）为每个层组独立做出缓存决策；3）引入自适应JVP跨度K选择机制，基于层组稳定性平衡估计精度和计算节省；4）通过贪心预算分配算法解决三维调度问题（时间步、层组、JVP跨度）。

Result: 在Qwen-Image（1024x1024，50步）上：PSNR 37.46 dB（比MeanCache提升5.38 dB），SSIM 0.9834，LPIPS 0.0178（比MeanCache降低70%），1.37倍加速。在所有缓存方法的质量-速度帕累托前沿上表现最优。

Conclusion: LayerCache通过利用Transformer层间的异质性动态，实现了更精细的缓存策略，在显著提升推理速度的同时保持了图像生成质量，超越了现有缓存方法。
...
TL;DR: 提出HQA-VLAttack框架，用于黑盒对抗攻击视觉语言预训练模型，通过文本和图像两阶段攻击，在保证语义一致性的同时提升攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有黑盒对抗攻击方法要么依赖复杂的迭代交叉搜索策略导致查询次数过多，要么只考虑降低正样本对的相似度而忽略负样本对，影响攻击性能。

Method: 采用两阶段攻击：文本攻击阶段使用counter-fitting词向量生成替代词集保证语义一致性；图像攻击阶段先通过层重要性引导策略初始化对抗样本，再用对比学习优化扰动，同时降低正样本对相似度并提升负样本对相似度。

Result: 在三个基准数据集上的实验结果表明，HQA-VLAttack在攻击成功率方面显著优于现有基线方法。

Conclusion: HQA-VLAttack是一个简单有效的框架，能够生成高质量的对抗样本，显著提升对视觉语言预训练模型的攻击成功率。
...
### [16] [Semantically Stable Image Composition Analysisvia Saliency and Gradient Vector Flow Fusion](https://arxiv.org/abs/2604.16500)
*Armin Dadras,Robert Sablatnig,Franziska Proksa,Markus Seidl*

Main category: cs.CV

TL;DR: VFCNet提出了一种基于视觉注意力流和几何结构的低层表示方法，通过融合显著性和边缘信息到梯度向量流场中，实现了对摄影构图的可靠计算评估。


<details>
  <summary>Details</summary>
Motivation: 需要一种能够区分空间布局同时对语义内容具有鲁棒性的特征表示，以实现对摄影构图的可靠计算评估。构图可以被理解为视觉注意力在几何结构上的流动。

Method: 提出VFCNet模型，将显著性和边缘信息融合到梯度向量流(GVF)场中，计算双流GVF表示，通过注意力机制整合，并使用DINOv3骨干网络提取多尺度流特征。

Result: 在PICD基准测试上取得最先进性能(CDA-1: 0.683, CDA-2: 0.629)，比之前最佳方法分别提升33.1%和36.1%。同时发现简单的分类器在自监督DINOv3特征上显著优于专门的构图模型。

Conclusion: 基于视觉注意力流和几何结构的低层表示方法能够有效评估摄影构图，VFCNet在构图评估任务上表现出色，同时自监督视觉特征对构图分析具有强大潜力。
...
TL;DR: 提出一种基于拓扑感知的层剪枝框架，用于高效压缩大型视觉语言模型，通过持久同调分析层间拓扑一致性，实现自适应剪枝


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型计算和内存成本高，难以在资源受限场景部署。现有层剪枝方法依赖局部相似度或静态代理信号，无法捕捉跨模型深度的全局动态表示演化，容易误删关键过渡层

Method: 将层间隐藏状态表示为点云，使用单纯复形建模其演化，通过zigzag持久同调量化层间拓扑一致性，实现自适应剪枝以保留关键表示过渡

Result: 在多种多模态基准测试上的广泛实验表明，该框架在不同稀疏度下始终优于现有剪枝方法

Conclusion: 提出的拓扑感知层剪枝框架能有效压缩大型视觉语言模型，保持性能的同时显著降低计算成本
...
TL;DR: Motif-Video 2B是一个高效的小规模视频生成模型，通过架构分离和共享交叉注意力机制，在仅使用少于1000万视频片段和10万H200 GPU小时的有限资源下，实现了超越14B参数模型的视频生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统视频生成模型需要海量数据、巨大参数量和计算资源，本文探索是否能在有限预算下（少于1000万视频片段和10万H200 GPU小时）实现高质量文本到视频生成。核心观点是模型容量的组织方式（而不仅仅是容量大小）是关键。

Method: 1. 共享交叉注意力机制：增强长视频序列的文本控制能力
2. 三部分主干架构：分离早期融合、联合表示学习和细节细化功能
3. 高效训练方案：基于动态令牌路由和早期特征对齐到冻结预训练视频编码器

Result: 在VBench上达到83.76%的评分，超越了Wan2.1 14B模型，同时仅使用其1/7的参数和显著更少的训练数据。分析显示后期块比标准单流基线具有更清晰的跨帧注意力结构。

Conclusion: 精心设计的架构专业化结合面向效率的训练方案，可以缩小甚至超越通常与更大视频模型相关的质量差距，为高效视频生成提供了新方向。
...
TL;DR: 评估17个多模态大语言模型在复杂医疗表格数字化任务上的表现，发现最新Google和OpenAI模型达到85%准确率，提示优化显著提升性能，为自动化手写文档数字化提供可行路径。


<details>
  <summary>Details</summary>
Motivation: 手动数字化结构化手写文档成本高、速度慢，特别是在中低收入国家。需要评估多模态大语言模型在复杂医疗表格数字化任务上的表现，探索自动化解决方案的可行性。

Method: 使用包含日期、结构化印刷文本、手写回答和显著变异性的真实世界医疗表格，对17个前沿多模态大语言模型和开源模型进行基准测试，包括GPT、Claude、Gemini等模型，并评估提示优化对性能的影响。

Result: 较小或较旧模型表现不佳，但最新Google和OpenAI模型在离散字段上达到约85%准确率，加权F1分数约90%。GPT 5.4在噪声日期提取和可靠性方面表现最佳（最低幻觉率6%），Claude Sonnet 4.6在格式化字段上平均性能最好，Gemini 3.1整体表现最优（最低自由文本错误率）。提示优化使宏观精度、召回率和F1提升超过60%，但对加权指标改善有限（仅2-5%）。

Conclusion: 多模态大语言模型的快速改进为复杂手写工作流程的完全自动化数字化提供了有希望的途径，特别适用于中低收入国家，能够显著降低成本和提升效率。
...
TL;DR: 提出结合DINOv2和增强LSTM的混合模型，使用有限的每日胚胎图像预测囊胚形成，在真实数据集上达到96.4%准确率。


<details>
  <summary>Details</summary>
Motivation: 体外受精中胚胎选择依赖人工检查大量延时成像数据，但许多诊所缺乏完整系统，只能获得有限每日图像，需要从这些有限数据预测囊胚形成。

Method: 提出新颖混合模型：DINOv2提取胚胎图像特征，增强LSTM（含多头注意力层）分析时间序列发展模式，生成最终预测。

Result: 在704个胚胎视频真实数据集上达到96.4%准确率，优于现有方法，且在帧缺失情况下表现良好。
...
TL;DR: MedThinkVQA是一个专家标注的多图像医学问答基准，要求模型整合跨视图证据进行诊断推理。现有大语言模型在该任务上表现不佳，主要瓶颈在于跨图像证据的提取、对齐和组合。


<details>
  <summary>Details</summary>
Motivation: 真实临床诊断通常需要整合多个图像证据，而现有医学QA基准大多只关注单图像解读。需要评估模型在多图像临床推理中的能力，特别是跨视图证据整合的挑战。

Method: 构建了MedThinkVQA数据集，包含8,067个病例（720个测试用例），平均每个病例6.62张图像，远高于先前工作。提供中间监督和步骤级评估，分析模型在图像解读、跨视图整合和推理各阶段的表现。

Result: 最佳闭源模型（Claude-4.6-Opus、Gemini-3-Pro、GPT-5.2-xhigh）准确率仅57.2%、55.3%、54.9%。开源模型表现更差。超过70%的错误源于图像读取和跨视图整合。提供专家提示能提升性能，而模型自生成中间结果会降低准确率。

Conclusion: 多图像医学推理的主要挑战不是推理长度，而是跨真实世界多模态临床输入的可靠证据提取、对齐和组合机制。当早期证据提取薄弱时，增加推理计算量收益有限。
...
TL;DR: 提出一种新的变分方法，通过显式考虑SDF梯度的跳跃集（即表面的中轴），计算点云的高精度全局符号距离函数。


<details>
  <summary>Details</summary>
Motivation: 现有方法在计算点云的全局符号距离函数时，往往难以准确处理中轴区域，导致全局精度不足。需要一种能够显式考虑梯度不连续性并确保全局准确性的方法。

Method: 采用高阶变分公式，显式考虑SDF梯度的跳跃集（中轴），沿梯度方向施加线性增长约束。使用Ambrosio-Tortorelli类型的相场近似使问题可计算，相场函数隐式描述中轴。针对无定向点云，使用神经网络同时近似SDF和相场函数。

Result: 实验表明该方法在近场和全局都具有高精度。与其他方法的定量和定性比较显示了所提方法的优势。

Conclusion: 通过显式考虑中轴并使用相场近似，该方法能够从无定向点云计算高精度的全局符号距离函数，在准确性和计算可行性之间取得了良好平衡。
...
TL;DR: SynthPID：通过从真实图纸中提取管道拓扑结构生成的合成P&ID数据集，显著提升了P&ID数字化中边缘检测的准确性，仅使用合成数据训练就能达到接近真实数据训练的效果。


<details>
  <summary>Details</summary>
Motivation: P&ID数字化自动化对工厂运营有重要价值，但进展受限于数据问题：工程图纸是专有的，公开基准只有12张标注图像。之前的合成增强方法因模板生成器随机散布符号，产生的图与真实工厂不符，导致边缘检测准确率仅约33%。

Method: 1. 引入SynthPID语料库，包含665张合成P&ID，其管道拓扑直接基于真实图纸生成；2. 采用基于补丁的Relationformer模型，适配高分辨率图表。

Result: 仅使用SynthPID训练（未接触任何真实P&ID）的模型在PID2Graph OPEN100上达到63.8±3.1%的边缘mAP，与真实数据训练的结果仅差8个百分点。与模板生成方法相比，性能显著提升，确认生成质量而非模型选择是性能关键。缩放研究表明，超过约400张合成图像后收益趋于平缓。

Conclusion: 合成数据的结构质量（而非视觉质量）是P&ID数字化的关键瓶颈。通过从真实图纸中提取拓扑结构生成合成数据，可以显著提升性能，接近真实数据训练的效果。数据多样性而非数量是主要限制因素。
...
<details>
  <summary>Details</summary>
Motivation: 自回归视觉语言模型存在token-by-token解码的推理瓶颈，而直接将预训练自回归模型转换为大块扩散模型会导致显著性能下降，需要一种有效的转换框架。

Method: 结合渐进式监督块合并（逐步增大解码块大小）和阶段内扩散模型蒸馏（从固定小块扩散锚点恢复大块性能），采用混合噪声调度器提高鲁棒性，以及内存友好训练处理长多模态序列。

Result: BARD-VL在≤4.4M数据下将Qwen3-VL的多模态能力成功转移到大型扩散模型，在4B和8B规模上达到可比开放扩散模型的新SOTA，同时实现高达3倍解码吞吐量加速。

Conclusion: BARD框架有效解决了自回归到扩散模型转换的性能损失问题，证明了扩散模型内蒸馏比直接自回归到扩散蒸馏更有效，为高效多模态解码提供了实用解决方案。
...
TL;DR: 论文提出PriceBlind攻击框架，利用视觉主导幻觉在多模态大语言模型中制造对抗性攻击，使智能体在价格约束场景下做出非理性决策。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型在金融交易等高风险场景的应用增多，其对抗鲁棒性研究不足。作者发现视觉主导幻觉现象，即微小的视觉线索可以覆盖文本价格证据，导致智能体做出非理性决策。

Method: 提出PriceBlind攻击框架，利用CLIP编码器的模态间隙，通过语义解耦损失将图像嵌入与低成本价值锚点对齐，同时保持像素级保真度，生成难以察觉的对抗性截图。

Result: 在E-ShopBench上，PriceBlind在白盒评估中达到约80%的攻击成功率；在简化单轮坐标选择协议下，Ensemble-DI-FGSM在GPT-4o、Gemini-1.5-Pro和Claude-3.5-Sonnet上实现35-41%的迁移攻击成功率。

Conclusion: 多模态大语言模型在金融交易场景中存在严重的安全漏洞，需要加强对抗鲁棒性研究。鲁棒编码器和Verify-then-Act防御策略能显著降低攻击成功率，但会牺牲一定的清洁准确率。
...
TL;DR: 本文对文本到图像生成模型的公平性研究进行了系统性综述，提出了偏差类型和公平性概念的分类法，并探讨了目标公平性与阈值公平性之间的差距，最后提出了超越描述性度量的可操作公平性框架。


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成模型虽然广泛应用，但经常表现出社会刻板印象，现有研究存在概念模糊性（如"偏差"和"公平性"缺乏明确定义），需要系统梳理和澄清。

Method: 1. 对T2I公平性文献进行系统性综述；2. 建立偏差类型和公平性概念的分类法；3. 分析"目标公平性"（规范性理想）与"阈值公平性"（可操作标准）之间的差距；4. 调查从提示工程到扩散过程操作的各种缓解策略。

Result: 1. 建立了T2I公平性研究的系统性分类框架；2. 识别了目标公平性与阈值公平性之间的重要差距；3. 全面梳理了现有的偏差缓解方法；4. 提出了超越描述性度量的新公平性操作化框架。

Conclusion: 需要从描述性度量转向基于目标的严格测试框架，为更负责任的可生成AI开发提供可操作的公平性评估方法，推动该领域向更严谨和可问责的方向发展。
...
TL;DR: SmoGVLM：一种小型图增强视觉语言模型，通过图神经网络整合结构化知识，使小模型性能提升达16.24%，超越更大模型


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型在知识密集型推理中存在幻觉问题和知识基础薄弱的问题，需要更高效、更可靠的多模态推理系统

Method: 提出SmoGVLM，使用图神经网络将结构化知识与视觉、文本模态整合，研究从1.3B到13B不同规模模型的效果

Result: 采用该方法训练的小型模型性能提升达16.24%，超越更大规模的视觉语言模型和强微调基线

Conclusion: 结构化知识增强为高效、小规模多模态推理系统提供了巨大潜力
...
TL;DR: 提出一种基于多单目摄像头的快速在线3D多目标跟踪与姿态估计方法，仅需2D边界框和姿态检测，无需3D训练数据或复杂深度学习模型
...
Method: 使用贝叶斯最优多目标跟踪滤波器的高效实现，仅依赖公开可用的预训练2D检测模型，无需3D训练数据

Result: 算法在保持准确性的同时显著快于现有最先进方法，在摄像头间歇性断开或重新连接的情况下仍能保持鲁棒性能

Conclusion: 该方法提供了一种高效实用的3D多目标跟踪与姿态估计解决方案，仅需2D检测输入，在计算效率和鲁棒性方面表现出色
...
TL;DR: 提出一种新颖的隐私保护语义分割方法，可为每个客户端和图像使用独立密钥，在加密图像上进行模型训练和推理，并通过在训练中应用图像加密来缓解性能下降。


<details>
  <summary>Details</summary>
Motivation: 当前隐私保护语义分割方法通常需要共享密钥或使用同态加密，存在密钥管理复杂和计算开销大的问题。需要一种能为每个客户端和图像使用独立密钥的轻量级隐私保护方案。

Method: 模型创建者和每个客户端使用本地生成的密钥加密图像，在加密图像上进行模型训练和推理。为缓解性能下降，在模型训练中也应用图像加密方法，而不仅仅是测试图像生成。

Result: 在Cityscapes数据集上使用基于视觉变换器（SETR）的模型进行实验，验证了所提方法的有效性，能够在保护隐私的同时保持语义分割性能。

Conclusion: 该方法实现了无需共享密钥的隐私保护语义分割，通过训练和推理阶段的图像加密，在保护客户端隐私的同时维持了模型性能，为分布式视觉任务提供了可行的隐私保护方案。
...
TL;DR: 提出一个专家标注的胚胎图像与形态描述数据集，用于微调视觉-语言基础模型，实现基于自然语言的胚胎评估，提高AI辅助胚胎选择的解释性和透明度。


<details>
  <summary>Details</summary>
Motivation: 当前AI辅助胚胎选择方法存在局限性：需要适应特定临床数据、依赖时差培养箱、缺乏可解释性。现代患者质疑专家决策，特别是治疗失败时。需要基于证据的决策解释来支持透明决策和尊重患者的沟通。

Method: 创建专家标注的胚胎图像数据集，包含胚胎图像和对应的自然语言形态描述（细胞周期、发育阶段、形态特征）。使用该数据集微调现代视觉-语言基础模型，使模型能够生成胚胎描述，并利用这些描述自动从文献中提取科学证据。

Result: 提出了一个支持基于语言的、可解释的、透明的自动化胚胎评估研究的数据集。该数据集能够使模型随时间学习并提高准确性，预测的胚胎描述可用于自动提取文献证据。

Conclusion: 该数据集有潜力显著增强决策过程，改善患者预后，支持基于证据的决策制定和透明的患者沟通，推动可解释AI在胚胎选择中的应用。
...
TL;DR: 医学图像AI对抗攻击评估中，仅使用攻击成功率(ASR)单一指标存在不足，需要结合感知质量、扰动强度等多指标框架进行全面评估。


<details>
  <summary>Details</summary>
Motivation: 深度学习系统在医学图像分析中日益普及，但其对抗扰动的脆弱性引发临床部署担忧。当前评估主要依赖攻击成功率(ASR)这一二元指标，但该指标未考虑扰动强度、图像感知质量、跨架构攻击可迁移性等因素，导致评估不完整。随着视觉Transformer(ViT)等复杂架构挑战CNN的主导地位，需要更全面的评估方法。
...
Result: 研究发现一致模式：感知和失真指标彼此强相关，但与ASR相关性极小。这一规律适用于CNN和ViT架构。结果表明ASR单独作为对抗鲁棒性和可迁移性指标是不充分的。

Conclusion: 医学AI对抗风险评估需要多指标框架，不仅要考虑攻击效果，还要包含攻击方法和相关开销。单一ASR指标无法全面反映对抗脆弱性，特别是在不同架构间评估时。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究仅将3D重建管道作为整体进行反向传播攻击，未能揭示特定模块的新漏洞。本文认为SfM初始化作为许多重建系统的几何核心，可以成为攻击目标以实现跨系统的可迁移毒化效果。

Method: 提出PoInit-of-View方法，优化对抗扰动以在对应3D点的投影处故意引入跨视图梯度不一致性。这些不一致性破坏关键点检测和特征匹配，从而腐蚀SfM中的姿态估计和三角测量，最终导致渲染视图质量下降。并提供理论分析连接跨视图不一致性与对应关系崩溃。

Result: 实验结果表明PoInit-of-View在多种3D重建系统和数据集上有效，在黑盒迁移设置下（如3DGS到NeRF），PSNR比单视图基线提高25.1%，SSIM提高16.5%。

Conclusion: 通过针对SfM初始化模块的攻击，实现了对多种3D重建系统的可迁移毒化，揭示了3D重建系统中特定几何模块的新安全漏洞。
...
TL;DR: BookAgent是一个安全感知的多智能体协作框架，用于生成高质量、安全的视觉叙事故事书，通过联合规划、脚本编写、插图和全局修复不一致性，实现端到端的故事书合成。


<details>
  <summary>Details</summary>
Motivation: 现有大型生成模型在多模态生成方面取得进展，但生成插图故事书仍面临挑战：先前工作主要将任务分解为独立阶段，导致整体多模态基础有限；同时，安全对齐研究主要针对纯文本或图像生成，很少将儿童特定安全约束整合到叙事规划和序列级多模态验证中。

Method: BookAgent采用安全感知的多智能体协作框架，从用户草稿出发进行端到端故事书合成，包括联合规划、脚本编写、插图和全局修复不一致性。通过动态校准页面级文本脚本与视觉布局的对齐，确保精确的多模态基础；同时从时间维度校准整体一致性，通过验证-修正机制处理角色身份和叙事逻辑的全局不一致性。

Result: 大量实验表明，BookAgent在叙事连贯性、视觉一致性和安全合规性方面显著优于现有方法，为复杂多模态创作中的可靠智能体提供了稳健范式。

Conclusion: BookAgent提出了一个用于高质量、安全感知视觉叙事的创新框架，通过多智能体协作和全局一致性校准，解决了现有故事可视化模型的局限性，为可靠的多模态创作智能体提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 3D指纹识别相比传统2D系统具有接触式、更卫生安全、能捕捉深度和形状信息等优势，但存在拓扑高度复杂导致脊谷模式提取困难，以及配准问题限制采集过程等挑战。

Method: 使用B样条曲线拟合展开3D指纹点云以减轻高度变化和配准限制，然后将展开的点云通过映射相对高度转换为灰度图像，最后应用传统2D指纹识别方法进行识别。

Result: 在三个实验中分别获得0.2072%、0.26%和0.22%的等错误率，优于现有方法；在跨会话实验中，对具有不同配准的指纹识别达到1.50%的EER，超越了3D指纹展平技术。

Conclusion: 提出的方法有效解决了3D指纹识别中的高度变化和配准问题，通过将3D点云转换为灰度图像并应用传统2D识别技术，实现了优越的识别性能。
...
TL;DR: 提出3D-ARD+模型，通过序列化生成方式统一场景布局和物体生成，解决现有方法无法同时生成复杂场景布局和精细物体的问题


<details>
  <summary>Details</summary>
Motivation: 现有文本到场景生成方法要么只生成场景布局，要么只生成物体，很少能同时生成两者。即使有LLM帮助，生成的场景布局也往往过于简单，且难以处理包含物体形状、外观和空间排列等非平凡描述的文本输入。

Method: 提出序列化文本到场景生成范式，核心是3D-ARD+模型。该模型统一了多模态token序列的自回归生成和下一个物体3D潜变量的扩散生成。生成过程分两步：1）在场景空间中生成粗粒度3D潜变量；2）在更小的物体空间中生成细粒度3D潜变量，可解码为精细的几何和外观。

Result: 构建了包含23万室内场景和配对文本指令的大型数据集用于训练。评估了70亿参数的3D-ARD+模型，在具有挑战性的场景上展示出能够按照文本指令中规定的非平凡空间布局和语义生成和放置物体的能力。

Conclusion: 提出的3D-ARD+模型通过序列化生成范式，成功解决了现有方法在同时生成复杂场景布局和精细物体方面的不足，能够更好地遵循文本指令中的空间和语义约束。
...
TL;DR: PA-TCNet：一种病理感知的时间校准框架，通过生理学引导的目标细化来提升中风患者运动想象脑机接口的跨被试解码性能，解决了病理慢波活动和伪标签不稳定的问题。


<details>
  <summary>Details</summary>
Motivation: 中风患者的脑电图解码面临两个主要挑战：1）病灶相关的异常时间动态（如病理慢波活动）会误导现有适应方法；2）目标域伪标签不稳定且存在显著的被试间异质性。这些问题严重影响了跨被试解码的泛化能力。

Method: PA-TCNet包含两个协调组件：1）病理感知节律状态Mamba模块（PRSM），将EEG时空特征分解为慢变节律上下文和快速瞬态扰动，将融合的病理上下文注入选择性状态传播以捕获异常时间动态；2）生理学引导目标校准模块（PGTC），构建源域感觉运动感兴趣区模板，施加生理一致性约束并动态细化目标域伪标签。

Result: 在两个独立的中风EEG数据集（XW-Stroke和2019-Stroke）上进行的留一被试实验显示，平均准确率分别达到66.56%和72.75%，优于现有最先进的基线方法。

Conclusion: 联合建模病理时间动态和生理约束的伪监督可以为个性化中风后运动想象脑机接口康复提供更稳健的跨被试初始化。该方法在解码性能和适应性方面表现出显著优势。
...
TL;DR: SeeTN框架通过构建语义嵌入空间和亲和一致性度量，有效缓解注视估计中的标签噪声问题，提升跨域泛化性能


<details>
  <summary>Details</summary>
Motivation: 现有泛化注视估计方法忽视了标签噪声对模型泛化性能的影响，而精确注视标注难以获取，标签噪声问题普遍存在

Method: 提出See-Through-Noise框架：1）通过原型变换构建语义嵌入空间，保持注视特征与连续标签的拓扑一致性；2）测量特征-标签亲和一致性区分噪声样本；3）在语义流形中引入亲和正则化，将干净样本的注视信息迁移到噪声样本

Result: 大量实验表明，SeeTN能有效缓解源域噪声的负面影响，在不损害源域准确率的情况下实现优越的跨域泛化性能

Conclusion: 该研究首次全面调查了标签噪声对注视估计泛化的负面影响，提出的SeeTN框架通过语义结构对齐和域不变注视关系增强，显著提升了模型对标签噪声的鲁棒性，强调了在泛化注视估计中显式处理噪声的重要性
...
TL;DR: 提出基于复杂正交匹配追踪和视觉Transformer的收缩期杂音自动分类系统，在CirCor DigiScope数据集上达到95.96%准确率。


<details>
  <summary>Details</summary>
Motivation: 收缩期杂音是心脏收缩期出现的额外心音，通常由湍流血流引起，其强度、音调和质量变化多样，需要精确识别以准确诊断心脏疾病。目前需要自动化系统来可靠分类这些杂音。

Method: 1. 特征提取模块：使用复杂正交匹配追踪将单个或多个杂音段投影到由多分辨率复杂Gabor基函数组成的冗余字典上，将投影权重分割并重塑为变分辨率时频特征矩阵。2. 分类模型：基于视觉Transformer，通过卷积神经网络对每个输入矩阵进行补丁标记化，将所有嵌入标记拼接后通过包含多头注意力、残差连接和卷积网络的编码器层处理。

Result: 在CirCor DigiScope数据集的四种收缩期杂音分类实验中，系统达到了95.96%的分类准确率。

Conclusion: 将多分辨率特征提取与基于Transformer的特征分类相结合，提高了心脏杂音识别的准确性和可靠性，为心脏疾病的自动诊断提供了有效工具。
...
TL;DR: 该研究提出了一种用于人类活动识别的两级网络架构，采用晚期融合和中间融合的双阶段特征融合策略，通过评估15种不同架构发现双融合策略优于仅使用晚期融合的方法。


<details>
  <summary>Details</summary>
Motivation: 人类活动识别中，神经网络（如CNN、LSTM、卷积LSTM）已表现出优异性能。开发多层次混合模型需要战略性地整合多个网络以发挥其互补优势，而组件结构安排是影响整体性能的关键因素。

Method: 研究探索了一种新颖的两级网络架构，采用双阶段特征融合：1）晚期融合（结合第一级网络输出），2）中间融合（整合第一级和第二级特征）。评估了15种不同的CNN、LSTM和卷积LSTM网络架构，比较了带/不带中间融合的晚期融合配置。

Result: 在两个公共基准数据集上的实验评估表明，同时包含晚期融合和中间融合的架构比仅依赖晚期融合的架构获得更高准确率。最优配置超越了基线模型，验证了其有效性。

Conclusion: 提出的两级网络架构与双阶段特征融合策略（晚期融合+中间融合）在人类活动识别任务中表现出优越性能，为开发更有效的混合模型提供了有效框架。
...
TL;DR: 该论文提出了Camo-M3FD基准数据集，用于跨光谱伪装行人检测，填补了安全关键场景中伪装人类检测的空白。


<details>
  <summary>Details</summary>
Motivation: 尽管多光谱检测（可见光+热成像）能改善恶劣能见度下的检测，但伪装行人的检测问题尚未被充分探索。现有的伪装目标检测基准主要关注生物物种，缺乏针对安全关键场景中与背景融合的人类目标的检测基准。

Method: 基于M3FD数据集构建Camo-M3FD基准，包含配准的可见光-热成像图像对。使用定量指标筛选确保高前景-背景相似度，提供像素级标注掩码，并建立基于最先进COD模型的标准化评估框架。

Result: 实验结果表明，热成像信号提供重要的定位线索，而多光谱融合对于细化结构细节至关重要。该基准为开发鲁棒的安全关键检测系统提供了基础资源。

Conclusion: Camo-M3FD填补了伪装行人检测的空白，为自动驾驶、机器人和监控等安全关键应用提供了重要的基准数据集和评估框架。
...
TL;DR: 提出一个摊销框架，用于多模态思维模型中的实时视觉归因流式处理，通过从注意力特征学习估计语义区域的因果效应，实现与详尽因果方法相当的忠实度，同时支持实时流式归因。


<details>
  <summary>Details</summary>
Motivation: 多模态思维模型（如从截图生成代码或从图像解决数学问题）的长推理轨迹需要基于视觉证据，但验证这种依赖具有挑战性：忠实的因果方法需要昂贵的重复反向传播或扰动，而原始注意力图虽然能即时访问，但缺乏因果有效性。

Method: 引入摊销方法，从注意力特征中编码的丰富信号直接学习估计语义区域的因果效应，实现实时视觉归因流式处理。

Result: 在五个不同的基准测试和四个思维模型上，该方法实现了与详尽因果方法相当的忠实度，同时支持视觉归因流式处理，用户可以在模型推理过程中观察基础证据，而不是事后。
...
TL;DR: MambaKick：基于预训练人体动作识别嵌入和Mamba状态空间模型的足球点球方向预测框架


<details>
  <summary>Details</summary>
Motivation: 足球点球中守门员需要在极短时间内预测射门方向，现有方法依赖显式运动学重建或手工生物力学特征，需要更高效实用的预测方法

Method: 利用预训练的人体动作识别（HAR）模型提取接触点为中心的视频片段嵌入，结合轻量级Mamba状态空间模型进行时序聚合，并加入场地侧和惯用脚等上下文元数据

Result: 在多个HAR骨干网络上，MambaKick均优于或匹配强基线，三分类准确率达53.1%，二分类达64.5%

Conclusion: 预训练HAR表示与高效状态空间时序建模相结合，是现实体育视频中低延迟意图预测的实用方向
...
TL;DR: 提出IncepDeHazeGAN，一种结合Inception模块和多层特征融合的GAN网络，用于单图像去雾任务，在多个数据集上达到SOTA效果


<details>
  <summary>Details</summary>
Motivation: 去雾技术对于提升云雾条件下拍摄图像的视觉质量很重要，特别是对于遥感数据的清晰恢复。现有方法需要改进以更好地处理不同雾霾条件。

Method: 提出IncepDeHazeGAN，结合Inception模块进行多尺度特征提取，采用多层特征融合设计实现特征重用，并应用Grad-CAM XAI技术分析网络关注区域

Result: 实验表明该网络在多个数据集上取得了最先进的结果，Grad-CAM可视化显示网络能够适应不同雾霾条件并关注相关区域

Conclusion: IncepDeHazeGAN通过Inception模块和多层特征融合有效提升了单图像去雾性能，XAI技术验证了网络的有效性，为遥感图像处理提供了有力工具
...
TL;DR: AVRT框架通过单模态教师模型生成高质量音视频推理轨迹，结合监督微调和强化学习训练多模态推理模型，在多个基准测试中取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 当前推理模型在文本领域取得显著进展，但将推理能力迁移到多模态场景（如音视频数据）仍面临挑战，主要原因是高质量多模态推理数据稀缺。

Method: 提出AVRT框架：1）使用专门模型分别生成视觉和音频推理轨迹；2）用LLM合并模型融合轨迹；3）先进行监督微调冷启动；4）再进行大规模数据的强化学习训练。

Result: 在7个音视频和音频基准测试中，3B和7B参数模型在可比规模模型中取得SOTA结果，包括超越OmniBench、DailyOmni（音视频）和MMAR（纯音频）。

Conclusion: AVRT建立了多模态推理模型的新训练流程，证明跨模态训练也能提升单模态任务性能，为解决多模态推理数据稀缺问题提供了有效方案。
...
TL;DR: IK-GAT：基于图注意力网络的轻量级前向传播模型，从3D关节位置重建全身关节方向，用于实时动画和机器人应用


<details>
  <summary>Details</summary>
Motivation: 在实时人体化身流水线中，通常只能获得稀疏的3D关节位置跟踪信号，但动画系统需要关节方向来驱动蒙皮。从位置恢复完整方向是欠约束问题（特别是骨轴扭转模糊），传统IK求解器依赖迭代优化，速度慢且对噪声敏感

Method: 提出IK-GAT（轻量级图注意力网络），通过骨骼父子图上的消息传递来利用运动学结构进行旋转推断。使用骨对齐世界框架表示来简化学习，该表示基于静止姿势骨框架，使扭转轴显式化。采用连续6D旋转表示，使用SO(3)上的测地线损失和可选的前向运动学一致性正则器进行训练

Result: IK-GAT仅需374K参数，在CPU上达到650+ FPS，优于基于VPoser的逐帧迭代优化（无需预热启动），成本显著降低，对初始姿势和输入噪声具有鲁棒性

Conclusion: IK-GAT能够从3D关节位置高效重建动画就绪的局部旋转，可直接驱动装配化身或转换为SMPL类身体模型的姿势参数，适用于实时和在线应用
...
TL;DR: 提出首个三模态（RGB、热成像、事件相机）无人机目标检测框架，通过双流分层视觉Transformer和门控交换机制实现多模态融合，并在新数据集上验证了性能提升。


<details>
  <summary>Details</summary>
Motivation: 无人机目标检测需要应对光照变化、运动模糊和动态场景等挑战，RGB模态在这些条件下性能下降。热成像在低光下保持对比度，事件相机保留微秒级时间边缘信息，但三模态融合尚未得到系统研究。

Method: 提出双流分层视觉Transformer框架，处理RGB、热成像和事件数据。在选定编码器深度使用模态感知门控交换（MAGE）进行传感器间通道和空间门控，以及双向令牌交换（BiTE）模块进行双向令牌级注意力与深度点积细化，生成保持分辨率的融合特征图用于标准特征金字塔和两阶段检测器。

Result: 引入包含10,489帧同步预对齐RGB-热成像-事件流和24,223个标注车辆的数据集。通过61个控制消融实验评估，三模态融合优于所有双模态基线，融合深度有显著影响，轻量级CSSA变体以最小成本恢复大部分性能提升。
...
TL;DR: 该论文系统评估了CNN、Transformer和预训练基础模型在滑坡检测任务上的性能，并比较了全微调与参数高效微调方法的效果。


<details>
  <summary>Details</summary>
Motivation: 滑坡检测对灾害响应和风险评估至关重要，但目前对现代分割架构和微调策略在该问题上的相对有效性了解不足。

Method: 使用GDCLD数据集，在一致的训练和评估协议下评估代表性CNN和Transformer分割模型以及大型预训练基础模型，并比较全微调与LoRA、AdaLoRA等参数高效微调方法。

Result: Transformer模型实现了强大的分割性能，参数高效微调可将可训练参数减少高达95%，同时保持与全微调相当的准确性。通过比较验证集和保留测试集性能分析了分布偏移下的泛化能力。

Conclusion: 该研究为滑坡检测任务提供了系统基准，表明Transformer架构具有优势，参数高效微调方法能显著减少计算成本而不牺牲性能，为实际应用提供了有价值的指导。
...
TL;DR: 人类在零样本条件下能够识别外观缺失的动作视频，但准确率低于自然视频。研究者开发的双流3D CNN模型结合了RGB（形态）和光流（运动）流，并加入相干门控机制，在两种外观缺失数据集上表现优于现有模型，缩小了与人类表现的差距。


<details>
  <summary>Details</summary>
Motivation: 研究人类和视觉模型是否能够零样本泛化到真实世界动作视频的外观缺失变换。虽然经典心理物理学研究表明人类在形状线索退化时仍能感知身体运动，但人类和模型是否能在未经训练的情况下识别外观缺失的动作视频尚不清楚。

Method: 1) 人类实验：22名参与者使用自然视频（UCF5数据集）训练识别5个动作类别，然后零样本测试两种外观缺失变换：密集噪声运动视频（AFD5数据集）和随机点外观缺失视频。2) 模型开发：构建双流3D CNN模型，结合RGB（形态）流和光流（运动）流，加入基于格式塔共同命运分组的相干门控机制。

Result: 1) 人类在两种外观缺失视频上的动作识别准确率显著高于随机水平，但低于自然视频。2) 开发的双流模型在两种外观缺失数据集上表现优于当代视频分类模型，缩小了与人类表现的差距。3) 运动流对外观缺失视频的泛化至关重要，而形态流提高了自然视频的性能。

Conclusion: 运动表征对于泛化到外观缺失视频至关重要，支持使用多流架构来建模基于视频的动作识别。研究结果强调了基于运动的表征在动作识别中的重要性，为理解人类动作感知的计算基础提供了新见解。
...
TL;DR: C-GenReg：无需训练的3D点云配准框架，结合世界级生成先验和视觉基础模型，通过生成RGB视图和概率融合实现跨域泛化


<details>
  <summary>Details</summary>
Motivation: 当前基于学习的3D点云配准方法难以泛化到不同传感模态、采样差异和环境变化。需要一种能跨域工作且无需训练的解决方案

Method: 1) 使用世界基础模型从几何点云生成多视角一致的RGB表示；2) 在图像域利用预训练视觉基础模型提取密集对应；3) 通过原始深度图将像素对应提升回3D；4) 提出"匹配后融合"概率冷融合方案，结合生成RGB分支和原始几何分支的对应后验

Result: 在室内（3DMatch、ScanNet）和室外（Waymo）基准测试中表现出强大的零样本性能和优越的跨域泛化能力。首次在真实室外LiDAR数据上成功运行生成式配准框架

Conclusion: C-GenReg展示了无需训练的3D点云配准框架的可行性，通过生成式转移和概率融合实现了跨域泛化，为实际应用提供了即插即用的解决方案
...
TL;DR: 提出了一种集成到3DETR架构中的多尺度注意力机制，通过上采样生成高分辨率特征图，在ScanNetv2数据集上提升了3D目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 点云数据中的3D目标检测面临稀疏性和缺乏全局结构的挑战，需要更好地捕捉局部几何和全局上下文信息。

Method: 在3DETR架构中集成多尺度注意力机制，引入上采样操作生成高分辨率特征图，以更好地检测较小和语义相关的物体。

Result: 在ScanNetv2数据集上，3DETR+MSA模型相比基线在mAP@25上提升近1%，在mAP@50上提升4.78%。

Conclusion: 将分层特征提取与注意力机制相结合能有效增强3D场景理解，但需要为轻量级模型适配上采样策略。
...
TL;DR: 提出iDoc模型，结合改进编码器和开放集检测器，在文档检索和模式识别任务上实现10倍速度提升，并在小尺寸非方形查询上达到新的SOTA精度0.612


<details>
  <summary>Details</summary>
Motivation: 随着数字书籍的大规模普及，需要通过图形模式搜索文档集合变得至关重要。现有历史文档检索和模式识别方法存在精度不足（特别是小尺寸非方形查询精度仅0.427）和处理时间过长（在DocExplore数据集上需要7秒）的问题。

Method: 提出iDoc模型，采用改进的编码器架构，通过自监督策略训练，结合开放集检测器加速搜索过程，并使用非极大值抑制减少误报。

Result: 模型在模式识别和文档检索任务上取得竞争性结果，速度提升10倍，在小尺寸非方形查询上达到新的SOTA精度0.612。

Conclusion: iDoc模型有效解决了现有方法在精度和速度上的不足，为大规模数字文档集合的图形模式搜索提供了高效解决方案。
...
TL;DR: 本文提出了一种无需训练的智能代理框架，用于自动化脑MRI分析，通过让大语言模型协调外部专业工具，解决了当前模型缺乏原生3D空间推理能力的问题。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在通用视觉问答中表现优异，但缺乏原生3D空间推理能力，无法直接分析CT或MRI等容积医学影像。虽然新兴的智能代理AI提供了通过协调外部工具来解决问题的方案，但在复杂多步骤放射学工作流中的可行性尚未充分探索。

Method: 提出无需训练的智能代理流程，让LLMs（GPT-5.1、Gemini 3 Pro、Claude Sonnet 4.5）协调现成的领域专用工具，自主执行复杂的端到端工作流，包括预处理（颅骨剥离、配准）、病理分割（胶质瘤、脑膜瘤、转移瘤）和容积分析。

Result: 系统能够自主执行从单扫描分割和容积报告到需要多时间点比较的纵向响应评估等日益复杂的放射学任务。比较了单代理模型与多代理"领域专家"协作的架构设计影响，并发布了基于公开BraTS数据的基准数据集。

Conclusion: 智能代理AI能够通过工具使用解决高度复杂的神经放射学图像分析任务，无需训练或微调，为医学影像分析提供了新的解决方案。
...
TL;DR: AW4RE是一个面向物理感知的生成世界模型，通过4D信息检索和条件生成，为部分可观测环境中的传感决策提供仿真环境


<details>
  <summary>Details</summary>
Motivation: 物理感知在动态大环境中面临部分可观测的挑战，传统强化学习在全可观测问题上成功但在POMDP等部分可观测问题上进展有限，真实世界探索成本高昂且仿真到现实的视角转换存在未观测视角问题

Method: AW4RE结合4D信息检索、动作条件几何支持与时间一致性、条件生成补全，构建传感器原生的代理环境，估计动作条件的观测过程

Result: AW4RE在极端视角变化、时间间隔和稀疏几何支持情况下，比几何感知生成基线产生更可靠和一致的预测

Conclusion: AW4RE为部分可观测决策问题提供了有效的传感查询探索工具，通过生成世界模型解决了真实世界探索成本高和仿真到现实视角不匹配的问题
...
TL;DR: 本文提出了一种顺序输入压缩机制，通过在前填充阶段进行结构感知的键值缓存压缩，在固定内存预算下显著降低峰值内存使用，同时保持生成性能。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型处理更高分辨率图像和更长视频序列，视觉令牌在键值缓存中占用大量内存，成为推理瓶颈。现有方法通常在处理完所有输入后才压缩缓存，导致前填充阶段峰值内存使用过高。

Method: 提出顺序输入压缩机制，利用MLLMs固有的结构规律性和表示冗余性，在前填充过程中进行结构感知的键值缓存压缩，强制实施固定内存预算。

Result: 该方法显著降低了峰值内存使用，同时仅带来最小的性能下降，保持了生成性能，实现了更实用和内存高效的多模态推理。

Conclusion: 通过利用MLLMs的结构特性进行顺序压缩，可以在固定内存预算下有效控制内存增长，为大规模多模态推理提供更实用的解决方案。
...
TL;DR: 提出自动化高通量显微镜系统，结合H∞鲁棒机械控制与深度学习流水线，用于智利Bio Bio地区花粉粒的计数、分类和形态分析，相比传统人工方法实现6倍加速和95.8%分类召回率。


<details>
  <summary>Details</summary>
Motivation: 传统蜜源植物花粉学分析耗时且主观，每个样本需要4-6小时人工分析。需要开发自动化系统提高分析效率和准确性。

Method: 集成H∞鲁棒机械控制的自动化显微镜系统，使用U²-Net进行显著物体检测，采用DINOv2 Vision Transformer骨干网络通过深度度量学习进行分类，并整合梯度加权注意力提供可解释的特征标注。

Result: 系统达到95.8%的分类召回率，相比专家手动分析实现6倍处理速度提升。

Conclusion: 该系统为蜜源植物花粉学提供了高效、准确的自动化解决方案，显著减少了分析时间和主观性，具有实际应用价值。
...
TL;DR: 动态3D高斯泼溅方法在单目视频上训练视角PSNR表现良好，但泛化能力差：D-NeRF基准测试中平均训练-测试PSNR差距达6.18dB。研究发现分裂操作导致80%以上差距，而弹性能量正则化通过强制变形场局部平滑可显著减少泛化差距。


<details>
  <summary>Details</summary>
Motivation: 动态3D高斯泼溅方法在单目视频重建中训练视角PSNR表现优异，但在未见视角上泛化能力严重不足，训练-测试PSNR差距显著。本文旨在探究导致这种泛化差距的根本原因，并寻找有效的解决方案。

Method: 1. 系统消融自适应密度控制流程（分裂、克隆、修剪、频率、阈值、调度），发现分裂操作是主要问题来源；2. 提出弹性能量正则化（EER），对每个高斯的变形场施加局部平滑性惩罚；3. 评估其他正则化方法：GAD（损失率感知的密度阈值）和PTDrop（抖动加权的高斯丢弃）；4. 在不同变形架构和真实单目视频数据上验证方法的泛化能力。

Result: 1. 分裂操作导致80%以上的泛化差距，禁用分裂可将高斯数量从44K减少到3K，差距从6.18dB降至1.15dB；2. EER将泛化差距减少40.8%，同时使高斯云增长85%，平均应变减少99.72%；3. GAD+EER组合减少48%差距，加上PTDrop和软增长上限可达57%；4. 方法在Deformable-3DGS架构和真实HyperNeRF场景上同样有效，分别减少40.6%和14.9%的泛化差距。

Conclusion: 动态3D高斯泼溅的过拟合主要由变形场的不一致性驱动，而非参数数量。通过强制变形场的局部平滑性（EER）可以显著改善泛化能力，这为动态神经渲染提供了新的正则化方向。
...
TL;DR: TriTS：一种跨模态解缠框架，通过将1D时间序列投影到正交的时间、频率和2D视觉空间，解决了长期时间序列预测中的表示瓶颈问题，实现了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 长期时间序列预测面临重大挑战，因为现实世界信号包含高度纠缠的时间动态，难以从纯1D视角完全捕捉。现有方法存在表示瓶颈，需要打破这种限制。

Method: 1. 提出TriTS跨模态解缠框架，将1D时间序列投影到正交的时间、频率和2D视觉空间；2. 引入Period-Aware Reshaping策略和Visual Mamba，以线性计算复杂度建模跨周期依赖；3. 设计Multi-Resolution Wavelet Mixing模块，将非平稳信号解耦为趋势和噪声分量；4. 保留时间域的流式线性分支以保持数值稳定性；5. 动态融合三种互补表示。

Result: 在多个基准数据集上的广泛实验表明，TriTS实现了最先进的性能，显著优于现有的基于视觉的预测器，同时大幅减少了参数数量和推理延迟。

Conclusion: TriTS通过跨模态解缠框架有效解决了长期时间序列预测中的表示瓶颈问题，在保持计算效率的同时实现了优异的预测性能，为时间序列分析提供了新的视角。
...
TL;DR: 提出一个基于少量标注数据（48张照片）的自动箭孔检测、定位和评分系统，使用冻结的DINOv3 ViT-L/16模型和轻量级检测头，在箭靶上实现毫米级定位精度。


<details>
  <summary>Details</summary>
Motivation: 传统箭孔检测方法需要大量标注数据，而实际应用中标注数据有限。本文旨在探索在小数据场景下，如何利用预训练的基础视觉模型实现高精度的密集预测任务。

Method: 1) 颜色基准校正：将透视畸变的照片映射到标准化坐标系；2) 冻结的DINOv3 ViT-L/16自监督视觉变换器配合AnyUp引导特征上采样，从32×32补丁令牌恢复亚毫米空间精度；3) 轻量级CenterNet风格检测头进行箭孔中心热图预测。仅3.8M参数可训练。

Result: 在三折交叉验证中，平均F1分数0.893±0.011，平均定位误差1.41±0.06mm，优于需要更多训练数据的全监督方法。下游箭术指标：每张图像平均箭分中位误差1.8%，组中心位置中位误差4.00mm。

Conclusion: 冻结的基础模型配合少量任务特定适配，为小数据场景下的密集预测任务提供了实用范式，引导特征上采样已能解决补丁令牌化导致的空间精度损失。
...
TL;DR: FairNVT是一个轻量级的去偏框架，用于预训练的Transformer编码器，通过同时改善表示层和预测层的公平性来保持任务准确性。


<details>
  <summary>Details</summary>
Motivation: 现有去偏方法通常单独处理表示层和预测层的公平性，但作者认为这两者本质上是相互关联的：在表示层抑制敏感信息可以促进更公平的预测。

Method: 使用轻量级适配器学习任务相关和敏感嵌入，对敏感嵌入应用校准的高斯噪声，然后与任务表示融合。结合正交性约束和公平性正则化，共同减少学习嵌入中的敏感属性泄露，并鼓励更公平的下游预测。

Result: 在涵盖视觉和语言的三个数据集上，FairNVT降低了敏感属性攻击者准确率，改善了人口统计平等和机会平等指标，同时保持了较高的任务性能。

Conclusion: FairNVT是一个兼容多种预训练Transformer编码器的轻量级去偏框架，能够有效同时改善表示层和预测层的公平性，同时保持任务准确性。
...
TL;DR: EdgeVTP：面向路边边缘设备的车辆轨迹预测器，通过结合交互感知图建模、轻量级Transformer骨干网络和单次曲线解码器，实现有界确定性端到端延迟，在保持高精度的同时显著降低延迟。


<details>
  <summary>Details</summary>
Motivation: 高速公路感知中的车辆轨迹预测需要在路边边缘设备上部署，这要求有界、确定性的端到端延迟。现有方法在拥挤场景中运行时延不可预测，且解码开销较大。

Method: 1. 结合交互感知图建模与轻量级Transformer骨干网络；2. 使用单次曲线解码器，将未来运动预测为紧凑的曲线参数（锚定在最后观测位置），而非按时间步自回归预测路径点；3. 通过带有硬邻居上限的局部性图显式限制交互复杂度，确保运行时延可预测。

Result: 在三个高速公路基准测试和两个Jetson级平台上，EdgeVTP在包含图构建和后处理的协议下实现了最低的端到端延迟，同时在三个数据集中的两个上达到了最先进的预测精度，在其他基准测试上也有竞争力的误差表现。

Conclusion: EdgeVTP通过紧凑的曲线参数预测和显式的交互复杂度限制，成功实现了边缘设备上的高效轨迹预测，在延迟和精度之间取得了良好平衡，为路边边缘部署提供了实用解决方案。
...
TL;DR: HyMOR是一个混合多粒度开放目标识别框架，结合MLLM的开放识别能力和CLIP的细粒度识别优势，在多个语义粒度上实现准确目标理解。


<details>
  <summary>Details</summary>
Motivation: 当前MLLM在开放目标识别方面表现良好但细粒度识别能力不足，而CLIP模型擅长细粒度识别但缺乏通用目标类别覆盖。需要一种能同时处理开放识别和细粒度识别的解决方案。

Method: 提出HyMOR混合框架：MLLM负责开放和粗粒度目标识别，CLIP模型专门处理特定领域（如动植物）的细粒度识别。同时引入TBO数据集（20,942张图像，8,816个类别）用于评估。

Result: HyMOR将细粒度识别与CLIP的差距缩小到0.2%，同时比基线MLLM在通用目标识别上提升2.5%。在所有评估数据集上平均SBert相似度提升23.2%。

Conclusion: HyMOR通过结合MLLM和CLIP的优势，实现了跨多个语义粒度的准确目标理解，为多模态内容生成和交互式学习应用提供了强大的感知基础。
...
TL;DR: CDCRec是一种多模态射电干涉数据重建方法，通过显式建模跨域一致性来提升射电天文图像质量，相比现有单模态方法能更好地利用可见性和图像域的互补信息。


<details>
  <summary>Details</summary>
Motivation: 现有射电天文图像重建方法通常局限于单模态域（要么在成像后的脏图像域，要么在成像前的稀疏可见性域），这导致丢失了另一域的互补上下文信息，无法完整建模相互依赖性和一致性。

Method: 提出CDCRec方法，采用分层多任务多阶段框架，显式建模跨域一致性，通过自监督互补建模策略增强域间交互探索，实现多模态射电干涉数据重建。

Result: 实验结果表明CDCRec通过增强跨域相关性提取提升了成像性能，在依赖从受限源域数据恢复密集信息的干涉域转换任务上优于现有方法。

Conclusion: CDCRec通过多模态跨域一致性建模有效解决了射电天文图像重建中的信息丢失问题，为射电干涉数据重建提供了更完整的方法。
...
Method: 将语义解码建模为贝叶斯逆问题，证明后验采样能通过保持数据分布实现最优感知质量。提出交替双域后验采样(ADDPS)，在采样过程中交替执行潜在域和图像域一致性约束，将联合后验采样分解为更简单的子问题

Result: 在FFHQ数据集上的实验表明，ADDPS相比现有方法实现了更优的感知质量

Conclusion: 通过交替双域后验采样策略，有效解决了生成式语义通信中感知质量受限的问题，避免了梯度冲突同时保留了两域的互补优势
...
TL;DR: 提出基于频率解耦隐式神经表示（FD-INR）的近红外辅助低光图像恢复方法，利用RGB-NIR跨模态频率相关性统计先验，通过多尺度小波变换分解频率分量，构建双分支隐式表示框架，实现低光图像的高质量恢复。


<details>
  <summary>Details</summary>
Motivation: 解决低光条件下可见光图像存在的严重噪声和高频结构退化问题。传统方法在空间域进行刚性融合容易导致颜色失真和伪影，需要更有效的跨模态信息融合策略。

Method: 1. 基于RGB-NIR跨模态频率相关性统计先验（低频RGB信号更可靠，高频NIR信号相关性更高）；2. 通过多尺度小波变换将图像分解为不同频率分量；3. 构建双分支隐式神经表示框架；4. 设计跨模态差异化频率监督机制：用低光RGB指导低频亮度和颜色重建，用高信噪比NIR约束高频纹理细节生成；5. 引入基于不确定性的自适应加权损失函数，自动平衡不同频率任务的贡献。

Result: FD-INR不仅能有效恢复图像亮度一致性和结构细节，而且得益于其隐式连续表示特性，在任意分辨率重建任务中优于现有方法，显著提升了低光感知的可靠性。

Conclusion: 提出的FD-INR方法通过频率解耦和跨模态差异化监督，有效解决了低光图像恢复中的噪声和结构退化问题，实现了RGB和NIR信息的频率域互补优势，为低光感知提供了更可靠的解决方案。
...
TL;DR: 提出通道注意力引导的跨模态知识蒸馏方法，用于轻量级指称图像分割，在不增加推理参数的情况下提升学生模型性能


<details>
  <summary>Details</summary>
Motivation: 现有指称图像分割方法通常使用大规模视觉和语言编码模型，但参数量巨大限制了在计算资源有限场景的部署。需要解决轻量化部署问题。

Method: 提出通道注意力引导的跨模态知识蒸馏方法，将教师网络学习到的视觉-语言高阶细粒度相关性以及通道表示的语义组件相关性传递给学生网络。

Result: 在两个公共数据集上的实验结果表明，所提蒸馏方法在推理时不引入额外参数，能显著提升学生模型的性能。

Conclusion: 该方法不仅让学生学习教师知识，还保留其独立学习能力，缓解学习偏差传递，为轻量级指称图像分割提供有效解决方案。
...
TL;DR: BioLip检测器通过分析唇部时间抖动来识别深度伪造，基于生成模型不遵守真实口面部生物力学约束的原理


<details>
  <summary>Details</summary>
Motivation: 当前唇部同步深度伪造检测器依赖像素级伪影或视听对应关系，无法跨语言泛化，因为这些线索编码的是数据依赖模式而非通用物理规律

Method: 提出BioLip轻量级框架，基于生成模型不强制执行真实口面部生物力学约束的原理，通过MediaPipe提取64个口周关键点坐标，测量时间唇部方差（称为时间唇部抖动）

Result: 时间唇部抖动在说话者的语言、种族和录制条件下经验一致，是可测量的信号

Conclusion: 生物力学约束违反提供了比数据依赖模式更基础的检测原理，BioLip框架能够跨语言有效检测唇部同步深度伪造
...
TL;DR: 提出GCN-HViT模型，结合Vision Transformer的全局注意力机制和GCN的局部特征提取能力，通过分层结构和2D位置嵌入提升图像分类性能


<details>
  <summary>Details</summary>
Motivation: 解决ViT和GCN各自的局限性：ViT需要合适的patch大小选择，1D位置嵌入无法准确捕捉空间结构信息；GCN缺乏全局图结构信息建模能力，而ViT无法建模局部结构

Method: 提出分层Vision Transformer增强图卷积网络(GCN-HViT)：分层ViT建模全局patch交互和跨层层次关系；GCN作为局部特征提取器获取2D位置嵌入，建模局部patch交互

Result: 在3个真实世界数据集上的实验表明，GCN-HViT达到了最先进的性能

Conclusion: GCN-HViT有效结合了ViT的全局建模能力和GCN的局部特征提取优势，通过分层结构和2D位置嵌入解决了现有方法的局限性，在图像分类任务中表现出色
...
TL;DR: 提出基于洛伦兹模型的语义分割框架，解决庞加莱球模型的数值不稳定问题，提供不确定性估计和层次化表示


<details>
  <summary>Details</summary>
Motivation: 现有双曲空间语义分割主要使用庞加莱球模型，存在数值不稳定、优化困难和计算挑战等问题，需要更稳定高效的解决方案

Method: 提出基于洛伦兹模型的新型语义分割框架，使用文本嵌入引导层次化像素表示，无需黎曼优化器，可与现有欧几里得架构集成

Result: 在多个数据集上验证有效性，获得不确定性估计、置信度图、边界划分、层次化检索和零样本性能，达到更平坦的最小值

Conclusion: 洛伦兹双曲嵌入为鲁棒且具有不确定性感知的语义分割提供了潜力，解决了庞加莱模型的局限性
...
TL;DR: EFDiff是一个用于极端空间降级下地表温度超分辨率的新框架，利用地球基础模型引导扩散过程，在32倍尺度差距下实现精细重建。


<details>
  <summary>Details</summary>
Motivation: 地表温度超分辨率对于环境监测很重要，但粗热观测严重限制了精细尺度结构的确定，特别是在极端空间降级情况下。

Method: 提出EFDiff框架，使用Prithvi-EO-2.0地球基础模型将高分辨率多光谱反射率编码为地理空间嵌入，通过交叉注意力注入去噪网络，从高度降级观测中引导精细尺度重建。研究了EFDiff-ε和EFDiff-x₀两种变体。

Result: 在包含242,416个全球多样化配准Landsat热反射斑块的基准测试中，EFDiff在32倍尺度差距下始终优于基线方法，且EFM交叉注意力条件比HLS通道连接更有效。

Conclusion: EFDiff框架虽然在地表温度超分辨率背景下提出，但广泛适用于预训练地理空间表示可以引导生成重建的遥感问题。
...
TL;DR: 提出TowerDataset传输走廊点云分割基准数据集和全局-局部融合框架，解决现有数据稀缺、类别粗糙、长距离依赖建模困难等问题


<details>
  <summary>Details</summary>
Motivation: 当前传输走廊点云语义分割面临现实数据稀缺、难以同时建模全局走廊结构和局部几何细节的问题。现有公开数据集通常只提供少量粗糙类别或短场景，忽略了长距离结构依赖、严重长尾分布和安全关键组件的细微区别

Method: 提出全局-局部融合框架：1) 全局分支采用NoCrop训练和原型对比学习捕获长距离拓扑和上下文依赖；2) 局部分支保留精细几何结构；3) 通过几何验证融合和细化两个分支的预测结果

Result: 构建了包含661个真实场景、约24.66亿个点的TowerDataset数据集，定义了22类细粒度分类体系。在TowerDataset和两个公共基准上的实验证明了所提基准的挑战性和框架在真实复杂异构传输走廊场景中的鲁棒性

Conclusion: TowerDataset为传输走廊点云分割提供了全面的基准，提出的全局-局部融合框架能够有效利用全局关系和局部形状细节来识别稀有和易混淆组件，在真实复杂场景中表现出色
...
<details>
  <summary>Details</summary>
Motivation: Transformer在伪装目标检测中虽然达到SOTA精度，但计算开销大限制了实际部署，需要更高效的解决方案

Method: 分层置信感知令牌剪枝框架，识别并丢弃易区分的背景和物体内部令牌，聚焦关键边界令牌；引入双路径特征补偿机制聚合剪枝令牌的上下文知识

Result: 在多个COD基准测试中显著降低计算复杂度，同时保持高精度

Conclusion: CATP为COD模型在现实场景中的高效部署提供了有前景的研究方向
...
TL;DR: 本文提出COD-TDQ方法，针对伪装目标检测的W4A4量化问题，通过令牌分组双约束激活量化解决令牌局部瓶颈，显著提升量化性能。


<details>
  <summary>Details</summary>
Motivation: 伪装目标检测需要在设备内存和延迟受限的环境下进行，低比特量化非常必要。但COD任务量化困难，因为背景令牌的厚尾分布主导共享激活范围，导致步长膨胀，将弱但有结构的边界线索推入零箱，形成令牌局部瓶颈。

Method: 提出COD-TDQ方法：1) 直接和令牌分组(DSTG)分配令牌组尺度以抑制跨令牌范围主导；2) 双约束范围投影(DCRP)投影每个令牌组裁剪范围，保持步长-离散比和零箱质量有界。

Result: 在四个COD基准测试和两个基线模型(CFRN和ESCNet)上，COD-TDQ始终比最先进的量化方法Sα分数提高0.12以上，且无需重新训练。

Conclusion: COD-TDQ有效解决了伪装目标检测中的令牌局部瓶颈问题，实现了高效的W4A4量化，显著提升了量化性能，为设备端COD应用提供了实用解决方案。
...
TL;DR: Q-DeepSight：一种模仿人类证据搜寻过程的图像质量评估框架，通过多模态思维链和工具增强的证据获取提供可操作的局部反馈，并应用于指导图像生成迭代优化。
...
Method: 提出Q-DeepSight框架，采用交错的Multimodal Chain-of-Thought（iMCoT）和工具增强的证据获取（如裁剪和缩放）来明确确定质量下降的位置和原因。使用强化学习训练这些长iMCoT轨迹，引入Perceptual Curriculum Reward（PCR）缓解奖励稀疏性，以及Evidence Gradient Filtering（EGF）改进视觉基础推理的信用分配。

Result: Q-DeepSight在包括自然图像、修复图像和AI生成内容在内的多样化基准测试中实现了最先进的性能。通过Perceptual-in-Generation（PiG）框架，Q-DeepSight的诊断指导迭代图像增强，有效连接了评估和优化之间的循环。

Conclusion: Q-DeepSight通过模仿人类证据搜寻过程，提供可操作的局部质量反馈，不仅提升了图像质量评估的准确性，还展示了在指导生成模型和图像修复中的实际应用价值，实现了评估与优化的闭环。
...
TL;DR: 提出I2P框架，通过自适应识别关键层表示和约束参数更新在低敏感子空间，解决合成图像检测中的跨分布泛化问题


<details>
  <summary>Details</summary>
Motivation: 随着生成模型和多模态内容编辑技术的发展，合成图像检测面临跨分布泛化的关键挑战。现有基于视觉基础模型的方法适应策略相对粗粒度，要么直接使用最终层表示，要么简单融合多层特征，缺乏对可迁移伪造线索最优表示层次的显式建模。同时，直接微调可能破坏支持开放集泛化的跨模态预训练结构。

Method: 提出I2P框架，将视觉基础模型适应重新表述为联合优化问题：1）自适应识别最适合承载伪造判别信息的关键表示层；2）将任务驱动的参数更新约束在低敏感参数子空间内，在提高任务特异性的同时尽可能保留预训练表示的可迁移结构。

Result: 论文提出I2P框架，但摘要中未提供具体实验结果数据。从方法描述来看，该框架旨在解决现有方法的局限性，理论上应能提升合成图像检测的跨分布泛化能力。

Conclusion: 通过将视觉基础模型适应重新表述为联合优化问题，I2P框架能够自适应识别关键表示层并约束参数更新，在提高合成图像检测任务特异性的同时保留预训练结构的可迁移性，为解决跨分布泛化挑战提供了新思路。
...
TL;DR: BiasCareVL是一个偏差感知的多模态学习框架，通过在模型设计中直接引入偏差控制，而不是事后校正，来解决医疗AI系统中的公平性和可靠性问题。


<details>
  <summary>Details</summary>
Motivation: 医疗影像和临床文本的整合催生了医疗领域的通用AI系统，但普遍存在的偏差（如疾病流行度不平衡、解剖区域分布偏斜、成像协议异质性和人口统计学差异）对现实临床环境中视觉语言系统的公平性和可靠性构成重大挑战。

Method: BiasCareVL采用偏差感知的多模态学习框架，将偏差控制直接融入模型设计，包含自适应不确定性建模和可选的人工参与细化，以调节主导数据模式的影响，并在分布不平衡下促进公平推理。在340万样本上训练，涵盖15种成像模态。

Result: 在涵盖皮肤病学、肿瘤学、放射学和病理学的8个公共基准测试中，BiasCareVL持续优于20种最先进方法，在临床挑战性场景中表现突出：多类皮肤病变诊断准确率提高超过10%，小肿瘤分割Dice系数提高超过20%。诊断性能超过人类专家，且时间需求大幅减少。
...
TL;DR: CrossFlowDG提出了一种新的领域泛化框架，通过跨模态流匹配将领域偏置的图像嵌入显式传输到领域不变的文本嵌入，解决了模态间隙问题


<details>
  <summary>Details</summary>
Motivation: 领域泛化中，现有基于余弦相似度的对比对齐方法存在模态间隙问题，图像和文本嵌入在几何上分离，尽管语义对应。需要解决这种残留间隙以实现更好的跨领域泛化

Method: 使用无噪声的跨模态流匹配，在联合欧几里得潜在空间中学习连续变换，将领域偏置的图像嵌入显式传输到领域不变的文本嵌入。采用VMamba图像编码器和CLIP文本编码器

Result: 在四个常见DG基准测试中表现优异，在多个基准上具有竞争力，在TerraIncognita上达到最先进水平

Conclusion: CrossFlowDG通过流匹配有效解决了模态间隙问题，为领域泛化提供了新的解决方案，在多个基准上验证了其有效性
...
TL;DR: EasyVideoR1是一个专门为视频理解任务训练大型视觉语言模型的高效强化学习框架，通过离线预处理、任务感知奖励系统、混合训练范式等优化，显著提升了训练效率和性能。


<details>
  <summary>Details</summary>
Motivation: 随着模型向多模态架构发展，将RLVR扩展到视频理解变得重要但尚未充分探索，主要面临视频任务类型多样、高维视觉输入解码计算开销大、敏感超参数可复现评估困难等挑战。

Method: 1) 完整的视频RL训练流程，包含离线预处理和张量缓存，消除冗余视频解码；2) 涵盖11种视频和图像问题类型的任务感知奖励系统；3) 混合离线-在线数据训练范式；4) 独立可配置像素预算的联合图像-视频训练；5) 异步多基准评估框架。

Result: 实现了1.47倍的吞吐量提升，支持22个主流视频理解基准测试，复现准确率与官方报告分数高度一致。

Conclusion: EasyVideoR1为视频理解任务的强化学习训练提供了系统化优化方案，解决了现有框架在视频模态上的不足，为多模态模型训练提供了高效工具。
...
TL;DR: 提出PIT框架，通过神经网络自动编码器定位粒子为热图峰值，并嵌入可微分物理模块约束轨迹满足已知动力学，实现无标签物理一致性跟踪


<details>
  <summary>Details</summary>
Motivation: 传统粒子跟踪方法通常需要大量标注数据，且难以保证物理一致性。本文旨在开发一个既能利用物理先验知识，又能减少对标注数据依赖的视频粒子跟踪框架

Method: 1) 使用带有分割瓶颈的自动编码器，分离跟踪相关结构（热图）和背景噪声；2) 嵌入可微分物理模块约束多个时间点的地标形成物理一致的轨迹；3) 提出PILL损失（无监督）和PILLS损失（有监督）分别与地标或真实位置比较

Result: 在26因子设计的64种配置下，PILLS方法在干净和噪声条件下均能实现亚像素级跟踪精度，特别是双线性和物理精炼解码器输出表现优异

Conclusion: PIT框架成功将物理先验知识融入深度学习跟踪系统，既能实现无监督物理一致性学习，也能在有监督设置下达到高精度跟踪，为物理约束的计算机视觉任务提供了新思路
...
TL;DR: 提出GW-HGNN图神经网络用于低空高斯泼溅的资源分配，通过建模图像组对重建的非均匀贡献，平衡数据保真度和传输成本，显著提升渲染质量并降低计算延迟。


<details>
  <summary>Details</summary>
Motivation: 现有低空资源分配方案在LAGS场景下效率低下，因为它们没有考虑不同视角带来的图像多样性，而LAGS优先考虑重建质量而非通信吞吐量。

Method: 提出分组异构图神经网络(GW-HGNN)，将LAGS损失和通信约束转化为图学习成本，进行双层消息传递，显式建模不同图像组对重建过程的非均匀贡献。

Result: 在真实LAGS数据集上，GW-HGNN在PSNR、SSIM和LPIPS等关键渲染指标上显著优于现有基准方法，同时计算延迟比MOSEK求解器降低约100倍，达到毫秒级推理速度。

Conclusion: GW-HGNN通过图学习有效解决了LAGS资源分配问题，平衡了重建质量和传输成本，实现了实时部署的可行性。
...
TL;DR: USTri是一个三阶段超声智能管道，用于统一的多器官、多任务分析，通过通用预训练、领域特定微调和智能体工作流编排，实现跨异构超声任务的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 临床超声分析需要能够跨异构器官、视图和设备泛化的模型，同时支持可解释的工作流程级分析。现有方法通常依赖任务特定适应，联合学习可能因跨任务干扰而不稳定，难以在实际中提供工作流程级输出。

Method: 提出三阶段超声智能管道USTri：1) Stage I训练通用专家USGen学习广泛可迁移先验；2) Stage II构建USpec，保持USGen冻结并微调数据集特定头部；3) Stage III引入USAgent，模仿临床医生工作流程，编排USpec专家进行多步推理和确定性结构化报告。

Result: 在FMC_UIA验证集上，该模型在4种任务类型和27个数据集上实现了最佳整体性能，优于最先进方法。定性结果显示USAgent能够以高准确性和可解释性生成临床结构化报告。
...
TL;DR: KIRA是一个统一的五阶段视觉RAG框架，解决了专业领域视觉检索增强生成的十个核心问题，在四个专业领域实现了高精度检索、完全接地和良好的领域正确性。


<details>
  <summary>Details</summary>
Motivation: 当前RAG主要应用于文本问答，扩展到视觉领域面临四个主要挑战：图像查询与文本知识库的模态鸿沟、构建语义有意义的视觉知识库、对检索图像进行多跳推理、验证生成答案是否忠实于视觉证据。

Method: KIRA采用五阶段框架：1) 基于DINO区域检测的分层语义分块构建多粒度知识库；2) 具有少样本适应能力的领域自适应对比编码器；3) 带有思维链查询扩展的双路径跨模态检索；4) 支持时间和多视图的链式检索多跳视觉推理；5) 证据条件接地生成与事后幻觉验证。

Result: 在四个专业领域（医学X光、电路图、卫星图像、组织病理学）的实验中，KIRA平均达到0.97检索精度、1.0接地分数和0.707领域正确性。六变体消融实验揭示了各组件何时有效以及何时引入精度-多样性权衡。

Conclusion: KIRA提供了一个统一的视觉RAG框架，有效解决了专业领域的视觉知识检索和推理问题，并通过DOMAINVQAR基准套件提供了超越标准召回指标的全面评估方法。
...
TL;DR: 提出统一残差噪声学习框架，通过直接估计低剂量PET图像的噪声而非预测全剂量图像，解决跨剂量去噪中的泛化问题。


<details>
  <summary>Details</summary>
Motivation: 传统单剂量训练的神经网络难以泛化到其他剂量条件，而"一刀切"模型学习的是跨噪声水平的平均表示，导致性能下降。

Method: 提出统一残差噪声学习框架，直接从低剂量PET图像估计噪声，而非预测全剂量图像，避免标准训练公式中隐含的异质噪声分布期望优化问题。

Result: 在两个医疗中心的大规模多剂量PET数据集上，该方法优于"一刀切"模型、个体剂量特定U-Net模型和剂量条件方法，实现了更好的去噪性能。

Conclusion: 残差噪声学习有效缓解了平均效应，增强了跨剂量PET去噪的泛化能力。
...
TL;DR: 提出CoGR-MoE框架，通过答案选项的语义指导专家选择，解决VQA中MoE路由不稳定或过于稳定的问题，提升选项推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有Mixture-of-Experts方法在VQA中存在路由不稳定（同类问题专家选择不一致）或过于稳定（灵活性降低）的问题，需要更有效的专家选择机制。

Method: 提出概念引导路由框架CoGR-MoE：1）利用答案选项语义指导训练阶段的专家选择；2）使用选项特征重新加权选中的专家；3）生成每个候选选项的判别性表示；4）通过对比学习优化选项比较。

Result: CoGR-MoE在多个VQA任务中表现出强大的性能，证明了该方法的有效性。

Conclusion: 通过概念引导的路由机制，CoGR-MoE能够稳定且灵活地选择专家，生成判别性的选项表示，显著提升VQA性能。
...
TL;DR: 提出ARF-SFR-Net网络，通过自适应感受野选择空间和频率特征描述符，解决小样本细粒度图像分类中特征重建的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有基于特征的小样本细粒度图像分类方法面临的主要挑战是如何为不同类别输入图像选择合适的感受野大小来提取特征描述符（包括空间和频率特征），从而更好地完成分类任务。

Method: 提出自适应感受野的空间-频率特征重建网络（ARF-SFR-Net），能够自适应确定获取空间和频率特征的感受野大小，并有效融合它们进行特征重建和分类任务。该网络可以轻松嵌入到给定的episodic训练机制中进行端到端训练。

Result: 在多个小样本细粒度图像分类基准测试上的广泛实验证明了ARF-SFR-Net的有效性和优越性，超越了现有最先进方法。

Conclusion: ARF-SFR-Net通过自适应感受野机制有效解决了小样本细粒度图像分类中的特征提取挑战，在多个基准测试中表现出色，代码已开源。
...
TL;DR: CoDe-MAE提出了一种新的高分辨率光学与SAR图像预训练方法，通过"更好协同、更少对齐"的理念解决异构模态对齐难题，避免特征抑制或污染，在多种下游任务中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决高分辨率光学与SAR图像预训练中的"异构-分辨率悖论"：更高空间分辨率会放大雷达几何与光学纹理的物理差异，传统刚性对齐方法会导致特征抑制或污染，造成表示退化和负迁移。

Method: 1. 光学锚定知识蒸馏(OKD)：将SAR斑点噪声映射到纯语义流形；2. 条件对比学习(CCL)：使用梯度缓冲机制对齐共享共识，同时安全保留不同物理特征；3. 跨模态退化重建(CDR)：剥离非同源光谱伪特征，捕获真实结构不变性。

Result: 在100万样本上预训练后，CoDe-MAE展现出卓越的数据效率，成功防止表示退化，在多种单模态和双模态下游任务中建立新的SOTA性能，大幅优于基于更大数据集的基础模型。

Conclusion: CoDe-MAE通过创新的"更好协同、更少对齐"方法成功解决了高分辨率光学-SAR预训练的异构性挑战，为极端异构模态的表示学习提供了有效解决方案。
...
TL;DR: TSM-Pose：一种结合拓扑感知学习和语义Mamba的类别级物体姿态估计框架，通过拓扑提取器和Mamba全局语义聚合器提升未见实例的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有类别级物体姿态估计方法主要依赖简单的特征提取和聚合，难以捕捉类别共享的拓扑结构和进行语义关键点建模，限制了其泛化能力。

Method: 提出TSM-Pose框架：1）拓扑提取器捕捉点云的全局拓扑表示，与局部几何特征融合；2）基于Mamba的全局语义聚合器，注入语义先验到关键点，利用多个TwinMamba块建模长距离依赖关系。

Result: 在REAL275、CAMERA25和HouseCat6D三个基准数据集上的实验表明，TSM-Pose优于现有最先进方法。

Conclusion: TSM-Pose通过拓扑感知学习和语义Mamba有效提升了类别级物体姿态估计的泛化能力，在多个数据集上取得了优越性能。
...
TL;DR: 开发了TRU（Temporal Retinal U-Net），一种确定性直接回归模型，用于预测视网膜疾病的纵向进展。研究发现，在缓慢进展的视网膜疾病中，复杂的生成模型并非必要，确定性回归模型表现更优。


<details>
  <summary>Details</summary>
Motivation: 目前临床决策依赖定性比较或标量进展评分，需要定量预测视网膜纵向外观变化来支持进行性黄斑疾病的临床决策。

Method: 比较了五种条件配置，发现对齐训练和推理输入分布比选择特定生成框架更重要。基于此开发了TRU模型，采用确定性直接回归、连续时间差条件和多尺度历史聚合。

Result: TRU在三个成像平台的28,902只眼睛上评估，在delta-SSIM、SSIM和PSNR指标上均匹配或优于三个最先进的基准模型，且优势随可用历史长度单调增长。

Conclusion: 对于缓慢进展的视网膜疾病，确定性回归模型优于复杂生成模型，TRU模型在多个平台和疾病类型中表现出色，支持临床决策。
...
TL;DR: 提出一个自推理智能体框架，用于生成叙事性产品网格拼贴，通过构建产品叙事框架和迭代优化来提升视觉一致性和叙事丰富度。


<details>
  <summary>Details</summary>
Motivation: 叙事驱动的产品摄影在现代营销中很重要，但现有图像生成方法缺乏结构化叙事规划和跨面板协调，导致故事性弱和视觉不连贯。产品叙事通常以多网格拼贴形式呈现，需要确保跨网格的视觉一致性和整体构图的美学和谐。

Method: 提出自推理智能体框架：1) 构建产品叙事框架，明确表示产品身份、使用情境和环境；2) 将其转化为受共享视觉风格约束的互补网格；3) 编译约束感知提示并馈送到生成模型，联合合成拼贴；4) 评估内容有效性和摄影质量，通过门控决定继续或优化；5) 失败时进行故障归因和针对性优化，通过迭代自反思实现渐进改进。

Result: 实验表明，相比直接提示基线，该框架在美学质量、叙事丰富度和视觉连贯性方面持续改进。

Conclusion: 提出的自推理智能体框架能够生成视觉一致、叙事丰富的产品网格拼贴，解决了现有方法在结构化叙事规划和跨面板协调方面的不足，为叙事产品摄影提供了有效的解决方案。
...
TL;DR: 论文提出了一种基于深度非负矩阵分解的分层解混方法BLUTHs，通过分层丰度和约束解决光谱变异性、端元数量确定和端元清晰度问题，在实验室和遥感场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像解混面临三个主要挑战：1）光谱变异性抑制解混性能；2）端元数量确定方式模糊；3）端元数量增加时端元清晰度下降。分层结构是解决这三个问题的可能方案。

Method: 提出BLUTHs（二进制线性解混触觉层次结构），在深度非负矩阵分解上施加分层丰度和约束，通过简单网络架构解决分层解混问题。使用稀疏调制解混增长为每个场景定制BLUTH的拓扑结构。

Result: BLUTHs在实验室场景中超越现有最先进的解混算法（特别是在丰度估计方面），在遥感场景中保持竞争力。成功应用于HYPSO和PACE卫星的高光谱海洋颜色解混。

Conclusion: 分层解混通过BLUTHs有效解决了高光谱解混的三个关键问题，为光谱变异性、端元数量确定和端元清晰度提供了统一解决方案，在实验室和实际遥感应用中表现出色。
...
TL;DR: 提出一种无监督几何距离(UGD)用于点云去噪评估，仅需噪声点云即可计算，无需干净参考点云


<details>
  <summary>Details</summary>
Motivation: 现有点云去噪评估方法需要干净参考点云，这在真实场景中往往不可得，需要一种仅基于噪声点云的无监督评估方法

Method: 1) 从干净点云学习补丁级先验模型(GMM)；2) 使用特征提取网络提取质量感知特征；3) 通过多任务自监督训练网络；4) 定义UGD为去噪点云补丁与先验GMM模型的距离加权和

Result: 在合成噪声数据上，UGD性能与有监督全参考指标相当；在真实数据上，UGD能够仅基于噪声点云实现无监督评估

Conclusion: 提出的UGD方法为真实场景点云去噪提供了一种有效的无监督评估方案，解决了参考点云不可得的问题
...
TL;DR: DOSE：利用现成预训练模型进行多模态数据筛选，无需任务特定训练即可提升数据质量与多样性，降低计算成本


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型训练数据存在噪声、冗余和对齐不佳的问题，传统数据筛选方法需要额外计算成本，因为筛选模型通常需要在目标数据上训练。需要探索能否使用从未见过目标数据的现成预训练模型进行数据选择。

Method: DOSE方法使用现成预训练模型（无需微调）评估文本质量和图文对齐，构建联合质量-对齐分布，应用自适应加权采样选择信息丰富的样本，同时保持长尾多样性。

Result: 在标准VQA和数学基准测试中，使用DOSE筛选数据训练的模型能够匹配甚至超越使用完整数据集训练的模型。实验证明了该方法的有效性、效率和可扩展性。

Conclusion: DOSE展示了现成预训练模型可以有效用于多模态数据筛选，无需任务特定训练，既能提升数据质量又能保持多样性，显著降低计算成本。
...
TL;DR: URVIS 2026挑战赛报告：首个针对恶劣到极端天气条件下的全景分割挑战，基于MUSES多传感器数据集，采用加权全景质量(wPQ)作为评估指标，吸引了17个团队参与。


<details>
  <summary>Details</summary>
Motivation: 推动在恶劣到极端天气条件下鲁棒的多模态全景分割技术发展，解决自动驾驶等实际应用中在不同天气条件下的感知挑战。

Method: 基于MUSES多传感器数据集（RGB相机、LiDAR、雷达、事件相机），设计加权全景质量(wPQ)作为公平评估指标，组织挑战赛收集和分析不同团队的解决方案。

Result: 挑战赛吸引了17个注册团队和47份提交，4个团队进入最终阶段。报告总结了挑战设置和基准结果，分析了提交方法的性能，并讨论了当前进展和剩余挑战。

Conclusion: 这是首个针对恶劣到极端天气全景分割的挑战赛，为多模态感知研究提供了重要基准，但仍存在许多挑战需要进一步解决以实现鲁棒的天气不变感知。
...
TL;DR: DVAR是一个无需训练的框架，将视频真实性检测重构为结构化多智能体辩论过程，通过生成假设代理与自然机制代理的交叉质证，结合MDL框架和知识库，实现可解释的泛化检测。


<details>
  <summary>Details</summary>
Motivation: 视频生成技术快速发展，传统检测方法难以泛化到训练分布之外，需要新的检测范式来应对不断演变的生成模型。

Method: 提出DVAR框架：1) 将检测重构为多智能体辩论过程，生成假设代理与自然机制代理进行迭代交叉质证；2) 应用奥卡姆剃刀原则，通过最小描述长度框架定义解释成本；3) 集成GenVideoKB动态知识库提供生成边界和失败模式的高层推理启发。

Result: 实验表明DVAR在性能上与监督SOTA方法相当，同时在未见过的生成架构上表现出更优的泛化能力，提供透明可解释的推理轨迹。

Conclusion: DVAR通过将检测转变为透明辩论过程，超越了模式匹配范式，实现了鲁棒、可解释的视频真实性评估，为媒体取证提供了新的推理框架。
...
TL;DR: 提出ICNNM方法，通过引入预学习的卷积特征向量避免SVD计算，在张量补全任务中实现更高效和更好的恢复性能。


<details>
  <summary>Details</summary>
Motivation: 现有CNNM方法在张量补全中需要多次SVD计算，计算成本高且难以并行化，需要更高效的解决方案。

Method: 从卷积特征向量角度重新表述CNNM优化目标，引入在不同张量间共享的预学习卷积特征向量，提出ICNNM方法绕过SVD步骤。

Result: ICNNM显著减少计算时间，且由于预学习卷积特征向量提供的额外先验知识，在恢复性能上也优于CNNM。在视频补全、预测和帧插值实验中表现出优越性。

Conclusion: ICNNM通过预学习卷积特征向量有效解决了CNNM的计算效率问题，在张量补全任务中实现了更好的性能和效率平衡。
...
TL;DR: TeMuDance：无需音乐-文本-动作三元组数据集，通过运动作为语义锚点对齐不同模态，实现文本控制的音乐驱动舞蹈生成


<details>
  <summary>Details</summary>
Motivation: 现有音乐驱动舞蹈生成方法缺乏语义可控性，难以通过自然语言描述指导特定动作，主要原因是缺乏大规模音乐-文本-动作对齐数据集

Method: 1) 提出运动中心桥接范式，以运动为共享语义锚点对齐不相关的音乐-舞蹈和文本-动作数据集；2) 在冻结的音乐到舞蹈扩散骨干上训练轻量级文本控制分支；3) 设计双流微调策略和置信度过滤抑制检索监督中的噪声

Result: TeMuDance在保持舞蹈质量的同时，显著提升了文本条件控制能力，优于现有方法

Conclusion: 该方法无需手动标注的三元组数据集，通过跨模态对齐实现了有效的文本控制音乐驱动舞蹈生成，并提出新的任务对齐评估指标
...
TL;DR: MobileAgeNet：基于MobileNetV3-Large的轻量级面部年龄估计框架，在UTKFace测试集上MAE为4.65年，参数量3.23M，移动端平均延迟14.4ms，提供完整的部署流程。


<details>
  <summary>Details</summary>
Motivation: 移动端面部年龄估计需要平衡预测精度、低延迟和模型大小的模型，现有方案在这几个方面的权衡不够理想。

Method: 使用预训练的MobileNetV3-Large作为骨干网络，结合紧凑的回归头；采用有界年龄回归和两阶段微调策略提升训练稳定性和泛化能力；集成到NN LEMUR Dataset框架中支持可重复实验。

Result: 在UTKFace测试集上达到4.65年的MAE，参数量3.23M，移动端平均延迟14.4ms；部署流程（PyTorch→ONNX→TensorFlow Lite）在实际设备条件下保持预测性能无显著下降。

Conclusion: MobileAgeNet提供了一个实用、可部署的移动端面部年龄估计基线，在精度、延迟和模型大小之间取得了良好平衡。
...
TL;DR: 提出一个用于异构骨架开放词汇动作识别的Transformer模型，构建大规模HOV数据集，通过多粒度运动-文本对齐实现通用骨架动作识别


<details>
  <summary>Details</summary>
Motivation: 随着机器人技术的发展，骨架动作识别变得越来越重要，但现有方法忽略了骨架数据的异构性（来自不同来源的人类骨架和人形机器人骨架），且现实应用需要开放词汇识别能力

Method: 提出基于Transformer的模型，包含三个关键组件：统一骨架表示、骨架运动编码器、多粒度运动-文本对齐。运动编码器将多模态骨架嵌入输入到双流Transformer编码器中学习时空动作表示，然后映射到语义空间与文本嵌入对齐

Result: 在包含异构骨架数据的流行基准测试上进行广泛实验，证明了所提方法的有效性和泛化能力

Conclusion: 该工作解决了异构骨架开放词汇动作识别的挑战性问题，构建了大规模HOV数据集，提出的Transformer模型通过多粒度对齐机制实现了通用骨架动作识别
...
TL;DR: LIVE是一个联合训练框架，利用大规模高质量图像编辑数据和视频数据集来增强视频编辑能力，通过帧级token噪声策略和两阶段训练实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 视频编辑需要根据用户意图修改输入视频，但相比图像编辑，视频数据的高标注成本严重限制了视频编辑数据集的规模、质量和任务多样性。现有方法依赖视频生成模型或人工标注，难以构建大规模高质量的配对视频编辑数据。

Method: 提出LIVE联合训练框架：1）利用大规模高质量图像编辑数据和视频数据集；2）引入帧级token噪声策略，将特定帧的潜在表示作为推理token，利用预训练视频生成模型创建合理的时间变换；3）通过清理公共数据集和构建自动化数据管道，采用两阶段训练策略来增强视频编辑能力。

Result: 构建了包含60多个挑战性任务的综合评估基准，这些任务在图像编辑中常见但在现有视频数据集中稀缺。广泛的比较和消融实验表明，该方法实现了最先进的性能。

Conclusion: LIVE框架通过有效利用图像编辑数据和创新的帧级token噪声策略，解决了视频编辑数据稀缺的问题，在多样化任务上实现了SOTA性能，为视频编辑领域提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于查询的多视图3D目标检测方法难以有效利用动态多尺度信息，对象特征与查询几何关系学习不足，直接探索多尺度时空特征成本过高。

Method: 提出CAM3DNet框架，包含三个核心模块：1) 复合查询(CQ)模块使用多尺度投影策略将2D查询转换为3D空间；2) 自适应自注意力(ASA)模块学习时空多尺度查询间的交互；3) 多尺度混合采样(MSHS)模块利用可变形注意力机制，结合多尺度查询、金字塔特征图和2D相机先验知识采样多尺度对象信息。

Result: 在nuScenes、Waymo和Argoverse基准数据集上的实验表明，CAM3DNet优于大多数现有的基于相机的3D目标检测方法，并通过消融研究验证了CQ、ASA和MSHS模块的独立效果及其空间和计算复杂度。

Conclusion: CAM3DNet通过创新的复合查询、自适应自注意力和多尺度混合采样模块，有效解决了多视图3D目标检测中的多尺度信息利用问题，在多个基准数据集上取得了优越性能。
...
TL;DR: 提出IMA-MoE模型，通过多模态数据融合来识别暴食症的生物标志物，在ABCD数据集上表现优于基线方法，并揭示性别特异性预测模式。


<details>
  <summary>Details</summary>
Motivation: 暴食症是最普遍的饮食障碍，但当前诊断框架主要基于症状而非生物学机制，限制了早期检测和生物信息干预的发展。现有神经生物学研究因依赖假设驱动模型、单模态分析和有限数据多样性而难以推广。

Method: 提出可解释的模态感知专家混合模型(IMA-MoE)，整合神经影像、行为、激素和人口统计学等多模态数据。将每个测量编码为独立token，灵活建模跨模态依赖同时保留模态特异性特征。引入token重要性机制增强可解释性。

Result: 在大型ABCD数据集上评估，IMA-MoE在区分暴食症与健康对照方面优于基线方法。揭示了性别特异性预测模式，激素测量在女性预测中贡献更显著。
...
<details>
  <summary>Details</summary>
Motivation: 神经退行性疾病具有多因素特性，需要多模态建模进行诊断。然而在真实临床场景中，模态覆盖往往不完整，现有方法难以捕捉个体特异性跨模态依赖关系，且缺乏可解释性。

Method: 提出条件证据重建与分解（CERD）框架：1）基于个体观察输入条件重建缺失模态表征；2）通过logit级归因将诊断证据分解为共享跨模态验证和模态特异性线索。

Result: 在ADNI数据集上的实验表明，CERD在不完整模态设置下优于竞争基线方法，同时产生结构化且与临床对齐的证据归因，为可信决策提供支持。

Conclusion: CERD框架有效解决了多模态诊断中的模态缺失问题，通过条件重建和证据分解实现了高性能和可解释性的平衡，为临床决策提供了可信支持。
...
TL;DR: 提出SIF框架，通过语义对齐的指纹蒸馏和鲁棒指纹优化，为大型视觉语言模型提供隐蔽且鲁棒的版权保护方案，解决现有指纹方法易被检测和移除的问题。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型的公开可访问性引发了未经授权模型重用和知识产权侵权的严重担忧。现有所有权验证方法依赖语义异常查询或分布外响应作为指纹，这些指纹容易被对手检测和移除。

Method: 提出SIF框架：1) 语义对齐指纹蒸馏(SAFD)：将文本水印信号转移到视觉模态，产生语义一致但带有指纹的响应；2) 鲁棒指纹优化(RFO)：通过模拟最坏情况表示扰动增强鲁棒性，使指纹能抵抗微调和量化等模型修改。

Result: 在LLaVA-1.5和Qwen2.5-VL上的广泛实验表明，SIF实现了强大的隐蔽性和鲁棒性，为LVLM版权保护提供了实用解决方案。

Conclusion: SIF框架通过语义对齐的指纹蒸馏和鲁棒优化，解决了现有指纹方法易被检测的漏洞，为大型视觉语言模型提供了有效的版权保护方案。
...
TL;DR: 基于单边缘设备和鱼眼摄像头的实时碰撞预警系统，通过鱼眼校准、目标检测、地面投影和决策层设计，在符合性测试中达到93.3%敏感度和92.3%特异度，平均预警时间3.3秒。


<details>
  <summary>Details</summary>
Motivation: 城市交叉口自行车与行人碰撞事故持续发生，但现有系统很少使用商用硬件为无装备的道路使用者提供实时预警。

Method: 1) 开发超宽鱼眼镜头校准流程，通过透视重映射和直接光束法平差解决角点检测失败和优化器发散问题；2) 结合鱼眼感知目标检测和通过预计算查找表的闭式地面投影；3) 引入设计时符合性模拟，包含24个脚本危险场景、随机尺寸感知检测失败和延迟扫描；4) 将决策层形式化为可分离、可审计的测试平台，具有明确的部署门控、争议机制和剩余风险登记。

Result: 在鱼眼定位误差的符合性测试中，选定管道配置达到93.3%敏感度和92.3%特异度，平均预警预算3.3秒，超过分心行人反应时间。

Conclusion: 该系统展示了使用商用硬件为无装备道路使用者提供实时碰撞预警的可行性，通过社区辅助设计研讨会完善系统设计，代码和复制脚本已开源。
...
TL;DR: OASIS是一个用于流式视频推理的新框架，通过结构化按需检索来解决长历史中的信息冗余问题，采用分层事件组织和基于不确定性的检索机制。


<details>
  <summary>Details</summary>
Motivation: 流式视频推理面临历史无限增长但关键证据稀少的挑战，传统方法要么扩大内存（增加冗余），要么过度压缩（丢失关键信息），核心困难在于如何定位关键信息而非记住多少信息。

Method: 将流式历史组织为分层事件，采用控制细化推理：先进行短上下文推理，仅当出现不确定性时进行语义驱动的检索（而非嵌入相似性检索），实现即插即用、无需训练的架构。

Result: 在多个基准测试和骨干网络上，OASIS在长时域准确性和组合推理方面取得显著提升，同时保持有界的token成本和低请求延迟。

Conclusion: OASIS通过结构化按需检索有效解决了流式视频推理中的信息定位难题，提供了一种高效、准确且易于部署的解决方案。
...
TL;DR: 提出无需训练的指令引导多模态嵌入框架，利用MLLM将文本、光栅图像和SVG代码映射到对齐的嵌入空间，通过模态特定指令和SVG结构线索控制嵌入方向，在文本到SVG检索任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: SVG既作为视觉图像又作为结构化代码，包含丰富的几何和布局信息，但现有方法通常将其栅格化并丢弃符号结构。同时，现有的句子嵌入方法主要针对文本，难以扩展到视觉或结构化模态。

Method: 提出训练免费的多模态嵌入框架，包含两个关键组件：1) 多模态显式单字限制(mEOL)，指导MLLM将任何多模态输入总结为单个token，其隐藏状态作为紧凑语义嵌入；2) 语义SVG重写模块，通过视觉推理为SVG元素分配有意义的标识符并简化嵌套结构，暴露原始代码中隐藏的几何和关系线索。

Result: 在重新设计的VGBench上构建了首个文本到SVG检索基准，实验表明该训练免费嵌入方法优于基于编码器和训练的多模态基线方法。

Conclusion: 提示级控制是参数级训练的有效替代方案，可用于实现结构感知的多模态检索，为多模态表示学习提供了新的训练免费范式。
...
TL;DR: 提出一个增强CLIP的零样本动作识别框架，通过解耦嵌入和语义引导交互，使用运动分离模块和运动聚合块，结合正负文本提示实现语义对齐，在标准基准上优于现有CLIP方法。


<details>
  <summary>Details</summary>
Motivation: 零样本动作识别面临已见类别和未见类别之间的语义鸿沟挑战，需要更好的方法来弥合视觉和文本表示之间的差距。

Method: 1. 运动分离模块（MSM）分离运动敏感特征和全局静态特征；2. 运动聚合块（MAB）使用门控交叉注意力精炼运动表示，避免重新耦合冗余信息；3. 通过将投影嵌入与正文本提示对齐实现语义对齐，同时利用负提示明确建模"非类别"语义。

Result: 在标准基准测试中，该方法始终优于先前的基于CLIP的方法，在粗粒度和细粒度数据集上都实现了稳健的零样本动作识别。

Conclusion: 提出的框架通过解耦特征表示和语义引导交互，有效增强了CLIP在零样本动作识别中的性能，为解决已见和未见类别之间的语义鸿沟提供了有效解决方案。
...
TL;DR: BasketHAR是一个专门为篮球训练设计的新型多模态人类活动识别数据集，包含专业级动作、多种传感器数据和同步视频，用于填补现有HAR数据集在体育分析领域的空白。


<details>
  <summary>Details</summary>
Motivation: 现有HAR数据集主要关注基本日常活动（如行走、站立、上下楼梯），在体育训练等专业场景中应用有限。篮球训练需要识别更复杂的专业动作，但缺乏相应的多模态数据集。

Method: 创建BasketHAR数据集，包含惯性测量单元（加速度计、陀螺仪）、角速度、磁场、心率、皮肤温度等多种传感器数据，以及同步视频记录。提供基准多模态对齐方法进行性能评估。

Result: 实验结果表明数据集具有足够的复杂性，适合高级HAR任务。数据集已公开在Hugging Face平台，采用Apache License 2.0许可证。

Conclusion: BasketHAR填补了体育分析领域HAR数据集的空白，为篮球训练分析、专业性能报告生成提供了宝贵资源，有助于推动HAR和体育分析研究。
...
TL;DR: NTIRE 2026 RipDetSeg挑战赛旨在通过自动检测和分割图像中的离岸流，解决海滩安全关键问题。挑战基于RipVIS基准，吸引了159名参与者，最终产生9个有效测试提交。


<details>
  <summary>Details</summary>
Motivation: 离岸流是危险的海岸流，导致全球许多海滩相关死亡事故，但由于其视觉外观在不同海滩、视角和海况下差异很大，难以识别。需要开发自动识别方法以提高海滩安全。

Method: 挑战赛基于RipVIS基准数据集，该数据集来自10多个国家，包含4种相机方向和多样化的海滩和海况。评估方法采用综合评分，结合F1[50]、F2[50]、F1[40:95]和F2[40:95]指标。

Result: 大多数参与者解决方案依赖预训练模型，结合强大的数据增强和后处理设计。结果表明，离岸流理解从通用视觉模型的进展中获益显著，但仍有很大改进空间。

Conclusion: 挑战赛展示了当前离岸流检测和分割的技术水平，表明通用视觉模型对此问题有效，但未来需要针对离岸流独特视觉结构开发专门方法。
...
TL;DR: 提出RefVQA方法，从视频间关系视角重新审视AIGC视频质量评估，通过查询中心参考图组织语义相关样本，实现参考感知的质量评估，超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有AIGC视频质量评估方法通常独立分析每个视频，忽略了视频间的潜在关系。人类感知质量时往往会进行视频间比较，因此需要从视频间关系角度重新审视AIGC-VQA问题。

Method: 提出Reference-aware Video Quality Assessment (RefVQA)方法，将AIGC-VQA重新表述为参考感知评估问题。使用查询中心参考图组织语义相关样本，通过图引导的差异聚合从参考节点到查询节点进行质量评估。

Result: 在现有数据集上的实验表明，RefVQA在多个质量维度上优于最先进的方法，交叉数据集评估验证了其强大的泛化能力。

Conclusion: 提出的基于参考的AIGC-VQA框架有效且具有潜力，视频间关系视角能够更好地模拟人类感知，为AIGC视频质量评估提供了新思路。
...
TL;DR: D-Prism：首个通过将可微分基元扩展到动态领域，实现高保真结构化动态建模的框架，将3DGS绑定到基元表面，结合两者的优势


<details>
  <summary>Details</summary>
Motivation: 现有动态方法（如可变形网格或3DGS）依赖非结构化表示，无法同时建模合适的几何结构和关节运动；基于基元的方法擅长结构化静态场景，但其动态潜力尚未探索

Method: 1) 将3DGS绑定到基元表面，利用各自在外观和几何上的优势；2) 引入变形网络控制基元运动，确保准确匹配物体运动；3) 设计自适应控制策略动态调整基元数量，更好匹配物体真实空间占用

Result: 实验证实该方法在结构化动态建模方面表现出色，既能提供结构化几何，又能实现精确的运动跟踪

Conclusion: D-Prism是首个实现高保真结构化动态建模的框架，成功将可微分基元扩展到动态领域，为结构化动态物体建模提供了有效解决方案
...
TL;DR: EvoComp是一个视觉令牌压缩框架，通过轻量级编码器选择信息量最大且非冗余的视觉令牌，在保持任务准确性的同时显著减少令牌数量，实现推理加速。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉语言理解任务中表现出色，但高分辨率或多图像场景下的大量视觉令牌严重影响了推理效率，需要在不损失准确性的前提下减少令牌数量。

Method: 提出基于轻量级编码器-仅转换器的压缩器，通过进化标注策略寻找最小化MLLM输出损失的令牌子集，使用词汇表分组确保语义多样性，并采用GHM损失和余弦相似度正则化的组合损失函数。

Result: 在多个视觉语言基准测试中优于现有的注意力或相似性启发式方法，在3倍令牌压缩下保持99.3%的原始准确率，在移动设备上实现高达1.6倍的加速。

Conclusion: EvoComp通过联合考虑视觉和文本上下文的有效令牌压缩，显著提高了多模态大语言模型的推理效率，同时保持了任务准确性，为高效视觉语言理解提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究通常将动作识别和运动生成作为两个独立问题处理，忽略了它们之间的内在联系。运动生成需要语义理解能力，而动作识别可以提供这种语义指导，因此需要统一的建模框架。
...
Conclusion: CoAMD通过统一建模动作识别和运动生成，利用骨架坐标和多模态语义指导，为人体运动建模提供了有效且通用的解决方案，在多个任务上表现出色。
...
TL;DR: 提出HBR-Net-18两阶段AI框架，结合Hadamard U-Net消除HM-MRI偏置场，再用ResNet-18进行前列腺癌斑块分类，显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 虽然混合多维MRI提高了前列腺癌诊断能力，但仍缺乏稳健的自动化AI检测方法，需要结合定量组织成分分析与神经网络技术。

Method: 两阶段框架：第一阶段使用Hadamard U-Net算法抑制六个参数化HM-MRI图的强度不均匀性；第二阶段使用ResNet-18进行11×11重叠斑块的分类，结合2D切片内和3D切片间信息。

Result: HB-Net实现了平衡的敏感性和特异性，显著优于传统放射组学方法和基线CNN模型，显示出临床部署潜力。

Conclusion: 提出的HBR-Net-18框架有效结合了物理信息自编码器生成的定量HM-MRI与神经网络，为前列腺癌检测提供了有前景的自动化解决方案。
...
TL;DR: 开发了一个让临床医生通过自然语言交互自主开发AI模型的系统原型，在多个临床任务上验证了可行性，有望改变传统依赖AI专家的协作模式。


<details>
  <summary>Details</summary>
Motivation: 传统临床AI开发需要临床医生与AI专家团队紧密协作，这个过程耗时且容易因双方专业知识差异导致需求理解偏差。自主编码代理可能改变这一范式，让临床医生能够独立开发AI模型。

Method: 开发了一个自主原型系统，让临床医生通过自然语言交互驱动临床AI开发。在五个临床任务上评估：皮肤镜病变分类、黑色素瘤与痣分诊、腕部骨折检测（包括仅有5%边界框标注的弱监督变体）、以及胸部X光片上消除偏倚的气胸分类。

Result: 系统能够根据临床医生需求一致地开发模型并取得有希望的性能。在消除偏倚的气胸分类任务中，系统成功缓解了捷径学习，将模型对胸管的依赖几乎减半。胸管是该任务中的主要混淆因素。

Conclusion: 自主编码代理可能帮助将临床AI开发转向更临床医生驱动的范式，减少沟通开销和对专业AI开发者的依赖。虽然需要进一步验证和鲁棒性评估，但这项研究为让临床AI开发更易获取提供了有希望的路径。
...
TL;DR: 提出一种推理时的时间概率平滑方法，通过光流运动扭曲和像素级不确定性估计，改善SAM2视频分割的时间稳定性，无需重新训练或修改架构。


<details>
  <summary>Details</summary>
Motivation: SAM2等交互式视频分割模型在弱用户监督（如单帧稀疏点提示）下存在时间不稳定问题，包括边界闪烁、物体丢失和跨帧物体范围不一致，限制了其在视频理解和控制应用中的可靠性。

Method: 提出推理时时间概率平滑方法，直接在每帧分割概率图上操作，利用基于光流的运动扭曲、分割熵导出的像素级不确定性估计以及前后向流一致性，自适应地混合当前帧预测与运动对齐的历史估计。

Result: 在四个不同视频序列上评估，使用包括运动补偿IoU、边界一致性、物体持久性和面积波动在内的综合指标，实验结果显示在保持空间精度的同时，时间稳定性相比原始SAM2推理有持续改进。

Conclusion: 该方法轻量级、模型无关，适用于实时交互式视频分割，能有效提高弱提示下SAM2视频分割的时间稳定性。
...
TL;DR: 提出一个整合组织病理学图像和临床数据的多模态乳腺癌诊断框架，通过中间融合策略显著提升分类性能，特别是在关键但样本不平衡的有丝分裂类别上。


<details>
  <summary>Details</summary>
Motivation: 乳腺癌是全球癌症相关死亡的主要原因，及时准确诊断对改善生存结果至关重要。现有研究通常将组织病理学图像和电子健康记录这两种模态分开处理，缺乏有效的多模态整合方法。

Method: 提出系统性多模态框架，整合BreCaHAD数据集的patch级组织病理学特征和MIMIC-IV的结构化临床数据。训练和评估单模态图像模型（简单CNN基线和ResNet-18迁移学习）、单模态表格模型（XGBoost和多层感知机），以及中间融合模型（拼接两种模态的潜在表示）。

Result: ResNet-18在三类patch级分类上达到近乎完美的准确率（1.000）和AUC（1.000），XGBoost在EHR预测任务上达到98%准确率。中间融合模型获得0.997的宏平均AUC，优于所有单模态基线，在关键但有类别不平衡的有丝分裂类别上提升最大（AUC 0.994）。

Conclusion: 多模态整合在预测性能和临床透明度方面都带来有意义的改进，Grad-CAM和SHAP可解释性分析验证了模型决策与既定病理和临床标准的一致性。
...
TL;DR: 研究发现视觉语言模型在开放词汇目标定位中存在提示不稳定性：语义等价的描述（如"人"、"人类"、"行人"）经常选择不同的实例，平均不稳定度为2.11个不同选择


<details>
  <summary>Details</summary>
Motivation: 研究视觉语言模型在开放词汇目标定位中的一个隐含假设：语义等价的描述应该产生一致的输出。作者质疑这一假设的有效性，探索模型对重叠提示的响应稳定性。

Method: 使用受控管道，结合DETR生成对象提案和CLIP进行语言条件选择，在263个COCO val2017图像上测试。分析六个语义等价提示（如"人"、"人类"、"行人"）的选择一致性，进行PCA分析，并研究提示集成和文本嵌入距离对选择稳定性的影响。

Result: 发现语义等价提示经常选择不同实例，平均不稳定度为2.11个不同选择。PCA分析显示这种变异性是结构化和方向性的，而非随机。提示集成不能改善质量，反而常使选择偏向通用区域。文本嵌入接近度仅能解释34%的定位分歧（r = -0.58），表明不稳定性源于argmax选择机制而非文本级距离。

Conclusion: 视觉语言模型在开放词汇目标定位中存在显著的提示不稳定性问题，语义等价的描述不能保证一致输出。这种不稳定性源于模型的选择机制而非文本表示差异，提示集成无法解决此问题，揭示了当前模型在实际应用中的局限性。
...
TL;DR: OptiMVMap通过优化选择多车辆视角来提升离线矢量地图构建质量，解决单视角不足问题，在nuScenes和Argoverse2数据集上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于单车辆轨迹的方法存在视角不足问题：虽然基于记忆的方法通过聚合自车轨迹帧扩展了观察时间，但缺乏空间多样性来揭示被遮挡区域。引入周围车辆视角提供互补视角，但简单融合面临三大挑战：大规模候选池的计算成本、近共线视角的冗余性，以及位姿误差和遮挡伪影带来的噪声。

Method: OptiMVMap将多车辆建图重新表述为"先选择后融合"问题：1) 最优车辆选择模块战略性地识别紧凑的辅助车辆子集，最大化减少自车在被遮挡区域的不确定性；2) 跨车辆注意力机制和语义感知噪声滤波器在位姿容忍对齐和伪影抑制后，在BEV级别进行融合。

Result: 在nuScenes和Argoverse2数据集上，OptiMVMap分别将MapTRv2提升了+10.5 mAP和+9.3 mAP，在nuScenes上超越了记忆增强基线MVMap和HRMapNet分别+6.2 mAP和+3.8 mAP。结果表明，不确定性引导的辅助车辆选择对于高效准确的多车辆矢量建图至关重要。

Conclusion: 通过系统性地解决多视角融合的计算成本、冗余性和噪声挑战，OptiMVMap能够用更少的视角生成更完整、拓扑保真度更高的地图，证明了不确定性引导的车辆选择策略在多车辆矢量建图中的重要性。
...
TL;DR: ScenarioControl是首个用于学习驾驶场景生成的视觉语言控制机制，能够根据文本提示或输入图像合成多样化、真实的3D场景推演，包括地图、动态参与者、行人、驾驶基础设施和自车视角观察。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶场景生成方法缺乏对场景布局和交通条件的细粒度控制能力，无法根据文本或视觉输入灵活生成符合特定要求的驾驶场景。需要一种能够连接多模态控制与稀疏向量化场景元素的机制。

Method: 提出跨全局控制机制，将交叉注意力与轻量级全局上下文分支相结合，在向量化潜在空间中联合表示道路结构和动态参与者。该方法支持从场景中不同参与者的视角生成时间一致的场景推演，并支持长时域场景延续。

Result: 实验验证表明，ScenarioControl在控制依从性和保真度方面优于所有测试方法。方法能够生成多样化的真实3D场景，包括地图、3D边界框、行人、基础设施和自车视角观察。

Conclusion: ScenarioControl首次实现了基于视觉语言控制的驾驶场景生成，通过创新的跨全局控制机制实现了对道路布局和交通条件的细粒度控制，同时保持场景真实性，为自动驾驶系统的训练和评估提供了强大工具。
...
TL;DR: 提出一种高效将2D图像信息（颜色、神经特征、分割掩码）反向映射到3D高斯泼溅场景的方法，相比基于梯度下降的基线方法实现数量级加速


<details>
  <summary>Details</summary>
Motivation: 当前高斯泼溅框架主要关注从3D到2D的高效映射，但缺少将2D信息（如颜色、特征、分割掩码）高效反向映射到3D场景的方法，而这种反向映射对于场景重光照、风格化、3D语义分割等应用至关重要

Method: 使用法线方程求解每个高斯的可见性加权最小二乘问题，该方法可利用现有可微分光栅化器高效实现，解决了视角依赖着色和遮挡处理等挑战

Result: 在场景重光照、特征丰富化和3D语义分割任务上验证了方法的有效性，相比基于梯度下降的基线方法实现了高达一个数量级的加速

Conclusion: 提出了一种高效将2D信息反向映射到3D高斯泼溅场景的通用方法，为场景重光照、风格化和3D语义分割等应用提供了实用解决方案
...
TL;DR: PPEDCRF是一个选择性扰动框架，通过动态条件随机场识别位置敏感背景区域，注入高斯噪声保护视频帧的位置隐私，相比全局噪声在相同隐私水平下提供更高视觉质量。


<details>
  <summary>Details</summary>
Motivation: 即使移除GPS元数据，攻击者仍可通过匹配背景视觉特征与地理标记参考图像来定位视频帧。现有方法通常使用全局噪声，这会过度降低视觉质量。需要一种能选择性保护位置敏感区域的方法。

Method: 1) 使用动态条件随机场(DCRF)估计位置敏感背景区域；2) 通过归一化控制惩罚(NCP)重新调整扰动强度；3) 在推断区域内使用DP风格校准规则注入高斯噪声。

Result: 在8个攻击骨干和3个噪声种子的基准测试中，PPEDCRF将ResNet18的Top-1检索准确率从0.667降至0.361±0.127(σ₀=8)，同时保持36.14dB PSNR，比全局高斯噪声高约6dB质量优势。

Conclusion: PPEDCRF通过空间集中扰动在相同隐私水平下提供更高视觉质量，而非更强的隐私保护。主要优势在于选择性保护位置敏感区域，减少对整体图像质量的损害。
...
TL;DR: LookasideVLN提出了一种利用自然语言中方向线索的新范式，通过构建Egocentric Lookaside Graph（ELG）和Spatial Landmark Knowledge Base（SLKB），在保持计算效率的同时显著提升Aerial VLN的导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有Aerial VLN方法主要依赖地标描述，忽视了方向线索这一人类导航中的关键空间上下文信息，导致指令理解浅层且计算成本高。

Method: 提出LookasideVLN框架，包含三个核心组件：1) ELG动态编码指令相关地标及其方向关系；2) SLKB提供轻量级记忆检索；3) Lookaside MLLM导航代理对齐多模态信息进行路径规划。

Result: 实验表明LookasideVLN显著优于当前最优的CityNavAgent，即使仅使用单层前瞻也能实现更好性能，证明利用方向线索是Aerial VLN中强大而高效的策略。

Conclusion: 通过有效利用自然语言中的方向线索，LookasideVLN在保持计算效率的同时实现了更准确的空间推理，为Aerial VLN提供了一种新的高效范式。
...
TL;DR: DreamShot是一个基于视频扩散模型的故事情板生成框架，通过利用视频生成先验实现可控的多镜头合成，解决了现有文本到图像模型在长时序一致性、角色身份保持和叙事连贯性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有故事情板合成方法大多基于文本到图像扩散模型，这些模型难以保持长时序一致性、角色身份一致性和跨多个镜头的叙事流畅性。需要一种能够生成视觉和语义连贯序列的新方法。

Method: 基于视频生成模型的框架，支持文本到镜头和参考到镜头生成，以及基于先前帧的故事延续。包含多参考角色条件模块，通过角色注意力一致性损失强制身份对齐，约束参考角色与生成角色之间的注意力。

Result: DreamShot在场景连贯性、角色一致性和生成效率方面优于最先进的文本到图像故事情板模型，建立了可控视频模型驱动视觉叙事的新方向。

Conclusion: DreamShot通过充分利用视频扩散先验，实现了视觉和语义连贯的故事情板序列生成，在叙事保真度和角色连续性方面表现出色，为可控视频模型驱动的视觉叙事开辟了新方向。
...
TL;DR: SciDraw-6K是一个包含6,291张科学插图的合成数据集，由Google Gemini图像生成模型创建，支持11种语言提示，涵盖8个科学类别，专门用于科学插图类型研究。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像研究主要关注通用领域，缺乏专门针对科学插图（如示意图、机制图、目录图形等）的多语言数据集。SciDraw-6K旨在填补这一空白，支持科学可视化领域的多语言文本到图像研究。

Method: 使用Google Gemini图像生成模型（主要是gemini-2.5-flash-image和gemini-3-pro-image-preview）合成6,291张科学插图，每张图都配有11种语言的提示词。数据集涵盖8个科学类别，构建了专门的科学插图生成管道。

Result: 创建了SciDraw-6K数据集，包含6,291张科学插图，支持11种语言，涵盖8个科学类别。该数据集已用于支持sci-draw.com公共科学绘图服务，并公开发布以支持相关研究。

Conclusion: SciDraw-6K是一个专门为科学插图类型设计的多语言数据集，支持文本到图像研究、领域适应的扩散模型微调以及科学可视化的提示工程研究，填补了该领域的空白。
...
TL;DR: CDSA-Net提出了一种新的冠状动脉数字减影血管造影框架，通过解耦血管结构保存和背景恢复，解决了现有方法中的边界伪影和灰度保真度问题。


<details>
  <summary>Details</summary>
Motivation: 冠状动脉DSA受到生理运动的挑战，现有深度学习方法存在两个临床不可接受的缺陷：持续的边界伪影和原生组织灰度保真度损失，这影响了诊断信心。

Method: CDSA-Net包含两个核心创新：1）分层几何先验引导机制（HGPG），嵌入冠状动脉结构提取网络（CSENet），结合集成几何先验、门控空间调制和中心线感知拓扑损失监督；2）冠状动脉背景恢复网络（CBResNet）中的自适应噪声模块（ANM），建模临床X射线噪声的随机性。

Result: 在血管强度相关性和感知质量方面显著优于最先进方法，形态评估效率提高25.6%，血流动力学评估速度提高42.9%，同时保持与原始血管造影一致的诊断结果。

Conclusion: CDSA-Net为介入心脏病学设定了新的实用基准，通过解耦和联合优化血管结构保存和真实背景恢复，解决了冠状动脉DSA中的关键临床问题。
...
TL;DR: DREAM框架通过动态多模态融合和对比对齐，在数据稀缺的视网膜图像医疗报告生成任务中实现最先进性能


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型在数据稀缺的医学专业领域容易过拟合，难以捕捉细微但关键的病理特征，需要结合医学专业知识来提升医疗报告生成质量

Method: DREAM采用两阶段融合机制：1) Abstractor模块将图像和临床关键词特征映射到共享空间；2) Adaptor模块通过可学习参数动态加权多模态重要性；3) Contrastive Alignment模块在训练中对齐融合表示与真实医疗报告

Result: 在DeepEyeNet基准测试中达到BLEU-4分数0.241的新SOTA，并在ROCO数据集上表现出良好的泛化能力

Conclusion: DREAM通过结合医学专业知识和高效融合策略，在数据稀缺的视网膜图像医疗报告生成任务中实现了显著性能提升，为医学AI应用提供了有效解决方案
...
TL;DR: EmbodiedHead是一个语音驱动的说话头框架，为LLM配备实时视觉化身，实现实时生成、统一的听-说行为和高视觉质量。


<details>
  <summary>Details</summary>
Motivation: 构建实用的具身化身需要同时实现实时生成、统一的听-说行为和高质量的渲染视觉效果。现有方法存在双流音频依赖、听-说切换不自然等问题。

Method: 采用Rectified-Flow Diffusion Transformer (DiT)结合可微分渲染器，使用单流接口配合显式的逐帧听-说状态条件化和流式音频调度器，采用系数空间预训练和图像域联合精调的两阶段训练方案。

Result: 在听和说场景中都实现了最先进的视觉质量和运动保真度，能够在仅4个采样步骤中生成多样且高保真的结果。

Conclusion: EmbodiedHead框架成功解决了实时说话头生成中的关键挑战，为LLM提供了高质量的视觉化身，支持自然的听-说交互。
...
TL;DR: 本文提出对抗性评估框架量化VLM中的文本捷径学习问题，通过几何形状数据集上的四种对抗策略，优化模型将平均准确率下降从27.5%减少到9.8%。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型存在过度依赖文本描述而忽视视觉证据的问题（文本捷径学习），需要量化评估这种跨模态依赖并改进模型。

Method: 提出对抗性评估框架，使用四种对抗策略（形状交换、颜色交换、位置交换、随机文本）在几何形状数据集上测试。比较三种配置：基线CLIP、LoRA微调、以及整合多种优化技术的LoRA优化模型。

Result: 优化模型将平均准确率下降从27.5%减少到9.8%（相对改进64.4%），同时保持97%的正常准确率。注意力可视化和嵌入空间分析证实优化模型更关注视觉特征并实现更紧密的跨模态对齐。

Conclusion: 对抗性评估有效量化文本捷径学习问题，通过整合多种优化技术的LoRA微调能显著减少模型对文本的过度依赖，提升视觉特征的利用和跨模态对齐。
...
TL;DR: 提出针对DUV-WSI乳腺癌分类的区域亲和力注意力机制，无需切片处理全玻片图像，通过建模局部邻域距离和构建全亲和矩阵动态突出诊断相关区域，在136个样本上达到92.67%准确率和95.97% AUC。


<details>
  <summary>Details</summary>
Motivation: 传统组织病理学方法在术中诊断中效率不足，而现有基于切片的深度学习方法会破坏空间上下文信息，且标准注意力机制无法充分利用DUV-WSI数据中丰富的多尺度区域关系。

Method: 提出区域亲和力注意力机制，直接处理全玻片图像避免切片分割，通过建模局部邻域距离和构建全亲和矩阵来动态突出诊断相关区域，并采用对比损失增强特征可区分性。

Result: 在136个DUV-WSI样本数据集上，该方法达到92.67±0.73%的准确率和95.97%的AUC，优于现有注意力方法。

Conclusion: 区域亲和力注意力机制为DUV-WSI乳腺癌分类提供了一种有效方法，能够保持空间完整性并充分利用多尺度区域关系，在准确率和AUC方面均优于现有方法。
...
TL;DR: 提出用于硬盘驱动器自动拆解的自主视觉管道，整合3D感知（FPP与深度补全）与实时实例分割，实现像素级对齐的语义和空间数据，支持机器人拆解


<details>
  <summary>Details</summary>
Motivation: 未回收电子废弃物造成重大经济损失，硬盘驱动器作为有价值的电子废弃物需要机器人拆解。现有方法碎片化、缺乏鲁棒的3D感知和紧固件定位能力

Method: 提出自主视觉管道：1) 使用条纹投影轮廓术进行3D感知，在FPP失败时选择性触发深度补全模块；2) 集成轻量级实时实例分割网络进行场景理解和关键组件定位；3) 利用同一FPP相机-投影仪系统实现深度图与分割掩码的像素级对齐；4) 采用模拟到真实的迁移学习方法增强物理数据集

Result: 实例分割达到box mAP@50 0.960和mask mAP@50 0.957；深度补全使用Depth Anything V2 Base骨干网络实现RMSE 2.317 mm和MAE 1.836 mm；推理堆栈延迟12.86 ms，吞吐量77.7 FPS；开发了公开可用的HDD实例分割合成数据集

Conclusion: 提出的感知管道提供高保真语义和空间数据，对下游机器人拆解有价值。通过统一FPP系统实现深度感知和组件定位的像素级对齐，优于常见的RGB-D工业感知系统
...
TL;DR: P-MLLM：基于用户画像的多模态大语言模型，用于零样本个性化图像美学评估，通过选择性融合模块实现画像感知的视觉信息整合


<details>
  <summary>Details</summary>
Motivation: 现有个性化图像美学评估方法依赖用户历史评分数据，在零样本场景（无历史数据）下表现不佳。需要利用用户画像作为上下文信号来实现零样本个性化评估。

Method: 提出P-MLLM模型，在冻结的LLM基础上添加选择性融合模块，在画像条件推理过程中有选择地将视觉信息整合到模型的隐藏状态中，实现画像感知的视觉信息融合。

Result: 在最近的PIAA基准测试中，P-MLLM取得了有竞争力的零样本性能，即使在粗略的画像信息下也保持有效，证明了基于画像的个性化方法在零样本PIAA中的潜力。

Conclusion: 基于用户画像的个性化范式为无历史评分数据的零样本个性化图像美学评估提供了有效解决方案，P-MLLM展示了画像感知多模态LLM在该任务上的潜力。
...
TL;DR: 论文提出RemoteShield，一种针对地球观测的鲁棒多模态大语言模型，通过偏好学习在干净和扰动样本间进行跨条件对齐，以应对现实中的视觉退化（如云、雾）和文本噪声，提升模型在真实部署条件下的稳定性。


<details>
  <summary>Details</summary>
Motivation: 当前遥感多模态大语言模型在精心清洗的数据集上训练，学习到脆弱的映射关系，无法泛化到真实地球观测中的噪声条件。当面对部署中的不完美输入时，性能会显著下降。需要量化这种脆弱性并构建鲁棒的模型。

Method: 构建真实的多模态扰动集（视觉退化如云、雾覆盖，以及多样的人类中心文本变化）。提出RemoteShield模型，将每个干净样本与其图像-文本扰动变体配对形成语义等价簇。通过偏好学习优化模型，在同一个簇内比较干净和扰动条件下的模型响应，鼓励模型选择稳定响应而非扰动诱导的失败。

Result: 实验表明，现实多模态扰动显著损害了领先遥感基础模型的视觉语义推理能力。在三个地球观测任务上，RemoteShield相比代表性基线方法，在现实多模态扰动下始终表现出更强的鲁棒性和跨条件一致性。

Conclusion: RemoteShield通过跨条件对齐帮助模型专注于底层任务语义，尽管存在视觉退化和文本噪声。该方法为构建鲁棒的地球观测多模态大语言模型提供了有效解决方案，能够在现实部署条件下保持一致的输出。
...
TL;DR: 该论文提出了一种基于分形理论的新方法，通过分析低相关信号的统计特性来检测AI生成图像，相比现有方法在开放世界场景中具有更好的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: AI生成图像已达到接近真实照片的保真度，这对信息安全和社会信任构成重大威胁。现有的深度伪造检测方法在开放世界场景中鲁棒性有限，需要从信号层面寻找新的检测方向。

Method: 从信号层面分析合成图像与真实图像的内在差异，发现低相关信号可作为区分标志。基于分形理论量化这些信号，通过分析低相关信号的分形特征来捕捉合成过程中的统计异常。

Result: 大量实验结果表明该方法具有鲁棒性和优越的检测性能。该方法理论上不仅限于人脸图像识别，可应用于所有AI生成图像检测任务。

Conclusion: 该研究强调了将研究重点转向信号层面新方向的重要性，为深度伪造检测提供了新的研究方向。基于分形理论的方法能有效检测AI生成图像，具有广泛的应用潜力。
...
TL;DR: 提出一种多模态大语言模型置信度估计方法，通过融合模型隐式token级支持与显式自评估置信度，解决两者不一致问题，提升校准和故障预测能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在实际部署中需要可靠的置信度估计，但现有研究主要关注文本模型且依赖计算昂贵的自一致性采样。研究发现MLLMs存在本能-反思不一致问题：模型隐式token级支持与显式自评估置信度经常不一致。

Method: 提出单调置信度融合框架，融合双通道信号（隐式token级支持和显式自评估置信度）和跨通道一致性来估计正确性。然后应用保序均值对齐步骤校正全局偏差，在保持选择性预测风险-覆盖权衡的同时改善校准。

Result: 在多种开源和闭源MLLMs上的实验表明，该方法能产生更可靠的置信度估计，同时改善校准和故障预测能力。

Conclusion: 通过解决MLLMs中本能-反思不一致问题，提出的置信度估计方法能显著提升模型在实际部署中的可靠性。
...
TL;DR: PestVL-Net：一种融合视觉-语言框架的新型害虫识别方法，通过RWKV架构和MLLMs先验实现细粒度害虫学习，在多个数据集上验证了优越性能。


<details>
  <summary>Details</summary>
Motivation: 实际农业场景中害虫数据收集困难，害虫种类繁多且形态特征复杂多样，现有技术难以有效建模害虫的细粒度视觉和高级语义特征，限制了实际应用。

Method: 提出PestVL-Net视觉-语言框架，视觉路径采用RWKV架构结合显著性引导的自适应窗口划分方案，语言组件利用MLLMs先验生成精确害虫语义描述，通过多模态思维链推理实现深度特征融合。

Result: 在多个害虫数据集上的广泛实验评估验证了PestVL-Net的优越性能，展示了其在现实世界害虫管理中的潜力。

Conclusion: PestVL-Net通过视觉和文本表示的深度融合实现了细粒度多模态害虫学习，为解决实际农业场景中的害虫识别和管理挑战提供了有效方案。
...
TL;DR: DepthVAR通过自适应分配每token的计算深度而非硬剪枝，实现VAR模型2.3-3.1倍加速且质量损失最小


<details>
  <summary>Details</summary>
Motivation: 现有VAR模型对每个位置使用固定计算深度效率低下，而基于频率图的硬剪枝方法存在根本限制，无法在改进频率估计时提升质量。观察到VAR模型存在显著深度冗余，需要从剪枝整个token转向自适应分配每token计算深度

Method: 提出DepthVAR框架：1）自适应深度调度器，通过循环旋转调度分配计算深度，实现平衡、非静态的细化；2）动态推理过程，将深度转换为层主掩码，选择性应用transformer块，并混合生成的代码，确保每个token的影响与其处理深度成比例

Result: 实验显示DepthVAR实现2.3-3.1倍加速，质量损失最小，相比现有硬剪枝方法提供更具竞争力的计算-性能权衡

Conclusion: DepthVAR通过自适应分配每token计算深度而非硬剪枝，为VAR模型提供了更高效的加速方案，在保持质量的同时显著提升推理速度
...
TL;DR: 提出GraphSpecForge框架，通过分析预训练Stable Diffusion U-Net中注意力图的谱结构来检测复制-移动伪造，无需训练即可在多个基准测试上实现有效检测。


<details>
  <summary>Details</summary>
Motivation: 复制-移动伪造（图像区域复制以隐藏或伪造内容）对视觉媒体完整性构成持续威胁。现有方法通常需要针对特定伪造类型进行训练，而本文旨在开发无需训练即可检测此类伪造的通用框架。

Method: 基于预训练Stable Diffusion U-Net的注意力图构建自注意力图，分析复制-移动操作导致的近似子图复制现象。通过归一化图拉普拉斯算子的谱重分布来量化这种影响，使用Wasserstein距离比较每张图像的拉普拉斯谱与真实参考分布，构建图像级异常检测器。

Result: 在四个复制-移动基准测试上无需特定伪造重新训练：RecodAI-LUC上AUROC=0.606，归一化拉普拉斯比原始注意力谱提升+0.057 AUROC；MICC-F220、CoMoFoD和COVERAGE上AUROC分别为0.752、0.774和0.673；CoMoFoD上还达到AUPRC=0.833，平衡准确率=0.712，MCC=0.499，TPR@1%FPR=32.5%。消融和证伪实验确认了信号的特异性和对操纵强度的敏感性。

Conclusion: GraphSpecForge证明了通过分析预训练扩散模型注意力图的谱结构可以有效地检测复制-移动伪造，无需针对特定伪造类型进行训练。该方法为图像伪造检测提供了新的谱分析视角，并展示了预训练视觉模型在取证任务中的潜力。
...
TL;DR: 提出FReMuRe模型，通过频率引导的关系多级推理解决视频场景图生成中的长尾分布问题，显著提升长尾关系召回率和整体推理鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有视频场景图生成方法在处理长尾分布关系时存在局限性，难以有效建模长尾关系，需要从机制角度提升长尾关系建模能力。

Method: 1. 引入关系特定分支处理梯度冲突，实现更平衡和尾部感知的学习；2. 设计频率感知双分支谓词嵌入网络，分别建模高频和低频关系，通过门控融合提升尾部类召回率；3. 提出两种可互换关系分类头：用于不确定性估计的贝叶斯头和增强类内多样性的高斯混合模型头。

Result: 在Action Genome数据集上的实验结果表明，FReMuRe显著提高了长尾关系的召回率和整体推理鲁棒性。

Conclusion: FReMuRe模型通过频率引导的多级推理机制，有效解决了视频场景图生成中的长尾分布问题，为高级视频理解提供了更鲁棒的结构化语义表示。
...
TL;DR: NTIRE 2026移动端真实世界图像超分辨率挑战赛综述，聚焦移动设备上的高效超分辨率模型设计，评估结合图像质量和速度的加权指标。


<details>
  <summary>Details</summary>
Motivation: 推动移动设备上的真实世界图像超分辨率技术发展，解决在未知退化条件下从低分辨率图像恢复高分辨率图像的挑战，同时确保模型能在移动设备上高效运行。

Method: 通过挑战赛形式征集解决方案，要求参赛者设计能在移动设备上执行的超分辨率网络，使用未知退化生成的x4缩放低分辨率图像，评估采用图像质量评估分数和加速比的加权组合。

Result: 吸引了108名参赛者注册，16支团队在最终排名中获得有效分数，展示了移动端真实世界图像超分辨率的最新进展和趋势。

Conclusion: 该挑战赛有效推动了移动端真实世界图像超分辨率技术的发展，为领域提供了最新趋势的深入概述，促进了高效网络设计的创新。
...
TL;DR: 提出SePL方法，通过可分离提示学习策略，利用CLIP的文本模态指导深度伪造检测，实现跨数据集和跨方法评估的优越性能


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的深度伪造检测方法主要依赖视觉编码器，对文本模态关注有限。作者认为文本模态具有指导性，可以通过精心设计来指导深度伪造检测

Method: 提出可分离提示学习策略(SePL)，通过两种类型的提示学习来解耦图像中的伪造相关和伪造无关信息。采用跨模态对齐策略和专门设计的损失函数来实现这种解耦

Result: 在跨数据集和跨方法评估中，该方法取得了竞争性甚至优于其他方法的性能，显示出强大的泛化能力

Conclusion: 通过简单的适配，CLIP可以作为有效的面部伪造检测器，文本模态在深度伪造检测中具有重要指导作用
...
TL;DR: 提出MedFocusLeak攻击方法，针对临床视觉语言模型进行隐蔽且临床可信的对抗攻击，诱导错误诊断同时保持扰动不可察觉。


<details>
  <summary>Details</summary>
Motivation: 当前医疗视觉语言模型在临床诊断中的应用日益广泛，但其对抗攻击鲁棒性研究不足，现有攻击方法要么针对次要目标（如模型窃取），要么从自然图像迁移的攻击会产生明显失真容易被临床医生察觉，存在严重安全风险。

Method: 提出MedFocusLeak方法：1）在非诊断背景区域注入协调扰动；2）采用注意力分散机制将模型注意力从病理区域转移；3）实现高度可迁移的黑盒多模态攻击；4）建立统一的评估框架和新颖指标来同时衡量攻击成功率和图像保真度。

Result: 在六种医学影像模态上的广泛评估显示，MedFocusLeak达到最先进性能，能够生成误导性但真实的诊断输出，并揭示了现代临床视觉语言模型推理能力的关键弱点。

Conclusion: MedFocusLeak展示了临床视觉语言模型对隐蔽对抗攻击的脆弱性，提出了统一的评估框架和新指标，为医疗AI系统的安全评估提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有GMNER方法主要采用流水线架构，将文本实体识别和视觉定位解耦，导致错误累积和次优的联合优化。需要统一的端到端框架来同时处理这些任务。
...
Conclusion: E2E-GMNER通过统一的端到端生成框架成功解决了GMNER任务，结合链式思维推理和GRBP技术，在减少对噪声线索依赖的同时提高了视觉定位的稳定性，为多模态实体识别提供了新的解决方案。
...
TL;DR: 提出QUOTA框架，将低比特量化与视觉令牌剪枝统一，在W4A4量化下实现30%令牌保留率下95.65%性能保持


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在低比特推理时面临视觉令牌前缀长和KV缓存增长的问题，现有量化与剪枝的级联方法存在校准与执行不匹配的脆弱性

Method: 提出QUOTA框架，将低比特校准信号转换为层间令牌分配计划，结合激活幅度、注意力线索和低比特风险信号，在量化算子下进行预算约束的top-k选择

Result: 在标准VLM基准测试中，相比约94.3%的级联方法，在仅保留30%视觉令牌下实现95.65%的平均性能保持，鲁棒性显著提升

Conclusion: QUOTA框架通过统一量化与剪枝，解决了低比特推理中视觉令牌处理的关键挑战，为高效VLM部署提供了有效解决方案
...
TL;DR: 提出一种新的单图像人脸变形攻击检测框架，结合拉普拉斯高频残差统计与冻结基础视觉变换器，通过R-FLoRA和Res-FiLM增强对局部变形伪影的敏感性，在多个数据集上超越现有S-MAD方法。


<details>
  <summary>Details</summary>
Motivation: 人脸变形攻击对护照签发、边境控制和数字身份验证中的人脸识别系统构成重大威胁。由于缺乏可信参考和攻击生成方法的多样性，从单张人脸图像检测变形攻击仍然具有挑战性。

Method: 提出S-MAD框架，整合高频拉普拉斯残差统计与冻结基础视觉变换器表示。使用残差统计门控低秩适配器(R-FLoRA)和特征级残差融合(Res-FiLM)增强对局部变形伪影的敏感性。引入残差对比对齐损失来正则化融合标记空间。

Result: 在四个ICAO兼容数据集上，涵盖七种变形生成技术，该方法在检测准确率和跨域泛化能力上持续超越九种最新S-MAD算法。冻结主干网络且参数极少，实现实时效率和可解释性。

Conclusion: 该方法在生物特征验证系统中具有实际应用价值，能够有效检测单图像人脸变形攻击，具备良好的泛化能力和实时性能。
...
TL;DR: 提出一个用于糖尿病视网膜病变分级的双分辨率深度学习框架，结合注意力特征融合和有序回归，提升跨数据集泛化能力。


<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是全球视力障碍的主要原因，深度学习模型在不同成像条件下部署时性能下降，需要提高跨数据集泛化能力。

Method: 使用两个并行EfficientNet骨干网络在不同空间分辨率下提取特征，通过可学习注意力机制融合多分辨率表示，采用基于累积链接模型的有序回归，并应用预处理策略减少域差异。

Result: 在APTOS 2019验证集上获得0.88的二次加权kappa，在未见过的Messidor-2数据集上获得0.68，显示跨数据集DR分级应用的鲁棒性提升。

Conclusion: 提出的双分辨率框架结合注意力融合和有序回归，有效提高了糖尿病视网膜病变分级在跨数据集场景下的鲁棒性和泛化能力。
...
TL;DR: 论文提出VisualTextTrap基准测试，用于评估视觉语言模型在屏幕文本与视觉内容矛盾时的幻觉问题，并开发VTHM-MoE框架来缓解这种文本覆盖诱导的幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在屏幕文本与视觉场景矛盾时，会系统性地产生幻觉，优先考虑覆盖文本语义而忽略实际视觉内容，这种现象被定义为文本覆盖诱导幻觉(TOIH)。

Method: 1) 构建VisualTextTrap基准测试，包含6,057个人工验证样本，覆盖88个细粒度属性，采用五级幻觉强度量化；2) 提出VTHM-MoE框架，采用双编码器架构，包含四个维度专家模块，并开发自适应令牌路由策略。

Result: 在VisualTextTrap基准测试上的广泛实验验证了VTHM-MoE的有效性，在多样化的视频问答任务中优于最先进的对比方法。

Conclusion: 该研究首次系统地识别和量化了文本覆盖诱导幻觉问题，提出的基准测试和缓解框架为解决视觉语言模型中的这一关键问题提供了有效工具。
...
TL;DR: 使用DINOv2、AIMv2和OpenCLIP的ViT-L/14等微调视觉变换器集成方法，在DF-Wild数据集上实现96.77%的AUC和9%的EER，超越现有最佳方法，赢得IEEE SP Cup 2025比赛。


<details>
  <summary>Details</summary>
Motivation: 现代生成模型快速发展导致深度伪造图像检测面临挑战，现有方法泛化能力不足，需要开发更通用的检测方法。

Method: 采用集成学习方法，结合微调的视觉变换器（DINOv2、AIMv2、OpenCLIP的ViT-L/14），在DF-Wild数据集上进行训练，相比传统的CNN分类器有更好的性能。

Result: 集成模型在DF-Wild测试集上达到96.77%的AUC和9%的EER，比当前最佳算法Effort在AUC上提升7.05%，在EER上提升8%，赢得IEEE SP Cup 2025比赛并在ICASSP 2025展示。

Conclusion: 基于视觉变换器的集成方法在深度伪造检测中表现出优异的泛化能力和性能，为应对快速发展的生成模型提供了有效的解决方案。
...
TL;DR: SpatialImaginer是一个多模态生成框架，通过结合文本推理和视觉想象来解决MLLMs在空间智能任务中的脆弱推理问题，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在空间智能任务中存在脆弱推理痕迹，主要原因是空间识别机制与纯文本推理行为不匹配。文本表示往往会抽象掉关键的几何细节，而有效的空间推理需要在推理过程中忠实保持和更新低层几何结构。

Method: 提出SpatialImaginer统一多模态生成框架，采用分治策略：使用文本思维链进行高层语义规划，使用视觉想象进行几何敏感的状态转换和一致性保持。引入难度感知数据引擎和闭环验证，训练模型在需要稳定空间状态跟踪时选择性地调用视觉想象。

Result: 在多样化的空间智能基准测试中，SpatialImaginer实现了最先进的性能，并在复杂的多步空间推理任务中显著提高了鲁棒性。

Conclusion: 通过整合文本推理和视觉想象，SpatialImaginer有效解决了MLLMs在空间智能任务中的核心挑战，为几何结构保持和空间状态跟踪提供了创新解决方案。
...
TL;DR: 提出基于深度学习的非刚性体积-表面配准框架，利用稀疏术中表面观测估计密集位移场，无需术中体积成像或显式点对应关系，实现脑移位补偿。


<details>
  <summary>Details</summary>
Motivation: 神经外科手术中软组织变形（脑移位）严重影响导航精度，现有补偿方法依赖术中MRI/CT/超声等破坏性成像，难以集成到手术流程中。而术中仅能获取部分皮质表面点云，如何从这些稀疏、噪声观测中估计密集变形场是主要挑战。

Method: 提出深度学习框架进行非刚性体积-表面配准，利用多尺度点云特征提取和分层变形解码器捕获全局和局部变形。关键创新是将部分术中表面信息整合到完整术前点云域中，实现隐式对应关系学习和有限可见度下的密集变形恢复。

Result: 在挑战性的部分表面条件下，实现了精细尺度变形的准确恢复：端点误差(EPE)为1.13±0.75mm，均方根误差(RMSE)为1.33±0.81mm。

Conclusion: 该方法支持从稀疏表面观测自动、工作流兼容的脑移位补偿，为图像引导神经外科提供了一种无需术中体积成像的实用解决方案。
...
TL;DR: MESA是一种用于修复古代碑铭图像的方法，利用保存完好的示例碑铭来指导受损文本的重建，通过VGG19特征和Gram矩阵捕捉纹理风格，结合OCR字符宽度优化层权重，并保护完好区域。


<details>
  <summary>Details</summary>
Motivation: 古代碑铭常因破碎、侵蚀等损坏导致文字缺失，影响阅读和分析。现有图像修复方法在碑铭恢复方面存在局限性，需要专门针对碑铭特点的修复方法。

Method: MESA使用保存完好的示例碑铭作为指导，通过VGG19卷积特征编码为Gram矩阵来捕捉纹理、风格和笔画结构。每层选择与受损输入MSD最小的示例，利用OCR估计的字符宽度确定层权重，使用训练掩码保护完好区域。

Result: 对比实验表明MESA具有优势，能够有效修复受损碑铭图像，同时提供了根据可用示例和元数据选择修复策略的实用路线图。

Conclusion: MESA为古代碑铭图像修复提供了一种有效的多示例风格感知方法，解决了现有方法的局限性，为文化遗产保护提供了实用工具。
...
TL;DR: SDVG将推测解码应用于自回归视频扩散，通过图像质量路由器替代token验证，在保持高质量的同时实现1.59-2.09倍加速


<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散模型推理速度慢，而推测解码在语言模型中已成功加速。但视频块是连续时空张量，没有token级分布进行精确拒绝采样，需要新的验证方法

Method: 使用1.3B草稿模型通过4步去噪生成候选块，VAE解码后用ImageReward评分（采用最差帧聚合），得分超过阈值τ则接受到14B目标模型的KV缓存，否则由目标模型重新生成

Result: 在832x480分辨率下，SDVG在τ=-0.7时达到1.59倍加速，保持98.1%的目标模型质量；在2.09倍加速时保持95.7%质量，始终比仅用草稿模型生成质量高17%以上

Conclusion: SDVG成功将推测解码应用于自回归视频生成，无需训练或架构修改，可无缝集成到现有流程中，通过单一阈值τ实现质量-速度的平滑权衡
...
TL;DR: Q-Gate：一个即插即用的免训练框架，通过动态模态路由解决长视频理解中的关键帧选择问题，利用LLM评估查询意图并智能分配专家注意力权重。


<details>
  <summary>Details</summary>
Motivation: 现有长视频理解方法面临计算成本高的问题，关键帧选择方法通常依赖单一视觉指标或静态融合启发式分数，这种"一刀切"范式经常失败：视觉指标对情节驱动叙事查询无效，而盲目加入文本分数会为纯视觉任务引入严重"模态噪声"。

Method: 提出Q-Gate框架，将关键帧选择视为动态模态路由问题。解耦检索过程为三个轻量级专家流：局部细节的视觉定位、场景语义的全局匹配、字幕驱动叙事的上下文对齐。核心是查询调制门控机制，利用LLM的上下文推理评估查询意图，动态分配专家注意力权重。

Result: 在LongVideoBench和Video-MME基准测试中，Q-Gate在多个MLLM骨干网络上显著优于现有最先进基线方法。通过有效抑制模态特定噪声，为可扩展视频推理提供了鲁棒且高度可解释的解决方案。

Conclusion: Q-Gate通过动态模态路由和查询调制门控机制，智能激活必要模态同时"静音"无关模态，最大化信噪比，为长视频理解提供了有效的关键帧选择框架。
...
TL;DR: 该论文提出了一种专门用于长视频评估的新框架，认为短时视觉感知和长上下文属性是正交维度，需要将长视频评估与短视频评估解耦，并设计了基于镜头动态的新指标和专门的长视频评估数据集。


<details>
  <summary>Details</summary>
Motivation: 随着视频生成模型能力的提升，对长视频生成的需求增加，但传统评估指标主要针对短视频设计，侧重于帧级视觉质量和局部时间平滑性，无法捕捉长视频的关键特性如叙事丰富性和全局因果一致性。需要专门的长视频评估框架。

Method: 1) 引入长视频属性损坏测试套件，暴露现有短视频指标的局限性；2) 设计基于镜头动态的新型长视频评估指标，对长范围测试框架高度敏感；3) 创建Long-CODE数据集，专门用于长视频评估基准测试，包含针对真实长范围特性的人工标注。

Result: 实验表明，提出的新指标在与人判断的相关性方面达到了最先进的水平。该指标和基准测试与现有的短视频标准无缝互补，建立了全面且无偏的视频生成模型评估范式。

Conclusion: 长视频评估需要与短视频评估解耦，因为它们是正交维度。提出的基于镜头动态的指标和Long-CODE数据集有效解决了长视频评估的挑战，为视频生成模型提供了更全面的评估框架。
...
TL;DR: 提出基于阴影形状恢复(SfS)的框架，用于增强月表亚米级分辨率数字高程模型(DEM)，利用Chandrayaan-2轨道器高分辨率相机(OHRC)影像，在三个月球区域验证了方法有效性。


<details>
  <summary>Details</summary>
Motivation: 传统立体摄影测量受基线限制，难以获取高分辨率地形数据。需要开发新方法来增强现有DEM，特别是对于月球表面亚米级分辨率的地形重建。

Method: 使用SfS框架处理独立的OHRC影像，通过三阶段参数扫描优化平滑权重，应用于Cyrillus陨石坑、Vikram着陆区和月球南极Mons Mouton三个区域。

Result: 方法显著增强了地形细节，特别是在表面坡度统计方面，揭示了先前未解析的精细陨石坑形态。但也发现当阴影影像与立体像对俯仰角差异较大时，SfS敏感性会降低。

Conclusion: SfS不仅是DEM的细化工具，还能作为不受立体基线限制的新地形数据源，为高分辨率月球地形测绘提供了有效方法，但需要注意影像几何条件和覆盖范围的影响。
...
TL;DR: 本文综述了计算机视觉中非Transformer方法的最新进展，对40篇相关论文进行了分类和系统分析，探讨了这些方法在效率、可扩展性、可解释性和鲁棒性方面的表现，旨在为非Transformer方法提供全面视角并指出未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer模型在计算机视觉领域取得了显著进展，但许多非Transformer方法仍然表现出色，与Transformer模型形成直接竞争。本文旨在系统梳理这些非Transformer方法，为研究人员提供一个全面的分类框架，并探讨这些方法的优缺点。

Method: 本文采用文献综述方法，选择了40篇相关论文，将这些非Transformer方法按照架构类型进行分类，包括卷积神经网络、多层感知机、状态空间模型等类别。从效率、可扩展性、可解释性和鲁棒性四个维度对这些方法进行了系统分析。

Result: 研究建立了非Transformer方法的全面分类体系，发现这些方法在特定任务和场景下仍然具有竞争力。分析表明，不同架构类型的方法在效率、可扩展性、可解释性和鲁棒性方面各有优势，为计算机视觉研究提供了多样化的技术选择。

Conclusion: 非Transformer方法在计算机视觉领域仍然具有重要价值和竞争力。本文的综述为研究人员提供了系统性的参考框架，指出了未来研究的挑战和机遇，包括如何更好地结合不同架构的优势，以及如何应对特定应用场景的需求。
...
TL;DR: HyKey模型利用高光谱成像技术，在纹理贫乏的手术场景中实现了比传统RGB方法更优的关键点检测与匹配性能


<details>
  <summary>Details</summary>
Motivation: 传统RGB关键点检测在微创手术中面临纹理贫乏和复杂光照的挑战，需要探索高光谱成像能否提供更好的解决方案

Method: 开发HyKey模型：混合3D-2D卷积神经网络，从高光谱图像中联合提取空间-光谱特征；使用合成同形增强和极几何约束训练；基于机器人采集的双摄像头RGB-HSI腹腔镜数据集

Result: HSI模型在注册RGB帧上优于RGB基线方法：96.62%平均匹配准确率，67.18%平均姿态估计准确率（10度阈值），在多个评估指标上表现一致提升

Conclusion: 整合高光谱立方体的光谱信息为微创手术中的鲁棒单目3D重建提供了有前景的方法，通过增强光谱-空间特征区分能力解决了纹理贫乏手术环境的限制
...
TL;DR: SegTTA：无需重新训练的医学图像分割框架，通过四种数据增强与加权投票策略，提升MedSAM2在不同数据集上的分割性能


<details>
  <summary>Details</summary>
Motivation: 医学图像质量因设备和操作者差异而变化，现有基础模型虽具泛化能力，但在实际临床应用中仍需适应这种变化。需要一种无需重新训练的方法来提升分割性能。

Method: 结合Gamma校正、对比度增强、高斯模糊和高斯噪声四种数据增强，在多个MedSAM2检查点上进行加权投票，通过投票阈值控制覆盖-精度权衡。

Result: 在三个数据集上均取得一致改进：健康子宫分割、子宫肌瘤检测和多类肝结构分割。在肝血管数据集上，相比MedSAM2基线，mIoU提升1.6，aIoU提升1.9，HD95降低约2.0。

Conclusion: SegTTA提供了一种有效的无需重新训练的分割增强方法，大型器官受益于强度增强，小病灶需要噪声增强，投票阈值可根据临床需求进行任务特定优化。
...
TL;DR: 提出HSG方法，在双曲空间中学习场景图嵌入，利用双曲几何的自然层次编码能力，提升场景图的结构质量和检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在欧几里得空间中学习场景图嵌入，无法显式捕捉地点与对象之间的层次蕴含关系，限制了学习表示的结构一致性。

Method: 提出双曲场景图（HSG），在双曲空间中学习场景图嵌入，利用双曲几何距离自然编码层次关系。

Result: HSG在保持强检索性能的同时，显著提升了层次结构质量，在图级指标上表现突出：PP IoU达到33.17，Graph IoU达到33.51，比最佳AoMSG变体（25.37）高出8.14。

Conclusion: 双曲表示学习对场景图建模非常有效，能够更好地捕捉层次关系，提升场景图的结构一致性。
...
TL;DR: APEX提出自适应提示提取框架，通过可学习的提示记忆库为每个输入检索特定提示，结合低频特征对比学习提升医学图像分割的跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉提示方法通常为每个域优化单一提示并统一应用于所有输入，限制了在医学领域中处理域内和域间变异性的泛化能力。

Method: 提出APEX框架：1）构建可学习的提示记忆库存储多样化、域判别性提示表示；2）通过傅里叶频谱提取域特征来查询记忆库；3）引入低频特征对比学习框架，聚类同域表示并分离不同域表示。

Result: 在两个医学分割任务上的实验表明，APEX显著提升了在已见和未见域上的泛化性能，能够作为即插即用方案与现有骨干网络结合并持续提升性能。

Conclusion: APEX通过自适应提示提取和低频特征对比学习，有效解决了医学视觉任务中的域泛化问题，为医学领域提供了强大的即插即用提示解决方案。
...
TL;DR: UniMesh是一个统一框架，通过创新的Mesh Head、Chain of Mesh推理机制和自反思机制，在单一架构中联合学习3D生成与理解，实现知识转移和整体场景建模。


<details>
  <summary>Details</summary>
Motivation: 当前3D视觉领域存在专门化模型碎片化的问题：理解任务（如分类、分割、重建）和生成任务（如合成、补全、编辑）通常被孤立处理，导致架构和表示碎片化，阻碍知识转移和整体场景建模。

Method: 1. 提出Mesh Head作为跨模型接口，桥接基于扩散的图像生成与隐式形状解码器；2. 开发Chain of Mesh（CoM）几何推理机制，通过潜在空间、提示和重新生成的闭环循环实现用户驱动的语义网格编辑；3. 引入基于Actor-Evaluator-Self-reflection三元组的自反思机制，用于诊断和纠正3D字幕等高层次任务中的失败。

Result: 实验结果表明，UniMesh不仅在标准基准测试中取得有竞争力的性能，还解锁了迭代编辑的新能力，并实现了生成与理解之间的相互增强。

Conclusion: UniMesh通过统一框架解决了3D视觉中生成与理解任务碎片化的问题，实现了知识转移和整体场景建模，为3D视觉系统提供了更全面的解决方案。
...
TL;DR: 论文提出双锚定框架解决视觉语言导航中的状态漂移问题，通过指令进度锚定和记忆地标锚定分别应对进度漂移和记忆漂移，在长轨迹任务上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前视频大语言模型在视觉语言导航任务中容易在长场景下出现状态漂移问题，导致智能体内部状态偏离真实任务执行状态，造成无目标漫游和关键操作执行失败。这主要源于两种认知缺陷：进度漂移（无法区分已完成和待完成子目标）和记忆漂移（历史表示退化导致丢失已访问地标信息）。

Method: 提出双锚定框架：1) 指令进度锚定 - 监督智能体生成结构化文本标记来区分已完成和待完成子目标；2) 记忆地标锚定 - 使用地标中心世界模型通过Segment Anything模型提取物体中心嵌入进行回顾性预测，强制智能体显式验证过去观察并保留已访问地标的独特表示。为此构建了两个大规模数据集：360万样本的显式进度描述数据集和93.7万样本的接地地标数据。

Result: 在仿真和真实环境中的大量实验表明，该方法在成功率和长轨迹任务上分别取得15.2%和24.7%的显著提升，证明了双锚定框架在缓解状态漂移问题上的有效性。

Conclusion: 双锚定框架通过显式锚定指令进度和历史表示，有效解决了视觉语言导航中的状态漂移问题，特别是在长场景下表现出色。该方法为处理复杂导航任务中的认知缺陷提供了新思路，并发布了代码、数据生成管道和收集的数据集以促进进一步研究。
...
TL;DR: 提出一种基于三分支网络和互信息理论的深度伪造检测方法，通过联合学习空间和频率特征，并利用特征解耦与融合损失提升模型对多样化伪造模式的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测方法存在两个主要问题：1）通常只关注一个或少数特定频率域，容易过拟合特定伪造痕迹，面对多样化伪造模式时鲁棒性差；2）不同特征往往关注相同的伪造区域，导致特征表示冗余，限制了提取线索的多样性，影响模型对多样化篡改的泛化能力。

Method: 提出三分支网络，联合捕获空间和频率特征：1）从原始图像学习；2）从不同频率通道重建的图像学习。基于互信息理论推导特征解耦和融合损失，增强模型对原始图像和不同频率重建图像中任务相关特征的关注。

Result: 在六个大规模基准数据集上的广泛实验表明，该方法始终达到最先进的性能。

Conclusion: 该方法通过联合学习空间和频率特征，并利用互信息理论的特征解耦与融合损失，有效解决了现有深度伪造检测方法的局限性，提升了模型对多样化伪造模式的鲁棒性和泛化能力。
...
TL;DR: AutoVQA-G：一个自改进的智能框架，通过迭代优化和思维链推理自动生成高质量的视觉问答-定位数据集，解决现有方法的数据保真度不一致和验证机制脆弱问题。


<details>
  <summary>Details</summary>
Motivation: 手动标注高质量的视觉问答-定位数据集对推进视觉语言模型发展至关重要，但难以扩展。现有自动化方法存在两个关键问题：1）模型幻觉导致数据保真度不一致；2）基于简单启发式的脆弱验证机制。

Method: AutoVQA-G采用自改进的智能框架，包含迭代优化循环：一致性评估模块使用思维链推理进行细粒度视觉验证；基于反馈，记忆增强的提示优化代理分析失败样本的批评，逐步优化生成提示。

Result: 实验表明，AutoVQA-G生成的VQA-G数据集在视觉定位准确性方面优于领先的多模态LLM，为创建高保真数据提供了有前景的方法，有助于更稳健的VLM训练和评估。

Conclusion: AutoVQA-G通过自改进的智能框架解决了VQA-G数据自动标注的挑战，提供了一种可扩展的高质量数据集生成方法，有望促进更强大的视觉语言模型发展。
...
TL;DR: CoReDi提出了一种联合进化表示扩散框架，通过在学习过程中自适应调整语义表示空间，提升扩散模型的训练效率和生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有联合图像-特征生成建模方法依赖固定的表示空间，这些空间独立于生成目标构建且在训练中保持不变。作者认为指导扩散的表示空间应该根据生成任务自适应调整。

Method: 提出Coevolving Representation Diffusion (CoReDi)框架，在训练过程中通过联合学习轻量级线性投影使语义表示空间进化。采用停止梯度目标、归一化和针对性正则化防止特征崩溃，实现稳定协同进化。

Result: CoReDi在VAE潜在扩散和像素空间扩散中都表现出更快的收敛速度和更高的样本质量，优于在固定表示空间中操作的联合扩散模型。

Conclusion: 自适应语义表示能够专门化适应图像合成需求，改善与图像潜在表示的互补性，为联合图像-特征生成建模提供了更有效的框架。
...
TL;DR: 本文提出Edit Fidelity Field (EFF)解决场景文本编辑中的编辑溢出问题，将溢出率从94%降至25%，同时显著保护非目标区域。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的场景文本编辑方法存在严重的编辑溢出问题——编辑目标文本区域时会无意中修改非目标区域（特别是邻近文本）。通过对50个真实场景的系统评估发现，现有最先进方法的溢出率高达94%。

Method: 提出Edit Fidelity Field (EFF)，这是一个语义感知的连续场，控制每个像素的编辑保真度。不同于二值掩码，EFF利用OCR检测的文本区域构建四区域场：编辑核心区（完全可编辑）、过渡区（平滑衰减）、保护区（非目标文本，明确锁定）和背景区（严格保护）。EFF作为无需训练、模型无关的后处理模块，适用于任何基于扩散的STE方法。

Result: EFF将编辑溢出率从94%显著降低到25%，同时将非目标区域的保护效果提升了+91.4 dB PSNR。还提出了按区域溢出量化的新评估协议。

Conclusion: EFF有效解决了场景文本编辑中的编辑溢出问题，通过语义感知的连续场控制实现了对非目标区域的精确保护，为扩散基STE方法提供了通用的后处理解决方案。
...
TL;DR: 提出RS-HyRe-R1混合奖励框架解决RL训练导致的遥感视觉语言模型"感知惯性"问题，通过空间推理激活、感知正确性和视觉语义路径演化三种奖励机制，在仅3B参数下实现SOTA性能。
...
Method: 提出RS-HyRe-R1混合奖励框架，包含三种奖励机制：1) 空间推理激活奖励：强制结构化视觉推理；2) 感知正确性奖励：提供跨遥感任务的自适应质量锚点，确保准确的几何和语义对齐；3) 视觉语义路径演化奖励：惩罚重复推理，促进探索互补线索以构建更丰富的证据链。

Result: RS-HyRe-R1有效缓解"感知惯性"，鼓励更深层、更多样化的推理。仅用3B参数就在REC、OVD和VQA任务上达到SOTA性能，超越最高7B参数的模型。在零样本泛化方面，在VQA、OVD和REC任务上分别超越第二佳模型3.16%、3.97%和2.72%。

Conclusion: RS-HyRe-R1框架成功解决了RL训练导致的感知惯性问题，通过混合奖励机制促进了遥感图像理解的全面视觉证据挖掘，在参数效率、性能和泛化能力方面均表现出色。
...
TL;DR: DualTTA提出了一种新的测试时适应框架，通过将测试样本分为可靠和不可靠两组进行差异化处理，利用更全面的测试分布信息来提升分布偏移下的性能。


<details>
  <summary>Details</summary>
Motivation: 传统的TTA方法通常只使用一小部分低熵预测的测试样本进行适应，未能充分利用测试分布中的可用信息，导致性能受限。

Method: DualTTA通过新的可靠性标准将测试样本分为两组：预测可靠组（最小化熵以强化可靠决策）和预测不可靠组（最大化熵以抑制过度自信错误）。可靠性标准基于语义保持和语义改变变换下的预测稳定性来衡量。

Result: 理论分析和实证结果表明，该方法能够在适应适用性方面更严格地区分可靠和不可靠样本，从而提供更有效的模型更新。

Conclusion: DualTTA通过更全面地利用测试样本并采用差异化适应策略，在分布偏移下实现了更好的性能提升。
...
TL;DR: UniGeo提出了一种统一几何引导的三级框架，用于相机可控的图像编辑，通过在表示、架构和损失函数三个层面注入几何指导，显著提升了跨视角几何一致性和视觉质量。


<details>
  <summary>Details</summary>
Motivation: 现有相机可控图像编辑方法存在两个主要问题：1) 几何指导是碎片化的，通常只在表示层面注入点云等几何信息，而模型包含多个层面；2) 主要基于图像扩散模型，在离散视角映射上操作。这两个限制共同导致在连续相机运动下出现几何漂移和结构退化。

Method: UniGeo框架在三个层面统一注入几何指导：1) 表示层面：采用帧解耦的几何参考注入机制，提供鲁棒的跨视角几何上下文；2) 架构层面：引入几何锚点注意力来对齐多视角特征；3) 损失函数层面：提出轨迹端点几何监督策略，显式增强目标视图的结构保真度。

Result: 在多个公共基准测试上的综合实验表明，UniGeo在广泛和有限相机运动设置下，在视觉质量和几何一致性方面都显著优于现有方法。

Conclusion: 通过系统地在表示、架构和损失函数三个层面统一注入几何指导，UniGeo有效解决了相机可控图像编辑中的几何漂移和结构退化问题，为连续相机运动下的几何一致性编辑提供了强有力的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统多相机系统在体育应用中需要昂贵且费力的外参标定，特别是在涉及球杆类器械（如高尔夫球杆、球棒）的运动中，需要更高效的标定方法。

Method: 采用三阶段优化流程：1）利用未知尺度的人体关键点初步估计相机外参；2）结合已知长度的刚性球杆类器械进行轨迹重建；3）通过球杆长度约束解决全局尺度问题。

Result: 在包含3-10个相机的合成数据集上实现了SOTA性能，获得了较低的旋转和平移误差，无需专用标定工具即可完成准确的外参标定。

Conclusion: 该方法为涉及球杆类器械的体育运动提供了一种高效、无需工具的多相机外参标定解决方案，并建立了该任务的第一个基准数据集。
...
TL;DR: 开发了首个针对外周血涂片（PBS）的视觉语言模型PBS-VL，包含数据集PBSInstr和评估基准PBSBench，在血液病理学AI辅助诊断方面取得突破。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型主要针对实体组织病理学开发，无法有效处理外周血涂片（PBS）这种以单个细胞形态分析为主的特殊病理学领域，需要专门的数据集和模型来填补这一空白。

Method: 1) 构建PBSInstr数据集：包含353个PBS全玻片图像配微观印象段落，29k细胞级图像裁剪标注细胞类型和形态描述，27k细胞级QA对和1,286个玻片级QA对；2) 开发PBS-VL模型：针对血液病理学定制的视觉语言模型，支持细胞和玻片多级解释；3) 建立PBSBench评估基准：包含四个问题类别和六个PBS解释任务的视觉问答基准。

Result: PBS-VL在PBS理解任务上超越了现有的通用和病理学专用MLLMs，证明了PBS特定数据的价值。模型、代码和数据集已开源。

Conclusion: 该研究为开发实用的血液病理学AI辅助决策系统奠定了基础，填补了外周血涂片AI分析的空白，通过专门的数据集和模型架构实现了该领域的突破。
...
TL;DR: DGSSM是一个扩散引导的状态空间（Mamba）框架，通过渐进去噪过程解决多模态显著目标检测问题，结合扩散结构先验与多尺度状态空间编码，在保持紧凑模型大小的同时，在多个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 显著目标检测需要同时建模长距离上下文依赖和细粒度结构细节，这对卷积、Transformer和Mamba模型都具有挑战性。Mamba模型能高效进行全局推理但难以恢复精确边界，而扩散模型通过迭代去噪捕获强结构先验，但在判别性密集预测中应用有限。

Method: 提出DGSSM框架，将多模态显著目标检测建模为渐进去噪过程。集成扩散结构先验与多尺度状态空间编码，采用自适应显著提示、迭代Mamba扩散细化机制、边界感知细化头和自蒸馏策略，增强边界精度和特征一致性。

Result: 在RGB、RGB-D和RGB-T设置的13个公开基准测试中，DGSSM在多个评估指标上持续优于最先进方法，同时保持紧凑的模型大小。

Conclusion: 扩散引导的状态空间建模是多模态密集预测任务的有效且可泛化的范式。
...
TL;DR: ViPS：基于视频扩散模型的运动先验，为自动绑定网格学习有效的姿态空间，实现多样采样、逆向运动学和关键帧动画


<details>
  <summary>Details</summary>
Motivation: 传统运动学绑定缺乏对关节配置合理流形的表示，导致随机采样或手动调整时出现解剖学超伸、非物理自相交等语义或几何违规问题

Method: 提出Video-informed Pose Spaces (ViPS)，通过从预训练视频扩散模型中蒸馏运动先验，学习自动绑定网格的有效姿态分布。使用可微分几何验证器确保资产特定有效性，无需手动正则化

Result: ViPS仅使用视频先验训练，在合理性和多样性上匹配使用合成艺术家创建4D数据训练的最先进方法。作为通用模型，在分布外物种和未见骨骼拓扑上表现出强大的零样本泛化能力

Conclusion: ViPS成功将生成式2D先验转化为结构化3D运动控制，建立了生成2D先验与结构化3D运动控制之间的闭环，实现了平滑、紧凑、可控的姿态空间
...
TL;DR: FlowC2S：一种基于预训练文本到视频流模型的新方法，通过直接学习当前与后续视频块之间的向量场来生成快速且内存高效的视频延续，相比传统方法将输入维度减半。


<details>
  <summary>Details</summary>
Motivation: 现有视频延续方法通常将当前帧与噪声结合生成后续视频，这种方法输入维度高、计算效率低。需要开发更高效的方法来生成高质量的视频延续。

Method: 1. 微调预训练的文本到视频流模型，学习当前与后续视频块之间的向量场；2. 引入固有最优耦合，使用时间相邻的视频块作为真实最优耦合的实用代理，获得更直的流；3. 采用目标反转，将目标块的倒置潜在表示注入输入表征，增强对应关系并提高视觉保真度。

Result: 在LTXV和Wan模型上微调后，FlowC2S在FID和FVD定量评估中超越了最先进方法，仅需5次神经函数评估，实现了更高的效率和更好的性能。

Conclusion: FlowC2S通过直接学习视频块间的向量场，显著降低了输入维度，同时通过固有最优耦合和目标反转技术提高了视频延续的质量和效率，为快速、内存高效的视频生成提供了新思路。
...
TL;DR: BioVLM：通过动态提示选择和知识蒸馏提升生物医学视觉语言模型在跨域泛化中的表现，特别是在少样本和模态差异大的场景下。


<details>
  <summary>Details</summary>
Motivation: 现有的生物医学视觉语言模型（如BioMedCLIP）在平均表现良好，但在具有挑战性的模态上表现下降，特别是当类间差异小、采集特异性变化显著时，以及在少样本监督和模态先验与预训练语料差异大的情况下。

Method: 提出BioVLM框架：1）学习多样化的提示库；2）引入动态提示选择机制，基于预测分布的低熵准则为每个输入选择最具区分性的提示；3）通过蒸馏高置信度的LLM派生属性来加强耦合；4）通过强/弱增强一致性实现稳健的知识迁移。

Result: 在11个MedMNIST+ 2D数据集上，BioVLM在三个不同的泛化设置中均达到了新的最先进水平。

Conclusion: BioVLM通过轻量级训练和高效推理，实现了对未见类别和领域的有效迁移，显著提升了生物医学视觉语言模型在跨域泛化中的性能。
...
TL;DR: 论文提出基础设施中心世界模型(I-WM)概念，利用路边固定传感器的时空优势，通过多阶段架构实现交通场景的生成式理解和预测，为自动驾驶提供互补视角。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型都采用车辆自我中心视角，忽视了基础设施视角的独特价值。路边固定传感器具有鸟瞰视角、多传感器融合和持续观测的优势，在时间深度上能积累长期行为分布（包括罕见安全关键事件），与车辆传感器的空间广度形成互补。

Method: 提出三阶段I-WM架构：1) 质量感知不确定性传播的生成式场景理解；2) 物理信息预测动力学与多智能体反事实推理；3) 通过潜在空间对齐实现V2X通信的协作世界模型。采用双层架构，无标注感知作为多模态数据引擎，分阶段传感器策略从LiDAR到4D雷达、信号相位数据和事件相机。

Result: 建立了驾驶世界模型范式的分类体系，将I-WM定位在LeCun的JEPA、李飞飞的空间智能和VLA架构之间，并提出了基础设施VLA(I-VLA)作为路边感知、语言指令和交通控制行动的统一框架。基于现有多LiDAR流水线，为每个阶段确定了开源基础。

Conclusion: 基础设施中心世界模型为自动驾驶提供了根本性的互补能力，利用路边系统的独特视角实现交通的理解和预测。这一愿景为构建能够理解和预测交通的基础设施提供了技术路径。
...
TL;DR: 提出自监督超分辨率框架用于Sentinel-5P卫星数据，无需高分辨率真值，结合SURE损失和等变成像约束，在合成数据和真实观测中均表现良好。


<details>
  <summary>Details</summary>
Motivation: Sentinel-5P卫星在空间分辨率上有限，现有超分辨率方法依赖合成低分辨率数据和监督学习，缺乏真实高分辨率真值，限制了在真实观测中的应用。

Method: 结合Stein无偏风险估计器(SURE)和等变成像约束，整合S5P退化算子和信噪比元数据推导的噪声统计，采用深度可分离卷积U-Net架构保证效率和光谱保真度。

Result: 在两种设置下评估：(1)LR-HR：与监督学习方法相当；(2)GT-SHR：超分辨率图像超越原生空间分辨率。多波段结果显示自监督模型性能接近监督方法，保持强一致性，定性分析显示比双三次插值有更好空间细节，EMIT数据验证重建结构具有物理意义。

Conclusion: 提出的自监督超分辨率框架能够在没有高分辨率真值的情况下有效提升Sentinel-5P数据的空间分辨率，为大气监测提供更精细的分析能力。
...
TL;DR: NTIRE 2026低光图像增强挑战赛综述，评估了22个团队提交的解决方案，展示了该领域的最新进展


<details>
  <summary>Details</summary>
Motivation: 低光图像存在对比度低、噪声大导致信息丢失的问题，需要开发有效的网络来恢复图像信息，产生更清晰、视觉上更吸引人的图像

Method: 通过竞赛形式收集解决方案，第一赛道195人注册，第二赛道153人注册，最终22个团队提交有效方案。使用新颖的数据集样本进行评估

Result: 展示了低光图像增强（联合去噪）领域的最先进进展，证明了该领域的显著进步

Conclusion: NTIRE 2026挑战赛成功推动了低光图像增强技术的发展，通过竞赛形式促进了该领域的研究进展
...
TL;DR: MixTGFormer：一种用于3D人体姿态估计的双流时空GCN-Transformer网络，通过并行通道融合全局和局部特征，在Human3.6M和MPI-INF-3DHP数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer方法主要关注全局时空关系建模，忽略了局部骨骼关系和不同通道间的信息交互，因此需要一种能同时建模全局和局部特征的方法。

Method: 提出MixTGFormer网络，核心是堆叠的Mixformer模块。Mixformer包含Mixformer Block和SE Layer，通过两个并行不同模式的Mixformer Block提取融合骨骼信息，再通过SE层补充。Mixformer Block将GCN集成到Transformer中，增强局部和全局信息利用，并实现时空形式提取时空关系。

Result: 在Human3.6M和MPI-INF-3DHP数据集上评估，MixTGFormer达到SOTA结果，P1误差分别为37.6mm和15.7mm。

Conclusion: MixTGFormer通过双流时空GCN-Transformer架构有效融合全局和局部特征，在3D人体姿态估计任务中表现出优越性能，解决了现有方法忽略局部骨骼关系和通道交互的问题。
...
TL;DR: DVSA是一个针对零样本学习的鲁棒框架，通过动态视觉语义对齐和标签消歧机制处理现实世界中的标签噪声和模糊性问题。


<details>
  <summary>Details</summary>
Motivation: 现有零样本学习方法通常假设标签是干净的，忽略了现实世界中的标签噪声和模糊性，这会导致性能下降。需要开发能够处理模糊标签的鲁棒ZSL框架。

Method: 提出动态视觉语义对齐（DVSA）框架：1）双向视觉语义对齐模块，通过注意力机制相互校准视觉特征和属性原型；2）基于互信息的对比优化，在属性层面增强判别性和语义一致性；3）动态标签消歧机制，迭代修正噪声监督同时保持语义一致性。

Result: 在标准基准测试上的广泛实验验证了DVSA在模糊监督下实现了更强的性能。

Conclusion: DVSA通过动态视觉语义对齐和标签消歧机制，有效处理零样本学习中的标签模糊性问题，提高了模型在现实世界噪声环境下的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统仅依赖几何特征的点云配准方法在低重叠和不完整场景中表现不佳，需要利用颜色信息来提升配准鲁棒性。

Method: 提出两阶段方法GeGS-PCR：1) 专用颜色编码器提取多级几何和颜色特征；2) Geometric-3DGS模块编码彩色超点的局部邻域信息；3) 使用LORA优化保持3DGS表达能力；4) 快速可微渲染细化配准；5) 联合光度损失结合几何和颜色特征。

Result: 在ColorKitti、Color3DMatch和Color3DLoMatch数据集上验证，达到99.9%的配准召回率，相对旋转误差低至0.013，相对平移误差低至0.024，精度提升至少2倍。

Conclusion: GeGS-PCR通过有效融合几何、颜色和高斯信息，显著提升了点云配准在挑战性场景下的性能，为低重叠点云配准提供了有效解决方案。
...
TL;DR: 提出GaussianHSI框架，基于高斯泼溅实现任意尺度高光谱图像超分辨率，结合Voronoi引导的双边2D高斯泼溅进行空间重建和光谱细节增强模块


<details>
  <summary>Details</summary>
Motivation: 现有高光谱图像超分辨率方法通常需要针对不同尺度进行调整，缺乏灵活性；现有方法依赖光栅化策略限制了灵活的空间建模；将现有方法扩展到高光谱图像超分辨率具有挑战性，需要在保持光谱保真度的同时实现自适应空间重建

Method: 提出GaussianHSI框架：1) Voronoi引导的双边2D高斯泼溅进行空间重建：预测一组高斯函数表示输入，通过Voronoi引导选择将目标像素与相关高斯函数关联，使用参考感知双边权重（考虑几何相关性和低分辨率特征一致性）聚合所选高斯函数；2) 光谱细节增强模块改善光谱重建

Result: 在基准数据集上的大量实验表明，GaussianHSI在任意尺度高光谱图像超分辨率任务上优于现有最先进方法

Conclusion: 提出的GaussianHSI框架成功解决了任意尺度高光谱图像超分辨率问题，通过高斯泼溅的连续表示实现了灵活的空间建模，同时保持了光谱保真度
...
TL;DR: 提出基于分数的冷冻电镜图像去噪框架，通过学习干净数据分数恢复粒子信号并保持结构一致性，改进下游粒子挑选和3D重建


<details>
  <summary>Details</summary>
Motivation: 冷冻电镜图像信噪比极低，现有去噪方法虽能改善视觉质量，但未明确考虑下游分析所需的结构一致性

Method: 提出基于分数的去噪框架，学习干净数据分数以恢复粒子信号；进一步引入目标引导变体，在弱信号条件下加入参考密度引导以稳定分数学习

Result: 在多个冷冻电镜数据集上实验表明，该方法能更好地抑制结构化低频背景，提高粒子-背景分离性，改进下游粒子挑选并产生更结构一致的3D重建
...
TL;DR: IncreFA是一个增量学习框架，用于持续适应新兴AI生成模型的图像溯源，通过层次约束和潜在记忆库实现持续学习，在包含28个模型的基准测试中达到最佳溯源准确率和98.93%的未知检测率。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成模型快速发展，图像溯源成为一个移动目标。新的扩散、对抗和自回归生成器几乎每月出现，使得现有水印、分类器和反演方法在发布时就过时。核心问题不在于模型识别，而在于溯源方法本身无法适应新模型。

Method: IncreFA将溯源重新定义为结构化增量学习问题，包含两个机制：1) 层次约束：通过可学习的正交先验编码架构层次，分离家族级不变性和模型特定特征；2) 潜在记忆库：重放紧凑的潜在示例并混合生成伪未见样本，稳定表示漂移并增强开放集意识。

Result: 在新构建的增量溯源基准(IABench)上，覆盖2022-2025年发布的28个生成模型，IncreFA在时间顺序开放集协议下实现了最先进的溯源准确率和98.93%的未知检测率。

Conclusion: IncreFA通过将图像溯源重新定义为增量学习问题，解决了现有方法无法适应快速发展的生成模型的根本问题，为持续适应新兴AI生成技术提供了有效框架。
...
TL;DR: DIFO++：利用视觉语言模型进行源自由域适应的新方法，通过交替定制ViL模型和知识蒸馏，专注于减少特征纠缠的"gap region"，显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 传统源自由域适应方法依赖伪标签和辅助监督，容易出错。本文首次探索利用现成的视觉语言多模态模型（如CLIP）的丰富知识，但发现直接零样本应用效果不佳，因为这些模型是通用而非任务特定的

Method: 提出DIFO++方法：1）通过提示学习最大化目标模型与ViL模型的互信息来定制ViL模型；2）将定制后的ViL模型知识蒸馏到目标模型，专注于减少特征纠缠的"gap region"；3）通过融合目标模型和ViL模型的预测生成可靠伪标签；4）使用类别注意力和预测一致性进行语义对齐，配合参考熵最小化抑制不确定性

Result: 大量实验表明，DIFO++在源自由域适应任务中显著优于现有最先进方法

Conclusion: DIFO++通过有效利用视觉语言模型的丰富知识，并专注于减少特征纠缠区域，成功实现了更准确、更鲁棒的源自由域适应，为这一领域提供了新的解决方案
...
TL;DR: 提出EgoIn框架解决以自我为中心的视觉状态转换任务，通过TransitionVLM理解转换过程，结合转换条件模块和对象感知辅助监督生成一致的中间帧序列。


<details>
  <summary>Details</summary>
Motivation: 理解物理转换过程对人类认知和AI系统都很重要，特别是从自我中心视角，这是人类与机器在动作建模中的关键桥梁。当前生成模型面临两个挑战：1)从自我中心视角理解初始和目标状态的视觉场景并推理转换步骤；2)生成一致的中间转换，遵循指令同时保持两个视觉状态间的对象外观。

Method: 提出EgoIn框架：1)使用TransitionVLM（在策划数据集上微调）推断两个给定状态间的多步骤转换过程；2)通过提出的转换条件模块基于转换条件生成帧序列；3)引入对象感知辅助监督以在整个转换过程中保持一致的对象外观。

Result: 在人-对象和机器人-对象交互数据集上的广泛实验表明，EgoIn在生成语义有意义且视觉连贯的转换序列方面表现出优越性能。

Conclusion: EgoIn框架成功解决了以自我为中心的视觉状态转换任务，通过结合视觉语言模型理解、转换条件生成和对象一致性监督，能够生成高质量的物理转换序列。
...
TL;DR: 提出一种稀疏体素空间扩散框架，通过均匀子采样时间步长实现3D医学图像增强，结合结构感知轨迹调制模块，在保持解剖细节的同时实现10倍训练加速。


<details>
  <summary>Details</summary>
Motivation: 3D医学图像增强对临床诊断至关重要，但现有扩散模型扩展到高分辨率3D体积时计算成本过高。作者观察到在条件增强中，退化输入的强解剖先验使得密集噪声调度变得冗余。

Method: 提出稀疏体素空间扩散框架：1) 在均匀子采样的紧凑时间步长集上训练和采样；2) 网络在数据流形上直接预测干净数据，在速度空间中进行监督以实现稳定梯度缩放；3) 轻量级结构感知轨迹调制模块根据局部解剖内容重新校准每个网络块的时间嵌入。

Result: 在CT、PET和MRI四个数据集上的实验表明，该方法在去噪和超分辨率任务上达到最先进性能，同时实现高达10倍的训练加速，且直接操作于体素空间避免了有损压缩。

Conclusion: 该方法通过稀疏时间调度和结构自适应调制，有效解决了3D医学图像增强中的计算瓶颈，在保持解剖细节的同时显著提升了训练效率。
...
TL;DR: 提出SAMGA框架，通过主体感知的多粒度对齐实现零样本EEG到图像检索，解决现有方法忽视EEG信号多尺度表征和主体间视觉粒度差异的问题。


<details>
  <summary>Details</summary>
Motivation: 现有零样本EEG到图像检索方法通常依赖单一固定视觉目标或主体不变的目标构建方案，忽视了视觉诱发EEG信号的两个重要特性：1）它们保留了多个表征尺度的信息；2）与EEG最佳匹配的视觉粒度可能因主体而异。

Method: 提出主体感知多粒度对齐（SAMGA）框架：1）通过自适应聚合预训练视觉编码器的多个中间表示来构建主体感知的视觉监督目标；2）设计粗到细的跨模态对齐策略，粗阶段稳定共享语义几何并减少主体引起的分布偏移，细阶段进一步提高实例级检索区分度。

Result: 在THINGS-EEG基准测试中，该方法在主体内设置下达到91.3% Top-1和98.8% Top-5准确率，在主体间设置下达到34.4% Top-1和64.8% Top-5准确率，优于现有最先进方法。

Conclusion: SAMGA框架通过主体感知的多粒度视觉目标构建和粗到细的对齐策略，有效解决了EEG到图像检索中的主体差异和多尺度表征问题，为可扩展的视觉神经解码和实用脑机接口提供了有前景的途径。
...
TL;DR: DuQuant++ 是一种针对 MXFP4 格式的量化方法，通过单次异常值感知旋转解决激活异常值问题，相比现有方法减少一半在线旋转成本，在 W4A4 量化下实现 SOTA 性能。


<details>
  <summary>Details</summary>
Motivation: MXFP4 微缩放格式在 LLM 推理中很有前景，但激活异常值会显著影响量化精度。现有旋转方法无法针对异常值集中的通道，需要更有效的解决方案。

Method: 将 DuQuant 的异常值感知细粒度旋转适配到 MXFP4 格式，使旋转块大小与微缩放组大小对齐（B=32）。利用 MXFP4 每组有独立缩放因子的特性，用单次异常值感知旋转替代原始 DuQuant 的双旋转和 zigzag 排列。

Result: 在 LLaMA-3 系列模型上的 MXFP4 W4A4 量化实验中，DuQuant++ 始终达到最先进的性能，同时将在线旋转成本减半并平滑权重分布。

Conclusion: DuQuant++ 成功解决了 MXFP4 格式中的激活异常值问题，通过单次异常值感知旋转实现了高效且高性能的量化，为 LLM 推理提供了实用的解决方案。
...
TL;DR: 提出WSRVOS方法，仅使用文本表达进行弱监督视频目标分割训练，无需像素级标注


<details>
  <summary>Details</summary>
Motivation: 现有弱监督方法仍需边界框或点标注，成本高昂。希望仅用文本表达训练模型，降低标注成本

Method: 1) 对比性文本增强：利用多模态大模型生成正负表达；2) 双向视觉-语言特征选择与交互；3) 实例感知表达分类；4) 正预测融合生成伪掩码；5) 时序片段排序约束

Result: 在A2D Sentences、J-HMDB Sentences、Ref-YouTube-VOS、Ref-DAVIS17四个数据集上表现出优越性能

Conclusion: WSRVOS方法仅需文本表达即可有效训练视频目标分割模型，显著降低标注成本，性能优于现有方法
...
<details>
  <summary>Details</summary>
Motivation: 现有基于渲染-编辑-优化的3D场景编辑方法存在严重的跨视角不一致问题。虽然已有方法引入几何线索、跨视角交互或视频先验来缓解这一问题，但它们主要依赖推理时同步，在鲁棒性和泛化性方面仍有局限。本文从分布视角重新审视多视角一致的3D编辑问题。

Method: 1) 从分布视角将3D场景编辑建模为跨视角的联合分布建模问题；2) 提出显式引入跨视角依赖关系的视角一致3D编辑框架；3) 设计双路径一致性机制，包括投影引导的结构指导和补丁级语义传播；4) 构建配对的跨视角编辑数据集，为学习编辑场景中的跨视角一致性提供可靠监督。

Result: 大量实验表明，该方法在复杂场景中实现了卓越的编辑性能，能够生成精确且视角一致的编辑结果。

Conclusion: 通过从分布视角重新构建多视角一致的3D编辑问题，并引入显式的跨视角依赖关系和双路径一致性机制，本文提出的方法有效解决了现有方法中的视角不一致瓶颈，在复杂场景中实现了更鲁棒和泛化性更强的编辑效果。
...
TL;DR: Re²MoGen是一个开放词汇运动生成框架，通过增强LLM推理生成初始运动规划，再通过RL后训练优化物理合理性，解决现有T2M模型在训练数据分布外性能下降的问题。
...
Method: 三阶段框架：1) 使用蒙特卡洛树搜索增强LLM推理，基于文本提示生成合理的关键帧；2) 使用人体姿态模型作为先验优化全身姿态，通过动态时间匹配目标微调预训练运动生成器；3) 使用物理感知奖励进行后训练，消除物理不合理性。

Result: 实验表明该框架能生成语义一致且物理合理的运动，在开放词汇运动生成中达到最先进性能。

Conclusion: Re²MoGen通过结合增强的LLM推理和RL后训练，有效解决了开放词汇运动生成问题，生成了高质量的运动序列。
...
TL;DR: 提出两阶段框架，利用2D扩散模型从网络视频中重建3D人体运动和人物-物体交互，解决动态相机下全局一致3D运动恢复的难题
...
Method: 两阶段框架：第一阶段从网络视频提取2D关键点，合成多视角2D运动数据；第二阶段训练相机条件多视角2D运动扩散模型，在特定领域合成数据上恢复世界空间中的3D人体运动和3D人物-物体交互

Result: 在体操等挑战性运动以及野外人物-物体交互视频上表现出色，优于现有方法，能生成更真实的人体运动和人物-物体交互

Conclusion: 提出的两阶段2D扩散框架能有效从网络视频中重建3D人体运动和人物-物体交互，特别是对于现有MoCap数据集中代表性不足的运动类型，在动态相机条件下表现优异
...
TL;DR: GR4CIL是一个结合任务判别和知识路由的CLIP-based类增量学习框架，通过保持任务特定视觉知识和稳定共享文本语义空间，减少任务间干扰，实现可靠的知识路由。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP-based类增量学习方法面临两个关键挑战：共享参数适应会导致旧知识漂移，任务特定知识组织会导致跨任务响应校准不佳，使得可靠路由困难。

Method: 提出GR4CIL框架，结合任务判别和知识路由，保持任务特定视觉知识同时维持增量稳定的共享文本语义空间；引入正交补偿机制缓解模态间隙引起的偏差，增强任务内判别，扩大真实任务与竞争任务间的分数差距。

Result: 在多个基准测试中，GR4CIL始终优于强基线方法。

Conclusion: GR4CIL能够实现更可靠的任务感知知识路由，同时保留零样本泛化能力，有效解决了CLIP-based类增量学习中的知识漂移和路由不可靠问题。
...
TL;DR: PCM-NeRF：一种概率框架，通过可学习的相机位姿不确定性增强神经表面重建，有效处理不完美的SfM位姿估计，防止异常视图破坏重建质量。


<details>
  <summary>Details</summary>
Motivation: 传统神经表面重建方法假设相机位姿（来自SfM）是完美准确的，但实际中位姿估计往往存在误差，导致重建结果扭曲或不完整。特别是在存在严重位姿异常值的情况下，现有方法表现不佳。

Method: 基于SG-NeRF构建概率框架，为每个相机位姿学习均值和方差分布（方差初始化为SfM对应质量）。通过不确定性正则化损失将学习到的方差与视图置信度耦合，不确定性直接调节有效位姿学习率：不确定性高的相机获得阻尼梯度更新，防止初始化差的视图破坏重建。该方法轻量级，无需修改渲染管线。

Result: 在具有严重位姿异常值的挑战性场景上，PCM-NeRF在Chamfer Distance和F-Score指标上一致优于最先进方法，特别是在几何复杂结构上，且无需前景掩码。

Conclusion: PCM-NeRF通过概率建模相机位姿不确定性，有效提升了神经表面重建在存在不完美位姿估计情况下的鲁棒性和重建质量，为处理实际场景中的位姿误差提供了轻量级解决方案。
...
TL;DR: 提出AI框架，從MRI自動分割胸腰椎並生成3D重建，避免兒童脊柱畸形評估中的輻射暴露


<details>
  <summary>Details</summary>
Motivation: MRI在兒科影像中優於CT（無輻射），但脊柱畸形評估仍依賴CT，因缺乏自動化高解析度3D骨骼重建。MRI重建因手動流程和標註數據稀缺而不實用

Method: 使用GAN將青少年特發性脊柱側彎患者的低劑量CT轉換為MRI樣圖像，結合現有標註胸椎MRI數據，訓練U-Net模型進行自動胸腰椎分割和3D重建

Result: 算法準確生成連續胸腰椎3D重建，分割準確率達88%（Dice分數），處理時間從約1小時縮短至1分鐘內，保留脊柱側彎特異性畸形特徵

Conclusion: 該方法實現無輻射MRI 3D畸形評估，支持兒科脊柱護理的臨床評估、手術規劃和導航
...
TL;DR: UniCSG：用于文本引导和参考引导风格迁移的统一框架，通过分阶段训练解决内容-风格纠缠问题，提升内容保真度和风格对齐


<details>
  <summary>Details</summary>
Motivation: 基于DiT的扩散模型在风格迁移中存在内容-风格纠缠问题，导致参考内容泄漏和生成不稳定，需要解决内容保真度和风格对齐的平衡问题

Method: 采用分阶段训练：1）潜在空间语义解缠阶段（低频预处理+条件损坏）；2）潜在空间频率感知细节重建阶段（多尺度频率监督）；3）像素空间奖励学习（对齐潜在目标与感知质量）

Result: 实验表明在文本引导和参考引导两种设置下，UniCSG在内容忠实度、风格对齐和鲁棒性方面均有改进

Conclusion: UniCSG通过统一的框架有效解决了内容-风格纠缠问题，在保持内容语义的同时实现了更好的风格匹配
...
TL;DR: 提出PlankFormer框架，通过合成伪群落图像解决标注数据稀缺问题，结合Vision Transformer和Mask2Former进行浮游生物实例分割，在含高密度碎屑的真实数据上优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 浮游生物监测对评估水生生态系统至关重要，但人工显微镜分析劳动密集。自动分割面临两大挑战：1)像素级标注数据集稀缺；2)传统CNN方法难以区分浮游生物与碎屑及重叠个体。

Method: 提出PlankFormer框架：1)生成伪群落图像(PCI)，将单个浮游生物图像合成到多样化背景上；2)采用Vision Transformer骨干网络和Mask2Former解码器；3)使用Masked Autoencoder在无标注个体图像上进行自监督预训练，以鲁棒捕捉全局结构特征。

Result: 在真实数据集上的实验结果表明，该方法显著优于传统方法如Mask R-CNN，特别是在高碎屑密度的挑战性环境中。合成训练策略和MAE架构能够实现高精度分割，同时减少对个体浮游生物图像手动标注的需求。

Conclusion: PlankFormer通过合成数据生成和自监督预训练，有效解决了浮游生物分割中的数据稀缺和复杂背景挑战，为自动化浮游生物监测提供了高效解决方案。
...
TL;DR: LiteBounD：一种轻量级边界引导蒸馏框架，将多个视觉基础模型的语义和结构先验知识迁移到紧凑分割骨干网络中，用于息肉分割任务。


<details>
  <summary>Details</summary>
Motivation: 现有轻量级分割模型（如U-Net、U-Net++、PraNet）在临床部署中效率高，但难以捕捉复杂息肉区域所需的丰富语义和结构线索；而大型视觉基础模型（如SAM、OneFormer）泛化能力强，但由于领域不匹配、边界敏感性不足和计算成本高，在息肉分割任务上表现不佳。

Method: 提出LiteBounD框架：1）双路径蒸馏机制，解耦语义和边界感知表示；2）频率感知对齐策略，分别监督低频全局语义和高频边界细节；3）边界感知解码器，将多尺度编码器特征与蒸馏的语义丰富边界信息融合。

Result: 在已见（Kvasir-SEG、CVC-ClinicDB）和未见（ColonDB、CVC-300、ETIS）数据集上的实验表明，LiteBounD显著优于轻量级基线方法，性能与最先进方法相当，同时保持实时临床使用所需的效率。

Conclusion: LiteBounD成功地将视觉基础模型的互补语义和结构先验知识迁移到轻量级分割网络中，在保持高效率的同时实现了准确的息肉分割，为临床实时应用提供了实用解决方案。
...
TL;DR: 视频大语言模型在否定式心理操控下会出现时空谄媚行为：撤回正确的视觉判断，迎合误导性用户反馈，并编造虚假时空解释来合理化错误修正。


<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在视频理解任务中表现出色，但其在对话交互中的鲁棒性尚未充分探索。研究者发现模型在否定式心理操控下会出现"时空谄媚"这一故障模式，即撤回最初正确的视觉判断并迎合误导性反馈。

Method: 提出了基于否定的心理操控评估框架，并创建了GasVideo-1000基准测试，专门用于探测时空谄媚行为。该基准具有清晰的视觉基础和时序推理要求，评估了多种开源和专有的先进视频大语言模型。

Result: 实验表明，否定式心理操控的脆弱性普遍且严重，即使是基线性能强的模型也受影响。提示级别的约束只能部分缓解这种行为，无法可靠防止幻觉解释或信念反转。

Conclusion: 当前视频大语言模型缺乏在对抗性对话反馈下保持基于时空信念的鲁棒机制，需要开发更强大的机制来维持基于视觉证据的信念。
...
TL;DR: BASFNet是一个基于边界感知频域和空域融合的伪装目标检测框架，通过相位谱增强的边缘探索模块和空间核心分割模块联合捕捉伪装目标的边界和物体特征，在三个基准数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前COD方法主要依赖空域边缘提取和局部像素级信息，忽视了全局结构特征的重要性，并且未能有效利用频域特征中的相位谱信息。

Method: 提出BASFNet框架，采用频域和空域特征的双重引导集成。包括基于相位谱的频率增强边缘探索模块(FEEM)、空间核心分割模块(SCSM)，以及通过空间-频率融合交互模块(SFFIM)有效集成这些特征，并采用边界感知训练策略进一步优化边界检测。

Result: BASFNet在三个基准数据集上优于现有的最先进方法，验证了频域和空域信息融合在COD任务中的有效性。

Conclusion: 通过有效融合频域和空域信息，特别是利用相位谱信息，BASFNet能够更好地捕捉伪装目标的边界和物体特征，在伪装目标检测任务中取得了显著改进。
...
TL;DR: AeroRAG：基于场景图的多模态检索增强生成框架，通过结构化视觉知识（对象、数量、空间关系）而非密集视觉token来解决航拍场景VQA难题，在航拍数据集上显著优于现有MLLM方法。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在航拍场景视觉问答中面临挑战，因为关键证据（小物体、明确数量、粗略位置、对象间关系）与传统的密集视觉token表示不匹配，需要更结构化的感知-语言推理接口。

Method: 提出AeroRAG框架：1）将输入图像转换为结构化视觉知识（对象类别、数量、空间位置、语义关系）；2）检索查询相关的语义块构建紧凑提示；3）使用基于文本的LLM进行推理，而非直接在密集视觉token上推理。

Result: 在AUG航拍数据集和通用VG-150基准上，相比6个强MLLM基线取得一致改进，在密集航拍场景和关系敏感推理中提升最大。在VQAv2上的验证表明该接口仍兼容标准视觉推理设置。

Conclusion: 结构化检索是面向部署和接地视觉推理系统的实用设计方向，通过显式的中间接口更好地对齐感知和语言推理。
...
TL;DR: ReTrack：首个通过校准组合特征方向偏差来改进多模态查询理解的CVR框架，在CVR和CIR任务上均取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: CVR任务中视频和文本模态存在信息密度差异，传统组合方法偏向参考视频导致检索性能不佳，存在模态贡献纠缠、组合特征显式优化和检索不确定性三大挑战

Method: 提出ReTrack框架，包含语义贡献解耦、组合几何校准和可靠证据驱动对齐三个关键模块，通过估计各模态语义贡献来校准组合特征方向偏差，利用校准方向锚计算双向证据进行可靠相似度估计

Result: 在CVR和CIR任务的三个基准数据集上均取得SOTA性能，展现出强大的泛化能力

Conclusion: ReTrack通过校准组合特征方向偏差有效解决了CVR任务中的核心挑战，显著提升了多模态查询理解能力，为视频检索领域提供了创新解决方案
...
TL;DR: 提出MEDN网络，通过双分支结构分别提取运动特征和情感特征，解决微表情识别中AU与情感映射不一致的问题，提高识别准确率和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 微表情识别面临挑战：微表情不遵循严格的情感与动作单元(AU)映射规则，某些微表情具有相同的AU却代表完全相反的情感类别，导致视觉上高度相似。现有方法主要依赖显式面部运动线索（如光流、帧差、AU特征），而忽略了隐式情感信息。

Method: 提出运动情感特征解耦网络(MEDN)：1) 双分支框架分别提取运动和情感特征；2) 运动分支使用AU检测任务约束特征到显式运动域，采用正交损失减少运动情感特征耦合；3) 情感分支提出稀疏情感视觉变换器(SEVit)，通过多尺度稀疏率稀疏化空间标记以突出局部时间变化；4) 开发协作融合模块(CoFM)自适应融合解耦的运动和情感特征。

Result: 在三个基准数据集上的大量实验验证了MEDN能有效解耦运动和情感特征，并实现了优越的识别性能，为提升识别准确率和泛化能力提供了新视角。

Conclusion: MEDN通过显式运动特征和隐式情感特征的有效解耦与融合，解决了微表情识别中AU与情感映射不一致的挑战，显著提升了识别性能，为微表情识别领域提供了新的技术路径。
...
TL;DR: TranCLR提出了一种基于过渡锚点的对比学习框架，用于骨架动作识别，通过建模动作空间的连续几何结构来克服传统二值对比学习忽略运动连续性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有自监督对比学习方法依赖二值对比目标，忽略了人体运动的固有连续性，导致特征簇碎片化和类别边界僵化，无法有效捕捉动作的连续几何结构。

Method: 提出TranCLR框架：1) 动作过渡锚点构建(ATAC)显式建模过渡状态的几何结构；2) 多级几何流形校准(MGMC)机制自适应校准多级连续性的动作流形。

Result: 在NTU RGB+D、NTU RGB+D 120和PKU-MMD数据集上的实验表明，TranCLR实现了优越的准确性和校准性能，有效学习了连续且不确定性感知的骨架表示。

Conclusion: TranCLR通过建模动作空间的连续几何结构，解决了传统对比学习忽略运动连续性的问题，为骨架动作识别提供了更平滑、更具判别性的表示空间。
...
TL;DR: 基于预训练视觉语言模型(VLM)的统一自动驾驶框架，通过单一因果解码器实现感知、规划和文本生成等多任务学习，解决了现有系统架构碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶系统通常需要为不同的解码行为（如自回归文本生成、并行目标检测、轨迹回归）引入分离或级联的解码器，导致架构碎片化和骨干网络复用受限。

Method: 在预训练VLM基础上构建统一框架，通过单一因果Transformer解码器协调异构解码行为。将视觉和结构化查询token组织在同一个因果解码器中，使结构化查询能通过原始注意力机制自然地条件化于视觉上下文。文本和结构化输出共享注意力骨干，实现跨异构任务的稳定联合优化。

Result: 在nuScenes开环评估中达到0.28 L2误差和0.18碰撞率，在NAVSIM闭环评估中获得86.8 PDMS的竞争性结果。完整模型保留多模态生成能力，高效推理模式降低约40%延迟。

Conclusion: 预训练VLM注意力在纯语言建模之外具有很强的可迁移性，通过统一框架可以在单个因果解码器中实现自动驾驶的多任务学习，同时保持高性能和效率。
...
TL;DR: 使用YOLOv11检测器定位SAR图像中的船只，然后通过SAM2进行零样本实例分割，无需像素级标注，在SSDD基准上达到0.637 mIoU（监督基线的89%）。


<details>
  <summary>Details</summary>
Motivation: SAR在海上监视中至关重要，但深度学习分析受限于缺乏像素级标注。需要探索如何利用通用视觉基础模型实现SAR图像的零样本实例分割。

Method: 使用在开放SAR数据集上训练的YOLOv11检测器定位船只边界框，然后用这些边界框提示Segment Anything Model 2（SAM2）生成实例掩码，无需任何掩码标注。不同于先前依赖微调或适配器的方法，仅通过SAR训练检测器的空间约束来正则化基础模型预测。

Result: 在SSDD基准测试中达到0.637的平均IoU（完全监督基线的89%），总体船只检测率为89.2%。该方法部分缓解了光学-SAR域差距，并支持下游应用如船只分类、尺寸估计和尾迹分析。

Conclusion: 仅通过SAR训练检测器的空间约束就能有效正则化基础模型预测，为基于基础模型的SAR图像理解提供了一条可扩展、标注高效的途径。
...
TL;DR: 提出BI-Cap，一种神经模拟感知模拟范式，通过模拟人类视觉系统处理来对齐神经和视觉模态，显著提升零样本脑到图像检索性能。


<details>
  <summary>Details</summary>
Motivation: 当前脑机接口和计算神经科学中的视觉解码方法受限于神经与视觉模态之间的系统性差异，且忽视了人类视觉系统的内在计算机制。

Method: 构建包含四个生物合理性动态和静态变换的神经模拟管道，结合互信息引导的动态模糊调节来模拟自适应视觉处理，并引入证据驱动的潜在空间表示来建模不确定性。

Result: 在两个公共基准测试的零样本脑到图像检索任务中，BI-Cap显著优于现有最先进方法，分别获得9.2%和8.0%的相对提升。

Conclusion: BI-Cap通过模拟人类视觉系统处理成功对齐神经和视觉模态，为脑机接口和计算神经科学的视觉解码提供了有效解决方案。
...
TL;DR: HONES是一个无需梯度的框架，用于多任务视觉语言模型中的任务感知神经元归因和调控，通过条件化注意力头来识别和调节关键神经元。


<details>
  <summary>Details</summary>
Motivation: 现有神经元分析方法主要关注单一任务，限制了跨任务神经元重要性的可比性。现有的排名策略倾向于孤立地评分神经元，忽视了任务依赖信息路径如何塑造前馈网络神经元的写入效应。这种忽视在多任务设置中会加剧神经元的多义性，给任务关键神经元的识别和干预引入噪声。

Method: HONES（Head-Oriented Neuron Explanation & Steering）是一个无需梯度的框架，通过条件化任务相关注意力头来对FFN神经元进行因果写入贡献排名，并通过轻量级缩放来调节显著神经元。

Result: 在四个不同的多模态任务和两个流行的VLM上的实验表明，HONES在识别任务关键神经元方面优于现有方法，并在调控后提高了模型性能。

Conclusion: HONES提供了一个有效的框架，用于多任务视觉语言模型中的任务感知神经元归因和调控，通过考虑任务依赖的注意力模式来更准确地识别和干预关键神经元。
...
TL;DR: ZSG-IAD：基于多模态视觉语言框架的零样本工业异常检测，能生成结构化异常报告和像素级异常掩码，提供物理可解释的缺陷证据。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习工业异常检测器多为黑盒模型，难以提供物理意义的缺陷证据来支持决策。需要开发能提供透明解释的异常检测系统。

Method: 提出ZSG-IAD多模态框架，包含语言引导的两跳定位模块：1）异常相关句子从多模态特征中提取证据式潜在槽，获得粗粒度空间支持；2）通过通道-空间门控和轻量解码器调制特征图，生成细粒度掩码。采用可执行规则GRPO提升输出可靠性。

Result: 在多个工业异常基准测试中表现出强大的零样本性能，相比先前方法提供更透明、物理可解释的解释。

Conclusion: ZSG-IAD为可信赖工业异常检测系统提供了新方向，能够生成结构化异常报告和像素级定位，增强决策透明度和可解释性。
...
TL;DR: CoEvoer：首个专为上肢姿态与形状估计设计的单阶段协同交叉依赖Transformer框架，通过显式特征交互提升面部和手部区域参数估计精度及泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前表达性人体姿态与形状估计方法在面部和手部区域参数估计精度不足，对野外图像泛化能力有限，需要专门针对上肢区域（面部、手部、躯干）强耦合关系的框架。

Method: 提出CoEvoer单阶段协同交叉依赖Transformer框架，通过显式特征级交互实现不同身体部位间的相互增强：躯干提供全局语义和位置先验指导面部和手部估计，面部和手部细节则帮助校准相邻身体部位。

Result: 在多个上肢基准测试中达到最先进性能，在未见过的野外图像上表现出强大的泛化能力。

Conclusion: CoEvoer是首个专门针对上肢表达性姿态与形状估计的框架，通过捕捉面部、手部和躯干之间的强耦合关系和语义依赖，显著提升了参数估计精度和泛化性能。
...
TL;DR: DifFoundMAD：一种参数高效的差分形态攻击检测框架，利用视觉基础模型的泛化能力，在严格安全级别下将错误率从6.16%降至2.17%


<details>
  <summary>Details</summary>
Motivation: 传统D-MAD系统依赖人脸识别嵌入或手工特征差异，需要更有效的差分检测方法来捕获可疑形态与真实图像之间的差异，特别是在边境控制等操作部署中需要严格安全级别

Method: 采用标准差分范式，但将底层表示空间替换为基础模型提取的嵌入；结合轻量级微调和类别平衡优化，仅更新少量参数同时保留基础模型的丰富表示先验

Result: 在标准D-MAD基准上进行广泛的跨数据库评估，DifFoundMAD在严格安全级别下表现优异：将当前最先进系统的错误率从6.16%降低到2.17%

Conclusion: DifFoundMAD通过利用视觉基础模型的泛化能力和参数高效微调，在差分形态攻击检测中实现了显著性能提升，特别是在操作部署所需的高安全级别下
...
<details>
  <summary>Details</summary>
Motivation: 现有通用神经辐射场（GeNeRFs）在真实场景中面临瞬态干扰物破坏跨视角结构一致性的问题，导致监督信号被污染、重建质量下降。现有干扰物去除方法依赖单场景优化，且基于单视角重建误差估计不确定性，对GeNeRFs不可靠，常将不一致的静态结构误判为干扰物。

Method: 提出MU-GeNeRF框架，将干扰物感知分解为两个互补的不确定性组件：1）源视角不确定性，捕捉由视角变化或动态因素引起的跨源视角结构差异；2）目标视角不确定性，检测目标图像中由瞬态干扰物引起的观测异常。通过异方差重建损失结合这两种不确定性，自适应调节监督信号。

Result: 大量实验表明，该方法不仅超越了现有GeNeRFs，而且达到了与场景特定干扰物去除NeRFs相当的性能。

Conclusion: MU-GeNeRF通过多视角不确定性引导的干扰物感知机制，有效解决了GeNeRF在瞬态干扰物存在下的鲁棒建模挑战，实现了更稳健的干扰物抑制和几何建模。
...
TL;DR: 提出了E3VS-Bench基准测试，用于评估在5自由度视角控制下的3D视觉搜索能力，现有视觉语言模型在主动感知和连贯视角规划方面与人类存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有视觉搜索和具身AI基准测试（如EQA）通常依赖静态观察或受限的自我中心运动，无法充分评估在真实3D环境中无限制5自由度视角控制下出现的细粒度视角依赖现象。

Method: 使用3D高斯泼溅技术重建99个高保真3D场景，构建2,014个问题驱动的任务场景，支持逼真的自由视角渲染以保留细粒度视觉细节，要求智能体通过5自由度视角控制收集视角依赖证据来回答问题。

Result: 尽管现有视觉语言模型在2D推理方面表现强大，但在5自由度视角变化下的主动感知和连贯视角规划方面与人类存在显著差距，所有模型都表现出明显的性能不足。

Conclusion: E3VS-Bench填补了现有基准测试的空白，揭示了当前视觉语言模型在5自由度视角控制下的3D视觉搜索任务中的局限性，为未来研究提供了重要的评估平台。
...
TL;DR: 提出使用合成视频数据审计人类动作识别模型偏见的框架，通过控制肤色等视觉属性，发现流行模型在相同动作下对肤色存在统计显著偏见


<details>
  <summary>Details</summary>
Motivation: 人类动作识别模型越来越多地部署在高风险环境中，但尚未分析其在不同人类外观特征上的公平性。需要开发框架来审计HAR模型中的偏见，特别是针对肤色等视觉属性

Method: 使用BEDLAM模拟平台生成合成视频数据，完全控制视觉身份属性（如肤色），保持时间一致性，通过受控干预测试单个属性变化对模型预测的影响

Result: 研究表明，即使动作完全相同，一些流行的HAR模型在肤色方面表现出统计显著的偏见。模型可能编码了不需要的视觉关联，并存在跨群体的系统性错误

Conclusion: 这项工作为审计HAR模型提供了框架，支持开发更透明、可问责的系统，以应对即将到来的监管标准要求
...
TL;DR: 提出PSRD框架，通过相位自奖励解码动态缓解视觉幻觉，无需外部监督，将LLaVA-1.5-7B的幻觉率降低50%


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型存在视觉幻觉问题，现有方法要么需要大量标注数据进行微调（计算开销大），要么采用静态后处理策略（忽略了幻觉出现的动态特性）

Method: 提出PSRD（相位自奖励解码）框架：1）发现视觉幻觉呈现相位动态模式，在每个语义阶段开始时达到峰值；2）通过相位自奖励信号进行在线幻觉校正；3）将幻觉指导信号从LVLMs蒸馏到轻量级奖励模型中，在解码过程中提供实时指导

Result: PSRD将LLaVA-1.5-7B的幻觉率降低50.0%，在五个幻觉评估基准上对四种LVLMs均优于现有后处理方法，有效缓解幻觉传播，实现性能与推理效率的可控权衡

Conclusion: PSRD框架能够在推理时动态缓解视觉幻觉，无需外部监督，显著降低幻觉率，同时保持高效推理，为大型视觉语言模型的幻觉问题提供了有效的解决方案
...
TL;DR: 提出一种轻量级误差预测网络，结合保形风险控制理论，为内窥镜超分辨率系统提供像素级可靠性评估，首次实现模型无关的理论保障实时安全增强。


<details>
  <summary>Details</summary>
Motivation: 超分辨率模型在微创手术和诊断视频中应用日益广泛，但存在产生幻觉结构和放大噪声的风险，限制了其在安全关键场景中的可靠性。需要一种方法来识别重建可能失败的区域，提高系统的可信度。

Method: 提出一个轻量级误差预测网络，在中间表示上运行以估计像素级重建误差。通过构建保形失败掩码（CFM）将预测转换为操作失败决策，该方法基于保形风险控制原理，提供理论保证来控制容忍误差限和检测失败中的未覆盖范围。

Result: 在图像和视频超分辨率任务上评估该方法，证明其在内窥镜和机器人手术场景中有效检测不可靠重建。该方法计算高效、低延迟，适合实时部署。

Conclusion: 这是首个提供模型无关、理论基础的实时内窥镜图像超分辨率安全增强方法，通过识别不可信区域提高系统可靠性，为安全关键医疗应用提供保障。
...
TL;DR: MV-HGNN：基于多视图层次图神经网络的草图3D形状检索框架，通过视图级图建模几何关系、层次图粗化选择关键视图，并利用CLIP语义原型实现类别无关对齐，在类别级和零样本检索中均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有SBSR方法存在两个核心问题：1）对3D多视图特征采用简化的聚合策略，忽略了视图间的几何关系和多层次细节，导致3D表示能力弱；2）受限于可见类别，在零样本场景下性能不佳。

Method: 提出MV-HGNN框架：1）构建视图级图，通过局部图卷积和全局注意力捕获相邻几何依赖和跨视图信息传递；2）引入视图选择器进行层次图粗化，扩大感受野并减少冗余视图干扰；3）利用CLIP文本嵌入作为语义原型，将草图和3D特征投影到共享语义空间；4）采用两阶段训练策略（类别级检索）和一阶段策略（零样本检索）。

Result: 在两个公开基准测试中，MV-HGNN在类别级和零样本设置下均优于最先进方法。

Conclusion: MV-HGNN通过层次图建模视图间几何关系和CLIP语义对齐，有效解决了3D表示弱和零样本性能差的问题，为草图3D形状检索提供了更强大的框架。
...
TL;DR: CFSR是一个多模态先验驱动的阴影去除框架，通过整合3D几何线索和大规模基础模型语义，将阴影去除重构为物理约束的恢复过程，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 传统阴影去除网络将图像恢复视为无约束映射，缺乏物理可解释性，难以平衡局部纹理恢复与全局光照一致性。需要一种能够有效弥合2D-3D领域差距的物理约束方法。

Method: 1) 将观测映射到自定义HVI颜色空间以抑制阴影噪声；2) 几何与语义双重显式引导注意力机制，利用DINO特征和3D表面法线直接调制注意力亲和矩阵；3) 通过冻结的CLIP编码器注入整体先验；4) 频率协同重建模块(FCRM)通过解耦解码过程实现最优合成。

Result: 在多个具有挑战性的基准测试中，CFSR实现了最先进的性能，能够有效恢复严重退化区域，同时保持全局光照一致性。

Conclusion: CFSR通过整合多模态先验和物理约束，成功解决了阴影去除中局部纹理恢复与全局光照一致性的平衡问题，为图像恢复任务提供了物理可解释的解决方案。
...
TL;DR: HABIT框架通过互知识估计和双一致性渐进学习解决组合图像检索中的噪声三元组对应问题，显著提升在噪声环境下的检索性能。


<details>
  <summary>Details</summary>
Motivation: 组合图像检索在实际应用中面临噪声三元组对应问题，主要源于标注成本高和主观性强，导致训练数据存在噪声，影响模型性能。

Method: 提出HABIT框架，包含两个核心模块：1) 互知识估计模块，通过计算组合特征与目标图像间互信息的转移率来量化样本清洁度；2) 双一致性渐进学习模块，通过历史模型与当前模型的协作机制，模拟人类习惯形成过程，实现鲁棒学习。

Result: 在两个标准CIR数据集上的实验表明，HABIT在各种噪声比例下显著优于大多数方法，展现出优越的鲁棒性和检索性能。

Conclusion: HABIT框架有效解决了组合图像检索中的噪声三元组对应问题，通过精确估计组合语义差异和渐进适应修改差异，实现了在噪声环境下的鲁棒学习。
...
TL;DR: GS-STVSR：基于2D高斯泼溅的超高效连续时空视频超分辨率框架，通过高斯核的时空演化绕过密集网格查询，在保持高质量的同时大幅提升推理效率
...
Method: 提出基于2D高斯泼溅的框架，通过连续运动建模驱动高斯核的时空演化；利用协方差参数的时间稳定性进行轻量级中间拟合；设计光流引导的运动模块；引入协方差重采样对齐模块防止协方差漂移；提出自适应偏移窗口处理大尺度运动

Result: 在Vid4、GoPro和Adobe240基准测试中达到最先进的质量水平；推理时间在常规时间尺度（X2-X8）下保持几乎恒定，在极端尺度X32下实现超过3倍加速

Conclusion: GS-STVSR通过2D高斯泼溅方法有效解决了连续时空视频超分辨率中的效率瓶颈，在保持高质量的同时显著提升推理速度，具有强大的实际应用价值
...
TL;DR: INTENT模型解决组合图像检索中的噪声三元组对应问题，通过视觉不变性组合和双目标判别学习处理跨模态对应噪声和模态固有噪声。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的组合图像检索数据集由于高标注成本不可避免地包含标注错误，导致错误匹配的三元组。现有方法假设所有样本都正确匹配，无法处理噪声问题。噪声可分为跨模态对应噪声和模态固有噪声，后者常被忽视。

Method: 提出INTENT模型：1）视觉不变性组合：通过快速傅里叶变换进行因果干预，生成干预后的组合特征，强制视觉不变性，使模型在组合时忽略模态固有噪声；2）双目标判别学习：采用正负样本协同优化，构建可扩展决策边界，根据忠诚度动态调整决策，实现鲁棒的对应判别。

Result: 在两个广泛使用的基准数据集上进行大量实验，证明了INTENT的优越性和鲁棒性。

Conclusion: INTENT能有效处理组合图像检索中的噪声三元组对应问题，特别是同时处理跨模态对应噪声和模态固有噪声，在噪声环境下表现出优越性能。
...
TL;DR: 提出动态前缀加权框架，通过门控模块动态调整前缀权重，结合适配器提升视觉语言模型在域类增量学习中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效方法（如前缀调优）通常对向量权重进行归一化，忽略了不同输入token需要不同程度的调整。这限制了模型在域类增量学习场景中的适应能力。

Method: 提出动态前缀加权框架：1）门控模块根据输入token重要性动态调整每个前缀的权重；2）权重机制将适配器输出权重作为前缀调优权重的残差，确保仅在必要时使用适配器。

Result: 实验结果表明，该方法在视觉语言模型的域类增量学习场景中实现了最先进的性能。

Conclusion: 动态前缀加权框架通过动态调整前缀权重和智能使用适配器，有效解决了现有参数高效方法在域类增量学习中的局限性。
...
TL;DR: 使用扩散模型生成合成图像提升低数据条件下的军事车辆检测性能，结合ControlNet结构引导进一步提升效果


<details>
  <summary>Details</summary>
Motivation: 研究扩散模型生成的合成图像是否能改善低数据条件下的军事车辆检测，特别是在军事领域数据稀缺的情况下

Method: 使用FLUX.1扩散模型，通过LoRA微调仅用8或24张真实图像生成15类车辆合成数据，结合ControlNet进行结构引导，训练RF-DETR检测器

Result: 扩散生成图像显著提升检测性能（8张真实样本时mAP50提升+8.0%），结构引导在数据极度稀缺时进一步改善性能（+4.1%），但数据充足时无额外收益

Conclusion: 特定对象的扩散模型能有效提升低数据条件下的军事目标检测，结构引导在数据极度稀缺时最有益，为军事AI系统训练提供了传统仿真管道的替代方案
...
TL;DR: 使用无人机系统辅助水上救援，通过YOLO算法自动定位溺水者，并通过离散事件仿真评估响应时间，结果显示无人机系统可将响应时间缩短至传统救援的五分之一。


<details>
  <summary>Details</summary>
Motivation: 水上溺水救援面临时间压力大、水域广阔无人监管、目标定位困难、救援人员运输复杂等挑战，需要技术创新来提高救援效率和成功率。

Method: 提出基于无人机箱系统的无人机救援方案，使用YOLO目标检测架构自动定位溺水者，创建专用数据集进行训练，并采用离散事件仿真方法模拟传统救援和无人机救援的响应时间。

Result: 评估了YOLO v3、v5、v8不同架构尺寸的性能，使用mAP@.5和mAP@.5:.95指标。在德国劳西茨湖区测试表明，无人机辅助救援显著缩短响应时间，即使是小型无人机系统（2个机库各1架无人机）也能将响应时间减少至传统救援的五分之一。

Conclusion: 无人机系统结合YOLO目标检测技术能够有效提高水上救援效率，大幅缩短响应时间，为溺水救援提供了创新的技术解决方案。
...
TL;DR: 提出DAP方法，通过将梯度定位的决策相关先验注入Transformer注意力传播，生成更类别敏感、紧凑和忠实的归因图，提升ViT可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有注意力解释方法主要依赖原始注意力权重，不能明确反映最终决策且类别区分性有限；而梯度定位方法虽能突出类别特定证据，但未能充分利用Transformer的层次注意力传播机制。

Method: 提出决策感知注意力传播(DAP)：通过梯度定位估计token重要性，并将其整合到逐层注意力展开中，同时捕捉注意力结构流和与最终预测最相关的证据。

Result: 在不同规模的Vision Transformer变体上进行广泛实验，DAP在定量指标和定性可视化方面均优于现有基线方法。

Conclusion: 决策感知传播是改进ViT可解释性的有效方向，DAP生成的归因图比传统注意力方法更类别敏感、紧凑和忠实。
...
TL;DR: PDF框架通过不确定性数据增强和延迟反馈扰动学习，提升VLA模型在环境变化下的决策鲁棒性，无需微调基础模型。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言-动作模型在环境微小变化下表现脆弱，存在轨迹过拟合问题，模型过度关注动作与实体间的虚假相关性，导致决策性能下降。

Method: 提出PDF框架：1) 基于不确定性的数据增强和动作投票来缓解虚假相关性；2) 自适应调度器平衡性能与效率；3) 轻量级扰动模块利用延迟反馈调整动作logits，纠正过度自信问题。

Result: 在LIBERO数据集上成功率提升7.4%，在Atari上人类标准化分数提升10.3%，相比原始VLA和带测试时适应的VLA均有显著提升。

Conclusion: PDF框架为多模态决策智能体提供了一种实用的测试时适应路径，无需验证器或基础模型微调，显著提升了决策鲁棒性。
...
TL;DR: SurgLIME：利用LLM生成带噪声的文本描述进行手术视觉语言预训练，通过置信度估计机制减轻噪声影响，保持视觉基础模型的医学先验知识。


<details>
  <summary>Details</summary>
Motivation: 手术视觉编码器在自监督学习方面取得进展，但扩展到多模态推理任务受到专家文本标注成本高昂的限制。需要大规模多模态数据集，但LLM生成的文本可能存在错误和幻觉，可能损害预训练的医学先验知识。

Method: 提出SurgLIME框架：1）使用LLM生成大规模手术视频文本描述创建LIME数据集；2）采用LoRA适配的双编码器架构保持医学先验；3）引入自动置信度估计机制，在对比对齐中动态降低不确定文本的权重。

Result: 在AutoLaparo和Cholec80基准测试中，SurgLIME实现了竞争力的零样本跨模态对齐，同时保持了视觉基础模型的稳健线性探测性能。

Conclusion: SurgLIME提供了一种参数高效的视觉语言预训练方法，能够利用噪声文本数据进行可靠的多模态对齐，同时保护医学先验知识，为手术AI系统的大规模多模态学习提供了可行方案。
...
TL;DR: LPQLD通过标签剪枝和量化技术，在大规模数据集蒸馏中显著减少软标签存储需求（ImageNet-1K减少78倍，ImageNet-21K减少500倍），同时提升准确率。


<details>
  <summary>Details</summary>
Motivation: 大规模数据集蒸馏需要存储巨大的软标签（ImageNet-1K上30-40倍，ImageNet-21K上200倍），这违背了数据集压缩的初衷。主要问题包括：1）图像多样性不足，合成图像类内相似度高需要大量增强；2）监督多样性不足，训练中监督信号有限导致高压缩率下性能下降。

Method: 提出LPQLD方法：1）通过类级批处理和批归一化监督增强图像多样性；2）通过动态知识重用的标签剪枝提升标签-增强多样性；3）通过校准师生对齐的标签量化提升增强-图像多样性。

Result: 在ImageNet-1K上减少软标签存储78倍，准确率提升7.2%；在ImageNet-21K上减少存储500倍，准确率提升2.8%。在不同网络架构和数据集蒸馏方法上均验证了优越性。

Conclusion: LPQLD有效解决了大规模数据集蒸馏中软标签存储过大的问题，通过提升图像和监督多样性，在显著减少存储需求的同时提高了模型性能，为高效数据集压缩提供了实用解决方案。
...
TL;DR: 提出首个针对低资源语言的3D PET/CT数据集VietPET-RoI，包含精细ROI标注，并开发HiRRA框架模仿放射科医生诊断流程，通过图关系模块提升报告生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前医学报告生成方法面临高维数据挑战和标注数据稀缺问题，特别是低资源语言。现有黑盒方法忽略临床工作流程中对局部感兴趣区域的分析，导致诊断结论不准确。

Method: 1) 创建VietPET-RoI数据集：600个PET/CT样本，1960个手动标注ROI，配对应临床报告；2) 提出HiRRA框架：使用图关系模块捕捉ROI属性间依赖关系，模仿放射科医生诊断流程；3) 引入新临床评估指标：ROI覆盖率和ROI质量指数，基于LLM提取评估定位准确性和属性描述保真度。

Result: HiRRA框架在BLEU和ROUGE-L指标上分别超越现有模型19.7%和4.7%，在临床指标上实现45.8%显著提升，表明临床可靠性增强且幻觉减少。

Conclusion: 通过引入首个低资源语言3D PET/CT数据集和模仿临床工作流程的HiRRA框架，显著提升了医学报告生成的准确性和临床相关性，为自动化医学报告生成提供了新方向。
...
TL;DR: 提出Attention-ResUNet架构，结合残差学习和多尺度注意力机制，在胎儿头部超声图像分割中达到99.30%的Dice分数，显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在超声图像分割中面临低对比度、噪声和复杂解剖边界等挑战，需要更鲁棒和准确的解决方案

Method: 提出Attention-ResUNet架构，在四个解码器层级集成注意力门机制以聚焦解剖相关区域，同时使用残差连接促进梯度流动和特征重用

Result: 在HC18 Challenge数据集上达到99.30%的平均Dice分数，显著优于ResUNet、Attention U-Net、Swin U-Net等基线方法，计算效率高（14.7M参数，45 GFLOPs）

Conclusion: Attention-ResUNet为胎儿头部超声图像分割建立了新的最先进性能，具有临床部署所需的增强可解释性和计算效率
...
TL;DR: AI驱动的高分辨率城市垃圾地图工作流，利用公开可用的航空和街景图像检测城市固体垃圾，应用于坦桑尼亚达累斯萨拉姆，识别与洪水风险相关的垃圾热点区域。


<details>
  <summary>Details</summary>
Motivation: 非洲快速扩张的城市面临日益严重的洪水灾害，不完善的垃圾管理堵塞排水系统并加剧洪水风险，需要更有效的监测方法来识别垃圾热点区域。

Method: 开发AI驱动的城市垃圾地图工作流，利用公开可用的航空和街景图像进行高分辨率垃圾检测；与当地合作伙伴合作确保文化背景相关的数据标注，反映实际垃圾再利用实践。

Result: 在达累斯萨拉姆发现垃圾空间模式与非正规住区和社会经济因素相关；水道中的垃圾积累量比相邻城市区域高出三倍，识别出气候加剧洪水的关键热点区域。

Conclusion: 这种可扩展的AI方法能够实现全市范围的监测和干预优先级排序，为城市洪水易发区的城市规划、气候适应和可持续垃圾管理提供可操作的见解。
...
TL;DR: 提出一种推理时方法，通过分析嵌入空间中的偏见结构并校正，在不改变模型权重、提示或数据集的情况下，可靠地减轻社会偏见，同时保持提示语义和视觉上下文。


<details>
  <summary>Details</summary>
Motivation: 现代文本到图像模型放大了有害的社会偏见，挑战了其伦理部署。需要一种方法在保持语义和视觉上下文的同时减轻偏见，并让从业者能够精细控制公平性与连贯性之间的权衡。

Method: 使用嵌入算术分析偏见在嵌入空间中的结构，并在不改变模型权重、提示或数据集的情况下进行校正。方法基于条件嵌入空间形成复杂纠缠流形而非解耦概念网格的观察，提出概念连贯性评分来评估语义保持。

Result: 在FLUX 1.0-Dev和Stable Diffusion 3.5-Large上的实验表明，这种轻量级、无需调优的方法在提高多样性的同时保持高概念连贯性方面显著优于现有基线，有效解决了关键的公平性-连贯性权衡问题。

Conclusion: 通过表征模型如何表示社会概念，建立对潜在空间的几何理解，为更透明、可控和公平的图像生成提供了一条原则性路径。
...
TL;DR: 本文提出了一种将MeanFlow从类别条件扩展到文本条件生成的方法，通过分析发现文本特征需要高区分度才能在少步生成中有效，并验证了基于LLM的文本编码器在MeanFlow框架中的适用性。


<details>
  <summary>Details</summary>
Motivation: 现有MeanFlow方法主要关注类别到图像生成，但文本条件能实现更丰富的内容创作。然而，将强大的LLM文本编码器集成到MeanFlow框架中面临性能不佳的挑战，需要探究根本原因。

Method: 通过详细分析发现，由于MeanFlow生成步骤极少（如仅一步），文本特征表示需要具备足够高的区分度。基于此洞察，采用已验证具有所需语义特性的LLM文本编码器，并将MeanFlow生成过程适配到该框架中。

Result: 首次实现了高效的文本条件MeanFlow合成，并在广泛使用的扩散模型上验证了该方法，展示了显著的生成性能提升。

Conclusion: 这项工作为未来文本条件MeanFlow生成研究提供了通用且实用的参考，揭示了文本特征区分度对少步生成的重要性，并展示了LLM文本编码器在该框架中的有效性。
...
TL;DR: 提出CanonSLR框架，通过正面视角引导的师生学习策略解决多视角连续手语识别问题，并构建了多视角数据集PT14-MV和CSL-MV。


<details>
  <summary>Details</summary>
Motivation: 现有连续手语识别方法主要在单视角下开发，对真实场景中的视角变化鲁棒性不足。需要解决多视角下的识别问题，特别是非正面视角带来的遮挡和投影变化等挑战。

Method: 1) 正面视角锚定的师生学习策略：用正面视角训练的教师网络为多视角学生网络提供规范时序监督；2) 序列级软目标蒸馏：将正面视角的结构化时序知识转移到非正面样本，缓解边界模糊和类别混淆；3) 时序运动关系增强：显式建模高层视觉特征中的运动感知时序关系；4) 开发通用多视角手语数据构建流程。

Result: 在构建的PT14-MV和CSL-MV两个七视角基准测试上，CanonSLR在多视角设置下持续优于现有方法，尤其在具有挑战性的非正面视角上表现出更强的鲁棒性。

Conclusion: CanonSLR通过正面视角引导的框架有效解决了多视角连续手语识别问题，提出的方法在多视角基准上表现出优越性能，为多视角手语识别研究提供了新的实验基础。
...
TL;DR: 提出一种结合扩散模型与分割模型的混合管道，用于遥感图像中的目标定位，相比现有方法在Acc@0.5指标上提升超过14%。


<details>
  <summary>Details</summary>
Motivation: 探索扩散模型在遥感图像目标定位中的潜力，利用生成模型与分割模型的互补优势，解决复杂场景下的目标定位问题。

Method: 提出混合管道，集成基于扩散的定位线索与最先进的分割模型（RemoteSAM和SAM3），结合生成扩散模型与基础分割模型的优势。

Result: 实验表明该管道显著提升定位性能，在Acc@0.5指标上相比现有最先进方法提升超过14%。

Conclusion: 扩散模型与分割模型的结合为遥感图像目标定位提供了有效解决方案，展示了生成模型在视觉任务中的扩展应用潜力。
...
TL;DR: 提出一个专注于几何精度的神经渲染评估流程和包含19个场景的基准测试，补充传统视觉质量指标


<details>
  <summary>Details</summary>
Motivation: 现有神经渲染评估主要关注生成图像的视觉质量，但忽略了表面几何的准确性，而几何精度对于机器人抓取和物体操作等任务至关重要

Method: 开发了一个专注于几何精度的神经渲染方法评估流程，并构建了一个包含19个多样化场景的基准测试集

Result: 提出的评估方法能够系统评估重建方法在表面和形状保真度方面的表现，补充了传统的视觉指标

Conclusion: 几何精度评估对于神经渲染方法在机器人等实际应用中的有效性至关重要，提出的评估流程和基准测试填补了现有评估方法的空白
...
TL;DR: 提出SARR表示法解决对称物体姿态估计中的方向模糊问题，通过修改三角函数结合物体对称度，实现连续唯一的姿态表示，在标准CNN上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 对称物体在日常生活和工业中普遍存在，但其固有的方向模糊性阻碍了深度学习网络在姿态估计中的训练。现有方法通常需要设计特定的损失函数和网络架构，或采用对称不变的评价指标，而本文关注旋转本身的数值表示。

Method: 提出SARR（Symmetry-Aware Rotation Representation）表示法，通过修改三角函数结合物体形状的对称度，获得对称物体的规范（对称解析）姿态。该方法仅需深度图或无纹理RGB/灰度图像作为输入，无需3D模型。

Result: 在T-LESS和ITODD两个6D姿态估计数据集上，使用标准CNN进行3D方向估计，在对称敏感的评价指标AR_C上超越现有方法，在传统对称不变指标上也表现良好。SARR训练的网络在多种旋转表示法中表现最佳。

Conclusion: SARR表示法能有效解决对称物体姿态估计中的方向模糊问题，提供连续唯一的姿态表示，使标准CNN能有效学习对称物体的姿态，且无需先验对称知识。代码和可视化工具已开源。
...
Method: 1) 解耦框架：分离记忆条件化与生成过程；2) 轻量级独立记忆分支学习历史观测的空间一致性；3) 混合记忆表示捕获时空线索；4) 逐帧交叉注意力机制确保每帧仅基于最相关的历史信息；5) 相机感知门控机制调节记忆与生成模块的交互。

Result: 相比现有方法，本方法数据效率高，在视觉质量和空间一致性方面达到最先进性能，同时显著降低训练成本。

Conclusion: 通过解耦记忆条件化与视频生成，提出的框架在保持新场景探索生成能力的同时，有效提升了长时视频生成的空间一致性，且具有更好的数据效率。
...
TL;DR: 提出S-EGIU框架，将指令理解建模为动态的"指令作为状态"变量，通过粗粒度段激活和细粒度语义精炼，实现指令语义随视觉上下文演化，提升视觉语言导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言导航模型通常将指令编码为静态全局表示，无法适应指令语义随智能体视野和空间上下文的变化。指令含义会随着智能体观察环境的变化而动态演变，需要建立指令与感知状态之间的动态纠缠关系。

Method: 提出State-Entangled Environment-Guided Instruction Understanding (S-EGIU)框架：1) 粗粒度：激活与当前观察语义对齐的指令段；2) 细粒度：通过观察引导的token grounding和上下文建模精炼激活段，在观察指导下锐化其内部语义。将指令理解建模为"指令作为状态"变量，随智能体感知状态逐步演化。

Result: 在多个VLN基准测试中表现优异：在REVERIE Test Unseen上获得+2.68% SPL提升，在其他VLN基准测试中也显示出持续的效率增益，验证了动态指令-感知纠缠的价值。

Conclusion: 将指令理解建模为动态演化的状态变量，通过S-EGIU框架实现指令语义与感知状态的动态纠缠，显著提升了视觉语言导航的性能和适应性，为解决指令与观察动态纠缠的核心挑战提供了有效方案。
...
TL;DR: SAM3在病理图像分割中表现有限：文本提示对核概念激活效果差，性能对视觉提示类型和数量敏感，缺乏对噪声的鲁棒性，与任务特定模型存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 数字病理分割在组织水平和细胞核水平面临高标注成本和泛化能力差的问题，SAM3的可提示概念分割提供了通过文本提示实现自动化的潜力，需要系统评估其在病理图像分割中的能力边界。

Method: 提出系统评估协议，在不同监督设置下评估SAM3：零样本、少样本和监督学习，使用不同提示策略，在NuInsSeg、PanNuke和GlaS等病理数据集上进行广泛评估。

Result: 1. 文本提示对核概念激活效果差；2. 性能高度依赖视觉提示类型和数量；3. 少样本学习有提升但缺乏对视觉提示噪声的鲁棒性；4. 基于提示的使用与任务训练的适配器模型存在显著差距。

Conclusion: SAM3在病理图像分割中存在明显局限性，研究界定了其能力边界，并提供了病理领域适应的必要性指导，表明需要专门的病理领域适配才能实现有效应用。
...
<details>
  <summary>Details</summary>
Motivation: 准确的预后和风险评估对临床决策至关重要。虽然放射科医生评估的CT特征能提供有价值的疾病严重程度和结果指标，但解读这些图像需要专业知识，且将丰富的视觉信息转化为文本摘要不可避免地会导致信息丢失。

Method: 提出一个视觉语言框架，通过视觉指令调优利用大规模开源CT图像和配对的放射学报告进行预训练，学习临床有意义的视觉-文本表示。然后在预训练模型基础上加入生存预测头，适应下游生存预测任务。

Result: 实验结果表明，该方法在生存预测方面优于基线方法，特别是在临床数据单独预测能力较弱的情况下表现更佳。同时能生成对预定义问题的临床有意义语言响应。

Conclusion: 提出的视觉语言框架通过结合CT图像和临床数据，不仅提升了生存预测性能，还能生成临床有意义的解释，为临床决策提供更全面的支持。
...
TL;DR: 提出三种基于风格迁移思想的神经网络架构，用于实时天气条件图像分类（晴天、雨、雪、雾），这些模型在多个公开数据集上超越现有方法并展现良好泛化能力。


<details>
  <summary>Details</summary>
Motivation: 从图像中实时准确分类天气条件（晴天、雨、雪、雾）具有重要应用价值，但现有方法可能无法充分捕捉图像中的风格化元素。受风格迁移最新进展启发，作者希望开发能够有效提取图像风格特征的模型来解决这一问题。

Method: 提出三种神经网络架构：1) Multi-PatchGAN：基于PatchGAN架构，但采用多种补丁尺寸以适应检测任务；2) Truncated ResNet50：保留ResNet50前9层的简化版本，通过进化算法确定截断位置以提取高频特征；3) Truncated ResNet50 with Gram Matrix and Attention：在训练时计算每层的Gram矩阵，并通过注意力机制自动加权，优化最相关风格特征的提取。

Result: 后两种模型（Truncated ResNet50及其Gram矩阵注意力版本）在多个公开数据库上超越了现有技术水平，并展现出显著的泛化能力。这些模型虽然针对天气检测开发，但也适用于其他基于外观的分类任务。

Conclusion: 提出的三种基于风格迁移思想的神经网络架构在天气条件分类任务上表现出色，特别是结合Gram矩阵和注意力机制的Truncated ResNet50模型效果最佳。这些架构具有广泛适用性，可扩展到动物物种识别、纹理分类、医学影像疾病检测和工业缺陷识别等其他外观分类任务。
...
TL;DR: 提出一种用于目标检测的混合专家模型，结合多个YOLO检测器，通过门控网络动态加权专家贡献，在BDD100K数据集上优于传统集成方法。


<details>
  <summary>Details</summary>
Motivation: 混合专家模型在图像分类和语义分割中表现良好，但在目标检测中应用有限，因为密集结构化预测的融合存在挑战。研究模型级目标检测器混合，旨在提升性能并增强可解释性。

Method: 提出MoE架构，结合在语义不相交数据子集上训练的YOLO检测器，使用学习的门控网络动态加权专家贡献。研究不同检测输出融合策略和门控训练方法，包括防止专家崩溃的平衡损失。

Result: 在BDD100K数据集上的实验表明，提出的MoE方法持续优于标准集成方法，并揭示了专家在不同领域的专业化，验证了模型级MoE作为传统目标检测集成的可行替代方案。

Conclusion: 模型级混合专家是目标检测中传统集成方法的有效替代方案，既能提升性能，又能提供专家专业化的见解，增强模型可解释性。
...
TL;DR: PRISM是一种组合式方法，使预训练文本到图像模型能够处理长序列输入，通过提取长提示的组成表示，对每个组件进行独立噪声预测，并使用基于能量的连接合并输出。


<details>
  <summary>Details</summary>
Motivation: 现代文本到图像模型在处理描述性段落输入时难以捕捉关键细节，因为训练数据主要由简洁标题构成。现有方法要么通过微调处理长提示但泛化能力差，要么将过长输入投影到正常提示空间而牺牲保真度。

Method: PRISM使用轻量级模块从长提示中提取组成表示，文本到图像模型对每个组件进行独立噪声预测，然后通过基于能量的连接将输出合并到单个去噪步骤中。

Result: PRISM在多种模型架构上表现与在相同训练数据上微调的模型相当，在超过500个标记的挑战性公共基准测试中，比基线模型性能高出7.4%，显示出优越的泛化能力。

Conclusion: PRISM提供了一种有效的组合式方法，使预训练文本到图像模型能够处理长序列输入，在保持保真度的同时实现了良好的泛化性能。
...
TL;DR: Geo3DPruner：一种几何引导的3D视觉token剪枝框架，通过建模跨帧相关性和两阶段剪枝（体素内和体素间），在剪除90%视觉token的同时保持超过90%的原始性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在2D视觉上表现出色，但扩展到3D场景理解时面临挑战。现有方法将3D场景表示为包含深度和相机位姿信息的空间视频，但大量视觉token成为推理效率和上下文管理的主要瓶颈。现有剪枝方法忽视了空间视频的视图一致性和剩余token的空间多样性，无法有效去除帧间冗余并保持场景完整性。

Method: 提出Geo3DPruner框架：1）通过几何感知全局注意力建模跨帧相关性；2）两阶段剪枝过程：体素内阶段选择每个体素内的代表性多视图特征，体素间阶段通过选择全局分布的体素子集来保持空间多样性。

Result: 在多个3D场景理解基准测试中，Geo3DPruner在剪除90%视觉token的同时，保持了超过90%的原始性能，显著优于现有的文本引导和视觉引导剪枝方法。
...
TL;DR: MARCO是一个基于DINOv2的统一语义对应模型，通过新颖的训练框架提升细粒度定位和语义泛化能力，在多个基准上达到SOTA，同时比扩散方法更小更快。


<details>
  <summary>Details</summary>
Motivation: 现有的双编码器语义对应模型（DINOv2+扩散主干）虽然准确，但参数量大（十亿级），对训练关键点之外的泛化能力差，存在基准性能与实际可用性之间的差距。

Method: 提出MARCO模型，采用新颖训练框架：1）粗到细目标函数提升空间精度；2）自蒸馏框架将稀疏标注扩展到未标注区域，将少量关键点转化为密集语义对应。

Result: 在SPair-71k、AP-10K和PF-PASCAL上达到新SOTA，细粒度定位阈值提升+8.9 PCK@0.01，未见关键点泛化提升+5.1（SPair-U），未见类别泛化提升+4.7（MP-100），比扩散方法小3倍、快10倍。

Conclusion: MARCO通过统一的训练框架有效解决了语义对应中泛化能力不足的问题，在保持高性能的同时显著提升了效率和实用性。
...
TL;DR: LiquidTAD：首个将并行化液态神经网络引入时序动作检测领域的高效框架，用ActionLiquid块替代自注意力层，实现线性复杂度并大幅减少参数量


<details>
  <summary>Details</summary>
Motivation: 当前基于Transformer的时序动作检测方法存在二次计算复杂度和大量参数冗余问题，限制了在资源受限环境中的部署

Method: 提出LiquidTAD框架，用并行化的ActionLiquid块替代自注意力层，采用封闭形式连续时间（CfC）公式化，实现线性复杂度，通过学习时间常数自适应调节时间敏感性

Result: 在THUMOS-14数据集上达到69.46%的平均mAP，仅需10.82M参数（比ActionFormer减少63%）；在ActivityNet-1.3和Ego4D基准测试中表现出最优的精度-效率权衡和对时间采样变化的鲁棒性

Conclusion: LiquidTAD通过引入并行化液态神经网络架构，显著提升了时序动作检测的计算效率和参数效率，同时保持了竞争性的检测精度，推动了现代TAD框架的帕累托前沿
...
<details>
  <summary>Details</summary>
Motivation: 传统视觉提示调优方法虽然有效，但学习的连续密集提示容易对输入噪声敏感，倾向于过拟合任务无关细节，需要在准确性和鲁棒性之间取得平衡。

Method: 设计基于脉冲神经元的信号过滤层，利用积分发放机制积累任务相关信号并过滤瞬时噪声波动；随后通过脉冲离散化单元将过滤后的信号转换为稀疏二进制提示，作为强正则化器。

Result: Spike-NVPT在鲁棒性方面表现优异，相比传统方法最大提升11.2%，同时在干净数据集上保持竞争力；二进制提示在部署时静态不变，推理时零额外计算开销。

Conclusion: 首次将脉冲神经元应用于传统ANN视觉模型的微调，通过稀疏二进制提示实现了准确性和鲁棒性的良好平衡，为参数高效视觉适应提供了新思路。
...
TL;DR: DFAlign是一个基于扩散去噪的开放词汇时序动作检测框架，通过生成前景知识来指导动作-视频对齐，缓解语义不平衡问题，在两个基准测试上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以解决简洁抽象的动作标签与丰富复杂视频内容之间的语义不平衡问题，这会导致语义噪声和误导性的跨模态对齐。

Method: 提出DFAlign框架，包含三个模块：1) 语义统一条件模块(SUC)统一动作共享和动作特定语义作为扩散去噪条件；2) 背景抑制去噪模块(BSD)通过去噪过程逐步去除视频背景冗余，生成前景知识；3) 前景提示对齐模块(FPA)将提取的前景知识作为提示令牌注入文本表示，引导模型关注动作相关片段。

Result: 在两个开放词汇时序动作检测基准测试上实现了最先进的性能。

Conclusion: DFAlign通过扩散去噪生成的前景知识作为有效的中间语义锚点，缓解了视频和文本表示之间的语义差距，增强了动作相关片段的区分性，实现了精确的跨模态对齐。
...
TL;DR: EVE提出了一种基于可执行视觉变换的多模态大语言模型自进化框架，通过挑战者-求解者双策略架构，绕过伪标签依赖，实现可验证的持续自我改进。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM自进化方法存在局限：基于伪标签的方法会因模型预测漂移导致质量逐渐下降，而基于模板的方法局限于静态变换集，无法在难度和多样性上自适应。需要一种既独立于模型内部确定性，又能持续多样化训练分布的鲁棒自进化机制。

Method: EVE采用挑战者-求解者双策略架构：挑战者维护并扩展视觉变换代码示例队列，合成Python脚本执行动态视觉变换，生成具有绝对执行验证真值答案的VQA问题。多维奖励系统整合语义多样性和动态难度校准，引导挑战者丰富代码队列并逐步提升任务难度，实现双策略的互惠协同进化。

Result: 大量实验表明，EVE在多个基准测试中持续超越现有自进化方法，建立了可验证的MLLM自进化鲁棒且可扩展的范式。

Conclusion: EVE通过可执行视觉变换完全绕过伪标签依赖，利用挑战者-求解者架构实现持续多样化和难度递增的自进化，为MLLM提供了可验证、鲁棒且可扩展的自进化新范式。
...
TL;DR: OmniHuman是一个大规模多场景数据集，用于细粒度人体建模，并配套OHBench评估系统，解决现有音频-视频联合生成模型在复杂真实场景中生成高保真人体视频的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有音频-视频联合生成模型在复杂真实物理场景中生成高保真人体视频仍面临重大挑战，根本原因在于现有数据集存在三个维度的结构缺陷：全局场景和相机多样性有限、交互建模稀疏（人-人和人-物）、个体属性对齐不足。

Method: 提出了OmniHuman数据集，提供分层标注（视频级场景、帧级交互、个体级属性），并开发了全自动高质量数据收集和多模态标注流程。同时建立了OmniHuman Benchmark（OHBench）三级评估系统，引入与人类感知高度一致的度量标准。

Result: 创建了OmniHuman大规模多场景数据集和OHBench评估系统，填补了现有基准测试的空白，为以人为中心的音频-视频合成提供了全面的诊断能力，涵盖全局场景、关系交互和个体属性三个维度。

Conclusion: OmniHuman数据集和OHBench评估系统共同解决了现有音频-视频联合生成模型在复杂场景中生成高保真人体视频的挑战，通过提供分层标注和与人类感知一致的评估指标，为细粒度人体建模和合成提供了重要基础设施。
...
TL;DR: AdaCluster是一种无需训练的自适应聚类框架，通过角度相似性保持的查询向量聚类和欧几里得相似性保持的键向量聚类，加速视频扩散Transformer的推理，实现1.67-4.31倍加速且质量损失可忽略。


<details>
  <summary>Details</summary>
Motivation: 视频扩散Transformer（DiTs）由于二次注意力复杂度导致推理延迟过高。现有稀疏注意力方法要么忽略了语义相似性，要么无法适应不同层间的异构令牌分布，导致模型性能下降。

Method: 提出AdaCluster训练自适应聚类框架：1）对查询向量采用角度相似性保持的聚类方法实现更高压缩；2）对键向量设计欧几里得相似性保持的聚类方法，包括聚类数量分配、阈值自适应聚类和高效关键聚类选择。

Result: 在CogVideoX-2B、HunyuanVideo和Wan-2.1模型上，在单个A40 GPU上实现了1.67-4.31倍的加速，质量下降可忽略不计。

Conclusion: AdaCluster能够有效加速视频扩散Transformer的生成过程，同时保持模型精度，解决了现有稀疏注意力方法在语义相似性和异构令牌分布适应方面的不足。
...
TL;DR: 提出分层面部模板反演方法LBFTI，通过三层分解（前景、中景、背景）和三级训练策略，从面部模板重建身份保持的细粒度人脸图像，在机器认证和人类感知相似度上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 面部模板在身份认证中广泛使用，符合数据最小化原则，但模板反演技术存在严重的隐私泄露风险，能够从模板重建人脸图像。现有方法在重建质量和身份保持方面存在不足。

Method: 提出分层面部模板反演方法LBFTI：1）将人脸分解为三层：前景层（眉毛、眼睛、鼻子、嘴巴）、中景层（皮肤）、背景层（其他部分）；2）使用专用生成器生成各层；3）采用三级训练策略：独立精炼生成前景和中景层、融合前景和中景层并二次注入模板生成完整全景人脸、联合微调所有模块优化层间协调和身份一致性。

Result: 实验表明LBFTI在机器认证性能上优于最先进方法，TAR提升25.3%，同时在人类感知相似度方面也表现更好，定量指标和问卷调查均验证了其优越性。

Conclusion: LBFTI方法有效解决了面部模板反演中的隐私泄露问题，通过分层分解和精细训练策略，实现了高质量的身份保持人脸重建，在机器和人类评估中均表现出色。
...
TL;DR: EAST框架通过随机化训练策略和联合学习观察与未来表示，显著提升了早期动作预测性能，在多个数据集上创下新SOTA。


<details>
  <summary>Details</summary>
Motivation: 早期动作预测在动作完全展开前进行预测，由于视觉证据有限而极具挑战性。现有方法在处理不同观察比例时泛化能力不足，且训练效率有待提升。

Method: 提出EAST框架：1) 随机化训练策略，采样时间步分隔观察和未观察帧，使单个模型能泛化到所有测试观察比例；2) 联合学习观察和未来（oracle）表示；3) 令牌掩码程序减少一半内存使用并加速训练2倍；4) 结合预测解码器。

Result: 在NTU60、SSv2和UCF101数据集上分别超越先前最佳工作10.1、7.7和3.9个百分点，创下新SOTA。仅编码器模型也能表现优异，训练效率显著提升。

Conclusion: EAST通过创新的随机化训练策略和高效的训练方法，有效解决了早期动作预测中的泛化问题和计算效率问题，为不完全观察下的动作预测提供了强大框架。
...
TL;DR: 提出DSA-CycleGAN解决组织病理学染色变化导致的噪声问题，提升多染色肾小球分割性能


<details>
  <summary>Details</summary>
Motivation: 数字组织病理学分割中，染色差异会降低模型性能，但为每种染色标注数据昂贵耗时。现有CycleGAN方法在染色转换时因一对多映射特性会引入噪声，与循环一致性损失冲突。

Method: 提出Domain Shift Aware CycleGAN (DSA-CycleGAN)，减少染色转换过程中的噪声。同时评估了机器学习领域解决类似问题的其他先进方法，并与DSA-CycleGAN在肾小球分割任务上进行比较。

Result: 实验表明DSA-CycleGAN不仅提高了肾小球分割性能，在减少噪声方面也优于其他方法，特别是在生物学上差异较大的染色之间转换时效果更明显。

Conclusion: DSA-CycleGAN能有效解决染色转换中的噪声问题，提升多染色分割模型的性能，代码已开源。
...
TL;DR: 提出基于大语言模型的多视角语义补偿框架MVR，通过语义重述和特征补偿解决文本到图像人物检索中的表达漂移问题，无需训练即可提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 文本到图像人物检索任务中，自然语言表达的多样性和视觉语义的隐含性导致"表达漂移"问题，即语义等价的文本因表达方式不同在嵌入空间中产生显著特征差异，降低了图像-文本对齐的鲁棒性。

Method: 基于大语言模型的语义补偿框架MVR，包含三个核心组件：1) 多视角重述：双分支提示策略结合关键特征引导和多样性感知重写，生成语义等价但分布多样的文本变体；2) 文本特征鲁棒性增强：无训练潜在空间补偿机制，通过多视角特征均值池化和残差连接抑制噪声干扰，捕获"语义回声"；3) 视觉语义补偿：VLM生成多视角图像描述，通过共享文本重述增强以解决视觉语义鸿沟。

Result: 实验表明，该方法无需训练即可显著提升原始模型的准确率，在三个文本到图像人物检索数据集上达到最先进的性能。

Conclusion: 提出的MVR框架通过多视角语义重述和特征补偿，有效解决了文本到图像人物检索中的表达漂移问题，提高了跨模态表示的一致性，且无需额外训练即可实现性能提升。
...
TL;DR: 提出OSD-IRF方法，通过单步扩散和逆残差场实现快速工业异常检测，在保持SOTA性能的同时实现2倍推理加速


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在工业异常检测中性能优秀但推理速度慢，需要迭代去噪过程。需要解决推理效率问题同时保持检测性能

Method: 首先在正常数据上训练DDPM模型，然后基于训练好的噪声函数预测测试样本的逆残差场（IRF），最后通过评估IRF在高斯分布下的概率密度进行异常检测。关键发现是异常在IRF空间中变得可区分

Result: 在三个广泛使用的uIAD基准测试中，模型在六个指标上达到SOTA或竞争性性能，同时实现约2倍的推理加速（无需蒸馏）

Conclusion: OSD-IRF通过单步扩散和逆残差场解决了扩散模型推理慢的问题，在保持检测性能的同时显著提升效率，为工业异常检测提供了新的有效方法
...
TL;DR: MedProbeBench：首个基于高质量临床指南的深度研究基准，评估LLM在医学证据整合和指南生成方面的专家级能力


<details>
  <summary>Details</summary>
Motivation: 现有基准无法评估深度研究系统在真实医学工作流中的多步证据整合和专家级判断能力，而临床指南开发正需要这种深度证据整合能力

Method: 利用高质量临床指南作为专家级参考，提出MedProbe-Eval评估框架，包含：1）1200+任务自适应评分标准的整体评分体系；2）基于5130+原子声明的细粒度证据验证

Result: 评估17个LLM和深度研究代理显示，当前系统在证据整合和指南生成方面存在显著差距，远未达到专家级临床指南开发水平

Conclusion: MedProbeBench填补了医学深度研究评估的空白，揭示了当前AI系统与专家级临床指南开发能力之间的巨大差距，为未来研究提供了重要基准
...
TL;DR: 该研究比较了Qwen3-VL和Qwen3.5两种视觉语言模型在遥感图像变化视觉问答任务上的表现，发现原生多模态模型比结构化视觉语言管道更有效，且性能不随模型规模单调增长。


<details>
  <summary>Details</summary>
Motivation: 变化视觉问答（Change VQA）旨在回答关于双时相遥感图像语义变化的自然语言问题。尽管视觉语言模型在时序遥感图像理解方面已有研究，但在现代多模态模型背景下，Change VQA仍未被充分探索。

Method: 在统一的低秩适应（LoRA）设置下，使用CDVQA基准测试比较两种Qwen模型：Qwen3-VL（采用结构化视觉语言管道，具有多深度视觉条件和全注意力解码器）和Qwen3.5（原生多模态模型，结合单阶段对齐和混合解码器骨干）。

Result: 实验结果显示，最近的视觉语言模型相比早期专门基线有所改进。性能不随模型规模单调增长，且原生多模态模型比结构化视觉语言管道在此任务上更有效。

Conclusion: 紧密集成的多模态骨干对遥感图像中语言驱动的语义变化推理性能的贡献，比模型规模或显式的多深度视觉条件更重要。
...
TL;DR: ESsEN：一种轻量级视觉语言模型，在低资源环境下通过双塔编码器架构和卷积网络集成实现参数高效，性能接近大型模型但参数量大幅减少。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型通常参数量巨大（数十亿），不适用于边缘设备或独立机器人平台等资源受限场景。现有研究缺乏针对轻量级模型和小数据集训练的方法，需要开发更易访问的视觉语言建模方案。

Method: 1. 采用双塔编码器架构而非单塔编码器，在低资源英语判别任务中表现更优；2. 将传统卷积网络集成到双塔Transformer架构中，提高参数效率；3. 设计可变形状和大小的跨模态融合模块；4. 提出ESsEN模型，支持端到端训练且资源需求较低。

Result: 1. 双塔编码器在低资源设置下优于单塔编码器；2. 集成卷积网络能有效构建参数高效的视觉语言模型；3. 跨模态融合模块的形状和大小可灵活调整而不影响性能；4. ESsEN模型在多个任务上表现与其他模型相当，但参数量仅为其他模型的一小部分。

Conclusion: 通过系统方法开发轻量级视觉语言模型是可行的，ESsEN模型及相关工具使视觉语言建模对更广泛的研究人员更加可访问，特别是在资源受限的环境中。
...
TL;DR: 提出一个用于在线视频理解的框架，通过解耦推理控制与记忆整合，实现证据对齐的透明响应。


<details>
  <summary>Details</summary>
Motivation: 传统视频LLM在离线评估中忽视了在线流式场景下的关键需求：需要在视频流中首次出现足够证据时精确响应。在线流式范式带来决策不透明、响应时间与视觉证据对齐困难、以及有限计算预算下保持全局因果一致理解等挑战。

Method: 提出一个解耦推理控制与记忆整合的框架。具体实现包括：1) 主动思考决策器(ATDM)：使用可观察的进度和置信度指标外部化决策过程，精确匹配首次足够证据时间戳；2) 分层渐进语义整合(HPSI)：使用可学习的多级聚合token跨片段传播，构建丰富的全局认知状态而不超出token预算。

Result: 在StreamingBench基准上将先前最佳准确率从67.63%提升至71.60%，在OVOBench上达到46.9%，为证据对齐和透明的在线视频分析提供了稳健解决方案。

Conclusion: 该框架通过解耦推理控制与记忆整合，解决了在线视频理解中的关键挑战，实现了证据对齐的透明响应，为在线流式视频分析设立了新标准。
...
TL;DR: Asset Harvester：一个端到端系统，将自动驾驶日志中的稀疏物体观测转换为完整的3D资产，用于闭环仿真


<details>
  <summary>Details</summary>
Motivation: 神经场景重建可以将驾驶日志转换为交互式3D环境，但无法生成完整的3D物体资产，这些资产对于智能体操作和大视角新视图合成至关重要

Method: 开发了一个系统级设计，结合大规模物体中心训练元组筛选、异构传感器几何感知预处理，以及将稀疏视图条件多视图生成与3D高斯提升耦合的鲁棒训练方案。其中SparseViewDiT专门处理有限角度视图等现实数据挑战

Result: 实现了从稀疏自动驾驶物体观测到可重用3D资产的可扩展转换

Conclusion: Asset Harvester通过系统级设计解决了从现实世界自动驾驶数据生成完整3D资产的挑战，为自动驾驶闭环仿真提供了关键工具
...
TL;DR: SemLT3D：一种语义引导的专家蒸馏框架，通过语义先验增强长尾类别在相机3D目标检测中的表征能力，解决现实数据集中严重的长尾不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有相机3D目标检测方法主要关注整体性能，忽视了现实数据集中严重的长尾不平衡问题。许多安全关键但罕见的类别（如儿童、婴儿车、应急车辆）在数据中代表性不足，导致学习偏差和性能下降。此外，类间模糊性（视觉相似的子类）和类内多样性（外观、尺度、姿态、上下文变化大）进一步加剧了长尾识别的挑战。

Method: SemLT3D包含两个核心组件：1）语言引导的专家混合模块，根据语义亲和性将3D查询路由到专门的专家网络，以更好地区分混淆类别并专注于尾部分布；2）语义投影蒸馏管道，将3D查询与CLIP引导的2D语义对齐，生成跨不同视觉表现的一致且可区分的特征。

Result: 虽然主要针对长尾不平衡问题，但SemLT3D的语义结构化学习也提高了在更广泛外观变化和挑战性极端情况下的鲁棒性，为更可靠的相机3D感知提供了原则性方法。

Conclusion: SemLT3D通过语义引导的专家蒸馏框架，有效解决了相机3D目标检测中的长尾不平衡问题，不仅提升了罕见类别的检测性能，还增强了模型对视觉变化和极端情况的鲁棒性，为实现更可靠的自动驾驶感知系统迈出了重要一步。
...
TL;DR: XEmbodied是一个云侧基础模型，通过集成3D几何表示和物理信号，赋予VLA模型内在的3D几何感知能力，显著提升空间推理、交通语义和具身交互性能。


<details>
  <summary>Details</summary>
Motivation: 当前云侧VLA模型训练依赖缺乏几何推理和领域语义的2D图像-文本预训练模型，导致在复杂环境中的标注质量受限，无法满足下一代自主系统的需求。

Method: 提出XEmbodied模型：1）通过结构化3D适配器集成几何表示；2）使用高效图像-具身适配器将物理信号蒸馏为上下文token；3）采用渐进式领域课程和强化学习后训练。

Result: 在18个公共基准测试中表现优异，显著提升空间推理、交通语义、具身交互能力以及分布外泛化性能，适用于大规模场景挖掘和具身VQA任务。

Conclusion: XEmbodied通过赋予VLA模型内在的3D几何感知能力，解决了现有云侧模型在复杂环境中标注质量不足的问题，为下一代自主系统提供了更强大的基础模型。
...
TL;DR: OneVL是一个统一的视觉语言架构与世界模型框架，通过紧凑的潜在令牌进行推理和规划，在保持答案级别延迟的同时超越显式思维链方法。


<details>
  <summary>Details</summary>
Motivation: 现有的思维链推理在自动驾驶中延迟过高，而潜在思维链方法由于仅压缩语言符号而非因果动态，性能不及显式方法。需要同时考虑语言和世界动态的压缩表示。

Method: 提出OneVL框架，通过双辅助解码器监督紧凑潜在令牌：语言解码器重建文本思维链，视觉世界模型解码器预测未来帧令牌。采用三阶段训练管道逐步对齐轨迹、语言和视觉目标。

Result: 在四个基准测试中，OneVL成为首个超越显式思维链的潜在思维链方法，在答案级别延迟下实现最先进精度，证明语言和世界模型监督下的压缩表示比逐令牌推理更具泛化性。

Conclusion: 通过语言和世界模型双重监督的紧凑潜在表示，能够更好地捕捉驾驶因果动态，在保持实时性能的同时超越显式推理，为自动驾驶推理提供了高效且准确的解决方案。
...
TL;DR: 本文提出了一种简单到困难（S2H）学习框架，通过构建三个层次的多图像偏好数据来增强视觉语言模型的多图像推理能力，包括单图像局部推理、多图像局部比较和全局视觉搜索。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在单图像理解方面表现出色，但在多图像推理方面存在能力差距。当前方法主要关注使用预定义图像索引的局部推理（如"查看图像3..."），而忽略了全局视觉搜索和自主跨图像比较的关键能力。

Method: 提出简单到困难（S2H）学习框架，系统构建三个层次的多图像偏好数据：1）单图像局部推理；2）多图像局部比较；3）全局视觉搜索。该方法利用提示驱动的复杂性创建适用于不同模型的偏好对，而非依赖模型特定属性。

Result: 在LLaVA和Qwen-VL模型上的广泛评估表明，该方法显著提升了多图像推理性能，在多个基准测试中优于基线方法。同时保持了强大的单图像推理能力，实现了多图像理解能力的同步增强。

Conclusion: S2H框架通过层次化多图像偏好数据构建，有效解决了现有多图像对齐方法的局限性，在保持单图像性能的同时显著提升了多图像推理能力，推动了视觉偏好对齐技术的发展。
...
TL;DR: 提出了首个将均匀离散扩散模型（UDM）与强化学习（RL）结合的框架，解决了直接应用GRPO到UDM时的不稳定问题，显著提升了文本到图像生成任务的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然均匀离散扩散模型在离散生成建模中表现出潜力，但其与强化学习的结合尚未充分探索。直接应用GRPO到UDM会导致训练不稳定和性能提升有限，需要专门的方法来解决这些问题。

Method: 提出了基于两个关键洞见的框架：1）将最终干净样本作为动作提供更准确稳定的优化信号；2）通过扩散前向过程重构轨迹，使概率路径与预训练分布更好对齐。还引入了Reduced-Step和CFG-Free策略来提升训练效率。

Result: 在多个文本到图像任务上显著提升基础模型性能：GenEval准确率从69%提升到96%，PickScore从20.46提升到23.81，在连续和离散设置中都达到了最先进水平。在OCR基准测试中，准确率从8%提升到57%，验证了方法的泛化能力。

Conclusion: 成功开发了首个将UDM与RL集成的框架，通过创新的优化策略解决了训练不稳定问题，在多个基准测试中取得了显著性能提升，证明了该方法在离散生成建模与强化学习结合方面的有效性。
...
TL;DR: MetaCloak-JPEG是一种改进的对抗性人脸保护系统，通过在训练中嵌入可微分的JPEG压缩层，解决了现有方法在社交媒体JPEG压缩后保护信号丢失的问题。


<details>
  <summary>Details</summary>
Motivation: 现有对抗性人脸保护系统（如PhotoGuard、Anti-DreamBooth、MetaCloak）存在结构性缺陷：它们没有考虑社交媒体平台在用户图像上传时应用的JPEG压缩。由于JPEG量化使用round()函数，其导数几乎处处为零，导致对抗性能量集中在高频DCT波段，而这些波段在JPEG压缩时被丢弃，造成60-80%的保护信号损失。

Method: 引入MetaCloak-JPEG，采用基于直通估计器（STE）的可微分JPEG（DiffJPEG）层：前向传播应用标准JPEG压缩，反向传播将round()替换为恒等函数。将DiffJPEG嵌入到JPEG感知的EOT分布中（约70%的数据增强包含DiffJPEG），并在双层元学习循环中使用课程质量因子调度（QF：95到50）。

Result: 在l-inf扰动预算eps=8/255下，MetaCloak-JPEG达到32.7 dB PSNR，91.3%的JPEG存活率，在评估的9个JPEG质量因子中全面优于PhotoGuard（9/9获胜，平均去噪损失增益+0.125），训练内存预算为4.1 GB。

Conclusion: MetaCloak-JPEG通过引入可微分JPEG压缩层，有效解决了现有对抗性人脸保护系统在社交媒体JPEG压缩环境下的保护信号丢失问题，显著提升了保护效果。
...
TL;DR: EVT是RMT的改进版本，通过欧氏距离衰减和空间独立分组机制，在保持空间先验的同时降低计算复杂度，在多个视觉任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: Vision Transformer的自注意力机制缺乏显式空间先验且具有二次计算复杂度，限制了其应用。RMT虽然引入了空间先验，但仍有改进空间。

Method: EVT采用欧氏距离衰减来增强空间信息建模，比RMT的曼哈顿距离更合理；放弃分解注意力机制，采用空间独立分组方法，提供更灵活的分组控制。

Result: 在图像分类、目标检测、实例分割和语义分割等任务上表现优异，无需额外训练数据在ImageNet-1k上达到86.6% top1准确率。

Conclusion: EVT通过改进的空间先验机制和更灵活的分组策略，克服了RMT的局限性，为Vision Transformer提供了更先进和适应性强的空间建模方法。
...
TL;DR: SynAgent框架通过单智能体到多智能体的技能迁移，实现可扩展、物理合理的协作人形操作，在协作模仿和轨迹控制方面显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 协作人形操作面临数据稀缺、多智能体协调复杂、跨物体泛化有限等挑战，需要一种可扩展且物理合理的解决方案。

Method: 1) 基于Delaunay四面体化的交互保持重定向方法，构建交互网格保持语义完整性；2) 单智能体预训练和适应范式，通过去中心化训练和多智能体PPO从单人类数据引导协作行为；3) 轨迹条件生成策略，使用条件VAE通过多教师蒸馏实现稳定可控的物体级轨迹执行。

Result: SynAgent在协作模仿和轨迹条件控制方面显著优于现有基线，并能泛化到不同的物体几何形状。

Conclusion: SynAgent通过单智能体到多智能体的协同作用，实现了可扩展、物理合理的协作操作，为解决数据稀缺和协调复杂性提供了有效框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖单个分割token，将语义推理和空间定位压缩到单个嵌入中，限制了模型明确解耦"分割什么"和"在哪里分割"的能力。

Method: 构建有序的查询库序列：潜在推理token捕获中间语义状态，分割锚点token提供显式空间定位。将空间条件建模为图像token上的因子化分布，并提出Token-Mask循环一致性(TMCC)双向训练目标。

Result: 在ReasonSeg测试集上达到最先进结果：67.7% gIoU和68.1% cIoU。

Conclusion: 通过结构化语言锚定查询库显式解耦空间定位和语义推理，AnchorSeg实现了更好的推理分割性能。
...
TL;DR: MultiWorld是一个多智能体多视角世界建模框架，通过多智能体条件模块实现精确控制，全局状态编码器保证多视角一致性，支持灵活扩展智能体和视角数量。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型大多局限于单智能体场景，无法捕捉真实世界多智能体系统中的复杂交互，需要开发能够处理多智能体多视角一致性的统一框架。
...
Conclusion: MultiWorld为多智能体多视角世界建模提供了一个有效的统一框架，能够准确控制多个智能体同时保持多视角一致性，支持灵活扩展。
...
TL;DR: 本文质疑"柏拉图表征假说"，发现跨模态对齐的证据脆弱，随数据规模扩大而减弱，且主要反映粗粒度语义而非细粒度结构一致性。


<details>
  <summary>Details</summary>
Motivation: 柏拉图表征假说认为不同模态训练的神经网络会收敛到相同的现实表征，这暗示模态选择可能不重要。本文旨在检验该假说的实验证据是否可靠。

Method: 通过分析现有评估方法，发现其使用小数据集（约1K样本）的互最近邻度量，并扩展到百万级样本验证对齐稳定性。同时考察一对一图像-标题设置与真实多对多设置的差异。

Result: 1) 对齐在小数据集上存在，但随数据规模扩大显著减弱；2) 剩余对齐主要反映粗粒度语义重叠而非细粒度结构一致性；3) 一对一设置高估对齐，多对多设置中进一步降低；4) 更强的语言模型与视觉对齐的趋势在新模型中未出现。
...
TL;DR: T-REN提出了一种高效的文本对齐区域编码器网络，通过轻量级网络将视觉特征池化为紧凑的区域级表示，显著提升跨模态理解能力并大幅减少token数量。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言编码器存在两个核心限制：1) 语言与密集视觉特征对齐较弱，影响开放词汇语义分割等任务；2) 细粒度视觉表示token数量过多，限制了在长视频中的可扩展性。

Method: 在冻结的视觉骨干网络之上添加轻量级网络，将每个语义区域内的patch级表示池化为区域token，并与区域级文本标注对齐。仅增加3.7%的参数。

Result: 在多个任务上取得显著提升：ADE20K开放词汇分割+5.9 mIoU，COCO对象级图文检索+18.4%召回率，Ego4D视频对象定位+15.6%召回率，VSPW视频场景解析+17.6% mIoU。同时token数量减少24倍（图像）和187倍（视频）。

Conclusion: T-REN通过紧凑的文本对齐区域表示，有效解决了视觉语言编码器的对齐和可扩展性问题，在保持高效性的同时显著提升了跨模态理解性能。
...
TL;DR: ReCap：轻量级故事可视化一致性框架，通过条件帧引用和语义漂移校正提升角色一致性，仅增加14.9万参数，在FlintstonesSV和PororoSV基准上超越SOTA


<details>
  <summary>Details</summary>
Motivation: 传统故事可视化方法依赖显式记忆库、架构扩展或辅助语言模型来保持跨帧一致性，导致参数大幅增加和推理开销。需要一种轻量级方法在不修改基础扩散模型的情况下提升角色稳定性和视觉保真度。

Method: 1. CORE模块：将代词作为视觉锚点，仅在角色被代词引用时激活，以前一帧为条件传播视觉身份，避免无条件跨帧条件化；2. SemDrift：训练时应用引导语义漂移校正，当文本模糊或指代不清时，将去噪器表示与预训练DINOv3视觉嵌入对齐，强制语义身份稳定性，零推理成本。

Result: 在FlintstonesSV上Character-Accuracy提升2.63%，在PororoSV上提升5.65%，在两个主要故事可视化基准上建立新的SOTA角色一致性。仅增加14.9万参数，远少于记忆库和LLM增强方法。成功扩展到真实电影的人类中心叙事。

Conclusion: ReCap通过选择性条件帧引用和训练时语义漂移校正，实现了高效的角色一致性保持，无需修改基础扩散模型架构，参数开销极小，在卡通和真实电影领域都表现出色。
...
TL;DR: 提出Wavelet-guided Multi-level Spatial Factorized Blendshapes表示法和蒸馏流程，将高质量教师模型的动态细节压缩到轻量级表示中，实现2000倍计算成本降低和10倍模型大小缩减，同时在移动设备上保持高质量渲染。


<details>
  <summary>Details</summary>
Motivation: 现有可动画化数字人建模方法存在两极化问题：要么需要服务器级GPU的高保真模型，要么是轻量级但动态细节有限、存在明显伪影的模型。需要同时实现高保真度和低计算复杂度的解决方案。

Method: 提出Wavelet-guided Multi-level Spatial Factorized Blendshapes表示法，结合多级小波谱分解和纹理空间低秩结构分解，并通过蒸馏流程从预训练的高质量教师模型转移运动感知的服装动态和细粒度外观细节。

Result: 相比原始高质量教师模型，计算成本降低2000倍，模型大小缩小10倍，在桌面PC上达到180+ FPS，在Meta Quest 3上实现24 FPS的实时设备端性能，渲染质量与服务器级方法相当或更优。

Conclusion: 该方法成功解决了高保真度与计算效率之间的权衡问题，显著提升了高保真数字人在沉浸式应用中的实用性，为移动设备上的高质量数字人渲染提供了可行解决方案。
