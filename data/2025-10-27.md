...
Motivation: 现有的适配器训练方法容易将目标属性（如身份）与图像中的偶然因素纠缠在一起，这种虚假相关性限制了模型的泛化能力并阻碍了文本提示的遵循。

Method: 在适配器训练过程中，通过ControlNet或LoRA等辅助模块为干扰因素建立快捷路径，消除适配器学习这些因素的动力，然后在推理时移除这些辅助模块。

Result: 在面部和全身身份注入等任务中，该方法显著提高了生成质量、多样性和提示遵循能力。

Conclusion: 当寻求解耦表示时，最有效的路径可能是为不应学习的内容建立快捷路径，这为大型模型时代提供了一个通用的设计原则。
...
Method: VAP将问题重新定义为上下文生成，利用参考视频作为直接语义提示，通过插拔式混合Transformer专家架构指导冻结的Video Diffusion Transformer，采用时间偏置位置嵌入消除虚假映射先验。

Result: VAP在开源方法中达到新的最先进水平，获得38.7%的用户偏好率，与领先的条件特定商业模型相当，并展示了强大的零样本泛化能力和对各种下游应用的支持。

Conclusion: VAP在实现通用、可控视频生成方面取得了显著进展，其统一模型架构和大型数据集VAP-Data为未来研究奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割对临床诊断和治疗至关重要，但传统CNN难以捕捉全局上下文信息和长距离依赖，影响对复杂边界和不同大小结构的精确分割。

Method: 结合卷积和Transformer组件，采用焦点调制注意力机制增强上下文感知，引入双向特征融合模块实现编码器-解码器跨尺度高效交互。

Result: 在8个公开数据集上的实验表明，FM-BFF-Net在Jaccard指数和Dice系数上持续超越现有最优方法。

Conclusion: FM-BFF-Net通过融合CNN和Transformer的优势，有效提升了医学图像分割的性能和适应性。
...
<details>
  <summary>Details</summary>
Motivation: 当前判别式模型在处理视觉遮挡和外观变化带来的不确定性时，只能回归到均值或众数，无法捕捉轨迹的多模态特性。

Method: 提出生成式点跟踪器(GenPT)，采用流匹配训练框架，结合判别式跟踪器的迭代细化、窗口相关先验和点坐标特定的方差调度，在推理时使用最佳优先搜索策略。

Result: 在PointOdyssey、Dynamic Replica和TAP-Vid基准测试中表现出色，特别是在遮挡点跟踪上达到最先进水平，同时保持可见点的竞争性性能。

Conclusion: GenPT能够有效捕捉点轨迹的多模态特性，显著提升遮挡点跟踪精度，为不确定性条件下的点跟踪提供了生成式解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在3D医学图像中难以准确定位解剖区域并进行逐步推理，这与临床诊断工作流程不匹配，限制了临床医生与AI的信任合作。

Method: 创建了3DReasonKnee数据集，包含7,970个3D膝关节MRI体积，每个样本包括3D MRI体积、诊断问题、3D边界框定位、临床医生生成的诊断推理步骤和结构化严重程度评估。

Result: 建立了ReasonKnee-Bench评估基准，对五种最先进的视觉语言模型进行了基准测试，为3D医学图像接地推理提供了基线性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D重建方法受光照和材质特性限制，无法有效处理透明、半透明等特殊材质物体。热偏振成像能够避免可见光偏振分析中的模糊性问题，为这些挑战性场景提供解决方案。

Method: 建立了偏振观测的一般理论，证明长波红外偏振成像不受可见光偏振模糊性影响。提出了基于多视角热偏振图像的3D形状恢复方法。

Result: 实验结果表明，该方法能够有效重建透明、半透明和异质物体的精细细节，性能优于现有技术。

Conclusion: 热偏振成像为3D形状重建提供了一种独立于光照和材质特性的有效方法，特别适用于传统方法难以处理的特殊材质物体。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉基础模型在分布偏移和标注稀缺领域表现不佳的问题，传统监督微调不可行，而现有的自监督学习策略对视觉编码器模型效果有限。

Method: 基于自蒸馏范式，利用多视角对象中心视频进行自监督微调，精心调整预测头并部署参数高效适应技术，防止模型遗忘预训练知识。

Result: 在3个视觉基础模型和2个数据集上的实验表明，VESSA在下游分类任务中相比基础模型和先前适应方法取得一致改进。

Conclusion: VESSA能够有效利用无标注的多视角对象视频进行领域适应，学习对多样化捕获条件的鲁棒性，提升视觉基础模型在目标领域的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有6D姿态估计流程在工业环境中的复杂条件下（如杂乱、光照差、复杂背景）性能下降，检测成为关键瓶颈，需要解决这些问题。
...
Conclusion: 该方法在工业环境中具有有效性和实用性，能够显著提升6D姿态估计的检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 地震后及时评估结构完整性对公共安全和应急响应至关重要。钢筋暴露是结构损伤的重要指标，但传统人工检测效率低且危险。

Method: 使用YOLOv11模型检测裂缝、剥落和钢筋暴露，并通过另一个YOLO模型区分不同损伤等级。结合数据增强、微调等技术构建混合框架。

Result: 开发了自动化分类框架，能够可靠地从输入图像中自动确定损伤等级，在土耳其2023年地震数据集上验证有效。

Conclusion: 研究表明，通过图像数据收集、标注和深度学习方法，可以在各种损伤情境下实现快速自动化的损伤检测。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D场景图生成方法局限于单视图、不支持增量更新、缺乏3D空间几何定位，这些在具身场景中至关重要。

Method: 利用VLM推理生成丰富的2D场景图，通过深度信息将其定位到3D空间，节点包含开放词汇对象特征、3D位置和语义上下文，边捕捉空间和语义关系及物体间距离。

Result: 在Replica和HM3D数据集上的实验表明，ZING-3D能够有效捕获空间和关系知识，无需任务特定训练。

Conclusion: ZING-3D框架成功实现了零样本的3D场景图生成，支持增量更新和几何定位，为下游机器人应用提供了有效的场景理解能力。
...
Method: 提出WaveSeg解码器：1）从输入图像学习高频分量作为显式先验；2）应用双域操作（DDO）进行多尺度融合；3）提出频谱分解注意力（SDA）块，利用Mamba的线性复杂度长程建模增强高频结构细节；4）应用重参数化卷积在小波域保持低频语义完整性；5）残差引导融合整合多尺度特征。

Result: 在标准基准测试上的广泛实验表明，WaveSeg在定量和定性上都持续优于最先进的方法，实现了高效精确的分割。

Conclusion: WaveSeg通过结合小波域频率先验和基于Mamba的注意力机制，在语义分割任务中取得了优异的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 传统深度学习模型在肠肌层神经丛分类任务中表现良好但缺乏可解释性，且不符合医生的决策方式，需要结合临床专业知识来提高模型的临床相关性。

Method: 使用基于对比语言-图像预训练的视觉语言模型，整合从医学教科书和论文中提取的专家文本概念，通过QuiltNet编码提示来对齐临床相关语义线索与视觉特征。

Result: 提出的模型在准确率(83.9%)、精确度(86.6%)和特异性(87.6%)方面优于VGG-19、ResNet-18和ResNet-50等CNN模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有的Retinex低光照图像增强方法在处理大尺寸图像时耗时较长，需要一种更快速有效的增强方法。

Method: 将Retinex模型从空间域扩展到直方图域，定义直方图位置矩阵和计数矩阵，构建两级优化模型，通过求解优化模型得到光照和反射率直方图的迭代公式，最后通过直方图匹配实现图像增强。

Result: HistRetinex在可见性和性能指标上优于现有增强方法，处理1000*664分辨率图像仅需1.86秒，比现有方法至少节省6.67秒。

Conclusion: HistRetinex是一种快速有效的低光照图像增强方法，在保持增强质量的同时显著提升了处理速度。
...
Method: 引入CLEVR-AVR仿真基准和AVR-152k大规模数据集，包含丰富的思维链标注。开发了PhysVLM-AVR模型，在更高阶马尔可夫决策过程中训练智能体进行不确定性识别、信息增益预测和行动选择。

Result: PhysVLM-AVR在CLEVR-AVR、具身推理(OpenEQA, RoboVQA)和被动视觉推理(GeoMath, Geometry30K)任务上达到最先进性能。分析表明当前具身MLLMs虽能检测信息不完整性，但在主动获取和整合信息方面存在困难。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法对垂直偏差、地面坡度敏感，缺乏对象身份信息，内存需求大，无法处理分裂合并情况，且忽略不确定性。

Method: 多分辨率NDT和点面ICP对齐，高度归一化，基于配准协方差和表面粗糙度的检测水平计算，几何代理引导跨时期关联，语义实例分割和类别约束二分图分配处理分裂合并。

Result: 在15个Subiaco区块上达到95.2%准确率、90.4% mF1、82.6% mIoU，优于Triplet KPConv，在Decreased类别上IoU达到74.8%，提升7.6个百分点。

Conclusion: 该方法在城市规模LiDAR变化检测中表现出色，特别在处理分裂合并和不确定性方面具有优势，为智能交通和数字孪生提供可靠支持。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通过嵌入可训练提示来微调冻结模型，但这些提示缺乏内在语义先验，限制了大规模模型的适应性。全参数微调需要更新大量参数，导致计算开销大、训练效率低。

Method: 构建轻量级动态混合局部先验提取器，通过异构卷积从输入图像中捕获多样局部先验，并使用门控网络动态输出专家先验。设计双向交互适配器，采用余弦对齐可变形注意力和通道导向自适应尺度增强来交互和重组冻结与可训练特征。

Result: 在31个最先进方法中表现出优越性，在多个二元目标分割任务中展示了优秀的分割性能和适应性。

Conclusion: Controllable-LPMoE方法通过动态先验微调范式，以更少的可训练参数实现了高效微调，显著提升了分割性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像安全数据集过于粗糙模糊，只提供宽泛的安全标签，无法隔离驱动安全差异的具体特征。需要系统区分良性图像和问题图像。

Method: 利用图像编辑模型对图像进行针对性修改，仅改变与安全策略相关的特征，从而翻转安全标签，同时保持安全无关细节不变。

Result: 构建了包含3,020个SafetyPair图像的基准数据集，涵盖9个安全类别，揭示了视觉语言模型在区分细微差异图像方面的弱点，并可作为有效的数据增强策略。

Conclusion: SafetyPairs为研究细粒度图像安全差异提供了首个系统性资源，既能评估模型能力，又能提高轻量级保护模型的训练效率。
...
Method: 1. 噪声注入探索策略：在视觉输入中添加高斯噪声；2. 贝叶斯优势估计：将优势估计建模为贝叶斯推理问题，结合噪声先验和轨迹奖励似然

Result: 在标准思维链质量、通用能力和幻觉基准测试中，NoisyGRPO显著提升了泛化性和鲁棒性，特别是在小规模MLLMs上

Conclusion: NoisyGRPO通过系统化的多模态强化学习框架有效解决了MLLMs推理泛化问题，为小规模模型提供了实用的强化学习训练方案
...
<details>
  <summary>Details</summary>
Motivation: CTPA依赖碘对比剂存在肾毒性和过敏反应风险，尤其对高危患者。本研究旨在开发一种从NCCT生成DCCTPA的方法，避免对比剂使用。

Method: 使用基于CycleGAN的级联合成器，在410对CTPA和NCCT扫描上进行训练和验证，其中249对用于内部训练验证，161对用于测试集评估。

Result: 该方法在定量指标上表现最佳（验证集MAE:156.28，PSNR:20.71，SSIM:0.98；测试集MAE:165.12，PSNR:20.27，SSIM:0.98），在下游任务中血管分割Dice系数达0.70，血管体积ICC为0.81，显著优于NCCT。

Conclusion: 提出的DCCTPA方法能有效增强血管结构，特别是在小血管方面，为规避对比剂风险提供了可行解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前实践通常用纯文本提示和VQA式评分来代理视觉空间智能，这会模糊几何信息、引入语言捷径，并削弱对真正空间技能的归因。需要一种能忠实表示空间结构的方法。

Method: 引入Spatial Intelligence Grid (SIG)：一种结构化的、基于网格的架构，显式编码物体布局、物体间关系和物理基础先验。作为文本的补充通道，SIG为基础模型推理提供场景结构的忠实组合表示。

Result: 在少样本上下文学习中，与仅使用VQA表示相比，SIG在所有VSI指标上都产生了更大、更稳定和更全面的增益。同时发布了SIGBench基准，包含1.4K个驾驶帧，带有真实SIG标签和人类注视轨迹。

Conclusion: SIG作为数据标注和训练架构，在学习和评估视觉空间智能方面展现出潜力，能够更准确地分离空间能力与语言先验。
...
<details>
  <summary>Details</summary>
Motivation: 传统的Flow Matching模型使用单一大型网络学习从噪声到数据的整个生成轨迹，难以同时捕捉不同时间步的信号特征，且推理成本高昂。

Method: BFM将生成轨迹划分为多个时间段，每个时间段由专门的小型速度块建模；引入语义特征引导模块，使速度块能够基于预训练表示的语义丰富特征进行条件生成；提出轻量级特征残差近似策略来保持语义质量并降低推理成本。

Result: 在ImageNet 256x256上的实验表明，BFM在保持相似生成性能的同时，实现了2.1倍到4.9倍的推理复杂度加速，建立了比现有Flow Matching方法更优的帕累托前沿。

Conclusion: BFM通过块状设计和语义特征引导，有效解决了传统Flow Matching模型的局限性，在提高生成质量的同时显著降低了推理成本。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的异常检测方法通常使用单一文本空间来对齐不同对象和领域的视觉语义，这种无差别的对齐方式阻碍了模型准确捕捉多样化的异常语义。

Method: 提出TokenCLIP框架，将token无关的文本空间扩展为一组正交子空间，通过最优传输问题动态地将每个视觉token分配到语义相关的文本子空间组合中，并使用top-k掩码来稀疏化传输计划。

Result: 大量实验证明了TokenCLIP的优越性。

Conclusion: TokenCLIP通过动态的token级视觉-文本空间对齐，实现了更精确的异常检测，在零样本设置下表现出色。
...
TL;DR: 提出知识增强基准演进(KBE)框架，通过图表示和知识集成将静态多模态基准转化为动态演进版本，解决数据污染和饱和问题。
...
Method: 使用图表示VQA样本，通过知识集成重构问题（重选视觉信息）和扩展问题（外部文本知识），实现难度可控的评估。
...
Method: 1. 独立训练四个基础模型，使用ArcFace和Circle损失的加权和；2. 应用测试时增强(TTA)和集成学习；3. 提出基于扩散和k-互惠重排序的新重排序方法。

Result: 在公开和私有排行榜上分别获得0.81219和0.81191的mAP@100分数。

Conclusion: 提出的方法在食品图像检索任务中表现优异，获得了竞赛第三名。
...
Method: 使用Arcface损失和Circle损失的适当组合，通过精心调优的配置训练模型，并进行模型集成。

Result: 该方法在竞赛中获得了第三名的成绩，证明了损失函数组合的有效性。

Conclusion: Arcface损失和Circle损失的适当组合能够提升细粒度食品识别性能，模型集成策略在竞赛中取得了良好效果。
...
<details>
  <summary>Details</summary>
Motivation: 捷径模型作为一种非对抗性生成模型范式，支持单步、少步和多步采样，但存在性能瓶颈阻碍其广泛应用。本文旨在解决五个关键问题：复合引导缺陷、固定引导不灵活、频率偏差、自一致性冲突和弯曲流轨迹。

Method: 提出了iSM统一训练框架，包含四个关键改进：内在引导提供动态控制，多级小波损失缓解频率偏差，缩放最优传输减少训练方差，双EMA策略平衡训练稳定性与自一致性。

Result: 在ImageNet 256×256上的广泛实验表明，该方法在单步、少步和多步生成中都显著优于基线捷径模型，FID指标有实质性提升。

Conclusion: iSM框架成功解决了捷径模型的核心限制，使其成为可行且具有竞争力的生成模型类别，为生成建模提供了新的非对抗性范式。
...
Conclusion: TSSR通过并行生成和创新的架构设计，在3D网格生成任务中实现了高效和高保真度的结果，为艺术家风格3D内容创作提供了有力工具。
...
Method: 提出SAGE-3D框架，包含对象中心语义标注（为3DGS添加对象级细粒度标注）和物理感知执行连接（嵌入碰撞对象并构建物理接口）。

Result: 构建了InteriorGS数据集（包含1K对象标注的3DGS室内场景）和SAGE-Bench基准（首个基于3DGS的VLN基准，包含2M VLN数据）。实验显示3DGS场景数据更难收敛但泛化性强，在VLN-CE Unseen任务上提升基线性能31%。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型由于输入分辨率受限，在处理高分辨率图像中的微小物体时面临挑战，特别是在杂乱背景下精确理解和定位视觉细节

Method: 采用粗到细的流程，包括全局语义探索(GSE)和局部感知细化(LPR)两个阶段，并引入定位知情的回顾奖励来耦合两个阶段

Result: 在FineRS-4k和公共数据集上的实验表明，该方法在指令引导分割和视觉推理任务上持续优于最先进的多模态大语言模型方法

Conclusion: FineRS框架有效解决了高分辨率图像中微小物体的精确分割和推理问题，通过两阶段强化学习方法提升了模型的视觉理解能力
...
<details>
  <summary>Details</summary>
Motivation: 由于难以将多模态表示的语义映射到统一概念集，视觉-语言对齐组件的可解释性尚未得到研究。

Method: 使用基于距离的编码器和两个模态特定解码器构建VL-SAE，通过余弦相似度测量多模态表示的语义相似性，并在自监督训练中鼓励语义相似表示具有一致的神经元激活。

Result: 在多种VLMs（如CLIP、LLaVA）上的实验表明，VL-SAE在解释和增强视觉-语言对齐方面具有卓越能力，能提高零样本图像分类和幻觉消除等下游任务的性能。

Conclusion: VL-SAE通过统一概念集成功解释了视觉-语言对齐，并能在概念层面加强这种对齐，从而提升下游任务表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有计算框架仅限于二维表示，无法捕捉细胞形态在扰动下的复杂性，这限制了准确虚拟细胞模型的发展。

Method: FORM包含形态编码器和基于扩散的扰动轨迹模块，使用多通道VQGAN学习紧凑的3D细胞形状表示，并在包含65,000多个3D细胞体积的大规模数据集上训练。

Result: FORM支持无条件形态合成和条件模拟扰动细胞状态，能够预测下游信号活动、模拟组合扰动效应，并建模未见扰动的形态动态转变。

Conclusion: FORM和MorphoEval评估套件共同推动3D虚拟细胞的实现，通过高分辨率预测模拟将形态、扰动和功能联系起来。
...
<details>
  <summary>Details</summary>
Motivation: 传统多尺度特征融合方法在复杂果园环境中面临挑战，无法充分处理局部与全局特征关系，且苹果叶病病变形态和分布具有显著异质性。

Method: 使用CNN提取局部病变细节特征，Vision Transformer捕获全局结构关系，自适应特征融合模块动态融合特征，并利用预训练CLIP权重实现视觉特征与疾病语义描述的深度对齐。
...
Method: 采用动态语义相关性生成器结合Transformer的相关图来探索语义相关性，并设计了剪枝方法以提高跟踪速度。

Result: 在多个无人机跟踪数据集上实现了有竞争力的性能，提供了多种模型变体以在速度和精度之间进行权衡。

Conclusion: 该方法有效解决了无人机跟踪中的语义感知不足问题，提高了在典型挑战下的跟踪准确性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 眼动注视能够提供关于注意力、短期意图和未来行动的有价值线索，是建模自我中心行为的强大信号。

Method: 使用注视正则化注意力机制，在训练时对齐模型关注与人类视觉注视，不依赖注视作为辅助输入信号。

Result: 相比无注视正则化的基线模型，未来事件预测的语义预测分数提升高达11%，当前活动理解提升约7%。

Conclusion: 注视引导的训练能够显著提高自我中心视觉语言模型的准确性和鲁棒性，为辅助机器人和人机协作等现实场景奠定了基础。
...
Method: 使用2.5D U-Net++网络（ResNet-34编码器），联合训练各解剖区域并进行区域微调。结合像素级L1损失和IMPACT-Synth感知损失。评估两种配准策略：Elastix（互信息）和IMPACT（基于特征的相似性度量）。

Result: 在本地测试集上，IMPACT配准实现了更准确和一致的解剖对齐，降低了MAE并产生更真实的解剖结构。但在公共验证集上，使用Elastix配准数据的模型得分更高，反映了配准偏差。
...
<details>
  <summary>Details</summary>
Motivation: 传统扩散模型使用固定步数生成高保真图像，但在云到设备场景中，有限带宽需要重度压缩，导致纹理丢失和计算浪费。需要一种能根据传输条件自适应调整生成质量的方法。

Method: 引入联合端到端训练策略，扩散模型以基于可用带宽的目标质量水平为条件。使用轻量级质量嵌入指导去噪轨迹，通过自适应调制去噪过程实现早期停止采样。

Result: 实验结果表明，该方法相比简单早期停止显著提高了带宽适应生成的视觉保真度，在带宽受限环境中提供了高效的图像传输解决方案。

Conclusion: 该方法为带宽受限环境中的高效图像传输提供了有前景的解决方案，仅需最小架构改动，通过质量嵌入指导实现自适应生成。
...
Method: 引入地理空间布局编码器，统一边界框和分割掩码输入，结合多尺度注入方案和掩码加权损失来显式编码空间约束。构建首个大规模多任务遥感布局生成数据集。

Result: TerraGen在多样化任务中实现了最佳生成图像质量，可作为通用数据增强生成器，显著提升下游任务性能，在完整数据和少样本场景下都展现出强大的跨任务泛化能力。

Conclusion: TerraGen通过统一框架解决了遥感视觉任务中数据增强的局限性，实现了空间可控的图像生成和跨任务泛化。
...
Method: 采用多阶段机制结合全局深度正则化约束增强对齐精度；在融合过程中使用图计算确定最优拼接缝并扩散软缝区域；引入重参数化策略优化计算效率。

Result: 实验证明该方法在性能上优于现有方法，能够有效减轻视差引起的对齐误差，实现自然无缝的拼接效果。

Conclusion: 所提出的深度一致性约束图像拼接方法能够有效解决视差问题，在保持最佳性能的同时显著提高算法效率。
...
<details>
  <summary>Details</summary>
Motivation: 针对长视频平台的实际需求，现有视频检索方法在处理未修剪视频和多模态查询时存在局限，需要专门的任务和基准来推动该领域发展。

Method: 构建MUVR基准，支持视频中心的多模态查询（长文本描述、视频标签提示、掩码提示），采用一对多检索范式，建立基于核心视频内容的多级视觉对应关系（6个级别），开发3个版本进行评估。

Result: 评估了3种最先进的视频检索模型、6种基于图像的VLM和10种MLLM，揭示了这些方法在处理未修剪视频和多模态查询时的局限性，以及MLLM在多视频理解和重排序方面的不足。
...
<details>
  <summary>Details</summary>
Motivation: 现有语言引导的视觉概念学习方法局限于少数预定义的基础维度（如颜色、形状），且通常在合成数据集中探索，无法处理真实世界中丰富多样的语义维度。

Method: 利用预训练视觉语言模型和通用提示策略识别多样化的图像相关概念轴；通过通用概念编码器自适应地将视觉特征绑定到发现的轴上；优化组合锚定目标确保各轴可独立操作。

Result: 在ImageNet、CelebA-HQ和AFHQ数据集上展示了卓越的编辑能力，能够处理过于多样化而无法手动预定义的真实世界概念，并在组合泛化方面优于现有方法。

Conclusion: 该方法为语言引导的视觉概念学习提供了可扩展的解决方案，能够自适应地发现和操作丰富的语义维度，在真实世界场景中表现出强大的编辑和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在生成关节化3D物体时难以同时保证精细几何结构、准确关节运动和逼真外观，特别是在处理关节依赖的可见性变化方面存在不足。

Method: 使用变分自编码器将稀疏体素表示和关节属性嵌入统一潜在空间，训练潜在扩散模型进行采样，并引入关节感知高斯解码器处理关节状态相关的可见性变化。

Result: 在PartNet-Mobility和ACD数据集上的实验表明，ArtiLatent在几何一致性和外观保真度方面优于现有方法。

Conclusion: 该框架为关节化3D物体合成和操作提供了可扩展的解决方案，能够生成具有精细几何、准确关节和逼真外观的3D物体。
...
TL;DR: 提出了一种各向异性池化方法，用于改进基于查找表（LUT）的CNN图像恢复方法，替代传统的平均池化，以更好地处理各向异性信号结构。


<details>
  <summary>Details</summary>
Motivation: 现有的LUT-based CNN图像恢复方法使用平均池化融合不同方向的像素块结果，但这种方法不适合处理各向异性信号结构，限制了性能提升。

Method: 首先引入广义中值池化方法，然后扩展为学习数据依赖的池化系数，使不同方向的像素块贡献能够自适应加权。

Result: 在各种恢复基准测试中，各向异性池化策略在感知质量和数值指标上都优于现有的LUT-realizable CNN方法。

Conclusion: 各向异性池化策略能够有效提升LUT-based CNN图像恢复方法的性能，特别是在处理各向异性信号结构时表现更优。
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D对象和场景层次结构建模方法存在显著限制：要么需要多次渲染来捕捉不同粒度的嵌入，显著增加推理时间；要么依赖预定义的封闭离散层次结构，对现实世界中多样化结构泛化能力差。

Method: 提出OpenHype方法，使用连续双曲潜在空间表示场景层次结构，利用双曲几何特性自然编码多尺度关系，并通过潜在空间中的测地线路径实现层次结构的平滑遍历。

Result: 在标准基准测试中，OpenHype方法优于最先进的方法，展示了在3D场景理解方面的卓越效率和适应性。

Conclusion: 通过使用双曲几何表示层次结构，OpenHype提供了一种更高效和适应性更强的3D场景层次建模方法，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 解决从有限真实世界视频数据学习物理一致动态模型的挑战，特别是针对具有空间变化物理属性的可变形物体。

Method: 首先在MPM模拟器中构建物理一致的数字孪生体，通过本构模型选择和全局到局部优化物理属性；然后对物理属性施加部分感知扰动，为数字孪生体生成各种运动模式，合成大量多样的演示；最后使用这些演示训练嵌入物理属性的轻量级GNN世界模型。

Result: PhysWorld能够对各种可变形物体进行准确快速的未来预测，并且对新交互具有良好的泛化能力。实验显示PhysWorld具有竞争力性能，同时推理速度比最先进方法PhysTinn快47倍。

Conclusion: PhysWorld通过模拟器合成物理演示的方法，有效解决了数据稀缺问题，实现了高效准确的可变形物体动态预测，具有快速推理和良好泛化能力。
...
TL;DR: 提出MoniTor方法，一种基于内存的在线评分队列方案，用于无需训练的在线视频异常检测，通过结合预训练视觉语言模型和LSTM启发的预测机制，在实时约束下有效识别异常行为。


<details>
  <summary>Details</summary>
Motivation: 在线视频异常检测由于实时约束和计算强度很少受到关注，而离线VAD已受益于大语言模型和视觉语言模型的进展。本文旨在解决在线VAD的固有复杂性。

Method: 使用流式输入到VLMs，结合LSTM启发的预测机制来捕获时间依赖性，设计评分队列和异常先验来动态存储最近分数并覆盖监控场景中的所有异常。

Result: 在UCF-Crime和XD-Violence两个大型数据集上的评估表明，MoniTor优于最先进方法，且无需训练即可与弱监督方法竞争。

Conclusion: MoniTor为在线视频异常检测提供了一种有效的无需训练解决方案，能够实时识别各种监控和真实场景中的异常行为。
...
TL;DR: VidSplice是一个新的视频修复框架，通过解耦视频修复为两个子任务：多帧一致的图像修复和掩码区域运动传播，引入间隔帧先验来指导修复过程，提升时空一致性。
...
Method: 提出VidSplice框架，包含CoSpliced模块通过首帧传播策略扩散内容，以及上下文控制模块在帧复制后编码一致先验并注入I2V生成主干。

Result: 广泛评估表明VidSplice在多样化视频修复场景中达到竞争性表现，显著改善了前景对齐和运动稳定性。

Conclusion: VidSplice的设计有效提升了视频修复的时空一致性，优于现有方法。
...
TL;DR: CXR-LanIC是一个用于胸部X光诊断的可解释AI框架，通过任务对齐的模式发现，将医学图像表示分解为可解释的视觉模式，实现透明且可验证的诊断解释。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在胸部X光诊断中准确性很高，但由于其黑盒性质，临床采用受限。临床医生需要透明、可验证的解释来信任自动化诊断并识别潜在失败模式。

Method: 训练基于transcoder的稀疏自编码器，在BiomedCLIP诊断分类器上分解医学图像表示。在MIMIC-CXR数据集的多模态嵌入上训练100个transcoder集合，发现约5,000个单语义模式。

Result: 发现了涵盖心脏、肺部、胸膜、结构、设备和伪影类别的约5,000个单语义模式。每个模式在具有特定放射学特征的图像间表现出一致的激活行为。在五个关键发现上达到竞争性诊断准确性。

Conclusion: CXR-LanIC证明医疗AI系统可以既准确又可解释，通过从针对特定诊断目标训练的分类器中提取可解释特征，确保发现的模式与临床决策直接相关，支持更安全的临床部署。
...
TL;DR: 提出双流架构模型，结合组织宏观特征和细胞级特征，通过循环变换器高效聚合细胞信息，实现组织与细胞的双向交互，在病理亚型分类任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有病理基础模型主要关注全局组织上下文，但忽略了细胞级特征建模，这在癌症亚型分类等细粒度任务中是一个关键限制。

Method: 采用双流架构，包含组织特征流和细胞特征流；提出接受加权的键值聚合模型（循环变换器）以线性复杂度捕获细胞间依赖关系；引入双向组织-细胞交互模块实现局部细胞线索与周围组织环境的相互关注。
...
TL;DR: GRAP-MOT是一种针对重叠多摄像头视角封闭区域视频中人员多目标跟踪的新方法，通过图加权解决方案在线更新人员身份标签，在拥挤场景中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决封闭区域多摄像头重叠视角下频繁发生的人员遮挡问题，提升在高度拥挤空间中的多目标跟踪性能。

Method: 采用图加权解决方案，基于轨迹和人员特征在线更新身份标签；包含位置估计模块为跟踪提供关键信息；深入研究了特征提取、跟踪和社区搜索等MOT过程的所有元素。

Result: 在封闭区域模型记录和满足高度拥挤空间要求的公共数据集上测试，显示该方法优于无位置数据的方法；分析表明IDF1比MOTA更适合此类比较。

Conclusion: GRAP-MOT在拥挤多摄像头场景中表现出优越性能；IDF1是更合适的评估指标；代码和数据集已公开。
...
Method: 基于小样本学习的EH-YOLOv11n模型，集成小波卷积和坐标卷积增强频率和空间特征提取，引入级联分组注意力机制抑制超声伪影和遮挡干扰，提高边界框定位精度。
...
Method: 构建了包含200万张图像和1.8亿个高质量区域级标注的Gran-29M数据集；开发了预训练-适应框架和自蒸馏机制；采用边界框到标题回归和标题到边界框回归来增强视觉表示和定位能力。
...
TL;DR: 本文提出了前瞻性引导（FSG）方法，通过统一视角将条件引导重构为固定点迭代，解决了现有CFG方法的效率问题。
...
Method: 将条件引导重构为固定点迭代，寻找潜在空间的黄金路径；提出FSG方法，在早期扩散阶段优先解决长间隔子问题并增加迭代次数。
...
TL;DR: 该论文提出了一种基于信号处理的方法来分析和编辑文本生成模型中注意力头的语义和视觉属性专业化，通过编辑少量注意力头即可可靠地抑制或增强目标概念。


<details>
  <summary>Details</summary>
Motivation: 尽管语言和视觉语言模型在各种任务中表现出色，但其内部机制仍不完全清楚。本研究旨在理解文本生成模型中个体注意力头如何专门化处理特定语义或视觉属性。

Method: 基于已有的可解释性方法，通过信号处理视角重新解释使用最终解码层探测中间激活的实践，从而以原则性方式分析多个样本，并根据注意力头与目标概念的相关性进行排序。

Result: 研究发现在单模态和多模态transformer中，注意力头层面存在一致的专业化模式。仅编辑使用该方法选择的1%的注意力头，就能可靠地抑制或增强模型输出中的目标概念。
...
TL;DR: Foley Control是一种轻量级视频引导Foley音效生成方法，通过冻结预训练的单模态模型，仅学习小型跨注意力桥接来实现视频到音频的同步生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要重新训练音频先验模型，计算成本高且缺乏模块化。本文旨在开发一种轻量级方法，在保持预训练模型性能的同时实现视频-音频同步。

Method: 连接V-JEPA2视频嵌入到冻结的Stable Audio Open DiT文本到音频模型，在文本跨注意力后插入紧凑的视频跨注意力，使用视频token池化减少内存需求。

Result: 在视频-音频基准测试中，Foley Control在时间和语义对齐方面表现优异，参数量远少于多模态系统，同时保持提示驱动的可控性和模块化。

Conclusion: 该方法提供了一种高效、模块化的视频引导Foley生成解决方案，桥接设计可扩展到其他音频模态如语音生成。
...
TL;DR: 提出了Chunk-GRPO方法，将GRPO优化范式从步级提升到块级，解决了优势归因不准确和忽略生成时间动态的问题，在文本到图像生成中取得了更好的偏好对齐和图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有GRPO方法存在两个关键限制：优势归因不准确，以及忽略了生成的时间动态特性。

Method: 将连续步骤分组为连贯的'块'来捕捉流匹配的内在时间动态，在块级别优化策略，并引入了可选的加权采样策略。

Result: 大量实验表明Chunk-GRPO在偏好对齐和图像质量方面都取得了优越的结果。
...
Method: MATrack包含三个核心模块：多尺度层次混合(MHB)增强静态和动态模板间的特征一致性，自适应关键令牌门准确识别复杂背景中的物体信息，夜间模板校准器(NTC)确保长序列中的稳定跟踪性能。

Result: 在UAVDark135基准测试中，MATrack的精确度、归一化精确度和AUC分别比最先进方法提高了5.9%、5.4%和4.2%，同时保持81 FPS的实时处理速度。在真实无人机平台上的进一步测试验证了系统的可靠性。

Conclusion: MATrack能够为夜间搜救和边境巡逻等关键机器人应用提供稳定有效的夜间无人机跟踪支持，解决了现有方法在实际部署中的主要局限性。
...
TL;DR: TIGER是一个两阶段文本图像超分辨率框架，通过"先文本后图像"的范式打破图像质量与文本可读性的权衡，在保持图像质量的同时显著提升文本可读性。


<details>
  <summary>Details</summary>
Motivation: 当前生成式超分辨率方法在自然图像上表现良好，但会扭曲文本，导致图像质量与文本可读性之间的基本权衡问题。

Method: 提出TIGER两阶段框架：首先显式解耦字形恢复与图像增强，先重建精确的文本结构，然后使用这些结构指导后续的全图像超分辨率。

Result: TIGER在UltraZoom-ST数据集上实现了最先进的性能，在增强可读性的同时保持了整体图像质量。

Conclusion: TIGER通过字形到图像的引导机制，成功解决了文本超分辨率中图像质量与可读性的权衡问题，为场景文本超分辨率提供了有效解决方案。
...
TL;DR: 本文提出两种深度学习模型（ANN和CNN）用于MEG记录中的癫痫发作间期棘波检测，在仅有单专家标注的真实临床数据上表现优于现有方法，并采用交互式机器学习策略提高标注质量。


<details>
  <summary>Details</summary>
Motivation: 耐药性癫痫的术前评估需要定位癫痫灶，MEG中的发作间期棘波是重要生物标志物。手动检测耗时且易错，现有自动化方法要么需要大量标注数据，要么在非典型数据上缺乏鲁棒性，不适合临床应用。

Method: 提出基于特征的ANN和CNN两种模型架构，在59名患者数据库上训练，使用交互式机器学习策略迭代改进数据标注质量，并与现有最优模型在短时间窗口信号分类上进行比较。

Result: 两种模型在10名保留测试患者上的表现均优于现有最优模型（F1分数：CNN=0.46，ANN=0.44），交互式机器学习策略证明模型对噪声标注具有鲁棒性。

Conclusion: 简单架构的模型在分析复杂和不完美标注数据时表现出鲁棒性，交互式机器学习方法为快速数据标注提供了潜力，所提模型是自动检测发作间期棘波的有效工具。
...
TL;DR: 提出了S3OD方法，通过大规模合成数据生成和模糊感知架构显著提升显著目标检测的泛化能力，在DIS和HR-SOD基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 显著目标检测任务受限于昂贵的像素级标注，导致相关子任务需要单独训练模型。

Method: 使用多模态扩散管道生成超过139,000张高分辨率图像的S3OD数据集，采用迭代生成框架优先处理具有挑战性的类别，并提出简化的多掩码解码器处理检测中的固有模糊性。

Result: 仅使用合成数据训练的模型在跨数据集泛化中实现20-50%的错误减少，微调版本在DIS和HR-SOD基准测试中达到最先进性能。

Conclusion: 大规模合成数据和模糊感知架构能够显著提升显著目标检测的泛化能力和性能。
...
Method: 采用随机扰动引入可控噪声模拟不确定性，结合嵌入平滑校准嵌入空间中的相似度分布，共同减少过度自信

Result: 在多个基准数据集上，Modest-Align在检索任务中优于最先进方法，仅用CLIP 1/100的训练数据和1/600的GPU时间即达到竞争性结果
...
<details>
  <summary>Details</summary>
Motivation: 尽管视频生成模型通过大潜在扩散变换器和整流流技术取得了巨大进展，但仍存在几何不一致、运动不稳定和视觉伪影等问题，破坏了真实3D场景的幻觉。3D一致视频生成对生成和重建任务的下游应用有重要影响。

Method: 使用成对极线几何约束通过基于偏好的优化来对齐扩散模型，直接解决不稳定的相机轨迹和几何伪影问题。该方法高效地强制执行几何原理，无需端到端可微分性。在静态场景和动态相机上进行训练以确保高质量测量。

Result: 评估表明，经典几何约束比现代学习指标提供更稳定的优化信号，后者会产生噪声目标并损害对齐质量。模型能有效泛化到各种动态内容。
...
TL;DR: 提出DAP-MAE方法，通过自适应整合跨域点云数据集知识来解决点云数据稀缺问题，在单一预训练下在多个下游任务中取得优异性能
...
Conclusion: DAP-MAE能有效解决跨域点云数据预训练中的知识对齐问题，为通用点云分析提供了一种高效的预训练解决方案
...
Conclusion: Gompertz-CNN通过模拟学习进程的动态蒸馏策略，显著提升了知识蒸馏效果，验证了阶段感知方法的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 解决纯IMU方法在多人场景下无法准确估计全局位置和相对位置的问题，因为惯性测量是自参考的，缺乏对他人的空间参考。

Method: 使用IMU+UWB传感器，通过结构化状态空间模型融合惯性观测和UWB测距距离，采用两步优化方法跟踪全局轨迹，并创建了首个IMU+UWB双人跟踪数据集。

Result: 在合成和真实数据评估中，该方法在准确性和鲁棒性方面优于现有最先进方法，展示了IMU+UWB在野外多人运动捕捉的潜力。

Conclusion: Group Inertial Poser方法通过结合IMU和UWB技术，成功实现了多人场景下精确的3D姿态估计和全局轨迹跟踪，为野外多人运动捕捉提供了有效解决方案。
...
TL;DR: 该论文系统研究了长尾识别方法在NACTI野生动物数据集上的应用，通过实验优化LTR损失函数和正则化方法，显著提升了物种识别准确率，并在域偏移测试中展示了更好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: NACTI数据集存在严重的类别不平衡问题，最大的'Head'类别占比超过50%，需要专门的长尾识别方法来处理这种不平衡分布。

Method: 基于PyTorch Wildlife模型，系统研究各种LTR损失函数和LTR敏感正则化方法，构建减少偏差的测试集评估域偏移情况。

Result: 最佳配置在NACTI测试集上达到99.40%的Top-1准确率，显著优于95.51%的基线；在域偏移测试中达到52.55%的准确率，优于WCE损失的51.20%。

Conclusion: LTR增强方法在野生动物识别领域表现优异，特别是在与先进LTR损失函数结合时，但仍有局限性，如在严重域偏移下对尾部类别的灾难性失效问题。
...
TL;DR: 提出一种基于无监督学习的方法，仅通过观察同一神经元内相邻突触比随机选择的突触更相似来对突触进行分类，无需预先知道突触类型数量。


<details>
  <summary>Details</summary>
Motivation: 传统方法需要监督学习，而本文旨在仅基于同一神经元内相邻突触更相似的观察来分离突触类型，无需预先指定类别数量。

Method: 利用同一神经元内相邻突触比随机选择的突触更相似的原理，构建无监督分类方法，应用于果蝇数据。

Result: 成功将突触分离成不同类别，无需预先知道类型数量，为选择具有代表性的突触结构提供了原则性方法。

Conclusion: 该方法提供了一种无需监督学习的突触分类新途径，能够自动确定突触类型数量，并为选择真实数据提供了理论基础。
...
TL;DR: 评估UNI和Virchow2两种基础模型在皮肤病理学全切片图像分类中的表现，发现Virchow2特征提取器在大多数分类器中表现更优，逻辑回归达到90%准确率。
...
Method: 使用UNI和Virchow2基础模型提取补丁级嵌入，通过均值聚合策略整合为切片级特征，训练多种机器学习分类器（逻辑回归、梯度提升树、随机森林）。

Result: Virchow2提取的特征在大多数切片级分类器中表现优于UNI，逻辑回归达到最高准确率90%，但差异无统计学意义。均值聚合方法提供了可靠的切片级特征表示。

Conclusion: 基础模型在自动化全切片图像分类中具有巨大潜力，为皮肤病理学诊断提供了可扩展且有效的方法，并为切片级表示学习的未来发展铺平了道路。
...
TL;DR: WorldGrow是一个用于无限扩展3D场景合成的分层框架，通过利用预训练3D模型的生成先验，实现大规模、连续且几何和外观一致的3D环境生成。
...
Method: 采用分层框架，包含三个核心组件：(1)数据筛选流程提取高质量场景块用于训练；(2)3D块修复机制实现上下文感知的场景扩展；(3)从粗到细的生成策略确保全局布局合理性和局部几何/纹理保真度。
...
TL;DR: 该研究将后验解释方法应用于基于Faster R-CNN的海豹检测模型，通过评估解释的定位保真度、忠实性和诊断效用，为生态保护监测提供可信的决策支持工具。
...
Result: 解释集中在海豹躯干和轮廓而非周围环境，移除海豹会降低检测置信度，揭示了海豹与黑冰和岩石混淆的系统性错误来源。

Conclusion: 通过将目标检测与后验可解释性相结合，可以超越"黑盒"预测，为保护监测开发可审计的决策支持工具，并提出了针对性数据整理和增强的改进方向。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法需要参考图像或大量训练，且通常只关注角色一致性的问题，实现无需训练的多视频一致生成。

Method: 基于对DiT注意力机制的分析，首先生成身份视频并缓存中间变量，然后将这些变量注入新生成视频的对应位置。
...
Conclusion: 在图像空间中操作提供了一个通用且实用的框架来近似解决著名的难题，为处理更广泛的几何任务打开了大门。
...
TL;DR: 提出了一个自动化框架来检测训练好的视觉模型对特定视觉属性的依赖，通过自反思代理系统性地生成和测试假设，迭代改进假设以准确解释模型行为。
...
Method: 使用自反思代理系统性地生成和测试关于模型可能依赖的视觉属性的假设，通过迭代过程基于实验结果改进假设，并采用自评估协议验证发现是否准确解释模型行为。

Result: 在包含130个模型的基准测试中，代理的性能随着自反思持续提升，显著优于非反思基线，并能识别CLIP视觉编码器和YOLOv8目标检测器等先进模型中的真实世界视觉属性依赖。

Conclusion: 自反思方法能有效检测视觉模型对特定视觉属性的依赖，为模型理解和改进提供了有力工具。
