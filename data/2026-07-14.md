...
TL;DR: 本文提出DUNE，一种无需训练的去伪影框架，通过检测扩散模型早期深层潜变量中的突变并施加抑制，提升生成保真度并减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 扩散模型的性能与去噪主干网络紧密相关，但现有研究缺乏对扩散组件中潜在波动与伪影之间关系的系统性分析。

Method: 提出DUNE框架，基于EMA标准检测深层低噪声内部潜变量的突变，并对检测到的条目施加主干特定的抑制策略，可扩展至U-Net和Transformer架构。

Result: 实验表明DUNE能提升生成保真度并减少幻觉。

Conclusion: DUNE揭示了扩散模型应在何处、何时进行控制，为改进扩散主干提供了新见解。
...
TL;DR: WiCAT是一个面向宽场钙成像数据的多主体预训练模型，通过自监督学习实现零样本行为解码和跨主体泛化。


<details>
  <summary>Details</summary>
Motivation: 宽场钙成像数据维度高、结构复杂且包含大量任务无关活动，现有模型多局限于单会话分析，缺乏可扩展性和泛化能力。多主体预训练模型在宽场数据上尚未被验证，且跨主体的零样本行为解码仍是难题。

Method: 提出WiCAT模型，采用基于图谱的令牌化方案（无会话特定组件），通过自监督预训练学习全局共享的时空表征，支持轻量级下游解码。

Result: 在多个宽场数据集上，WiCAT优于单会话基线模型；支持跨主体、跨任务、跨数据集的迁移；实现了对未见主体的鲁棒零样本连续行为解码和留出脑区重建。

Conclusion: WiCAT是宽场数据基础建模的第一步，证明了多主体预训练在宽场钙成像上的可行性，并实现了跨主体零样本行为解码。
...
TL;DR: 提出RSLoRA，一种无需训练和梯度的秩分配方法，通过激活空间几何和虚拟表征探测机制，自动为不同层分配合理的LoRA秩，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统LoRA对所有层使用统一秩分配，忽略了神经网络层的功能异质性；现有秩分配方法要么计算开销大（训练式），要么无法捕捉动态任务表征流形（预分配式）。

Method: 提出虚拟表征探测机制：通过结构化低秩噪声模拟适应过程，利用有效秩（Effective Rank）和弗雷歇距离（Fréchet Distance）测量流形位移，识别高敏感度模块并分配更高秩。

Result: 在主流基准上，RSLoRA持续优于AdaLoRA、GoRA等最先进分配器，且无需迭代训练调整和反向梯度。

Conclusion: RSLoRA提供了一种高效、鲁棒且表征感知的大模型适应方案，在计算效率与分配合理性之间取得了良好平衡。
...
TL;DR: 本文提出ReflectWorld-MM，一种面向开放视频流的实体导向多媒体记忆系统，通过分层长期记忆（多尺度情景记忆、演化实体语义记忆、程序记忆）和感知前端，在六个长视频和终身记忆基准上取得最佳准确率。


<details>
  <summary>Details</summary>
Motivation: 现有系统将记忆放在模型上下文或扁平特征存储中，以帧而非持久实体组织记忆，局限在有界视频中，难以追踪跨时间重复出现的实体。

Method: 系统由三部分组成：(1) 感知前端：将流视频转化为实体解析的观察，置于有界短时记忆中；(2) 分层长期记忆：基于人类记忆理论，包括多尺度情景记忆、演化的实体中心语义记忆和程序记忆；(3) 完整实现：可接入任意流和现有助手。

Result: 在六个长视频和终身记忆基准测试中取得全部最优准确率，超过强基线记忆模型和前沿模型。

Conclusion: 实体导向的分层记忆结构能有效处理开放视频流，提升跨时间实体追踪能力，为持续学习的多模态智能体提供了新范式。
...
TL;DR: 提出了一种知识约束的形状优化框架，结合MoE-NO代理模型和不确定性估计，实现工程感知的约束优化，在汽车气动优化中取得4%-10%的减阻效果。


<details>
  <summary>Details</summary>
Motivation: 传统形状优化依赖专家手动设置参数，且代理模型在异构几何数据库和分布外设计下不可靠，亟需自动化、可靠的优化框架。

Method: 1) 基于DFFD的知识约束变形算子；2) MoE-NO代理模型提升异构数据集预测精度；3) 基于MoE-NO编码器和马氏距离的不确定性估计，检测分布外几何并触发物理求解器反馈。

Result: MoE-NO在MPV/SUV/Sedan数据集上MAPE=1.16%，趋势预测准确率94.34%；优化实验获得CFD验证的4%-10%减阻效果。

Conclusion: 知识约束框架有效降低了工程专家依赖，MoE-NO提升了异构气动数据的预测可靠性和优化鲁棒性。
...
TL;DR: 本文首次实现了基于低功耗MCU（多核RISC-V处理器GAP8）的自动车牌识别（ALPR）边缘设备，采用SSDlite-MobilenetV2检测+LPRNet识别的多模型推理管线，在117mW功耗下达到1.09FPS，相比树莓派3方案能效提升73倍。


<details>
  <summary>Details</summary>
Motivation: 现有ALPR系统多依赖云端或高性能嵌入式平台（如树莓派），功耗高、部署受限。作者希望将ALPR全流程搬到极低功耗的MCU级边缘设备上，实现真正嵌入式、低成本的实时车牌识别。

Method: 采用GAP8（9核RISC-V处理器）+超低功耗QVGA灰阶图像传感器；检测阶段使用SSDlite-MobilenetV2，识别阶段使用LPRNet；通过模型压缩与优化策略（687 MMAC计算量）在MCU上实现多模型推理管线。

Result: 检测mAP 38.9%，字符识别率>99.13%；车牌小至30x5像素仍可识别；系统吞吐量1.09 FPS，功耗仅117 mW；相比树莓派3方案能效提升73倍。

Conclusion: 首次证明了MCU级设备能够承载如此复杂的多模型推理ALPR任务，且无需专用硬件加速器，保持算法灵活性，为超低功耗边缘视觉应用提供了可行方案。
...
TL;DR: 提出Time Imprint框架，将时间作为实体级模态，通过三视图对比学习对齐时间、文本和视觉表示，并设计紧凑时间戳子集选择与注意力池化，在多模态知识图谱链接预测任务上达到SOTA，Hits@1提升最高6.07%。


<details>
  <summary>Details</summary>
Motivation: 现有MMKG方法难以区分多模态特征高度相似的实体，而时间信息虽可作为额外模态进行消歧，但面临两大挑战：(1) 时间语义稀疏，难以与更丰富的文本/图像模态对齐；(2) 多时间戳引入噪声，降低表征鲁棒性。

Method: (1) 将时间作为实体级独立模态；(2) 三视图对比学习（时间-文本-视觉）联合对齐；(3) 紧凑时间戳子集选择设计空间；(4) 注意力池化聚合选定时间戳为判别性嵌入，平衡时间特异性和鲁棒性。

Result: 在三个MMKG基准上达到SOTA链接预测性能，Hits@1整体提升最高6.07%，在top-1%歧义样本子集上提升高达58%，且训练开销仅适度增加。

Conclusion: 将时间作为独立模态在多模态知识图谱实体消歧中有效，紧凑子集选择与注意力池化能缓解多时间戳噪声问题，时间模态在歧义性强的样本上收益最显著。
...
TL;DR: 本文提出广义深度非负矩阵分解（G-DNMF）方法，通过放弃逐层分解策略，使用拉格朗日乘子法推导全局最优更新规则，解决了现有DNMF方法在SAR目标识别中误差累积和局部最优的问题，在MSTAR和OpenSARship数据集上取得了更好的稳定性和识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度非负矩阵分解（DNMF）方法采用逐层分解策略，导致误差累积和局部最优问题，限制了多层特征提取能力，使得随着层数增加识别精度无法持续提升。

Method: 提出G-DNMF方法，放弃逐层分解策略，使用拉格朗日乘子法推导各参数的全局最优更新规则。该方法统一了基于编码矩阵和混合矩阵的DNMF方法，具有理论普适性。

Result: 在MSTAR和OpenSARship数据集上，G-DNMF在稳定性和识别性能上优于现有DNMF算法及其衍生方法。通过各层特征图和重构原图验证了方法的多层特征可解释性。

Conclusion: G-DNMF通过全局最优策略有效克服了DNMF的误差累积和局部最优问题，显著提升了多层特征提取能力和识别性能，是一种鲁棒的SAR目标识别方法。
...
TL;DR: 本文提出了WHIP模型，利用多模态消费级传感器（如手机、手表、眼镜、鞋垫）数据重建全身动作，并发布了包含50种活动的多模态数据集，系统研究了传感器之间的互补性。


<details>
  <summary>Details</summary>
Motivation: 现有动作捕捉研究大多依赖固定传感器配置（如IMU套装或头显设备），无法泛化到不同传感器组合。研究希望利用日常穿戴设备（如手机、手表等）进行轻量化、非侵入式的全身动作重建。

Method: 构建大规模多模态数据集（50种活动，包含日常任务、运动、社交互动等），提出WHIP基线生成模型，可从任意传感器子集重建动作，鲁棒处理缺失模态并生成物理合理的运动。

Result: WHIP模型能有效处理传感器缺失情况下的动作重建，并通过系统研究量化了不同传感器模态之间的互补性。

Conclusion: 动作捕捉应优先使用轻量级日常设备，多传感器模态具有互补性，WHIP为灵活传感器配置下的动作重建提供了有效基线方案。
...
TL;DR: 本文研究了真实图像扩散反演中的质量-成本权衡，发现噪声压缩存在元素级优于子空间的非对称性，并据此提出NARC方法，在存储400倍减少的同时PSNR提升3.24 dB。


<details>
  <summary>Details</summary>
Motivation: 扩散模型反演面临质量与计算/存储成本的权衡困境，现有方法在效率与保真度之间难以兼顾，亟需理解反演机制并找到高效的存储噪声反演策略。

Method: 通过前向高斯噪声锚点分析扩散轨迹，揭示两个关键机制：1）元素级int8全维锚点优于低维子空间压缩；2）反演依赖轨迹匹配和分数先验。基于此提出NARC，存储单个int8潜变量锚点，采用噪声级别依赖的锚点权重调度。

Result: 在PIE-Bench++和Stable Diffusion 1.5上，NARC超越五种非精确基线方法，PSNR比PnP DirectInv提高3.24 dB，同时存储开销减少约400倍。该方法还迁移至SDXL 1024^2。

Conclusion: 元素级压缩优于子空间压缩的反直觉发现和锚点-轨迹耦合机制为高效扩散反演提供了理论指导，NARC是一种无需训练、存储高效且质量优越的反演方案。
...
TL;DR: 本文系统综述了面向视觉任务的持续自监督学习（CSSL），分析了评估协议的不一致性，探讨了自监督目标对灾难性遗忘的鲁棒性原因，建立了统一的分类法，并指出了向大规模持续预训练范式发展的未来方向。


<details>
  <summary>Details</summary>
Motivation: 传统持续学习依赖有标签数据，而现实应用（如机器人）需从无标签流中持续适应，CSSL领域虽发展迅速但缺乏系统性综述，因此本文旨在填补这一空白。

Method: 通过分析现有评估协议的不一致性，探究自监督目标抗遗忘鲁棒性的原因（任务无关表征和平滑损失景观），并基于遗忘缓解策略（蒸馏、回放、正则化、架构方法、模型合并、目标级适应）建立统一分类法。

Result: 揭示了评估标准不统一的现状，解释了自监督学习对灾难性遗忘具有更强鲁棒性的内在机制，并系统梳理了六类遗忘缓解方法。

Conclusion: CSSL需从小规模基准测试向大规模持续预训练范式转变，同时面临可扩展性和快速适应性等开放挑战。
...
TL;DR: 本文提出了首个基于扩散模型的针对2D距离图像语义分割的无限制对抗攻击方法，通过分割损失进行对抗引导，在保持LiDAR数据流形真实性的同时产生结构化的分割错误。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击主要研究图像分类和3D点云分割，但针对2D距离图像的无限制对抗攻击尚未被充分探索，而这对于自动驾驶的安全性至关重要。

Method: 基于扩散模型，在采样过程中直接施加分割损失的对抗引导，生成位于LiDAR数据流形上的无限制对抗样本，诱导结构化分割错误。

Result: 在SemanticKITTI数据集上针对RangeNet++和CENet的实验表明，该方法可实现可调节的降解效果，且在不同分割架构间具有可迁移性。相比FGSM和SegPGD，该方法提供了更好的有效性-真实性的权衡。

Conclusion: 该扩散对抗攻击方法在保证视觉真实性和分布真实性的同时，实现了可控的白盒攻击和迁移攻击，为自动驾驶感知系统的安全性评估提供了新工具。
...
TL;DR: 针对现有sMRI脑网络方法依赖单一图构建策略的局限，本文提出多视图掩码图神经网络（MVMGNN），通过联合节点-边掩码机制和跨视图门控融合，在ADNI数据集上实现了优于对比方法的AD分类性能，并能识别关键脑区。


<details>
  <summary>Details</summary>
Motivation: 早期诊断对延缓阿尔茨海默病进展至关重要，但现有基于sMRI的脑网络方法仅采用单一图构建策略，无法同时捕获脑区间的空间关系和形态相似性，限制了诊断性能。
...
Conclusion: MVMGNN有效提升了sMRI脑网络的AD诊断性能，且具有良好的可解释性，代码已开源。
...
TL;DR: 本文提出一个五维参数空间框架，用于理解和生成具有丰富纹理特征的头发发丝。通过将发丝纹理与整体方向解耦，并结合生成式AI技术，实现了基于纹理参数的发丝创建与编辑。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以精确描述和生成具有复杂纹理特征（如卷曲、扭结等）的头发发丝，缺乏一个既能定性直观又能定量精确的纹理表征框架。

Method: 1) 提出与自然发丝纹理双射的五维参数空间；2) 通过中心线几何将发丝映射到规范纹理空间，解耦纹理与整体方向；3) 提出新颖的中心线构建方法分离纹理与风格；4) 使用生成式AI和神经网络监督训练，根据五参数描述生成新发丝。

Result: 在多种头发类型上验证了框架的有效性，实现了基于纹理迁移或用户输入的毛发编辑。

Conclusion: 该框架为头发发丝的纹理理解与生成提供了一种兼具定性可访问性和定量精确性的统一方案，可广泛应用于毛发编辑与生成领域。
...
TL;DR: 本文研究在三层个人视觉记忆（个人资料、短期焦点、观察记录）辅助下，智能体如何在仅接收图像的情况下完成工具调用与参数选择。通过800张图片与合成记忆块的消融实验，完整记忆结构使工具查询相关性提升11.2%（5分制下0.47绝对提升），端到端效用提升9.7%。


<details>
  <summary>Details</summary>
Motivation: 现有视觉识别系统仅能回答“图像中有什么”，但无法结合用户个人记忆判断“下一步值得查询什么”。在仅接收图像的相机优先场景中，智能体需自主决定调用的工具与参数，现有方法缺乏用户个性化记忆的引导机制。

Method: 提出三层个人视觉记忆架构：个人资料层（长期用户画像）、短期焦点层（当前会话上下文）、观察记录层（历史交互观测）。每轮加载该记忆结构，条件化LLM工具调用循环（工具选择+参数生成），并包含冲突感知的回写机制以更新用户模型。

Result: 在800张图片与合成记忆块的消融实验中，完整三层记忆系统相较无记忆基线：工具查询相关性下降0.47（4.21→3.74，5分制，相对下降11.2%），端到端效用下降0.082（0.842→0.760，相对下降9.7%）。

Conclusion: 个人视觉记忆能显著提升智能体在图像输入场景下的工具选择与参数对齐效果。但当前结论受限于固定合成记忆块的单轮实验，尚未验证多会话回写机制与真实用户历史数据的有效性。
...
TL;DR: 本文提出了一种基于深度学习的框架，利用数字平板捕获的在线手写数据，通过融合手工设计的运动学特征、嵌入特征（MOMENT）以及连续小波变换（CWT）和格拉姆角场（GAF）的图像表征，实现客观的书写困难检测。在DiaGraMo数据集上的实验表明，GAF、MOMENT与手工运动学特征的融合方案优于其他方案。


<details>
  <summary>Details</summary>
Motivation: 书写困难（Dysgraphia）是一种影响学龄儿童 handwriting 连贯性、质量、流畅度和可读性的特定学习障碍，但传统诊断依赖临床医生的主观观察，耗时且易产生差异。因此，需要一种客观、自动化的检测方法。

Method: 提出双分支深度学习框架：1）信号分支从原始时间序列提取手工运动学特征和基于MOMENT的嵌入特征；2）图像分支利用连续小波变换（CWT）和格拉姆角场（GAF）将时间信号转化为图像表征。两分支特征进行融合，利用互补优势进行书写困难检测。

Result: 在DiaGraMo数据集上评估了四种表征的单独及联合性能，发现GAF、MOMENT与手工运动学特征的融合方案优于每种单独表征以及其他融合方案。

Conclusion: 图像表征与信号表征之间具有互补性，融合多种表征可以有效提升书写困难检测的客观性和准确性。
...
TL;DR: 本文提出了一种基于自然语言任务描述的条件性视觉显著性模型，能够根据不同的观看意图生成任务相关的显著性图，更准确地模拟目标导向的视觉注意力。


<details>
  <summary>Details</summary>
Motivation: 现有显著性模型大多假设自由观看条件，忽略了人类注意力常受具体任务目标影响这一事实，因此需要一种能结合任务语义的显著性预测方法。

Method: 提出一个以自然语言任务描述为条件输入（conditioning on natural-language task descriptions）的显著性预测模型，生成任务依赖的显著性图（task-dependent saliency maps）。

Result: 定量和定性分析表明，引入显式任务语义能够更真实地建模目标导向的视觉注意力。

Conclusion: 将任务语义显式纳入显著性模型有助于更准确地模拟不同观看意图下的注意力分布。
...
TL;DR: 该研究系统评估了YOLO26与之前三代YOLO模型（YOLOv5u、YOLOv8、YOLO11）在鱼类死亡检测任务中的性能，发现所有模型在充足数据下精度相当，但YOLOv8数据效率最高，YOLO26在边缘设备上推理速度最快，表明模型选型需综合考虑数据、硬件和推理需求。


<details>
  <summary>Details</summary>
Motivation: YOLO26架构虽声称适合资源受限的边缘设备水产养殖应用，但其在具体水产场景中的性能、效率和部署适用性尚未得到系统验证。

Method: 对YOLO26与YOLOv5u、YOLOv8、YOLO11三种前代模型的nano、small、medium三种规模进行基准测试，评估检测精度、不同数据集大小的训练效率，以及在NVIDIA A100 GPU和仅CPU的Raspberry Pi 5上的推理性能。

Result: 1) 在完整数据集上，所有模型mAP50仅相差1.04个百分点，精度相当；2) YOLOv8数据效率最高，仅需400张图像即达90% mAP50，而YOLO26 nano/small需1000张；3) YOLO26n在Raspberry Pi 5上推理速度最快（7.51 FPS），YOLOv5mu在CPU硬件上优于所有中型架构。

Conclusion: 架构新颖性不足以指导模型选型，实际部署时应联合考虑训练数据可用性、目标硬件和推理需求来选择目标检测模型。
...
TL;DR: 本文在Mendeley LBC数据集上研究了宫颈细胞学分类中的类别不平衡感知和校准感知集成方法，发现校准（温度缩放）比集成大小更能提升模型可靠性，所有分类误差仅局限于高级别病变与癌变之间的边界。


<details>
  <summary>Details</summary>
Motivation: 现实中的液基细胞学数据集通常规模小且类别不平衡，而现有分类模型多在平衡基准上评估，缺乏对实际场景的适配研究。

Method: 使用三种轻量级架构（Swin-Tiny、TinyViT-5M、DenseNet121），采用加权随机采样处理类别不平衡，结合两种软投票集成（Hybrid-2、Hybrid-3），并使用温度缩放进行后处理校准，校准集从训练部分中独立划分。

Result: 校准显著降低了ECE、Brier score和NLL，且不影响判别指标；集成大小在完全校准后未带来额外可靠性收益；所有错误分类仅限于HSIL与SCC之间。

Conclusion: 对于该数据集，校准是提升可靠性的主导因素，而非集成大小，但结论受限于数据集规模较小。
...
TL;DR: 本文提出 Prompting-MammAlps——首个相机陷阱文本-视频检索基准，并设计了一种细粒度、可解释的方法：用 Vision Transformer 进行时空动作定位并生成结构化文本，再通过 LLM 编码代理解析查询并检索视频。在 135 个查询、775 个视频的测试集上 F1 达 34%，远超零样本 VLM 的 18%。


<details>
  <summary>Details</summary>
Motivation: 从大规模相机陷阱数据集中自动检索视频仍然困难。现有基于大型视频-语言模型的文本-视频检索方法缺乏时空理解能力，且在生态数据上泛化性差。

Method: 训练 Vision Transformer 进行时空动作定位，将输出转化为结构化文本描述视频；同时用 LLM 驱动的编码代理处理基于行为学设计的查询，通过自定义解析库函数执行检索，降低 LLM 幻觉风险，提升可解释性。

Result: 在 Prompting-MammAlps 基准上，135 个生态相关查询和 775 个候选视频上达到 34% 的集合 F1 分数，而最佳零样本 VLM 仅 18%。

Conclusion: 所提方法在相机陷阱视频检索任务上显著优于零样本 VLM，且具备可解释性，为生态监测中的自动化视频检索提供了有效方案。
...
TL;DR: 本文提出一种时空亮度传感器融合协议，通过联合眼跳追踪与瞳孔光反射的双流挑战-响应框架，构建同步矩阵评估生物延迟交叉相关性，以抵御针对眼部生物特征的高级呈现攻击。


<details>
  <summary>Details</summary>
Motivation: 现有基于单一生理指标（如注视追踪或瞳孔光反射）的活体检测方法容易被独立欺骗，高分辨率视频回放和实时生成深度伪造攻击可轻松绕过静态检测。需要多模态联合验证机制来提高安全性。

Method: 提出时空亮度传感器融合协议：1) 生成随机时变视觉刺激（空间轨迹+亮度强度同时变化）；2) 构建名为"同步矩阵"的状态空间似然模型；3) 通过蒙特卡洛模拟评估平滑追踪与瞳孔收缩的预期生物延迟之间的连续互相关。

Result: 蒙特卡洛模拟证明该方法在理论上能够区分真实与模拟攻击条件；多轮挑战设计在存在非零渲染延迟差距时能有效检测生成式深度伪造攻击。

Conclusion: 为下一代动态欺骗防御提供了理论框架支撑，但需进行人类受试者验证后才能做出实际部署声明。
...
TL;DR: 提出ShapKO动态训练策略，利用Shapley值自适应调整模态淘汰概率，提升多模态医学模型在模态缺失时的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 多模态医学模型在输入缺失时性能下降，且训练中存在模态主导现象（过度依赖高预测性模态，忽视互补模态），现有静态淘汰方法无法适应训练过程中模态效用变化。

Method: ShapKO：周期性评估模态子集性能→计算Shapley值估计模态重要性→动态更新淘汰概率（高频抑制主导模态），无需修改架构。

Result: 在三个数据集（多任务临床分类、生存预测、癌症检测）上，ShapKO在模态缺失条件下持续提升性能，并产生可解释的掩码学习行为轨迹。

Conclusion: 自适应模态淘汰策略能有效缓解模态主导问题，提升多模态模型在临床实际中常见的不完整输入场景下的鲁棒性。
...
TL;DR: 本文提出Cat2Real多阶段对比学习范式，将产品识别重新定义为跨域检索问题，通过嵌入空间匹配解决真实店铺图像与商品目录之间的域差距，实现了对未见产品的零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 大规模零售智能中，真实店铺图像与商品目录图像之间存在严重的域差距（影棚精美照片 vs 噪声丰富的现场拍摄），导致传统分类方法难以扩展，需要一个能够高效匹配且具有零样本泛化能力的解决方案。

Method: 将产品识别重构为基于嵌入的跨域检索任务，提出Cat2Real多阶段对比学习框架，通过利用物品级和图像级相似性进行针对性难负样本挖掘，微调视觉主干网络来弥合域差距。

Result: 该范式能够无缝扩展到未见过的产品和类别，在完全没有真实世界训练图像的情况下也能展现出卓越的零样本泛化性能。

Conclusion: Cat2Real成功解决了零售领域产品识别的大规模扩展瓶颈，验证了基于对比学习的跨域检索方案在域差距显著场景下的有效性。
...
TL;DR: 本文对连续手语识别（CSLR）的强基线CorrNet进行了两个自然扩展：1）将BiLSTM时序头替换为Transformer编码器；2）在时序池化后注入运动线索（MotionGate模块）。实验发现Transformer未超越BiLSTM，MotionGate退化为恒等映射，表明CorrNet的相关性编码已足够有效，自然扩展需谨慎验证。


<details>
  <summary>Details</summary>
Motivation: 探究CorrNet在连续手语识别中的两个自然扩展（Transformer时序头、运动线索注入）是否有效，验证这些看似合理的架构改进能否提升性能。

Method: 在复现的CorrNet系统上进行两个扩展实验：1）将BiLSTM替换为Transformer编码器并调整训练策略；2）设计轻量级MotionGate模块在时序池化后注入运动线索。

Result: 1）Transformer头未超越BiLSTM基线，两者计算成本相当；2）MotionGate退化为恒等映射，门控失去运动选择性，注入的残差变为微弱非选择性扰动。

Conclusion: CorrNet的相关性编码已充分捕获运动信息，显式运动注入冗余；自然架构扩展在CSLR中需谨慎验证，不能假设其有效。
...
TL;DR: 本文提出Banshee，首个通过声学攻击干扰无人机云台系统，实现视觉跟踪目标切换的物理可实现攻击方法，仿真和实物实验均达到超过93%的成功率。


<details>
  <summary>Details</summary>
Motivation: 现有工作虽表明声学信号可干扰云台内部组件，但针对无人机视觉跟踪与跟随这类真实应用的攻击缺乏研究，且未解决现实攻击中目标运动不确定性和运行时延迟等挑战。

Method: Banshee精心构造声学波形，诱导云台产生优化的对抗性振荡，造成相机视野方向性偏移，破坏帧间目标关联，从而迫使跟踪器从原目标切换到攻击者选定的目标上。

Result: 仿真中在两种商用云台系统和五种跟踪器上达到93.6%成功率；真实场景下的台架实验和飞行黑盒攻击中综合成功率达95.5%。

Conclusion: 揭示了声学与视觉之间的跨域安全漏洞，强调云台系统及应用需要更鲁棒的设计。
...
TL;DR: UniPose9D提出了一种类别无关的9D物体姿态估计基础模型，无需类别标签、CAD模型或平均形状先验，仅需实例掩膜和RGB-D/RGB+深度图即可预测旋转、平移和尺寸，在六个数据集上达到或超越了专家方法。


<details>
  <summary>Details</summary>
Motivation: 现有物体姿态估计方法虽在现有基准上表现优异，但往往过拟合，对未见类别和新场景的泛化能力有限。作者希望构建一个无需类别标签、CAD模型或参考视图的通用姿态估计基础模型。

Method: 1) 从观测物体几何中采样点对，使用DINOv2和PointNet提取特征预测NOCS坐标；2) 提出基于点对的RANSAC N-hop Kabsch-Umeyama算法，带有自适应阈值；3) 使用Flow Matching处理对称歧义；4) 通过整理和配准公开数据集构建大规模训练集。

Result: 在六个数据集上的实验表明，单一统一模型能够匹配或超越专用方法的性能，同时对未见物体和野外场景具有良好的泛化能力。

Conclusion: UniPose9D证明了类别无关的姿态估计基础模型的可行性，为物体姿态估计提供了一种通用、泛化能力强的解决方案。
...
TL;DR: 本文提出AdaViG方法，通过模型内部信号（生成意图和视觉保真度）在早期阶段动态判断是否中止视觉生成步骤，从而解决统一多模态模型中视觉推理步骤可能引入错误证据和导致计算开销大的问题，在提升精度的同时大幅降低计算和延迟。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型（UMMs）在视觉数学推理中生成中间视觉步骤，但自生成的视觉步骤可能引入错误视觉证据误导后续推理，且频繁触发视觉步骤带来大量计算和内存开销，影响推理效率。

Method: 提出AdaViG，一种无需训练的适应性视觉门控方法。利用两个内部信号——生成意图（模型是否有具体的文本绘图计划）和视觉保真度（视觉生成是否忠于原始输入图像），在视觉生成的早期阶段动态评估每个触发的视觉步骤，当两个信号均较弱时中止生成，避免误导性视觉证据进入推理链条并减少不必要的计算。

Result: 准确率提升最高达5.7%，视觉生成FLOPs降低25.0%-91.0%，端到端延迟降低15.4%-45.6%。

Conclusion: 通过早期检测模型内部信号来判断视觉步骤是否有利，可以有效平衡UMMs中视觉推理的精度与效率，验证了"并非所有视觉步骤都有益"这一关键洞察。
...
TL;DR: FlashBEV提出了一种全新的执行策略，通过消除中间张量的显式物化，将采样式视图变换（Sampling-VT）的峰值GPU内存从O(BCXYZ)降低到O(BCXY)，实现了数量级的内存节省和推理加速，且精度完全无损。


<details>
  <summary>Details</summary>
Motivation: 现有的采样式视图变换（Tensorized Sampling-VT）尽管支持高分辨率、长距离的BEV感知，但其标准张量化实现会显式物化大量高度相关的中间张量，导致内存和延迟成本随垂直分辨率和相机数量增加而急剧膨胀，成为部署瓶颈。

Method: 从算子执行角度重新审视Tensorized Sampling-VT，发现其本质是gather-reduction模式：每个BEV查询独立地跨相机和高度bin累积贡献。基于此，提出FlashBEV——一种完全融合且IO感知的执行策略，通过线程本地累加和即时重计算替代中间张量的物化，在数学上等价但大幅减少全局内存访问和内核启动开销。

Result: 实验表明，FlashBEV相比标准实现实现了超过一个数量级的峰值GPU内存降低，推理延迟显著加速，且内存消耗与高度bin数量无关，算子峰值内存降至O(BCXY)（仅输出）。在固定部署预算下，支持更高的BEV范围/分辨率和垂直离散化。

Conclusion: FlashBEV的贡献在于执行层面的重新设计——相同数学计算，不同执行方式——消除了采样式VT在部署中的关键可扩展性障碍，使其在内存受限设备上可行。
...
TL;DR: 提出一种无需标签和传感器标定的两阶段跨模态匹配范式，通过蒸馏预训练和极线引导自蒸馏，实现事件相机与图像数据间的像素级对应，在MVSEC和TUM-VIE上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有跨模态匹配方法严重依赖标注数据或严格对齐的硬件，无法应用于无标签、无约束的真实场景。本文旨在解决无监督条件下事件与图像数据间的像素级对应问题。

Method: 两阶段训练范式：1）标签无关的蒸馏预训练：利用大规模数据，通过分布损失和对比损失优化目标，学习通用表示；2）极线引导自蒸馏：在无标签下游数据上，利用一致性验证筛选鲁棒匹配，结合极线几何先验的置信度引导模型自我进化。

Result: 在MVSEC和TUM-VIE姿态估计任务上达到SOTA。并基于TUM-VIE构建了新的跨模态评估基准（物理分离相机，不同内参和分辨率）。

Conclusion: 提出的无监督跨模态匹配方法有效克服了对标注数据和硬件标定的依赖，在真实未约束场景下性能优异，为事件与图像传感器融合提供了实用方案。
...
TL;DR: 提出CVKD-UDA方法，通过改变体素大小构造域相似表示，并利用跨视角知识蒸馏和特征解耦技术提升3D无监督域适应分割中自训练方法的表现。


<details>
  <summary>Details</summary>
Motivation: 现有3D UDA分割方法依赖源域监督或输出级对抗对齐来获取热身模型，但跨域差距大导致泛化性差和训练不稳定。构造域相似表示是弥合这一差距的有效手段。

Method: (1) 通过调整体素大小生成两个互补视图，引入跨视角知识蒸馏增强模型泛化能力和目标域感知；(2) 设计轻量级Decouple-Adapter和辅助模仿分类器，解耦跨视角知识转移以平衡可迁移性和判别性。

Result: 在两个基准数据集上的大量实验表明，CVKD-UDA有效提升了自训练方法的性能，为3D UDA分割提供了新视角。

Conclusion: CVKD-UDA通过体素大小设计和跨视角蒸馏，有效解决了大域差距下的热身模型初始化问题，提升了3D无监督域自适应分割的效果。
...
TL;DR: 提出EMBRACE多任务深度学习框架，实现从静态胚胎显微图像中同时进行细胞质碎片分割、发育阶段分类和卵裂球对称性分级，在9,137张标注图像上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 传统体外受精中胚胎目视评估受观察者变异影响，尤其当碎片区域小、不规则或对比度低时难以准确判断，因此需要自动化、客观的多任务评估方法。

Method: EMBRACE框架包含共享ResNet-50骨干网络、C-MSFF多尺度特征融合模块、U-Net风格分割解码器及两个分类头，联合完成碎片分割、发育阶段分类和对称性分级三个任务。

Result: 碎片分割Dice 0.781、IoU 0.677；发育阶段分类准确率0.995、macro-F1 0.994、AUC 1.000；对称性分级平衡准确率0.901、macro-F1 0.907、QWK 0.859。

Conclusion: 证明将空间可解释的碎片定位与胚胎形态评估整合到单一框架是可行的，但仍需外部和前瞻性验证方可临床部署。
...
TL;DR: 本文介绍了Lensless Face Dataset (LFD)，一个包含21,080张无透镜相机采集的人脸数据集，涵盖不同光照、角度和距离下的原始测量数据、重建图像和标准图像，旨在解决无透镜成像中人脸识别的数据集缺失问题。


<details>
  <summary>Details</summary>
Motivation: 传统人脸识别依赖传统相机，存在体积大、成本高、隐私保护有限等问题。无透镜相机虽具优势，但重建图像常有伪影，现有方法难以泛化到真实场景，且缺乏专门针对无透镜图像伪影的人脸数据集。

Method: 构建LFD数据集，包含21,080张图像，涵盖三种不同类型无透镜相机的采集数据，包括室内外多种光照、角度和距离条件下的原始测量、重建图像和标准人脸图像。

Result: 通过全面评估和分析，证明LFD能够有效捕捉不同无透镜成像设备的共享特征和伪影，可作为推进无透镜人脸识别研究的重要数据集。

Conclusion: LFD数据集填补了无透镜人脸识别领域的数据空白，通过提供多样化的真实世界数据和多种硬件平台，有助于推动无透镜人脸识别技术的泛化能力和实际应用。
...
TL;DR: DynaFilter是一种动态过滤技术，让卫星边缘设备在不解压的情况下，直接在压缩域中根据云端的语义查询进行感兴趣区域（RoI）的识别和选择性传输，大幅减少数据传输量、降低能耗并加速推理。


<details>
  <summary>Details</summary>
Motivation: 卫星边缘系统带宽有限且连接不稳定，现有方案要么需要完全解压后进行预处理，要么无差别地传输所有压缩数据，导致资源浪费。

Method: 利用低层压缩语法（JPEG中的DC系数/AC能量、视频流中的运动向量）与高层语义查询之间的强相关性，建立压缩域特征与云查询语义的精确映射，实现不解压的RoI推理和数据筛选。

Result: 图像方面减少1.6x-7.1x解码像素数据量；视频流节省92.0%带宽；降低43.1-88.6%设备能耗；实现1.6x-3.0x推理加速。

Conclusion: DynaFilter通过压缩域动态过滤策略有效解决了卫星边缘场景下的带宽和能耗瓶颈，证明了低层压缩语法与高层语义之间可建立有效映射。
...
TL;DR: 提出WeaveEarth，一种无需训练的高分遥感图像理解框架，通过结构化证据构建与推理，在有限计算预算下实现全局-局部联合推理，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有UHR遥感图像理解方法存在被动感知丢失细节、主动感知高延迟与误差累积的问题，亟需一种更高效的路径来平衡全局场景与局部细节的捕捉。

Method: 1) 全局感知证据构建：选择紧凑、低冗余、空间互补的最小支持证据集；2) 结构化证据推理：将局部证据、空间元数据与相对拓扑整合为统一推理接口，实现全局-局部联合推理。

Result: 在多个UHR遥感基准和多种冻结VLM骨干上，WeaveEarth一致优于强基线和现有UHR方法。

Conclusion: UHR图像理解的有效路径不在于获取更多信息，而在于更好地组织信息。WeaveEarth通过无需训练的结构化方式验证了这一观点。
...
TL;DR: 提出TextGaze，一种利用大视觉语言模型(LVLM)作为语义指导的统一跨模态架构，用于视线目标估计，在无需额外微调的情况下实现了具有竞争力的性能和跨数据集泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法中，多分支设计需要额外标注，增加负担且阻碍领域迁移；而精简设计则更关注低层视觉显著性而非真实注视意图，导致预测注意力与真实注视目标不对齐。

Method: 提出TextGaze，使用冻结编码器提取视觉特征，利用LVLM获取与视线对齐的文本线索，设计基于Transformer的融合模块配合分层文本监督，轻量化解码头联合预测视线热图和帧内/帧外状态。

Result: 在四个主流数据集上评估，关键指标上表现具有竞争力，且无需额外微调即可实现稳健的跨数据集泛化。

Conclusion: 提供了一种替代传统设计的精简方案，突显了LVLM作为视线估计中可访问的辅助指导的潜力。
...
TL;DR: FlowPainter提出了一种置信度引导的软修复扩散光流框架，通过轻量网络预测粗糙流和置信度掩码，将可靠简单区域与困难区域区分，实现更高效稳定的扩散去噪，在Sintel、KITTI、Spring等基准上达到强精度。


<details>
  <summary>Details</summary>
Motivation: 现有扩散光流方法对整个稠密流场（包括简单区域）从高斯噪声去噪，增加了去噪负担，导致收敛慢、训练不稳定。作者希望将简单区域的可靠判别先验与扩散模型的生成能力结合，降低不必要的计算开销。

Method: 1) 轻量置信度感知网络预测粗糙光流和像素级置信度掩码，区分可靠简单区域与不确定困难区域；2) 基于置信度的初始化将简单流先验注入扩散过程；3) 置信门控残差引导实现迭代去噪中对简单区域的约束；4) 动态衰减引导强度，稳定早期去噪并保留晚期细节精修灵活性。

Result: 在Sintel、KITTI、Spring等公开基准上，FlowPainter在可比训练设置下达到强精度，比现有扩散光流方法收敛更高效，在具有挑战性的数据集子集上提升显著。

Conclusion: FlowPainter提供了一种实用方式，将可靠的判别先验与扩散精修相结合用于光流估计，代码已公开。
...
TL;DR: 提出REVA-PO框架，通过动态KL权重调节（RER）和验证锚点策略重置（VAPR）稳定强化学习训练，在胸部X光报告生成任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有RL训练因固定KL正则化和静态参考策略导致训练不稳定、探索受限，累积KL压力随时间增大。

Method: 1) RER：基于优势值和参考策略熵动态调整每条响应的KL权重；2) VAPR：周期性地将参考策略和当前策略同步到最佳验证检查点；3) 三阶段训练流程：热身训练→分类器引导的SFT→RL。

Result: MIMIC-CXR上BLEU-4提升5.1%，IU-Xray上提升3.6%；CheXpert F1提升4.5%，RadGraph F1提升12.8%。

Conclusion: REVA-PO有效解决了RL训练中的稳定性和探索问题，在语言质量和临床准确性上均取得新标杆。
...
TL;DR: 本文提出EmoStyle框架，通过LLM预测情感线索并编码为条件向量注入扩散模型，结合风格专用LoRA适配器，实现情感感知的艺术图像生成，在AffectiveArt Challenge 2026 Track 1中获第一名。


<details>
  <summary>Details</summary>
Motivation: 训练数据中的视觉和情感属性在测试时不可用，导致生成器难以同时控制"画什么"和"如何表达情感"，存在测试提示与细粒度情感条件之间的控制鸿沟。

Method: 1) LLM推理器预测情感线索(效价-唤醒度、主导情绪、治疗效果标签)和宽高比；2) 将情感场编码为情感条件向量，通过AdaLN调制注入去噪块；3) 为每种艺术风格桶训练专用LoRA适配器；4) 轻量级VLM引导的候选选择步骤进行排序筛选。

Result: 在AffectiveArt Challenge 2026 Track 1中获得第一名。

Conclusion: EmoStyle通过结构化生成状态和风格专用LoRA有效桥接了情感感知艺术生成中的控制鸿沟，验证了将情感线索编码为条件向量直接引导特征生成的有效性。
...
TL;DR: BiLoG-Net是一个用于乳腺钼靶图像中肿块分割与恶性分类的联合深度学习框架，通过双上下文位置感知特征建模和分割引导注意力机制，在CBIS-DDSM和INBreast数据集上取得了Dice 94.20%/93.10%、分类准确率95.20%/93.60%、AUC 97.10%/96.00%的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 乳腺癌是女性最常见的恶性肿瘤，但钼靶图像中的肿块检测因强度变化细微、组织密度不均、边界模糊等问题而极具挑战性，现有方法难以同时兼顾精确分割与可靠分类。

Method: 提出BiLoG-Net，包含Fire-based特征提取、轻量级全局与局部特征增强模块、自适应位置感知门控机制，以及分割引导的注意力机制；采用紧密耦合的多任务设计，实现像素级定位与图像级诊断的相互强化。

Result: 在CBIS-DDSM上Dice 94.20%、分类准确率95.20%、AUC 97.10%；在INBreast上Dice 93.10%、分类准确率93.60%、AUC 96.00%，大幅超越现有CNN和Transformer基线。

Conclusion: BiLoG-Net以单一端到端模型实现了精确边界描绘与可靠恶性评估的统一，具备在临床计算机辅助检测系统中辅助放射科医生优先处理可疑病例、提升筛查效率的潜力。
...
TL;DR: 本文提出 ScratNet，一种结合改进 Swin Transformer 与定制解码器的端到端划痕分割网络，通过多尺度膨胀聚合、主干集成模块和边界锐化分支，有效解决了半导体制造中不规则、低对比度划痕缺陷的检测难题。


<details>
  <summary>Details</summary>
Motivation: 半导体制造中表面划痕缺陷具有形状不规则、对比度低、尺度多变等特点，传统检测方法和基于 CNN 的深度学习方法难以可靠捕获细粒度边缘细节。

Method: 提出 ScratNet 框架，包含：1) 改进的 Swin Transformer 主干网络；2) 多尺度膨胀聚合模块捕获局部与全局上下文；3) 主干集成模块恢复空间细节；4) 精度优化分支利用各向异性卷积增强边界清晰度。

Result: 大量实验表明 ScratNet 在检测细薄和不规则缺陷上显著优于现有方法，提供了可扩展且鲁棒的自动化划痕检测方案。

Conclusion: ScratNet 通过阶段自适应特征聚合与边界感知精炼，在高精度制造中实现了更优的划痕分割性能。
...
TL;DR: 本文提出PhenoEmbed，一种自监督的树冠时间嵌入模型，在18日期UAV多光谱时间序列基准HeideBench上训练，利用对比学习和掩码重建目标学习树冠的季节性表现变化，生成256维嵌入向量，在5,885个树冠上实现了紧凑的最近邻结构。


<details>
  <summary>Details</summary>
Motivation: 树冠是鲁棒AI的挑战性目标，因为其光谱响应、内部纹理、半透明度和表观边界在生长季节中发生显著变化，现有方法难以应对这种季节性动态。

Method: 提出PhenoEmbed自监督冠层时间嵌入模型，采用对比学习和掩码重建目标在HeideBench基准上训练；以分割的树冠多边形为锚点，提取对齐的冠层中心作物序列，为每棵树学习256维向量。

Result: 在5,885个树冠上，嵌入在低维空间中结构良好，前两个主成分解释25.1%方差，最近邻检索中位top-1余弦相似度0.946；相比手工特征和均值池化基线，具有更紧凑的最近邻结构。

Conclusion: PhenoEmbed可作为可重用的森林冠层表示学习器，未来可测试这些特征是否能在季节性变化下改进树级模型。
...
TL;DR: 提出CoSAG，通过解耦语义场构建与存储，将每个3D高斯绑定到空间锚定表，并利用空间预测熵编码实现亚MB级存储，在性能匹敌/超越SOTA的同时将场大小缩小37-76倍。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D场景理解方法（如CLIP嵌入3D高斯）每个高斯附着高维特征导致场景膨胀至GB级别，存储成为瓶颈；已有紧凑方法未压缩代价最高的逐高斯分配（per-Gaussian assignment），且将场构建与存储纠缠在一起。

Method: 1）解耦构建与存储：无需逐场景训练，通过闭合形式的透射加权提升（transmittance-weighted lift）构建语义场；2）空间锚定语义锚点（spatially grounded semantic anchors）和多视图去噪；3）存储采用空间预测熵编码（spatially predictive entropy coder），无需解码器，利用锚点的空间可预测性压缩逐高斯绑定。

Result: 在2D渲染、3D选择和密集LSeg三个协议上达到或超越SOTA，相比LangSplatV2场大小缩小37-76倍，实现亚MB级存储。

Conclusion: CoSAG证明逐高斯绑定是可高度压缩的，通过空间预测编码和解耦设计，在保持高精度的同时极大压缩存储，为开放词汇3D场景理解的实际部署提供了可行方案。
...
TL;DR: 本文提出了Dynamic Affective Reasoning (DAR)，首个面向观众情感变迁与因果推理的大规模视频基准，包含15,087个视频和36,908条情感标注，并提出了DAR-R1两阶段框架（SFT+GRPO），在多个多模态大模型上取得了动态情感推理的最佳效果。


<details>
  <summary>Details</summary>
Motivation: 现有视频情感分析将每个片段视为独立静态分类单元，忽略了情感因连续因果事件累积变化这一心理学事实。

Method: 构建DAR基准（15,087视频、36,908情感片段、27类情感），提出DAR-R1两阶段框架：监督微调（SFT）+ 组相对策略优化（GRPO），定义三项任务：情感分割、细粒度情感分类、情感推理。

Result: 在10+多模态大模型上，DAR-R1在情感定位和情感推理两方面均达到最先进水平。

Conclusion: 以观众为中心视角、精细情感标注与因果推理链相结合的动态情感分析范式，优于传统静态分类方法。
...
TL;DR: 当前短答案VQA评测将语义正确性与表面形式匹配混为一谈，导致大量语义正确但因形式不匹配而被误判为错误的答案。通过人工验证的语义评判器对37k+错误进行审计，发现文本密集型基准中高达一半的错误是表面形式不匹配导致的，且可提取性和多跨度答案对评测器更敏感。提示和上下文重写会进一步扰动结果。建议官方得分应附带语义审计和答案类型诊断。


<details>
  <summary>Details</summary>
Motivation: 揭示短答案VQA评测中语义正确性与表面形式匹配之间的混淆问题，量化其对模型评估结果的影响。

Method: 使用人类验证的语义评判器（97.6%精确率）审计6个视觉语言模型在6个基准上的37k+官方错误；使用纯文本评判器复现基准级假阴性模式；分析答案类型（可提取性、多跨度vs标量）对评测敏感性的影响；进行提示和上下文重写扰动实验；实施确定性CPU-only契约修复。

Result: 文本密集型基准中高达一半的错误是表面形式不匹配导致的语义可接受答案；可提取性和多跨度答案比标量答案对评测器更敏感；提示和上下文重写显著改变项目级正确性；部分欠计数可通过契约修复恢复。

Conclusion: 官方短答案VQA得分应附带语义审计和答案类型诊断以保持可解释性。
...
TL;DR: 论文提出了一种几何感知的分类框架，通过Gaussian expert模块从预训练的视觉-语言特征中提取结构先验，并嵌入轴向自注意力机制，用于宫颈细胞学图像分类，在Mendeley和SIPaKMeD数据集上分别达到99.48%和96.08%的准确率。


<details>
  <summary>Details</summary>
Motivation: 宫颈细胞学图像分类是自动化宫颈癌筛查的关键，但现有方法中卷积模型缺乏长程关系建模能力，注意力模型缺乏显式结构引导，导致难以处理细胞形态复杂、类内变异细微、类间相似性强的问题。

Method: 提出几何感知分类框架：1) 利用预训练视觉-语言特征提取全局语义信息；2) 设计Gaussian expert模块生成轴方向先验，捕获细胞核排列和空间组织等结构规律；3) 将先验嵌入轴向自注意力模块，沿水平和垂直方向调制相似性计算，增强长程依赖建模和结构敏感的特征交互。

Result: 在Mendeley液基细胞学数据集上准确率达99.48%，在SIPaKMeD数据集上准确率达96.08%，在召回率、精确率和整体分类性能上均有平衡提升。可视化分析显示学习到的先验能突出诊断相关的细胞区域。

Conclusion: 该方法有效融合了结构先验与注意力机制，提升了宫颈细胞学图像分类的准确性和可解释性，可作为面向筛查的决策支持工具。
...
TL;DR: InterPet4D是首个大规模多模态人宠（人狗）交互数据集，包含680万帧、13只狗、23人，并提出InterPetMoGen运动生成框架，FID达11.21，显著优于基线。


<details>
  <summary>Details</summary>
Motivation: 人宠交互估计与生成因缺乏高质量大规模数据集而研究不足。

Method: 构建InterPet4D数据集：多视角同步捕捉系统记录人狗服从任务，提供多视角/第一人称视频、分割、2D/3D关键点、网格、音频等标注。提出InterPetMoGen框架用于交互运动生成。

Result: InterPetMoGen实现FID 11.21，大幅超越Seq2Seq和DiT基线。

Conclusion: InterPet4D能有效支持真实的类人宠交互建模。
...
TL;DR: 本文提出SESAD框架，通过结构化证据选择和几何判别模块，在弱监督视频异常检测中有效解耦场景与行为语义，在三个基准数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督视频异常检测方法容易依赖场景相关统计线索而非真实行为偏差，导致检测性能不稳定；真实视频中异常行为的视觉外观和时间跨度变化大，且场景外观与动作动态紧密纠缠。

Method: 提出结构化证据选择框架(SESAD)，将异常检测重构为基于片段级视觉证据的结构化推理过程；通过场景和动作约束进行上下文条件选择，自适应增强异常相关语义并抑制场景干扰；引入轻量级几何判别模块，在嵌入空间构建双原型结构，通过相对几何关系进行异常判别。

Result: 在UBnormal、ShanghaiTech和UCF-Crime数据集上分别达到67.92、97.99和88.46 AUC，同时保持高计算效率和稳定的异常判别性能。

Conclusion: SESAD通过结构化的证据选择机制有效解决了弱监督下场景与行为语义纠缠的问题，在性能和稳定性上均优于现有方法。
...
TL;DR: 本文提出图表同步编辑任务(VLCE)，构建ChartSync基准(870个三元组,9类图表,4种任务类型)，评估14个图像编辑模型和1个代码管线，发现仅前沿闭源模型具备初步VLCE能力，开源模型在几何同步上严重不足。


<details>
  <summary>Details</summary>
Motivation: 现有生成式图像编辑模型在处理结构化统计图表时，无法应对需要几何同步的数据修改。传统方法局限于局部文本替换，缺乏对依赖感知的级联更新的支持。

Method: 形式化定义"视觉-逻辑级联编辑"(VLCE)任务；通过程序化渲染管线构建ChartSync基准(870个三元组，含235个几何耦合VLCE实例)；设计两阶段评估框架(客观视觉指标+视觉语言模型评判)。

Result: 14个图像编辑模型和1个代码管线中，多数开源模型在几何同步上严重下降，仅2个前沿闭源模型展现出初步VLCE能力，但存在语义隔离和背景损坏等问题。

Conclusion: 图表级联编辑任务对现有模型构成显著挑战，错误分析揭示了核心元能力缺口，为未来多模态架构设计提供指导。
...
TL;DR: 本文提出VVM-Tuning训练框架，通过模态合成与模态上下文让大多模态模型（LMMs）学会从不同视觉模态中解耦不变语义与模态特征，实现零样本泛化到未见过的视觉模态；同时发布VVM-Bench基准，在6种真实/合成模态上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有LMMs在RGB视觉上表现优异，但泛化到未见过的视觉模态（如深度图、热成像等）的能力尚未被充分探索。作者认为不同视觉模态是对同一物理世界的不同采样，因此模型需要同时具备模态无关的语义感知能力和模态自适应的灵活性。

Method: 1）模态合成：从RGB场景生成外观多样的图像，训练模型解耦不变语义与视觉外观变化；2）模态上下文：在提示词中引入模态描述，通过指令微调将外观变化映射回模态相关属性，实现零样本适应未见模态。

Result: 在5个模型上实验，仅通过合成模态训练，模型在真实世界和新型合成模态上均展现出持续的性能提升，无需在目标模态上进行训练。

Conclusion: VVM-Tuning能有效提升LMMs在未见视觉模态上的泛化能力，为多模态模型的跨模态泛化提供了可行的训练范式。
...
TL;DR: 提出CloakDiff框架，利用扩散模型和可逆网络实现对VLM文本查询攻击的高保真、可逆隐私保护。


<details>
  <summary>Details</summary>
Motivation: 现有可逆对抗样本在纯视觉任务有效但无法应对多模态场景，而VLM上的对抗样本依赖高频噪声严重降低视觉质量，亟需高保真可逆的隐私保护方法。

Method: 结合扩散对抗编辑与可逆网络嵌入原始图像实现无损恢复；扰动像素嵌入并操纵交叉注意力图保证跨模型/跨提示迁移性；设计EDM启发式采样策略优化对抗引导。

Result: 在多个数据集和VLM上验证，CloakDiff实现了多模态隐私保护，同时保持高视觉质量和可逆性。

Conclusion: CloakDiff是首个针对VLM文本查询攻击的可逆高保真隐私保护框架，有效兼顾安全性、视觉质量与可恢复性。
...
TL;DR: 本文提出GRC-ProbNet，利用深度集成生成多个分割掩码并提取不确定性特征，用于心血管疾病分类，在MM-WHS和ASOCA数据集上AUROC达92.92%，优于基线GRC-Net的91.25%。


<details>
  <summary>Details</summary>
Motivation: GRC-Net使用确定性分割掩码，忽略了心脏解剖固有的模糊性，需要引入不确定性建模来提升CVD分类的鲁棒性和性能。

Method: 采用深度集成（deep ensemble）为每个输入生成多个分割掩码，从中提取多种不确定性特征，并分析这些特征与分割误差的相关性及其对下游CVD分类的影响。

Result: 在MM-WHS和ASOCA数据集上，GRC-ProbNet的CVD分类AUROC达到92.92%，显著优于GRC-Net的91.25%。发现最能反映分割质量的不确定性特征不一定对下游分类贡献最大。

Conclusion: 利用不确定性特征能有效提升CVD分类性能，且分割质量与分类任务对不确定性特征的偏好存在差异，值得分别考虑。
...
TL;DR: 该论文通过统一基准测试评估了15种基础模型在乳腺X光图像外部域迁移下的鲁棒性，发现乳腺专用视觉-语言模型表现最佳但并非完全鲁棒，数据集层面的OOD评估应作为核心评价标准。


<details>
  <summary>Details</summary>
Motivation: 基础模型越来越多地被用作乳腺X光图像的特征提取器，但它们在外部域迁移下的鲁棒性尚不明确，现有缺乏系统性的跨数据集评估。

Method: 采用统一冻结骨干+线性探测协议，在3个源数据集上训练，并在12个任务兼容的OOD数据集上评估15种基础模型，涵盖乳腺密度、BI-RADS严重程度和癌症状态三个任务，并进行了特征空间分析。

Result: 乳腺专用视觉-语言模型(Mammo-FM、MaMA)在平均OOD性能上最强，但鲁棒性不完全由乳腺数据暴露解释；DINOv3是竞争力强的视觉基线；乳腺适应预训练未一致提升泛化能力；领先模型在不同数据集上表现不均；特征空间保留临床信号的同时仍包含数据集和采集结构信息。

Conclusion: 数据集层面的OOD评估应作为评估乳腺X光表示质量的核心标准。
...
TL;DR: 提出GradSkip方法，通过自适应注意力头加权和跳跃感知传播，提升了ViT的可解释性，在ImageNet1K和BloodMNIST上达到最先进忠实度，且计算量减少14倍以上。


<details>
  <summary>Details</summary>
Motivation: 现有ViT可解释方法未充分考虑注意力头重要性不均和残差连接的特性，导致相关性传播不准确。

Method: 提出GradSkip方法，包含自适应头部加权（为不同注意力头分配不同重要性权重）和跳跃感知传播（动态分配注意力路径与残差路径之间的相关性）。

Result: 在ImageNet1K和BloodMNIST上达到最先进忠实度，计算量比最优方法降低14倍以上；分割任务中定位和区域对齐效果更优。

Conclusion: GradSkip通过更精细的架构感知传播机制，有效提升了ViT可解释性，兼顾效果与效率。
...
TL;DR: 提出了一种跨异构骨骼的运动混合框架，结合语义编码器和扩散解码器，通过隐空间插值实现不同骨架拓扑间的平滑运动合成。


<details>
  <summary>Details</summary>
Motivation: 现有运动混合方法受限于固定或近似的骨骼拓扑结构，无法处理异构骨架（不同骨骼结构）之间的运动混合，限制了动画制作的灵活性。

Method: 设计语义编码器提取逐帧运动隐表示，结合扩散解码器根据隐条件重建特定角色的运动；推理时通过对两个输入运动的隐表示进行插值来实现混合。

Result: 在Truebones Zoo数据集上，针对相同和不同骨骼拓扑的运动进行了训练和评估，展示了在多种场景下实现平滑且合理的运动混合能力。

Conclusion: 该方法成功突破了传统运动混合对骨架拓扑一致性的限制，为异构骨骼间的运动合成提供了有效方案。
...
TL;DR: 本文提出ABot-N1，一种视觉语言导航基础模型，通过解耦认知与控制的慢-快架构（慢速视觉-语言推理器+快速动作专家），利用像素目标作为通用接口，在多种导航任务中实现鲁棒、可泛化且可解释的导航，在城市级导航中POI到达率提升35.0%，达到77.3%，并在复杂场景中取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言导航基础模型通常采用端到端的单一策略，将观测直接映射到动作，存在坐标漂移、无法处理长尾语义、缺乏可解释性等问题，难以同时实现通用性、鲁棒性和透明性。

Method: 提出慢-快（slow-fast）双流架构：慢速视觉-语言推理器进行显式CoT推理并生成像素目标（pixel goal），作为通用接口；快速动作专家利用文本线索和像素引导，以原生控制频率生成连续航点。通过像素锚点与显式语言轨迹桥接高层意图与低层控制。

Result: 在城市级导航中POI到达率提升35.0%（达77.3%），室内外复杂场景成功率达95.4%/92.9%；在物体到达、人员跟随、指令跟随等任务中保持优越鲁棒性；开源了新Point-Goal/POI-Goal基准。

Conclusion: ABot-N1通过解耦认知与控制的架构设计，实现了通用、鲁棒且可解释的视觉语言导航，在多项基准上达到SOTA，为城市级导航领域提供了新的方向。
...
TL;DR: 本文挑战ViT仅使用最后一层的惯例，提出"可恢复性"概念，发现中间层能纠正最后一层的错误。作者设计VFusion方法，通过可学习映射将内部层次特征融合到低维潜在空间，在分布内和分布外均优于传统集成方法，缩小了与理论最优性能的差距。


<details>
  <summary>Details</summary>
Motivation: ViT虽富含中间表征，但实践中仅将最后一层用于下游任务，忽视了中间层蕴含的潜在信息。作者希望探究中间层能否纠正最后一层的错误预测。

Method: 提出VFusion，一种垂直聚合策略，通过可学习映射将ViT内部各层的特征融合到一个低维潜在空间，实现跨层特征的合成与选择。

Result: 在16个数据集上，中间层探针能纠正18%-76%的最后一层错误分类。VFusion在分布内和分布外设置中均显著优于现有聚合基线，缩小了45%的最佳单层与理论最优之间的精度差距。

Conclusion: ViT内部层次存在冗余-正确性对应关系，VFusion提供了一种鲁棒且高效的水平集成替代方案，并在不同模型大小和预训练设置下保持泛化能力。
...
TL;DR: SSMatte 是一个自监督框架，首次实现了无需任何人工标注的自动抠图，性能与全监督方法相当。它通过语义锚定（利用冻结的自监督ViT特征）和细节抠图（基于不动点损失）两个模块来分解问题。


<details>
  <summary>Details</summary>
Motivation: 高质量alpha抠图标注成本极高，现有弱监督方法仍需逐像素标注（trimap/mask），限制了可扩展性和泛化能力。作者首次探索能否仅用RGB图像、无需任何人工标注训练自动抠图模型。

Method: 提出SSMatte自监督框架：(1) 语义锚定——从冻结的自监督ViT特征中，通过基于广义Rayleigh商的新型语义锚定损失传播类token种子，生成语义抠图提示；(2) 细节抠图——该提示引导细节抠图网络，通过基于不动点（fixed-point）的损失函数优化，保证alpha-RGB一致性。

Result: 在肖像数据集上，SSMatte超越现有弱监督方法，性能与全监督模型持平；展现出良好的数据扩展性和泛化能力。

Conclusion: 该工作将自动抠图推向了一个全新、完全无需标注的范式，证明了无监督条件下达到全监督性能是可行的。
...
TL;DR: SynthDocBench是一个全合成的长上下文视觉文档理解基准，通过系统控制文档长度、布局结构、模态组合和问题类型等因子，揭示了现有VLM在长文档理解中的三个失败模式：长度增加时性能急剧下降、文档中间三分之一区域的位置敏感性、以及长文档中图表理解的崩溃。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在视觉文档理解基准上表现良好，但真实文档涉及长度、布局复杂度、模态和问题难度等多个因子交织，难以将模型失败归因到具体原因。作者希望构建一个能系统分离并控制这些因子的基准，以揭示模型的真实失败模式。

Method: 采用组合设计（combinatorial design），每个因子独立变化；通过LLM流水线端到端生成六种布局原型的文档，并引入40%的随机覆盖防止模型利用虚假相关性；生成了比现有基准更长、结构更多样的长上下文文档。

Result: 评估了7个前沿VLM，发现三个现有基准无法发现的失败模式：1) 文档长度增加时性能急剧下降；2) 5/6模型对文档中间三分之一区域存在系统性位置敏感性，5/6模型呈现负向Early-to-Late趋势（最大降幅8.3个百分点）；3) 长文档中图表理解能力崩溃。

Conclusion: 当前VLM可能过拟合于现有基准的伪影，并未真正实现鲁棒的长上下文视觉文档理解能力。
...
TL;DR: 提出了一种结合Tiny Vision Transformer和病理感知原型蒸馏的自我监督病理表征学习框架(TVT-PAPD)，通过可学习的病理原型库发现和保留代表性组织形态模式，在TCGA和IPD-Brain数据集上实现了优异的胶质瘤分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有自我监督学习方法主要学习通用视觉特征，未能显式捕捉对疾病表征至关重要的病理特异性形态模式。

Method: 提出Tiny Vision Transformer与病理感知原型蒸馏(TVT-PAPD)框架，包含可学习的病理原型库模块，用于发现和保留代表性组织形态模式，促进语义相似的病理区域学习一致且判别性强的表征。

Result: 在TCGA LGG/GBM数据集上达到93.02%的加权F1分数，在IPD-Brain数据集上达到90.23%的加权F1分数，并展现出强跨队列泛化能力。

Conclusion: TVT-PAPD在保持90M参数的计算效率的同时，有效增强了病理感知的特征学习能力，并在胶质瘤分类任务中取得优异性能与泛化性。
...
TL;DR: 本文提出了一种基于条件变分自编码器（cVAE）的生成模型，用于生成逼真且无碰撞的3D人体间紧密交互动作，通过自动数据增强和基于体积代理的自监督碰撞损失解决了数据稀缺和碰撞感知两大挑战。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以生成逼真且物理正确（无碰撞）的3D人体间紧密交互动作，主要受限于数据稀缺和生成方法缺乏碰撞感知能力。

Method: 1）采用条件变分自编码器（cVAE），以一个人体姿态为条件生成另一个人的交互姿态；2）提出自动化监督数据增强策略，生成合成但逼真的交互姿态数据；3）提出基于体积代理（volumetric proxies）的自监督碰撞解决损失，确保生成姿态物理正确。

Result: 生成了比现有方法更多样、合理且物理正确的紧密交互动作，现有最先进方法无法达到此效果。

Conclusion: 该生成模型能有效解决数据稀缺和碰撞问题，实现可控、多样且物理正确的紧密人体交互生成。
...
TL;DR: 本文提出SPORT框架，通过解耦原型为共享与视图特定组件、结构感知对比学习及混合缺失视图插补策略，解决了现有原型基不完全多视图聚类中忽略视图特定信息、缺乏聚类级关系保持、插补不准确三大问题。


<details>
  <summary>Details</summary>
Motivation: 现有原型基不完全多视图聚类方法存在三个局限：1) 仅关注跨视图原型一致性，忽略原型中的视图特定信息，限制了多视图表达能力；2) 基于实例级对比学习，未能保持聚类级关系结构；3) 缺失视图插补仅依赖全局原型，未考虑局部几何邻域结构，导致恢复不准确。

Method: 提出SPORT框架：1) 将原型解耦为正交的共享组件（对齐以捕获共识语义）和视图特定组件（去相关以保留互补信息）；2) 引入结构感知对比学习，显式建模跨视图学习的聚类级关系；3) 混合插补策略结合全局原型匹配与局部邻域匹配，联合利用语义原型和流形结构恢复缺失视图。

Result: 在六个基准数据集上，SPORT在不同缺失率下均取得优于现有最先进方法的聚类性能。

Conclusion: 通过解耦原型、结构感知对比学习和混合插补，SPORT有效解决了现有方法的三大局限性，显著提升不完全多视图聚类效果。
...
TL;DR: 本文提出BOCCHI真实模糊检测基准和MSDCT-UNet频率感知网络，通过多尺度DCT先验解决局部运动模糊检测中的梯度捷径问题，在域内和跨数据集迁移上均取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有模糊检测基准存在梯度捷径（sharp区域与blur区域梯度分布不重叠），导致模型学到的特征无法泛化到真实场景。

Method: 1）构建BOCCHI真实拍摄基准，确保清晰区域与模糊区域的梯度分布重叠，消除梯度捷径；2）提出MSDCT-UNet编码器-解码器结构，通过DCT Attention和FiLM注入多尺度DCT先验，增强频率感知能力。

Result: MSDCT-UNet在BOCCHI上域内mIoU和边界定位均排第一；仅用633张BOCCHI训练图像训练的模型在跨数据集迁移上优于所有其他训练源。

Conclusion: BOCCHI基准有效消除了梯度捷径，MSDCT-UNet通过频率先验显著提升了模糊检测的泛化能力，少量真实数据即可超越大规模合成数据训练的效果。
...
TL;DR: 本文提出使用自监督的有限标量量化(FSQ)点云自编码器，通过单一几何token替代传统的类别标签和姿态标注进行3D场景合成，实验表明该token能同时编码类别和方向信息，且可通过训练目标控制旋转内容的编码。


<details>
  <summary>Details</summary>
Motivation: 传统3D场景合成依赖两个人工标注通道（类别标签和规范姿态），本文探索是否可以用单个自监督token替代两者，从而减少对人工标注的依赖。

Method: 采用有限标量量化(FSQ)点云自编码器，在3D-FUTURE家具数据集上进行Chamfer距离训练，无需任何标签或姿态标注。通过诊断探针恢复细粒度类别、超类别和偏航角信息。

Result: 从编码中可恢复出细粒度类别(62.6%)、超类别(85.6%)和偏航角(52.7度)信息。通过改变Chamfer目标（从旋转到未旋转点云）可消融旋转信号同时提升类别恢复。在ShapeNet上的迁移效果呈现类别依赖性。

Conclusion: 单个自监督几何token可同时编码类别和姿态信息，其旋转内容的编码可通过训练目标控制，但在不同数据集间的迁移存在类别依赖性问题。
...
TL;DR: 本研究通过自由颜色命名实验，考察了红、黄、绿三种颜色类别在不同明度和饱和度下的命名一致性。结果显示，绿色命名一致性最高，黄色最不稳定（常被描述为金色或棕色），红色居中。表明不同颜色类别在知觉空间中的范围广度存在差异。


<details>
  <summary>Details</summary>
Motivation: 颜色命名是人类颜色感知的重要组成部分，但颜色类别之间的边界往往不清晰，且不同饱和度和亮度会影响颜色感知。现有研究对颜色命名一致性的系统比较尚不充分，因此有必要探究不同颜色类别在视觉变化下的命名稳健性。

Method: 从COLIBRI数据集中选取18个颜色样本（红、黄、绿不同色调），招募92名参与者，在哈萨克语、俄语或英语中进行自由颜色命名（无预设类别限制），分析各颜色类别的命名一致性。

Result: 绿色类别在不同视觉变化下始终被一致命名为绿色；黄色类别获得的名称差异较大，常出现"金色""棕色"等描述；红色类别的命名一致性处于中等水平。

Conclusion: 不同颜色类别在知觉空间中占据的范围广度不同，有些类别（如绿色）对视觉变化更稳健。研究结果可用于开发更具感知合理性的颜色模型与颜色命名系统。
...
TL;DR: 本文构建了包含8,204帧对的真实光流评估基准FlowFactor，发现现有模型在合成基准（Sintel、KITTI、Spring）上的性能与真实场景准确性关联较弱，且光照变化和大位移是影响真实性能的关键因素，单纯扩大训练数据无法弥合这一差距。


<details>
  <summary>Details</summary>
Motivation: 光流模型主要依赖合成数据和特定领域基准进行评估，但真实世界部署才是最终目标。合成数据与真实场景之间存在巨大鸿沟，然而这一失配问题的严重性尚未被系统量化。

Method: 1) 构建FlowFactor真实基准（1,000帧HD对），按大位移、重复纹理、遮挡、光照变化四个混淆因素组织；2) 结合TAP-Flow和Slow Flow，共8,204帧对；3) 使用标准checkpoint评估多种现代光流模型。

Result: 1) 光照变化和大位移与真实准确性相关性最强；2) 大运动改进可能牺牲小运动/静态场景的鲁棒性；3) Sintel/KITTI/Spring性能对真实数据预测力弱；4) 单纯扩大训练数据无法弥合差距。

Conclusion: 需要一个广泛、真实的基准来评估光流模型，未来研究应聚焦于创新方法而非简单扩展数据与算力。
...
TL;DR: 本文提出Grassmannian Splatting，一种基于格拉斯曼流形的动态场景表示方法。其基本单元是时空R^4中由3-平面支撑的高斯体，通过参数化实现时空协方差矩阵的秩-3约束，使得条件化到时间后每个帧上的基元为秩-2的圆盘（surfel），而非传统4D高斯溅射中的体积椭球。该方法无需学习形变场或自定义CUDA，可直接通过标准3DGS栅格化器渲染，在MonoDyGauBench上训练速度最快（比最优基线快4.9-5.6倍），指标排名第二。


<details>
  <summary>Details</summary>
Motivation: 现有4D高斯溅射方法使用全秩时空协方差矩阵，条件化到每一帧后得到的是体积椭球（3D高斯），而非更自然的2D圆盘（surfel）。作者希望找到一种参数化方式，使得时空协方差矩阵的秩恰好为3，从而条件化降秩后得到秩-2的圆盘，更符合动态场景的几何本质（运动物体表面可视为时空中的2D流形）。

Method: 每个基元由单位法向量n∈S^3/{±1}≅Gr(3,4)（格拉斯曼流形上的点）和自由矩阵L∈R^{4×3}参数化。协方差矩阵定义为Σ_4D=(P_n L)(P_n L)^T，其中P_n=I-nn^T是到n正交补的投影。条件化到时间后，每个基元退化为秩-2的圆盘（surfel）。法向量n编码了圆盘的法向及其沿法向的运动速度，L决定了圆盘形状和中心的切向漂移。通过Schur分母中的软钳制来正则化静态方向，使静态和动态基元形成连续族。

Result: 在MonoDyGauBench的17个HyperNeRF场景上，训练速度是所有对比方法中最快的（比最强质量基线快4.9-5.6倍），同时PSNR、MS-SSIM和LPIPS指标排名第二。

Conclusion: 通过格拉斯曼流形参数化实现秩-3时空协方差，可以在动态场景表示中自然地得到每帧的圆盘基元，无需额外形变场或自定义CUDA，兼顾了渲染质量和训练效率。
...
TL;DR: NanoVSR是一种面向边缘设备的全卷积视频超分架构，利用结构重参数化在推理时坍缩为标准卷积，无需显式光流或Transformer，通过渐进训练隐式学习时空对齐，在REDS4基准上以极低参数量实现了精度与速度的出色平衡。


<details>
  <summary>Details</summary>
Motivation: 现有VSR方法依赖Transformer和显式光流，计算开销大且难以在TensorRT等硬件加速器上部署，亟需一种兼顾效率与硬件兼容性的轻量级方案。

Method: 提出全卷积架构NanoVSR，采用结构重参数化技术使推理时坍缩为标准卷积；通过渐进训练策略让网络隐式学习时空对齐，避免显式运动补偿。

Result: 在REDS4上，NanoVSR-644k达28.64 dB PSNR，Jetson Orin NX上27.2 FPS；NanoVSR-1.7M达29.15 dB PSNR，19.58 FPS，显著优于同量级模型。

Conclusion: NanoVSR证明了纯卷积架构在边缘设备视频超分中的巨大潜力，在不依赖Transformer和光流的前提下实现了精度与速度的优异平衡，为硬件友好型VSR提供了新方向。
...
TL;DR: AMID是一个面向医学影像的自主多智能体框架，通过数据条件方法规划和验证引导的两阶段优化，在20项医学影像挑战任务中超越通用MLE系统，部分任务接近人类设计水平。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体虽开始自动化机器学习工程（MLE），但迁移到医学影像领域面临挑战——每项任务有模态特定的实验要求、严格的验证协议和预测产物标准，缺乏专门的自动化框架。

Method: AMID包含两个核心组件：1) 数据条件方法规划（Data-Conditioned Method Planning）——基于任务特定数据分析将粗粒度搜索空间细化为可执行、可并行化的方法通道；2) 验证引导的两阶段优化（Verification-Guided Two-Stage Optimization）——从广泛探索多种方法通道转向选择性利用有前景的候选方案，全程严格验证验证协议、指标计算和预测产物。

Result: 在涵盖多种模态和预测类型的20项医学影像挑战任务上，AMID超越了评估的通用MLE系统，并且在多项任务上逼近或匹配了人类精心设计的挑战解决方案。

Conclusion: AMID能将医学影像模型开发从定制化手工工程转变为可审计、高性能的智能体工作流，适用于异构任务。
...
TL;DR: 本文系统研究了自回归（AR）图像生成模型的样本多样性问题，发现现有token级解码策略因AR模型的高熵和高冗余视觉token空间而效果有限，提出了一种新的聚类级熵截断采样策略p-less cluster，在多个AR T2I模型和数据集上实现了更好的多样性-质量权衡。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在文本到图像生成中虽质量优异但存在样本多样性坍塌问题，而自回归模型因其序列生成特性可兼容文本生成中的多样性解码策略，有望在图像质量和多样性之间实现更好的Pareto最优，但目前缺乏对AR图像生成模型多样性的系统研究。

Method: 提出p-less cluster解码策略，在聚类级别而非token级别进行基于熵的截断采样。首先对视觉token空间进行聚类，然后在聚类层面计算熵并执行截断，保留高熵（不确定性高）的聚类，从而在全局结构层面增加多样性。

Result: 在4个自回归T2I模型和2个数据集上的综合评估（涵盖图像质量、提示对齐和多样性）显示，p-less cluster在大多数模型和数据集上实现了最大的多样性增益，同时保持了图像质量和提示对齐能力。

Conclusion: 自回归图像生成中token级解码策略对多样性提升有限，根源在于其高token级熵和视觉token空间的高度冗余；聚类级的p-less cluster策略能有效突破这一限制，为AR模型在多样性-质量权衡上开辟了新路径。
...
TL;DR: PA3AD提出了一种基于物理启发的伪异常生成策略和原型特征引导机制，有效解决了3D点云异常检测中真实异常样本稀缺的问题，在多个基准数据集上取得了最优性能。


<details>
  <summary>Details</summary>
Motivation: 3D点云异常检测面临真实异常样本稀缺且获取成本高的问题，且训练数据本身不含异常，导致模型难以有效学习正常与异常样本间的判别性特征。

Method: 提出PA3AD框架，包含两大创新：(1)物理启发的伪异常生成模块，通过多物理场建模从正常数据生成多样化的物理合理伪异常点云；(2)动量更新原型与差异感知融合模块，捕获稳定的正常表示及与伪异常之间的差异，学习分布偏移。

Result: 在Anomaly-ShapeNet和Real3D-AD数据集上，PA3AD一致性地超越了现有最先进方法。

Conclusion: 通过物理启发的伪异常生成与原型引导的分布偏移学习，有效解决了真实异常稀缺问题，为3D点云异常检测提供了鲁棒且高效的解决方案。
...
TL;DR: 提出一种结合惩罚加权最小二乘（PWLS）和引导全变分（GTV）的量子压缩感知CT重建方法，将光子计数统计与解剖异质性纳入QUBO框架，在稀疏视角条件下实现了优于传统方法的重建质量（PSNR达36.64 dB）。


<details>
  <summary>Details</summary>
Motivation: 现有基于QUBO的稀疏视角CT重建忽略了光子计数统计与解剖异质性，限制了重建质量。

Method: 将PWLS（基于光子计数可靠性加权投影残差）和GTV（利用先验图像梯度引导边缘保持与噪声抑制）通过二元编码统一构建QUBO模型，采用D-Wave混合量子-经典求解器优化。

Result: 在4个40×40的CT图像及10视角扇束几何下，PWLS-GTV在胸部案例中PSNR达36.64 dB，远超SART的22.48 dB；GTV优于传统TV；模拟退火与D-Wave混合求解器结果相近，梯度下降效果差。

Conclusion: 该方法在不改变QUBO二次形式的前提下，将光子统计加权与结构引导正则化整合进来，为量子辅助稀疏视角CT重建提供了概念验证。
...
TL;DR: 本文提出一个基于外观、场景构成和采集几何的水下图像域标签框架，系统研究了域偏移对水下目标检测中人工标注质量和深度学习检测器性能的影响，揭示了显著的域相关差异，并展示了该框架在指导数据采集、设计更信息化的基准测试和评估检测器鲁棒性方面的应用价值。


<details>
  <summary>Details</summary>
Motivation: 现有水下目标检测评估使用聚合指标掩盖了特定环境下的失败，且域泛化基准多依赖不反映真实条件的合成变化，缺乏对真实域偏移因素的系统研究。

Method: 提出一个通过外观、场景构成和采集几何三个维度对水下图像分配域标签的框架，并基于此框架系统研究域因素对人工标注质量和深度学习检测器性能的影响。

Result: 发现水下目标检测在不同域之间存在显著的域相关性能差异，人工标注质量和检测器性能均受域因素的显著影响。

Conclusion: 通过引入物理有意义的域标签，域偏移变得可表征、可测量、可基准化和可操作，可用于指导数据收集与标注、设计更信息化的基准以及评估检测器在多样化水下环境中的鲁棒性。
...
TL;DR: 本文提出DiffUE，通过将噪声注入图像的语义空间（而非像素空间）来生成不可学习样本，在保持视觉质量的同时有效防御多种重学习策略，显著提升了图像质量与不可学习性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有不可学习样本方法主要依赖像素级噪声，容易被对抗训练、图像变换、压缩等重学习策略绕过，且提升鲁棒性往往严重损害图像质量和实用性。

Method: DiffUE利用基于扩散的自动编码器框架，将噪声注入图像的语义空间（高层语义特征）而非像素空间，生成自然且目标明确的外观修改，从而抵抗高级重学习策略。

Result: 在CIFAR-10、CIFAR-100、CelebA-HQ和ImageNet四个数据集上的大量实验以及主观用户研究，验证了DiffUE在图像质量与不可学习性之间取得了更优的权衡。

Conclusion: DiffUE通过语义空间扰动提供了一种更鲁棒且有效的解决方案，用于保护个人数据免受日益严重的AI隐私侵犯。
...
TL;DR: 本文系统评估了三种代表性车辆再识别方法（CLIP-ReID、MSINet、AdaSP）在无人机视角下受恶劣天气（雾天、雨天）影响时的鲁棒性。通过合成雾天和雨天数据集进行实验，发现恶劣天气显著降低检索性能，雨天影响大于雾天，其中AdaSP表现最鲁棒，并呼吁未来研究应关注天气感知模型设计。


<details>
  <summary>Details</summary>
Motivation: 现有无人机车辆再识别研究主要关注正常天气条件，但恶劣天气（雾、雨）会严重破坏细粒度外观线索（如颜色、纹理），导致车辆匹配失败。目前缺乏对现有方法在恶劣天气下鲁棒性的系统性评估，亟需填补这一空白。

Method: 采用控制变量对比实验：选择三种代表性方法（CLIP-ReID、MSINet、AdaSP）；通过分析性天气效应管线合成VRU和UAV-VeID数据集的雾天和雨天变体，保持原始身份标签和数据划分不变；在匹配的干净、雾天、雨天条件下分别训练和评估。

Result: 恶劣天气一致降低所有方法的检索性能，雨天比雾天影响更大；AdaSP鲁棒性最强，在VRU-Large上雾天mAP 93.0%、雨天mAP 88.5%，在UAV-VeID-Test上雾天mAP 88.7%、雨天mAP 76.2%。

Conclusion: 模拟恶劣天气显著增加了无人机车辆再识别的难度，不同方法之间鲁棒性差异明显，未来研究需要引入天气感知的模型设计和评估协议。
...
TL;DR: 本文提出了一种通过对比专家与新手操作视频来检测异常动作帧的方法，用于提取未记录在手册中的隐性维护知识，在配电板模拟实验中实现了66.9%的提取率，比传统方法提升50个百分点。


<details>
  <summary>Details</summary>
Motivation: 关键基础设施维护面临熟练工人减少的挑战，而传统访谈方法难以提取专家自身也未意识到的隐性操作知识。

Method: 通过对比基于手册的常规操作视频与专家维护工人的操作视频，检测包含隐性知识的候选动作异常帧。

Result: 针对11种未在手册中描述的动作类型，实现了66.9%的提取率，较传统方法提升50个百分点。

Conclusion: 该方法能有效揭示隐性维护知识，有助于提升关键基础设施维护中的技能转移和劳动力发展。
...
TL;DR: 本文基于YOLOv8-nano架构，利用无人机无线实时视频流构建了实时目标检测管线。在VisDrone2019数据集上训练，并在不同飞行高度进行实验。结果表明，在16-25米高度区间检测帧率稳定在41 FPS以上（峰值50 FPS），实现了近完全的检测可靠性，有效解决了因高度变化导致的目标尺度变化问题。


<details>
  <summary>Details</summary>
Motivation: 现有监控框架在无人机高度变化导致目标尺度显著变化时，难以保持检测一致性，造成关键信息缺失。因此需要一种能够适应不同飞行高度、实现实时稳定检测的无人机监控方案。

Method: 基于YOLOv8-nano架构构建实时检测管线，通过无线传输获取无人机实时视频流进行目标检测。在VisDrone2019数据集上训练模型，并在多个飞行高度下开展大量飞行实验以评估检测性能。

Result: 模型在VisDrone2019上达到57.4%精确率、41%召回率、44.8% mAP和20.3% mAP50:95。在真实环境中，16-25米高度区间检测帧率稳定在41 FPS以上（峰值50 FPS），实现近完全检测可靠性。

Conclusion: 该方案有效解决了无人机空中观测中不同尺度目标的识别挑战，实现了近准确的定位，能够从空中平台通过无线传输进行实时人员检测。
...
TL;DR: 本文提出一种语言引导的视觉AI框架，结合视觉-语言模型和多模态大语言模型，通过视觉问答范式实现垃圾分类，在自建WasteVQA数据集上BLIP模型取得BLEU 0.8291、BERTScore 0.9273的优异性能，超越传统CNN方法，符合印度废物管理法规。


<details>
  <summary>Details</summary>
Motivation: 现有自动垃圾分类系统存在三大局限：单模态视觉处理、缺乏上下文理解能力、与法规对齐不足。这些问题导致分类精度低、监管合规性差，亟需一种多模态智能方法提升垃圾分类效率和规范性。

Method: 提出语言引导的视觉AI框架，集成视觉-语言模型（如BLIP）和多模态大语言模型，采用视觉问答（VQA）范式；构建涵盖21类、13,500问答对的WasteVQA数据集，并严格对齐印度2016年固体废物管理规则。

Result: BLIP模型在WasteVQA数据集上BLEU得分0.8291，BERTScore达0.9273，性能显著优于传统CNN方法，验证了多模态方法在垃圾分类任务中的有效性。

Conclusion: 该工作提升了源头级分类精度，确保法规合规性，支持市政和用户端的可扩展部署，推动多模态AI在可持续城市基础设施中的应用。
...
TL;DR: SC-Flow是一种基于潜空间谱一致性校正的3D医学图像翻译框架，仅需单次函数评估即可通过布朗桥过程实现模态间映射，并利用频谱校正机制恢复精细解剖结构。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像翻译方法存在模态纠缠、过度平滑和伪影问题，尤其是潜空间平均速度场的隐式低通调制导致高频细节丢失。

Method: 将医学图像翻译建模为随机布朗桥过程，在潜空间中预测支持正则化的平均速度场；提出谱一致性校正器（Spectral Consistency Corrector），通过可学习的频域增益调制动态正则化功率谱密度演化，建立空间纹理与频谱能量流之间的显式桥梁。

Result: 在四个数据集上的实验表明，SC-Flow在各种翻译场景下实现了更准确、一致且鲁棒的性能。

Conclusion: SC-Flow通过频谱层面的显式校正，在保持全局结构一致性的同时恢复了精细的解剖保真度，为3D医学图像翻译提供了一种高效且高质量的解决方案。
...
TL;DR: 提出SpecFlow，一种无需训练的视觉token压缩框架，通过谱热流保持结构一致性、自适应空间划分保证覆盖率、核集汇聚维持统计守恒，在剪枝88.9%视觉token时仍保留95.6%原始性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉token剪枝方法在高压缩率下性能严重退化，原因在于盲目丢弃信息、破坏空间结构或牺牲多样性。VLMs推理时需处理长序列视觉token，成本高昂。

Method: 将视觉token视为kNN图节点，通过三个核心模块进行保守性压缩：(i) 谱热流计算稳定重要性场，保持结构连贯性；(ii) 自适应空间划分分配预算，保证覆盖；(iii) 将丢弃信息汇聚到核集sink中，维持统计守恒。无需训练，即插即用，兼容FlashAttention。

Result: 在多种任务、VLM架构和剪枝比例下均优于SOTA方法。LLaVA-1.5使用SpecFlow在剪枝88.9%视觉token时仍保持95.6%的原始性能，实现了优异的效率-精度平衡。

Conclusion: 提出从破坏性剪枝转向保守性汇聚的新范式，通过严格的空间覆盖和统计守恒保证稳定性，为VLM高效推理提供了有效的训练-free解决方案。
...
TL;DR: 提出了一种基于答案条件链式思维（CoT）蒸馏的方法，通过让前沿VLM生成带有正确标签的可视化解释，然后用LoRA微调3B参数的小模型，在仅18-30张标注图像的情况下，在四项工业分类任务上优于直接微调，平均提升1.7-4.4个百分点。


<details>
  <summary>Details</summary>
Motivation: 工业视觉检测中需求频繁变化、新缺陷类型不断出现且大规模标注数据稀缺，现有方法难以快速适应新任务。

Method: 1) 前沿VLM接收训练图像和正确标签，生成基于正确答案的可视化推理解释；2) 使用LoRA对3B参数的小模型在推理增强的样本上进行微调；3) 通过答案条件化确保推理过程指向正确结论。

Result: 在4项工业分类任务（3种图像模态）上仅用18-30张标注图像，32次训练中16个种子-任务组合均优于直接微调，平均提升1.7-4.4pp。消融实验表明无答案条件化时性能下降17.8pp。在焊缝X光片分类上，微调后的3B模型比GPT-4.1高10.0pp。

Conclusion: 答案条件化的CoT蒸馏是一种高效、数据节俭的工业视觉检测模型适应方法，能在极小标注数据下显著提升小模型性能，甚至超越大型前沿模型。
...
TL;DR: HyperBank是一个由经典图像处理算子组成的可微分库，在少量标注支持图像上适配，在球状体分割任务中与大型基础模型竞争力相当，尤其在小规模、对比度驱动的数据上表现更优，同时保持可解释性。


<details>
  <summary>Details</summary>
Motivation: 少样本球状体分割需要适应新细胞系、显微镜和光照条件，但现有基础模型虽然准确，其大规模不透明骨干网络难以解释视觉线索的成功或失败原因，因此需要一种可解释的替代方案。

Method: 提出HyperBank，一个可微分经典图像处理算子库（包含Frangi血管增强、Sauvola阈值金字塔、结构张量响应、梯度幅值、LoG滤波器），在少量标注支持图像上适配，并在三个独立采集的球状体数据集上评估。

Result: 在少量标注图像上适配后，HyperBank与小样本基础模型竞争力相当，在小规模、对比度驱动的数据上甚至超越大型模型；基础模型在外部来源、纹理主导的球状体数据上仍更强。消融实验表明有用的小样本信号分布在不同算子家族中，并可通过形态学操作增强。

Conclusion: HyperBank证明经典算子库可作为紧凑、可解释的少样本显微镜分析流水线，并能作为分析先验探针揭示哪些经典线索承载少样本信号。
...
TL;DR: 提出一种在线增量重建框架，通过对密集几何高斯表示直接三角化来增量重建和更新高保真显式网格，支持高质量渲染和表面重建，并显著降低长序列处理中的内存和计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法需要将优化后的高斯离线转换为中间隐式场才能提取显式网格，阻碍了下游任务的集成，缺乏在线增量重建能力。

Method: 1）提出在线框架，直接对密集几何高斯表示进行三角化以重建和更新显式网格；2）设计直接网格化算法，从高斯集中高效提取和更新网格；3）引入基于平面的拉动约束，将高斯原语动态对齐到近似局部表面；4）动态冻结完全优化的历史区域以降低长序列处理的内存和计算开销。

Result: 在公共数据集上，该方法在渲染质量和重建精度上均优于传统基于高斯的方法。

Conclusion: 该方法实现了在线、高效的增量场景重建，避免了离线转换步骤，同时保持了高质量渲染和表面重建能力，适用于实际应用中的长序列处理。
...
TL;DR: 提出一种无需训练的图像合成检测方法NoiseCluSID，利用Noiseprint++提取噪声残差指纹，结合冻结的ViT提取多尺度特征并自适应融合，再通过少量真实样本初始化K-Means聚类中心，实现无监督分类。在四个基准数据集上平均准确率82.2%，优于现有方法，尤其擅长检测扩散模型生成的图像。


<details>
  <summary>Details</summary>
Motivation: 现有合成图像检测方法依赖大量标注数据进行监督训练，成本高昂且对未知生成模型的泛化能力差。为克服这些缺陷，作者探索一种无需训练的、仅需少量真实样本即可工作的检测方案。

Method: 1) 使用预训练Noiseprint++模型提取噪声残差指纹；2) 用冻结的Vision Transformer从残差中提取多尺度特征；3) 对特征进行自适应加权融合；4) 仅用少量真实样本初始化K-Means聚类中心，以无监督方式区分真实与合成图像。

Result: 在四个基准数据集上平均准确率82.2%，优于现有最先进的检测方法。在扩散模型生成的合成图像上表现尤为出色，消融实验验证了各模块的有效性。

Conclusion: 提出的NoiseCluSID方法无需训练即可有效检测合成图像，具有良好的泛化能力，尤其对扩散模型生成的图像检测效果显著，为被动式盲图像取证提供了新思路。
...
TL;DR: 本文提出通过在2D像素空间中自回归生成轨迹来微调视觉语言模型（VLM），使其直接在像素层面学习导航交互，避免了引入深度或3D几何信息。实验表明，像素空间轨迹监督显著提升了VLN-CE任务的性能，并以有限的计算资源和训练数据达到了SOTA水平。


<details>
  <summary>Details</summary>
Motivation: 现有VLN-CE方法依赖将视觉嵌入投影到语言空间以实现泛化，但VLMs主要训练于RGB图像对话，难以理解涉及深度感知的三维空间交互（如"走到沙发尽头左转"）。作者希望在不引入VLMs预训练中罕见的深度/3D信息的前提下，让VLMs学会导航交互。

Method: 提出在2D像素空间中通过自回归轨迹生成进行微调：给定语言指令和历史观察，模型从当前观察的底部中心开始，依次预测一系列像素坐标，形成一条轨迹。这是一种像素级轨迹监督方法，而非离散动作学习。

Result: 实验验证像素空间轨迹监督显著优于像素目标（pixel-goal）监督和离散动作学习；旗舰模型在有限计算资源和训练数据下达到SOTA性能。

Conclusion: VLN任务可分解为一系列子任务，每个子任务对应一个三维空间交互过程。通过将导航交互转化为2D像素空间中的轨迹生成，可以避免VLM处理不熟悉的深度/3D信息，同时有效提升导航性能。
...
TL;DR: 提出SAWRD-Net，利用水面的不完全反射对称性，通过二面体群等变卷积和矩阵分解解码器，实现自然水面场景中反射轴的高精度检测，TPR达0.890。


<details>
  <summary>Details</summary>
Motivation: 水面反射会导致目标检测和语义分割产生大量误检和漏检，而现有方法无法有效检测水中的反射轴，因此需要一种能够感知对称性的反射轴检测网络。

Method: 1) 二面体群等变卷积层提取显式编码旋转和镜像对称的几何一致特征图；2) 多尺度反射等变块聚合多尺度特征并使用对称注意力机制；3) 矩阵分解解码器将高维特征分解为低秩参数空间和置信度空间，直接回归反射轴上的关键点；4) 鲁棒主成分分析拟合最终反射轴。

Result: 在最大的水面反射场景数据集上，SAWRD-Net的真阳性率(TPR)达到0.890，超过所有现有的水面反射检测器。

Conclusion: 利用水的非完美反射对称性，结合群等变卷积与矩阵分解，能够有效检测反射轴，提升水场景下视觉系统的可靠性。
...
TL;DR: 本文提出TriCons-Pose框架，通过三角形不变几何一致性学习实现类别级物体姿态估计。包含结构一致关键点检测器（SCKD）和姿态不变几何聚合器（PIGA），在REAL275等数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有类别级物体姿态估计方法过度依赖特征学习，忽略了所建立的对应关系在不同扰动下是否具有几何稳定性，导致在类内形状变化和遮挡场景下姿态恢复脆弱。

Method: 提出TriCons-Pose框架：(1) 结构一致关键点检测器(SCKD)，通过归一化成对距离匹配强制跨视角结构一致性来识别鲁棒关键点；(2) 姿态不变几何聚合器(PIGA)，将基于三角形的姿态不变描述子注入局部到全局注意力机制来增强关键点表示；(3) 额外引入几何一致性损失进行优化。

Result: 在REAL275、CAMERA25和HouseCat6D三个数据集上的大量实验验证了该方法在类别级物体姿态估计任务上的有效性。

Conclusion: 通过引入三角形不变几何一致性学习，有效稳定了关键点检测并聚合了姿态不变线索，实现了更可靠的规范映射和准确的姿态估计，为类别级物体姿态估计提供了新的解决思路。
...
TL;DR: TOLiD提出了一种基于跨模态知识蒸馏的自监督预训练方法，通过LiDAR骨干与视觉Transformer（ViT）学生网络耦合，利用Frustum Pooling和Frustum Attention实现点云与图像patch token的对齐，在多个LiDAR数据集上展现了更强的迁移学习性能。


<details>
  <summary>Details</summary>
Motivation: 现有跨模态蒸馏方法通常将视觉基础模型（VFM）作为固定特征源，迫使3D骨干网络去弥合模态差异和架构差异（如密集ViT token与稀疏3D编码器），导致学习效率低。TOLiD旨在通过引入ViT学生网络和兼容的patch-token表示来缩小这一差距。

Method: 1) 将LiDAR骨干与从冻结VFM教师初始化的学生ViT耦合；2) 使用Frustum Pooling和Frustum Attention将每个图像patch视锥内的点特征转换为token；3) 进行带可见性掩码的token级蒸馏；4) 通过掩码双线性采样将token特征提升回逐点表示，避免包含LiDAR点稀疏的patch。

Result: 在五个异构LiDAR数据集和四个跨传感器适应对上进行评估，TOLiD在使用冻结骨干和轻量级头部的情况下展现出更优的迁移性能。

Conclusion: TOLiD通过设计兼容的patch-token蒸馏机制，有效缓解了模态与架构差异，为LiDAR自监督表示学习提供了一种高效的预训练方案。
...
TL;DR: RED-Sphere是一个即插即用的鲁棒性框架，通过估计捷径敏感性干扰、残差软门控衰减、反事实一致性约束和归一化球面原型，解决医学图像分类在未见外部人群分布偏移下的鲁棒性问题，在AMD和DR两个任务上平均提升1.28和2.98个F1点。


<details>
  <summary>Details</summary>
Motivation: 医学图像分类器通常在单一源人群上训练，但临床部署需要面对外观、采集方式和疾病流行率不同的外部人群。现有公平性和鲁棒性方法要么需要群体监督信息，要么将外观变化视为无差别干扰，这无法有效处理人群相关的低层线索与病变证据在边缘和纹理结构上纠缠的问题。

Method: 提出RED-Sphere框架：(1) 利用边缘和特征能量先验估计捷径敏感性的干扰响应；(2) 通过残差软门控衰减主导响应；(3) 用反事实启发的一致性和分离损失正则化掩蔽的干扰视图；(4) 使用归一化球面原型预测标签，偏好角度语义证据而非源相关激活幅度，同时保留病变结构。

Result: 在严格的白人-only Harvard-FairVision协议下，RED-Sphere在20个任务和骨干网络对比中均提升了外部人群的macro-F1，AMD平均提升1.28个F1点，DR平均提升2.98个F1点。AUC和PR-AUC、可视化诊断、消融实验和敏感性分析也支持更强的外部语义对齐和更稳定的角度疾病几何结构。

Conclusion: RED-Sphere是一种有效的即插即用鲁棒性框架，不局限于视网膜解剖结构，可适配到其他模态中，只要存在外观捷径与语义证据纠缠的场景均可应用。
...
TL;DR: 提出了一种无需重新训练的扩散世界模型控制方法，通过采样时注入可微能量函数来引导轨迹生成，但发现生成视频尚未完全跟随引导轨迹，需加强跨流耦合。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶世界模型基于视频扩散主干生成真实场景，但难以控制：强制遵守交通规则通常需要重新训练主干或依赖手工布局。本文探究是否可以在不重新训练的情况下实现可控性。

Method: 基于整流流（Rectified Flow）的驾驶世界模型，联合生成未来视频和规划的自车轨迹。在采样时通过可微能量函数（编码驾驶规范）注入能量引导（Energy Guidance），无需对扩散主干进行知识特定的重新训练。实验基于Open-Sora 2.0的MM-DiT主干。

Result: 成功通过能量引导使模型在反事实目标处刹车，但生成的视频并未完全跟随引导的轨迹。原因是主干中的联合自注意力机制尚未能充分耦合视频生成与轨迹引导。

Conclusion: 跨流耦合（cross-stream coupling）是实现端到端可控滚动生成的关键需求，当前模型在此方面仍有不足。
...
TL;DR: 本文系统性研究了低倍率病理图像在弱监督分割任务中的表现，发现图像重建质量不能完全预测分割性能，并识别出小尺度结构定位显著下降的关键退化点。


<details>
  <summary>Details</summary>
Motivation: 高倍率全切片图像存储和传输资源消耗大，且像素级标注耗时费力。因此需要探究低倍率图像结合弱监督方法能否达到类似性能，以降低存储和标注成本。

Method: 从高分辨率图像块出发，模拟低倍率输入，使用插值和深度学习重建方法恢复原始尺寸，再应用弱监督分割管线，定量评估不同分辨率退化程度下的分割表现。

Result: 重建质量指标不足以预测下游分割性能；存在一个关键退化点，在此之后小尺度结构的定位性能显著下降。

Conclusion: 该研究为设计高效的数字病理存储系统同时保持可靠的自动化分析提供了实用指导。
...
TL;DR: 本文提出将AIGC视频检测从低级伪影检测重新定义为"事实保真度验证"，并通过视觉-语言双视角分类法系统梳理了221篇工作，揭示了从伪影匹配到语义验证的范式转变。


<details>
  <summary>Details</summary>
Motivation: 传统基于伪影的检测方法在AI生成视频日益逼真的背景下已显不足，亟需从低级像素检测转向高级语义真实性验证的新范式。

Method: 提出视觉-语言双视角分类法，将检测方法组织为四层层次结构：内在线索分析、时空一致性建模、跨模态一致性推理、语言引导的世界级推理，并系统综述了221篇相关工作。

Result: 揭示了从传统深度伪造的伪影匹配到基于视觉-语言模型和智能推理管道的证据式语义验证的根本转变；综述了生成范式、检测方法、评估指标与基准。

Conclusion: 总结了当前挑战，指出了向鲁棒、可解释、可信赖检测的有前景方向。
...
TL;DR: MAC-Splat提出了一种基于多属性一致性损失（MAC loss）的框架，利用MASt3R几何骨干网络和DINOv3编码器获取的语义对应关系作为3D锚点，直接对稀疏视图下的3D高斯属性（位置、形状、外观）进行联合正则化，在ScanNet++上相比Splatt3R提升PSNR超过4.5 dB。


<details>
  <summary>Details</summary>
Motivation: 现有可泛化3D高斯泼溅方法在稀疏视图下由于仅依赖2D光度损失监督，无法解决深度和对应关系的歧义性，导致几何伪影严重。

Method: 基于MASt3R几何骨干网络和冻结的DINOv3编码器获取语义感知的2D对应关系作为几何锚点，定义多属性一致性损失（MAC loss），同时对匹配高斯体的位置、形状和外观在共同世界坐标系下进行正则化，并具备异常值鲁棒性和协方差矩阵几何一致性。

Result: 在ScanNet++数据集上，MAC-Splat超越多个强基线方法，尤其在不同重叠率场景下表现突出；相比Splatt3R，平均PSNR提升超过4.5 dB，LPIPS降低，且在相机位姿间距增大时仍能保持性能。

Conclusion: 直接的多属性3D一致性目标函数结合高质量对应关系，能有效解决稀疏视图重建这一病态问题。
...
TL;DR: 本文提出了一种基于证据驱动的多模态推理框架，利用Bloom分类法作为分层推理协议，通过两阶段认知语言化（字面证据总结+Bloom语言化）将多编码器专家输出转化为可解释的推理轨迹，提升了LVLM的感知与推理能力，并减少了幻觉。


<details>
  <summary>Details</summary>
Motivation: 当前LVLM在处理视觉语言问题时，对不同认知层次的问题采用相同的感知-推理路由，缺乏差异化处理；同时多专家模型集成缺乏可解释性和可信度，容易产生幻觉。

Method: 提出证据驱动的多模态推理框架：(1) 利用Bloom分类法作为分层推理协议；(2) 两阶段认知语言化：先产生字面证据总结（将专家输出分解为原子证据陈述），再进行Bloom语言化（构建分层推理轨迹）；(3) 轻量级推理轨迹模块，定量分析推理过程和证据使用。

Result: 感知和推理能力均有提升；推理轨迹模块提供了定量证据，表明不同查询会引发不同的认知进入水平和证据使用轨迹，支持细粒度分析。

Conclusion: 该框架通过认知分层的证据驱动推理，提升了LVLM的可解释性和可信度，减少幻觉，为多模态推理提供了可量化的分析工具。
...
TL;DR: 本文提出Compositional Context Fine-Tuning (CCFT)方法，将装配动作分解为语义元素（动词、物体、工具），并结合Layer-Partitioned Alternating Training (LP-AT)进行多任务微调，在装配动作理解任务上显著优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 装配动作理解对于人机协作装配至关重要，但由于动作细微且手-物交互精细，现有方法难以有效识别。本文旨在将视觉-语言模型（VLM）适配到这一挑战性领域。

Method: 提出CCFT方法，将装配动作分解为Verb、Object、Tool三个语义元素，通过模板化问答对微调VLM；同时提出LP-AT方法，为每个元素分配不同模型层并通过元素特定的低秩适配器交替更新权重，减少跨任务干扰。

Result: 在HA-ViD-VQA和IKEA-ASM-VQA数据集上，该方法一致优于强动作识别基线，并提供可解释的元素级预测。

Conclusion: CCFT与LP-AT能有效将VLM适配到装配动作理解领域，实现高精度、可解释的动作识别，支持多种下游应用。
...
TL;DR: 本文提出h-Flow，一种基于Doob's h-Transform的无训练、理论上严谨的流模型图像编辑框架，通过构造等价SDE并设计专用h函数，实现源一致性与目标对齐的可控权衡。


<details>
  <summary>Details</summary>
Motivation: 现有基于预训练文本到图像流模型的编辑方法，要么依赖基于反演的流程，要么依赖启发式的源到目标轨迹构建，往往对架构设计和超参数敏感，缺乏理论支撑。

Method: 1) 将Doob's h-Transform从SDE模型扩展到确定性的RF（Rectified Flow）框架，构造具有相同边际分布的等价SDE；2) 为源一致性和目标对齐分别设计专用h函数，推导出闭式重建引导和基于速度的语义编辑信号；3) 引入速度正交分解，解耦重建方向与编辑方向，实现两个目标的可控权衡。

Result: 实验表明h-Flow在多种场景下实现了有效、鲁棒且灵活的图像编辑，无需训练。

Conclusion: h-Flow提供了一种理论驱动、无训练、对超参数鲁棒的流模型图像编辑框架，通过正交分解实现了重建与编辑的可控权衡。
...
TL;DR: 提出了3D-DefectBench，一个系统分析基于VLM的3D缺陷检测管道的基准框架。通过平衡析因设计对四个管道因素进行系统评估，发现模型选择是影响与人类标注一致性的最大因素，但其他因素也会影响性能并与模型选择交互。在标准化管道下，最佳VLM判断仍落后于训练有素的人类标注者。


<details>
  <summary>Details</summary>
Motivation: 自动化评估对规模化3D生成系统至关重要，但自动化判断的可靠性取决于整个评估管道（不仅是VLM模型本身，还包括渲染方式、视觉证据、任务规范和人类参考标签的构建方式），现有研究缺乏对这一完整管道的系统分析。

Method: 构建3D-DefectBench基准框架，采用平衡析因设计，对四个管道因素（VLM、相机协议、视觉输入、提示模式）进行系统变化，涵盖84种推理设计和约320万个评分缺陷决策，并在更广泛的前沿模型上进行分阶段验证。

Result: 模型选择是与人类标注一致性最大的决定因素，但其余三个管道因素也会影响性能、与模型选择产生交互，并能改变最佳配置。紧凑的六视图RGB协议与更密集的多视图设置或增加深度/法线输入的方案表现相当。在标准化管道下，最佳的12个VLM判断者仍落后于训练有素的人类标注者。使用更嘈杂的银标签替换专家共识标签时，纹理一致性会急剧下降。

Conclusion: 自动化判断应作为完整管道进行评估并在不同人类参考体系中进行校准，而非仅作为独立模型进行基准测试。
...
TL;DR: OmniX是一个前馈式4D重建框架，能从大相机运动视频中为每个像素预测密集3D点轨迹，通过解耦动态运动与静态几何、使用紧凑动态令牌表示运动，并在包含80K场景和1.28M多视角视频的大规模数据集上训练，在轨迹预测、点跟踪、深度估计和相机位姿估计任务上达到领先性能。


<details>
  <summary>Details</summary>
Motivation: 现有前馈式4D重建方法要么预测每帧静态点云而忽略前景运动，要么估计点云轨迹但局限于小相机运动，无法在大视角变化下聚合观测数据并重建完整动态场景。

Method: OmniX将动态运动建模与静态几何预测解耦，利用3D运动的稀疏低秩结构，通过一组紧凑的动态令牌为所有像素生成轨迹场，同时高效保持全局交互。还构建了基于UE5的自动4D数据引擎，生成包含80K场景和1.28M多视角视频的大规模数据集用于训练。

Result: 在密集3D点轨迹预测和3D点跟踪任务上达到最优性能，同时在视频深度估计和相机位姿估计上也取得了具有竞争力的结果。

Conclusion: OmniX通过解耦策略和紧凑动态令牌表示，成功突破了前馈式4D重建在大相机运动下的限制，实现了高质量的动态场景重建。
...
TL;DR: 提出Align and Segment (AnS)方法，通过空间变换模块同时学习标签对齐和语义分割，无需真实标签即可处理遥感图像中标签与图像不对齐的问题。


<details>
  <summary>Details</summary>
Motivation: 遥感图像中航空/卫星图像与来自其他来源（如OpenStreetMap）的标签进行配准时，标签可能存在空间错位，导致分割模型性能显著下降。现有监督学习方法要求图像与标签严格对齐，因此需要一种能容忍并自动纠正标签错位的训练方法。

Method: 基于空间变换器模块（STN）对错位标签进行仿射变换以实现对齐；与标准语义分割网络联合训练；引入自监督正则化损失防止网络学习错位标签的捷径解；无需任何真实标签即可训练。

Result: 在合成数据和多个城市的真实数据上，AnS方法同时实现了高质量的建筑分割和精确的标签-图像对齐；自监督正则化损失与数据增强互补，尤其适用于系统性错位的情况。

Conclusion: AnS方法有效解决了遥感图像中标签错位问题，实现了无需黄金标签即可同时进行对齐和分割，实验证明其在真实场景下表现优异。
...
TL;DR: 本文提出Locus框架，利用预训练分割基础模型提取解剖形状先验，通过自适应注意力正则化引导分类器关注医学图像中的诊断相关区域，在8个不同模态数据集上验证了分类性能提升和注意力改善。


<details>
  <summary>Details</summary>
Motivation: 医学图像分类模型应关注诊断相关区域，但标准分类损失缺乏空间监督。显式解剖分割掩膜虽有效但标注和计算成本高，因此希望利用已有分割基础模型的能力来引导分类器注意力。

Method: 提出Locus框架：利用预训练分割基础模型提取解剖结构先验，引入自适应平衡注意力正则化项，在解剖（前景）和背景区域间动态调整注意力权重，惩罚背景注意力过度主导的情况，而非强制像素级对齐。

Result: 在dermoscopy、X-ray、histopathology和cardiac MRI等8个多样化医学影像数据集上验证，Locus在分类性能和注意力解剖学合理性方面均取得一致性提升。

Conclusion: 利用预训练分割基础模型无需额外训练即可提供有效的解剖空间先验，自适应注意力正则化方法能有效引导分类器关注诊断相关区域，提升分类性能与可解释性。
...
TL;DR: 本文提出了一种无需重新训练的方差校正时间偏移方法，通过在采样时对网络查询的时间步进行偏移并缩放得分，解决了高温采样中方差膨胀的问题，从而在保持样本质量的同时有效提升生成多样性，适用于DiT、Stable Diffusion和Motion Diffusion等模型。


<details>
  <summary>Details</summary>
Motivation: 扩散模型能够忠实重现训练分布，但也会继承其中的不平衡性，导致稀有或欠表示的模式难以采样。高温采样虽然能通过重加权模式来缓解这一问题，但会引入方差膨胀，破坏逆向扩散过程并降低样本质量。本文旨在解决方差膨胀问题，使高温采样成为一种实用的多样性控制手段。

Method: 提出方差校正时间偏移（variance-corrective time shifting），一种无需训练的方法：在采样时，将网络查询的时间步偏移到 shifted timestep，然后将得到的得分缩放γ倍，从而在保留模式重加权效果的同时消除方差膨胀。

Result: 在DiT、Stable Diffusion和Motion Diffusion模型上，该方法在最小化样本质量和条件保真度损失的前提下，一致地提升了生成多样性。此外，温度干预的时间点可实现从粗到细的控制：高噪声阶段驱动跨模式的组合多样性，低噪声阶段在固定组合下驱动局部外观变化。

Conclusion: 方差校正时间偏移将高温采样转化为一种实用的多样性控制旋钮，适用于预训练的扩散模型和流匹配骨干网络，无需重新训练即可有效提升稀有模式的采样能力。
...
TL;DR: EquiME是一个由AU引导的图像到视频生成技术构建的合成微表情数据集，包含75K个视频、15K张源人脸图像和五种目标情绪，并带有自动推断的人口统计元数据和视频质量测量。在SAMM和CASME II上的跨数据集实验表明，基于EquiME训练的模型取得了有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 现有微表情数据集规模小、人口覆盖范围窄、情感标签受限，亟需一个大规模、多样化的数据集来推动微表情识别研究。

Method: 采用AU（动作单元）引导的图像到视频生成技术，从15K张源人脸图像生成75K个视频，覆盖五种目标情绪；自动推断人口统计元数据并测量视频质量。

Result: 在SAMM和CASME II上的跨数据集实验中，基于EquiME训练的模型取得了有竞争力的性能，且在四种不同的评估架构上表现出一致性。

Conclusion: EquiME可作为有效的合成微表情识别资源，其结构化AU条件生成流程和全面的质量评估为后续研究提供了坚实基础。
...
TL;DR: 本文提出X-GuideAR，一种增强现实（AR）框架，用于在骨科手术中通过生成合成X射线预览来指导最佳透视角度，从而减少辐射暴露。实验表明，在S2AI螺钉置入中可减少62.3%的X光次数，并显著提升螺钉安全直径（12.95mm vs 5.9mm）。


<details>
  <summary>Details</summary>
Motivation: 传统骨科手术中，医生需反复拍摄X光进行位置校准（"fluoro-hunting"），导致患者和手术团队承受过多辐射暴露。本文旨在通过AR技术减少辐射，同时确保螺钉置入的精确性。

Method: 构建X-GuideAR框架：1）生成合成X射线预览，引导医生在无辐射下找到最佳解剖视角；2）确认视角后拍摄真实X光；3）将钻探轨迹预览叠加到真实X光上，辅助精确置入。以S2AI螺钉为例进行验证。

Result: 专家脊柱外科医生执行8条S2AI轨迹的初步研究显示：X光拍摄次数减少62.3%；X-GuideAR组的平均安全螺钉直径为12.95mm，传统工作流为5.9mm，表明骨包容性更优，潜在生物力学效益更好。

Conclusion: X-GuideAR能有效减少辐射暴露并简化S2AI螺钉置入流程，为更安全、高效的手术提供了有前景的方向。
...
TL;DR: 本文提出了一个统一框架，将自监督稀疏视图CT重建中的分裂策略分解为划分策略、预处理和推理三个组件，系统比较了不同设计选择的影响，发现最优划分策略依赖于噪声结构，并提出了多划分分裂和替代推理策略两种扩展方法。


<details>
  <summary>Details</summary>
Motivation: 自监督数据分裂方法在稀疏视图CT重建中表现出潜力，但关键设计选择（划分策略、预处理、推理）的影响尚未被充分理解，缺乏系统性比较，导致实际应用中缺乏指导原则。

Method: 提出统一框架将分裂重建分解为三个组件；引入两种增量扩展：多划分分裂（multi-partition splitting）和替代推理策略；在模拟LoDoPaB-CT数据（独立噪声和关联噪声）以及真实2DeteCT数据集上进行实验评估。

Result: 最优划分策略依赖于测量噪声结构：基于格子的分裂在独立噪声下表现更好，而角度掩蔽在关联噪声和真实数据下更鲁棒；多划分分裂在多种设置下一致优于纯投影分裂；LPIPS和HaarPSI等指标揭示了掩蔽策略间PSNR/SSIM不易察觉的差异。

Conclusion: 为自监督稀疏视图CT重建方法的设计提供了实用指南，揭示了常见独立性假设在真实成像环境中的局限性。
...
TL;DR: 本文提出DP-Splat，用狄利克雷过程（DP）先验替代固定混合权重，在3D高斯泼洒中实现组件数量的自适应选择。通过截断棒-breaking DP和稀疏过拟合有限狄利克雷两种方法，在保持闭形式坐标上升更新的同时，使有效组件数K̂随场景复杂度自适应。实验表明DP先验贡献在于复杂度选择，在3D场景中能以5.9-7.6倍更少的组件达到或超过VBGS的性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼洒方法（如3DGS、VBGS）中，高斯组件数量K要么由启发式密度控制设定，要么由用户上限固定，无法真正自适应数据复杂度。VBGS虽然引入变分推断，但K仍然固定。作者希望用贝叶斯非参数方法让组件数量自动适应场景复杂度。

Method: 将有限对称狄利克雷先验替换为截断棒-breaking狄利克雷过程（DP）先验，以及理论支持的稀疏过拟合有限狄利克雷先验。所有更新保持闭形式坐标上升步骤；提出自然梯度随机变体使每步计算成本与点数无关。给出了单调性保证、截断误差界修正，并严格分析了拟合组件数的估计含义。

Result: (i) 在分离良好的合成数据上，有效复杂度K̂自适应并恢复真实K±1；(ii) DP先验贡献是复杂度选择而非组件效率——在匹配预算下DP拟合超过单次固定K的VBGS 2.7dB，但与等收敛的固定K基线持平；在3D场景中，DP-Splat以5.9-7.6倍更少的组件匹配或超过VBGS；(iii) 后验预测颜色方差在模型匹配的合成数据上校准良好；(iv) 在平均场坐标上升下，DP先验抵抗过度分裂，而稀疏有限混合饱和其截断，这一差距在N三个数量级上均被记录。

Conclusion: DP-Splat实现了3D高斯泼洒中组件数量的自适应选择，能以显著更少的组件达到同等或更好的渲染质量。研究还揭示了变分实践中DP先验与有限混合先验的差距，以及理论与实际后验渐近行为的不一致。
...
TL;DR: 本文提出了一种无监督的管道方法，通过原始车辆轨迹直接识别交叉口的入口和出口区域，无需人工标注、相机标定或交叉口几何先验知识。该方法通过聚类轨迹起点和终点生成空间区域多边形，以线性成本实现轨迹分类。在25个摄像头（包括16个未见位置）上，中位分类误差约3%，GEH值满足工程阈值，且比轨迹聚类基线方法更稳定、计算成本更低。


<details>
  <summary>Details</summary>
Motivation: 转向交通流计数对交叉口管理至关重要，但目前的采集方式主要依赖人工，因为每台摄像头的区域标注成本高昂。现有方法需要大量人工干预或先验知识，缺乏自动化和可扩展性。

Method: 提出一个六步骤的无监督管道：1) 通过目标检测和多目标跟踪提取原始车辆轨迹；2) 提取每条轨迹的起点和终点位置；3) 对这些起点和终点进行聚类，生成入口和出口区域多边形；4) 利用点-多边形包含关系（point-in-polygon）对后续轨迹进行分类；5) 通过19,152次管道执行进行系统统计分析，评估可配置参数；6) 使用参数和非参数检验框架识别三个显著参数并给出推荐配置。

Result: 在25个摄像头（包括16个未见位置）上，中位分类误差约3%，GEH值满足工程阈值。与两种轨迹聚类基线方法相比，本文管道在不同相机视角下更稳定，计算成本更低，但中位误差略高。此外，校准片段至少60分钟和高峰时段选择可进一步提升区域估计质量。

Conclusion: 本文证明了一种无需人工标注、无需相机标定、无需交叉口几何先验知识的无监督方法能够有效识别交叉口转向区域，且性能可满足实际工程需求。该方法具有较好的泛化能力和实用性，为大规模自动化交通流采集提供了可行路径。
...
TL;DR: OKA-CT提出一个器官层次知识增强框架，通过将放射报告解析为器官条件知识，并在两阶段学习中利用层次结构（器官级监督+结构化对比学习），在CT-报告预训练中显著提升了零样本异常诊断和图文对齐性能。


<details>
  <summary>Details</summary>
Motivation: 现有CT-报告VLP方法要么全局对齐、要么局部对齐，未能充分利用放射报告中"按器官组织、描述异常及属性"的结构化特性。

Method: 1) 通过报告解析+LLM辅助语义结构化，将自由文本转为器官条件知识；2) Stage1：器官级细粒度监督注入解剖学证据；3) Stage2：器官特异性报告证据引导结构化对比学习，利用层次语义软目标将共享器官发现的非配对样本视为弱正例。

Result: 在CT-RATE和RAD-ChestCT上零样本AUROC分别达84.9和72.2，优于此前CT VLP基线；检索和补丁遮挡分析显示更好的报告-图像对齐及对病变解剖区域的敏感性。

Conclusion: 器官层次知识增强能有效利用放射报告的结构化特性，提升CT-报告预训练的表征质量和可解释性。
...
TL;DR: EquiFusion首次提出运动学无关的随机3D人体运动预测模型，通过排列等变潜扩散架构，将骨骼连接作为显式输入，实现跨数据集泛化、零样本预测及更高效训练。


<details>
  <summary>Details</summary>
Motivation: 现有模型硬编码骨骼运动学结构，导致泛化能力差、无法跨数据集训练、需要复杂数据重定向，亟需一种运动学无关的通用解决方案。

Method: 提出EquiFusion，基于排列等变架构的潜扩散模型，将运动学连接作为显式输入参数，使内部计算天然与关节顺序和图结构无关。

Result: 在多个主流基准上达到SOTA，模型体积最高减小75%，训练和推理速度更快，支持跨数据集泛化及零样本任务（如部分遮挡预测、目标肢体生成）。
...
TL;DR: 本文提出MED-DSLC方法，通过领域监督训练和领域级logit缩放，解决了多专家领域分类中因独立训练导致的logit全局不可比问题，在保持领域内精度的同时显著提升跨领域鲁棒性（平均准确率提升15%）。


<details>
  <summary>Details</summary>
Motivation: VLMs（如CLIP）在微调至细粒度领域（如LoRA）时，虽提升了领域内精度，但导致领域外精度下降，形成碎片化的专家模型生态。多专家领域分类试图合并独立训练的LoRA，但独立训练使各领域专家无法产生全局校准的logit，异构logit尺度引发跨领域干扰和错误预测。

Method: 提出MED-DSLC，包含两个核心组件：1）领域监督训练（Domain Supervised Training），引入领域标签信息进行训练；2）领域级logit缩放（Domain-wise Logit Scaling），对每个领域的logit进行独立缩放以恢复全局可比性。该方法轻量级，仅需少量数据即可实现。

Result: 在多个细粒度基准上，MED-DSLC平均准确率提升15%，显著改善了跨领域鲁棒性，并且随着多领域分类问题规模增大，展现了良好的可扩展性。

Conclusion: 恢复输出级别的校准（logit全局可比性）是在高度数据不平衡的多领域专业化场景下实现真正零样本VLM的关键。
...
TL;DR: 提出了一种名为MMA-Former的3D Transformer架构，通过多窗口混合头注意力机制（WS-MoH）实现胆管癌神经侵犯（PNI）的非侵入性预测，在168例MRI数据上AUC达0.752，优于CNN和标准Transformer方法。


<details>
  <summary>Details</summary>
Motivation: 胆管癌中神经侵犯（PNI）是关键预后因素，但基于3D MRI的非侵入性预测极具挑战性，现有模型难以同时高效捕捉细粒度细节和全局上下文信息。

Method: 提出MMA-Former架构，核心包含Coarse-Fine Transformer (CFT)结构实现并行多尺度特征提取，并创新性地集成Window-Specific Mixture-of-Head attention (WS-MoH)机制。WS-MoH为每个3D窗口生成表征，并将整个窗口动态路由到专门或通用注意力头，实现空间自适应特征提取。

Result: 在168例T1加权MRI扫描回顾性数据集上，MMA-Former取得AUC 0.752，优于最佳CNN（0.708）和标准Transformer基线（0.681）。

Conclusion: WS-MoH机制通过增强注意力头专门化并减少冗余，在不增加参数的情况下提升了模型性能，验证了该架构在3D医学图像分析中的有效性。
...
TL;DR: TreeSoc 提出了一种基于深度优先搜索的树状推理框架，将足球视频问答分解为层次化子任务，通过自适应工具路由实现多步推理，在 SoccerBench 和 NExT-QA 上取得了 SOTA 性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型在复杂足球视频理解中存在跨模态对齐浅层、多步推理能力不足以及工具集成受限等问题，亟需一种更结构化的推理范式。

Method: 采用动态深度优先搜索（DFS）将复杂查询分解为有序子任务，构建树状推理结构；每个推理节点可自适应路由并调用领域专用模块，输出上下文相关的预测结果。

Result: 在 SoccerBench 上，TextQA、ImageQA、VideoQA 分别达到 85.2%、87.4%、82.2%；在 NExT-QA 上达到 74.16%，均达到 SOTA。

Conclusion: 结构化、工具增强的树状推理是鲁棒视频理解的有效范式，代码已开源。
...
TL;DR: 本文提出LoSA-Net，一种局部化、尺度自适应的3D MRI架构，通过Talking Neighborhood Attention (TNA)、Scale-Adaptive Feature Mixing (SAFM)和Cross-Scale Refinement and Alignment (CSRA)三个模块，实现对胆管癌神经侵犯（PNI）的术前预测，在168例患者数据上达到0.7567的AUC，优于多个基线模型。


<details>
  <summary>Details</summary>
Motivation: 神经侵犯（PNI）是肿瘤侵袭性的重要临床指标，影响手术决策，但MRI上PNI的细微特征常与邻近解剖结构相似，传统下采样或全局特征聚合会削弱这些细节信息，因此需要一种能保留边界细节的局部化、尺度自适应模型。

Method: LoSA-Net包含三个核心模块：1) Talking Neighborhood Attention (TNA)——通过局部自注意力与头部混合保留神经对齐细节；2) Scale-Adaptive Feature Mixing (SAFM)——利用多尺度深度可分离处理调制感受野；3) Cross-Scale Refinement and Alignment (CSRA)——在跨阶段间维护语义上下文与高分辨率边界的一致性。

Result: 在168例胆管癌增强MRI扫描数据上，LoSA-Net的AUC达到0.7567，在相同预处理和优化设置下优于代表性卷积和Transformer基线模型。

Conclusion: LoSA-Net通过局部化注意力与尺度自适应机制，有效解决了PNI预测中细微神经边界难以捕捉的问题，为术前无创评估提供了更可靠的深度学习方案。
...
TL;DR: 本文系统揭示了基于CLIP的开放词汇目标检测器（GroundingDINO、OWL-ViT、YOLO-World）存在两种固有偏差：尺度偏差（大物体得分虚高）和语义偏差（泛化查询得分被抑制），并证明阈值调整无法消除这些偏差，提出无参数温度缩放校正可提升小物体Recall@10达19.6%，但会牺牲部分排序精度。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇目标检测器通过视觉-语言相似度生成置信度分数，但这些分数并非可靠的位置概率估计，而是混杂了视觉尺度和语义查询特异性的偏差，限制了模型在检测任务中的可信度与泛化能力。

Method: 在COCO和LVIS数据集上，对GroundingDINO、OWL-ViT、YOLO-World三种基于基础模型的检测器进行控制实验，量化分析尺度偏差和语义偏差，并验证无参数温度缩放校正方法的效果。

Result: 尺度偏差（alpha=+0.064, r=0.579, p=1.29e-58）导致大物体分数系统性偏高；语义偏差（beta=-0.705, p=5.23e-41）抑制泛化查询的分数。阈值调整无效，无参数温度缩放校正将小物体Recall@10提升19.6%（p<0.01），但带来排序精度的轻微下降。

Conclusion: 从图像级预训练的基础模型迁移到区域级检测任务存在根本性局限，偏差部分可逆但无法完全消除。
...
TL;DR: 本文提出了一种非对称架构，将几何建模（粗粒度）与外观建模（细粒度）解耦，通过双边连接实现相互指导，从而大幅减少计算冗余。在32视图960P输入上，模型匹配优化方法的同时实现近800倍加速，并在参数更少、训练/推理开销更低的情况下超越现有可泛化模型。


<details>
  <summary>Details</summary>
Motivation: 现有可泛化3D高斯泼溅模型在处理长序列新视图合成时存在大量计算冗余，研究发现：（1）高质量NVS不需要高精度几何；（2）外观学习通常比几何恢复更容易。因此需要减少冗余、更高效地分配计算资源。

Method: 提出非对称架构，解耦几何与外观建模：几何分支处理粗粒度token（大部分参数），负责多视图重建；外观分支处理细粒度token（少量参数），负责细节捕获。两分支通过双边连接交互，实现任务间的相互指导。

Result: 在32视图960P输入上，模型匹配优化方法的同时实现近800倍加速，超越现有可泛化模型，参数更少、训练/推理开销更低。

Conclusion: 任务感知的非对称计算分配能有效减少冗余、提高参数效率，使小模型也能取得优异性能，整体提升效率。
...
TL;DR: 针对精确事件定位（PES）任务中自蒸馏方法忽略运动线索的问题，本文提出时间特征蒸馏（Temporal Feature Distillation），通过对齐骨干网络的时间敏感特征而非投影头输出，并结合监督预热与门控移位模块（Transformer Gate Shift），在仅用10%标注数据时即在FSPerf上提升mAP 4.54个点。


<details>
  <summary>Details</summary>
Motivation: 直接应用DINO等自蒸馏方法于精确事件定位（PES）效果不佳，因为缺少监督引导时会抑制微小但关键的运动线索，导致表征对事件边界不敏感。

Method: 提出半监督时间特征蒸馏（Temporal Feature Distillation），对齐骨干网络的时间敏感特征而非投影头输出；采用带预热调度的监督训练稳定学习过程；引入Transformer Gate Shift多尺度门控移位模块注入运动感知时间信息。

Result: 在四个细粒度体育基准数据集上，一致优于全监督和半监督基线；在FSPerf上仅用10%标注数据时mAP提升4.54个点；仅用80%标注数据时在两个数据集上匹配或超越全监督100%基线。

Conclusion: 所提方法有效保留运动敏感和边界感知线索，在半监督精确事件定位中达到甚至超越全监督性能。
...
TL;DR: SynCLIP提出了一种同义词连贯的语言-图像预训练框架，通过语义一致的注意力对齐（SSA）和空间注意力细化（SAR）模块，解决了CLIP-based开放词汇稠密感知中同义词引发的不一致性问题，并构建了同义词增强视觉语料库（SEViC），在多个基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP-based开放词汇稠密感知方法存在同义词引发的定位不一致问题——语义等价的表达（如"汽车"vs"轿车"）会产生不同的空间注意力模式，严重影响模型的鲁棒性和实际应用效果。

Method: 提出SynCLIP框架，包含三个核心组件：1）语义一致空间注意力对齐（SSA）模块，最小化原始表达与同义表达之间的注意力图差异；2）空间注意力细化（SAR）模块，选择性增强注意力图中语义最相关的空间区域；3）同义词增强视觉语料库（SEViC），为每个类别扩充多个同义词和文本定义以支持训练。

Result: 在多个基准数据集上的实验表明，SynCLIP显著提升了模型在多种语言变体下的定位一致性，并在CLIP-based OVDP方法中达到最先进性能。

Conclusion: SynCLIP通过解决同义词引发的注意力不一致问题，有效提升了开放词汇稠密感知的鲁棒性和准确性，证明了语义一致性约束在视觉-语言预训练中的重要性。
...
TL;DR: 本文提出了一种基于参考图像的人脸超分辨率方法，利用空间变换器对齐模块和自适应聚合函数，在较小模型规模下实现了多个数据集上的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 人脸超分辨率是病态问题，现有方法在高保真度重建上仍有挑战，且常用可变形卷积对齐不稳定。

Method: 1) 使用空间变换器（Spatial Transformer）代替可变形卷积进行更稳定的图像对齐；2) 设计自适应聚合函数，能从参考图像中提取高质量信息或在信息不可用时抑制其作用；3) 采用参考图像引导的超分辨率框架。

Result: 在多个数据集上达到最先进（state-of-the-art）结果，且模型规模相对较小。

Conclusion: 所提出的基于参考图像的人脸超分辨率方法，结合稳定的空间变换器对齐和智能聚合机制，有效提升了超分辨率性能，证明了小模型也能取得SOTA结果。
...
TL;DR: 提出了RTFVE模型，一种可集成于任何视频解码器、在普通CPU上实时运行的人脸视频增强方法，能有效提升低比特率压缩视频的感知质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频会议应用受带宽限制影响，深度学习增强方法难以集成到现代压缩标准中，且需要专用GPU硬件，实用性差。

Method: 提出RTFVE（Realtime Face Video Enhancement）模型，可与任何视频解码器轻松集成，在普通CPU上实时运行。

Result: 在多个低比特率设置下，RTFVE模型相比压缩视频基线显著提升了感知质量。

Conclusion: RTFVE为低带宽视频会议场景提供了一种实用、高效的实时人脸视频增强解决方案，代码已开源。
...
TL;DR: 本文提出一种快速面部视频增强框架，仅需10帧人脸图像、在100秒内训练模型，即可在低计算设备上实时增强低带宽视频通话质量，且无需修改现有视频会议应用。


<details>
  <summary>Details</summary>
Motivation: 全球仍有数百万用户因带宽限制而体验低质量视频通话，尽管他们拥有必要的硬件设备。现有解决方案往往需要修改应用或计算开销过大。

Method: 提出一个轻量级框架，可作为独立层置于现有视频会议应用之上。先利用10帧人脸图像快速训练（<100秒）增强模型，然后在通话中实时拦截并增强视频帧，最终在低算力设备（如笔记本CPU）上运行。

Result: 实验表明，该方法在定量指标和感知质量上均显著提升了压缩人脸视频的质量。

Conclusion: 该框架为低带宽视频通话提供了一种实用、高效的增强方案，无需修改现有应用即可部署，具备良好的实时性和可推广性。
...
TL;DR: WiFi-JEPA是一个自监督框架，通过预测掩码CSI嵌入而非重建原始信号，结合射线追踪仿真数据，在WiFi人体姿态估计任务上取得了SOTA结果，并解决了环境迁移和标注成本问题。


<details>
  <summary>Details</summary>
Motivation: 现有WiFi姿态估计器在环境变化时性能下降，且依赖昂贵的摄像头标注流水线，限制了规模化应用。

Method: 提出CSI特有的token化和链路掩码策略，对通道、时间、链路三个维度建模；使用射线追踪仿真生成多样化无标注CSI数据进行预训练；通过预测掩码潜在嵌入而非重建原始信号，避免硬件伪影干扰。

Result: 在Person-in-WiFi-3D上，单人和多人3D姿态估计均超越现有基线；仿真CSI与真实CSI互补；视觉SSL方法会降低性能，而WiFi-JEPA持续提升下游任务。

Conclusion: WiFi-JEPA通过自监督学习和仿真数据有效解决了WiFi姿态估计的环境泛化与标注瓶颈问题，验证了CSI-native预训练策略优于直接迁移视觉SSL方法。
...
TL;DR: 本文提出了一种基于双域细化的去雾网络（DDR-Net），通过雾霾先验提取器、细节增强块和空间-频率瓶颈细化三个模块，有效解决了现有去雾网络特征细化不足和局部结构表示弱的问题，在真实世界数据集上取得了领先性能。


<details>
  <summary>Details</summary>
Motivation: 现有端到端去雾网络存在两个关键问题：1）瓶颈阶段特征细化不足；2）编码器-解码器架构中局部结构表示能力弱。这些问题限制了去雾效果。

Method: 提出DDR-Net，包含三个核心模块：1）HPE（雾霾先验提取器）：在下采样雾图上直接提取多尺度雾霾先验；2）DE Blocks（细节增强块）：通过梯度感知卷积捕获多尺度结构信息，增强边缘和纹理恢复；3）SFBR（空间-频率瓶颈细化）：在瓶颈处联合利用空间和频率信息进行特征细化。

Result: 在真实世界基准数据集上优于现有去雾方法，在合成数据集上达到具有竞争力的性能。

Conclusion: DDR-Net通过双域细化和多尺度先验提取，实现了更有效的特征表示与重构，显著提升了单图像去雾效果。
...
TL;DR: 本文通过系统诊断实验发现，Video-LLM在长视频角色追踪任务上的高准确率并非来自真正的角色追踪，而是依赖性别等粗粒度线索。模型在更换问题中角色姓名时很少改变答案，且仅在跨性别时有所反应，开放生成任务中准确率大幅下降，表明模型无法区分同性别个体。


<details>
  <summary>Details</summary>
Motivation: 质疑当前Video-LLM在长视频角色追踪基准测试（如InfiniBench）中高分的真实性，验证其是否真的理解角色身份并实现追踪，还是利用更简单的捷径（如性别线索）来答题。

Method: 设计九种诊断条件的测试协议，对三种架构不同的开源Video-LLM（以Gemini 2.5 Flash为参照）进行系统测试，包括：更换角色姓名、分析性别维度、采用开放式提问、控制输入视频帧数量与字幕等。

Result: 更换角色姓名后模型仅4-31%改变答案；跨性别更换时反应更大（差距13-28%）；开放式提问下开源模型准确率下降18-25%，151个答案无全对；增加字幕、最佳帧或双倍帧数均无改善。

Conclusion: 当前Video-LLM在长视频角色追踪任务上的表现并非来自真正的角色追踪能力，而是利用性别等表层线索作答。模型无法将视频内容与问题中指定的角色姓名建立可靠关联，基准测试分数存在误导性。研究团队发布了用于审计此类基准测试的诊断工具包。
...
TL;DR: 本文分析了视频扩散Transformer中注意力头对运动与结构的表征差异，提出无需参数更新的头部感知运动迁移框架，通过运动专用头的语义对应引导和选择性特征注入实现精确运动迁移。


<details>
  <summary>Details</summary>
Motivation: 现有视频DiT在运动迁移任务中表现不佳，主要因为对模型内部运动与结构表征的理解不足，缺乏细粒度的可控机制。

Method: 1) 分析视频DiT注意力头，识别运动专用头和结构专用头；2) 从运动专用头提取运动线索，结合语义对应引导优化；3) 通过选择性特征注入保留结构信息；4) 整个框架无需参数更新。

Result: 实现精确的运动迁移，同时为目标提示对齐提供结构保持能力，并且具有可解释性。

Conclusion: 头部级别的控制机制为DiT的可控视频生成提供了新的可解释基础，在不更新参数的前提下实现了有效的运动迁移。
...
TL;DR: 提出CUST（聚类单元级相似性 Transformer），通过跨窗口的相似块聚合与重叠注意力窗口，在图像超分辨率中实现全局-局部信息的高效融合，降低计算复杂度的同时保持高性能。


<details>
  <summary>Details</summary>
Motivation: ViT在图像超分中表现优异，但其二次计算复杂度导致高延迟和内存消耗；现有窗口注意力机制虽降低复杂度，却牺牲了长距离依赖建模能力。亟需一种兼顾效率与全局建模的方法。

Method: 1）聚类单元级相似性：每个patch在扩大区域范围内聚合相似patch，捕获全局上下文；2）重叠注意力窗口：捕获局部依赖；3）高频细节提取：通过原始特征与下采样-上采样特征的残差差分，显式提取高频信息。

Result: 在计算效率与恢复性能之间达到实用平衡；相比近期全局上下文或轻量模型，内存占用更低、推理速度更快。

Conclusion: CUST通过创新的聚类单元级相似性机制，有效解决了ViT超分模型中全局建模与计算效率的矛盾，为实际部署提供了可行方案。
...
TL;DR: 提出了一种无需相机标定的低光原始图像去噪方法，通过预测黑电平误差来校正颜色偏差，在多个数据集上表现优异，并发现SIDD数据集存在颜色偏差问题。


<details>
  <summary>Details</summary>
Motivation: 现有低光去噪方法依赖大量干净-噪声图像对或精确的相机标定，成本高且不实用，亟需一种相机无关、免标定的去噪方案。

Method: 识别黑电平误差导致的颜色偏差是性能下降的主因，提出偏置估计网络（bias estimator network）将黑电平误差作为噪声输入的全局特征进行预测，从而实现颜色校正。

Result: 在ELD、SID、LRID数据集上超越了其他盲去噪方法，尤其在颜色校正方面表现突出，部分情况下甚至超过强监督方法。同时发现SIDD数据集存在颜色偏差问题并提供了修正后的基准。

Conclusion: 提出了一种有效的免标定低光原始图像去噪范式，揭示了数据集偏差问题，为实际应用提供了更可行的解决方案。
...
TL;DR: 本文提出基于特征差异和熵差异的门控机制（FDG和EDG），用于U-Net中全局与局部特征融合，实验表明优于现有注意力融合方法。


<details>
  <summary>Details</summary>
Motivation: 现有注意力融合方法仅从全局特征或全局-局部相关性推导注意力权重，忽略了特征流之间的差异信息。

Method: 提出两种差异门控方法：FDG直接使用全局与局部特征的绝对差值生成自适应门控图；EDG通过信息熵衡量各特征流的表征确定性，利用符号熵差推导注意力权重。两种方法均生成耦合门控图同时调制全局和局部特征。

Result: 在医学图像分割、遥感图像去云和语音分离任务上，两种方法均优于现有注意力融合方法，其中EDG表现更佳。

Conclusion: 提出了一种基于差异信息的多尺度特征融合新范式，为U-Net结构设计提供了新思路。
...
TL;DR: 本文提出一种面向全向立体视觉的置信度估计训练策略，通过重新解释3D编解码器的匹配响应作为内在置信度信号，并引入扫描特征体素重采样技术联合学习表面法线等元信息，在无需额外模块的情况下提升深度一致性和置信度估计效果。


<details>
  <summary>Details</summary>
Motivation: 宽基线全向立体匹配中存在大量模糊匹配，现有置信度估计方法需依赖辅助头或多遍推理，效率低且泛化性差。

Method: 1) 利用3D编解码器匹配响应的期望值作为内在置信度信号；2) 引入扫描特征体素重采样，通过回归的正匹配索引重采样3D CNN响应特征，再由2D CNN预测表面法线等元信息。

Result: 所提方法在置信度估计和表面法线预测上均取得性能提升，同时保持了部署实用性。

Conclusion: 直接惩罚模糊响应并联合学习几何正则化可有效提升深度一致性，无需额外模块即可实现高效、泛化的预测。
...
TL;DR: FlowPET 提出了一种基于辛几何的物理信息框架，通过可分离哈密顿系统参数化后验动力学，实现体积守恒的PET重建，有效防止弱病灶信号在重建过程中被数值湮灭，在多个数据集上超越现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型在低计数PET重建中存在相空间收缩导致的耗散性，使得弱病灶信号被数值湮灭（wash-out），缺乏对低对比度病变信号的几何保护机制。

Method: 将PET重建重新表述为辛相空间中的体积守恒输运问题；使用可分离哈密顿系统参数化后验动力学，保证无散度向量场；引入基于Range-Null空间分解的共轭边界条件，在范围空间强制执行数据一致性，在零空间注入随机不确定性；采用辛流匹配训练和辛跳点积分器推理。

Result: 在BrainWeb、临床儿科和UDPET数据集上，FlowPET在SSIM和PSNR指标上超越最先进的确定性和随机性基线方法，特别是在低对比度病灶恢复方面表现优异。

Conclusion: 施加哈密顿结构约束为高噪声环境下的医学逆问题提供了稳健的几何保护机制，证实了辛几何框架在低计数PET重建中的有效性。
...
TL;DR: 针对多模态大模型在"用图像思考"范式中频繁调用外部工具导致高计算开销和推理延迟的问题，提出BEE（Beyond the Eye）隐式视觉工具范式。BEE通过将视觉工具调用行为融入训练目标，并设计两阶段训练（形式化思维链微调+自调节奖励对齐），使模型具备自调节的工具调用机制，在细粒度视觉感知任务上达到SOTA，同时显著降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在"Thinking with Images"范式下需要频繁调用外部视觉工具和重复编码图像，导致巨大的计算开销和推理延迟。目标是构建一种能够自调节、避免冗余工具调用的高效视觉感知范式。

Method: 提出BEE范式：(1) 形式化思维链监督微调——构建带有结构化工具槽位和混合调用状态的CoT轨迹，激活模型隐式工具表征和自适应切换能力；(2) 自调节奖励驱动对齐——提出净工具增益指标量化冗余工具使用，并设计自调节奖励机制惩罚无效工具依赖，鼓励模型在内部知识不足时才调用隐式工具。

Result: 在细粒度视觉感知任务上达到SOTA，在通用推理任务上保持竞争力，同时大幅提升推理效率。

Conclusion: BEE通过隐式工具范式和自调节机制有效解决了外部工具频繁调用带来的效率瓶颈，为多模态大模型的高效视觉感知提供了新思路。
...
TL;DR: 提出GHOST框架，利用视觉基础模型将透明物体区域转换为不透明且结构一致的RGB表示，无需重训练下游模型，显著提升深度估计与3D重建精度。


<details>
  <summary>Details</summary>
Motivation: 透明物体违反朗伯假设，导致深度估计和3D重建中几何信息严重退化，现有方法难以处理此类问题。

Method: GHOST框架包含四个模块：(1)TransDINO和(2)TransDecomp分离掩码与透明物理属性；(3)DAF-Net恢复表面法线先验编码几何曲率；(4)GeoSemTransNet融合多模态线索合成富含纹理的不透明RGB图像。

Result: 在透明物体上显著提升了当前最先进的深度估计与重建模型的精度，恢复了关键的光度线索。

Conclusion: 几何引导的预处理框架能有效将透明物体转换为不透明表示，无需重新训练下游模型即可提升性能。
...
TL;DR: 该论文提出Affective Marker Fusion (AMF)系统，结合情感专用文本、音频、视觉表示和语言犹豫线索，通过AP加权集成在固定决策阈值下，针对ABAW 2026 BAH挑战赛中的矛盾/犹豫（A/H）识别任务，在测试集上达到0.731宏F1分数。


<details>
  <summary>Details</summary>
Motivation: 解决ABAW 2026 BAH挑战赛中的矛盾/犹豫（A/H）识别问题，即从短视频中预测人物是否表现出矛盾或犹豫情绪。

Method: 提出AMF（Affective Marker Fusion）系统：1）结合情感专用文本、音频、视觉表示；2）利用ASR擦除时间特征（从语音识别删除的填充词和犹豫停顿中提取16个特征）作为非语言通道；3）通过可靠性门控进行融合；4）采用AP加权集成，在固定决策阈值下进行分类。

Result: ASR擦除时间特征是最强且最独立的非语言通道（AP 0.718，与所有其他成员的相关性0.11-0.36）。AP加权固定阈值方法在测试集上达到0.731宏F1，优于在验证集上过拟合的加权方法（验证集0.741，测试集0.690）。

Conclusion: 跨模态冲突设计对BAH任务帮助不大；语言是最强的通道，情感专用音频是有用的第二通道；校准比架构更重要。简单方法结合适当校准可超越复杂过拟合方案。
...
TL;DR: 提出了SISA-Rec，一种将语义信息（BERT文本嵌入）与项目ID嵌入通过门控融合和语义自注意力机制结合的Transformer序列推荐框架，在高度稀疏数据集上显著优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的序列推荐主要依赖项目ID嵌入，未能充分利用项目的语义信息，导致在数据稀疏和冷启动场景下表现不佳。

Method: 1) 通过门控融合模块融合项目ID嵌入与BERT文本嵌入；2) 将语义相似度注入自注意力机制；3) 基于注意力的聚合模块构建用户表示；4) 联合优化BPR损失和对比对齐损失，对齐行为空间与语义空间。

Result: 在Amazon Beauty和Amazon Toys & Games（稀疏度99.93%）上，SISA-Rec全面超越SOTA。相比BERT4Rec，HR@10提升16.6%/23.1%，NDCG@10提升10.3%/17.9%，冷启动场景下提升尤为显著。

Conclusion: 将语义信息融入注意力机制可提高推荐的准确性和可靠性，尤其在用户行为数据稀缺时，语义信息能有效弥补行为信号的不足。
...
TL;DR: TC-MAF提出了一种基于锚点的多证据融合框架，结合多模态检测器、Dinomaly证据和跨模态一致性线索，通过固定像素级融合公式和训练分散置信度（TDC）项，在MVTec-3D上达到了0.979图像级AUROC和0.990像素级AUPRO，取得了最优结果。


<details>
  <summary>Details</summary>
Motivation: 多模态异常检测中，RGB重建的辅助信号在不同产品类别间可靠性不一致，且测试时无法按类别选择策略，现有方法缺乏统一的、无需类别先验的融合方案。

Method: 提出了TC-MAF，包含一个基础锚定的多证据融合设计，结合多模态检测器、Dinomaly证据和跨模态一致性线索，使用固定像素级融合公式。引入轻量级训练分散置信度（TDC）项，仅利用正常训练统计数据来缩放辅助模态的参与程度。

Result: 在MVTec-3D上达到0.979图像级AUROC和0.990像素级AUPRO，在检测和定位任务上均取得最佳均值结果。融合结构本身是主导因素，TDC提供较小但可复现的校准增益。

Conclusion: TC-MAF在多种设置下（池化统计变体、辅助分支/骨干替换、小样本、缺失3D、跨数据集）均保持有效，代码已开源。
...
TL;DR: 本文发现给VLM输入深度图反而会损害其空间推理能力，并提出无需训练的深度序数提示方法（DOP），将单目深度转化为针对查询对象的文本序数线索，从而有效提升空间推理性能。


<details>
  <summary>Details</summary>
Motivation: VLM在进行空间推理（如物体远近、前后关系、3D布局）时表现不佳。虽然深度图看似是自然的补救措施，但实验发现输入深度图像反而会使性能下降，因此需要探索更有效的深度信息注入方式。

Method: 提出Depth-Ordinal Prompting (DOP)，一种无需训练的方法。将单目深度估计结果转化为针对查询对象的文本序数提示（如"object A is closer than object B"），不添加深度图像、不训练模块、不注入特征、不使用标签。

Result: 在多个基准、模型和深度估计器上，DOP在伪深度提供可靠物体级排序时显著提升空间推理能力，在原始图像本身表现良好时保持中性。与最强的免训练深度提示方法相比具有竞争力，且更简单、更有针对性。

Conclusion: 核心发现是"形式依赖性"：同样的深度信号以图像形式呈现会损害性能，但以文本形式告知则能帮助模型。这表明深度信息并非无法被VLM获取，而是在以图像呈现时难以被下游推理有效利用。
...
TL;DR: GDP.pdf是一个由10个领域专业人士创建的100道PDF问答基准测试，覆盖文本提取、表格/图表理解、跨引用、空间推理等11种能力。评估7个前沿多模态模型，最佳模型通过率仅15%，最差仅1%。主要错误源于表格错位、图表误读、脚注遗漏、扫描噪声等重复性模式。


<details>
  <summary>Details</summary>
Motivation: 现有文档AI基准测试（如OCR、布局分析、图表问答等）各自孤立评估能力，高分不反映模型能否回答专业人士对PDF的实际问题。需要一个直接衡量真实场景问答能力的基准。

Method: 由10个领域专业人士撰写问题-文档对，仅当至少两个前沿多模态模型在"实质性"方面（错误答案、遗漏关键证据、捏造信息）失败时才保留问题。每项附带原子评分标准，支持分级评分和严格通过率，并用三级11种能力标签分类。

Result: 评估7个前沿模型，最佳通过率15%，最差1%。主要错误模式包括：表格对齐错误、图表误读、脚注和排除条款被跳过、楼层平面图符号计数错误、扫描噪声干扰、修正条款未被正确覆盖。

Conclusion: 当前多模态模型在处理专业领域PDF的真实问答任务上表现极差，存在系统性缺陷。完整的100项基准已公开发布，可用于推动模型在文档AI真实场景中的能力提升。
...
TL;DR: Slot-RAE是一个简单、完全集成的框架，直接在视觉基础模型（如DINOv3）的连续语义特征空间中运行，采用特征空间扩散过程，无需VAE瓶颈和预训练生成先验，在COCO数据集上实现了SOTA性能，具有更高的重建质量和零样本组合能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的物体中心模型通常依赖重型的预训练生成先验（如Stable Diffusion）和外部VAE潜在空间，导致管线复杂、计算成本高。作者希望设计一个更简单、高效且完全集成的框架，避免这些外部依赖。

Method: 提出Slot-RAE框架，直接在冻结的视觉基础模型（DINOv3）特征空间上运行；采用Slot Attention进行物体分解，使用Diffusion Transformer (DiT)作为解码器进行特征空间扩散，并引入Representation Alignment (REPA)头进行特征对齐。整个生成核心（Slot Attention和DiT）从头训练，无需文本到图像的先验知识。

Result: 在COCO数据集上，Slot-RAE在无监督物体发现方面表现与SOTA相当，在图像重建保真度上更高，零样本组合能力更强，同时速度更快、计算效率显著优于现有物体中心潜在扩散模型。

Conclusion: 证明了一个简单、无需外部生成先验和VAE瓶颈的框架可以直接在VFM特征空间中实现SOTA性能，为物体中心场景理解提供了一条更简洁高效的技术路线。
...
TL;DR: 本文提出DynEval，一个动态评估框架，通过构建50万对GenDB数据集和25万条DynEvalInstruct指令数据集，结合课程学习策略微调小型评估器（2B/4B），在11个基准上实现与人类判断更高的一致性，并能细粒度分析36个T2I模型在42个子类别和9个语义维度上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有自动评估器依赖静态提示集，难以捕捉部分提示错位、组合错误、视觉合理但语义不正确等细微失败模式，且缺乏可扩展的大规模训练数据。

Method: 构建GenDB（50万对提示-图像，来自DiffusionDB的分层生成策略）和DynEvalInstruct（25万条三元组，结构化评估管道蒸馏）；使用课程学习对小型VLM进行全参数微调，蒸馏大型教师模型的评估能力。

Result: 在11个基准上，DynEval-2B/4B与人类判断的相关性高于现有评估器；可对36个T2I模型在42个子类别和9个语义维度进行细粒度能力和失败模式分析。

Conclusion: DynEval通过大规模合成数据与知识蒸馏结合的方法，有效提升了T2I模型评估的准确性和细粒度，为自动化评估提供了可扩展的新范式。
...
TL;DR: 提出了一种利用轻微视角变化的两帧图像进行肖像抠图的方法，通过前景-背景视差提供互补信息，在小幅相机运动（如连拍）场景下有效提升抠图质量。


<details>
  <summary>Details</summary>
Motivation: 单图像抠图在纹理复杂的前景/背景场景下效果不佳，现有方法依赖绿幕、偏振光等特殊采集设备，缺乏实用性。作者希望利用连拍中自然产生的小幅视角变化带来的视差信息，实现更实用的两帧抠图。

Method: 输入两幅微小视角变化的图像，先估计trimap和前景/背景运动，构建对齐视图；然后网络使用背景对齐对进行直接融合，同时通过交叉注意力机制利用前景对齐线索进行误差补偿，最终输出高质量抠图结果。

Result: 在复杂肖像抠图场景下，该方法比强单图像抠图基线方法恢复了更精细的细节和更准确的前景颜色。

Conclusion: Parallax Portrait Matting是一种实用且有效的两帧抠图方法，利用自然产生的视差信息克服了单图像抠图的局限性，无需特殊采集设备，适用于连拍等常见拍摄场景。
...
TL;DR: ARLA是一种自适应分辨率标签聚合方法，在评估分割模型时动态调整标签和预测的分辨率，以减少标签噪声和边界不一致性对评估结果的影响，并在洪水预测模型中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注用弱标签训练模型，但缺乏在不可靠标签下评估模型性能的工具。标签在类别边界处常存在不一致、噪声或模糊性，导致评估结果不可靠。

Method: 提出ARLA方法，在计算评估指标之前，动态自适应地调整标签和模型预测的分辨率（即进行聚合），通过可调参数适应标签精度或噪声水平。

Result: ARLA成功克服了森林区域标签不一致以及厚云覆盖区域标签错误的问题，在真实洪水预测模型中更好地分析了模型行为。

Conclusion: ARLA提供了一种评估分割模型的新范式，通过从噪声中提取更清晰的信号来揭示模型的真实性能，核心是利用标签信息同时最小化标签误差。
...
TL;DR: HandFlow 是一种基于生成式流匹配（flow matching）的框架，用于从单目视频中实现时间上连贯的 3D 手部姿态和形状估计。它通过单次 ODE 积分去噪整个时间窗口的 MANO 参数，采用 Flux 风格的双流 Transformer 捕获长程依赖，并引入置信度感知的连续掩码机制处理噪声或缺失观测。在 DexYCB 和 HOT3D 上取得了 SOTA 结果，世界空间姿态误差降低 30% 以上，加速误差最低，且推理速度达 47 fps，比最快的先前方法快约 12 倍。


<details>
  <summary>Details</summary>
Motivation: 现有的逐帧判别式回归器缺乏时间上下文，预测抖动；时序模型虽提升一致性但仍是确定性回归器，在遮挡和运动模糊导致的模糊观测下表现脆弱。生成式建模通过学习合理手部运动序列的先验，能在视觉证据不完整时实现连贯的手部状态恢复。

Method: 提出 HandFlow，一种全生成式流匹配框架。给定视觉和骨骼观测，通过单次 ODE 积分去噪整个时间窗口的 MANO 参数。采用 Flux 风格的双流 Transformer 跨全序列建模长程依赖（无自回归解码），以及置信度感知的连续掩码机制，将观测特征与可学习掩码令牌混合以处理噪声或缺失观测。

Result: 在 DexYCB 和 HOT3D 上达到 SOTA，世界空间姿态误差相比最强基线降低 30% 以上，加速误差在所有方法中最低，逐帧姿态精度具有竞争力。单 GPU 上处理 150 帧序列速度为 47 fps，比最快的先前视频方法快约 12 倍。

Conclusion: HandFlow 验证了生成式流匹配在单目 4D 手部重建中的有效性，在时间平滑性、世界空间精度和推理效率上均显著优于现有方法，为处理遮挡和运动模糊等模糊观测提供了新的方向。
...
TL;DR: 本文提出STAR-VTON，一种两阶段自回归虚拟试穿框架。第一阶段在潜空间进行结构合成（如衣物变形、人物布局），第二阶段在像素空间通过匹配精化器恢复精细细节。相比扩散模型，速度提升4倍以上且质量不降，像素空间精化器可作为插件模块提升现有VTON方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的VTON方法在压缩潜空间中丢失高频细节，而视觉自回归模型虽生成质量高、推理快，却缺乏有效的双条件机制用于虚拟试穿。作者进一步发现，不同子任务（结构合成 vs. 细节恢复）应在不同表示空间处理，由此提出两阶段框架。

Method: 首先提出VAR-VTON，将衣物条件和结构引导引入VAR模型用于潜空间VTON。在此基础上提出STAR-VTON，解耦为两阶段：(1) 潜空间结构合成（衣物变形、人体姿态等）；(2) 像素空间细节恢复，通过匹配精化器建立第一阶段生成结果与源衣物之间的密集对应，直接映射像素级细节。

Result: STAR-VTON在效率与保真度之间取得优异平衡：VAR-VTON相比扩散方法速度提升至少4倍且质量不降；像素空间精化器有效恢复精细细节，并可作为即插即用模块提升现有VTON方法。

Conclusion: 不同VTON子任务应在不同表示空间处理（潜空间负责结构合成，像素空间负责细节恢复），这一解耦思路既提升了效率又保证了生成质量，且精化器模块具有良好的泛化性。
...
TL;DR: 本文提出了一种基于分布式光纤传感（DOFS）的软质感知垫，利用光学频域反射（OFDR）技术将单模光纤嵌入柔性硅胶基质中，在2.6mm栅距下提供2250个传感点。在6名志愿者坐姿靠背实验中，该垫实现了呼吸频率和心率的二维空间映射监测，结果与参考系统吻合，展示了DOFS在生物医学近身感知中的应用潜力。


<details>
  <summary>Details</summary>
Motivation: OFDR基DOFS在生物医学应用中尚缺乏探索，尽管存在对高灵敏度、空间分辨率高且适形传感界面的需求。本研究旨在填补这一空白，开发一种大面积、软质的传感界面用于生理监测。

Method: 将单模光纤以蛇形布局嵌入柔性硅胶基质中，形成软质感知垫，栅距2.6mm，在50Hz采样频率下提供2250个传感位点。在6名健康志愿者的坐姿靠背配置下进行测试，并与参考可穿戴系统对比。

Result: 系统成功实现了二维映射输出，反映背部机械耦合和心肺诱导扰动；估算的心率和呼吸率与参考测量结果具有良好一致性，且空间和时间模式具有生理一致性。

Conclusion: 研究证明了将大面积分布式感知、空间映射和定量心肺监测结合在基于DOFS的软质近身界面中的可行性。
...
TL;DR: 提出LaGuadia框架，通过临床语言引导的自适应知识蒸馏，用87M参数的小模型在病理图像任务上媲美GigaPath、UNI等大规模基础模型。


<details>
  <summary>Details</summary>
Motivation: 病理基础模型（PFMs）计算成本巨大，现有多教师知识蒸馏方法采用统一的权重分配，忽视了组织异质性，导致蒸馏效率低下。

Method: 三阶段流水线：1）从病理报告中提取临床关键词；2）通过Vision-Language元教师（MedSigLIP）对齐视觉特征与关键词；3）基于教师与临床语义的对齐程度进行自适应加权蒸馏。

Result: 87M参数的LaGuadia模型在WSI描述生成、视觉问答和slide级分类任务上达到或超越GigaPath、UNI等大规模基础模型，具备强事实一致性和鲁棒泛化能力。
...
TL;DR: 本文提出双净化框架（DPF），通过对抗性发型净化（AHP）和对比几何净化（CGP）两种正则化方法，解决发型迁移中参考图像身份泄露和秃头图像瑕疵泄露的问题，实现高保真、保留身份的发型迁移。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在零样本发型迁移中面临两个泄露问题：1）发型特征携带参考图像的原始身份或姿态信息（身份泄露）；2）"秃头"图像中的残留伪影会传播到最终合成结果（瑕疵泄露）。

Method: 提出双净化框架（DPF），包含两个训练时正则化器：1）对抗性发型净化（AHP）：基于互信息启发的对抗目标，抑制发型特征中的身份可预测性；2）对比几何净化（CGP）：在ControlNet路径引入对比学习目标，减少模型对秃头条件中几何伪影的依赖。

Result: 在多个基准数据集上达到最先进性能，实现高保真、身份保持的发型迁移。

Conclusion: 通过联合净化发型表征和几何路径，DPF有效解决了发型迁移中的双重泄露问题，验证了双净化策略的有效性。
...
TL;DR: SalientGS提出了一种统一的SfM-to-3DGS流水线，通过重要性引导的MCMC高斯分配（基于多视图残差聚合）实现端到端重建，15分钟内达到SOTA感知质量。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景重建方法受限于昂贵的SfM预处理和固定的位姿接口，缺乏端到端的统一框架。

Method: 提出重要性引导的MCMC高斯分配：聚合多视图残差为每高斯的欠拟合/冗余信号，定义平滑的重要性加权采样分布，引导高斯生成和迁移到欠拟合区域，同时保持SGLD动力学不变。

Result: 15分钟内实现端到端重建，达到SOTA感知质量（LPIPS等指标）。

Conclusion: SalientGS通过统一的SfM-to-3DGS流水线和自适应高斯分配策略，显著提升了重建效率与质量。
...
TL;DR: 本文提出了一个统一框架，结合人工标注、CT增强技术和自监督基础模型（60,000张未标注CT预训练），构建了包含1598例病例、14种心脏结构的最大规模标注数据集。该框架在五个外部数据集上优于现有工具，自监督预训练显著提升了低数据场景下的标注效率，且数据质量比模型架构更重要。框架支持人群表型分析，并开源了数据、代码、模型权重等资源。


<details>
  <summary>Details</summary>
Motivation: 心脏CT结构量化受限于测量的可扩展性，缺乏能用于常规临床实践的高效、全面的分割方法，现有工具在准确性和全面性上存在不足。

Method: 采用人机协同标注流程、心脏CT数据增强技术，以及基于60,000张未标注心脏CT扫描预训练的自监督基础模型。构建了包含1598例、14种心脏结构的大规模标注数据集，并在多个架构（卷积、Transformer、状态空间模型）上进行基准测试。

Result: 在五个外部数据集上，该框架的分割准确性和全面性均优于现有开源工具。自监督预训练在低数据量外部评估中提升最显著。不同架构性能相近，表明数据质量和预训练比架构选择更关键。

Conclusion: 该工作通过开源最大规模心脏CT标注数据集及相关资源，为常规CT扫描中的机会性心脏表型分析提供了可复现的基础，证明了数据质量和预训练策略的重要性。
...
TL;DR: 本文提出通过联合建模MRI图像与DICOM元数据，实现解剖结构与对比度相关表现的分离表示学习，并基于此构建了跨模态、跨站点的统一解剖保持协调模型。


<details>
  <summary>Details</summary>
Motivation: MRI采集存在显著变异性（不同扫描仪/协议下相同解剖结构呈现不同图像），导致学习到的表示混淆了生物学结构与采集相关表现，限制了可解释性、泛化能力和临床部署。

Method: 利用大规模临床脑MRI数据，联合建模MRI图像与DICOM元数据，学习分离解剖结构（anatomical representation）与对比度依赖表现（contrast representation）的表征；在此基础上构建解剖保持的协调模型，支持基于图像或元数据条件的跨模态/跨站点适应。

Result: 对比度表示能组织异质采集、支持序列理解、检测图像-元数据不一致；解剖表示能抑制采集特定变异同时保留生物学相关信息。

Conclusion: 采集变异性是成像过程中的结构化组成部分，可以被建模、审计和控制，为大规模医学影像中的采集感知表示学习奠定基础。
...
TL;DR: 针对事件相机在远距离无人机感知中因事件稀疏、碎片化导致的跟踪不稳问题，提出ASUMOT框架，通过运动一致性建模将碎片化事件聚合成身份一致的无人机轨迹，并发布了高精度事件级无人机数据集ES-UAV。


<details>
  <summary>Details</summary>
Motivation: 事件相机虽具备微秒级时间分辨率和高动态范围，但远距离无人机产生的事件响应稀疏、碎片化且含噪声，一个语义目标可能分裂为多个空间分离的斑块，导致异步跟踪产生重复轨迹和不稳定身份。

Method: 1) 局部运动一致性估计器触发可靠候选斑块；2) 轻量级多任务验证器提供无人机置信度和运动方向线索；3) 运动一致性聚类将碎片化斑块聚合成身份一致的无人机轨迹。

Result: 在公开无人机跟踪数据和自建ES-UAV数据集上，ASUMOT在保持异步事件处理的同时，改善了准确率与效率的权衡。

Conclusion: ASUMOT通过运动一致性有效解决了事件相机远距离无人机感知中的碎片化问题，实现了更鲁棒的异步检测与跟踪。
...
TL;DR: SLVMBench是首个联合评估视频大模型能否从长视频记忆中学习技能并应用于实时任务的基准。该基准将2-3小时视频流中包含的教程嵌入无关视频中，要求模型从中提取程序性知识并回答实时问题。实验表明，当前最先进的视频LLM在此任务上表现挣扎，揭示其从长视频中学习和应用技能的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有长视频理解基准只强调被动理解，而技能学习基准依赖简短即时演示，缺乏评估模型从长视频记忆中提取程序性知识并迁移到实时任务的完整能力。

Method: 构建包含2-3小时视频流的基准，其中嵌入教程视频和无关视频；设计需应用所学技能回答的实时问题；采用亚秒级时间校准的人工标注、消除常识猜测的工程化问题以及精心整理的教程。

Result: 最先进的专有和开源视频LLM在学习与应用技能知识方面表现困难，且当技能知识位于长视频记忆中时性能显著下降。

Conclusion: 现有视频LLM在从长视频记忆中学习并实时应用技能方面存在关键局限，SLVMBench成为首个研究该问题的基准。
...
TL;DR: HierCAD是一个层次化文本到CAD框架，通过将CAD构建树分解为对象级和部件级推理轨迹，并结合结构对齐与参数接地学习策略，在CAD序列生成和模型重建上超越先前方法。


<details>
  <summary>Details</summary>
Motivation: 现有文本生成CAD方法依赖大语言模型，但在复杂设计中难以保持结构一致性，几何参数接地也不准确。

Method: 提出HierCAD框架：(1) 将CAD构建树分解为对象级程序推理和部件级拓扑推理轨迹；(2) 提出SAPG学习策略，包括结构对齐（对齐拓扑推理与CAD参数段）和参数接地（通过结构保持的参数扰动和排序监督缓解捷径学习）。

Result: 在CAD序列生成和重建CAD模型评估中，HierCAD均优于现有最优方法。

Conclusion: 层次化分解与结构化学习策略能有效提升文本到CAD生成的结构一致性和参数准确性。
...
TL;DR: LMV-Net是一种结合解剖学互补的CC和MLO视图与显式时间对齐的纵向多视角乳腺癌风险预测模型，在EMBED和CSAW-CC数据集上优于现有方法。
...
Method: 提出LMV-Net，在显式对齐的纵向框架中联合分析CC和MLO两个互补视图，实现时空信息的融合建模。

Result: 在EMBED和CSAW-CC数据集上，LMV-Net在整体风险预测性能以及不同乳腺密度和癌症亚组中均一致优于现有方法。

Conclusion: 纵向多视角建模能增强风险分层能力，为个性化筛查、高危患者早期识别和筛查资源优化分配奠定基础。
...
TL;DR: 本文系统比较了PyTorch、ONNX Runtime、OpenVINO和TensorRT四种框架在工业机器视觉边缘部署场景下的推理性能，发现OpenVINO在CPU上最优，TensorRT在GPU上最优，但TensorRT对Transformer模型未优于PyTorch。


<details>
  <summary>Details</summary>
Motivation: 工业机器视觉系统对低延迟、数据安全和有限连接性有严格要求，边缘部署是优选方案，但现有研究缺乏对多种推理框架在工业部署条件下系统性的性能比较。

Method: 在多种CPU和GPU硬件平台上，对比四种框架（PyTorch、ONNX Runtime、OpenVINO、TensorRT）对传统CNN和Transformer视觉模型的推理时间。

Result: OpenVINO在CPU上推理时间最低，TensorRT在GPU上推理时间最低；但对于Transformer模型，TensorRT未优于原生PyTorch。

Conclusion: 不同推理框架在异构硬件和不同模型架构下的表现存在显著差异，工业部署时需根据硬件类型和模型架构针对性选择最优框架。
...
TL;DR: 本文提出ETBQ，一种在低比特PTQ前对FP模型进行预调谐的方法，通过模拟量化误差扰动引导模型进入对后续量化不敏感的区域，从而提升低比特量化精度，且不增加QAT的计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有低比特PTQ方法主要在后量化阶段最小化误差，但忽略了FP模型自身的量化误差容忍度。本文认为PTQ精度受限于FP模型本身对量化误差的鲁棒性，而非仅靠后处理重建。

Method: 在PTQ前，对SGD优化模型的FP权重施加模拟的权重量化和激活量化误差分布扰动，通过这种预调谐使模型收敛到对后续量化不敏感的损失景观区域。ETBQ不训练伪量化部署模型，输出FP模型给任意PTQ后端使用。

Result: 在CIFAR-100、Tiny-ImageNet、ImageNet和Cityscapes上，ETBQ在W2A4设置下分别提升2.14%（Tiny-ImageNet top-1）和5.80%（Cityscapes mIoU）等，一致证明了有效性。

Conclusion: ETBQ是一种轻量级的预调谐方法，能有效提升低比特PTQ精度，且无需QAT的高计算成本，兼容各种PTQ后端。
...
TL;DR: 本研究评估了自监督学习（SSL）在精准农业中，使用多传感器、多区域厘米级多光谱无人机影像的预训练效果。基于MoCo-v3和Masked Autoencoders预训练的Swin Transformer在作物-杂草语义分割任务中表现最优，超越了已有模型，并展示了跨传感器和跨区域的泛化能力。同时公开了芬兰多年多光谱无人机数据集。


<details>
  <summary>Details</summary>
Motivation: 自监督学习虽有潜力减少标注工作，但在高分辨率多光谱无人机影像上的有效性尚未被充分探索，主要受限于数据不足。

Method: 使用MoCo-v3和Masked Autoencoders对Transformer编码器进行预训练，采用四种光谱波段（绿、红、红边、近红外）以确保跨传感器兼容性，并在芬兰农业用地收集的多传感器、多年份、多区域无人机数据集上训练。

Result: Swin Transformer + MoCo-v3在两项下游任务（德国RedEdge-M和瑞士Sequoia数据集）上均表现最佳，超越了Doornbos等人的模型，并展示了跨传感器、跨区域的泛化能力。

Conclusion: 自监督预训练能够有效提升高分辨率多光谱无人机影像的语义分割性能，且模型具有良好的跨传感器和跨区域迁移能力。
...
TL;DR: 本文提出两种不确定性估计方法（高斯不确定性和分位数不确定性），用于地球观测回归任务（如建筑高度、冠层高度、生物量估计），利用Sentinel-1 SAR和Sentinel-2 MSI时间序列数据，在10米分辨率下实现了与确定性模型相当或更优的性能，同时提供校准良好的置信度估计。


<details>
  <summary>Details</summary>
Motivation: 地球观测回归任务（建筑高度、冠层高度、生物量估计）在 urban 规划、森林监测和气候政策中至关重要，但现有深度学习模型仅提供确定性预测，缺乏逐像素的可靠性指示。由于地表异质性、目标分布偏斜、传感器噪声和高值信号饱和等问题，不确定性估计对于可靠推理至关重要。

Method: 提出两种互补方法：(i) 高斯不确定性（Gaussian UC），在高斯假设下联合预测均值和标准差；(ii) 分位数不确定性（Quantile UC），估计第10、50、90百分位数以捕捉非对称和异方差误差分布。利用全年Sentinel-1 SAR和Sentinel-2 MSI时间序列数据建模 aleatoric 不确定性。

Result: 两种方法在三个代表性EO回归任务上匹配或超越了确定性基准和现有全球产品，同时提供校准良好、可解释且操作上有用的置信度估计。两种模型在冠层高度估计任务上均优于当前10米最先进的不确定性感知模型。

Conclusion: 该研究有效填补了地球观测回归任务中不确定性估计的空白，证明了在遥感回归中引入不确定性建模不仅能提供可靠性指示，还能保持甚至提升预测精度，对实际应用具有重要意义。
...
TL;DR: 本文提出了一种基于视频Transformer的远程身份证件验证系统（RIDVS），利用智能手机拍摄视频中的全息图检测来验证证件真伪，在中等规模数据集上实现了接近完美的准确率，相比现有最优方法召回率提升26.86%，准确率提升17.93%。


<details>
  <summary>Details</summary>
Motivation: 随着DeepFake和AI工具的发展，伪造身份证件日益猖獗，而远程身份认证的数字化需求又愈发迫切。传统安全特征检测需要红外传感器等专用设备，难以普及。全息图虽难以伪造且可通过智能手机摄像头检测，但尚未被充分利用，因此需要一种仅依赖手机视频的远程证件验证方案。

Method: 提出RIDVS系统，采用客户端（智能手机）拍摄视频、服务端验证的架构。核心是全息图检测方法，基于视频Transformer模型，该模型源自相关研究领域已验证的鲁棒模型，通过迁移学习适配到证件全息图检测任务。

Result: 在MIDV-Holo数据集上，相比最佳基线，召回率提升26.86%，准确率提升17.93%，达到接近完美的性能。即使在中等和小型数据集上训练也能保持高水平精度。此外还验证了模型在训练样本和计算资源上的节俭性（frugality）。

Conclusion: 基于视频Transformer的全息图检测方法在远程身份证件验证中表现优异，显著超越现有SOTA，且具有良好的数据效率和计算效率，为仅依赖智能手机摄像头的远程身份认证提供了可行且安全的解决方案。
...
TL;DR: 本文提出TECO框架，对CNN的三个维度（深度、宽度、分辨率）进行协同剪枝，通过两阶段重要性评估和启发式剪枝算法，在嵌入式硬件上实现精度与效率的最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 现有剪枝方法通常只关注单一维度（如深度或宽度），忽略了多维度协同剪枝的潜力，难以在嵌入式硬件上达到最优的执行效率。

Method: 1) 提出两阶段重要性评估框架，分别评估每个剪枝单元在维度内（局部重要性）和跨维度间（全局重要性）的综合重要度。2) 基于该评估，设计启发式渐进式剪枝算法，对三个维度进行联合剪枝，寻求精度与效率的最优权衡。

Result: 在多个基准测试上，TECO优于现有的最先进（SOTA）方法。

Conclusion: TECO通过多维协同剪枝，有效提升了CNN在嵌入式硬件上的执行效率，验证了跨维度联合剪枝的可行性与优越性。
...
TL;DR: HyperGS是一种前馈式、无需逐视频优化的方法，通过分解时空Transformer和查询式Transformer直接从视频中预测高斯表示，实现极快编解码速度并泛化到高分辨率视频。


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯散射的视频表示方法需要针对每个视频进行逐帧优化，导致编码速度慢且无法跨视频泛化。

Method: 设计分解时空Transformer提取视频token，再通过可学习查询式Transformer预测每帧的8参数高斯表示；引入基于秩的几何正则化器动态稳定训练，避免高斯退化。

Result: 编码速度比逐视频优化快10^4-10^5倍，零样本泛化到720p视频；在K400、SSv2、UCF101上PSNR提升+2.9-3.1 dB，且表示尺寸更小。

Conclusion: HyperGS将高斯散射的快速灵活渲染与前馈预测的速度和泛化能力结合，推动高斯表示成为实用且通用的视频编码方向。
...
TL;DR: 提出CFR-Net，通过共享教师-学生特征精炼和跨空间一致性学习，在医学图像异常检测中实现领域自适应，兼顾局部、多尺度和方向敏感性特征。


<details>
  <summary>Details</summary>
Motivation: 自然图像预训练网络对医学图像适应性有限，医学异常表现为细粒度局部偏移、多尺度上下文不匹配及方向敏感结构偏差，需要专门设计以弥合领域差异。

Method: CFR-Net包含解码前共享教师-学生特征精炼（MPFRM模块，共享参数处理多路径规则）和解码后跨空间一致性学习；同时引入方差敏感目标和动态"作业集"重组，实现层自适应一致性学习。

Result: 在医学基准上，仅用正常数据训练即达到具有竞争力的异常分类和强异常定位性能。

Conclusion: CFR-Net通过特征层面的领域对齐与多视角异常模式建模，有效解决了医学图像异常检测中的领域迁移和细粒度异常表征难题。
...
TL;DR: 本文提出Vinci2，一个基于连续自我中心视频的主动辅助系统。作者构建了EgoServe基准（3000+服务实例，10类服务，4种时间记忆范围），并提出EgoMemo——一种免训练、记忆增强的智能体，通过多尺度时间摘要、语义知识图谱和视觉嵌入档案三种记忆表征，在无需训练的情况下进行检索增强推理，判断是否需要提供上下文相关的辅助。


<details>
  <summary>Details</summary>
Motivation: 现有智能助手要么被动等待用户查询，要么对每个检测到的事件都做出响应，未考虑用户的当前活动、历史上下文或是否真的需要帮助。作者希望将主动辅助重新定义为上下文相关的决策问题，让智能体不仅感知正在发生的事情，还能基于累积的时间上下文推理何时以及是否应该介入。

Method: 提出了EgoMemo，一种免训练、记忆增强的智能体。维护三种互补的记忆表征：多尺度时间摘要（Multi-scale Temporal Summaries）、语义知识图谱（Semantic Knowledge Graph）和视觉嵌入档案（Visual Embedding Archives）。在每个时间步，通过检索增强推理（Retrieval-Augmented Reasoning）来判断是否需要提供辅助，并在需要时生成上下文相关的响应。

Result: 在EgoServe基准上建立了强基线，同时在现有自我中心视频基准上保持了竞争力。EgoServe包含超过3000个服务实例，覆盖10个服务类别，沿4种时间记忆范围组织（从即时安全警报到长期习惯辅导）。

Conclusion: Vinci2通过EgoMemo实现了从被动响应到主动辅助的转变，证明了基于时间上下文推理的主动辅助是可行且有效的。基准和代码已公开。
...
TL;DR: 提出了PSC-AVDN框架，通过三阶段解析-搜索-确认推理管道和结构化空间记忆，在训练-free设置下实现高空无人机导航任务，达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将多模态大语言模型直接应用于高空无人机导航存在方向定位不准确和缺乏显式空间记忆的问题，导致导航不可靠。

Method: 设计了PSC-AVDN框架：(1) 三阶段推理管道：解析阶段用LLM将模糊对话指令转为几何方向/目的地线索，搜索阶段用S-CoT逐步探索目标，确认阶段用C-CoT进行细粒度验证；(2) 结构化空间记忆（SSM）：融合多尺度视觉观察、空间视觉记忆、结构化几何记忆三种空间线索，提供全局上下文和长期一致性。

Result: 在ANDH和ANDH-Full数据集上，PSC-AVDN在训练-free设置下达到SOTA，甚至匹配或超越了若干需要微调的方法。

Conclusion: 训练-free框架通过结构化的推理和记忆机制，可以有效解决高空无人机对话导航中的方向定位和空间记忆问题。
...
TL;DR: 本文提出了一种基于扩散模型的分类框架，结合Transformer架构并引入自适应路由机制，用于术前从MRI预测胆管癌的神经侵犯（PNI），在降低计算开销的同时实现了AUC 0.731。


<details>
  <summary>Details</summary>
Motivation: 术前从MRI预测胆管癌的神经侵犯（PNI）对预后至关重要，但PNI的影像学特征微弱且超出肿瘤边界，传统CNN难以捕获长程空间依赖，Transformer虽能建模全局信息却对瘤周区域的细微噪声敏感，而扩散分类器虽有效但计算开销巨大。

Method: 将PNI预测建模为扩散分类问题，用Transformer作为去噪网络；为提高效率，引入自适应路由机制，在注意力头、空间token和MLP宽度三个维度进行动态路由。

Result: 在MRI数据集上，所提方法达到AUC 0.731，计算量为257.57 GFLOPs。

Conclusion: 通过扩散分类+Transformer+自适应路由的组合，在保持较好预测性能的同时显著降低了计算开销，为术前PNI预测提供了一种可行的方案。
...
TL;DR: 针对广角主摄像机仅能捕捉10-16名球员的局限性，本文量化了隐去场外球员对足球空间指标（如控球控制）的扭曲。通过模拟广播视口的开放基准测试，发现忽略场外球员会导致隐藏区控球误差高达25-27个百分点。提出一种无需训练、在线可用的角色锚定质心投票法，可将隐藏区误差减半至12-14个百分点。将插值方法集成到广播视频GSR流程后，能显著改变下游控球质量评分甚至逆转判决类别。


<details>
  <summary>Details</summary>
Motivation: 现有足球空间指标（如pitch control）假设可获取全部22名球员位置，但广播主摄像机视频最多只能看到10-16人。视频游戏状态重建（GSR）流程若不添加插值层，就会默认忽略场外球员，导致显著误差。目前缺乏对此类扭曲的量化基准和有效的在线插值方法。

Method: 建立了开放可复现的基准测试：用全场追踪数据（Metrica Sports三场比赛）模拟广播视角视口。评估了从"忽略场外球员"基线到一系列无需训练、在线可用的插值方法。最佳方法为角色锚定质心投票法：每个可见球员减去其运行角色偏移后投票估计全队质心，从而减弱视口子集偏差。

Result: 忽略场外球员导致隐藏区控球误差25.1-26.9个百分点，控球份额平均绝对误差11.1-13.4点。角色锚定质心投票法将隐藏区误差减半至12.2-13.8点，在36-60米视口宽度下将控球份额误差降至忽略策略的28-48%。对于<=9.6秒的遮挡（最接近已有先验学习工作的场景），中位数位置误差3.3-8.9米；但50-57%的隐藏球员超出此范围。集成到GSR后，Space-Creation Index变化15.6和17.2点，在真实世界杯广播窗口上翻转了判决类别。

Conclusion: 简单有效的在线插值方法能显著改善广播视频场景下的足球空间指标计算，且无需训练数据或事先学习，可直接集成到现有GSR流程中。但仍有相当比例的遮挡时间超出当前方法可处理范围，提示未来需要更鲁棒的方案。
...
TL;DR: 提出SAKD，利用学生动态特征作为条件生成视角扰动，实现单阶段、参数自由的知识蒸馏，在效率和多样性间取得更好平衡。


<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法存在监督信号多样性受限（单教师）或计算成本过高（多教师）的问题。单教师生成虚拟视图的方法在效率与多样性之间存在权衡：随机扰动缺乏可控多样性，结构化增强需要多阶段训练且参数线性增长。

Method: 提出Shift-Augmented Knowledge Distillation (SAKD)，利用学生模型不断演化的特征作为动态条件，通过参数自由的循环移位（cyclic shift）生成自适应、多样化的扰动视图，实现单阶段训练。

Result: 在CIFAR-100和ImageNet上，SAKD一致优于随机扰动方法，准确率与两阶段方法相当，但参数显著减少且无需预训练。

Conclusion: 将视角从教师静态特征转向学生动态特征，是一种更高效的知识蒸馏设计选择，为平衡效率与多样性提供了新思路。
...
TL;DR: 本文介绍了CVPR 2026@AdvML Workshop上关于自动驾驶视觉语言模型对抗攻击挑战赛的设计、规则、评估方案及结果，并总结了五项领先方案的共同发现：图像侧攻击更受青睐、场景级多视角优化优于单视角、QA类型和图结构可用于分配攻击预算、特征空间目标可提升黑盒迁移性、摄像头图像中的排版内容暴露了持续存在的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在自动驾驶场景中越来常被用于安全关键推理，然而这类模型在面对对抗性攻击时的鲁棒性尚未得到充分研究。本挑战旨在系统性地评估和推动对自动驾驶VLA模型的对抗攻击方法研究，填补这一安全评估空白。

Method: 基于DriveLM风格的多视角视觉问答，每个场景使用6个同步摄像头图像和结构化问答对。参赛者生成对抗性图像和纯文本后缀扰动，要求模型输出偏离参考答案，同时保持图像保真度并限制文本成本。比赛分两阶段，第二阶段引入隐藏的黑盒模型评估迁移性。

Result: 五项领先方案展现出五类共性模式：1) 图像侧攻击因文本后缀惩罚更受欢迎；2) 场景级多视角联合优化优于单视角；3) QA类型和图结构可作为攻击预算分配的先验；4) 特征空间目标函数有助于提升黑盒迁移性；5) 摄像头图像中的排版文字是持续可利用的脆弱点。

Conclusion: 这些发现为未来多模态自动驾驶系统的鲁棒性评估和防御设计提供了实用的参考依据。
...
TL;DR: MonkeyOCRv2是一个面向文档AI的视觉-文本预训练模型，构建了包含1.13亿张图片、覆盖17种语言的最大文档图像预训练语料库MonkeyDoc v2，并提出联合图像到文本生成与像素级文档重建的预训练策略。在五个代表性文档分析任务中均提升性能，作为冻结视觉编码器与0.7B轻量语言模型搭配，在MDPBench上超越3B模型，以约11倍小的视觉编码器取得2.8%的绝对提升，并在八个文档理解基准上优于基于CLIP、DINO和SAM的模型。


<details>
  <summary>Details</summary>
Motivation: 主流视觉编码器在自然图像上预训练，无法有效处理文档图像，因为密集文字和精细字符笔画需要字符级视觉感知。现有预训练方法缺乏专门针对文档图像的大规模预训练。

Method: 构建最大文档图像预训练语料库MonkeyDoc v2（1.13亿张、17种语言）；提出联合学习图像到文本生成（对齐视觉表示与文本内容）和像素级文档重建（保留字符笔画和布局细节）的预训练策略。

Result: 在文本识别、公式识别、文本检测、文档篡改检测、重叠文本分割五个任务上一致提升性能。作为冻结视觉编码器搭配0.7B轻量语言模型，在MDPBench上以约11倍小的编码器超越3B模型2.8%；在八个文档理解基准上优于基于CLIP、DINO、SAM的模型。

Conclusion: 面向文档的视觉预训练可以独立成为文档智能的基础，为文档AI提供专用视觉编码器。
...
TL;DR: 本文提出CycleGRPO框架，通过"区域→文本→区域"的循环强化学习范式，让MLLM同时成为演员和评论家，仅需区域输入即可联合优化区域理解和定位能力，无需文本标注，在多项基准上取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在区域理解（生成文本）和区域定位（文本→空间）两个任务上采用分离式管道，忽略了二者间的内在对偶性；同时依赖大量人工文本标注，成本高昂。

Method: 提出CycleGRPO框架：(1) 单一MLLM先作为演员生成区域字幕；(2) 然后切换为评论家，将生成的文本映射回空间域；(3) 采用质量感知的token级循环一致性奖励，通过物理定位精度评估文本语义可辨别性。整个框架仅需区域输入（掩码/边界框），无需文本真值。

Result: 基于SAMTok构建，无需任务特定微调，在区域字幕、区域VQA、接地对话、引用分割等多个基准上取得一致性能提升。

Conclusion: CycleGRPO提供了一种简单可扩展的方式，仅通过循环自评估机制即可同时提升MLLMs的区域理解和定位能力，为推进像素级能力提供了新范式。
...
TL;DR: 本文提出了一种基于高斯模型的移位变体滤波反投影神经网络，用于非圆形轨迹锥束CT的高效重建，通过可训练的2D高斯模型替代传统滤波过程中的轨迹相关部分，将参数量减少99%，训练时间缩短至原来的1/4，仅牺牲少量重建质量。


<details>
  <summary>Details</summary>
Motivation: 传统可微移位变体FBP模型参数过多、训练效率低，难以在实际非圆形轨迹锥束CT重建中高效应用，因此需要一种参数量更少、收敛更快的模型。

Method: 在传统可微移位变体FBP模型的滤波过程中，引入可训练的2D高斯模型来表示与扫描轨迹相关的部分，从而大幅减少可训练参数数量。

Result: 参数量减少99%，每个轨迹的训练时间缩短至原来的1/4，重建质量仅有轻微下降。

Conclusion: 该方法在保持重建质量的前提下，显著提升了模型的实用性和效率，为非圆形轨迹锥束CT在实际场景中的应用提供了有价值的解决方案。
...
TL;DR: FoundationGeo是一个两阶段框架，通过空间校准和精心设计的数据，显式桥接相对和度量预测。第一阶段用DINOv3初始化并在1020万样本上训练，学习仿射不变几何模型；第二阶段引入轻量级逐像素校准场（尺度场+射线方向校正场）实现度量一致的点图。同时发现焦距分布不匹配是关键瓶颈，通过Blender数据引擎合成多焦距数据提升鲁棒性。在7个基准上超越最强基线5.2%。


<details>
  <summary>Details</summary>
Motivation: 现有几何预测方法在相对预测（如深度、法线）与度量预测（如绝对尺度3D点图）之间存在鸿沟，且跨域泛化能力和相机内参（尤其是焦距）分布不匹配导致的零样本度量泛化性能下降问题尚未得到充分解决。

Method: 两阶段框架：Stage 1用DINOv3初始化，在包含1020万样本的多领域语料上训练，辅以局部细节监督，学习仿射不变几何模型。Stage 2引入逐像素校准：尺度场处理空间变化的度量对齐，射线方向校正场减轻点图几何中的方向偏差。同时利用Blender数据引擎合成不同焦距的训练数据以覆盖欠采样区域。

Result: 在七个零样本基准上，FoundationGeo展现出最强的跨域鲁棒性，一致性排名靠前，避免其他方法中出现的剧烈跨域性能下降，平均超越更重的基线方法5.2%以上。

Conclusion: 通过解耦相对几何学习与度量校准，并系统解决焦距分布偏差问题，FoundationGeo实现了鲁棒且度量一致的3D几何预测，为实际跨域应用奠定了重要基础。
...
TL;DR: 提出了一种基于MinHash相似度调度的渐进式课程学习框架，用于LLM驱动的神经网络架构搜索（NAS），在CIFAR-10上达到60%峰值成功率，并发现接口修复与课程调度针对不同失效模式。


<details>
  <summary>Details</summary>
Motivation: 现有LLM-based NAS缺乏对架构代码相似性的结构化利用，导致生成架构质量不稳定；同时存在权重漂移（weight drift）问题，侵蚀评估器接口先验知识。

Method: 使用128排列MinHash签名对归一化7-gram源码分片进行处理，将参考池划分为相似度带，按递增架构异质性呈现；各阶段最佳LoRA适配器被累积合并回主干模型。

Result: 在CIFAR-10上，高相似度水平达到60%峰值成功率（无需后处理修复）。消融实验显示：无修复时，基础模型（47% SR）远优于课程模型（7% SR）；加入部分接口修复后，两者均达53% SR。SVHN跨数据集迁移中，无课程预热的基础模型生成仅获27% SR，准确率60.5%。

Conclusion: 接口修复与课程调度解决不同的失效模式；合并级权重漂移会渐进式擦除评估器接口先验；课程预热对高难度合成任务（如SVHN上的unq-family锚点架构）尤为关键。
...
TL;DR: 提出了BPUL，一个即插即用的不确定性学习框架，通过可学习扰动调制器、先验引导重建模块和双空间对比损失，有效提升多种去雾骨干网络在真实世界场景下的性能，且推理阶段几乎不增加额外开销。


<details>
  <summary>Details</summary>
Motivation: 真实世界图像去雾面临空间非均匀、光照依赖和物理模糊等挑战，现有端到端方法将去雾视为确定性映射，忽视了退化特征、雾先验和跨域负样本中隐藏的不确定性信息。

Method: 1) 提出LPUM，通过可重参数化随机扰动估计通道和空间维度的特征敏感性；2) 提出PURM，利用透射率和大气光先验从恢复结果重建有雾图像，强制退化一致性；3) 提出D3CL对比损失，在干净恢复和雾图重建两个空间中使用真实与合成负样本进行正则化。

Result: 在五个真实世界成对去雾基准上，BPUL持续提升了多种代表性骨干网络的去雾性能，且推理时仅保留LPUM，额外开销极小。

Conclusion: 不确定性建模和先验引导的退化一致性约束能有效提升真实世界去雾质量，BPUL作为即插即用框架具有良好通用性和实用性。
...
TL;DR: 本文提出Motion4Motion，一种无需训练的视频运动迁移框架，通过建模运动流而非骨骼结构，实现了跨物种（如不同动物）之间的运动迁移。


<details>
  <summary>Details</summary>
Motivation: 现有基于骨骼的视频运动迁移方法依赖预定义人体骨架结构和有标注数据，难以泛化到不同物种的动物，且缺乏多样骨骼标注数据。

Method: 跳出骨骼框架，提出Motion4Motion，通过建模视频中角色的运动流（motion flow）来实现无需训练的运动迁移，不依赖骨架结构。

Result: 大量实验和新型应用表明，该方法在基线对比中表现优异，尤其擅长跨物种运动迁移。

Conclusion: Motion4Motion通过无骨架的运动流建模，有效解决了跨物种运动迁移的难题，且无需额外训练和标注数据。
...
TL;DR: 针对高速公路环境下RGB摄像头夜间感知性能严重退化的问题，本文提出JEAT框架，通过自适应扩展卡尔曼滤波融合事件流与RGB帧实现动态跟踪，并构建了SEHN大规模合成数据集以支持相关研究。


<details>
  <summary>Details</summary>
Motivation: 高速公路夜间缺乏人造光源、车辆高速行驶导致运动模糊、曝光不足和信噪比低，RGB感知系统可靠性严重受损，现有多传感器跟踪器依赖僵硬的硬编码优先级，无法灵活适应环境变化。

Method: 提出JEAT框架，将异步事件流与RGB帧统一为联合数据关联优化问题；采用自适应扩展卡尔曼滤波，通过NIS统计量连续估计测量噪声，动态加权融合两种模态：黑暗/高速时优先利用事件流，明亮/静态时利用RGB帧。

Result: 构建了SEHN数据集（CARLA生成），涵盖白天、夜间、无人工照明夜间等多种环境条件及不同交通密度，提供同步RGB图像与事件流。

Conclusion: JEAT框架通过自适应融合事件相机与RGB相机，有效应对高速公路夜间感知挑战，SEHN数据集为多模态融合研究提供了基准平台。
...
TL;DR: 本文提出特征空间候选引导（FSCG），一种无需训练即可减少条件扩散模型生成超声图像与真实图像之间特征域差异的采样策略，通过局部k-NN特征校正和特征空间能量筛选，在三个数据集上大幅降低FID等指标。


<details>
  <summary>Details</summary>
Motivation: 现有条件扩散模型生成的超声图像虽在解剖结构上合理，但B-mode外观（如散斑纹理、组织对比度、衰减等）不够真实。标准模型仅基于解剖掩膜进行条件生成，缺乏对超声图像特有采集依赖属性的建模，导致生成图像与真实图像在特征表征空间中存在明显差距。

Method: 提出FSCG（Feature-Space Candidate Guidance），一种无需重新训练的采样策略。在采样过程中，使用冻结的超声基础模型提取特征，对潜在样本进行局部k-NN特征校正，并生成多个随机候选样本，依据其在特征空间中的能量选择最佳样本。掩膜负责解剖结构，FSCG引导样本向真实超声域靠近。

Result: 在三个不同数据集上，相比标准条件扩散采样，FSCG平均降低FID64达56%、FID192达57%、最近邻特征距离达47%，优于其他推理时引导基线方法。

Conclusion: 域感知的深度特征表示能够揭示并缩小医学扩散合成中的真实性差距，且无需重新训练生成器，表明利用预训练基础模型进行推理时引导是提升生成超声图像真实感的有效途径。
...
TL;DR: ABot-3DWorld 0是一个通用多模态3D世界模型，能将文本、图像、视频输入转化为高保真、可探索的3D世界。核心是统一的空间生成原语(SGP)，通过全景图+点云描述3D空间，结合3D全景视频生成与重建管道，支持丰富输入（多视图、视频）的几何精确恢复和单图/文本的创造性生成，并锚定地理兴趣点实现地图原生探索。


<details>
  <summary>Details</summary>
Motivation: 当前3D内容创建门槛高，缺乏一个统一的多模态3D世界模型，能够将多种输入形式（文本、图像、视频）高效地转化为高质量的、可自由探索的3D世界。

Method: 提出Spatial Generative Primitive (SGP)作为统一的3D空间描述（全景图+点云）；多模态输入先提升为SGP；然后通过3D一致的全景视频生成器沿规划轨迹探索SGP；最后用全景视频重建引擎将生成视频转化为3D高斯泼溅(3DGS)世界。支持两种模式：丰富输入通过几何精确恢复，单图/文本通过生成式创造。

Result: 在开源方法中达到SOTA，在丰富多模态输入下场景保真度优于Marble。

Conclusion: ABot-3DWorld 0提供了一个低门槛的通用3D内容创建引擎，能够锚定地理兴趣点，支持消费级的地图原生空间探索。
...
TL;DR: 本文提出了一种基于光照自适应三维查找表（C²LUT）的色彩校正框架，采用Tucker张量分解降低计算开销，并发布了包含1473种光谱分布的大规模光照数据集，在多个相机和数据集上实现了比现有方法最高20%的色差（ΔE₀₀）降低和18%的角度误差降低。


<details>
  <summary>Details</summary>
Motivation: 由于传感器响应与CIE XYZ色彩空间之间存在非线性关系，以及高色度、光谱复杂的LED光源日益增多，传统色彩校正方法难以实现高精度校正。

Method: 提出一种光照自适应三维查找表（C²LUT），结合色度感知的光照表示与非线性色彩变换；采用Tucker张量分解压缩LUT，使其在计算资源受限的相机ISP中可部署；构建包含1473种光谱分布的大规模光照数据集。

Result: 在多个相机、光照条件、反射率数据集和真实拍摄图像上，CIE ΔE₀₀降低高达20%，角度误差降低高达18%，且兼容现代相机硬件约束。

Conclusion: 所提出的C²LUT框架在色彩校正精度上显著超越现有方法，同时具备在相机ISP流水线中实际部署的可行性。
...
TL;DR: 本文提出SVI360，一种双分支框架，通过结合原始图像帧及其旋转正交视图来处理球面视频中的极地畸变，在视频插值任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频插值方法难以处理球面视频中靠近极点的严重畸变问题，导致插值质量下降。

Method: 提出SVI360双分支框架，融合原始图像帧与旋转正交视图，通过强化原始视图与正交视图之间光流位移的等变性来提升中间帧预测效果。

Result: 在四个公开基准数据集上，该方法在插值质量和光流准确性方面均优于现有最先进方法。

Conclusion: SVI360有效解决了球面视频的畸变问题，为全向视频插值提供了新的有效方案，代码和预训练模型已开源。
...
TL;DR: 提出GFR-SAM，一种无需训练的"生成-过滤-精炼"三阶段框架，通过将SAM3扩展为跨图像推理、DINOv3原型对比过滤以及几何语义精炼，在Ref-COD任务上超越现有无训练方法8.7% Fβ^w，媲美监督方法。


<details>
  <summary>Details</summary>
Motivation: 现有Ref-COD方法存在两大痛点：(1)监督方法依赖大量像素级标注；(2)无训练方法（如点提示）对定位误差敏感。本文旨在探索一种鲁棒且无需任务微调的训练自由范式。

Method: 三阶段框架：(1) In-Context Exemplar-guided Segmentation：利用SAM3的跨图像推理潜力，通过全局视觉样本生成候选掩码；(2) Region-Global Contrastive Filtering：基于DINOv3的原型对齐机制，对候选掩码排序，抑制背景干扰；(3) Geometric-Semantic Refinement：结合边界框与文本提示，恢复精细边界并提高实例召回率。

Result: 在R2C7K基准上，GFR-SAM在加权F-measure（Fβ^w）上超越现有无训练方法8.7%，并与有监督的SOTA方法性能相当。

Conclusion: 本文证明了释放SAM3潜能的可行性，使其能够进行跨图像上下文提示，建立了通用基础模型与专业密集预测任务之间的桥梁，无需任务特定微调。
...
TL;DR: 针对活体显微成像中细胞谱系重建问题，提出高阶细胞追踪Transformer（HOCT），通过边中心架构和3D几何先验解决候选追踪图中细胞分裂导致的路径纠缠和拓扑信息无效两大结构性障碍，在Cell Tracking Challenge和细菌分裂基准上达到SOTA，且易于微调。


<details>
  <summary>Details</summary>
Motivation: 现有细胞追踪方法在构建候选图时面临两个未被充分解决的结构性障碍：1）细胞分裂使得不同谱系路径在节点嵌入空间中相互纠缠；2）共享节点的边之间标签一致性接近随机，导致候选图拓扑信息对图神经网络聚合无帮助。

Method: 提出HOCT（高阶细胞追踪Transformer），一种边中心（edge-centric）架构，让候选细胞链接之间在3D几何先验下相互关注（相互注意力），从而解决上述两个问题。

Result: 在Cell Tracking Challenge和细菌分裂基准上取得SOTA结果，无需深度预训练图像编码器；在人在回路（human-in-the-loop）场景中，仅用400个标注即可减少59%追踪错误，比LoRA微调竞争基线提升6.75%。

Conclusion: HOCT通过边中心设计和3D几何先验有效克服了候选追踪图的固有结构缺陷，在细胞谱系重建任务上表现优异且易于微调，具有实际应用价值。
...
TL;DR: StoryTeller是一个无需训练的框架，通过维护经过验证的叙事记忆，实现长篇视频的连贯故事感知音频描述，在多个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频语言模型只能独立处理短片段，无法在场景间保持角色、事件和故事上下文的连贯性，导致盲人和低视力观众难以理解电影情节。

Method: 维护一个经过验证的叙事记忆模块，跨场景传递故事相关信息；通过语义过滤和VLM验证确保信息准确性；可选利用公开电影元数据；无需字幕、脚本、微调等额外资源。

Result: 在标准AD基准测试和多样化长视频上，StoryTeller在自动评估、QA评估和人工评估中均显著优于强基线方法，在叙事连贯性、事实依据和故事理解方面表现更佳。

Conclusion: 提出了一种无需训练的框架，有效解决了长视频音频描述中的叙事连贯性问题，为盲人和低视力群体提供更连贯、信息更丰富的观影体验。
...
TL;DR: 提出MM-ToolSandBox基准，评估视觉引导的工具调用智能体。涵盖500+工具、16个应用领域，测试12个模型发现最佳模型成功率不足50%，主要瓶颈在于视觉精度而非规划能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏对视觉引导工具调用智能体的系统性评估，尤其是多图像、多轮交互和状态变更等复杂场景下的能力测评。

Method: 构建了包含500+工具和16个应用领域的状态化执行环境，通过信息流引导的自动化场景生成管线产出多样化场景，并设计了多图像、多轮对话任务。

Result: 评估12个模型（4B到前沿闭源模型），最佳模型成功率低于50%；53%的失败源于图像信息提取错误；小模型败于规划，大模型败于视觉感知。

Conclusion: 视觉工具调用能力仍不成熟，视觉精度是主要瓶颈；不同规模模型面临不同挑战——小模型需提升决策规划，大模型需提升视觉感知精度。
...
TL;DR: 本文提出MicroCharNet，一个超轻量级车牌字符检测模型，仅0.08M参数量和0.096 GFLOPs，在UFPR-ALPR数据集上达到与YOLO系列竞争的精度，并可在边缘设备上实时部署。


<details>
  <summary>Details</summary>
Motivation: 现有高精度车牌字符检测模型依赖大规模架构，计算开销大，难以部署在资源受限的边缘设备上，需要一种能在精度与效率之间取得平衡的轻量化方案。

Method: 采用C2f模块构建紧凑骨干网络，集成CoordAtt模块增强特征提取并保留空间信息；使用轻量级C3k2-based neck融合多层级特征；配合单级无锚框检测头实现端到端预测。

Result: 在UFPR-ALPR数据集上，MicroCharNet仅0.08M参数、0.096 GFLOPs，检测精度优于多个近期YOLO基线模型，硬件评估证实其可在边缘设备上实时运行。

Conclusion: 精心设计的超轻量级架构能够有效平衡车牌字符检测任务的精度与效率，为资源受限场景下的实时部署提供了可行方案。
...
TL;DR: Cycle-World通过循环一致性约束和反向预测模型，在训练和推理阶段抑制自回归扩散模型在长视频生成中的误差累积，实现稳定且时序一致的60秒视频合成。


<details>
  <summary>Details</summary>
Motivation: 自回归扩散模型在长视频生成中存在误差累积问题，微小的预测偏差随时间放大，导致生成漂移、结构坍塌和视觉退化。

Method: 提出Cycle-World框架：(1) 训练阶段引入反向预测模型，通过循环一致性目标将因果约束嵌入前向生成器，强制生成可逆序列；(2) 推理阶段利用冻结的反向模型作为运行时校正器，通过梯度引导的循环指导迭代优化潜在表示，主动抑制误差累积。

Result: 在VBench基准上，Cycle-World实现了60秒长视频生成的最优整体质量和长时间时序一致性，显著缓解了误差漂移问题。

Conclusion: 通过训练-推理双阶段的循环一致性协同机制，有效限制了前向生成漂移，为长视频生成提供了稳定且时序一致的解决方案。
...
TL;DR: HASTE是一个无需编码的Web平台，允许非机器学习专家的分析师使用灾后卫星影像进行逐建筑损伤评估。平台提供两种方法：一种基于单场景标注训练小规模语义分割模型，另一种利用预训练视觉模型嵌入建筑足迹并通过逻辑回归快速评分。实验表明，基于基础模型嵌入的方法仅需20%的标签量即可匹敌全监督ResNet-50基线。自2023年以来，HASTE已在30多次真实灾害响应中应用。


<details>
  <summary>Details</summary>
Motivation: 大型灾害发生后，救援人员需要在数小时内获得建筑损坏地图。现有模型依赖灾前/灾后配准影像和来自相似历史事件的训练集，而新灾害初发时通常无法满足这些条件。因此，需要一种能让非机器学习专业分析师快速生成灾害损伤地图的平台。

Method: 提供两种方法共享统一界面：1）用户标注灾后场景中的多边形，训练小型语义分割模型处理整个影像，并将逐像素输出与现有建筑足迹结合；2）用预训练视觉模型嵌入每个建筑足迹，用户标注少量建筑，在浏览器中拟合逻辑回归模型，数秒内完成其余场景评分。

Result: 在xBD数据集上的初步实验表明，基于基础模型嵌入的方法仅需全监督ResNet-50基线20%的标签量即可区分受损与完好的建筑。自2023年以来，HASTE已支持30多次真实灾害响应（涵盖地震、飓风、台风、洪水、野火、龙卷风等），在影像可用后数小时至数天内交付结果。

Conclusion: HASTE证明了无需代码、无需灾前影像、无需大规模历史训练集即可在灾害发生后快速进行建筑损伤评估的可行性。未来最有前景的方向包括视觉-语言评估、主动学习以及道路等基础设施的损伤模型。
...
TL;DR: 本文提出了一种基于级联LoRA的多模态融合框架，用于医疗培训环境中的动作与活动识别。该框架通过参数高效的逐模态适配和顺序融合，先整合相关性强的模态，再融入异构模态，无需重训练已有组件。在NurViD和Nurse Training两个数据集上的初步实验表明，该方法优于单模态模型，性能与已有基准相当，是一种有前景的参数高效多模态融合方法。


<details>
  <summary>Details</summary>
Motivation: 医疗培训环境中的动作与活动识别需要融合多种异构模态数据，但现有方法通常假设固定的融合结构，扩展性差且训练成本高。作者希望提出一种参数高效、可扩展的多模态融合方法，能够灵活适配不同模态集合并避免重复训练。

Method: 提出级联LoRA多模态融合框架：采用逐阶段顺序融合策略，先融合关联性强的模态，再逐步融入其他异构模态；每个模态通过LoRA进行参数高效适配，新模态加入时无需重训练已有组件。

Result: 在NurViD和Nurse Training两个医疗培训数据集上，级联融合策略优于单模态模型，性能与已有数据集特定基准方法具有竞争力。

Conclusion: 级联LoRA融合是一种有前景的参数高效方法，适用于医疗培训动作与活动识别中的异构多模态融合任务。
...
TL;DR: 本文提出了SportMV-Bench，首个多视角体育视频理解基准，包含787个多视角视频包和2592个问答对。同时提出SportMV-Agent框架，通过主动视角选择、感知工具执行和基于证据的推理迭代循环，相比最强MLLM基线提升14.46%。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在单视角视频理解上表现不错，但体育视频存在密集遮挡、快速运动和复杂交互，单视角难以解决。实际体育赛事有多视角记录，但缺乏评估MLLM多视角理解能力的基准。

Method: 构建SportMV-Bench基准（含LLM生成+MLLM验证+人工过滤的管道），并提出SportMV-Agent框架，通过主动视角选择、感知工具执行和证据推理的迭代循环来利用多视角信息。

Result: 当前MLLM未能有效利用多视角信息，瓶颈在于细粒度视觉感知和视角选择而非逻辑推理或领域知识。SportMV-Agent相比最强基线提升14.46%。

Conclusion: 多视角体育视频理解仍有挑战，主要瓶颈在视觉感知和视角选择层面，SportMV-Agent证明了通过显式视角选择与工具推理可以有效提升性能。
...
TL;DR: 本文提出E-VQA任务，要求Video LLM同时输出语义答案和精准的时空证据（时间片段+密集跟踪分割掩码）。构建了首个像素级基准ST-Evidence，发现QA准确率与真实视觉感知存在脱节，提出自动化生成管道构建16万规模数据集ST-Evidence-Instruct，微调后7B模型提升+27.2 t-mean和+13.8 J&F。


<details>
  <summary>Details</summary>
Motivation: 现有Video LLM在视频问答中缺乏可验证的视觉证据，解释性工作仅依赖文本理由或稀疏边界框，无法捕捉视频中的遮挡、非刚性变形等复杂动态。

Method: 1) 提出E-VQA新任务，要求模型输出答案+时间片段+密集跟踪对象分割掩码；2) 构建首个经人工验证的ST-Evidence基准；3) 开发自动化生成管道，构建16万规模的ST-Evidence-Instruct数据集用于微调。

Result: 发现QA准确率与真实视觉感知存在脱节，仅靠扩大模型规模无法弥合；微调后的7B模型相比UniPixel基线在t-mean上提升27.2，在J&F上提升13.8。

Conclusion: E-VQA任务和ST-Evidence基准为可解释、有证据支持的视频理解建立了稳健基线，代码和数据已开源。
...
TL;DR: 本文提出一种无需额外训练的身份微调方法，通过在预训练冻结编码器的潜在空间中寻找语义方向，实现对文本到图像个性化模型中特定身份的细粒度编辑，同时保持跨图像的身份一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于通用文本到图像模型的个性化和编辑方法缺乏对人脸进行精细编辑所需的精度，即使是微小的修改也可能显著改变人物的感知身份。

Method: 探索预训练冻结编码器的潜在空间，利用其现有架构发现潜在语义方向。该空间由一组潜在token构成，每个token在捕捉身份不同方面扮演不同角色，并常对应特定的空间或语义面部区域。通过在这些空间及选定token子空间中识别有意义的方向，实现局部、精细且语义一致的编辑，无需额外训练。

Result: 通过定性和定量实验验证，该方法能够实现多样化的局部面部编辑，同时保持跨图像的身份一致性。

Conclusion: 提出了一种有效的细粒度身份微调方法，在不进行额外训练的前提下，利用冻结编码器的潜在空间实现精准的人脸编辑，为文本到图像个性化中的身份编辑提供了新思路。
...
TL;DR: 提出SpectraReward，一种无需训练的奖励函数，通过计算生成图像反向重建原始提示的对数似然作为奖励信号，用于图像生成强化学习。同时提出Self-SpectraReward，实现统一多模态模型的自改进框架。


<details>
  <summary>Details</summary>
Motivation: 现有的图像生成强化学习方法依赖人工偏好标签或额外的奖励模型微调，成本高昂且难以扩展。作者希望利用预训练多模态大模型（MLLM）已有的图文对齐能力，无需额外训练即可获得有效的奖励信号。

Method: SpectraReward：在生成图像上执行单次图像条件的前向传播，计算原始提示在图像条件下的平均对数似然作为奖励。Self-SpectraReward：对于统一多模态模型，用模型自身的理解分支作为生成分支的奖励模型，形成闭环自改进框架。

Result: 在两种扩散模型、三种RL算法、九种MLLM主干（4B-235B）及五个分布外基准上验证，SpectraReward和Self-SpectraReward显著且一致地提升生成性能，优于先前基于MLLM的奖励训练方法。

Conclusion: 更大的奖励MLLM不一定更好；Self-SpectraReward可匹敌或超越更大的外部奖励模型，表明奖励-策略对齐是图像生成强化学习的关键因素。
