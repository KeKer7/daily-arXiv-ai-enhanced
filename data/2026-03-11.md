...
TL;DR: 提出Granulon，一种基于DINOv3的多模态大语言模型，通过自适应粒度增强解决现有视觉编码器在细粒度理解与粗粒度语义对齐之间的平衡问题。
...
Method: 提出文本条件粒度控制器动态调整视觉抽象级别，以及自适应令牌聚合模块进行粒度引导池化和关系感知聚类，生成紧凑且语义丰富的视觉令牌。

Result: 实验显示Granulon在相同设置下优于所有视觉编码器，准确率提升约30%，幻觉减少约20%。
...
TL;DR: 提出一种用于3D高斯泼溅表示的原生水印框架，通过专家模块选择载体，门控机制分配资源，组掩码控制梯度传播，在保持视觉质量的同时实现鲁棒水印嵌入。


<details>
  <summary>Details</summary>
Motivation: 随着3D高斯泼溅成为交互式3D资产的事实标准表示，需要开发既鲁棒又不可感知的水印技术来保护这些资产。

Method: 1. Trio-Experts模块直接在高斯基元上操作，为载体选择提供先验知识；2. 安全预算感知门控(SBAG)将高斯分配到水印载体和视觉补偿器；3. 通道级组掩码控制载体和补偿器的梯度传播，限制参数更新；4. 解耦微调提供每个高斯的归因分析。

Result: 方法实现了视图一致的水印持久性和对压缩、噪声等常见图像失真的强鲁棒性，相比现有方法PSNR提升+0.83 dB，比特准确率提升+1.24%，同时提供可审计的解释性。

Conclusion: 该框架在3D高斯泼溅表示中实现了高质量的水印嵌入，通过解耦设计平衡了鲁棒性和视觉质量，并提供了解释性分析能力，为3D资产保护提供了有效解决方案。
...
TL;DR: VisionCreator-R1：首个具有显式反思机制的原生视觉生成智能体，通过反思-规划协同优化训练方法，解决多图像工作流中的视觉错误修正问题


<details>
  <summary>Details</summary>
Motivation: 现有视觉生成智能体主要是规划驱动的，缺乏系统性的反思机制来修正生成过程中的视觉错误，限制了多图像工作流的生成质量

Method: 提出VisionCreator-R1原生视觉生成智能体，采用反思-规划协同优化训练方法：1）在自建的VCR-SFT数据集上训练（包含反思强的单图像轨迹和规划强的多图像轨迹）；2）在VCR-RL数据集上通过强化学习进行协同优化

Result: VisionCreator-R1在现有基准测试和自建的VCR-bench上一致优于Gemini2.5Pro，覆盖单图像和多图像任务，实现了更可靠的视觉生成

Conclusion: 通过引入显式反思机制和反思-规划协同优化训练方法，VisionCreator-R1显著提升了视觉生成智能体在多图像工作流中的错误修正能力和生成质量
...
TL;DR: 本文提出了一种基于计算机视觉和YOLOv8的智能停车系统，通过多摄像头图像融合和逆透视映射技术检测空闲车位，并以3D可视化方式引导用户停车。
...
Method: 采用计算机视觉和YOLOv8目标检测模型，通过逆透视映射（IPM）技术融合四个摄像头的图像，提取空闲车位信息，并构建3D停车环境模拟系统。

Result: 开发了一个成本效益高、易于实施的智能停车系统，能够动态评估视觉输入并适应变化的停车布局，通过3D笛卡尔坐标图可视化空闲车位来引导用户。

Conclusion: 计算机视觉方法在车辆识别和车位检测方面优于传统传感器系统，提出的系统为智慧城市停车管理提供了更准确、适应性更强的解决方案。
...
TL;DR: 开发了一个多癌症肿瘤定位模型MuCTaL，通过平衡训练四种癌症数据，实现了在训练癌症类型上的高精度（ROC-AUC 0.97）和在未见胰腺癌上的泛化能力（ROC-AUC 0.71），并提供公开代码和模型。


<details>
  <summary>Details</summary>
Motivation: 基于深度学习的肿瘤检测模型在特定癌症上训练后，应用到不同肿瘤类型时鲁棒性下降。研究旨在探索通过平衡训练多种癌症数据，是否能在中等规模下实现高性能并泛化到未见肿瘤类型。

Method: 使用DenseNet169进行迁移学习，在79,984个非重叠图像块上训练多癌症肿瘤定位模型（MuCTaL），训练数据来自四种癌症：黑色素瘤、肝细胞癌、结直肠癌和非小细胞肺癌。建立了可扩展的推理流程生成空间肿瘤概率热图。

Result: 在四种训练癌症的验证数据上达到块级ROC-AUC 0.97，在独立的胰腺导管腺癌队列上达到0.71。提供了与现有数字病理工具兼容的空间肿瘤概率热图生成工作流程。

Conclusion: 平衡训练多种癌症数据可以在中等规模下实现高性能的肿瘤定位，并具备一定的跨癌症泛化能力。公开的代码和模型为肿瘤区域定位研究提供了实用工具。
...
Result: 大量实验表明，HECTOR在视觉质量、参考保持和运动可控性方面优于现有方法，能够合成满足复杂时空约束且高度忠实于参考的连贯视频。

Conclusion: HECTOR解决了当前视频生成模型缺乏显式组合控制的问题，通过混合参考条件化和轨迹指定实现了细粒度的组合操作，为视频生成提供了更强的可控性。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶视觉语言模型正在兴起，但其对物理对抗攻击的鲁棒性尚未得到充分研究。当前VLM设计在安全关键应用中可能存在不足，需要系统性评估

Method: 使用黑盒优化结合语义同质化进行公平比较，在CARLA仿真环境中评估三种VLM架构（Dolphins、OmniDrive、LeapVAD）对物理可实现补丁攻击的鲁棒性

Result: 所有架构都表现出严重漏洞，存在持续多帧故障和关键物体检测性能显著下降。分析揭示了不同的架构脆弱性模式

Conclusion: 当前VLM设计未能充分应对安全关键自动驾驶应用中的对抗威胁，需要改进架构以增强鲁棒性
...
TL;DR: 该论文提出了视觉查询分割(VQS)新范式，旨在通过外部视觉查询分割未修剪视频中目标对象的所有像素级出现，并发布了首个专用基准VQS-4K和有效方法VQ-SAM。


<details>
  <summary>Details</summary>
Motivation: 现有视觉查询定位(VQL)方法仅用边界框定位目标的最后出现，缺乏全面性和精确性。VQS旨在实现更全面（所有对象出现）和精确（像素级掩码）的定位，更适合实际应用场景。

Method: 提出了VQ-SAM方法，扩展SAM 2，利用视频中的目标特定线索和背景干扰线索，通过新颖的多阶段框架和自适应内存生成(AMG)模块逐步演化内存，专门用于VQS任务。

Result: 在VQS-4K基准测试中，VQ-SAM取得了有希望的结果，超越了所有现有方法，证明了其有效性。VQS-4K包含4,111个视频、130万帧、222个对象类别。

Conclusion: VQS超越了当前VQL范式，VQS-4K基准和VQ-SAM方法将推动未来VQS研究和实际应用。所有资源将公开提供。
...
TL;DR: 论文提出一种多核门控适配器（MKGA），用于解决甲状腺超声多任务学习中的跨中心域偏移问题，提升分割和恶性风险评估的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 甲状腺超声自动化需要同时处理全局几何驱动的结节分割和局部纹理驱动的恶性风险评估。在跨中心域偏移下，这些线索会不对称地退化，而现有多任务管道通常使用单一共享骨干网络，容易导致负迁移。

Method: 提出轻量级解码器侧适配器家族：多核门控适配器（MKGA）及其残差变体（ResMKGA）。这些适配器使用互补感受野细化多尺度跳跃特征，并应用语义、上下文条件门控来抑制易产生伪影的内容，然后再进行融合。

Result: 在两个超声基准测试中，提出的适配器提高了跨中心鲁棒性：增强了域外分割性能，在CNN设置下，相比标准多任务基线，在临床TI-RADS诊断准确性方面取得了明显提升。

Conclusion: 通过分析CNN和ViT在不同任务中的优势（ViT更适合几何分割，CNN在强偏移下更可靠地保留纹理线索），提出的MKGA适配器有效解决了多任务学习中的负迁移问题，提升了跨中心应用的鲁棒性。
...
TL;DR: MedCBR是一个结合临床指南的概念推理框架，通过多模态对比对齐、概念监督和诊断分类的多任务目标训练，将医学图像特征、概念和病理联系起来，并生成结构化临床叙述来解释诊断。


<details>
  <summary>Details</summary>
Motivation: 在医学影像中，概念瓶颈模型虽然提供了可解释性，但离散的概念表示往往忽略了更广泛的临床背景（如诊断指南和专家启发式），降低了复杂病例中的可靠性。需要将临床指南整合到概念推理中，以增强可靠性和可解释性。

Method: 提出MedCBR框架：1) 将标注的临床描述符转化为符合指南的文本；2) 训练概念模型，采用多任务目标（多模态对比对齐、概念监督、诊断分类）联合学习图像特征、概念和病理；3) 使用推理模型将预测转化为结构化临床叙述，模拟基于指南的专家推理。

Result: MedCBR在超声和乳腺X光检查中分别达到94.2%和84.0%的AUROC，在非医学数据集上达到86.1%的准确率。框架在诊断和概念层面均表现出优越性能。

Conclusion: MedCBR通过整合临床指南和推理模型，增强了医学影像分析的可解释性，形成了从图像分析到决策制定的端到端桥梁，为可解释AI在医疗领域的应用提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 微表情是自发产生的面部微小运动，反映被抑制的情绪。随着多模态大语言模型的发展，为微表情分析提供了新的技术途径。MEGC 2026旨在推动微表情研究向更复杂的视频理解和问答任务发展。

Method: 挑战赛设置两个任务：1) ME-VQA：在相对短视频序列上进行视觉问答，利用MLLMs/LVLMs处理各种微表情相关问题；2) ME-LVQA：扩展到长时视频序列，要求模型处理时间推理和跨长时间段的微表情检测。

Result: MEGC 2026正式推出这两个新任务，要求参赛算法在公共排行榜上提交结果，为微表情研究社区提供标准化评估平台。

Conclusion: 通过引入基于多模态大语言模型的视频问答任务，MEGC 2026将推动微表情分析向更复杂、更实用的方向发展，促进微表情识别、检测和理解技术的进步。
...
TL;DR: TIDE是一种无需训练的文本到图像外推方法，解决DiT在高分辨率生成时的注意力稀释问题，支持任意分辨率和宽高比生成。
...
Method: 提出TIDE方法：1）识别提示信息丢失的核心因素，引入文本锚定机制纠正文本和图像token之间的不平衡；2）设计动态温度控制机制，利用扩散过程中光谱进展模式消除伪影。

Result: 广泛评估表明TIDE提供高质量分辨率外推能力，并能与现有最先进方法无缝集成，无需额外采样开销。

Conclusion: TIDE是一种有效的训练免费T2I外推方法，解决了DiT在高分辨率生成中的结构退化问题，支持任意分辨率和宽高比生成。
...
TL;DR: 本文提出一个合成基准，评估视觉语言模型在生成数字孪生植物仿真配置方面的性能，使用Gemma 3和Qwen3-VL从无人机遥感图像直接生成JSON格式的仿真参数。


<details>
  <summary>Details</summary>
Motivation: 功能结构植物模型虽然能模拟农业环境中的生物物理过程，但其高复杂性和低吞吐量限制了大规模部署。需要一种可扩展的方法来自动生成植物仿真配置。

Method: 利用Helios 3D程序化植物生成库创建合成牛豆地块数据集，测试五种上下文学习方法，使用Gemma 3和Qwen3-VL等开源视觉语言模型从无人机图像生成JSON配置，评估JSON完整性、几何评估和生物物理评估三个类别。

Result: 视觉语言模型能够解释结构元数据并估计植物数量和太阳方位角等参数，但常因上下文偏差或视觉线索不足而依赖数据集均值，导致性能下降。在真实世界无人机正射影像数据集上的验证进一步表征了模型的推理能力与对上下文先验的依赖。

Conclusion: 这是首个利用视觉语言模型生成植物仿真结构JSON配置的研究，为农业数字孪生的3D地块重建提供了一个可扩展框架。
...
<details>
  <summary>Details</summary>
Motivation: 尽管病理学正在快速数字化，但数百万叙述性报告中积累的经验知识仍然难以访问。数字化存储的数据如果没有有效的检索和推理机制，档案就会变成被动的数据仓库，无法为患者护理提供有意义的信息。真正的进步需要病理学家在评估新诊断难题时能够实时查询类似病例。

Method: 提出PathoScribe，一个统一的检索增强大型语言模型框架。该框架支持自然语言病例探索、自动队列构建、临床问答、免疫组化面板推荐和提示控制的报告转换。在70,000份多机构外科病理报告上进行了评估。

Result: PathoScribe在自然语言病例检索中实现了完美的Recall@10，检索增强推理质量高（平均评分4.56/5）。自动队列构建仅需平均9.2分钟，与人工评审的一致性达91.3%，且没有错误排除符合条件的病例，相比传统人工图表审查大幅减少了时间和成本。

Conclusion: 这项工作为将数字病理档案从被动存储系统转化为主动临床智能平台建立了可扩展的基础，使病理学积累的经验知识能够真正为患者护理提供支持。
...
TL;DR: BiCLIP是一个极简的跨模态对齐框架，利用少量锚点样本估计几何变换来增强视觉语言模型在专业领域的适应能力，在11个基准测试中达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在零样本能力上表现出色，但适应专业领域仍具挑战。基于理论发现：独立训练的VLMs通过规范变换相关联，本文假设不同领域的图像特征也通过可恢复的几何变换关联，少量样本可作为锚点估计此变换。

Method: 提出BiCLIP框架，对多模态特征应用针对性变换以增强跨模态对齐。方法极其简单且参数少，利用小样本分类中的标记样本作为锚点来估计领域间的几何变换。

Result: 在EuroSAT、DTD、FGVCAircraft等11个标准基准上评估，BiCLIP始终达到最先进结果。通过分析学习变换的正交性和角度分布，实证验证了现有几何发现，确认结构化对齐是鲁棒领域适应的关键。

Conclusion: BiCLIP通过极简的几何变换方法有效增强了视觉语言模型在专业领域的适应能力，证明了结构化对齐在领域适应中的重要性，代码已开源。
...
TL;DR: 本文提出了首个免示例的持续学习基准用于音频-视觉分割，并提出了ATLAS方法，通过音频引导预融合条件调制和低秩锚定技术来应对动态环境中的分布变化。


<details>
  <summary>Details</summary>
Motivation: 现实世界环境是动态的，音频和视觉分布会随时间演变，而现有的音频-视觉分割系统假设静态训练设置，无法适应这种变化。需要解决持续学习场景下的音频-视觉分割问题。
...
Conclusion: 本文建立了首个免示例的音频-视觉分割持续学习基准，提出的ATLAS方法通过创新的音频调制和遗忘缓解技术，在动态环境中表现出色，推动了多模态持续学习的发展。
...
TL;DR: SVG-EAR：一种用于视频扩散Transformer的无参数线性补偿方法，通过聚类中心补偿跳过的注意力块，结合误差感知路由，在保持生成质量的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏注意力方法要么丢弃部分注意力块导致信息损失，要么依赖学习预测器引入训练开销和输出分布偏移。需要一种无需训练的方法来恢复跳过的注意力块的贡献。

Method: 1）语义聚类：将键值按语义聚类，用少量聚类中心总结每个块；2）无参数线性补偿分支：用聚类中心近似跳过的块；3）误差感知路由：轻量级探针估计补偿误差，选择误差-成本比最高的块进行精确计算。

Result: SVG-EAR在视频扩散任务中改善了质量-效率权衡，在相同生成保真度下提高吞吐量。在Wan2.2和HunyuanVideo上分别实现1.77倍和1.93倍加速，PSNR分别达到29.759和31.043。

Conclusion: SVG-EAR建立了优于先前方法的帕累托前沿，通过聚类中心补偿和误差感知路由，无需训练即可有效恢复稀疏注意力中跳过的块的贡献，显著提升计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射(3DGS)虽然能实现实时新视角合成，但训练成本高昂，特别是在后致密化阶段，反向传播占用了大量计算时间。作者观察到该阶段存在大量更新冗余：许多采样视图的损失已接近平台期，提供的梯度收益递减，但标准训练仍进行完整的反向传播。

Method: 提出SkipGS方法，采用新颖的视图自适应反向门控机制。该方法始终执行前向传播以更新每视图损失统计，当采样视图的损失与其近期基线一致时，选择性跳过反向传播，同时强制执行最小反向预算以确保优化稳定性。
...
TL;DR: 提出基于扩散模型的防伪认证框架，通过联合利用原始二进制模板、印刷CDP和打印机身份表征，实现多类打印机分类，有效区分真伪印刷品。


<details>
  <summary>Details</summary>
Motivation: 随着高分辨率打印扫描设备和生成式深度学习的普及，传统防伪认证系统难以区分高质量伪造品和真品，需要更强大的认证方法。

Method: 提出扩散认证框架，将认证问题转化为多类打印机分类任务，通过空间和文本条件控制，扩展ControlNet用于类别条件噪声预测。

Result: 在Indigo 1 x 1 Base数据集上，该方法优于传统相似度度量和现有深度学习方法，且能泛化到训练中未见过的伪造类型。

Conclusion: 扩散模型在防伪认证中表现出色，通过联合利用多种信息源和打印机身份表征，能够有效应对现代伪造技术的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 高光谱图像中的弱光谱响应通常被主导端元和传感器噪声掩盖，导致丰度估计不准确。现有方法在处理弱信号时容易崩溃，需要专门针对弱信号解混的解决方案。

Method: 提出WS-Net框架，包含多分辨率小波融合编码器、混合骨干网络（集成Mamba状态空间分支用于长程依赖建模）、弱信号注意力分支（选择性增强低相似性光谱线索）、可学习门控机制自适应融合两种表示，以及解码器使用KL散度正则化增强主导和弱端元之间的可分离性。

Result: 在一个模拟数据集和两个真实数据集（Samson和Apex）上的实验表明，WS-Net在六个最先进基线方法中表现最优，实现了RMSE降低55%、SAD降低63%的显著改进。在低信噪比条件下保持稳定精度，特别是对弱端元。
...
TL;DR: SpectralDiff：基于频谱结构扩散的单幅图像去雨框架，通过结构化频谱扰动引导多方向雨纹抑制，并采用全乘积U-Net架构提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 雨纹具有方向性和频率集中的结构特征，在多个尺度上重叠，使得单幅图像去雨特别困难。传统的空间域扩散方法没有明确考虑这种结构化频谱特性。

Method: 提出SpectralDiff框架，通过结构化频谱扰动引导渐进式雨纹抑制；设计全乘积U-Net架构，利用卷积定理将卷积操作替换为逐元素乘积层，提高计算效率同时保持建模能力。

Result: 在合成和真实世界基准测试上的广泛实验表明，SpectralDiff在去雨性能上具有竞争力，同时相比现有基于扩散的方法，模型更紧凑且推理效率更高。

Conclusion: SpectralDiff通过结合结构化频谱扰动和高效的全乘积U-Net架构，为单幅图像去雨提供了一个有效的扩散框架，在性能和效率之间取得了良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 传统火灾检测系统通常只进行简单检测，缺乏对火灾与周围物体距离的量化评估和风险评估能力，无法提供可操作的危害优先级信息。

Method: 使用双模型架构：主模型为YOLOv8实例分割模型检测火灾和烟雾；次模型为预训练在COCO数据集上的目标检测模型识别周围实体（人、车辆、基础设施）。通过像素距离计算和像素到米的比例转换，结合火灾证据、物体脆弱性和距离暴露度进行风险评估。

Result: 在9,860张标注图像数据集上训练，性能优异：精确率、召回率和F1分数均超过90%，mAP@0.5超过91%。系统能生成包含火灾位置、检测物体、估计距离和上下文风险信息的标注视觉输出。
...
TL;DR: GST-VLA：通过高斯空间标记器和3D深度感知思维链，将视觉语言模型与几何感知结合，提升机器人操作任务的性能。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型（VLA）将视觉观测编码为2D图像块标记，缺乏内在的几何结构信息。这限制了在需要精确几何推理的机器人操作任务（如抓取、导航）中的性能。

Method: 1. 高斯空间标记器（GST）：将深度和语义特征转换为128个各向异性3D高斯基元，编码局部表面方向和几何置信度。2. 3D深度感知思维链（DA-CoT）：监督四个结构化中间空间思维（3D物体定位、抓取接触几何、成对度量距离、粗略SE(3)路径点）。3. 在VLM的每个transformer块中加入交叉注意力层，直接访问高斯场。4. 300M参数的流匹配动作专家解码7-DoF动作增量。

Result: 在LIBERO数据集上达到96.4%（提升2.0%），在SimplerEnv上达到80.2%（提升5.4%）。消融实验确认了GST各组件、DA-CoT各思维和训练阶段的独立和协同增益。

Conclusion: GST-VLA通过将几何结构显式地整合到视觉语言模型中，显著提升了需要精确几何推理的机器人操作任务的性能。该方法为VLA模型提供了内在的3D几何感知能力。
...
TL;DR: OmniEdit是一个无需训练即可实现唇部同步和音视频编辑的框架，通过替换FlowEdit中的编辑序列为目标序列，提供无偏估计并建立平滑稳定的编辑轨迹。
...
TL;DR: 论文提出PPVG框架，将物理视频生成视为因果连接事件序列的生成，通过物理驱动的事件链推理和过渡感知跨模态提示实现物理一致性。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型在理解常识物理知识方面存在挑战，现有方法依赖大语言模型将物理概念嵌入提示，但缺乏建模因果进展的条件机制，导致只能生成由提示定义的单一时刻物理现象。

Method: 设计两个核心模块：1) 物理驱动的事件链推理：将物理现象分解为基本事件单元，利用思维链推理，嵌入物理公式作为约束以确保确定性因果依赖；2) 过渡感知跨模态提示：将因果事件单元转换为时间对齐的视觉语言提示，通过交互编辑逐步合成单个事件的视觉关键帧。

Result: 在PhyGenBench和VideoPhy基准测试上的综合实验表明，该框架在多个物理领域中生成物理合理视频方面实现了优越性能。

Conclusion: 该研究将PPVG视为因果连接和动态演化事件序列的生成，通过事件分解和跨模态提示实现了物理一致性视频生成，为建模真实世界物理现象提供了新范式。
...
TL;DR: 本文提出了一种知识驱动的认知编排方法（MedKCO），通过两阶段课程学习和自适应对比损失来优化医学视觉语言预训练，显著提升了模型在分布偏移下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉语言预训练方法通常强迫模型同时学习简单和复杂概念，这种反认知过程导致特征表示不理想，特别是在分布偏移情况下。需要一种更符合认知规律的方法来提升模型性能。

Method: 提出了知识驱动的认知编排方法（MedKCO），包含两个核心组件：1）基于诊断敏感性和类内样本代表性的两级课程学习来排序预训练数据；2）考虑医学图像类间相似性的自调节非对称对比损失来动态调整预训练目标参与度。

Result: 在三个医学成像场景的多个视觉语言下游任务中评估，与多种课程学习方法相比，该方法在所有基准测试中都显著超越了基线模型。

Conclusion: MedKCO通过模拟人类认知过程的数据排序和自适应学习目标，有效提升了医学视觉语言预训练模型的泛化能力，特别是在分布偏移情况下表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法主要关注语义绑定，忽视了提示中指定的多样化运动类别理解。需要解决用户提示中的语义模糊性，并实现对不同运动类型的解耦控制。

Method: 采用运动分解框架：1) 规划阶段：在运动图上推理运动规律，获取每个实例形状和位置的逐帧变化，将用户提示组织为结构化表示；2) 生成阶段：通过条件分支以解耦方式调制不同运动类别的合成，稳定静止区域外观、保持刚体几何、正则化局部非刚体变形。

Result: 在真实世界基准测试中实现了令人印象深刻的运动合成性能，框架具有模型无关性，可无缝集成到各种扩散模型架构中。

Conclusion: 提出的运动分解框架有效解决了视频生成中的运动理解问题，通过先规划后生成的范式实现了对复杂运动的精细控制，为实际应用场景提供了有力工具。
...
TL;DR: 该论文提出了一种基于Transformer的层次化组合查询框架，用于皮肤癌的视觉-语言检索，通过全局-局部对齐和临床感知加权提升医学图像检索性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像检索在临床诊断、教育和质量控制中至关重要。实际应用中，查询通常结合参考病灶图像和文本描述（如皮肤镜特征），但现有方法难以有效处理这种组合查询。
...
Conclusion: 该框架有效解决了皮肤癌医学图像检索中的组合查询问题，通过层次化表示和全局-局部对齐策略提升了检索性能，具有实际临床应用价值。
...
TL;DR: VIVID-Med：利用冻结LLM作为结构化语义教师来预训练医疗视觉Transformer的新框架，通过JSON结构化表示和正交注意力分解提升性能，训练后仅保留轻量级ViT骨干网络。


<details>
  <summary>Details</summary>
Motivation: 当前医疗视觉语言预训练方法使用one-hot标签或自由文本，无法有效捕捉临床发现之间的复杂语义关系，需要更有效的语义监督方法。

Method: 1) 使用冻结LLM作为结构化语义教师，将临床发现转换为可验证的JSON字段-状态对（UMS统一医疗模式）；2) 采用可回答性感知掩码进行优化聚焦；3) 使用结构化预测分解（SPD）将交叉注意力划分为正交正则化的查询组，提取互补视觉特征；4) 训练后丢弃LLM，仅保留轻量级ViT骨干网络。

Result: 在CheXpert线性探测中达到0.8588宏AUC，比BiomedCLIP提升6.65个百分点且使用数据量少500倍；在NIH ChestX-ray14上零样本跨域迁移达到0.7225宏AUC；在CT模态上表现优异：LIDC-IDRI肺结节分类0.8413 AUC，OrganAMNIST 11器官分类0.9969宏AUC。

Conclusion: VIVID-Med提供了一种高效、可扩展的替代方案，可在临床环境中部署轻量级视觉骨干网络，避免了资源密集型视觉语言模型的部署负担。
...
TL;DR: PRLF框架通过自适应模态可靠性估计和渐进交互模块，有效处理多模态情感分析中的模态缺失问题，在多种缺失场景下优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现实世界应用中常因噪声、硬件故障或隐私限制导致模态缺失，现有方法依赖完整模态，直接融合缺失模态会扭曲已学习到的完整模态表示。

Method: 提出PRLF框架：1) AMRE模块动态量化各模态可靠性，基于识别置信度和Fisher信息确定主导模态；2) ProgInteract模块迭代对齐其他模态与主导模态，增强跨模态一致性并抑制噪声。

Result: 在CMU-MOSI、CMU-MOSEI和SIMS数据集上的实验表明，PRLF在模态间和模态内缺失场景下均优于现有最先进方法，展现了鲁棒性和泛化能力。

Conclusion: PRLF框架通过自适应可靠性估计和渐进对齐策略，有效解决了多模态情感分析中的模态缺失问题，为实际应用提供了更鲁棒的解决方案。
...
TL;DR: QUSR是一个用于真实世界图像超分辨率的扩散模型，通过质量感知先验和不确定性引导的噪声生成模块，自适应处理不同区域的噪声强度，在未知和非均匀退化场景中实现高保真重建。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的图像超分辨率方法在真实世界场景中面临挑战，因为退化通常是未知且空间非均匀的，导致细节丢失或视觉伪影。需要一种能够处理这些复杂退化情况的方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统DXA诊断骨质疏松仅关注骨密度，忽略骨微结构和周围软组织信息；现有HR-pQCT分析主要关注矿化骨区域，大量图像数据未被充分利用

Method: 使用SegFormer架构进行全自动多区域HR-pQCT分割（皮质骨、松质骨、皮肤、肌腱肌肉、脂肪组织），提取939个放射组学特征，降维后训练6种机器学习分类器

Result: SegFormer分割平均F1分数95.36%；基于肌腱肌肉组织特征的分类准确率80.08%，AUROC 0.85，优于骨特征模型；患者层面，软组织放射组学将AUROC从0.792提升至0.875

Conclusion: 自动化多区域HR-pQCT分割能够提取超出骨组织的临床信息，证明整合组织评估对骨质疏松检测的重要性，软组织特征提供了有价值的诊断信号
...
TL;DR: EQ-VMamba：首个旋转等变的视觉Mamba架构，通过旋转等变交叉扫描策略和组Mamba块实现，在保持参数效率的同时提升旋转鲁棒性和性能


<details>
  <summary>Details</summary>
Motivation: 旋转等变性是视觉数据最重要的结构先验之一，但当前基于Mamba的视觉架构都忽略了这一特性，导致模型对图像旋转敏感，限制了其鲁棒性和跨任务泛化能力

Method: 提出EQ-VMamba架构，包含精心设计的旋转等变交叉扫描策略和组Mamba块，并通过理论分析证明其端到端的旋转等变性

Result: 在图像分类、语义分割、图像超分辨率等多个基准测试中，EQ-VMamba相比非等变基线达到优越或竞争性性能，同时参数减少约50%，显著提升了旋转鲁棒性
...
TL;DR: 提出基于Agentic AI的联邦学习控制系统，用于6G网络中管理分布式设备上的模型训练，将学习任务与网络管理结合，通过专业代理实现资源优化和自适应训练。


<details>
  <summary>Details</summary>
Motivation: 随着用户定制化设备端学习的兴起，无线系统面临新挑战：需要在多样化分布式数据上训练模型，同时满足严格的延迟、带宽和可靠性约束。传统方法将联邦学习视为纯学习问题，忽略了网络条件的影响。

Method: 提出Agentic AI作为联邦学习在6G网络中的控制层，将高层任务目标转化为网络感知的行动。系统包含专注于检索、规划、编码和评估的专业代理，利用监控工具和优化方法处理客户端选择、激励结构、调度、资源分配、自适应本地训练和代码生成。采用闭环评估和记忆机制持续优化决策。

Result: 案例研究证明了Agentic AI系统利用工具实现高性能的有效性。系统能够考虑变化的信噪比、带宽条件和设备能力，持续优化联邦学习过程。

Conclusion: 将联邦学习重新定义为学习与网络管理的综合任务，通过Agentic AI控制层可以更好地应对6G网络中设备端学习的挑战，实现高效、自适应的分布式模型训练。
...
TL;DR: RTFDNet是一个用于RGB-热成像语义分割的三分支编码器-解码器网络，通过协同特征融合和跨模态解耦正则化，在模态部分缺失时仍保持鲁棒性能。


<details>
  <summary>Details</summary>
Motivation: 传统RGB-热成像分割方法过度强调模态平衡，在传感器信号部分缺失时鲁棒性差、性能下降严重。现有方法通常将模态融合和模态适应解耦，需要多阶段训练或教师-学生框架。

Method: 提出RTFDNet：1）协同特征融合(SFF)通过通道门控交换和轻量空间注意力注入互补信息；2）跨模态解耦正则化(CMDR)从融合表示中分离模态特定组件；3）区域解耦正则化(RDR)在置信区域强制类别选择性预测一致性。

Result: 在广泛实验中证明了RTFDNet的有效性，在不同模态条件下均表现出稳定性能，支持测试时高效独立推理。

Conclusion: RTFDNet统一了融合和解耦，增强了单模态路径而不降低融合流性能，为RGB-T语义分割提供了鲁棒解决方案。
...
TL;DR: RubiCap：基于LLM评分标准的强化学习框架，用于提升密集图像描述的质量和多样性，超越监督蒸馏和现有RL方法


<details>
  <summary>Details</summary>
Motivation: 密集图像描述对视觉语言预训练和文本到图像生成至关重要，但专家级标注成本高昂。现有方法如监督蒸馏输出多样性有限、泛化能力弱，而强化学习在开放域描述任务中缺乏可靠的奖励信号

Method: 提出RubiCap框架：1）生成多样候选描述集合；2）使用LLM编写评分标准，提取共识优势并诊断当前策略缺陷；3）将洞察转化为显式评估标准，让LLM法官进行结构化多维度评估，替代粗糙的标量奖励

Result: 在CapArena上获得最高胜率，超越监督蒸馏、先前RL方法、人类专家标注和GPT-4V增强输出。在CaptionQA上展示卓越的词效率：7B模型匹配Qwen2.5-VL-32B-Instruct，3B模型超越其7B版本。使用RubiCap-3B作为描述器训练的VLM优于基于专有模型描述训练的VLM

Conclusion: RubiCap通过LLM生成的细粒度评分标准解决了开放域密集图像描述中强化学习的奖励信号瓶颈，实现了高质量、多样化的描述生成，为视觉语言对齐提供了有效解决方案
...
TL;DR: PS-Mamba：一种渐进式分割Mamba框架，通过几何一致性分区和对称跨尺度捷径，解决2D图像恢复中状态空间模型的空间拓扑破坏和长程衰减问题，在保持线性复杂度的同时实现局部结构保持和全局一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：卷积网络感受野有限，Transformer全局注意力计算复杂度高（二次复杂度）。最近的状态空间模型（如Mamba）虽然提供线性时间的长程依赖建模，但直接扩展到2D图像存在两个内在缺陷：1）将2D特征图展平为1D序列会破坏空间拓扑结构，导致局部性失真；2）SSM的稳定性驱动递归动态会引起长程衰减，逐渐减弱远距离空间位置的信息，削弱全局一致性。

Method: 提出渐进式分割Mamba（PS-Mamba）框架：1）几何一致性分区：不顺序展平整个特征图，而是进行保持邻域完整性的分区处理；2）渐进式分割层次结构（半、四分之一、八分之一）：实现结构化多尺度建模，同时保持线性复杂度；3）对称跨尺度捷径路径：直接传输低频全局上下文信息，稳定大空间范围内的信息流，抵消长程衰减效应。

Result: 在超分辨率、去噪和JPEG伪影减少等任务上的广泛实验表明，PS-Mamba相比最近的Mamba基和注意力基模型有显著提升，取得了明显的性能优势。

Conclusion: PS-Mamba成功解决了状态空间模型在2D图像恢复中的拓扑破坏和长程衰减问题，通过拓扑感知的分层状态空间框架，在保持线性复杂度的同时实现了局部结构保持和高效全局传播的平衡，为高保真图像恢复提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于编码器的3D MLLM方法存在语义不对齐、分辨率敏感和计算开销大的问题，需要开发更高效、对齐更好的端到端解决方案。

Method: 1) 轻量级3D分词器：结合几何采样、邻域聚合和向量量化将点云转换为离散token；2) 偏好优化训练策略：使用基于语义对齐的奖励机制，专门针对开放式3D问答任务。

Result: 在多个3D理解基准测试中，SAGE超越了现有基于编码器的方法，同时在计算效率、跨LLM骨干网络的泛化能力和输入分辨率变化的鲁棒性方面具有显著优势。

Conclusion: SAGE证明了端到端处理原始3D数据的可行性，将3D数据视为"外语"扩展LLM词汇表，为3D多模态理解提供了更高效、更对齐的解决方案。
...
TL;DR: MM-Zero是首个实现零数据自演化的视觉语言模型推理框架，通过多角色（提议者、编码者、解决者）设计和强化学习训练，无需初始图像数据即可提升多模态推理性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型的自演化通常需要至少一些种子图像数据来启动过程，这限制了完全自主的自我改进能力。研究旨在探索如何实现零数据的VLM自演化，突破传统双模型范式的限制。

Method: 提出MM-Zero框架，包含三个专门角色：提议者生成抽象视觉概念和问题；编码者将概念转换为可执行代码来渲染图像；解决者对生成的视觉内容进行多模态推理。所有角色从同一基础模型初始化，使用组相对策略优化（GRPO）训练，结合执行反馈、视觉验证和难度平衡的奖励机制。

Result: 实验表明MM-Zero在广泛的多模态基准测试中提升了VLM推理性能，建立了可扩展的多模型系统自演化路径。

Conclusion: MM-Zero首次实现了视觉语言模型的零数据自演化，扩展了自我改进的前沿，为多模态模型的自演化系统提供了可扩展的路径。
...
TL;DR: 提出基于手部关节角度特征的几何感知度量学习框架，用于跨语言少样本手语识别，通过20维角度描述符消除相机视角、手部尺度和记录条件带来的域偏移，在四个手语字母表上显著提升性能。
...
Result: 在ASL、LIBRAS、阿拉伯手语和泰语手语四个手指拼写字母表上评估，角度特征比归一化坐标基线在域内性能提升高达25个百分点，并实现了冻结跨语言迁移，经常超过域内准确率。
...
TL;DR: TubeMLLM：基于多模态大语言模型的医学血管状解剖统一基础模型，通过拓扑先验和自然语言提示增强拓扑感知，在15个数据集上实现SOTA性能，显著减少拓扑误差并具备优秀的零样本跨模态迁移能力。


<details>
  <summary>Details</summary>
Motivation: 医学血管状解剖建模面临拓扑结构复杂和对数据集偏移敏感的问题，现有任务特定模型常出现拓扑不一致性（如人工断开和虚假合并）。受多模态大语言模型零样本泛化能力的启发，需要开发统一的基础模型来结合结构化理解和可控生成。

Method: 提出TubeMLLM模型，通过显式自然语言提示集成拓扑先验，并在共享注意力架构中与视觉表示对齐。构建TubeMData多模态基准数据集，包含全面的拓扑中心任务，并引入自适应损失加权策略来强调训练中的拓扑关键区域。

Result: 在15个多样化数据集上的实验显示优越性能：在彩色眼底摄影中，β₀数值误差从37.42降至8.58；在未见过的X射线血管造影上实现零样本跨模态迁移，Dice分数达67.50%，β₀误差降至1.21；对模糊、噪声和低分辨率等退化保持鲁棒性；在拓扑感知理解任务中，掩模拓扑质量评估准确率达97.38%。

Conclusion: TubeMLLM通过结合拓扑先验和自然语言提示，显著提升了医学血管状解剖的拓扑感知能力，实现了优异的零样本泛化和跨模态迁移性能，为医学图像分析提供了强大的统一基础模型。
...
TL;DR: 提出一种用于训练分布式卷积神经网络(DisCNN)的新型损失函数，仅识别特定正类样本，将正类映射到高维空间的紧凑集合，负类映射到原点，实现正类特征解耦。


<details>
  <summary>Details</summary>
Motivation: 传统CNN需要学习所有类别的特征，而实际应用中往往只需要识别特定正类。现有方法无法有效解耦正类特征，导致模型复杂且泛化能力有限。

Method: 设计新型损失函数，训练DisCNN将正类样本映射到高维空间的紧凑集合，负类样本映射到原点。模型仅需提取少量正类特征，具有轻量级架构。

Result: 模型在测试数据上表现出优秀的泛化能力，对未见类别仍保持有效。能够轻松处理嵌入复杂背景中的正类样本检测任务。

Conclusion: 提出的DisCNN方法通过特征解耦实现了高效的正类识别，模型轻量且泛化能力强，为特定目标检测任务提供了有效解决方案。
...
TL;DR: 提出一个统一的MRI场强增强框架，通过利用多模态和任务间的共享退化模式，结合3D基础模型和场感知光谱校正机制，显著提升增强性能
...
Method: 提出统一框架，包含三个主要贡献：1）利用预训练的3D基础模型直接处理3D体积信息，而非传统2D切片方法；2）引入场感知光谱校正机制（FASRM），针对不同场强定制光谱校正；3）构建并公开大规模配对多场MRI数据集

Result: 实验表明该方法优于现有最先进方法，平均PSNR提升约1.81 dB，SSIM提升9.47%

Conclusion: 通过统一框架、3D基础模型集成、场感知光谱校正和大规模数据集，实现了更通用和高效的MRI场强增强，为临床诊断和研究提供了有力工具
...
TL;DR: HelixTrack：首个完全事件驱动的螺旋桨状物体跟踪与转速估计方法，在无人机和旋转机械的微秒级延迟感知中表现优异


<details>
  <summary>Details</summary>
Motivation: 无人机和旋转机械的安全关键感知需要微秒级延迟跟踪快速周期性运动，但传统基于帧和事件的跟踪器在螺旋桨等周期性物体上会漂移或失效，因为周期性特征违反了它们的平滑运动假设

Method: 完全事件驱动的方法，通过动态估计的单应性将事件从图像平面反投影到转子平面，使用卡尔曼滤波器维护瞬时相位估计，批量迭代更新通过耦合相位残差与几何来细化物体姿态

Result: 在TQE数据集上，HelixTrack以约11.8倍实时速度处理全速率事件，具有微秒级延迟，在联合跟踪和转速估计方面持续优于适应转速估计的逐事件和聚合基线方法

Conclusion: HelixTrack填补了周期性物体跟踪的空白，能够可靠地跟踪螺旋桨状物体并估计其转速，为安全关键应用提供了有效的解决方案，并发布了首个针对此类任务的公开数据集
...
TL;DR: BridgeDiff是一个基于扩散模型的虚拟试穿框架，通过两个互补组件桥接人体观察与平面服装合成，解决现有方法在未观察区域补全不一致和服装结构不稳定的问题。


<details>
  <summary>Details</summary>
Motivation: 现有VTOFF方法通常将虚拟试穿视为直接的图像转换，依赖于局部掩码或纯文本提示，忽略了在体外观与平面布局之间的差距，导致未观察区域补全不一致和服装结构不稳定。
...
Result: 在标准VTOFF基准测试上的广泛实验表明，BridgeDiff实现了最先进的性能，生成更高质量的平面服装重建结果，同时保留了细粒度外观和结构完整性。

Conclusion: BridgeDiff通过显式桥接人体中心观察与平面服装合成，有效解决了虚拟试穿中的外观-布局差距问题，为标准化展示和下游虚拟试穿应用提供了更可靠的解决方案。
...
TL;DR: 提出RAE-NWM导航世界模型，在密集视觉表示空间建模导航动态，通过条件扩散Transformer和去耦扩散头提升结构稳定性和动作精度


<details>
  <summary>Details</summary>
Motivation: 当前导航世界模型通常在VAE压缩潜在空间中学习状态演化，但空间压缩会丢失细粒度结构信息，阻碍精确控制。研究发现DINOv2特征在动作条件转移中表现出更强的线性可预测性，因此提出在密集视觉表示空间建模导航动态

Method: 提出表示自编码器导航世界模型(RAE-NWM)：1)在密集视觉表示空间建模导航动态；2)使用带去耦扩散Transformer头的条件扩散Transformer(CDiT-DH)建模连续转移；3)引入独立的时间驱动门控模块进行动态调节，控制生成过程中的动作注入强度
...
TL;DR: 提出Geometric Semantic Decoupling (GSD)方法，通过几何约束移除语义成分，提升AI生成图像检测的泛化能力
...
Method: 提出几何语义解耦(GSD)模块，使用冻结的VFM作为语义指导，可训练的VFM作为伪迹检测器，通过批量统计估计语义方向并用几何约束将其投影移除，迫使检测器依赖语义不变的取证证据

Result: 在跨数据集评估中达到94.4%视频级AUC(提升1.2%)，在未见过的操作上鲁棒性提升3.0%(DF40)，在通用场景合成图像检测上也有提升(UniversalFakeDetect提升0.9%，GenImage提升1.7%)

Conclusion: GSD通过显式移除语义成分，有效解决了VFM检测器的语义回退问题，显著提升了AI生成图像检测的泛化能力和鲁棒性
...
TL;DR: Poly-DETR将实例分割重新定义为稀疏顶点回归，使用极坐标表示替代密集像素掩码预测，解决了高分辨率输入与实时推理的冲突，在多个数据集上表现优异且更轻量。


<details>
  <summary>Details</summary>
Motivation: 当前实例分割存在高分辨率输入需求与轻量实时推理之间的冲突瓶颈，传统密集像素掩码预测方法计算开销大，需要更高效的表示方法。

Method: 提出Poly-DETR，将实例分割重新定义为稀疏顶点回归，采用极坐标表示；引入极坐标可变形注意力机制和位置感知训练方案，动态更新监督并聚焦边界线索。

Result: 在MS COCO test-dev上比最先进的极坐标方法提升4.7 mAP；在高分辨率场景下内存消耗减少近一半；在PanNuke和SpaceNet数据集上全面超越掩码基准方法。

Conclusion: Poly-DETR通过极坐标表示实现了更轻量高效的实例分割，特别适用于高分辨率场景和规则形状实例，为实时实例分割提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习与计算机视觉技术对自动驾驶汽车的发展至关重要，能够帮助车辆感知和理解周围环境，实现安全导航和实时决策。然而，自动驾驶面临诸多挑战，需要更有效的解决方案。

Method: 采用预训练和自定义神经网络，结合几何与颜色变换的数据增强、图像归一化、迁移学习等技术。使用GTSRB、道路车道分割、车辆检测数据集以及Udacity模拟器数据进行评估。
...
TL;DR: 提出DiP框架，通过模态特定伪节点实现多模态图的动态信息路径学习，支持自适应、表达性强且稀疏的消息传播


<details>
  <summary>Details</summary>
Motivation: 现实应用中多模态图（节点包含图像、文本等异质特征）日益普遍，但现有方法大多基于传统GNN扩展，依赖静态结构或密集注意力，限制了灵活性和节点嵌入学习的表达能力

Method: 引入模态特定伪节点，通过邻近引导的伪节点交互实现模态内动态消息路由，在共享状态空间中通过高效信息路径捕获模态间依赖关系，实现线性复杂度的自适应、表达性强且稀疏的消息传播
...
TL;DR: 本文提出了一种基于网络房间游览视频的大规模视频指令框架，用于视觉语言导航（VLN），通过隐式几何表示从RGB帧提取空间线索，无需脆弱的3D重建，显著提升了数据利用率和导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLN数据集受限于模拟器生成的数据，缺乏多样性和可扩展性，无法捕捉真实环境的复杂性。需要从真实人类行走演示中学习，以开发更具泛化性和实际应用价值的导航代理。

Method: 1) 从网络房间游览视频构建大规模视频指令框架；2) 整合开放式描述增强轨迹和3D重建的动作增强轨迹；3) 引入隐式几何表示，直接从RGB帧提取空间线索，避免脆弱的3D重建过程。

Result: 在多个VLN基准测试（CVDN、SOON、R2R、REVERIE）上取得新的最先进性能，并能够开发出强大的零样本导航代理。隐式几何表示显著提高了数据利用率，释放了大量先前无法使用的视频数据。

Conclusion: 通过将大规模网络视频与隐式空间推理相结合，该工作推动了具身导航向更具可扩展性、泛化性和实际应用价值的方向发展，为真实世界导航解决方案提供了新途径。
...
Result: 在自定义工业数据集上的大量实验表明，相比最先进方法，该方法在语义泛化和几何保真度方面表现优越。

Conclusion: ForgeDreamer通过多专家LoRA集成和跨视图超图几何增强，有效解决了工业场景文本到3D生成的领域适应和几何推理问题，为精密制造应用提供了更可靠的3D生成解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 虽然3D高斯泼溅在渲染质量和效率上优于NeRF，但进一步提高学习3D高斯的效率仍是一个研究挑战。现有方法需要处理较长的像素高斯列表，限制了渲染效率。

Method: 1) 定期重置高斯尺度以缩小每个高斯的尺寸，使小高斯覆盖更少的邻近像素；2) 在alpha混合过程中引入熵约束，锐化每条射线上高斯的权重分布；3) 集成到渲染分辨率调度器中，通过渐进式分辨率提升进一步提高效率。

Result: 在广泛使用的基准测试中与最先进方法比较，结果显示在保持渲染质量的同时，效率方面具有显著优势。

Conclusion: 提出的训练策略和损失函数有效缩短了像素渲染所需的高斯列表，显著提升了3D高斯泼溅的效率，为实时渲染应用提供了更好的解决方案。
...
TL;DR: SVOR是一个鲁棒的视频物体移除框架，通过MUSE、DA-Seg和课程两阶段训练三个关键技术，实现了无阴影、无闪烁、对缺陷掩码容忍的物体移除，在真实世界不完美条件下表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的视频修复模型在真实世界不完美条件下（如阴影、突变运动、缺陷掩码）难以保持时间稳定性和视觉一致性，需要更鲁棒的解决方案。

Method: 1. MUSE：窗口化联合策略，在时间掩码下采样时保留每个窗口内观察到的所有目标区域；2. DA-Seg：轻量级分割头，配备去噪感知AdaLN，通过掩码退化训练提供扩散感知定位先验；3. 课程两阶段训练：第一阶段在未配对真实背景视频上进行自监督预训练，第二阶段在合成对上使用掩码退化和副作用加权损失进行精炼。

Result: 在多个数据集和退化掩码基准测试中取得了新的最先进结果，将视频物体移除从理想设置推向真实世界应用。

Conclusion: SVOR通过三个关键设计实现了鲁棒的视频物体移除，能够有效处理真实世界的不完美条件，包括阴影、突变运动和缺陷掩码，显著提升了时间稳定性和视觉一致性。
...
TL;DR: 提出首个前馈模型用于开放世界凸分解，通过学习特征场实现高质量3D形状凸分解，适用于物理模拟碰撞检测等应用。


<details>
  <summary>Details</summary>
Motivation: 解决长期存在的凸分解问题，传统方法难以处理开放世界对象，需要一种能够泛化到各种3D表示形式的高质量凸分解方法，以加速物理模拟中的碰撞检测等应用。

Method: 采用特征学习方法，学习连续特征场，通过自监督的纯几何目标（基于凸性的经典定义）进行聚类，实现凸分解。支持单形状优化和基于大规模数据集的可扩展自监督学习。

Result: 实验表明，该方法产生的凸分解质量优于现有方法，能够泛化到开放世界对象，并跨表示形式工作（网格、CAD模型、高斯溅射等）。

Conclusion: 该方法首次实现了学习型开放世界凸分解模型，通过特征学习和自监督目标，能够产生高质量的凸分解，并具有良好的泛化能力。
...
TL;DR: CogBlender是一个文本到图像生成框架，能够连续、多维度地干预图像的认知属性（如情感、记忆性），通过认知空间与语义流形的映射实现认知驱动的图像生成。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型虽然能生成语义连贯的内容，但难以控制图像的认知属性（如情感效价、记忆性等），无法满足特定的心理意图需求。
...
TL;DR: MDTrack提出了一种用于多模态目标跟踪的新框架，通过模态感知融合和解耦时序传播来解决现有方法中模态差异被忽略和时序信息纠缠的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态跟踪器采用统一的融合策略，忽略了模态间的固有差异，并且通过混合token传播时序信息，导致时序表示纠缠且区分性不足。

Method: 1) 模态感知融合：为红外、事件、深度和RGB等模态分配专用专家，通过MoE的门控机制动态选择最优专家进行自适应融合；2) 解耦时序传播：引入两个独立的SSM结构分别存储和更新RGB和X模态流的隐藏状态，通过交叉注意力模块促进隐式信息交换。
...
TL;DR: DenoiseSplat：一种用于噪声多视图图像的3D高斯溅射方法，在RE10K基准上表现出色


<details>
  <summary>Details</summary>
Motivation: 大多数NeRF和3D高斯溅射方法假设输入干净，在真实噪声和伪影下性能下降，需要处理噪声多视图图像的方法

Method: 提出DenoiseSplat，基于轻量级MVSplat风格前馈骨干网络，仅使用干净的2D渲染作为监督，无需3D真值进行端到端训练

Result: 在噪声RE10K基准上，DenoiseSplat在PSNR/SSIM和LPIPS指标上优于原始MVSplat和两阶段基线（IDF + MVSplat），适用于多种噪声类型和强度

Conclusion: DenoiseSplat能有效处理噪声多视图图像，为3D场景重建和视图合成提供了鲁棒的解决方案
...
TL;DR: 提出IntroSVG框架，通过视觉感知反馈的闭环生成系统，解决现有文本到SVG生成方法因缺乏视觉反馈而质量受限的问题
...
Method: 提出内省式SVG生成框架(IntroSVG)，构建统一视觉语言模型同时担任生成器和批评者角色，通过监督微调学习生成SVG并提供渲染反馈，将早期失败转化为高质量纠错数据，再利用大容量教师模型构建偏好数据集，通过直接偏好优化对齐生成策略，推理时采用"生成-评审-优化"迭代循环

Result: 在多个关键评估指标上达到最先进性能，生成的SVG具有更复杂的结构、更强的语义对齐和更好的可编辑性

Conclusion: 将显式视觉反馈纳入生成循环是有效的，验证了视觉感知反馈对提升SVG生成质量的重要性
...
TL;DR: CLoE通过专家一致性学习框架解决多模态医学图像分割中模态缺失问题，通过决策级专家一致性控制和可靠性感知特征重校准，提升分割鲁棒性


<details>
  <summary>Details</summary>
Motivation: 多模态医学图像分割在推理时经常面临模态缺失问题，这导致模态专家之间的分歧，使得融合不稳定，特别是在小前景结构上

Method: 提出CLoE框架，采用双分支专家一致性学习目标：模态专家一致性强制专家预测全局一致以减少部分输入下的案例漂移；区域专家一致性强调临床关键前景区域的一致性以避免背景主导的正则化。使用轻量级门控网络将一致性分数映射到模态可靠性权重，实现可靠性感知的特征重校准

Result: 在BraTS 2020和MSD Prostate数据集上的实验表明，CLoE在不完整多模态分割中优于现有方法，同时表现出强大的跨数据集泛化能力，并提高了临床关键结构的鲁棒性

Conclusion: CLoE通过一致性驱动的专家学习框架有效解决了多模态医学图像分割中的模态缺失问题，在保持全模态性能的同时显著提升了部分模态情况下的分割鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 自主空间操作（如在轨服务和主动碎片清除）需要航天器部件级语义理解和精确相对导航，但收集大规模真实轨道数据成本高且不可行。现有合成数据集存在目标多样性有限、单模态感知、标注不完整等问题。

Method: 使用Unreal Engine 5构建高保真空间模拟环境，开发全自动流水线进行数据采集、多阶段质量控制，并转换为主流格式。数据集包含时间同步的1024×1024 RGB图像、毫米级精度深度图、256束LiDAR点云，以及像素级和点级的7类部件级语义标签和精确6-DoF姿态真值。

Result: 基准测试了5个代表性任务（目标检测、2D语义分割、RGB-LiDAR融合的3D点云分割、单目深度估计、姿态估计），发现两个关键问题：1)感知小尺度部件（如推进器和全向天线）和零样本泛化到未见航天器仍是当前方法的瓶颈；2)增加训练卫星数量能显著提升对新目标的性能。

Conclusion: SpaceSense-Bench为航天器感知研究提供了大规模、多样化的基准数据集，证明了大规模多样化数据集对空间感知研究的重要性。数据集、代码和工具包已公开。
...
TL;DR: 提出OddGridBench基准测试评估多模态大语言模型的视觉差异感知能力，发现现有模型表现远低于人类水平，并提出OddGrid-GRPO强化学习框架提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在低层次视觉感知，特别是细粒度视觉差异检测方面的能力尚未得到充分探索和系统分析，需要专门的评估基准。

Method: 构建OddGridBench基准测试，包含1400+网格图像，每个图像中有一个元素在颜色、大小、旋转或位置等视觉属性上与其他元素不同。提出OddGrid-GRPO强化学习框架，结合课程学习和距离感知奖励机制。

Result: 实验显示所有评估的MLLM（包括开源和专有系统）在视觉差异检测任务上表现远低于人类水平。OddGrid-GRPO框架显著提升了模型的细粒度视觉辨别能力。
...
TL;DR: STAR Benchmark是一个用于评估LLM在竞争性动态环境中战略推理能力的多智能体框架，通过1v1零和对抗交互，揭示了推理深度与执行时效之间的权衡关系。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法将推理视为单次能力，忽略了对抗环境中的对手感知决策、时间约束和压力下执行等挑战，需要新的评估框架来研究LLM在交互式动态环境中的战略智能。

Method: 提出STAR Benchmark：1）多智能体评估框架，支持回合制和实时两种设置；2）模块化架构，包含标准化API和完整执行引擎；3）战略评估套件，不仅评估胜负，还评估执行效率和结果稳定性等战略行为质量。

Result: 广泛评估显示明显的策略-执行差距：推理密集型模型在回合制环境中占优，但其推理延迟导致在实时场景中表现较差，而更快的指令调优模型在实时环境中表现更好。
...
TL;DR: 提出Predictive Spectral Calibration (PSC)方法，通过块谱匹配扩展子空间对齐，用于图像回归任务中的测试时适应，无需源数据且模型无关。


<details>
  <summary>Details</summary>
Motivation: 图像回归任务的测试时适应研究远少于分类任务，现有分类方法难以直接迁移到连续回归目标。虽然近期通过子空间对齐方法取得进展，但仍有改进空间。

Method: 提出PSC框架，联合对齐目标特征在源预测支持子空间内，并在正交补空间中校准残差谱松弛。该方法无需源数据，模型无关，可与现成预训练回归器兼容。

Result: 在多个图像回归基准测试中，相比强基线方法取得一致改进，在严重分布偏移下获得特别明显的性能提升。

Conclusion: PSC通过扩展子空间对齐到块谱匹配，为图像回归的测试时适应提供了简单有效的源无关解决方案，在分布偏移下表现优异。
...
TL;DR: 提出EPPINN框架，结合证据深度学习和物理信息建模，用于CT灌注成像中不确定性感知的灌注参数估计，提高急性缺血性卒中评估的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有基于PINN的方法在CT灌注成像去卷积问题中是确定性的，无法量化违反物理约束相关的不确定性，限制了可靠性评估。

Method: 提出EPPINN框架，使用坐标网络建模动脉输入、组织浓度和灌注参数，在物理残差上放置Normal-Inverse-Gamma分布来表征体素级的偶然和认知不确定性，无需贝叶斯采样或集成推理。

Result: 在数字幻影数据、ISLES 2018基准和临床队列上，EPPINN比传统去卷积和PINN基线获得更低的归一化平均绝对误差，在稀疏时间采样和低信噪比条件下表现更好，同时提供具有高经验覆盖率的保守不确定性估计。
...
TL;DR: 提出M3GCLR框架，通过博弈论对比学习方法解决骨架动作识别中视角差异建模不足、对抗机制缺乏和增强扰动不可控的问题。
...
Result: 在NTU RGB+D 60上达到三流82.1%(X-Sub)和85.8%(X-View)准确率，在NTU RGB+D 120上达到72.3%(X-Sub)和75.0%(X-Set)，在PKU-MMD Part I和II上分别达到89.1%和45.2%，均达到或超越最先进性能。
...
TL;DR: 提出MIL-PF框架，结合冻结的基础模型编码器和轻量级多示例学习头，用于乳腺X光片分类，显著降低训练复杂度并实现最先进性能


<details>
  <summary>Details</summary>
Motivation: 现代基础模型提供高度表达性的视觉表示，但适应高分辨率医学影像仍面临挑战，主要由于标注有限和弱监督。乳腺X光片具有图像大、多视图可变、主要使用乳腺级别标签等特点，使得端到端微调计算成本高且不实用

Method: 提出MIL-PF框架：使用冻结的基础模型编码器预计算语义表示，然后训练轻量级多示例学习头（仅4万参数）。通过基于注意力的聚合机制，显式建模全局组织上下文和稀疏局部病灶信号

Result: 在临床规模上实现最先进的分类性能，同时大幅降低训练复杂度。通过仅训练小型任务特定聚合模块，实现高效实验和适应，无需重新训练大型骨干网络

Conclusion: MIL-PF为高分辨率医学影像提供可扩展解决方案，结合冻结基础编码器和轻量级MIL头，在保持高性能的同时显著降低计算需求，代码已开源确保可复现性
...
TL;DR: SinGeo是一个简单而强大的跨视角地理定位框架，采用双判别学习架构和课程学习策略，实现了单一模型在各种视野条件下的鲁棒性能，无需额外模块或显式变换。


<details>
  <summary>Details</summary>
Motivation: 现有跨视角地理定位方法依赖于固定视野训练范式，当测试时遇到未见过的视野和未知方向时会失效，需要部署多个模型来覆盖不同变化。虽然已有研究探索动态视野训练，但未能实现跨多样条件的鲁棒性，隐含假设所有视野难度相同。

Method: SinGeo采用双判别学习架构，增强地面和卫星分支内的视图内判别能力，并首次引入课程学习策略实现鲁棒跨视角地理定位。该方法不需要额外模块或显式变换。

Result: 在四个基准数据集上的广泛评估表明，SinGeo在多样化条件下取得了最先进的结果，显著优于专门为极端视野训练的方法。此外，SinGeo还表现出跨架构的可迁移性。

Conclusion: SinGeo实现了单一模型在各种视野条件下的鲁棒跨视角地理定位，并提出了一种一致性评估方法来定量评估模型在不同视图下的稳定性，为理解和推进未来CVGL研究的鲁棒性提供了可解释的视角。
...
TL;DR: EventVGGT：首个将视觉几何基础模型VGGT的时空先验知识蒸馏到事件相机深度估计的框架，通过三级蒸馏策略显著提升深度预测的时空一致性和准确性。
...
Conclusion: 通过将事件流建模为视频序列并蒸馏VGGT的时空先验，EventVGGT显著提升了事件深度估计的准确性和时间一致性，为挑战性条件下的鲁棒3D感知提供了有效解决方案。
...
TL;DR: MIDAS：基于扩散的无训练隐写框架，通过潜在空间融合实现多图像隐藏和用户特定访问控制
...
Method: 提出MIDAS框架，包含随机基机制抑制残留结构信息，以及潜在向量融合模块重塑聚合潜在向量以对齐扩散过程

Result: MIDAS在访问控制功能、隐写图像质量和多样性、噪声鲁棒性以及抗隐写分析方面均优于现有无训练隐写基线方法

Conclusion: MIDAS为访问控制的无载体隐写提供了实用且可扩展的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 推动文档图像机器翻译（DIMT）研究，通过联合建模文本内容和页面布局，弥合OCR与NLP之间的差距，促进多模态文档理解领域的发展。

Method: 设立两个赛道：OCR-free（无需OCR）和OCR-based（基于OCR），每个赛道又分为小模型（<1B参数）和大模型（>1B参数）两个子任务。参赛者提交统一的DIMT系统，可选择使用提供的OCR转录文本。

Result: 竞赛吸引了69个团队，共收到27份有效提交。Track 1有34个团队和13份有效提交，Track 2有35个团队和14份有效提交。大规模模型方法为复杂布局文档图像翻译建立了有前景的新范式。
...
TL;DR: 提出了YOLO-NAS-Bench，首个针对YOLO检测器的代理基准，通过自进化机制提升预测器性能，发现超越官方YOLO基线的新架构。


<details>
  <summary>Details</summary>
Motivation: 目标检测的神经架构搜索面临高昂评估成本，现有NAS基准主要针对图像分类，缺乏针对检测器的可比基准。

Method: 定义YOLO风格检测器的搜索空间，采样1000个架构，构建LightGBM代理预测器，提出自进化机制逐步对齐预测器训练分布与高性能前沿。

Result: 自进化机制将架构池扩展到1500个，提升预测器R2从0.770到0.815，Sparse Kendall Tau从0.694到0.752，进化搜索发现的架构在COCO-mini上超越所有官方YOLO基线。

Conclusion: YOLO-NAS-Bench填补了检测NAS基准空白，自进化机制有效提升预测器性能，为高效发现高性能检测架构提供了可靠工具。
...
TL;DR: FCDM是一种完全卷积的扩散模型，使用类似ConvNeXt的卷积架构，相比Transformer扩散模型具有更高的训练效率和硬件友好性。
...
Method: 提出完全卷积扩散模型(FCDM)，采用类似ConvNeXt的卷积架构，专门为条件扩散建模设计。该架构保持了卷积网络的效率优势，同时适用于生成任务。

Result: FCDM-XL仅使用DiT-XL/2 50%的FLOPs，在256×256和512×512分辨率下分别用7倍和7.5倍更少的训练步骤达到竞争性能。FCDM-XL可在4-GPU系统上训练，显示出色的训练效率。

Conclusion: 现代卷积设计为扩散模型提供了竞争性强且高效的替代方案，ConvNeXt作为简单而强大的构建块在高效生成建模中具有复兴价值。
...
TL;DR: RiO-DETR是首个实时定向目标检测Transformer，通过内容驱动角度估计、解耦周期性优化和定向密集监督解决了DETR在定向检测中的三个关键挑战，实现了速度与精度的新平衡。


<details>
  <summary>Details</summary>
Motivation: 将DETR应用于定向边界框检测面临三个主要挑战：语义依赖的方向性、角度周期性破坏标准欧几里得优化、以及增大的搜索空间导致收敛缓慢。需要设计既能解决这些问题又能保持实时效率的方法。

Method: 1. 内容驱动角度估计：将角度与位置查询解耦，结合旋转校正正交注意力捕捉互补线索；2. 解耦周期性优化：结合有界粗到细更新和最短路径周期性损失；3. 定向密集监督：向密集监督注入角度多样性以加速角度收敛。

Result: 在DOTA-1.0、DIOR-R和FAIR-1M-2.0数据集上的广泛实验表明，RiO-DETR在实时定向检测中建立了新的速度-精度平衡。

Conclusion: RiO-DETR是首个实时定向检测Transformer，通过任务原生设计解决了DETR在定向检测中的关键挑战，在保持实时效率的同时实现了优异的检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常直接将不同领域的DLA数据集合并训练，但由于忽略了不同领域在标注风格、文档类型和语言等方面的布局结构差异，导致模型性能不佳。需要一种能够有效整合领域先验知识的方法来提升跨领域泛化能力。

Method: 提出PromptDLA，包含一个独特的领域感知提示器，能够根据数据领域的特定属性定制提示。这些提示作为线索，引导DLA模型关注数据中的关键特征和结构，从而增强模型在不同领域的泛化能力。

Result: 在DocLayNet、PubLayNet、M6Doc和D$^4$LA等数据集上的大量实验表明，该方法在这些数据集上达到了最先进的性能。

Conclusion: PromptDLA通过领域感知提示器有效整合领域先验知识，显著提升了文档布局分析在跨领域场景下的泛化能力和性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有全身姿态估计器在复杂场景中缺乏鲁棒性，产生解剖学上不合理的预测。作者认为这是由于从视觉上下文中学习到的虚假相关性导致的，这可以通过结构因果模型形式化为混淆问题。

Method: 提出CIGPose框架，包含新颖的因果干预模块：通过预测不确定性识别受混淆的关键点表示，并用学习到的上下文不变规范嵌入替换它们。然后使用层次图神经网络在局部和全局语义层次上对人体骨架进行推理，确保解剖合理性。

Result: CIGPose在COCO-WholeBody上达到新的SOTA，CIGPose-x模型达到67.0% AP，超越依赖额外训练数据的先前方法。结合UBody数据集后，进一步提升至67.5% AP，显示出优越的鲁棒性和数据效率。

Conclusion: 通过因果干预方法解决姿态估计中的虚假相关性问题是有效的，CIGPose框架能够产生解剖学上合理的预测，在复杂场景中表现出更强的鲁棒性，为姿态估计领域提供了新的因果推理视角。
...
TL;DR: 提出一种用于轨迹预测的元学习框架，通过双层优化和自适应模型更新机制，在测试时实现快速准确的在线适应，优于现有方法。
...
Method: 1) 提出元学习框架，在预训练阶段通过模拟测试时适应任务进行双层优化；2) 测试时引入数据自适应模型更新机制，基于在线偏导数和困难样本选择动态调整学习率和更新频率。

Result: 在nuScenes、Lyft和Waymo等跨数据集分布偏移场景下，该方法在适应准确性上优于最先进的测试时训练方法，且在次优学习率和高FPS需求下表现优异。

Conclusion: 该方法通过元学习优化和自适应更新机制，有效解决了轨迹预测中的测试时分布偏移问题，展现了鲁棒性和实用性。
...
TL;DR: 提出首个端到端类增量运动预测框架，解决自动驾驶中感知不完美和物体分类随时间演变的问题，通过伪标签策略和基于查询特征方差的回放采样来缓解灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有运动预测方法在封闭世界假设下运行，需要固定物体分类和高质量感知，但在现实世界中感知不完美且物体分类会随时间演变，因此需要开放世界运动预测框架。

Method: 提出首个端到端类增量运动预测框架：1) 使用伪标签策略为新类别生成运动预测伪标签，通过视觉语言模型过滤不一致和过度自信的预测；2) 采用基于查询特征方差的回放采样策略，选择具有信息性运动模式的先前序列来缓解灾难性遗忘。

Result: 在nuScenes和Argoverse 2数据集上的广泛评估表明，该方法成功抵抗灾难性遗忘，保持对已学习类别的性能，同时提高对新类别的适应能力，并支持零样本迁移到真实世界驾驶。

Conclusion: 该工作首次提出开放世界运动预测设置和相应的类增量学习框架，能够持续适应自动驾驶系统，为现实世界自动驾驶中的动态物体预测提供了有效解决方案。
...
TL;DR: 本文提出GIIM图神经网络框架，通过建模病灶在单视图内的依赖关系和跨视图的动态变化，解决医学影像CADx中多视图关系建模不足和数据缺失问题，在CT、MRI和乳腺X光等多种模态上显著提升诊断准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前计算机辅助诊断系统难以模拟临床专家对多视图影像的综合分析过程，现有多视图CADx方法忽略了病灶在单视图内的依赖关系和跨视图的动态变化，且对临床常见的数据缺失问题处理不足，导致预测可靠性降低。

Method: 提出GIIM图神经网络框架，将诊断任务重构为关系建模问题。该框架同时捕获病灶在单视图内的依赖关系（intra-view dependencies）和跨视图的动态变化（inter-view dynamics），并采用特定技术处理临床常见的数据缺失问题，确保诊断鲁棒性。

Result: 在CT、MRI和乳腺X光等多种医学影像模态上进行广泛评估，结果表明GIIM模型相比现有方法显著提升了诊断准确性和鲁棒性，验证了该方法的通用性和有效性。

Conclusion: GIIM通过建模病灶在单视图内和跨视图的复杂关系，并有效处理数据缺失问题，为未来CADx系统建立了更有效的框架，显著提升了医学影像诊断的准确性和临床实用性。
...
TL;DR: OncoAgent：一种无需训练的指南感知AI代理框架，可将文本临床指南直接转换为三维靶区轮廓，在食管癌放疗中实现与监督学习相当的零样本性能，并具有出色的临床可接受性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习模型依赖专家标注数据，当临床指南更新时需要昂贵的重新训练，限制了其在放疗靶区勾画中的实际应用和适应性。

Method: 提出OncoAgent框架，将文本临床指南转换为三维靶区轮廓，采用训练免费的方法，无需依赖大量标注数据进行模型训练。

Result: 在食管癌病例中，零样本Dice相似系数达到0.842（CTV）和0.880（PTV），与监督学习的nnU-Net基线相当。临床盲评中医生更偏好OncoAgent，认为其指南依从性更好、修改工作量更少、临床可接受性更高。框架还能零样本泛化到其他食管指南和前列腺等其他解剖部位。

Conclusion: 基于代理的范式提供了近乎即时的指南适应性，为放疗治疗计划提供了一条可扩展且透明的可解释性路径，超越了单纯的体积重叠指标。
...
<details>
  <summary>Details</summary>
Motivation: 现有的Vision-Language-Action模型在自动驾驶中存在两个主要问题：1）解冻视觉编码器后感知性能下降；2）长期规划中累积的不稳定性。需要一种方法来同时解决感知退化和规划不稳定的挑战。

Method: 提出协作感知-规划蒸馏框架，包含两个核心组件：1）自锚定视觉蒸馏：利用自锚定教师提供视觉锚定约束，通过轨迹引导的关键区域感知来正则化学生表示；2）先知引导轨迹蒸馏：使用未来感知的先知教师，通过粗到细的轨迹优化和蒙特卡洛dropout采样生成高质量轨迹候选，选择最优轨迹指导学生预测。

Result: EvoDriveVLA在开环评估中达到了最先进的性能，并在闭环评估中显著提升了性能表现。

Conclusion: 该论文提出的协作感知-规划蒸馏框架有效解决了VLA模型在自动驾驶中的感知退化和长期规划不稳定问题，通过自锚定视觉约束和先知引导轨迹优化实现了性能提升。
...
TL;DR: TopoOR提出了一种新的高阶拓扑表示方法，用于建模手术室的多模态场景，超越了传统场景图的二元结构限制，在多个手术任务上优于传统图方法和LLM基线。


<details>
  <summary>Details</summary>
Motivation: 现有手术场景图方法主要依赖成对消息传递或标记化序列，这会扁平化关系结构的流形几何，丢失结构信息。传统方法无法有效建模手术室中复杂的高阶关系和多种模态数据。

Method: 提出TopoOR范式，将多模态手术室建模为高阶拓扑结构，将实体间的交互提升为高阶拓扑单元。同时提出高阶注意力机制，在层次关系注意力中显式保持流形结构和模态特定特征。

Result: 在无菌违规检测、机器人阶段预测和下一动作预测等任务上，TopoOR显著优于传统图方法和基于LLM的基线方法。该方法避免了将3D几何、音频和机器人运动学融合到单一潜在表示中，保持了安全关键推理所需的多模态结构。

Conclusion: TopoOR通过高阶拓扑表示提供了比传统场景图更强的表达能力，能够更好地建模手术室中的复杂动态和多模态交互，为安全关键的手术推理任务提供了更有效的框架。
...
TL;DR: 开发了首个大规模开放OCR和语言学资源Patrologia Graeca Corpus，用于处理19世纪古希腊语双语印刷文本，通过专门流程实现高精度识别，创建了包含600万词元的标注语料库。


<details>
  <summary>Details</summary>
Motivation: 解决Patrologia Graeca（PG）剩余未数字化卷册的OCR问题，这些文本采用复杂的希腊语-拉丁语双语排版，包含高度退化的多调希腊语字体，现有OCR系统难以处理。

Method: 采用专门流程，结合YOLO-based布局检测和CRNN-based文本识别技术，处理复杂的双语布局和退化的多调希腊语字体。

Result: 字符错误率（CER）1.05%，词错误率（WER）4.69%，远超现有多调希腊语OCR系统。创建了包含约600万词元的语料库，包含词形还原、词性标注，并与完整OCR和布局注释对齐。

Conclusion: 该语料库不仅具有文献学价值，还为嘈杂多调希腊语OCR建立了新基准，为未来模型（包括LLMs）提供了训练材料，填补了该领域资源空白。
...
TL;DR: OmniEarth是一个用于评估遥感视觉语言模型的综合性基准，包含感知、推理和鲁棒性三个维度，涵盖28个细粒度任务，支持多种任务形式，旨在解决现有VLM在复杂地理空间任务上的不足。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在通用领域任务上表现出色，但在遥感领域的应用缺乏系统性评估基准。现有遥感视觉语言模型评估不够全面，需要建立一个能够真实反映地球观测场景的基准来评估模型性能。

Method: 构建OmniEarth基准，包含三个能力维度（感知、推理、鲁棒性），28个细粒度任务，涵盖多源遥感数据和多样化地理空间上下文。支持两种任务形式：多项选择VQA和开放式VQA（包括文本输出、边界框输出和掩码输出）。采用盲测协议和五重语义一致性要求来减少语言偏见并验证视觉证据依赖。

Result: 基准包含9,275张经过质量控制的图像（包括吉林一号卫星影像）和44,210条人工验证的指令。系统评估了基于对比学习的模型、通用闭源和开源VLM以及RSVLM。结果显示现有VLM在处理地理空间复杂任务时仍存在困难，揭示了遥感应用中的明显差距。

Conclusion: OmniEarth为遥感视觉语言模型提供了首个系统性评估基准，揭示了现有模型在复杂地理空间任务上的局限性，为未来遥感VLM的发展指明了方向。基准已公开可用，将促进遥感领域视觉语言模型的研究与改进。
...
TL;DR: PruneSID是一种无需训练的双阶段视觉token压缩方法，通过语义聚类和组内去冗余，在保持信息多样性的同时大幅减少视觉token数量，显著提升视觉语言模型的推理效率。
...
Method: 提出PruneSID双阶段流程：1）PSCA将token聚类为语义连贯的组，确保概念覆盖完整性；2）组内NMS去除冗余token，保留关键代表性token。还包含基于图像复杂度的动态压缩比机制。

Result: 在LLaVA-1.5上仅保留11.1% token达到96.3%准确率，在LLaVA-NeXT上极端压缩率（5.6%）下达到92.8%准确率，比先前方法提升2.5%，预填充速度比原始模型快7.8倍。

Conclusion: PruneSID是一种高效、无需训练的视觉token压缩框架，在保持模型性能的同时显著提升推理速度，具有良好的跨模型和跨模态泛化能力。
...
TL;DR: 提出一种组件感知、自优化的草图到图像生成框架，通过两阶段架构解决现有方法在细节重建、空间对齐和跨域适应方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有GAN和扩散模型在将手绘草图转换为逼真图像时面临挑战，包括难以重建细粒度细节、保持空间对齐以及适应不同草图域的风格多样性。

Method: 采用三组件架构：1) 基于自注意力的自动编码器网络(SA2N)从组件级草图区域提取局部语义和结构特征；2) 坐标保持门控融合模块(CGF)将这些特征整合到连贯的空间布局中；3) 基于改进StyleGAN2的空间自适应细化修正器(SARR)通过空间上下文引导的迭代细化增强真实感和一致性。

Result: 在面部(CelebAMask-HQ, CUFSF)和非面部(Sketchy, ChairsV2, ShoesV2)数据集上均表现出色，显著优于现有GAN和扩散模型。在CelebAMask-HQ上，FID提升21%，IS提升58%，KID提升41%，SSIM提升20%。
...
TL;DR: Diagonal Distillation：一种针对视频扩散模型的高效蒸馏方法，通过非对称生成策略（早期多步、后期少步）和隐式光流建模，在保持视频质量的同时实现277倍加速，达到31FPS实时生成。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散蒸馏方法主要借鉴图像特定技术，忽略了时间依赖性，导致运动连贯性差、长序列错误累积和延迟-质量权衡问题。需要一种能更好利用时间信息的方法来支持实时视频流生成。

Method: 提出对角线蒸馏方法：1）非对称生成策略：早期视频块使用更多去噪步骤，后期块使用较少步骤，让后期块继承早期块的丰富外观信息；2）使用部分去噪块作为后续合成的条件输入；3）引入隐式光流建模以在严格步数限制下保持运动质量。

Result: 方法在5秒视频生成上达到2.61秒（最高31FPS），相比未蒸馏模型实现277.3倍加速，同时保持了视频质量和运动连贯性。

Conclusion: 对角线蒸馏通过更好地利用时间信息解决了现有视频蒸馏方法的局限性，实现了高质量实时视频生成，为视频流应用提供了实用解决方案。
...
TL;DR: EvoPrompt：一种通过控制提示演化轨迹来避免灾难性遗忘的视觉语言模型少样本适应框架


<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型在有限标注数据下适应下游任务时，现有参数高效的提示学习方法经常遭受灾难性遗忘预训练知识的问题。需要一种能够稳定、保持知识的微调方法。

Method: 提出EvoPrompt框架：1）使用模态共享提示投影器从统一嵌入空间生成分层提示；2）进化训练策略将低秩更新解耦为方向和幅度分量，保持早期学习的语义方向仅调整幅度；3）特征几何正则化强制特征去相关防止表示崩溃。

Result: 大量实验表明，EvoPrompt在少样本学习上达到最先进性能，同时鲁棒地保持了预训练视觉语言模型的原始零样本能力。

Conclusion: 通过显式控制提示演化轨迹，EvoPrompt实现了稳定、知识保持的视觉语言模型适应，解决了灾难性遗忘问题，在少样本学习中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 手术场景多任务联邦学习在机器人辅助微创手术中很重要，但面临两个关键挑战：1) 组织多样性：局部模型难以适应特定部位的组织特征；2) 任务多样性：服务器端聚合仅依赖梯度聚类，在任务异质性下产生次优参数更新。

Method: 提出SurgFed框架，包含两个核心设计：语言引导通道选择(LCS) - 轻量级个性化通道选择网络，使用预定义文本输入增强站点特定适应；语言引导超聚合(LHA) - 层间交叉注意力机制，使用文本输入建模跨站点任务交互，指导超网络进行个性化参数更新。
...
TL;DR: 本文提出Context-Nav方法，通过全局探索先验和3D空间推理来解决文本目标实例导航问题，无需任务特定训练即可在InstanceNav和CoIN-Bench上达到SOTA性能。
...
Method: 提出两阶段方法：1) 计算密集文本-图像对齐生成价值地图，将整个描述编码为全局探索先验，引导探索朝向与描述一致的区域；2) 观察候选对象时进行视点感知关系检查，采样合理观察姿态，对齐局部坐标系，仅当从至少一个视点满足空间关系时才接受目标。

Result: 在InstanceNav和CoIN-Bench基准测试中达到最先进性能，无需任务特定训练或微调。消融实验显示：完整描述编码到价值地图避免无效运动，显式的视点感知3D验证防止语义合理但错误的停止。
...
TL;DR: 该研究评估了视觉语言模型在驾驶助手应用中的一致性和时序推理能力，发现现有模型存在响应不一致和有限时序推理的问题，提出了FutureVQA基准数据集和基于思维链的自监督调优方法。


<details>
  <summary>Details</summary>
Motivation: 可靠的驾驶助手应基于观察信息进行时序推理并提供一致响应。当前研究假设强大的视觉理解能力自然能实现一致的未来推理和可靠决策，但这一假设需要验证。需要探究VLMs是否真正理解当前观察如何塑造未来结果，还是仅仅反映训练中记忆的模式。

Method: 采用现有评估方法，并引入FutureVQA——一个专门用于评估未来场景推理的人工标注基准数据集。提出简单有效的自监督调优方法，结合思维链推理，无需时序标签即可改善一致性和时序推理能力。

Result: 研究发现VLMs存在两大限制：响应不一致（微小输入扰动导致不同答案或接近随机猜测）和有限时序推理（无法从当前观察推理和对齐序列事件，导致错误甚至矛盾响应）。具有强大视觉理解的模型在需要时序推理的任务上不一定表现最佳，表明模型倾向于过度依赖预训练模式而非建模时序动态。

Conclusion: 该研究揭示了VLMs作为驾驶助手在一致性和时序推理方面的局限性，提出了评估基准和改进方法，为开发更可靠的驾驶助手系统提供了重要见解。
...
TL;DR: RESBev是一个即插即用的BEV感知增强方法，通过潜在世界模型学习BEV状态转移，预测干净的语义特征来重建受损观测，提升现有BEV模型对各种干扰和对抗攻击的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: BEV感知在自动驾驶中至关重要，但实际部署面临传感器退化和对抗攻击的挑战，这些干扰会导致感知异常并危及系统安全。现有方法缺乏对这些干扰的鲁棒性。

Method: 将感知鲁棒性重构为潜在语义预测问题。构建潜在世界模型提取序列BEV观测的时空相关性，学习BEV状态转移来预测干净的BEV特征，在Lift-Splat-Shoot流程的语义特征层面进行恢复，无需修改底层骨干网络。

Result: 在nuScenes数据集上的大量实验表明，通过少量微调，RESBev能显著提升现有BEV感知模型对各种外部干扰和对抗攻击的鲁棒性，且能泛化到自然干扰和对抗攻击。

Conclusion: RESBev提供了一个即插即用的鲁棒性增强框架，能有效提升BEV感知系统在现实世界部署中的可靠性，为自动驾驶安全提供了重要保障。
...
TL;DR: DCAU-Net：一种新型医学图像分割框架，通过差分交叉注意力（DCA）和通道-空间特征融合（CSFF）策略，在降低计算复杂度的同时提升分割精度。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像分割方法面临两个主要问题：1）Transformer虽然能建模长距离依赖，但标准自注意力计算复杂度高且会关注无关区域，降低对判别性结构的关注；2）编码器-解码器架构中的传统融合策略（如简单拼接或求和）无法自适应地整合高层语义信息和低层空间细节。

Method: 提出DCAU-Net框架，包含两个核心创新：1）差分交叉注意力（DCA）：计算两个独立softmax注意力图的差异，自适应地突出判别性结构；用窗口级汇总标记替代像素级键值标记，大幅降低计算复杂度。2）通道-空间特征融合（CSFF）：通过顺序的通道和空间注意力，自适应地重新校准跳跃连接和上采样路径的特征，抑制冗余信息并放大显著线索。

Result: 在两个公共基准测试上的实验表明，DCAU-Net在保持计算效率的同时，实现了具有竞争力的分割性能，提升了分割精度和鲁棒性。

Conclusion: DCAU-Net通过创新的差分交叉注意力和通道-空间特征融合策略，有效解决了医学图像分割中长距离依赖建模和特征融合的挑战，在计算效率和分割精度之间取得了良好平衡。
...
Method: 采用强化学习后训练策略：1) 热身阶段使用混合数据集；2) 提出统一策略优化框架，将GRPO扩展到多模态设置；3) 联合建模文本和图像生成，使用混合奖励函数；4) 加入过程级奖励提供逐步指导
...
TL;DR: 提出了DIVRR框架，通过动态感知的视角优化和相关性引导的自适应记忆选择，解决动态环境中具身问答的感知非平稳性问题，在保持推理效率的同时提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统具身问答评估在静态环境中进行，但在动态、有人类活动的场景中，人类活动和遮挡会引入显著的感知非平稳性：任务相关线索是瞬时的且视角依赖的，而"先存储后检索"策略会过度积累冗余证据并增加推理成本。

Method: 提出了DIVRR（动态感知的视角优化和相关性引导的自适应记忆选择）框架，该框架结合了相关性引导的视角优化和选择性记忆准入机制。在提交模糊观察之前进行验证，并仅保留信息性证据。

Result: 在DynHiL-EQA数据集和已建立的HM-EQA数据集上的广泛实验表明，DIVRR在动态和静态设置中均持续优于现有基线，同时保持较高的推理效率。

Conclusion: DIVRR框架通过验证模糊观察和选择性保留信息性证据，提高了遮挡下的鲁棒性，同时通过紧凑记忆保持快速推理，为动态环境中具身问答的挑战提供了有效解决方案。
...
TL;DR: 该研究对多种飞行时间非视距成像方法进行了系统比较，在统一硬件条件下评估了不同方法的性能，发现现有方法在相同硬件约束下具有相似的分辨率、可见性和噪声敏感性限制。


<details>
  <summary>Details</summary>
Motivation: 飞行时间非视距成像技术发展迅速，但各种方法采用不同的公式和硬件实现，使得理论评估和实验比较变得困难。需要建立一个统一的评估框架来客观比较不同方法的性能。

Method: 首先建立通用的前向测量模型，将不同方法统一到相同的数学框架中。然后讨论简化模型与Radon变换的关系，以及频域分析与基于相位的虚拟视距成像模型的关系。最后在相同的硬件设置和光子计数条件下，对代表性方法进行实验评估。

Result: 实验表明，在相同的硬件约束下，现有方法在空间分辨率、可见性和噪声敏感性方面具有相似的局限性。不同方法之间的差异主要来自方法特定的参数设置。

Conclusion: 该研究为飞行时间非视距成像领域提供了一个标准化的评估方法学，可作为未来研究的参考框架，用于客观比较现有和新方法的性能。
...
TL;DR: GeoSolver是一个用于遥感图像理解的可验证过程监督强化学习框架，通过构建大规模过程监督数据集和训练token级过程奖励模型，实现了逐步推理的可信度验证和性能提升。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在遥感解释方面取得了显著进展，但让它们执行复杂的逐步推理仍然极具挑战。现有的思维链方法虽然显示出潜力，但确保中间步骤的视觉忠实性仍然是一个关键瓶颈。

Method: 1) 构建Geo-PRM-2M数据集：通过熵引导的蒙特卡洛树搜索和有针对性的视觉幻觉注入合成大规模token级过程监督数据；2) 训练GeoPRM模型：基于该数据集训练token级过程奖励模型，提供细粒度的忠实性反馈；3) 提出Process-Aware Tree-GRPO算法：将树结构探索与忠实性加权奖励机制结合的强化学习算法，精确分配中间步骤的信用。

Result: GeoSolver-9B模型在多个遥感基准测试中实现了最先进的性能。GeoPRM解锁了强大的测试时缩放能力，作为通用地理空间验证器，不仅能无缝扩展GeoSolver-9B的性能，还能直接增强通用视觉语言模型，显示出卓越的跨模型泛化能力。

Conclusion: GeoSolver框架通过引入可验证的过程监督强化学习，成功解决了遥感推理中视觉忠实性的关键瓶颈，为复杂遥感任务提供了可靠且可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言预训练模型主要依赖全局图像-文本对齐，未能有效整合多粒度视觉和文本信息，限制了模型在复杂细粒度任务中的性能，特别是在遥感领域需要精确捕捉图像细节的应用中。

Method: 提出GeoAlignCLIP框架：1）学习多粒度语义对齐，实现图像区域与文本概念的精确视觉-语义对齐；2）引入模态内一致性学习；3）构建RSFG-100k细粒度遥感数据集，包含场景描述、区域级标注和困难负样本，为模型训练提供分层监督。

Result: 在多个公开遥感基准测试上的广泛实验表明，GeoAlignCLIP在多样化任务中持续优于现有的遥感特定方法，展现出更鲁棒和准确的细粒度视觉语言对齐能力。

Conclusion: GeoAlignCLIP通过多粒度对齐和模态内一致性学习，有效解决了遥感图像与文本细粒度对齐的挑战，新构建的数据集为模型训练提供了重要支持，在遥感视觉语言任务中取得了显著性能提升。
...
TL;DR: 提出全景语言建模（PLM）范式，通过全景稀疏注意力模块让现有针孔视觉语言模型处理全景图像，无需重新训练，并在PanoVQA数据集上验证了全景推理的优势。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（VLMs）针对针孔图像设计，通过拼接多个窄视场输入来理解全景场景，但这种方法忽略了全景图像固有的整体空间和上下文关系。需要一种统一的全景视觉语言推理方法。
...
Conclusion: PLM范式为全景视觉语言理解提供了统一框架，通过全景稀疏注意力模块有效利用全景图像的整体空间关系，在复杂场景下展现出优于传统多视图方法的表现。
...
TL;DR: 提出BinaryAttention方法，将注意力机制中的queries和keys二值化到1-bit，用位运算替代浮点运算，在保持相似关系的同时显著降低计算成本，速度比FlashAttention2快2倍以上。


<details>
  <summary>Details</summary>
Motivation: Transformer注意力模块的计算复杂度是视觉任务的主要瓶颈，现有方法主要使用8-bit或4-bit量化来平衡效率和精度。本文通过理论分析发现注意力二值化能保持本质的相似关系，因此提出更极端的1-bit量化方法。

Method: 1) 仅保留queries和keys的符号进行注意力计算；2) 用位运算替代浮点点积；3) 通过可学习偏置缓解1-bit量化的信息损失；4) 采用量化感知训练和自蒸馏技术减少量化误差并确保符号对齐的相似性。
...
Conclusion: BinaryAttention为全精度注意力提供了高效有效的替代方案，推动了低比特视觉和扩散Transformer的前沿发展。该方法在保持精度的同时显著加速了注意力计算。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到动作生成方法主要关注整体动作生成，难以准确反映涉及特定身体部位的动作。最近的部件级方法虽然尝试解决这个问题，但存在两个关键限制：缺乏文本语义与单个身体部位对齐的明确机制，以及由于整合独立生成的部件动作而导致整体动作不连贯。

Method: ParTY框架包含三个核心组件：1) 部件引导网络：首先生成部件动作作为引导，然后生成整体动作；2) 部件感知文本对齐：多样化转换文本嵌入并适当与每个身体部位对齐；3) 整体-部件融合：自适应融合整体动作和部件动作。

Result: 通过部件级和连贯性级的广泛实验表明，ParTY相比先前方法取得了显著改进，在增强部件表达能力的同时生成了连贯的整体动作。

Conclusion: ParTY成功解决了现有方法在部件表达和整体连贯性之间的基本权衡，通过创新的部件引导和融合机制，实现了更准确、更自然的文本到动作生成。
...
TL;DR: 该论文提出了一种受人类视觉系统启发的选择性注意力机制，利用DINO视觉变换器的注意力图来模拟眼动扫视，在图像分类任务中实现高效处理。


<details>
  <summary>Details</summary>
Motivation: 人类视觉系统在严格代谢约束下仍能实现卓越感知性能，关键在于选择性注意力机制和眼动扫视。传统AI系统通常平等处理整个图像，效率较低。研究旨在从人类视觉系统汲取灵感，创建更智能、更高效的图像处理模型。

Method: 使用自监督视觉变换器DINO生成类似人类注视模式的注意力图，探索受扫视启发的方法来聚焦处理视觉空间中的关键区域。在ImageNet数据集的标准分类任务中，测量每次连续扫视对模型类别分数的影响。

Result: 选择性处理策略保留了大部分全图像分类性能，在某些情况下甚至能超越全图像分类。与为人类注视预测建立的显著性模型相比，DINO为选择信息区域提供了更优越的注视引导。

Conclusion: 视觉变换器注意力为受生物启发的主动视觉提供了有前景的基础，并为高效、神经形态的视觉处理开辟了新方向。
...
Method: 使用显式高斯表示，定制高斯参数嵌入组织物理特性；采用物理基础的体积渲染策略模拟MRI信号形成；设计砖块式顺序无关光栅化方案实现高度并行3D计算

Result: 在两个公共MRI数据集上的实验显示，该方法在重建质量和效率方面均表现优越，展示了临床MRI超分辨率的潜力

Conclusion: 提出的零样本MRI超分辨率框架成功平衡了数据需求和计算效率，通过显式高斯表示和物理启发渲染实现了高质量、高效率的MRI图像重建
...
TL;DR: QDR框架通过FP32自蒸馏、解码器自由蒸馏、可学习幅度重加权和边缘友好模型设计，解决了量化感知训练在图像恢复任务中的三个关键瓶颈，实现了高效的边缘部署。


<details>
  <summary>Details</summary>
Motivation: 虽然量化感知训练与知识蒸馏结合在模型压缩方面很有前景，但在精度敏感的图像恢复任务中，直接应用会面临教师-学生容量不匹配、空间误差放大和优化冲突三个关键瓶颈，需要专门解决方案。

Method: 提出QDR框架：1) FP32自蒸馏消除容量不匹配；2) 解码器自由蒸馏在瓶颈处校正量化误差；3) 可学习幅度重加权动态平衡竞争梯度；4) 边缘友好模型包含可学习退化门控动态定位空间退化。

Result: Int8模型恢复FP32性能的96.5%，在NVIDIA Jetson Orin上达到442 FPS，下游目标检测性能提升16.3 mAP，在四个图像恢复任务上验证了有效性。

Conclusion: QDR框架成功解决了量化感知训练在图像恢复任务中的关键挑战，实现了高效的边缘部署，为精度敏感的低层视觉任务提供了实用的量化蒸馏解决方案。
...
TL;DR: 提出基于视觉-语言框架的遥感图像合成数据增强与评估方法，创建包含40万张图像的大规模数据集ARAS400k，通过语义组合、描述冗余度和跨模态一致性实现自动化评估，实验表明合成数据增强能提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有合成数据评估指标通常基于潜在特征相似度，难以解释且与下游任务贡献相关性不强。需要一种可解释的合成数据增强与评估框架，特别是在遥感领域。

Method: 提出视觉-语言融合框架，结合生成模型、语义分割和图像描述技术。创建ARAS400k数据集（10万真实图像+30万合成图像），每张图像都配有分割图和描述。通过分析语义组合、最小化描述冗余、验证视觉结构与语言描述的跨模态一致性来实现自动化评估。

Result: 实验结果表明：仅使用合成数据训练的模型能达到竞争性性能水平，而使用增强数据（真实+合成）训练的模型始终优于仅使用真实数据的基线模型。

Conclusion: 该工作为遥感任务（特别是语义分割和图像描述）建立了可扩展的基准，提供了一种可解释的合成数据增强与评估框架，证明合成数据增强能有效提升下游任务性能。
...
TL;DR: X-GS是一个可扩展的开放框架，统一了多种技术，实现实时3D高斯泼溅的在线SLAM，并融入语义信息，为下游多模态模型提供支持。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法大多孤立，专注于特定领域（如在线SLAM、语义增强或非标定图像处理），缺乏统一的框架来整合这些技术并支持下游多模态应用。

Method: 核心是X-GS-Perceiver高效管道，处理非标定RGB（或RGB-D）视频流，协同优化几何和姿态，从视觉基础模型中提取高维语义特征并蒸馏到3D高斯中。采用在线向量量化模块、GPU加速的网格采样方案和高度并行化管道设计实现实时性能。

Result: 在真实世界数据集上的实验展示了X-GS框架的有效性、效率和新解锁的多模态能力。语义3D高斯可通过X-GS-Thinker组件被视觉语言模型利用，支持物体检测、零样本字幕生成等下游任务。

Conclusion: X-GS提供了一个统一的开放框架，将实时3DGS在线SLAM与语义增强相结合，为下游多模态模型和应用搭建了桥梁，展示了在空间AI应用中的广阔潜力。
...
TL;DR: 提出一种结合点线特征的立体视觉惯性里程计系统，使用基于熵正则化最优传输的深度学习描述子进行线段匹配，在低纹理和光照变化场景中提高鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在低纹理场景和剧烈光照变化下，传统点特征变得稀疏不稳定，导致关联模糊和估计不足。现有基于点引导的线段关联方法在点特征支持弱时会失效并产生偏差约束。

Method: 1) 为线段配备专门的深度学习描述子，通过采样和池化网络特征图计算；2) 使用熵正则化最优传输公式进行线段匹配，处理模糊性、异常值和部分观测；3) 分析线段测量噪声影响，引入可靠性自适应加权来调节优化过程中线段约束的影响。

Result: 在EuRoC和UMA-VI数据集上的实验表明，相比代表性基线方法，该系统在低纹理和光照挑战环境中提高了精度和鲁棒性，同时保持实时性能。

Conclusion: 提出的点线VIO系统通过深度学习描述子和熵正则化最优传输的线段匹配，有效解决了低纹理和光照变化场景中的鲁棒性问题，实现了更稳定准确的位姿估计。
...
TL;DR: KV-Lock：一种针对DiT视频扩散模型的免训练框架，通过动态调度背景KV融合比和CFG尺度，在保持背景一致性的同时提升前景生成质量。
...
Method: 提出KV-Lock框架，基于幻觉检测动态调度两个关键组件：1）缓存背景KV与新生成KV的融合比例；2）CFG尺度。当检测到幻觉风险时，加强背景KV锁定并增强条件引导。
...
TL;DR: DiffWind是一个基于物理的可微分框架，用于从视频中建模风驱动物体动力学，统一了风-物体交互建模、视频重建和前向模拟，显著优于现有方法。
...
Method: 1) 将风表示为基于网格的物理场，物体表示为基于3D高斯泼溅的粒子系统；2) 使用物质点法(MPM)建模风-物体交互；3) 通过可微分渲染和模拟联合优化时空风力场和物体运动；4) 引入格子玻尔兹曼方法(LBM)作为物理约束，确保符合流体动力学定律。

Result: 方法在重建精度和模拟保真度上显著优于先前的动态场景建模方法，支持新条件下的前向模拟和风重定向等新应用，并引入了WD-Objects合成和真实世界风驱动场景数据集。

Conclusion: DiffWind为基于视频的风-物体交互建模开辟了新途径，通过物理约束的可微分框架实现了准确的风驱动物体动力学重建和模拟。
...
TL;DR: VarSplat是一种不确定性感知的3D高斯溅射SLAM系统，通过学习每个溅射的外观方差来显式处理测量可靠性，提高在低纹理区域、透明表面和复杂反射特性场景中的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS-SLAM方法隐式处理测量可靠性，导致姿态估计和全局对齐在低纹理区域、透明表面或复杂反射特性区域容易产生漂移。需要显式的不确定性建模来提高SLAM系统的鲁棒性。

Method: VarSplat通过学习每个溅射的外观方差，利用总方差定律和alpha合成，通过高效的单通道光栅化渲染可微分的每像素不确定性图。该图指导跟踪、子图配准和环路检测关注可靠区域。

Result: 在Replica（合成）和TUM-RGBD、ScanNet、ScanNet++（真实世界）数据集上的实验表明，VarSplat提高了鲁棒性，在跟踪、建图和新颖视图合成渲染方面达到或优于现有密集RGB-D SLAM研究。

Conclusion: VarSplat通过显式学习每个溅射的外观方差并渲染不确定性图，有效提高了3DGS-SLAM系统在具有挑战性场景中的鲁棒性和性能，为密集RGB-D SLAM提供了更可靠的解决方案。
...
TL;DR: FootMR是一种脚部运动精炼方法，通过将2D脚部关键点序列提升到3D来改进现有人体恢复模型的脚部运动估计，在复杂脚部运动重建上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前3D人体运动恢复方法在捕捉精细关节（特别是脚部）方面存在不足，这限制了步态分析和动画等应用。主要原因是训练数据集中脚部标注不准确且脚部运动多样性有限。

Method: FootMR通过将2D脚部关键点序列提升到3D来精炼现有模型的脚部运动估计。为避免图像-3D标注对的不准确性，不直接使用图像输入，而是利用大规模运动捕捉数据。为消除2D到3D提升的歧义性，结合膝盖和脚部运动作为上下文，仅预测脚部运动的残差。通过全局关节表示和广泛数据增强提高对极端脚部姿态的泛化能力。

Result: 在MOOF、MOYO和RICH数据集上的实验表明，FootMR优于现有最先进方法，在MOYO数据集上将踝关节角度误差降低了30%（相比最佳视频方法）。

Conclusion: FootMR通过2D到3D提升方法有效解决了脚部运动恢复的精度问题，避免了图像标注的不准确性，在复杂脚部运动重建上取得了显著改进。
...
TL;DR: 该研究探索基于Transformer的越南语视觉问答系统，结合文本和视觉预训练，并系统比较多语言设置下的自动评估指标。


<details>
  <summary>Details</summary>
Motivation: 越南语VQA研究需要结合越南语语言理解（如PhoBERT）和视觉表示学习（如ViT）的优势，同时需要系统评估不同自动评估指标在多语言环境下的有效性。

Method: 使用基于Transformer的架构，结合文本（PhoBERT）和视觉（ViT）预训练模型，在越南语VQA数据集（ViVQA、OpenViVQA、ViTextVQA）上进行实验，并系统比较BLEU、METEOR、CIDEr、Recall、Precision、F1-score等评估指标。

Result: 研究展示了Transformer架构在越南语VQA任务上的应用效果，比较了不同评估指标在多语言环境下的表现，并探讨了大语言模型在改善自动评估与人工判断对齐方面的潜力。

Conclusion: 基于Transformer的多模态融合方法在越南语VQA任务中具有良好前景，系统评估指标比较为低资源多语言VQA研究提供了重要参考，大语言模型可能进一步提升评估质量。
...
TL;DR: TemporalDoRA：一种视频专用的参数高效微调方法，通过在低秩瓶颈中插入轻量级时序多头注意力，提升手术视频问答的时序建模能力和语言变化鲁棒性。
...
Method: TemporalDoRA扩展了权重分解低秩适应方法：1）在视觉编码器的低秩瓶颈中插入轻量级时序多头注意力；2）仅对可训练的低秩分支应用权重分解而非完整适配权重。该方法在保持冻结主干网络的同时实现时序感知更新。

Result: 在REAL-Colon-VQA数据集上，TemporalDoRA提升了模板外问题的性能；消融研究表明低秩分支内的时序混合是性能提升的主要驱动力。在EndoVis18-VQA数据集上也观察到一致的改进。

Conclusion: TemporalDoRA通过低秩适应子空间内的时序信息混合，引导更新朝向时序一致的视觉线索，以最小的参数开销提高了手术视频问答的鲁棒性和性能。
...
TL;DR: TriFusionSR：基于小波引导的条件扩散框架，用于联合三模态医学图像融合与超分辨率，通过频率感知的跨模态交互显著提升图像质量


<details>
  <summary>Details</summary>
Motivation: 多模态医学图像融合有助于综合诊断，但现有方法通常将图像融合和超分辨率分开处理，导致伪影和感知质量下降。在三模态设置中，解剖模态（MRI、CT）与功能扫描（PET、SPECT）结合时，频率域不平衡问题更加突出。

Method: 提出TriFusionSR框架：1）使用2D离散小波变换将多模态特征分解为频带，实现频率感知的跨模态交互；2）引入修正小波特征策略进行潜在系数校准；3）采用带门控通道-空间注意力的自适应空间-频率融合模块进行结构驱动的多模态细化。
...
TL;DR: ProGS：一种基于八叉树结构的渐进式3D高斯泼溅压缩方法，实现45倍存储压缩和10%以上视觉质量提升
...
Method: 提出ProGS方法，将3DGS数据组织成八叉树结构，实现渐进式编码。通过互信息增强机制减少结构冗余，利用八叉树层次中节点间的相关性。通过调整八叉树结构和动态锚节点，实现可扩展的数据压缩。

Result: 相比原始3DGS格式，ProGS实现了45倍的存储压缩，同时视觉性能提升超过10%。该方法为不同网络条件下的实时应用提供了稳健解决方案。

Conclusion: ProGS是一种流媒体友好的编解码器，能够有效支持3D高斯泼溅的渐进式编码，显著提高压缩效率和视觉保真度，解决了现有方法在渐进式编码方面的局限性。
...
TL;DR: GSStream是一个用于3D高斯泼溅(3DGS)体素场景的实时流式传输系统，通过协作视口预测和DRL比特率自适应来优化带宽使用和视觉质量。


<details>
  <summary>Details</summary>
Motivation: 3DGS技术虽然实现了实时辐射场渲染，但产生了大量数据，对带宽要求极高。现有方法难以实现实时分发，需要更高效的流式传输系统。

Method: 1) 协作视口预测模块：结合多用户协作先验和用户历史视口序列预测未来行为；2) 基于深度强化学习的比特率自适应模块：解决状态和动作空间可变性挑战；3) 构建首个体素场景用户视口轨迹数据集。

Result: 实验证明GSStream在视觉质量和网络使用方面优于现有代表性体素场景流式传输系统。

Conclusion: GSStream通过创新的协作预测和DRL自适应机制，有效解决了3DGS数据实时流式传输的带宽挑战，实现了高效的体素场景分发。
...
TL;DR: 提出Matrix Attention机制，通过帧级矩阵操作处理视频，解决3D注意力效率与局部注意力性能的权衡，构建FrameDiT模型实现高效高质量视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型面临效率与性能的权衡：全3D注意力效果好但计算昂贵，局部因子化注意力高效但时间建模能力有限。需要一种既能保持全局时空结构又能高效处理显著运动的机制。

Method: 提出Matrix Attention机制，将整个帧作为矩阵处理，通过矩阵原生操作生成查询、键、值矩阵，实现帧级时间注意力。基于此构建FrameDiT-G架构，并进一步开发FrameDiT-H，结合Matrix Attention与Local Factorized Attention以同时捕捉大运动和小运动。

Result: FrameDiT-H在多个视频生成基准测试中取得最先进结果，提供改进的时间一致性和视频质量，同时保持与Local Factorized Attention相当的效率。

Conclusion: Matrix Attention机制有效解决了视频扩散模型中全局时空建模与计算效率的权衡问题，FrameDiT-H模型在保持高效的同时实现了高质量的时空一致性视频生成。
...
TL;DR: 提出EXPLORE-Bench基准，评估多模态大语言模型在自我中心视角下对长时序动作物理后果的推理能力，发现现有模型与人类存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型被视为具身智能体的基础，但尚不清楚它们能否从自我中心视角可靠地推理动作的长期物理后果。需要系统评估模型在这方面的能力。

Method: 提出新任务Egocentric Scene Prediction with LOng-horizon REasoning：给定初始场景图像和一系列原子动作描述，预测所有动作执行后的最终场景。构建EXPLORE-Bench基准，包含真实第一人称视频数据，提供结构化最终场景标注（物体类别、视觉属性、物体间关系）。

Result: 实验表明，现有专有和开源MLLMs与人类表现存在显著性能差距，长时序自我中心推理仍是主要挑战。分步推理能一定程度上提升性能，但带来显著计算开销。

Conclusion: EXPLORE-Bench为衡量和推进自我中心具身感知的长时序推理提供了原则性测试平台，揭示了当前模型的局限性。
...
Method: 提出FetalAgents多智能体系统，采用轻量级智能体协调框架，动态编排专业视觉专家。系统支持端到端视频流总结：自动识别多解剖平面的关键帧，通过协调专家进行分析，并结合患者元数据合成结构化临床报告。

Result: 在八个临床任务的多中心外部评估中，FetalAgents相比专业模型和多模态大语言模型(MLLMs)始终提供最稳健和准确的性能，为胎儿超声分析和报告提供了可审计、工作流对齐的解决方案。

Conclusion: FetalAgents是首个用于全面胎儿超声分析的多智能体系统，通过智能体协调框架实现了诊断、测量和分割任务的最佳性能平衡，并支持端到端视频流总结和结构化报告生成，显著提升了临床工作流程的效率和准确性。
...
TL;DR: M²-Occ：一种用于自动驾驶的语义占据预测框架，专门处理不完整多相机输入，通过多视图掩码重建和特征记忆模块保持几何结构和语义一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于相机的语义占据预测方法通常假设完整的环视观测，但在实际部署中常因遮挡、硬件故障或通信故障导致观测不完整。需要解决在不完整多相机输入下的语义占据预测问题。

Method: 提出M²-Occ框架，包含两个互补模块：1）多视图掩码重建（MMR）模块，利用相邻相机间的空间重叠在特征空间直接恢复缺失视图表示；2）特征记忆模块（FMM），引入可学习的记忆库存储类别级语义原型，通过检索和集成这些全局先验来细化模糊体素特征。

Result: 在nuScenes-based SurroundOcc基准测试中，引入系统性的缺失视图评估协议。在安全关键的后视图缺失设置下，IoU提升4.93%；随着缺失相机数量增加，鲁棒性差距进一步扩大，例如在五个视图缺失时IoU提升5.01%。这些提升不影响完整视图性能。

Conclusion: M²-Occ有效解决了不完整多相机输入下的语义占据预测问题，通过特征空间重建和全局语义先验集成，显著提升了在视图缺失情况下的几何结构和语义一致性保持能力。
...
<details>
  <summary>Details</summary>
Motivation: 目前缺乏在真实工业场景中同时捕获自我中心视角和外部视角的数据集，这阻碍了开发支持工人安全和效率的系统。

Method: 在真实工业场景中采集360个同步的自我中心和外部视角视频，并进行时空标注，创建ENIGMA-360数据集。

Result: 提供了三个基础任务的基准实验：时序动作分割、关键步骤识别和自我中心人机交互检测，展示了现有方法在这个挑战性场景中的局限性。

Conclusion: 需要开发能够稳健理解真实世界环境中ego-exo关系的新模型，数据集已公开发布供研究使用。
...
TL;DR: LAP使用语言感知规划，通过VLM将视觉观察转为文本描述，利用文本嵌入在扩散模型中规划动作序列，在多个基准测试中取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖视觉观察，但不同动作可能视觉相似，存在固有模糊性。语言描述在潜在空间中提供更具区分性的表示，可以更好地连接视觉观察与规划
...
Conclusion: 语言感知规划通过利用语言的表达能力来桥接视觉观察和规划，显著提升了程序规划的性能，证明了语言表示在解决视觉模糊性方面的优势
...
Method: 1) 将目标字符作为图像输入而非文本提示；2) 分析联合注意力机制识别核心token；3) 注入信息最丰富的注意力图来整合字符结构和视觉设计；4) 通过层间注意力图聚合减轻注意力偏移

Result: 大量实验和用户研究表明，该方法在多语言Logo生成方面达到最先进性能

Conclusion: LogoDiffuser提供了一种无需训练的方法，能够生成保持字符结构完整性的多语言Logo设计，解决了现有方法的局限性
...
TL;DR: 提出全景室内环境中的整体可供性接地任务，开发PanoAffordanceNet框架，包含失真感知谱调制器和全球形密集化头，构建首个高质量全景可供性接地数据集360-AGD，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前可供性接地主要关注对象中心化且局限于透视视图，而360°空间中的具身智能体需要全局感知能力，因此需要解决全景环境中的整体可供性接地问题。

Method: 提出PanoAffordanceNet端到端框架，包含失真感知谱调制器（DASM）进行纬度相关校准，全球形密集化头（OSDH）从稀疏激活恢复拓扑连续性，集成像素级、分布级和区域-文本对比目标的多级约束。

Result: 构建首个高质量全景可供性接地数据集360-AGD，实验表明PanoAffordanceNet显著优于现有方法，为具身智能中的场景级感知建立了坚实基础。

Conclusion: 该研究填补了全景环境中整体可供性接地的空白，提出的框架有效解决了ERP几何失真、语义分散和跨尺度对齐等挑战，在低监督条件下抑制语义漂移，推动了具身智能的场景级感知发展。
...
TL;DR: 提出一种基于模型内部注意力机制的高效个性化方法，通过提取代表目标概念的视觉token作为记忆，实现个性化描述而无需额外训练


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型的个性化方法要么依赖额外训练阶段（限制通用性和可扩展性），要么使用外部预训练模块的工程化流程（影响部署效率），需要更高效的个性化方案

Method: 利用模型固有的注意力机制提取主要代表目标概念的视觉token，将这些token作为特定概念的记忆，使模型能够在测试图像中识别并描述该概念

Result: 在单概念、多概念和视频个性化等多种设置下进行综合评估，相比SOTA方法展现出显著性能提升，同时保持最小的个性化开销

Conclusion: 提出的方法通过利用模型内部机制实现高效个性化，避免了额外训练或复杂工程化流程，为AI助手提供可扩展的个性化解决方案
...
Method: 通过对比干净和触发表示来估计特征空间中的替代触发器后门方向，开发特征引导攻击联合优化目标预测和方向对齐，并从理论上证明替代触发器的存在性。

Result: 理论上证明替代触发器存在且是后门训练的必然结果，实证验证替代触发器能可靠激活后门，移除训练触发器的防御通常无法消除后门。

Conclusion: 研究结果表明需要开发针对表示空间后门方向的防御，而非仅关注输入空间触发器，这为后门防御提供了新的研究方向。
...
TL;DR: 该论文提出XAI方法应关注"编码缺失"概念——即概念缺失导致神经元激活增强的因果关系，现有主流方法难以揭示此类关系，作者提出了两种简单扩展方法来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 现有可解释人工智能方法主要关注概念存在与神经元激活的正向关系，但忽视了"编码缺失"这种重要因果模式，即概念缺失反而增强神经元激活。这种被忽视的关系在实际模型中普遍存在且重要。

Method: 提出了两种简单扩展：1) 对归因方法的扩展，使其能识别导致概念缺失的输入特征；2) 对特征可视化方法的扩展，使其能揭示导致神经元激活的概念缺失模式。这些扩展基于对现有主流XAI方法的改进。

Result: 实验表明：1) 扩展后的XAI方法能有效揭示编码缺失关系；2) ImageNet模型确实利用了编码缺失机制；3) 考虑编码缺失能改进去偏任务的效果。

Conclusion: 编码缺失是深度神经网络中普遍存在的重要因果模式，现有XAI方法需要扩展以揭示此类关系，考虑编码缺失能提升模型解释的完整性和去偏效果。
...
TL;DR: 提出首个测试时自我-他者视角适应任务(TE²A³)，设计DCPGN网络通过多标签原型增长和双线索一致性模块，实现在测试时无需目标视角训练数据的情况下进行视角适应和动作预测。


<details>
  <summary>Details</summary>
Motivation: 现有自我-他者视角适应方法严重依赖目标视角数据进行训练，增加了计算和数据收集成本。需要探索在测试时进行在线适应的新范式，以降低实际应用中的资源需求。

Method: 提出双线索增强原型增长网络(DCPGN)：1) 多标签原型增长模块(ML-PGM)，通过多标签分配和置信度重加权平衡多个正类，使用熵优先级队列策略更新类记忆库；2) 双线索一致性模块(DCCM)，引入轻量级叙述器生成表示动作进展的文本线索，补充包含各种物体的视觉线索，通过约束推断的文本和视觉logits构建双线索一致性。

Result: 在新提出的EgoMe-anti和现有EgoExoLearn基准测试上进行了广泛实验，方法显著优于相关最先进方法，证明了在测试时进行自我-他者视角适应的有效性。

Conclusion: 首次探索了测试时自我-他者视角适应任务，提出的DCPGN方法通过多标签知识积累和跨模态线索整合，有效解决了多动作候选和显著时空视角差距的挑战，为实际应用提供了更高效的解决方案。
...
TL;DR: 论文提出了一种细粒度的音频-视觉学习任务RA-SSU，构建了两个数据集f-Music和f-Lifescene，并提出了SSUFormer模型，在声音源分割和描述方面取得SOTA性能。
...
Method: 1) 新定义了RA-SSU任务，实现区域感知、帧级别、高质量的声音源理解；2) 构建了两个细粒度数据集f-Music和f-Lifescene；3) 提出了SSUFormer模型，包含Mask Collaboration Module和Mixture of Hierarchical-prompted Experts两个模块。

Result: 在两个新构建的数据集上进行了广泛实验，验证了任务的可行性、数据集的有效性，并证明了SSUFormer在声音源理解基准测试中达到了最先进的性能。

Conclusion: 论文成功定义了细粒度的音频-视觉学习任务RA-SSU，提供了高质量的数据集和有效的SSUFormer模型，为场景理解和交互提供了更精细的感知能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：基于回归的方法无法重建未见区域，而相机引导的扩散模型由于噪声点云投影或相机姿态条件不足，容易偏离预定轨迹。需要一种能在遵循指定相机姿态的同时完成未见区域重建的方法。

Method: 提出ConfCtrl框架：1）初始化扩散过程时结合置信度加权的投影点云潜变量和噪声作为条件输入；2）采用卡尔曼启发的预测-更新机制，将投影点云视为噪声测量，使用学习到的残差校正来平衡姿态驱动预测与噪声几何观测；3）让模型依赖可靠投影，同时降低不确定区域的权重。

Result: 在多个数据集上的实验表明，ConfCtrl能生成几何一致且视觉合理的新视角，在大视角变化下有效重建遮挡区域。

Conclusion: ConfCtrl通过置信感知的视频插值框架，成功解决了仅有两张输入图像且视角变化大的新视角合成问题，实现了稳定、几何感知的生成效果。
...
TL;DR: BrainSTR是一个时空对比学习框架，用于可解释的动态脑网络建模，通过自适应相位划分、注意力机制和图结构生成器来识别疾病相关的关键时空模式，在ASD、BD和MDD等神经精神疾病诊断中表现出色。


<details>
  <summary>Details</summary>
Motivation: 动态功能连接性能够捕捉时变脑状态，有助于神经精神疾病的诊断和时空可解释性。然而，可靠的解释性面临重大挑战：诊断信号通常微弱且稀疏分布在时间和拓扑中，而干扰波动和非诊断性连接普遍存在。

Method: 提出BrainSTR框架，包含：1) 数据驱动的自适应相位划分模块学习状态一致的相位边界；2) 注意力机制识别诊断关键相位；3) 增量图结构生成器通过二值化、时间平滑性和稀疏性正则化提取每个相位中的疾病相关连接；4) 时空监督对比学习方法利用诊断相关的时空模式来优化样本相似性度量。

Result: 在自闭症谱系障碍(ASD)、双相情感障碍(BD)和重度抑郁症(MDD)上的实验验证了BrainSTR的有效性，发现的关键相位和子网络提供了与先前神经影像学发现一致的可解释证据。

Conclusion: BrainSTR通过时空对比学习框架成功解决了动态脑网络建模中的可解释性挑战，能够识别诊断相关的关键时空模式，为神经精神疾病的诊断提供了可靠且可解释的工具。
...
TL;DR: VLM-Loc是一个利用大型视觉语言模型进行文本到点云定位的框架，通过将点云转换为BEV图像和场景图，结合部分节点分配机制，实现精确的空间定位。


<details>
  <summary>Details</summary>
Motivation: 现有文本到点云定位方法主要依赖浅层的文本-点云对应关系，缺乏有效的空间推理能力，在复杂环境中定位精度有限。需要利用大型视觉语言模型的空间推理能力来提升定位准确性。

Method: 将点云转换为鸟瞰图图像和场景图，共同编码几何和语义信息；利用VLM学习跨模态表示；引入部分节点分配机制，显式关联文本线索与场景图节点，实现可解释的空间推理。

Result: 在CityLoc基准测试中，VLM-Loc相比现有最先进方法展现出更高的准确性和鲁棒性。同时发布了代码、模型和数据集。

Conclusion: VLM-Loc通过利用大型视觉语言模型的空间推理能力和结构化场景表示，显著提升了文本到点云定位的精度，为复杂环境中的空间理解提供了有效解决方案。
...
TL;DR: 提出多智能体第一人称视频理解新问题，建立MA-EgoQA基准，包含1.7k个多视角问题，并提出EgoMAS基线模型，发现现有方法难以有效处理多智能体第一人称视频流。


<details>
  <summary>Details</summary>
Motivation: 随着具身智能体发展，人类未来将与多个具身AI智能体协作。需要解决多智能体系统与人类用户间的有效通信，包括并行处理多智能体感知输入和正确聚合多个第一人称视频以构建系统级记忆。

Method: 1. 形式化定义多智能体长时程第一人称视频理解问题；2. 提出MA-EgoQA基准，包含5类1.7k个多视角问题；3. 提出EgoMAS基线模型，利用智能体间共享记忆和智能体级动态检索机制。

Result: 通过MA-EgoQA对多种基线模型和EgoMAS的评估发现，现有方法无法有效处理多个第一人称视频流，突显了在跨智能体系统级理解方面需要未来进展。

Conclusion: 多智能体第一人称视频理解是一个重要且具有挑战性的新问题，MA-EgoQA为研究提供了系统评估基准，EgoMAS展示了共享记忆和动态检索的潜力，但现有技术仍需显著改进。
...
TL;DR: MissBench是一个多模态情感计算基准测试框架，针对现实应用中模态缺失不平衡的问题，提出了两种诊断指标来评估模型公平性和优化平衡性。


<details>
  <summary>Details</summary>
Motivation: 现实应用中，文本、声学和视觉模态的可用性通常不平衡（某些模态更脆弱或更昂贵），而标准评估假设所有模态同等可用，这导致训练偏差和评估不准确。

Method: 提出MissBench基准框架，在四个广泛使用的情感数据集上标准化共享和失衡缺失率协议；定义两种诊断指标：模态公平指数（MEI）衡量不同模态在不同缺失配置下的公平贡献，模态学习指数（MLI）通过比较训练中模态特定梯度范数来量化优化不平衡。

Result: 实验表明，在共享缺失率下表现稳健的模型，在失衡条件下仍可能表现出明显的模态不公平和优化不平衡。MissBench与MEI、MLI成为在实际不完整模态设置中压力测试和分析多模态情感模型的实用工具。

Conclusion: MissBench为解决多模态情感计算中模态缺失不平衡问题提供了标准化评估框架和诊断工具，有助于更真实地评估模型在实际应用中的性能。
...
TL;DR: InternVL-U是一个轻量级4B参数统一多模态模型，通过统一上下文建模和模态特定模块化设计，在保持强大语义理解的同时获得生成能力，在生成任务上超越更大规模模型。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型在保持语义理解和获得生成能力之间存在固有权衡，需要解决这一矛盾，同时降低模型规模以实现更广泛的应用。

Method: 采用统一上下文建模和模态特定模块化设计，结合解耦视觉表示，将最先进的多模态大语言模型与MMDiT视觉生成头集成。构建针对高语义密度任务的数据合成流程，使用思维链方法对齐用户意图与视觉生成细节。

Result: InternVL-U仅用4B参数，在多种生成和编辑任务上持续超越BAGEL(14B)等更大规模统一基线模型，同时保持强大的多模态理解和推理能力，实现了优越的性能-效率平衡。

Conclusion: InternVL-U通过创新的架构设计和数据策略，成功解决了统一多模态模型中语义理解与生成能力之间的权衡问题，为轻量级高性能统一多模态模型的发展提供了有效方案。
...
TL;DR: DISPLAY：基于稀疏运动引导（手腕关节坐标和形状无关物体边界框）的人类-物体交互视频生成框架，通过对象强调注意力和多任务辅助训练策略实现高质量可控生成。


<details>
  <summary>Details</summary>
Motivation: 现有人类中心视频生成方法难以产生可控且物理一致的人类-物体交互视频，依赖密集控制信号、模板视频或精心设计的文本提示，限制了灵活性和对新物体的泛化能力。

Method: 1）稀疏运动引导：仅使用手腕关节坐标和形状无关物体边界框；2）对象强调注意力机制：增强稀疏条件下的物体鲁棒性；3）多任务辅助训练策略：结合高质量HOI数据和辅助任务的数据整理流程。

Result: 综合实验表明，该方法在多样化任务中实现了高保真度、可控的人类-物体交互视频生成，优于现有方法。

Conclusion: DISPLAY框架通过稀疏运动引导和创新的训练策略，解决了人类-物体交互视频生成中的可控性和物理一致性问题，为直观用户控制提供了有效解决方案。
...
TL;DR: CourtSI是首个针对体育场景的大规模空间智能数据集，包含超过100万问答对，涵盖羽毛球、网球、乒乓球等运动，用于评估和提升视觉语言模型的空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 体育作为测试人类身体和认知极限的领域，为评估视觉语言模型的空间智能提供了天然测试平台。现有空间智能基准在体育场景中存在局限性，需要专门的数据集来推动模型在动态运动理解方面的发展。

Method: 1) 构建CourtSI数据集：利用定义明确的球场几何作为度量锚点，开发半自动数据引擎重建体育场景，系统化覆盖空间计数、距离测量、定位和关系推理；2) 创建CourtSI-Bench评估基准：包含3,686个经过人工验证的高质量问答对；3) 在Qwen3-VL-8B模型上进行微调实验。

Result: 1) 评估25个专有和开源VLM显示存在人机性能差距，且现有空间智能基准泛化能力有限；2) 在CourtSI上微调Qwen3-VL-8B使CourtSI-Bench准确率提升23.5个百分点；3) 微调后的模型能有效泛化到类似但未见过的运动（CourtSI-Ext），并展现出增强的空间感知评论生成能力。

Conclusion: CourtSI为推进视觉语言模型在体育场景中的空间智能提供了可扩展的途径，体育场景暴露了现有基准在空间智能能力捕捉方面的局限性，专门的数据集能显著提升模型性能并促进泛化能力。
...
TL;DR: WikiCLIP提出了一种简单高效的开放域视觉实体识别框架，通过对比学习范式显著提升性能并大幅降低计算成本
...
Result: 在OVEN等基准测试中显著优于现有方法，在挑战性的OVEN未见集上提升16%，同时推理延迟比领先的生成模型AutoVER降低近100倍

Conclusion: WikiCLIP为开放域视觉实体识别建立了一个强大而高效的基线，证明了对比学习范式在该任务上的优越性和实用性
...
TL;DR: Chamfer距离优化存在梯度结构性问题，会导致多对一坍塌，而非局部耦合是抑制坍塌的必要条件
...
Method: 分析Chamfer距离的梯度结构，发现每点梯度导致多对一坍塌；推导坍塌抑制的必要条件（非局部耦合）；在2D设置中使用共享基变形提供全局耦合，在3D形状变形中使用可微分MPM先验

Result: 非局部耦合能有效抑制坍塌：在20个定向对中一致减少Chamfer差距，在拓扑复杂的龙模型上实现2.5倍改进

Conclusion: Chamfer优化成功与否取决于是否存在非局部耦合，这为优化点级距离度量的任何流水线提供了实用的设计标准
...
TL;DR: 提出基于关节角度的可解释运动表示方法，通过伪图像映射和Vision Transformer实现细粒度文本-运动对齐，在检索性能和可解释性上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有文本-运动检索方法主要使用双编码器框架，将运动和文本压缩为全局嵌入，丢弃了细粒度的局部对应关系，降低了检索精度，且结果可解释性有限。

Method: 1) 提出基于关节角度的可解释运动表示，将关节级局部特征映射为结构化伪图像，兼容预训练Vision Transformer；2) 文本到运动检索使用MaxSim令牌级延迟交互机制，并通过掩码语言建模正则化增强，促进鲁棒可解释的文本-运动对齐。

Result: 在HumanML3D和KIT-ML数据集上的大量实验表明，该方法在文本-运动检索性能上优于最先进方法，同时提供文本和运动之间的可解释细粒度对应关系。

Conclusion: 提出的方法不仅提升了文本-运动检索的准确性，还通过细粒度的局部对应关系增强了结果的可解释性，为跨模态检索提供了更有效的解决方案。
...
Method: 采用自适应临床感知扩散模型，通过渐进去噪学习不完全多模态观察与目标模态之间的映射，结合自适应融合机制和GPT-4o编码的临床语义指导，使用三个专用生成器实现sMRI、FDG-PET和AV45-PET之间的双向合成。

Result: 在ADNI数据集上评估，ACADiff实现了优越的生成质量，即使在80%极端缺失情况下仍保持稳健的诊断性能，优于所有现有基线方法。

Conclusion: ACADiff框架能够有效合成缺失的多模态脑成像数据，提升阿尔茨海默病诊断的鲁棒性，代码已开源以促进可重复性研究。
...
TL;DR: 提出基于Margin Disparity Discrepancy的新型无监督域适应框架，用于将CT标注数据迁移到CBCT，提升肝脏分割性能。


<details>
  <summary>Details</summary>
Motivation: 介入放射学中的CBCT图像缺乏公开标注数据集，而CT有丰富标注数据。CBCT与CT存在模态差异（有限视野、特定伪影、动脉内造影剂），需要解决域适应问题。

Method: 提出基于Margin Disparity Discrepancy（MDD）的新型无监督域适应框架，通过重新制定原始MDD优化框架来缩小源域（CT）和目标域（CBCT）之间的分布差异。

Result: 在CT和CBCT肝脏分割数据集上，该方法在无监督域适应和少样本设置下均达到最先进性能。

Conclusion: 提出的MDD改进框架能有效利用未标注的CBCT数据和标注的CT数据，成功解决介入放射学中CBCT数据稀缺问题，提升肝脏分割性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统心脏磁共振（CMR）流程采用"先重建后分析"的序列范式，存在两个主要问题：1）需要从欠采样的k空间恢复高维像素阵列（图像），这是一个病态中间步骤，会引入可避免的伪影和信息瓶颈；2）诊断实际需要的是低维生理标签而非完整图像，这造成了数学上的根本矛盾。

Method: 提出k-MTR（k空间多任务表示）框架：1）将欠采样k空间数据与全采样图像对齐到共享语义流形；2）利用大规模受控模拟（42,000名受试者）训练k空间编码器，直接在潜在空间中恢复因欠采样丢失的解剖信息；3）绕过显式逆问题，为下游分析提供密集潜在空间嵌入高层次的生理语义。

Result: 在连续表型回归、疾病分类和解剖分割三个任务上，k-MTR均取得了与最先进的图像域基线方法相竞争的性能。实验表明，精确的空间几何结构和多任务特征可以直接从k空间表示中成功恢复。

Conclusion: k-MTR展示了直接从k空间表示中恢复精确空间几何和多任务特征的可行性，为任务感知的心脏MRI工作流程提供了稳健的架构蓝图，解锁了k空间的直接诊断潜力。
...
TL;DR: 该论文提出基于专家与非专家病理学家分歧的Whole Slide Difficulty (WSD)概念，并通过多任务和加权分类损失两种方法将其应用于前列腺癌Gleason分级，显著提升了分类性能。


<details>
  <summary>Details</summary>
Motivation: 组织病理学中，全玻片图像(WSI)的诊断通常由专家病理学家标注，但非专家诊断困难且存在分歧。为解决这一问题，作者提出基于专家与非专家分歧的Whole Slide Difficulty (WSD)概念，旨在利用难度信息提升MIL模型的分类性能。

Method: 提出两种利用WSD的方法：1) 多任务方法，同时预测分类标签和难度分数；2) 加权分类损失方法，根据难度调整损失权重。这些方法应用于前列腺癌Gleason分级任务，结合不同的特征编码器和MIL方法进行验证。

Result: 实验结果表明，在训练中整合WSD能够持续提升不同特征编码器和MIL方法的分类性能，特别是对于更高Gleason等级（即更差的诊断结果）的提升效果更为显著。

Conclusion: 基于专家与非专家分歧的Whole Slide Difficulty (WSD)概念是有效的，通过多任务和加权分类损失两种方法整合WSD能够显著提升组织病理学图像分类性能，尤其对于困难病例的诊断具有重要价值。
...
TL;DR: C2FMAE是一个从粗到细的掩码自编码器，通过分层学习语义、实例和像素三个粒度的视觉表示，解决了对比学习和掩码图像建模之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 自监督视觉预训练方法存在内在矛盾：对比学习能捕获全局语义但丢失细粒度细节，而掩码图像建模能保留局部纹理但会因语义无关的随机掩码导致"注意力漂移"问题。

Method: 提出C2FMAE框架，包含两个协同创新：1) 级联解码器按顺序从场景语义到对象实例再到像素细节进行重建，建立明确的跨粒度依赖关系；2) 渐进掩码课程动态将训练焦点从语义引导掩码转向实例引导掩码，最后到随机掩码。构建了包含128万ImageNet-1K图像的大规模多粒度数据集。

Result: 在图像分类、目标检测和语义分割任务上取得了显著的性能提升，验证了分层设计在学习更鲁棒和可泛化表示方面的有效性。

Conclusion: C2FMAE通过显式学习分层视觉表示，成功解决了自监督预训练中全局语义与局部细节之间的权衡问题，为学习更鲁棒的视觉表示提供了新框架。
...
<details>
  <summary>Details</summary>
Motivation: 在线新颖视角合成面临挑战，需要从连续且通常无姿态的观察中进行鲁棒的场景重建。现有方法在使用预测姿态进行推理时会出现训练与推理的分布不匹配问题。

Method: 提出ReCoSplat模型：1）使用Render-and-Compare模块，从预测视角渲染当前重建并与输入观察比较，提供稳定的条件信号以补偿姿态误差；2）采用混合KV缓存压缩策略，结合早期层截断和块级选择性保留，将100+帧的KV缓存减少90%以上。

Result: ReCoSplat在不同输入设置下（有姿态/无姿态、有/无相机内参）的分布内和分布外基准测试中都达到了最先进的性能。

Conclusion: ReCoSplat通过Render-and-Compare模块有效解决了训练与推理时的姿态分布不匹配问题，并通过高效的KV缓存压缩支持长序列处理，为在线新颖视角合成提供了鲁棒的解决方案。
