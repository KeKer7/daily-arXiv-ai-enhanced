...
TL;DR: V-Agent是一个多智能体视频搜索平台，通过微调视觉语言模型和检索向量增强，在共享多模态表示空间中处理视频帧和音频转录，实现上下文感知的视频搜索，在MultiVENT 2.0基准测试中达到最先进的零样本性能。


<details>
  <summary>Details</summary>
Motivation: 传统基于文本的检索系统在多模态场景中存在局限性，无法有效处理视频中的视觉和语音内容。需要开发能够同时理解视觉和语音内容、支持交互式对话的先进视频搜索系统。

Method: 1. 微调视觉语言模型（VLM）并使用小规模视频偏好数据集增强检索向量；2. 使用VLM检索模型将视频帧和ASR音频转录嵌入到共享多模态表示空间；3. 构建三个协作智能体：路由智能体、搜索智能体和聊天智能体；4. 搜索智能体结合VLM检索模型和重排序模块提升检索质量。

Result: 在MultiVENT 2.0基准测试中实现了最先进的零样本性能，证明了该框架在学术研究和实际应用中的潜力。

Conclusion: V-Agent通过多智能体协作和共享多模态表示空间，成功克服了传统文本检索在多模态视频搜索中的局限性，为高级视频搜索和交互式对话提供了有效的解决方案。
...
TL;DR: 本研究开发了一个使用CNN和VGG-16深度学习模型快速识别色情图像内容的系统，通过比较发现CNN模型在检测色情内容方面比VGG-16更有效，准确率达到94.87%。


<details>
  <summary>Details</summary>
Motivation: 2020年印尼政府封锁了59,741个包含色情等负面内容的网站，但公众仍可通过VPN访问这些网站，因此需要开发能够快速识别色情内容的系统。

Method: 使用深度学习中的卷积神经网络（CNN）和视觉几何组16（VGG-16）模型，对两个模型进行全面和整体的探索比较，确定哪个模型在快速检测色情内容方面最有效。

Result: 在第八次实验中，CNN模型在epoch值为50、学习率为0.001时取得了最佳测试结果，准确率达到0.9487（94.87%），表明CNN模型比VGG-16模型更有效。

Conclusion: CNN模型在快速准确检测色情内容方面比VGG-16模型更有效，可用于开发识别色情网站的系统，帮助解决通过VPN访问被封锁网站的问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型在模拟神经响应时，难以清晰分离稳定的视觉编码和条件特定的适应，这限制了它们在跨刺激和跨个体场景中的泛化能力。

Method: AVM采用结构保持框架：冻结Vision Transformer编码器捕获一致的视觉特征，同时使用独立训练的调制路径处理刺激内容和个体身份引起的神经响应变化。通过模块化子网络实现条件感知适应，不修改核心表示。

Result: 在三个实验设置（刺激水平变化、跨个体泛化、跨数据集适应）中，AVM在两个大规模小鼠V1数据集上优于最先进的V1T模型约2%的预测相关性。在跨数据集适应设置下，解释方差（FEVE）提高了9.1%。
...
TL;DR: 提出一种将Finer-CAM解释与TLS投影中的结构树特征相关联的新方法，系统评估哪些特征驱动树种分类，在7个欧洲树种上达到96%准确率，发现模型主要依赖冠层特征但不同树种有差异。


<details>
  <summary>Details</summary>
Motivation: 尽管新型传感器和深度学习方法在树种分类中取得了最先进的准确率，但其决策过程仍不明确。需要理解分类模型的决策过程，以揭示数据集和模型的局限性、偏差，并建立对模型预测的信心。

Method: 提出一种将Finer-CAM解释与TLS投影中代表结构树特征的片段相关联的新方法，系统评估哪些特征驱动树种区分。使用来自7个欧洲树种2,445棵树的TLS数据，训练和验证了五个YOLOv8模型，并进行交叉验证。

Result: 模型达到96%的平均准确率（SD=0.24%）。对630个显著性图的分析显示，模型主要依赖TLS投影中的冠层特征进行树种分类，但不同树种有差异：银桦、欧洲山毛榉、英国橡树和挪威云杉主要依赖冠层特征，而欧洲白蜡树、苏格兰松和花旗松更多地依赖树干特征。模型认为相似的树种与人类专家的判断一致。

Conclusion: 研究结果强调需要改进对树种分类模型决策过程的理解，以帮助揭示数据集和模型的局限性、偏差，并建立对模型预测的信心。提出的方法为系统评估模型决策提供了新途径。
...
TL;DR: 提出一种电影制作式的多阶段视频生成方法，通过LLM生成剧本、文本到图像模型创建角色视觉锚点，再合成各场景视频，显著提升角色一致性


<details>
  <summary>Details</summary>
Motivation: 当前文本到视频AI在生成长篇、连贯视频故事时面临角色一致性不足的挑战，需要更系统的方法来保持角色身份连续性

Method: 采用多阶段分解方法：1) LLM生成详细制作剧本；2) 文本到图像模型为每个角色创建一致视觉锚点；3) 视频生成模型基于视觉锚点单独合成每个场景

Result: 视觉锚点机制对角色一致性至关重要，移除后一致性分数从7.99骤降至0.55；同时发现当前模型在印度与西方主题生成中存在文化偏见差异

Conclusion: 多阶段分解和视觉锚点是实现角色一致视频生成的关键，同时揭示了当前模型的文化偏见问题，为未来改进提供了方向
...
<details>
  <summary>Details</summary>
Motivation: 当前视频tokenization方法采用固定压缩率，无法适应视频内容的信息密度变化，导致冗余或信息丢失。需要一种自适应方法来根据信息丰富度分配token。

Method: 基于香农信息理论，提出InfoTok框架，证明现有数据无关训练方法在表示长度上不是最优的，并提出基于证据下界(ELBO)的新算法逼近理论最优。开发基于transformer的自适应压缩器实现自适应tokenization。
...
<details>
  <summary>Details</summary>
Motivation: 内窥镜视频分割需要大量标注数据，但医学图像标注成本高且耗时。为了解决标注数据有限的问题，需要开发能够有效利用未标记数据的半监督分割方法。

Method: 提出四种策略：1) 两个独立网络之间的交叉监督；2) 基于不确定性的伪标签生成，选择高置信度区域；3) 联合伪标签监督，聚合两个网络的可靠像素；4) 特征和图像层面的相互学习。此外还使用单独的校正网络利用时空信息。

Result: 在两个临床应用（肾结石激光碎石术和结肠息肉筛查）上评估，与最先进的分割方法相比，Endo-SemiS在有限标注数据下取得了显著优越的结果。

Conclusion: Endo-SemiS通过有效利用未标记数据和多种监督策略，能够在标注数据有限的情况下实现可靠的内窥镜视频分割，在临床应用中表现出色。
...
TL;DR: LongShOTBench是一个诊断性基准测试，用于评估长视频多模态理解能力，包含开放式问题、多轮对话和多模态推理任务，并提供参考答案和评分标准。LongShOTAgent是一个通过预处理、搜索和迭代优化的代理系统。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试要么关注时间长度，要么关注多模态丰富性，但很少同时兼顾两者。大多数基准测试依赖单一准确率分数，掩盖了失败模式，需要更全面、可解释的评估方法。

Method: 通过可扩展、人工验证的流水线创建LongShOTBench基准，包含开放式意图驱动问题、单轮/多轮对话、多模态推理和工具使用任务。同时提出LongShOTAgent系统，采用预处理、搜索和迭代优化的方法分析长视频。

Result: 在LongShOTBench上，最先进的MLLMs表现不佳：Gemini-2.5-Flash达到52.95%，开源模型低于30%，LongShOTAgent达到44.66%。结果显示现实世界长视频理解的难度。

Conclusion: LongShOTBench为评估和改进MLLMs提供了实用、可复现的基础，突显了长视频多模态理解的挑战性。所有资源已在GitHub开源。
...
TL;DR: 提出4D-RGPT模型，通过P4D训练框架增强4D感知能力，并建立R4D-Bench基准测试，在4D视频问答任务上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在3D结构和时序动态推理能力有限，现有4D视频问答基准测试缺乏区域级提示和动态场景深度感知。

Method: 1) 4D-RGPT：专门设计的MLLM，从视频输入中捕获4D表示并增强时序感知；2) P4D训练框架：从冻结专家模型蒸馏4D表示到4D-RGPT；3) R4D-Bench：通过混合自动化与人工验证流程构建的深度感知动态场景基准测试。

Result: 4D-RGPT在现有4D VQA基准测试和提出的R4D-Bench基准测试上都取得了显著改进。

Conclusion: 通过4D-RGPT模型、P4D训练框架和R4D-Bench基准测试，有效提升了多模态大语言模型在4D感知和时序理解方面的能力。
...
TL;DR: FORMSpoT 是一个基于SPOT卫星时间序列的法国全国森林冠层高度制图系统，分辨率达1.5米，能检测单个树木尺度的扰动，显著优于现有产品。
...
Method: 使用SPOT-6/7年度合成数据，通过分层transformer模型（PVTv2）训练高分辨率机载激光扫描数据生成冠层高度图；开发专门的后处理流程结合配准和时空总变差去噪实现稳健变化检测。
...
<details>
  <summary>Details</summary>
Motivation: 现有相机控制视频生成方法存在两个主要问题：基于重投影的方法对深度估计误差敏感，现有数据集相机轨迹多样性有限，导致生成视频的相机姿态保真度和质量不高

Method: 1) 无限单应性扭曲：在视频扩散模型的2D潜在空间中直接编码3D相机旋转信息；2) 数据增强管道：将现有合成多视角数据集转换为具有多样轨迹和焦距的序列；3) 端到端训练预测残差视差项

Result: 实验结果表明InfCam在相机姿态准确性和视觉保真度方面优于基线方法，能够从合成数据很好地泛化到真实世界数据

Conclusion: InfCam通过深度无关的方法解决了相机控制视频生成中的关键挑战，实现了高相机姿态保真度，为动态场景的新视角视频生成提供了有效的解决方案
...
TL;DR: 提出了一种可解释的效用相似性(IUS)度量方法，用于评估合成医学图像与真实图像在深度学习临床决策支持系统中的效用相似性。


<details>
  <summary>Details</summary>
Motivation: 当前评估合成医学图像与真实图像相似性的方法主要依赖用户评估、基于Inception的度量或分类性能，缺乏可解释性，无法解释为什么一个合成数据集比另一个更有用。

Method: 受广义神经可加模型启发，提出可解释效用相似性(IUS)度量，基于临床相关图像特征解释合成数据集的效用，适用于各种医学成像模态。

Result: 在多种彩色医学成像模态（内窥镜、皮肤镜、眼底）上实验表明，使用IUS选择高效用相似性的合成图像可使分类性能相对提升高达54.6%，在灰度X光和超声模态上也验证了通用性。

Conclusion: IUS提供了一种可解释的度量方法，能够评估合成医学图像数据集在深度学习临床决策支持系统中的效用相似性，有助于选择高质量的合成数据用于模型训练。
...
TL;DR: DGH是一个动态高斯头发框架，通过数据驱动方法学习头发动态和外观，无需物理模拟，支持可微分渲染和多样化发型泛化。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖静态捕捉和物理模拟，需要手动调参处理不同发型和运动，计算量大且难以扩展。需要一种能高效学习头发动态和外观的数据驱动方法。

Method: 提出动态高斯头发框架：1) 从粗到细的模型学习跨发型的时序一致头发动态；2) 发丝引导的优化模块学习动态3D高斯表示，支持可微分渲染，实现基于梯度的运动下视角一致外观学习。

Result: DGH在几何和外观方面取得良好效果，提供可扩展的数据驱动替代方案，能无缝集成到3D高斯化身框架中，实现逼真可动画的头发。

Conclusion: DGH是完全数据驱动的框架，可扩展且能泛化到各种发型和头部运动序列，为高保真化身表示提供了现实可行的头发动画解决方案。
...
TL;DR: 该综述系统回顾了海事雷达预测建模方法，重点分析了transformer架构在时空序列预测中的应用，发现transformer已用于声纳帧预测但尚未应用于海事雷达帧预测，指出了明确的研究空白。


<details>
  <summary>Details</summary>
Motivation: 海事自主系统需要强大的预测能力来预测船舶运动和环境动态。虽然transformer架构已在AIS轨迹预测中取得突破，并在声纳帧预测中证明了可行性，但将其应用于海事雷达帧预测仍未被探索，而雷达在导航中具有全天候可靠性，这形成了一个关键的研究空白。

Method: 该综述系统性地回顾了与海事雷达相关的预测建模方法，特别关注transformer架构在时空序列预测中的应用。通过分析现有代表性方法的数据类型、架构和预测时间范围，对相关研究进行了系统梳理。

Result: 综述发现，虽然文献已展示了基于transformer的声纳传感帧预测方法，但没有任何先前工作涉及基于transformer的海事雷达帧预测。这定义了一个明确的研究空白，并为该领域的未来研究指明了具体方向。

Conclusion: 基于transformer的海事雷达帧预测是一个尚未被探索的研究领域，具有重要的实际应用价值。该综述不仅识别了这一研究空白，还为未来在该方向的研究提供了明确的动机和方向。
...
TL;DR: 提出SDUM通用框架，通过模块化设计和元数据条件化，实现单一模型在多种MRI协议上的高性能重建，展现基础模型式的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 临床MRI存在多种成像协议（不同解剖部位、对比度、采样模式、加速因子），当前深度学习重建方法通常是协议特定的，这限制了泛化能力和部署效率。

Method: 提出可扩展深度展开模型(SDUM)：结合Restormer重建器、学习型线圈灵敏度图估计器(CSME)、采样感知加权数据一致性(SWDC)、元数据通用条件化(UC)以及渐进级联扩展训练。

Result: 单一SDUM模型在CMRxRecon2025所有四个赛道（多中心、多疾病、5T、儿科）均达到SOTA，无需任务特定微调；在CMRxRecon2024上超越冠军方法+0.55dB；在fastMRI脑部数据上超越PC-RNN+1.8dB。

Conclusion: SDUM展示了基础模型式的可扩展行为（PSNR与参数对数相关），为通用、可扩展的MRI重建提供了实用路径，验证了各组件设计的有效性。
...
TL;DR: 提出一种将普通人物照片转换为专业肖像的方法，通过UV空间转换和重姿态技术，结合多图像微调来保持人物身份特征。


<details>
  <summary>Details</summary>
Motivation: 专业摄影师拍摄的人物照片通常在光线、姿态和品质上更优，而普通人自拍则缺乏这些专业元素。现有数据集中缺乏同一人物在"野外"和专业摄影环境下的配对数据，因此需要一种无需配对数据的方法来生成专业肖像。

Method: 1) 将输入照片和人脸转换到规范的UV空间，结合重姿态技术处理遮挡和新视角合成；2) 利用UV空间操作可以充分利用现有的非配对数据集；3) 通过多图像微调实现个性化输出。

Result: 该方法能够生成高质量的重姿态肖像，在真实世界图像上表现出优异的定性和定量性能。

Conclusion: 提出的方法成功解决了在缺乏配对数据的情况下，将普通人物照片转换为专业肖像的挑战，同时保持了人物的独特身份特征。
...
TL;DR: 提出一个文档中心化的背景生成框架，支持多页面编辑和主题连续性，通过潜在掩码保持文本可读性，自动优化可读性，并维持跨页面视觉一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的文档背景生成方法难以同时保证文本可读性、多页面主题连续性和设计美观性。传统方法要么会破坏文本区域，要么无法维持跨页面的视觉一致性，需要大量人工干预。

Method: 1. 潜在掩码：在扩散空间中软衰减更新，保护文本区域；2. 自动可读性优化：自动放置半透明圆角背景形状，满足WCAG 2.2对比度标准；3. 多页面一致性：通过摘要-指导过程，将每页压缩为紧凑表示来递归指导后续生成；4. 分层处理：将文档视为结构化组合，分别处理文本、图形和背景层。

Result: 该免训练框架能够生成视觉连贯、文本保留且主题对齐的文档，在保持文本可读性的同时实现美观的背景设计，并确保跨页面视觉主题的连贯演化。

Conclusion: 该框架将生成建模与自然设计工作流程相结合，通过自动化方法平衡了一致性与定制灵活性，实现了文档背景生成的实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前火灾预测方法主要局限于二值掩模建模，信号稀疏，无法捕捉火灾的复杂动态。虽然世界模型在视频生成方面有潜力，但其物理不一致性给火灾预测带来了重大挑战。需要结合物理知识来提升火灾蔓延预测的准确性和物理真实性。

Method: 提出PhysFire-WM模型：1）从物理模拟器编码结构化先验来内化燃烧动力学，纠正物理不一致性；2）采用跨任务协作训练策略（CC-Train），通过参数共享和梯度协调，整合热辐射动态和空间边界划分，缓解掩模建模中信息有限的问题。

Result: 在细粒度多模态火灾数据集上的大量实验表明，PhysFire-WM在火灾蔓延预测方面具有优越的准确性。验证强调了物理先验和跨任务协作的重要性。

Conclusion: PhysFire-WM为将物理信息世界模型应用于灾害预测提供了新见解，通过整合物理知识和跨任务协作，显著提升了火灾蔓延预测的物理真实性和几何准确性。
...
TL;DR: LGCLIP提出了一种无需人工标注图像-文本对的轻量级视觉语言模型框架，通过LLM生成类别提示指导扩散模型合成参考图像作为视觉原型，实现零样本图像分类。
...
Method: LGCLIP框架：1）使用LLM生成类别特定的提示词；2）用扩散模型根据提示词合成参考图像作为视觉原型；3）提取真实图像的视觉特征并与原型特征比较进行预测；4）仅使用视觉编码器，保持轻量高效。
...
TL;DR: ABE-CLIP：无需训练的CLIP属性绑定增强方法，通过语义精炼机制和局部token-patch对齐提升属性-对象绑定能力


<details>
  <summary>Details</summary>
Motivation: CLIP在组合式图像-文本匹配中表现不佳，特别是在准确关联对象与其属性方面，因为其全局表示往往忽略了细粒度语义。现有方法需要额外训练或大量负采样，但对新组合概念的泛化能力有限。

Method: 提出ABE-CLIP，包含：1）语义精炼机制：精炼文本中对象和属性短语的token嵌入，减轻属性混淆；2）局部token-patch对齐策略：计算精炼后的文本token与最相关图像patch的相似度分数，通过聚合局部相似度得到最终图像-文本相似度。

Result: 在多个数据集上的实验表明，ABE-CLIP显著提升了属性-对象绑定性能，甚至超过了需要大量训练的方法。

Conclusion: ABE-CLIP是一种无需训练的有效方法，能够增强CLIP类模型的属性绑定能力，解决全局表示忽略细粒度语义的问题。
...
<details>
  <summary>Details</summary>
Motivation: 城市绿化评估存在客观测量（如植被数量）与主观感知（如人们感受）之间的差异，这种差异对城市规划和发展具有重要意义。研究旨在测量这种差异并解释其背后的因素。

Method: 使用街景图像提取上下文信息，收集来自5个国家1000人的城市视觉感知调查数据，包含详细的人口统计和个性信息。分析客观指标（如绿视率GVI）与主观评分之间的差异，考察人类和视觉因素（如年龄组、场景绿化空间变化）能否解释这些差异。

Result: 研究发现：1）客观与主观绿化评估的差异在全球范围内相似；2）人口统计和个性因素对感知没有显著影响；3）感知和测量的绿化在地理上具有一致性；4）居住地是解释感知差异的最重要因素（在七个特征中排名前二）。

Conclusion: 文化、环境和经验因素显著影响个体对城市绿化的观察方式。居住地是解释绿化感知差异的关键因素，这对考虑当地背景的城市规划具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 移动设备（如自动驾驶汽车）通常配备多相机系统和IMU，视觉和惯性信息的相对位姿估计在许多领域有重要应用，需要提高多相机系统相对位姿估计的精度

Method: 1. 使用仿射对应建立关于相对旋转角度的代价函数，最小化几何约束的代数误差；2. 基于特征方程及其一阶导数为零，将全局优化问题转化为两个未知数的两个多项式；3. 使用多项式特征值求解器解相对旋转角度，从特征向量获取平移向量；4. 针对小旋转情况提出新的线性解
...
TL;DR: ARCD是一种即插即用的医学视觉语言模型幻觉缓解方法，通过解剖区域引导的三层对比解码，在标记、注意力和对数层面动态重加权，提升区域理解和诊断准确性。
...
Method: 提出解剖区域引导对比解码（ARCD），利用解剖掩码指导三层对比解码过程：在标记层面、注意力层面和对数层面动态重加权，将模型注意力引导到指定区域，增强解剖理解并抑制事实错误输出。

Result: 在胸部X光、CT、脑MRI和眼部超声等多种数据集上的广泛实验表明，该方法能有效改善区域理解、减少幻觉并提高整体诊断准确性。

Conclusion: ARCD为医学视觉语言模型提供了一种有效的幻觉缓解策略，通过解剖区域引导的针对性校正，在无需昂贵标注的情况下提升了模型的可靠性和临床适用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型虽然性能好但计算量大、耗时；端到端模型结构简单、缺乏先验知识，性能有限。需要结合两者优势，实现高效高质量的图像融合

Method: 四阶段训练策略：1) 对增强的SOTA扩散模型进行单步蒸馏，将50步推理压缩为1步；2) 将单步扩散模型与端到端模型通过轻量级集成块融合；3) 获得轻量级网络Fose

Result: 在三个常用基准测试上显著提升性能，相比基线扩散模型实现7.42倍加速比，同时性能更好

Conclusion: Fose方法成功融合了扩散模型和端到端模型的优势，实现了高效且高质量的图像融合，为实际应用提供了实用解决方案
...
TL;DR: Reasoning Palette：通过变分自编码器引入潜在变量，在生成前进行内部推理策略采样，增强大语言模型的探索能力和推理多样性


<details>
  <summary>Details</summary>
Motivation: 大语言模型在推理时采样往往产生冗余的推理路径，缺乏高层次多样性，限制了推理性能和强化学习训练效果

Method: 提出Reasoning Palette框架：使用VAE从问答对嵌入中推断潜在变量，采样不同推理上下文；将潜在变量解码为可学习的前缀标记，调制模型的内部推理轨迹；通过SFT预热让模型适应潜在条件

Result: 在多个推理基准测试中，该方法实现了对模型策略行为的可解释和可控控制，相比标准RL方法获得了一致的性能提升

Conclusion: Reasoning Palette通过潜在调制框架，在生成前进行内部推理策略采样，显著增强了模型的探索效率和持续学习能力
...
TL;DR: CheXPO-v2是一个针对医学视觉语言模型幻觉问题的对齐框架，通过知识图谱一致性奖励机制，从结果监督转向过程监督，显著提升临床可靠性和数据效率。


<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型容易产生幻觉，影响临床可靠性。现有的强化学习方法（如GRPO）依赖稀疏的结果奖励，导致模型"过度思考"——生成冗长、复杂且不可验证的思维链推理来证明答案，这种关注结果的方式掩盖了事实错误并带来安全风险。

Method: 提出CheXPO-v2对齐框架，核心创新是知识图谱一致性奖励机制，通过实体关系匹配实现。方法将推理步骤明确解析为结构化的"疾病、关系、解剖"三元组，提供细粒度监督，在原子级别惩罚不连贯逻辑和幻觉。结合硬样本挖掘策略，实现过程监督而非结果监督。

Result: 在MIMIC-CXR-VQA等基准测试中显著优于GRPO和最先进模型。仅使用5k样本就实现了新的最先进准确率，展示了卓越的数据效率，同时产生临床合理且可验证的推理。

Conclusion: CheXPO-v2通过从结果监督转向过程监督，有效解决了医学视觉语言模型的幻觉问题，提高了临床可靠性和推理可验证性，同时保持了数据效率，为医学AI安全提供了重要解决方案。
...
TL;DR: DAVE是一个专为文档理解和网页代理任务设计的视觉编码器，通过自监督预训练和监督预训练两阶段方法，结合模型融合和集成训练策略，提升视觉语言模型在文档和网页任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型（VLMs）使用的视觉编码器存在根本性弱点：其低层特征缺乏文档理解和网页代理任务所需的鲁棒结构和空间信息。现有模型在文档和网页任务中表现不足，需要专门设计的视觉编码器。

Method: 采用两阶段训练流程：1）自监督预训练阶段使用大量未标注图像；2）监督自回归预训练阶段使用有限高质量数据学习解析和定位任务。采用两种策略：模型融合方案结合不同文本解码器训练的编码器；集成训练融合通用编码器特征与文档/网页专用特征。

Result: 在经典文档任务、视觉问答、网页定位和基于代理的基准测试中进行了广泛实验，验证了方法的有效性，将DAVE确立为文档和网页应用的强大视觉编码器。

Conclusion: DAVE是一个专门为文档理解和网页代理任务设计的视觉编码器，通过创新的训练策略和架构设计，有效解决了现有视觉语言模型在文档和网页任务中的局限性，为相关应用提供了强大的视觉编码基础。
...
TL;DR: AOM是一个通用的遥感基础模型，能够处理任意波段组合、传感器类型和分辨率尺度，解决了现有模型在波段缺失、跨传感器融合和未见空间尺度方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 不同光学传感器在波段组成和空间分辨率上存在显著差异，而现有遥感基础模型通常针对固定波段配置和分辨率进行预训练，导致在真实场景中面对波段缺失、跨传感器融合和未见空间尺度时泛化能力受限。

Method: 1) 频谱独立分词器：为每个通道分配专用波段嵌入，明确编码频谱身份；2) 多尺度自适应补丁嵌入机制：动态调整感受野以捕捉从亚米级到百米级图像的纹理和上下文模式；3) 多尺度语义对齐机制和通道级自监督掩码重建预训练策略：联合建模频谱-空间关系，保持跨分辨率全局语义一致性。
...
TL;DR: 提出一种基于几何结构和视觉相似性的全局描述符聚合模块，通过批挖掘策略和对比损失训练，无需人工标注即可提升大规模环境中的视觉定位性能。
...
Method: 提出聚合模块，学习同时符合几何结构和视觉相似性的全局描述符；采用基于重叠分数的批挖掘策略和修改的对比损失，无需人工地点标签即可训练。

Result: 在具有挑战性的基准测试中，该方法在大规模环境中实现了显著的定位性能提升，同时保持了计算和内存效率。

Conclusion: 通过联合考虑几何结构和视觉相似性学习全局描述符，能够纠正不可靠重叠分数导致的错误关联，提升视觉定位的鲁棒性和准确性。
...
TL;DR: 提出一个课程学习框架来解决多模态大语言模型中的"视觉遗忘"问题，通过分离抽象推理和视觉感知技能，让模型学会"何时看"的策略。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在复杂长链视觉推理任务中表现脆弱，存在"视觉遗忘"问题——随着推理延伸，模型逐渐失去视觉基础。这源于当前训练范式过早地将抽象逻辑推理和战略视觉感知两种认知技能纠缠在一起。

Method: 提出两阶段课程学习框架：1) 解耦监督微调课程，先在纯文本数据上构建稳健的抽象推理骨干，然后通过感知锚定的思维链范式将其与视觉连接；2) 将时机选择建模为强化学习问题，设计关键感知奖励，通过将感知动作与认知不确定性语言标记耦合来教导模型何时查看。

Result: 论文提出了形式化的两种缺陷定义和原则性的两阶段框架，将模型从启发式观察者转变为战略性、有基础的推理者。

Conclusion: 通过分离"如何思考"和"何时看"两种认知技能，并教导模型自主的视觉基础策略，可以有效解决多模态大语言模型在长链视觉推理中的视觉遗忘问题。
...
TL;DR: VideoDetective：一种用于长视频问答的高效问题感知记忆机制，通过迭代处理视频片段和压缩策略，使MLLM能在有限上下文长度内处理超长视频


<details>
  <summary>Details</summary>
Motivation: 长视频问答面临巨大上下文和过载信息的挑战，现有方法通过减少视觉token或扩展上下文长度可能丢失有用信息或计算成本高。实际上回答问题只需要少量关键信息

Method: 提出VideoDetective方法：1）迭代处理视频子片段；2）每个子片段使用问题感知压缩策略，引入特殊记忆token进行有目的压缩；3）递归聚合存储记忆token更新历史上下文供后续使用

Result: 方法使32K上下文长度的MLLM能高效处理100K token（3600帧，1小时视频），仅需2分钟和37GB GPU内存。在多个长视频基准测试中表现更有效

Conclusion: 提出的问题感知记忆机制能有效从海量信息中寻找关键线索，解决了长视频问答的计算和内存挑战，并引入了新的评估数据集GLVC
...
TL;DR: Mitty是一个基于扩散Transformer的视频上下文学习模型，能够直接将人类演示视频端到端地生成机器人执行视频，无需动作标签或中间表示，在Human2Robot和EPIC-Kitchens上达到SOTA效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖关键点或轨迹等中间表示，会引入信息损失和累积误差，损害时间和视觉一致性。需要直接从人类演示视频学习，实现可扩展和泛化的机器人学习。

Method: 基于预训练视频扩散模型构建Diffusion Transformer，通过视频上下文学习实现端到端Human2Robot视频生成。演示视频被压缩为条件token，在扩散过程中通过双向注意力与机器人去噪token融合。为解决配对数据稀缺问题，开发了从大型第一人称数据集自动合成高质量人机配对数据的流程。

Result: 在Human2Robot和EPIC-Kitchens数据集上达到最先进的结果，展现出对未见环境的强大泛化能力，为从人类观察中实现可扩展机器人学习提供了新见解。

Conclusion: Mitty通过视频上下文学习实现了直接从人类演示视频到机器人执行视频的端到端生成，避免了中间表示的信息损失，为可扩展的机器人学习提供了有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 胸部X光解剖分割面临两大挑战：1）全面标注数据稀缺；2）真实世界采集条件差异大。需要一种无需大量真实标注就能实现鲁棒分割的方法

Method: 1）多阶段域随机化（MSDR）引擎：从3D CT体积生成10万+解剖保真且高度多样化的合成X光片；2）条件联合标注正则化（CAR）学习策略：在潜在空间中通过强制解剖一致性来利用部分和不完美标签

Result: 仅使用合成数据训练的AnyCXR在多个真实世界数据集上实现强零样本泛化，能在PA、侧位和斜位视图中准确分割54个解剖结构。分割结果支持下游临床任务，包括自动心胸比估计、脊柱曲率评估和疾病分类

Conclusion: AnyCXR为解剖感知的胸部X光分析建立了可扩展且可靠的基础，提供了一条减少标注负担同时提高跨多样化成像条件鲁棒性的实用路径
...
Method: 提出WDFFU-Mamba分割网络，集成小波引导增强和双注意力特征融合于U型Mamba架构。包含小波去噪高频引导特征模块增强低层表示，以及双注意力特征融合模块有效融合跳跃连接和语义特征。

Result: 在两个公共BUS数据集上的实验表明，WDFFU-Mamba在Dice系数和95%Hausdorff距离指标上显著优于现有方法，实现了优越的分割精度。

Conclusion: 小波域增强和基于注意力的融合相结合，在保持计算效率的同时，大大提高了BUS图像分割的准确性和鲁棒性。该模型具有良好的泛化能力，有望用于真实临床应用。
...
Method: 通过Universal UltraSound Image Challenge 2025竞赛，使用11,644张图像训练算法，在包含未见中心数据的2,479张图像独立测试集上评估多器官分类和分割性能

Result: 最佳模型SMART在5个分割任务中平均DSC达0.854，二分类AUC为0.766；但在乳腺癌分子分型任务中，从未见外部中心的AUC从0.571降至0.508，显示泛化挑战

Conclusion: 通用AI模型在多项任务中具有高准确性和效率，但在未见数据上的性能下降表明领域泛化是未来临床部署的关键挑战
...
Method: 提出VLMIR框架：1）VLM特征提取阶段：通过CLIP等模型提取互补的视觉和语言表示，使用LoRA微调对齐低质量/高质量图像描述嵌入，通过退化预测器分解退化与干净图像内容嵌入；2）扩散模型修复阶段：通过交叉注意力机制将视觉和文本嵌入整合到扩散模型中。

Result: 在通用和退化特定的图像修复任务中均取得优异性能，验证了整合视觉语言模型知识对提升图像修复能力的关键作用。

Conclusion: VLMIR框架通过有效利用视觉语言模型的视觉感知和语义先验知识，显著提升了图像修复的质量和语义一致性，为图像修复领域提供了新的有效方法。
...
TL;DR: HiSIR-Net：用于PCB组装缺陷检测的高分辨率自监督重建框架，通过SIR-Gate和ROPS机制减少误报，并在4K分辨率图像上实现像素级定位。


<details>
  <summary>Details</summary>
Motivation: PCB组装（PCBA）的自动缺陷检测面临三大挑战：1）标记数据不足；2）微缺陷只有几个像素大小；3）视觉复杂的高分辨率图像。现有方法难以在4K分辨率下有效定位微小缺陷。
...
Result: 在自收集的SIPCBA-500数据集（500张图像）和公共基准测试上进行实验，展示了优越的定位性能，同时保持实用运行速度。生成干净的高分辨率异常图，误报率低。

Conclusion: HiSIR-Net有效解决了高分辨率PCBA缺陷检测的挑战，通过自监督重建框架实现了像素级精确定位，为工业检测提供了实用解决方案。代码和数据集将在论文接受后公开。
...
TL;DR: ProCache：一种无需训练的动态特征缓存框架，通过非均匀缓存调度和选择性计算，显著加速扩散变换器（DiTs）的推理速度，最高可达2.9倍加速，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器（DiTs）在生成建模中表现出色，但计算成本高阻碍实时部署。现有特征缓存方法存在两个关键问题：1）均匀缓存间隔无法匹配DiT的非均匀时间动态；2）大缓存间隔下的简单特征重用会导致严重误差累积。

Method: ProCache包含两个核心组件：1）约束感知缓存模式搜索模块，通过离线约束采样生成非均匀激活调度，适应模型的时间特性；2）选择性计算模块，在缓存段内对深层块和高重要性token进行选择性计算，以最小开销缓解误差累积。

Result: 在PixArt-alpha和DiT上的实验表明，ProCache实现了最高1.96倍和2.90倍的加速，质量下降可忽略，显著优于现有缓存方法。

Conclusion: ProCache通过分析DiT特征演化规律，提出动态特征缓存框架，有效解决了现有缓存方法的局限性，为DiT的高效部署提供了实用解决方案。
...
TL;DR: 提出生成高质量PBR纹理的框架，通过微调预训练VAE来整合新材料通道，并引入局部性正则化保持空间对应关系，实现更好的纹理保真度。


<details>
  <summary>Details</summary>
Motivation: 由于大规模PBR纹理数据集稀缺，现有方法冻结嵌入网络会导致分布偏移，阻碍后续扩散训练，需要一种能有效利用预训练图像生成模型的方法。

Method: 1) 微调预训练VAE以最小化潜在分布偏差整合新材料通道；2) 引入局部性正则化，通过裁剪潜在补丁、解码并对齐图像区域来保持像素-潜在空间对应关系；3) 学习材料潜在空间MatLat。

Result: 消融研究和与先前基线的比较表明，该框架提高了PBR纹理保真度，每个组件对实现最先进性能都至关重要。

Conclusion: 提出的生成框架通过微调VAE和局部性正则化，有效利用预训练图像生成模型的嵌入空间和扩散先验，在稀缺数据集条件下实现了高质量的PBR纹理生成。
...
TL;DR: 提出EMAG（指数移动平均引导）方法，在扩散变换器中通过基于统计的自适应层选择规则修改注意力机制，生成更难的负样本以提升生成质量
...
Method: 提出训练免费的EMAG机制，在推理时修改扩散变换器的注意力，采用基于统计的自适应层选择规则，生成更精细的语义忠实负样本

Result: EMAG相比CFG提升人类偏好评分(HPS)0.46分，并能与APG、CADS等先进引导技术自然组合，进一步改善HPS

Conclusion: EMAG通过自适应生成更难的负样本，有效提升了扩散变换器的生成质量，为引导技术提供了新的改进方向
...
Method: 采用三阶段流程：1）基于高分辨率图像构建高难度可验证的视觉问答对；2）冷启动SFT阶段收集工具轨迹作为训练数据；3）RL阶段引入冗余惩罚策略优化，鼓励模型进行自我反思推理

Result: DRIM在视觉理解基准测试中表现出优越性能

Conclusion: DRIM通过自我反思和冗余惩罚优化实现了可靠的深度多轮推理，解决了现有模型难以纠正错误推理轨迹的问题
...
TL;DR: CodeDance：使用可执行代码作为视觉推理通用求解器，通过代码定义、组合和执行来协调多个工具，实现透明、可自检的推理，在多个基准测试中超越GPT-4o等先进模型。


<details>
  <summary>Details</summary>
Motivation: 现有开源方法主要依赖纯文本链、固定视觉模式或单步流水线，限制了在复杂任务上的灵活性、可解释性和可迁移性。虽然o3等模型展示了类似人类的"图像思维"推理能力，但需要更通用的解决方案。

Method: 引入CodeDance框架，将可执行代码作为视觉推理的通用求解器。通过代码定义、组合和执行来协调多个工具，计算中间结果，渲染视觉元素（如边界框、线条、图表）。引入平衡自适应工具调用的奖励机制，平衡探索与效率，避免工具过度使用。

Result: 在视觉搜索、数学、图表问答等推理基准测试中，CodeDance不仅持续优于模式驱动和纯文本基线，还超越了GPT-4o等先进闭源模型和更大的开源模型。RL训练中观察到新颖的涌现行为：未见过的工具调用、组合和跨任务迁移。

Conclusion: CodeDance展示了可执行代码作为视觉推理通用求解器的潜力，实现了透明、可自检的推理过程。涌现行为表明这是一种通用且可扩展的机制，无需任务特定微调即可实现跨任务迁移。
...
TL;DR: ADK框架通过离线生成丰富的类别描述提示，为视觉语言模型提供辅助描述知识，提升少样本适应性能而不增加推理开销。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型具有出色的零样本能力，但在下游任务中面对分布偏移时表现不佳。现有的参数高效微调方法受限于固定的人工提示，无法充分理解类别语义。虽然已有研究提出使用图像诱导提示，但会带来过高的推理计算开销。

Method: ADK框架首先利用大语言模型为每个类别离线生成丰富的描述性提示。这些预计算的特征以两种方式部署：1）作为组合知识（平均表示），提供丰富的语义信息；2）作为实例特定知识，通过轻量级非参数注意力机制为给定图像动态选择最相关的描述。

Result: ADK作为参数免费、即插即用的组件，能够持续提升多种PEFT基线的性能，在各种场景下创造了新的最先进水平。

Conclusion: ADK框架通过高效丰富文本表示而不影响效率，有效增强了视觉语言模型在少样本适应任务中的性能，解决了现有方法在语义理解和计算效率方面的局限性。
...
TL;DR: RSHR-Bench是一个超高分辨率遥感视觉理解与推理基准，包含5,329张长边至少4,000像素的全场景图像，设计了四种任务类型，通过对抗性过滤减少语言先验依赖，用于评估MLLMs在超高分辨率场景下的真实视觉理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有遥感基准大多使用低分辨率图像，部分高分辨率基准存在推理任务设计缺陷。研究发现纯文本LLMs无需访问图像就能在遥感推理任务上达到与多模态模型竞争的性能，表明当前基准与视觉理解评估目标存在严重不匹配。

Method: 从广泛使用的遥感语料库和无人机采集数据中收集5,329张超高分辨率图像（单张图像最多约3×10^8像素），设计四种任务家族：多项选择VQA、开放式VQA、图像描述和单图像评估，覆盖9个感知类别和4种推理类型，采用强LLMs进行对抗性过滤后人工验证。

Result: 构建了3,864个VQA任务、3,913个图像描述任务和500个人工编写/验证的单图像评估VQA对。评估显示开源、闭源和遥感专用VLMs在超高分辨率场景下均存在持续性能差距。

Conclusion: RSHR-Bench为遥感视觉理解提供了忠实评估基准，揭示了当前多模态大语言模型在超高分辨率场景下的局限性，为未来模型改进提供了重要参考。
...
TL;DR: EMMA是一个评估概念擦除技术的新基准，测试5个关键维度12个指标，发现现有方法在间接提示和视觉相似概念上表现不佳，部分方法还会放大性别和种族偏见。


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成的广泛应用引发了隐私、偏见和版权问题，概念擦除技术提供了一种无需完全重新训练即可从预训练模型中移除特定概念的解决方案。然而，现有方法通常在有限概念集上评估，依赖过于简单直接的提示，需要更全面的评估框架来测试概念擦除的真正效果。

Method: 提出了EMMA基准，评估概念擦除技术的5个关键维度（包括12个指标），超越了传统的图像质量和时间效率指标，测试了在挑战性条件下的鲁棒性，包括间接描述、视觉相似的非目标概念，以及潜在的性别和种族偏见分析。

Result: 使用EMMA分析了5个概念擦除方法在5个领域（物体、名人、艺术风格、NSFW内容和版权内容）的表现。结果显示现有方法在隐式提示（间接引用擦除概念时仍能生成）和视觉相似的非目标概念（无法生成与擦除概念相似的非目标概念）方面表现不佳，部分方法相比原始模型还放大了性别和种族偏见。

Conclusion: 概念擦除技术尚未完全解决其声称的问题，特别是在处理间接提示和视觉相似概念方面存在不足，且可能引入新的偏见问题。EMMA基准为更全面评估概念擦除方法提供了框架，揭示了现有方法的局限性，为未来改进指明了方向。
...
<details>
  <summary>Details</summary>
Motivation: 为眼科血管分析提供多样化、高质量的标注数据集，支持机器学习算法在真实世界图像质量和采集条件下的稳健训练与评估。

Method: 从纵向鹿特丹研究中采样彩色眼底图像，使用定制标注界面在独立图层上标注动脉、静脉和未知血管，基于初始血管分割掩码，并通过连通分量可视化工具验证和校正连通性。

Result: 数据集包含1024x1024像素PNG图像，提供三种模态：原始RGB眼底图像、对比度增强版本和RGB编码的动静脉掩码。图像质量差异大，包含通常被自动质量评估系统排除但具有血管信息价值的挑战性样本。

Conclusion: 该数据集提供了丰富且异质的彩色眼底图像资源，支持在真实世界图像变异条件下进行机器学习模型的稳健基准测试和训练，有助于开发临床适用的视网膜血管分析工具。
...
<details>
  <summary>Details</summary>
Motivation: 传统视频帧预测在动态场景中因缺乏下一帧信息而产生预测误差。事件相机通过异步捕获像素亮度变化提供高时间分辨率信息，但现有基于事件的光流预测方法在像素位移不准确时会产生空洞和模糊问题。

Method: 提出DESSERT框架：1）事件到残差对齐变分自编码器（ER-VAE），将锚帧和目标帧之间的事件帧与对应残差对齐；2）基于事件数据条件的扩散模型去噪残差潜在表示；3）引入多样长度时间增强（DLT）提高鲁棒性。

Result: 实验结果表明，该方法在事件重建、基于图像的视频帧预测、基于事件的视频帧预测以及单边事件视频帧插值等任务上均优于现有方法，能生成更清晰、时间更一致的帧合成结果。

Conclusion: DESSERT通过扩散模型和残差训练策略，有效解决了事件视频帧预测中的空洞和模糊问题，实现了更高质量的视频帧合成。
...
TL;DR: 开发了Polysome工具生成合成指令，创建了HISTAI-Instruct数据集（24,259张全切片图像，110万指令-响应对），并训练了ANTONI-α视觉语言模型，在组织识别、肿瘤检测和鉴别诊断任务上优于MedGemma。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在病理学应用中存在局限性：要么只关注全切片图像中的小区域，要么只能提供静态的切片级输出，或者依赖非公开数据限制了可复现性。同时，包含详细临床报告的全切片图像配对数据稀缺，阻碍了透明且可泛化的视觉语言模型发展。

Method: 1. 开发Polysome标准化工具用于合成指令生成；2. 将Polysome应用于公开的HISTAI数据集，生成HISTAI-Instruct大型全切片指令调优数据集；3. 使用HISTAI-Instruct训练ANTONI-α视觉语言模型，支持视觉问答任务。

Result: ANTONI-α在组织识别、肿瘤检测和鉴别诊断等全切片视觉问答任务上优于MedGemma。还比较了不同数据量训练的ANTONI-α变体性能。所有方法、数据和代码均已公开。

Conclusion: 通过开发标准化工具生成合成指令、创建大规模公开数据集、训练高性能视觉语言模型，解决了病理学视觉语言模型发展的关键瓶颈，为病理学家提供了有效的辅助工具。
...
TL;DR: SynergyWarpNet是一个三阶段注意力引导的协同变形框架，用于高保真说话头合成，通过显式变形、参考增强校正和置信度引导融合来解决传统方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统显式变形方法在准确运动转移和恢复缺失区域方面存在困难，而基于注意力的变形方法虽然有效，但复杂度高且几何基础弱。需要一种能平衡结构对齐和视觉一致性的解决方案。

Method: 提出三阶段渐进式动画框架：1）显式变形模块使用3D密集光流进行粗空间对齐；2）参考增强校正模块利用跨注意力机制从多个参考图像中语义补全遮挡或扭曲区域；3）置信度引导融合模块通过学习的置信度图自适应融合变形输出。

Result: 在基准数据集上的综合评估展示了最先进的性能表现。

Conclusion: SynergyWarpNet通过协同变形框架有效解决了传统方法的局限性，实现了高保真的说话头合成。
...
<details>
  <summary>Details</summary>
Motivation: 全向图像超分辨率中，ERP投影引入纬度相关的几何畸变（赤道附近畸变小，两极附近畸变严重），现有方法采样范围和特征提取能力有限，难以捕捉大范围的畸变模式

Method: 提出多级畸变感知可变形网络（MDDN），包含三个并行分支：可变形注意力机制（膨胀率=1）和两个膨胀可变形卷积（膨胀率2和3），通过多级特征融合模块自适应融合特征，并使用低秩分解策略降低计算成本

Result: 在公开数据集上的大量实验表明，MDDN优于现有最先进方法，证明了其在全向图像超分辨率中的有效性和优越性

Conclusion: MDDN通过扩展采样范围和感受野，能够有效捕捉ERP图像中的几何畸变，为全向图像超分辨率提供了一种有效的解决方案
...
TL;DR: 提出一种像素级映射预处理方法，通过破坏图像像素值分布来消除检测器过度依赖的语义捷径，从而提升AI生成图像检测器的跨生成器泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前AI生成图像检测器存在严重泛化问题，对未见过的生成模型检测性能大幅下降。主要原因是检测器过度拟合特定生成模型的语义特征，而非学习通用的生成痕迹。

Method: 引入简单的像素级映射预处理步骤，破坏图像的像素值分布，打破检测器常利用的脆弱、非本质的语义模式，迫使检测器关注图像生成过程中更基础、可泛化的高频痕迹。

Result: 在GAN和扩散模型生成器上的综合实验表明，该方法显著提升了最先进检测器的跨生成器性能。分析验证了破坏语义线索是提升泛化能力的关键。

Conclusion: 通过破坏像素值分布来消除语义捷径，能够有效提升AI生成图像检测器的泛化能力，使其更关注通用的生成痕迹而非特定模型的语义特征。
...
TL;DR: 提出情感图像过滤(AIF)任务，将文本中的抽象情感转化为视觉具体的图像，创造情感共鸣的结果


<details>
  <summary>Details</summary>
Motivation: 社交媒体用户通过文本和图像表达情感，但如何将文本中的抽象情感有效转化为视觉具体的图像是一个挑战

Method: 1) 提出AIF数据集和模型框架；2) 基于多模态transformer的AIF-B模型；3) 基于预训练扩散模型的AIF-D模型，实现更深层情感反映

Result: AIF模型在内容一致性和情感保真度上优于现有方法，用户研究表明能更有效唤起特定情感

Conclusion: AIF模型具有重要价值和潜力，能够将文本情感有效转化为视觉表达，创造情感共鸣的图像
...
TL;DR: RadImageNet-VQA是一个大规模放射学视觉问答数据集，包含75万张CT/MRI图像和750万个问答对，涵盖8个解剖区域和97种病理类别，旨在解决现有医学VQA数据集规模小、X射线主导、存在文本捷径的问题。
...
Method: 构建了RadImageNet-VQA数据集，包含专家标注的750K张CT和MRI图像，配以7.5M个问答样本。数据集涵盖三个核心任务：异常检测、解剖结构识别和病理识别，覆盖8个解剖区域和97种病理类别，支持开放式、封闭式和多项选择题型。

Result: 实验表明，即使是最先进的视觉语言模型在细粒度病理识别任务上仍然表现不佳，特别是在开放式设置中，即使经过微调也难以达到理想效果。仅文本分析显示，没有图像输入时模型性能接近随机水平，证实了数据集不存在语言捷径。

Conclusion: RadImageNet-VQA是一个大规模、高质量的放射学VQA基准数据集，有效解决了现有数据集的局限性。该数据集公开可用，为医学视觉问答研究提供了重要资源，并揭示了当前模型在细粒度医学图像理解方面的不足。
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络在癌症诊断等关键应用中需要可解释的结果，但常用的遮挡法计算时间长，影响开发效率和与病理学家的交互

Method: 首先建立解释方法比较框架，确定合适的比较标准和相应指标，然后基于结果选择替代的解释方法

Result: 成功找到替代方法，将所需解释时间减少至少10倍，且对输出质量没有负面影响

Conclusion: 该加速使模型开发和调试能够快速迭代，推动AI辅助前列腺癌检测在临床环境中的应用，且该方法可推广到其他相关应用
...
<details>
  <summary>Details</summary>
Motivation: 现有洪水检测数据集存在地理范围有限、标注细节不足的问题，阻碍了鲁棒、泛化的计算机视觉方法的发展。需要构建一个全面、公开、全球多样化的数据集来推动灾害响应和风险评估的进步。

Method: 收集了2022-2024年间来自64个国家、6大洲、230次不同洪水事件的470张高分辨率航空影像，构建了包含三个互补任务的数据集：1) 图像分类（环境类型、相机角度、大陆识别）；2) 语义分割（洪水、天空、建筑物的像素级掩码）；3) 视觉问答（支持自然语言推理的灾害评估）。

Result: 建立了基于最先进架构的所有任务基准测试，展示了数据集的复杂性和在推进气候韧性领域通用AI工具方面的价值。数据集具有全球多样性和时间相关性，填补了现有洪水检测数据集的空白。

Conclusion: AIFloodSense作为一个全面、公开可用的数据集，通过支持多个互补任务，能够显著推动洪水检测和灾害评估领域的发展，为构建更鲁棒、泛化的计算机视觉方法提供了重要资源。
...
TL;DR: 小米开源了MiMo-VL-Miloco-7B及其量化版本，这是一个专注于智能家居场景的视觉语言模型，在家庭场景理解和通用多模态推理方面表现优异。
...
Method: 采用两阶段训练流程：监督微调结合基于Group Relative Policy Optimization的强化学习，利用高效多领域数据，并融入思维链监督和token预算感知推理。

Result: 在家庭场景理解（手势识别等）和多个多模态基准测试（Video-MME、Video-MMMU、Charades-STA等）上超越闭源和开源基线模型，同时在语言理解基准（MMMU-Pro、MMLU-Pro）上也取得一致提升。

Conclusion: 针对家庭场景的训练不仅增强了活动和手势理解能力，还改善了纯文本推理能力，在文档中心任务上只有适度的权衡。模型已开源以支持智能家居应用的研究和部署。
...
TL;DR: LangDriveCTRL是一个自然语言可控框架，用于编辑真实世界驾驶视频以合成多样交通场景。它通过3D场景分解将驾驶视频表示为场景图，并使用智能体管道将用户指令转换为执行图，协调专门工具进行细粒度编辑。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶视频编辑方法在细粒度控制、真实感和多对象行为编辑方面存在局限。需要一种能够通过自然语言指令精确控制驾驶场景编辑的框架，以合成多样且真实的交通场景。

Method: 1) 使用3D场景分解将驾驶视频表示为包含静态背景和动态对象的场景图；2) 采用智能体管道：Orchestrator将用户指令转换为执行图，协调三个专门智能体：Object Grounding Agent建立文本描述与场景图中对象的对应关系，Behavior Editing Agent从语言指令生成多对象轨迹，Behavior Reviewer Agent迭代审查和优化轨迹；3) 编辑后的场景图通过视频扩散工具进行渲染和细化。

Result: LangDriveCTRL支持对象节点编辑（移除、插入、替换）和基于单条自然语言指令的多对象行为编辑。定量评估显示，其指令对齐度比之前最先进方法提高近2倍，在结构保持、照片真实感和交通真实性方面表现优异。

Conclusion: LangDriveCTRL通过结合3D场景表示和智能体管道，实现了对真实世界驾驶视频的自然语言可控编辑，能够合成多样且真实的交通场景，显著提升了编辑质量和用户控制能力。
...
TL;DR: 提出多模态海事数据集MULTIAQUA，包含RGB、热成像、红外、激光雷达等同步数据，用于训练能在恶劣视觉条件下保持性能的多模态方法，特别是仅用白天数据训练就能在夜间场景中可靠工作的网络。


<details>
  <summary>Details</summary>
Motivation: 无人水面车辆在操作中会遇到各种复杂的视觉环境，有些情况难以解析。虽然大多数情况可以用彩色相机图像解决，但某些天气和光照条件需要额外信息。现有海事数据有限，需要多模态数据来提升场景理解能力。

Method: 创建MULTIAQUA多模态海事数据集，包含同步、校准和标注的RGB、热成像、红外、激光雷达等传感器数据。提出训练方法使多模态方法仅用白天图像就能训练，在夜间测试集上评估多种多模态方法。

Result: 提出的数据集和方法使多模态方法能在近乎完全黑暗的条件下保持可靠性能。训练方法允许仅使用白天图像训练鲁棒的深度神经网络，显著简化了数据采集、标注和训练过程。

Conclusion: MULTIAQUA数据集为开发能在各种视觉条件下提供高质量场景理解的多模态方法提供了重要资源。提出的训练方法使网络仅用白天数据就能在夜间场景中可靠工作，解决了海事视觉系统中的关键挑战。
...
TL;DR: 3D-RE-GEN是一个从单张图像重建可编辑3D纹理网格场景的框架，通过组合多个先进模型解决现有方法在物体分解、空间关系和背景缺失方面的问题，满足影视游戏行业艺术家的实际需求。


<details>
  <summary>Details</summary>
Motivation: 当前3D场景生成方法虽然视觉效果不错，但生成的表示形式难以满足艺术家工作流程需求，特别是需要可编辑的3D纹理网格场景用于视觉效果和游戏开发。现有纹理网格场景重建方法存在物体分解错误、空间关系不准确、背景缺失等问题，距离艺术家实际使用要求还很远。

Method: 提出一个组合式框架，集成资产检测、重建和放置模型，将特定领域的最先进模型组合使用。将遮挡物体获取视为图像编辑任务，使用生成模型在一致的照明和几何条件下进行场景级推理。采用新颖的4自由度可微分优化来对齐重建物体与估计的地平面，生成全面的背景以在优化过程中空间约束物体。

Result: 3D-RE-GEN在单图像3D场景重建中达到最先进性能，通过精确相机恢复和空间优化的组合生成指导，产生连贯、可修改的场景，为视觉效果和游戏中的真实照明和模拟任务提供基础。

Conclusion: 通过组合特定领域的最先进模型，3D-RE-GEN解决了艺术家对可编辑3D纹理网格场景的需求，在场景重建性能上达到最先进水平，为影视游戏行业提供了实用的解决方案。
...
Method: 提出TwinSegNet框架，结合卷积编码器和Vision Transformer瓶颈来捕获局部和全局上下文；采用联邦学习，各机构在私有数据上微调全局模型形成个性化数字孪生

Result: 在9个异质MRI数据集（包括BraTS 2019-2021）上评估，Dice分数高达0.90%，敏感性和特异性超过90%，在非独立同分布数据上表现出鲁棒性，性能与集中式模型相当
...
Method: LumiCtrl包含三个核心组件：1) 基于物理的沿普朗克轨迹的光照增强，创建标准光照下的微调变体；2) 使用冻结ControlNet的边缘引导提示解耦，确保提示专注于光照而非结构；3) 掩码重建损失，专注于前景对象学习，同时允许背景进行上下文适应（上下文光照适应）。

Result: LumiCtrl在光照保真度、美学质量和场景连贯性方面显著优于现有的T2I定制方法。人类偏好研究进一步证实用户对LumiCtrl输出有强烈偏好。

Conclusion: LumiCtrl为文本到图像模型提供了有效的光照个性化控制方法，使内容设计师能够精确操纵生成图像的照明效果，从而控制情绪、氛围和视觉美感。
...
TL;DR: 论文提出了多模态地标数据集MMLANDMARKS，包含美国18,557个地标的四种模态数据：航拍图像、地面图像、文本描述和地理坐标，用于训练和评估地理空间多模态任务。
...
Method: 构建了MMLANDMARKS数据集，包含197k高分辨率航拍图像、329k地面图像、文本信息和地理坐标，所有模态间具有一一对应关系。采用简单的CLIP风格基线模型进行多模态训练。

Result: 该数据集支持多种地理空间任务，包括跨视角地面到卫星检索、地面和卫星地理定位、文本到图像和文本到GPS检索。基线模型在各项任务中展现出广泛泛化能力和与现有模型的竞争性能。
...
TL;DR: GroundingME是一个新的视觉定位基准，系统性地挑战多模态大语言模型在四个关键维度上的能力：区分相似对象、理解空间关系、处理有限可见性、识别不可定位查询，揭示了当前模型与人类水平之间的巨大差距。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在现有基准上表现优异，但无法确定它们是否真正具备人类水平的视觉语言理解能力，还是仅仅在简化数据集上进行模式匹配。现有基准未能捕捉现实世界的复杂性，如处理模糊引用和识别不可定位查询。

Method: 提出GroundingME基准，通过自动化生成和人工验证相结合的方式，精心构建了1,005个具有挑战性的示例，涵盖四个关键维度：区分性、空间性、有限性和拒绝性。评估了25个最先进的多模态大语言模型。

Result: 评估结果显示显著的能力差距：最佳模型仅达到45.1%的准确率，大多数模型在拒绝任务上得分为0%，会幻觉对象而非承认其不存在。通过测试时缩放和混合数据训练，分别将复杂定位准确率提升2.9%，拒绝准确率从0%提升至27.9%。

Conclusion: GroundingME既是一个诊断工具，揭示了当前多模态大语言模型在视觉定位方面的局限性，也为实现人类水平的视觉定位提供了路线图，强调了模型安全部署的重要性。
...
TL;DR: 该研究首次在中东地区（伊拉克库尔德斯坦）验证了前列腺癌AI诊断和Gleason分级系统，证明AI模型达到病理学家水平，且低成本紧凑扫描仪可用于非数字化环境。


<details>
  <summary>Details</summary>
Motivation: 当前病理AI系统主要在欧美大型中心验证，缺乏对代表性不足人群的研究。这些地区可能从AI支持中获益最大，需要全球AI采用所需的验证研究。

Method: 收集伊拉克库尔德斯坦地区339个前列腺活检标本（185名患者，2013-2024年），评估任务特定端到端AI模型和两个基础模型，比较AI与病理学家的分级一致性，并测试三种扫描仪（Hamamatsu、Leica、Grundium）的跨扫描仪一致性。

Result: AI与病理学家的分级一致性（Cohen's加权kappa 0.801）与病理学家间一致性（0.799）相似（p=0.9824）。所有AI模型和扫描仪对的跨扫描仪一致性都很高（加权kappa > 0.90），包括低成本紧凑扫描仪。

Conclusion: AI模型在前列腺组织病理学评估中达到病理学家水平性能。紧凑扫描仪可为非数字化环境提供验证研究途径，并在样本量有限的实验室中实现成本效益的AI采用。这是中东地区首个公开可用的数字病理数据集，支持全球公平AI病理学研究。
...
TL;DR: InsertAnywhere是一个新的视频对象插入框架，通过4D感知掩码生成和扩散模型实现几何一致的对象放置和外观真实的视频合成，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 基于扩散的视频生成在可控视频编辑方面取得了进展，但真实的视频对象插入仍然面临挑战，包括有限的4D场景理解、遮挡处理和光照效果不足等问题。

Method: 1) 4D感知掩码生成模块重建场景几何并在帧间传播用户指定的对象放置，保持时间一致性和遮挡一致性；2) 扩展扩散视频生成模型联合合成插入对象及其周围局部变化（如光照和阴影）；3) 构建ROSE++光照感知合成数据集进行监督训练。

Result: 在多样化真实场景中产生几何合理且视觉一致的对象插入效果，显著优于现有研究和商业模型。

Conclusion: InsertAnywhere框架通过结合4D场景理解和扩散视频生成，实现了高质量的视频对象插入，解决了几何一致性和外观真实性的关键挑战。
...
TL;DR: FALCON-SFOD是一个新的源自由目标检测框架，通过空间先验感知正则化和不平衡感知噪声鲁棒伪标签技术，增强域偏移下的目标聚焦特征表示，解决现有方法伪标签不可靠的问题。


<details>
  <summary>Details</summary>
Motivation: 现有源自由目标检测方法主要依赖Mean-Teacher自标签，但域偏移会削弱检测器的目标聚焦表示能力，导致在背景杂波上产生高置信度激活，从而产生不可靠的伪标签。先前工作主要关注优化伪标签，而忽视了加强特征空间本身的需求。

Method: 提出FALCON-SFOD框架，包含两个互补组件：1) SPAR（空间先验感知正则化）：利用视觉基础模型的泛化能力，通过OV-SAM生成的类无关二值掩码来正则化检测器特征空间，引导网络关注目标区域；2) IRPL（不平衡感知噪声鲁棒伪标签）：在严重前景-背景不平衡下促进平衡且噪声容忍的学习。

Result: 通过理论分析将这些设计与更紧密的定位和分类误差边界联系起来，FALCON-SFOD在SFOD基准测试中取得了有竞争力的性能。

Conclusion: FALCON-SFOD通过加强特征空间本身而非仅仅优化伪标签，有效解决了源自由目标检测中域偏移导致的目标聚焦表示弱化问题，为SFOD领域提供了新的解决方案。
...
TL;DR: PathBench-MIL是一个用于组织病理学中多实例学习的开源AutoML和基准测试框架，自动化MIL流程并提供模型评估


<details>
  <summary>Details</summary>
Motivation: 解决组织病理学中多实例学习流程构建的复杂性，提供标准化、可复现的基准测试环境，促进MIL方法的研究和比较

Method: 开发端到端的AutoML框架，自动化预处理、特征提取和MIL聚合，集成可视化工具、统一配置系统和模块化扩展架构

Result: 实现了数十种MIL模型和特征提取器的可复现基准测试，提供了快速实验和跨数据集任务标准化的能力

Conclusion: PathBench-MIL为组织病理学MIL研究提供了全面的开源工具，促进了方法标准化和可复现性，有助于加速该领域的研究进展
...
Conclusion: 通过显式建模视觉退化的结构化推理链，Robust-R1实现了最先进的鲁棒性，为MLLM在极端真实世界视觉退化下的实用鲁棒性提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需要固定输入视角和大量3D训练数据，限制了语言嵌入3D高斯重建的应用范围

Method: 提出无3D标注训练框架，利用大规模视频数据；实例引导对比学习对齐2D语义与3D表示；几何-语义层次稀疏化策略降低计算成本

Result: 在任意稀疏或密集视角下高效重建语言嵌入3D高斯表示，在几何精度、外观保真度和语义对齐方面优于现有方法

Conclusion: FLEG实现了无需3D标注的任意视角语言嵌入3D重建，为大规模3D语义理解提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有3D人体网格恢复方法主要针对紧身服装，在处理宽松服装时性能较差。需要一种能够准确估计穿着多样服装的人体形状和姿态的方法。

Method: ClothHMR包含两个模块：1) 服装裁剪模块，通过身体语义估计和身体边缘预测来裁剪服装使其贴合身体轮廓；2) 基于FHVM的网格恢复模块，通过将3D网格的中间表示与基础人体视觉模型的推断结果持续对齐来优化初始参数。

Result: ClothHMR在基准数据集和真实世界图像上显著优于现有最先进方法，能够准确恢复穿着多样服装的3D人体网格，精确估计身体形状和姿态。

Conclusion: ClothHMR通过服装裁剪和基础视觉模型对齐有效解决了宽松服装下的3D人体网格恢复问题，已开发出在线时尚购物应用，展示了实际应用价值。
...
TL;DR: G3Splat：通过几何先验增强3D高斯泼溅表示，解决仅依赖视图合成监督的几何模糊问题，在几何重建、姿态估计和新视角合成方面实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于多视图结构预测网络的方法主要依赖视图合成监督来回归3D高斯参数，但仅凭视图合成损失无法恢复几何上有意义的泼溅表示，存在几何模糊问题。

Method: 提出G3Splat方法，在自监督姿态无关的通用泼溅学习中，通过强制几何先验来获得几何一致的3D场景表示，解决3D高斯泼溅学习的模糊性问题。

Result: 在RE10K数据集上训练，在几何一致重建、相对姿态估计和新视角合成方面达到SOTA性能；在ScanNet上展示强大的零样本泛化能力，在几何恢复和姿态估计方面显著优于先前工作。

Conclusion: 仅依赖视图合成监督不足以学习几何有意义的3D高斯泼溅，通过引入几何先验的G3Splat方法能够获得几何一致的场景表示，在多个任务上实现优异性能。
...
TL;DR: 该研究开发了一个统一的FLAIR高信号分割模型，使用Attention U-Net架构在约5000个不同肿瘤类型和采集时间点的FLAIR图像上进行训练，实现了跨肿瘤类型和采集时间点的泛化能力。
...
Result: 统一模型在不同肿瘤类型上取得良好性能：术前脑膜瘤平均Dice分数88.65%，术前转移瘤80.08%，术前胶质瘤90.92%，术后胶质瘤84.60%，术前低级别胶质瘤84.47%，术后低级别胶质瘤61.27%。统一模型在各自数据集上与数据集特定模型性能相当，并实现跨肿瘤类型和采集时间点的泛化。
...
Method: 提出RoomEditor++扩散架构，采用参数共享的双扩散主干（兼容U-Net和DiT），统一参考图像和背景图像的特征提取和修复过程；同时构建RoomBench++数据集，包含112,851个训练对和1,832个测试对

Result: RoomEditor++在定量指标、定性评估和人类偏好研究中均优于现有方法，展现出对未见室内场景和一般场景的强大泛化能力，无需任务特定微调

Conclusion: RoomEditor++结合RoomBench++数据集为虚拟家具合成提供了有效的解决方案，在保持几何一致性和视觉真实性的同时实现了高质量的家具合成，推动了该领域的发展
...
TL;DR: 提出首个针对单热点调制视频SCI的算法，将重建转化为视频修复问题，结合回归初始化与扩散细化，并采用双光路硬件设计缓解空间退化。
...
Method: 1) 利用单热点调制解耦特性将重建转化为生成式视频修复问题，设计与硬件压缩过程对齐的SDE前向过程；2) 提出一步回归初始化+一步扩散细化的混合框架；3) 硬件层面采用双光路设计，利用互补信息增强修复视频质量。

Result: 在合成数据集和真实场景实验中验证了方法的有效性，这是首个将扩散模型集成到视频SCI重建中的工作。

Conclusion: 提出的算法成功解决了单热点调制视频SCI的重建问题，通过混合框架和双光路设计有效缓解了时间混叠和空间退化，为视频压缩成像提供了新思路。
...
TL;DR: 该研究系统评估了241个生物医学图像分析竞赛的数据集，发现存在地域、模态和问题类型偏见，且数据访问受限、文档不完整，导致基准测试与临床实际脱节。


<details>
  <summary>Details</summary>
Motivation: 虽然基准测试竞赛对医学影像AI发展至关重要，但尚不清楚这些基准是否提供足够代表性、可访问和可重用的数据来支持具有临床意义的AI。本研究旨在评估基准测试的公平性，包括数据集是否代表真实世界临床多样性，以及是否符合FAIR原则。

Method: 对241个生物医学图像分析竞赛（涵盖458个任务、19种成像模态）进行大规模系统研究，从两个维度评估公平性：1）数据集是否代表真实世界临床多样性；2）数据集是否可访问且符合FAIR原则。

Result: 研究发现数据集组成存在显著偏见，包括地理位置、模态和问题类型相关偏见，表明当前基准测试未能充分反映真实世界临床多样性。尽管影响广泛，但竞赛数据集常受限于限制性或模糊的访问条件、不一致的许可实践和不完整文档，限制了可重复性和长期重用。

Conclusion: 这些缺陷暴露了基准测试生态系统的基础公平性限制，突显了排行榜成功与临床相关性之间的脱节。需要更公平、透明和临床相关的基准测试来推动医学影像AI的进步。
...
TL;DR: HeadHunt-VAD：一种无需调优的视频异常检测新范式，通过直接定位MLLM中异常敏感的注意力头来避免文本生成，实现高效准确的异常检测。


<details>
  <summary>Details</summary>
Motivation: 传统VAD方法依赖大量标注数据且计算成本高，而基于MLLM的免调优方法虽然前景好，但依赖文本输出会导致信息损失、正常性偏差和提示敏感性，难以捕捉细微异常线索。

Method: 提出HeadHunt-VAD范式，绕过文本生成，直接在多模态大语言模型内部寻找稳健的异常敏感注意力头。核心是鲁棒头识别模块，通过显著性和稳定性的多标准分析系统评估所有注意力头，识别出跨不同提示都具判别性的稀疏头子集。这些专家头的特征输入轻量级异常评分器和时序定位器。

Result: 在两个主要VAD基准测试中，HeadHunt-VAD在免调优方法中实现了最先进的性能，同时保持高效率。

Conclusion: 验证了在MLLMs中进行头级探测作为现实世界异常检测的强大实用解决方案，为视频异常检测提供了高效准确的新方法。
...
TL;DR: MGRegBench是首个大规模公开的乳腺X光配准基准数据集，包含5000多对图像和100对人工标注的解剖标志点，用于公平比较各种配准方法。
...
Method: 创建MGRegBench数据集，包含5000多对图像，其中100对包含人工标注的解剖标志点和分割掩码。在此基础上，对多种配准方法进行基准测试，包括经典方法（ANTs）、学习基方法（VoxelMorph、TransMorph）、隐式神经表示（IDIR）、经典乳腺X光特定方法和最新深度学习方法（MammoRegNet）。

Result: MGRegBench成为最大的公开2D配准数据集之一。通过该资源，首次在乳腺X光模态上对多种方法进行了公平比较，并对深度学习基配准方法进行了深入分析。

Conclusion: MGRegBench为乳腺X光配准研究提供了首个大规模公开基准，建立了公平比较的基础资源，将推动该领域的未来研究。代码和数据已公开。
...
TL;DR: 提出一种用于多输出分割模型的半监督学习方法，通过旋转和翻转增强，不依赖概率假设，适用于医学图像分割任务。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割需要大量高质量标注数据，但获取成本高昂。多输出CNN模型能提高分割精度，但现有半监督方法对其研究不足。

Method: 基于旋转和翻转的数据增强方法，不假设模型响应的概率特性，适用于具有独立输出分支的多输出分割架构。

Result: 该方法在ImageTBDA数据集上验证，能够有效利用未标注数据提升多输出分割模型的性能。

Conclusion: 提出的通用半监督学习方法能有效解决多输出分割模型的标注数据不足问题，为医学图像分割提供实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 高分辨率定量MRI弛豫测量提供客观组织表征，但采集时间长限制了临床应用。需要开发无需高分辨率qMRI真值训练的超分辨率方法

Method: 提出物理信息自监督框架，将超分辨率建模为贝叶斯最大后验推断，最小化两个差异：1）超分辨qMRI图合成图像与加权MRI引导间的差异；2）低分辨率qMRI与下采样预测间的差异。使用深度神经网络学习超分辨率映射

Result: 消融实验显示T1加权图像主要增强T1图，T2加权图像改善T2图，组合引导同时优化所有参数。在不同qMRI序列的独立体内数据验证了跨序列泛化能力。1分钟采集的超分辨图质量与5分钟参考扫描相当

Conclusion: 通过解耦训练与高分辨率qMRI需求，该框架利用常规临床图像增强快速qMRI采集，为定量弛豫测量融入临床工作流提供实用途径，仅需可接受的额外扫描时间
...
TL;DR: StereoMV2D通过跨时间立体建模增强多视角3D目标检测，利用相邻帧间的视差信息解决单帧2D检测的深度模糊问题，提升检测精度而不显著增加计算开销。


<details>
  <summary>Details</summary>
Motivation: 多视角3D目标检测需要在检测精度和计算效率之间取得平衡。现有基于稀疏查询的检测器利用2D检测结果初始化查询，但单帧2D检测存在固有的深度模糊问题，限制了3D查询生成的准确性。

Method: 提出StereoMV2D框架，将时间立体建模集成到2D检测引导的多视角3D检测器中。通过利用相邻帧间同一目标的跨时间视差增强深度感知，在2D感兴趣区域内高效计算。引入动态置信度门控机制，通过学习帧间匹配矩阵的统计模式和外观一致性来自适应评估时间立体线索的可靠性。

Result: 在nuScenes和Argoverse 2数据集上的大量实验表明，StereoMV2D实现了优越的检测性能，且没有显著增加计算开销。

Conclusion: StereoMV2D通过时间立体建模有效解决了2D检测的深度模糊问题，提升了多视角3D目标检测的精度，同时保持了计算效率，为自动驾驶感知提供了更可靠的解决方案。
...
TL;DR: PathFLIP是一个用于全切片图像（WSI）解释的病理学细粒度语言-图像预训练框架，通过将幻灯片级描述分解为区域级子描述，生成文本条件区域嵌入，实现精确的视觉-语言对齐，在多个任务上优于现有方法且需要更少训练数据。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在计算病理学中取得进展，但全切片图像的千兆像素尺度和空间异质性给多模态理解带来挑战。现有对齐方法难以捕捉数千个图像块中文本描述与视觉线索之间的细粒度对应关系，影响下游任务性能。

Method: PathFLIP将幻灯片级描述分解为区域级子描述，生成文本条件区域嵌入以促进精确的视觉-语言对齐。利用大语言模型（LLMs）无缝遵循多样化的临床指令并适应不同的诊断场景。

Result: 在四个代表性基准测试中，PathFLIP优于现有的大规模病理学视觉语言模型，同时需要显著更少的训练数据。能够高效处理幻灯片级分类和检索、细粒度病变定位以及指令跟随等任务。

Conclusion: PathFLIP为临床实践中实现细粒度、指令感知的全切片图像解释铺平了道路，展示了在多种范式下的多功能能力，适应不同的临床指令和诊断场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有HOI检测方法基于封闭世界假设，只能识别预定义的小规模动词集，难以泛化到现实世界中未见或模糊的交互。虽然MLLM具备开放词汇理解所需的世界知识，但因其计算成本高昂而无法与现有HOI检测器结合。

Method: 提出GRASP-HO框架：1) 提取混合交互表示；2) 设计轻量级可学习的认知引导模块(CSC)，将细粒度视觉证据注入冻结的MLLM进行推理；3) 引入混合指导策略，结合语言建模损失和辅助分类损失，解决分类数据集与生成模型的监督不匹配问题。

Result: 实验展示了最先进的封闭集性能和强大的零样本泛化能力，实现了判别感知与生成推理的统一范式，适用于开放世界的HOI检测。

Conclusion: GRASP-HO成功地将HOI检测从封闭集分类任务重构为开放词汇生成问题，通过轻量级认知引导模块有效结合视觉感知与MLLM的推理能力，为开放世界HOI检测提供了统一框架。
...
TL;DR: ReCo提出了一种新的教学视频编辑范式，通过联合去噪和正则化约束建模，解决编辑区域不准确和token干扰问题，并构建了大规模数据集ReCo-Data。
...
Method: ReCo采用宽度级联源视频和目标视频进行联合去噪，并引入两种正则化：潜在正则化（增加编辑区域差异，减少非编辑区域差异）和注意力正则化（抑制编辑区域token对源视频对应token的注意力）。同时构建了50万对指令-视频数据集ReCo-Data。

Result: 在四种主要的基于指令的视频编辑任务上进行了广泛实验，证明了该方法的优越性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管AI视频分析技术已能精确量化面部表情、头部动作等行为，但现有工具主要面向工程背景用户，软件栈复杂，难以直接用于假设驱动的研究，阻碍了行为科学和临床研究的广泛采用。

Method: Bitbox基于可重复性、模块化和可解释性原则设计，提供标准化接口，整合多种面部、头部和身体处理模块，支持轻松添加新测量指标，已在临床样本上测试验证。

Result: Bitbox为行为研究人员提供无需工程专业知识的高级行为指标，同时为计算机科学家提供向实际应用领域传播方法的机制，旨在加速计算行为测量在行为、临床和心理健康研究中的整合。

Conclusion: Bitbox作为社区驱动的开源工具，有望降低计算行为测量的使用门槛，弥合转化差距，促进AI视频分析技术在心理学、精神病学和神经科学研究中的广泛应用。
...
TL;DR: ST-Gaze：结合CNN骨干网络、通道注意力与自注意力模块的视频注视估计方法，通过时空建模在EVE数据集上取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 视频注视估计需要捕捉人类注视的时空动态特性，现有方法在空间和时间关系建模上存在局限，性能受限于单帧特征表示和帧间关系

Method: 提出ST-Gaze模型，结合CNN骨干网络、专用通道注意力和自注意力模块，融合眼部和面部特征，将融合特征视为空间序列捕捉帧内上下文，并通过时间传播建模帧间动态

Result: 在EVE数据集上实现最先进性能（无论是否进行个性化适应），消融研究表明：通过时空循环保持和建模帧内空间上下文比过早空间池化更优越

Conclusion: ST-Gaze为使用普通摄像头实现更鲁棒的视频注视估计铺平了道路，时空建模方法在视频注视估计中具有根本优势
...
TL;DR: 扩散模型在图像超分辨率等逆问题中表现出色，但现有条件化方法需要大量超参数调优。研究发现条件化步长比扩散步数对性能影响更大，最佳步长范围为[2.0, 3.0]。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在单图像超分辨率等逆问题中表现出强大潜力，但现有条件化方法如DPS和MCG虽然能显著提升重建质量，却引入了需要仔细调优的额外超参数。这在实际应用中带来了复杂性。

Method: 在FFHQ超分辨率任务上进行实证消融研究，分析应用条件化到预训练扩散模型时影响性能的主要因素。特别关注条件化步长和扩散步数这两个关键超参数。

Result: 研究发现条件化步长对性能的影响显著大于扩散步数的影响。实验表明，步长在[2.0, 3.0]范围内能获得最佳整体性能。

Conclusion: 条件化步长是影响扩散模型在逆问题中性能的关键因素，优化步长参数比调整扩散步数更为重要。这为简化超参数调优提供了实用指导。
...
TL;DR: FlexAvatar是一个灵活的3D头部重建模型，可以从单张或稀疏图像生成高保真、具有详细动态形变的3D头像，无需相机姿态或表情标签。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要相机姿态或表情标签，难以从单张或稀疏图像生成具有详细动态形变的3D头像，特别是在捕捉皱纹、露齿等关键表情细节方面存在挑战。

Method: 1) 使用基于transformer的重建模型，以结构化头部查询token作为规范锚点，聚合灵活数量的输入；2) 引入轻量级UNet解码器，基于UV空间位置图生成详细表情相关形变；3) 采用数据分布调整策略平衡训练集中关键表情；4) 10秒轻量级细化增强特定身份细节。

Result: 实验表明FlexAvatar在3D一致性、详细动态真实感方面优于先前方法，为可动画3D头像创建提供了实用解决方案。

Conclusion: FlexAvatar实现了从单张或稀疏图像生成高保真3D头像，无需相机姿态或表情标签，通过创新的架构和训练策略实现了详细的动态形变，具有实际应用价值。
...
TL;DR: SAVeD是一个从社交媒体收集的大规模视频数据集，专注于ADAS车辆相关的碰撞、险情和系统失效事件，包含2119个第一人称视角视频，提供帧级标注，用于分析感知和决策失败。


<details>
  <summary>Details</summary>
Motivation: 现有数据集主要局限于模拟环境或人类驾驶车辆数据，缺乏真实ADAS车辆在风险条件下的行为数据，这限制了安全关键驾驶行为研究的发展。

Method: 从公开社交媒体内容中收集并整理大规模视频数据集，包含帧级标注；提出结合语义分割和单目深度估计的实时碰撞时间计算框架；使用广义极值分布建模不同道路类型的极端风险；建立VLLM基准测试。

Result: 创建了包含2119个第一人称视频的SAVeD数据集，涵盖多种位置、光照和天气条件；提出的TTC计算框架有效；GEV分布成功量化了不同道路类型的极端风险；VLLM基准测试显示SAVeD的详细标注通过领域适应显著提升了模型在复杂险情场景中的性能。

Conclusion: SAVeD填补了ADAS车辆真实风险行为数据集的空白，为安全关键驾驶行为研究提供了重要资源，其标注和分析方法有助于理解ADAS系统的感知和决策失败，推动自动驾驶安全研究发展。
...
TL;DR: MambaMIL+：一种新的多实例学习框架，通过重叠扫描、选择性条纹位置编码和上下文令牌选择机制，在计算病理学中有效建模全切片图像的超长序列和空间上下文。


<details>
  <summary>Details</summary>
Motivation: 全切片图像（WSIs）具有千兆像素分辨率且缺乏细粒度标注，传统深度学习模型难以处理。多实例学习（MIL）将WSI视为补丁级实例的包，但有效建模超长序列和丰富空间上下文仍然困难。Mamba虽然在线性扩展方面有优势，但在空间上下文建模和内存衰减方面存在局限。

Method: 提出MambaMIL+框架：1）重叠扫描：重构补丁序列以嵌入空间连续性和实例相关性；2）选择性条纹位置编码（S2PE）：编码位置信息同时减轻固定扫描顺序的偏差；3）上下文令牌选择（CTS）：利用监督知识动态扩展上下文内存以实现稳定的长程建模。

Result: 在20个基准测试上进行广泛实验，涵盖诊断分类、分子预测和生存分析。在三种特征提取器（ResNet-50、PLIP和CONCH）下，MambaMIL+始终实现最先进的性能，展示了其在计算病理学中的有效性和鲁棒性。

Conclusion: MambaMIL+通过显式集成空间上下文同时保持长程依赖建模而不遗忘内存，解决了现有MIL和Mamba方法在WSI分析中的局限性，为大规模计算病理学提供了有效且鲁棒的解决方案。
...
TL;DR: 提出Diff-Gen数据集和AdaptPrompt框架，利用CLIP模型实现跨域深度伪造检测，在25个测试集上达到SOTA性能，支持少样本泛化和生成器溯源。


<details>
  <summary>Details</summary>
Motivation: 随着图像生成技术的进步，深度伪造检测面临泛化性挑战，现有检测器在遇到未见过的生成模型时性能下降。需要开发能够跨不同生成技术识别合成内容的通用检测方法。

Method: 1. 构建Diff-Gen大规模基准数据集，包含10万张扩散生成图像，捕捉广泛的光谱伪影；2. 提出AdaptPrompt参数高效迁移学习框架，联合学习任务特定的文本提示和视觉适配器，同时冻结CLIP骨干网络；3. 通过层剪枝策略，修剪视觉编码器的最后一个Transformer块以保留高频生成伪影。

Result: 在25个挑战性测试集（涵盖GAN、扩散模型和商业工具生成的合成内容）上建立了新的最先进性能，在标准和跨域场景中均表现优异。仅使用320张图像即可实现少样本泛化，并在闭集设置中实现生成器架构的精确溯源。

Conclusion: 通过Diff-Gen数据集和AdaptPrompt框架，成功利用大型视觉语言模型实现了跨生成技术的通用深度伪造检测，解决了现有检测器的泛化性问题，为实际应用提供了有效的解决方案。
...
TL;DR: Pix2NPHM：使用视觉Transformer从单张图像直接回归神经参数化头部模型参数，实现高保真3D人脸重建


<details>
  <summary>Details</summary>
Motivation: 神经参数化头部模型(NPHMs)相比传统3DMM能提供更高几何细节，但将其拟合到视觉输入具有挑战性，因为其潜在空间表达性强。需要一种能够直接从单张图像回归NPHM参数的方法。

Method: 提出Pix2NPHM，基于视觉Transformer架构，使用特定领域预训练的ViT作为骨干网络。在混合3D数据上训练，包括超过10万NPHM配准数据（提供SDF空间直接监督）和大规模2D视频数据（使用法线估计作为伪地面真值几何）。支持推理时优化进一步提升几何保真度。

Result: 相比现有方法，能够重建更可识别的人脸几何和更准确的表情。在交互帧率下实现3D重建，通过推理时优化可进一步提升几何保真度，在野外数据上实现前所未有的面部重建质量。

Conclusion: Pix2NPHM通过直接回归NPHM参数的方法，成功解决了神经参数化头部模型拟合到视觉输入的挑战，实现了高质量、可扩展的3D人脸重建。
...
TL;DR: LiteGE：一种轻量级方法，通过PCA处理UDF样本构建紧凑的形状描述符，用于高效计算3D表面测地距离，相比现有方法减少300倍内存和推理时间，支持稀疏点云输入，并能实现快速形状匹配。
...
Method: LiteGE通过应用主成分分析（PCA）处理信息体素上的无符号距离场（UDF）样本，构建紧凑的类别感知形状描述符。该方法无需高容量网络，支持稀疏点云输入（最少300个点）。

Result: LiteGE相比现有神经方法减少高达300倍的内存使用和推理时间。在非等距形状对上保持可比精度的同时，相比最先进的基于网格的方法实现高达1000倍的加速。支持点云输入，在稀疏点云上表现稳健。

Conclusion: LiteGE提供了一种高效轻量的3D测地距离计算方法，解决了现有方法的内存和延迟问题，同时通过利用测地距离与形状对应的内在关系，实现了快速准确的形状匹配。
...
Result: 在20-85%云覆盖的合成云掩模实验中，UrbanDIFF始终优于插值基线，特别是在密集云遮挡下，在85%云覆盖时达到SSIM 0.89、RMSE 1.2K、R2 0.84，且随着云密度增加性能下降更慢

Conclusion: UrbanDIFF展示了去噪扩散模型在空间地表温度重建中的有效性，特别是在高云覆盖条件下，为城市热岛连续监测提供了可靠工具
...
Method: 混合方法：1）扩展传统畸变模型加入高阶项；2）使用神经网络进行残差校正；3）将3D坐标转换为GIS坐标进行可视化

Result: 显著提升长距离定位性能，能够估计5公里距离物体的3D位置，实验验证了框架的鲁棒性和有效性

Conclusion: 该混合框架为CCTV相机长距离摄影测量应用提供了实用校准方案，解决了传统畸变模型在长距离定位中的局限性
...
TL;DR: AniX：结合静态世界生成与可控实体模型，支持用户指定角色在3DGS场景中执行开放动作的视频生成系统


<details>
  <summary>Details</summary>
Motivation: 现有世界模型分为两类：静态世界生成模型（无主动代理）和可控实体模型（单个实体有限动作）。需要结合两者的优势，支持用户指定角色在静态环境中执行开放动作。

Method: 基于预训练视频生成器，将问题建模为条件自回归视频生成。用户提供3DGS场景和角色，通过自然语言指导角色执行从基础运动到物体交互的多样化行为。

Result: AniX能合成时间连贯的视频片段，保持场景和角色的视觉保真度。训练策略显著增强了运动动态，同时在动作和角色间保持泛化能力。

Conclusion: AniX成功结合了静态世界生成的真实性和可控实体模型的交互性，支持用户指导角色在3D环境中执行开放动作，在视觉质量、角色一致性、动作可控性和长时程连贯性方面表现良好。
...
<details>
  <summary>Details</summary>
Motivation: 虽然3DGS已成为高保真场景表示方法，但直接从其基元编码丰富、通用的特征仍未充分探索。本文旨在填补这一空白，通过从2D基础模型蒸馏信号来学习3DGS场景编码器。

Method: 提出Chorus框架，使用共享3D编码器和教师特定投影器，从语言对齐、通用和物体感知的教师模型中学习，鼓励共享嵌入空间捕获从高级语义到细粒度结构的信号。

Result: 在开放词汇语义和实例分割、线性和解码器探测以及数据高效监督等任务上表现优异。仅使用高斯中心、颜色和估计法线作为输入的变体在点云基准测试中表现优于基线，且训练场景减少39.9倍。

Conclusion: Chorus成功学习了一个通用的3DGS场景编码器，通过多教师蒸馏实现了强大的3D场景理解能力，并提出了渲染-蒸馏适应方法促进域外微调。
...
TL;DR: ReX-MLE是一个针对医学影像领域的自主编码代理基准测试，包含20个挑战任务，评估代理在完整端到端工作流程中的表现，结果显示当前最先进的代理在复杂医学影像任务上表现远低于人类专家。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的自主编码代理在通用软件和机器学习任务上表现良好，但在复杂、领域特定的科学问题上仍然无效。医学影像领域尤其具有挑战性，需要长训练周期、高维数据处理以及专门的预处理和验证流程，这些能力在现有代理基准测试中未能充分衡量。

Method: 引入ReX-MLE基准测试，包含20个从高影响力医学影像竞赛中提取的挑战任务，涵盖多种模态和任务类型。该基准测试评估完整的端到端工作流程，要求代理在现实的算力和时间约束下独立管理数据预处理、模型训练和提交。
...
Conclusion: ReX-MLE揭示了当前自主编码代理在复杂医学影像任务上的瓶颈，为开发领域感知的自主AI系统提供了基础。该基准测试暴露了代理在领域专业知识和工程实践方面的局限性，强调了开发能够处理复杂科学工作流程的AI系统的必要性。
...
TL;DR: InfSplign是一种无需训练、推理时的方法，通过在每个去噪步骤中调整噪声来改善文本到图像扩散模型的空间对齐能力，在多个基准测试中达到新的SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像扩散模型在捕捉文本提示中的空间关系方面存在不足，主要原因是训练数据缺乏细粒度空间监督，以及文本嵌入无法编码空间语义。

Method: 提出InfSplign方法，在推理时通过复合损失调整每个去噪步骤的噪声。该方法利用从骨干解码器提取的不同层级的交叉注意力图，确保准确的物体放置和平衡的物体存在。

Result: 在VISOR和T2I-CompBench上的综合评估显示，InfSplign建立了新的SOTA，显著优于现有的推理时基线方法，甚至超过了基于微调的方法。
...
Method: 开发模拟驱动的去噪框架：1）建立统计基础的噪声模型，全面建模主要噪声源；2）基于模型生成生物真实的拉曼光谱；3）训练级联深度神经网络，联合抑制随机探测器噪声和荧光基线干扰

Result: 使用真实实验数据模拟的人体皮肤光谱作为验证案例，结果表明该方法能显著提高光谱质量，实现更快更准确的拉曼组织分析

Conclusion: 物理信息学习在改善光谱质量和加速拉曼组织分析方面具有巨大潜力，为生物医学诊断提供了有效解决方案
...
TL;DR: 提出CBAM-VGG16模型，通过集成卷积块注意力模块(CBAM)增强植物叶片病害检测的准确性和可解释性，在多个数据集上达到98.87%的准确率。
...
Result: 在五个不同的植物病害数据集上训练，模型优于现有技术，准确率高达98.87%，表现出强大的泛化能力。通过多种可视化方法验证了模型的可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 传统扩散模型将数据完全扩散为白噪声，然后重建，这导致预测任务极其困难且计算不可行。需要克服这一限制，同时保持生成多样性和质量。

Method: 提出InSPECT（不变频谱特征保持扩散模型），在前后向过程中保持不变的频谱特征。前向过程结束时，傅里叶系数平滑收敛到指定随机噪声，实现特征保持的同时维持多样性和随机性。

Result: 在CIFAR-10、Celeb-A和LSUN数据集上，InSPECT在10K迭代下相比DDPM平均降低FID 39.23%，提高IS 45.80%。模型表现出增强的视觉多样性、更快的收敛速度和更平滑的扩散过程。

Conclusion: 保持不变的频谱特征在扩散模型中具有显著优势：实现更优的生成质量和多样性，同时提高计算效率并实现更快的收敛速度。这是首次在扩散模型中分析和保持不变频谱特征的尝试。
...
TL;DR: 现有视觉语言模型在视觉提示基准测试中表现脆弱，微小细节变化（如标记颜色、大小）就能显著改变模型排名，甚至让较弱模型超越较强模型。


<details>
  <summary>Details</summary>
Motivation: 当前VLM评估存在缺陷：视觉提示基准测试对看似无关的细节（如视觉标记设计）过于敏感，导致模型排名不稳定，无法准确反映模型真实视觉感知能力。

Method: 评估9个常用开源和闭源VLM在两个视觉提示任务上的表现，分析视觉标记设计、数据集大小等基准设置细节对性能的影响，并创建VPBench基准（包含16种视觉标记变体）。

Result: 视觉标记的微小变化（如颜色从红变蓝、大小略微增加）能完全改变模型排名；低层推理选择（如JPEG压缩级别）也能导致排名变化；这些影响远大于传统语义VLM评估。

Conclusion: 视觉提示基准测试存在严重不稳定性问题，需要更鲁棒的评估方法；VPBench提供了更全面的测试集来缓解这一问题，并揭示了当前VLM评估的脆弱性。
...
<details>
  <summary>Details</summary>
Motivation: 当前自解释模型需要复杂的训练过程和特定架构，不实用。随着基于ViT的基础模型发展，这种不实用性变得更加严重，需要新方法来为ViT基础模型提供透明性和可靠性

Method: 提出关键点计数分类器(KCCs)，基于ViT自动识别图像间匹配关键点的能力，创建易于解释的决策过程，可在输入中可视化。该方法可将任何训练好的ViT模型转换为自解释模型，无需重新训练

Result: 广泛评估显示，KCCs相比最近基线方法改善了人机通信效果

Conclusion: KCCs是使ViT基础模型更加透明和可靠的重要一步
...
TL;DR: RadarGen：基于扩散模型，从多视角相机图像生成逼真的汽车雷达点云，通过BEV表示和视觉线索引导，提升多模态生成仿真的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有雷达数据获取成本高且有限，需要一种能够从丰富的视觉数据生成逼真雷达点云的方法，以实现跨传感模态的统一生成仿真。

Method: 1. 将雷达测量表示为鸟瞰图形式，编码空间结构、雷达截面积和速度属性；2. 采用高效的图像-潜在扩散模型适应雷达域；3. 引入轻量级恢复步骤从生成的地图重建点云；4. 利用预训练基础模型提取BEV对齐的深度、语义和运动线索，引导生成过程。

Result: 在大规模驾驶数据上的评估表明，RadarGen能够捕捉特征性的雷达测量分布，并缩小感知模型在真实数据上训练的差距，展示了跨传感模态统一生成仿真的潜力。

Conclusion: RadarGen通过从视觉图像生成逼真雷达点云，为多模态生成仿真提供了可扩展的方向，是迈向跨传感模态统一生成仿真的重要一步。
...
TL;DR: MAGNet：基于扩散强迫变换器的统一多智能体运动生成框架，支持多种交互任务和可变智能体数量


<details>
  <summary>Details</summary>
Motivation: 理解和生成多人交互对机器人和社会计算至关重要，但现有方法多为任务特定且难以泛化到灵活的多智能体场景，面临长时间跨度、强智能体间依赖和可变群体规模等挑战

Method: 提出MAGNet（多智能体扩散强迫变换器），基于扩散强迫框架引入关键修改，在自回归去噪过程中显式建模智能体间耦合，支持二元预测、伙伴修复和完整多智能体运动生成，架构可扩展且对智能体数量不敏感

Result: 在二元基准测试中与专门方法表现相当，同时自然扩展到涉及三个或更多人的多元场景，能捕捉紧密同步活动（如舞蹈、拳击）和松散结构社交互动，可自回归生成数百帧的超长序列

Conclusion: MAGNet为多智能体运动生成提供了统一框架，通过灵活的条件设置和采样支持广泛的交互任务，实现了跨智能体的协调运动生成，为解决多人交互建模难题提供了有效方案
...
TL;DR: 论文研究了LLaVA-1.5-13B和Meta Llama 3.2 Vision-8B-2两种视觉语言模型对抗攻击的鲁棒性，发现尽管Llama 3.2 Vision基准准确率较低，但在对抗攻击下性能下降更小。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习模型复杂度增加，理解AI系统识别物体的机制变得困难，攻击者可能通过添加不可见元素来混淆AI识别。因此需要研究当代视觉语言模型在视觉模态上的对抗鲁棒性。

Method: 使用无目标PGD（投影梯度下降）攻击视觉输入模态，在VQA v2数据集子集上对LLaVA-1.5-13B和Meta Llama 3.2 Vision-8B-2进行实证评估，使用标准VQA准确率指标量化攻击效果。

Result: Llama 3.2 Vision在基准准确率较低的情况下，在对抗攻击下表现出更小的性能下降，特别是在较高扰动水平时。视觉模态是降低当代开放权重VLM性能的有效攻击向量。

Conclusion: 对抗鲁棒性不一定与标准基准性能直接相关，可能受到底层架构和训练因素的影响。视觉模态是当代视觉语言模型的重要攻击面。
...
TL;DR: DWM是一个场景-动作条件视频扩散框架，通过静态3D场景渲染和手部运动序列生成逼真的人-场景交互视频，为交互式数字孪生提供基础。


<details>
  <summary>Details</summary>
Motivation: 当前3D重建技术能创建逼真的数字孪生，但主要限于静态场景，缺乏具身交互能力。需要一种方法将人类灵巧动作与3D场景动态变化结合起来，实现交互式数字孪生。

Method: 提出Dexterous World Model (DWM)，基于视频扩散框架，条件输入包括：(1) 指定相机轨迹的静态场景渲染确保空间一致性，(2) 编码几何和运动线索的自我中心手部网格渲染。使用混合交互视频数据集训练，结合合成数据和真实世界视频。

Result: DWM能够生成逼真且物理合理的交互视频，如抓取、打开和移动物体，同时保持相机和场景一致性。实验验证了框架的有效性。

Conclusion: DWM是首个基于视频扩散的交互式数字孪生框架，实现了从自我中心动作到具身仿真的转换，为交互式数字孪生发展迈出重要一步。
...
TL;DR: Re-Depth Anything：基于测试时自监督的框架，通过融合Depth Anything V2与大规模2D扩散模型先验，提升单目深度估计在真实世界图像上的性能。


<details>
  <summary>Details</summary>
Motivation: 当前基础模型（如Depth Anything V2）在处理与训练分布差异较大的真实世界图像时表现不佳，存在领域差距问题。需要一种方法能够在不依赖标注数据的情况下，在测试时自适应地提升深度估计质量。

Method: 提出测试时自监督框架，通过重新照明预测的深度图并增强输入图像，利用2D扩散模型的强大先验。采用基于形状从阴影（SfS）线索的重新合成方法，在生成式上下文中使用分数蒸馏采样（SDS）替代传统光度重建。为防止优化崩溃，采用针对性优化策略：冻结编码器，仅更新中间嵌入并微调解码器。

Result: 在多个基准测试中，Re-Depth Anything相比Depth Anything V2在深度准确性和真实感方面取得显著提升，展示了通过增强几何推理实现自监督的新途径。

Conclusion: Re-Depth Anything成功弥合了深度估计模型在真实世界图像上的领域差距，通过融合扩散模型先验和测试时自监督，为单目深度估计提供了有效的改进方案，开辟了基于生成模型增强几何推理的自监督新方向。
...
TL;DR: 本文提出了一种系统框架，将面向理解的编码器特征适配到生成任务中，通过语义-像素重建目标正则化潜在空间，实现紧凑表示，在文本到图像生成和图像编辑任务中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前潜在扩散模型通常使用低层VAE潜在空间，主要优化像素级重建。为了统一视觉生成和理解，新兴趋势是采用表示编码器的高维特征作为生成潜在空间。但实证发现两个基本障碍：1) 判别性特征空间缺乏紧凑正则化，导致扩散模型容易产生偏离流形的潜在表示，造成不准确的物体结构；2) 编码器固有的弱像素级重建能力阻碍生成器学习准确的细粒度几何和纹理。

Method: 提出系统框架适配面向理解的编码器特征用于生成任务。引入语义-像素重建目标来正则化潜在空间，将语义信息和细粒度细节压缩到高度紧凑的表示中（96通道，16×16空间下采样）。基于此表示设计统一的文本到图像生成和图像编辑模型。

Result: 与各种特征空间相比，该方法实现了最先进的图像重建、更快的收敛速度，在文本到图像生成和编辑任务中取得显著的性能提升，验证了表示编码器可以有效适配为强大的生成组件。

Conclusion: 通过语义-像素重建目标正则化潜在空间，可以将面向理解的编码器特征有效适配到生成任务中，实现紧凑且语义丰富的表示，为统一视觉生成和理解提供了有效解决方案。
