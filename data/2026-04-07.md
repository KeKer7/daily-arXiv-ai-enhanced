...
TL;DR: SafeScreen是一个安全优先的视频筛选框架，通过提取个性化安全标准、多模态视频分析和LLM决策，在满足个体安全约束的前提下推荐个性化视频，特别适用于儿童和护理场景。


<details>
  <summary>Details</summary>
Motivation: 开放域视频平台虽然提供丰富的个性化内容，但其以参与度优化的推荐算法可能让脆弱用户接触不当或有害内容，特别是在儿童导向和护理（如痴呆症护理）场景中，需要满足个体化安全约束。

Method: SafeScreen采用安全优先的框架，将安全视为前提条件，通过三个关键组件进行顺序审批：1）基于配置文件提取个体化安全标准；2）通过自适应问题生成和多模态VideoRAG分析进行证据基础评估；3）LLM决策验证安全性、适当性和相关性。

Result: 在痴呆症护理案例研究中，使用30个合成患者档案和90个测试查询，SafeScreen在80-93%的情况下优先考虑安全而非参与度，与YouTube的参与度优化排名形成鲜明对比，同时保持高水平的安全覆盖、合理性和基础性。

Conclusion: SafeScreen提供了一种可解释的实时视频筛选方法，无需依赖预计算的安全标签，能够有效保护脆弱用户免受有害内容影响，特别适用于需要个体化安全约束的护理和教育场景。
...
TL;DR: 提出基于SoC FPGA的智能摄像头平台，用于工业喷焰实时分割与表征，通过优化UNet模型实现125倍参数减少和7.5倍延迟提升，达到30FPS实时处理性能。
...
Method: 采用SoC FPGA平台（Ultra96），使用Vitis框架优化UNet分割模型，将750万参数减少到5.9万参数（125倍压缩），结合多线程和批归一化技术，实现模型在可重构逻辑上的高效并行执行。

Result: 优化后模型处理延迟降低7.5倍，达到30FPS实时性能，同时保持Dice Score评估指标精度不变，参数减少125倍（从750万到5.9万）。

Conclusion: 提出的智能摄像头平台成功实现了工业喷焰的实时边缘处理，为火灾安全管理提供了可行的解决方案，该框架可扩展到其他火灾安全应用场景。
...
TL;DR: SpikeVPR：结合事件相机和脉冲神经网络的生物启发视觉地点识别方法，在极端光照、视角变化下实现鲁棒识别，参数量减少50倍，能耗降低30-250倍。


<details>
  <summary>Details</summary>
Motivation: 传统深度网络在动态现实条件下的视觉地点识别存在计算和能耗高的问题，而哺乳动物导航系统提供了高效生物启发的解决方案灵感。

Method: 结合事件相机和脉冲神经网络，采用端到端的代理梯度学习训练，引入EventDilation增强策略提升对速度和时序变化的鲁棒性。
...
TL;DR: 提出3D-IDE方法，通过几何自监督让3D感知作为涌现特性自然产生，而非显式编码，实现零延迟开销的3D场景理解


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在室内场景理解中融合2D-3D表示存在权衡问题：显式3D位置编码和嫁接外部3D基础模型的方法都有局限性，导致部署效果不佳

Method: 提出3D-Implicit Depth Emergence方法，基于隐式几何涌现原理，通过细粒度几何验证器和全局表示约束构建信息瓶颈，迫使模型最大化视觉特征与3D结构的互信息

Result: 在多个3D场景理解基准测试中超越SOTA，推理延迟降低55%，同时在不同下游任务中保持强大性能

Conclusion: 从外部嫁接转向隐式涌现的范式转变，代表了视觉语言模型中3D知识整合的根本性重新思考，通过精心设计的辅助目标实现无依赖的3D理解
...
TL;DR: 提出Cross-Stage Attention Residuals (XAttnRes)，一种通过全局特征历史池和轻量级伪查询注意力实现跨阶段选择性聚合的机制，在分割网络中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 在大型语言模型中，注意力残差已证明通过学习性、选择性聚合所有前层输出可以超越固定残差连接。但在分割网络的编码器-解码器多尺度架构中，如何将这种机制应用于不同维度的Transformer层与多尺度阶段之间仍存在挑战。

Method: 提出XAttnRes机制：1）维护包含编码器和解码器阶段输出的全局特征历史池；2）通过轻量级伪查询注意力，每个阶段从所有先前表示中选择性聚合；3）引入空间对齐和通道投影步骤，以处理跨分辨率特征并最小化计算开销。

Result: 在四个数据集和三种成像模态上，XAttnRes一致提升了现有分割网络的性能。即使不使用跳跃连接，仅XAttnRes也能达到与基线相当的性能，表明学习性聚合可以恢复传统上由预定连接提供的跨阶段信息流。

Conclusion: XAttnRes通过学习性跨阶段聚合机制，有效提升了分割网络的性能，证明了在视觉任务中全局特征历史池和选择性注意力聚合的价值，为改进编码器-解码器架构提供了新思路。
...
Method: 提出MoViD框架：1) 视角估计器建模关键关节关系预测视角信息；2) 正交投影模块解耦运动和视角特征；3) 基于物理的跨视角对比对齐增强；4) 逐帧推理流水线，根据估计视角自适应激活翻转细化。

Result: 在9个公开数据集和新收集的多视角无人机、步态分析数据集上评估：相比SOTA方法减少24.2%姿态估计误差；在严重遮挡下保持鲁棒性能，训练数据减少60%；在NVIDIA边缘设备上实现15FPS实时推理。

Conclusion: MoViD通过解耦视角和运动特征，实现了视角不变的3D人体姿态估计，在精度、鲁棒性和效率方面均有显著提升，适合实际边缘部署。
...
TL;DR: 研究发现在严格的上行链路限制下（仅传输紧凑嵌入向量），基于嵌入向量的管道在遥感场景变化下仍然有效，但最佳决策头取决于具体任务：kNN检索在云分类上表现更好，而类质心在时序变化检测上更优。


<details>
  <summary>Details</summary>
Motivation: 为了解决下行链路瓶颈问题，需要开发能够在不上传原始像素的情况下优先处理危险事件的机载系统。研究探索在严格的上行链路限制下（仅传输紧凑嵌入向量和元数据），嵌入向量管道是否能在遥感场景变化（跨时间、跨事件/地点、跨站点云、跨城市AOI）下保持有效性。

Method: 使用OlmoEarth嵌入向量在扩展的公共多任务基准上进行评估（27个Sentinel-2 L2A场景，15个云站点，5个SpaceNet-2 AOI；10次随机种子）。机载系统执行向量搜索来分类新捕获数据，比较kNN检索和类质心两种决策头在不同任务上的表现。

Result: 所有有效方法都依赖于相同的上行嵌入向量，但最佳决策头是任务依赖的：kNN检索在云分类上显著优于质心方法（0.92 vs 0.91；p<0.01），而类质心在时序变化检测上表现更好（0.85 vs 0.48；p<0.01）。嵌入向量上行是实现的关键，一旦嵌入向量在机载系统中，系统可以按任务选择最佳决策头而无需额外上行成本，每次查询的遥测数据小于1KB。

Conclusion: 嵌入向量专用上行链路是可行的解决方案，能够在严格的带宽限制下实现有效的机载危险事件优先处理。系统设计应允许根据具体任务动态选择最佳决策头，从而在保持低通信开销的同时获得最佳性能。
...
TL;DR: 该论文指出当前多模态大语言模型在物理世界理解方面存在严重不足，特别是对连续体对象动态的理解。作者提出了两个基准任务来评估这种能力，并开发了Scene Dynamic Field方法，利用物理模拟器进行多任务微调，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在图像和视频理解方面表现出色，但在物理世界理解方面存在明显不足。当前模型在高层次物理推理方面表现不佳，特别是在理解连续体对象动态方面存在重大局限。需要专门评估和改进模型对物理世界的基本理解能力。

Method: 1. 提出了两个基础基准任务：Next Frame Selection（下一帧选择）和Temporal Coherence Verification（时间一致性验证），用于专门评估模型对连续体对象动态的理解能力。
2. 开发了Scene Dynamic Field方法，利用物理模拟器在多任务微调框架中增强模型的物理理解能力。

Result: 实验表明，即使是当前最先进的多模态大语言模型在这些基础任务上也表现不佳。提出的Scene Dynamic Field方法显著提升了性能，在流体任务上实现了高达20.7%的性能提升，并且在未见过的物理领域表现出强大的泛化能力。

Conclusion: 该研究不仅揭示了当前多模态大语言模型在物理理解方面的重要缺陷，还提出了一种有前景的成本效益方法，用于开发更具物理基础的多模态大语言模型。Scene Dynamic Field方法为解决模型物理理解不足问题提供了有效途径。
...
TL;DR: HVG-3D：基于3D控制信号的扩散模型框架，用于生成高质量、时空可控的手-物交互视频


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖2D控制信号，缺乏空间表达能力，且无法充分利用合成的3D条件数据，限制了手-物交互视频合成的质量和可控性。

Method: 提出HVG-3D统一框架，包含：1）基于扩散的3D感知HOI视频生成架构，通过3D ControlNet编码几何和运动线索实现显式3D推理；2）混合管道构建输入和条件信号，支持训练和推理阶段的灵活精确控制。

Result: 在TASTE-Rob数据集上，HVG-3D在空间保真度、时间一致性和可控性方面达到最先进水平，能够有效利用真实和模拟数据，生成高保真、时间一致的视频。

Conclusion: HVG-3D通过显式3D表示和混合控制管道，显著提升了手-物交互视频合成的质量、可控性和数据利用效率，为3D感知视频生成提供了有效解决方案。
...
TL;DR: 提出IDCL方法，首次将基于密度信息的模型训练策略引入图像聚类，通过课程学习方案和密度核心指导聚类分配，提升鲁棒性和性能
...
Method: IDCL方法：1) 基于输入数据密度信息设计课程学习方案，提供更合理的学习节奏；2) 使用密度核心而非单个聚类中心指导聚类分配

Result: 在基准数据集上与最先进聚类方法进行广泛比较，证明该方法在鲁棒性、快速收敛性以及对数据规模、聚类数量和图像内容的灵活性方面具有优越性

Conclusion: IDCL首次将密度信息驱动的模型训练策略引入图像聚类，通过创新的课程学习和密度核心机制，显著提升了深度聚类方法的鲁棒性和性能
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在细粒度任务中容易产生感知相关的幻觉，因为它们的推理主要局限于语言域，将视觉输入视为静态的、与推理无关的前置内容，而不是动态参与者。模型作为被动观察者，无法重新检查视觉细节来支撑其演进的推理状态。

Method: 提出V-Reflection框架，采用两阶段蒸馏策略：1) Box-Guided Compression模块通过显式空间定位建立稳定的像素到潜在目标；2) Dynamic Autoregressive Compression模块将模型隐藏状态映射为动态探针，询问全局视觉特征图。通过将BCM教师的空间专业知识蒸馏到DAC学生中，使模型内化定位任务关键证据的能力。

Result: 在六个感知密集型基准测试上的广泛实验证明了V-Reflection的有效性，显著缩小了细粒度感知差距。可视化确认潜在推理能够自主定位任务关键的视觉证据。

Conclusion: V-Reflection通过"思考-观察"视觉反射机制，将MLLMs从被动观察者转变为主动询问者，有效解决了细粒度任务中的感知幻觉问题，同时保持了端到端自回归解码的高效性。
...
TL;DR: 部署基于边缘计算的农田积水检测系统，使用树莓派级设备结合环境传感器，通过有限状态机动态选择本地或云端推理，在间歇性连接和移动计算预算下平衡精度、延迟和能耗。


<details>
  <summary>Details</summary>
Motivation: 农田积水威胁车辆通行和作物健康，需要实时检测。传统方法面临间歇性连接、移动计算预算有限、能耗与精度平衡等挑战，需要自适应系统在边缘设备上实现可靠检测。

Method: 使用树莓派级设备（可选Jetson加速），结合摄像头和环境传感器（湿度、压力、温度）。采用有限状态机作为架构决策引擎，根据连接状态和计算预算动态选择本地或云端推理。使用多模型YOLO集成进行图像评分，结合昼夜基线传感器融合调整检测阈值。所有决策按帧记录，支持硬件在环重放。

Result: 在10种配置和传感器变体上测试，使用相同农田序列和帧级真实标注。结果显示：自适应分层、多模型共识和昼夜传感器融合的组合相比静态本地基线提高了洪水检测性能，比始终使用云端推理的简单策略更节能，在真实农业环境中保持有界尾部延迟。

Conclusion: 提出的边缘架构通过自适应推理分层、多模型集成和环境传感器融合，在资源受限的农业环境中实现了高效、可靠的积水检测，平衡了精度、延迟和能耗需求。
...
TL;DR: TreeGaussian：基于树引导级联对比学习的框架，用于3D高斯溅射的层次语义理解，通过构建多级对象树和两阶段对比学习解决现有方法在层次结构表示和特征学习方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯溅射方法难以表示复杂场景中的层次3D语义结构和捕获整体-部分关系，同时密集成对比较和不一致的2D先验层次标签阻碍了特征学习，导致分割效果不佳。

Method: 1) 构建多级对象树以结构化学习对象-部分层次关系；2) 两阶段级联对比学习策略，从全局到局部逐步细化特征表示；3) 一致性分割检测机制和基于图的去噪模块，对齐跨视图分割模式并抑制不稳定高斯点。

Result: 在开放词汇3D对象选择、3D点云理解和消融研究中表现出有效性和鲁棒性，显著提升了分割一致性和质量。

Conclusion: TreeGaussian通过显式建模层次语义关系和减少对比监督冗余，成功解决了3D高斯溅射在复杂场景语义理解中的局限性，为神经场景理解提供了更强大的表示能力。
...
Method: 提出简单的推理时优化框架，受基于扩散的随机最优控制启发，通过控制能量目标函数显式正则化预训练扩散模型的过渡轨迹

Result: 推理时优化该目标函数能够产生具有保真度和时间连贯性的运动过渡，这是首个提供显式过渡建模的受控长距离人体运动生成通用框架

Conclusion: 该工作为解决长距离人体运动生成中的语义连贯过渡问题提供了有效的解决方案，特别适用于舞蹈编排等需要流畅跨风格转换的应用场景
...
TL;DR: PollutionNet是一个基于Vision Transformer的框架，通过整合Sentinel-5P卫星数据和地面观测，实现了对大气NO₂和SO₂的高精度监测，相比传统模型误差减少达14%。


<details>
  <summary>Details</summary>
Motivation: 传统大气污染物监测方法存在局限性：卫星观测空间覆盖广但数据不完整，地面传感器时间分辨率高但空间覆盖有限。需要一种能够整合两者优势的方法来准确评估NO₂和SO₂浓度，以支持环境政策和公共健康保护。

Method: 提出PollutionNet框架，基于Vision Transformer架构，整合Sentinel-5P TROPOMI垂直柱浓度数据与地面观测数据。利用自注意力机制捕捉传统CNN和RNN模型难以处理的复杂时空依赖关系。

Result: 在爱尔兰（2020-2021）的案例研究中，PollutionNet实现了最先进的性能：NO₂的RMSE为6.89 μg/m³，SO₂的RMSE为4.49 μg/m³，相比基线模型预测误差减少达14%。

Conclusion: PollutionNet不仅提高了预测精度，还提供了一个可扩展且数据高效的工具，能够在监测网络稀疏的地区进行稳健的污染评估。这展示了先进机器学习方法在增强气候相关空气质量研究、支持环境管理和可持续政策决策方面的潜力。
...
TL;DR: CardioSAM：结合SAM编码器与心脏专用解码器的混合架构，通过解剖拓扑先验和边界细化模块提升心脏MRI分割精度，在ACDC基准上达到93.39% Dice系数，超越nnU-Net和专家间一致性水平。


<details>
  <summary>Details</summary>
Motivation: 心脏MRI图像的手动分割耗时且存在观察者间差异，现有基础模型（如SAM）虽具强泛化能力但缺乏临床所需的边界精度，需要开发更精确的心脏结构分割方法。

Method: 提出CardioSAM混合架构：冻结SAM编码器用于通用特征提取，搭配轻量可训练的心脏专用解码器。解码器包含两个关键创新：1) 融入解剖拓扑先验的心脏专用注意力模块；2) 改善组织界面描绘的边界细化模块。

Result: 在ACDC基准测试中达到Dice系数93.39%、IoU 87.61%、像素精度99.20%、HD95 4.2mm。相比nnU-Net提升3.89% Dice，超过报告的专家间一致性水平（91.2%）。

Conclusion: CardioSAM通过结合SAM的泛化能力与心脏特定解剖知识，实现了可靠且临床适用的心脏分割，在精度上超越了现有方法和专家间一致性，具有临床应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有PEFT方法（如LoRA）在多模态任务中存在局限性，它们在各模态内独立操作，无法有效捕捉跨模态交互，导致双流架构中的基础模型适配效果不佳。

Method: 提出Cross-Modal Low-Rank Adaptation (CoLA)框架，在标准模态内适配路径基础上增加专门的跨模态适配路径，形成双路径设计，避免模态特定学习与跨模态学习之间的干扰。

Result: 在视觉语言（RefCOCO, RefCOCO+, RefCOCOg）和视听（AVE, AVS）基准测试中，CoLA持续优于LoRA，分别获得约3%和2%的相对增益，同时保持参数效率。CoLA首次实现了视觉定位的多任务PEFT框架。

Conclusion: CoLA通过引入跨模态适配路径，有效解决了多模态任务中基础模型的高效适配问题，为参数高效的多模态学习提供了新思路，填补了高效多模态适配的关键空白。
...
Method: 提出StoryBlender框架，采用三阶段流程：1) 语义-空间锚定：构建连续性记忆图，将全局资产与镜头特定变量解耦；2) 规范资产实例化：在统一坐标空间中实例化实体以保持视觉身份；3) 时空动态：通过视觉指标实现布局设计和电影化演变。系统采用分层多智能体架构，在验证循环中通过引擎验证反馈迭代自校正空间幻觉。

Result: 实验表明，StoryBlender在一致性和可编辑性方面显著优于基于扩散的方法和3D基础方法。生成的本地3D场景支持直接精确编辑摄像机和视觉资产，同时保持跨镜头连续性。

Conclusion: StoryBlender成功解决了故事板生成中跨镜头一致性和可编辑性的双重挑战，通过3D基础框架和故事中心反射方案，为视觉叙事提供了高效、可控的自动化解决方案。
...
TL;DR: 该论文研究了大型视觉语言模型中的注意力汇聚现象，将视觉汇聚分为两类：视觉编码器产生的V-汇聚和LLM深层产生的L-汇聚，揭示了全局先验与局部感知的权衡，并提出了无需任务监督的层间汇聚门控模块来平衡两者。


<details>
  <summary>Details</summary>
Motivation: 虽然单模态Transformer中的注意力汇聚已被研究，但在大型视觉语言模型中，跨模态的注意力汇聚影响仍不清楚：它们是冗余的伪影还是必要的全局先验？需要探索视觉汇聚在LVLM中的作用机制。

Method: 首先将视觉汇聚分为V-汇聚（来自视觉编码器）和L-汇聚（来自LLM深层）。然后提出层间汇聚门控（LSG），这是一个轻量级的即插即用模块，动态调节V-汇聚与其他视觉token的注意力贡献。LSG通过标准的下一token预测训练，无需任务特定监督，保持LVLM主干冻结。

Result: 分析揭示了基本性能权衡：汇聚能有效编码全局场景级先验，但其主导地位会抑制局部感知所需的细粒度视觉证据。LSG在大多数层中都能改善代表性多模态基准测试性能，有效平衡全局推理和精确的局部证据。

Conclusion: 视觉汇聚在LVLM中既有积极作用（编码全局先验）也有消极影响（抑制局部感知）。通过LSG动态调节汇聚贡献，可以在不改变模型架构的情况下优化多模态理解，平衡全局推理与局部证据提取。
...
TL;DR: 提出一种基于预训练和基础模型的可扩展AI方法，无需人工标注数据即可自动检测面对面协作学习中的注视行为，在复杂情境下表现出优越的跨配置鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法需要大量标注数据进行训练，且训练数据难以覆盖教育情境中的所有情况，导致跨配置鲁棒性不足。需要开发无需人工标注、具有更好泛化能力的注视行为检测方法。

Method: 使用预训练YOLO11进行人员跟踪，YOLOE-26（具有文本提示能力）检测教育相关物体，Gaze-LLE模型预测注视目标，构建端到端的注视行为检测系统。

Result: 方法在视频数据中检测学生注视行为的F1分数达到0.829，对笔记本电脑注视和同伴注视表现良好，但对其他注视目标表现较弱。相比其他监督学习方法，在复杂情境下表现更优越稳定，跨配置鲁棒性更好。
...
TL;DR: EgoMind是一个免几何的空间推理框架，通过角色扮演描述构建跨帧语言场景图，结合渐进式空间分析，仅用少量数据就能提升MLLM的空间认知能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么需要3D先验或几何监督（数据准备和对齐成本高），要么纯2D方法在多帧空间推理中表现不佳。需要一种既高效又不需要复杂几何信息的空间推理方法。

Method: 提出EgoMind框架：1）Role-Play Caption：通过角色扮演描述联合构建跨帧的连贯语言场景图；2）Progressive Spatial Analysis：渐进式推理解决任务特定问题。整个框架不需要几何信息。

Result: 仅用5K自动生成的SFT样本和20K RL样本，就在VSI-Bench、SPAR-Bench、SITE-Bench和SPBench上取得了有竞争力的结果，证明了其在增强MLLM空间推理能力方面的有效性。

Conclusion: EgoMind展示了语言推理在空间认知中的潜力，提供了一种高效且免几何的空间推理方法，仅需少量数据就能显著提升多模态大语言模型的空间推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有COVID-19 CT扫描检测模型在单一机构数据上表现良好，但在多中心数据（不同扫描仪、成像协议、患者群体）上泛化能力差，主要原因是模型偏向于训练数据量大的中心

Method: 采用多任务学习方法，共享EfficientNet-B7骨干网络，同时预测COVID-19诊断和数据来源；对数据来源分类头使用logit-adjusted交叉熵损失处理数据分布不均；预处理采用SSFL框架和KDS，每扫描选择8个代表性切片

Result: 在308个扫描的验证集上达到F1分数0.9098和AUC-ROC 0.9647

Conclusion: 通过多任务学习同时预测疾病诊断和数据来源，能够学习到跨中心的通用特征表示，有效提升模型在多中心CT数据上的泛化性能
...
<details>
  <summary>Details</summary>
Motivation: 智能制造中的质量检测需要识别超越可见几何形状的内在材料和表面属性，但仅依赖视觉的方法容易受到遮挡和反射的影响，因此需要结合多模态感知。

Method: 使用模态特定编码器和双Q-Former提取语言相关的视觉和触觉特征，通过对比学习对齐模态并耦合视觉与触觉，构建VitaSet多模态数据集，采用LoRA微调。

Result: 在多个基准测试中表现最佳：HCT和TVL整体最优，SSVTP保持竞争力；VitaSet上硬度准确率88.89%，粗糙度75.13%，描述符召回率54.81%；缺陷识别准确率高达100%（2类）、96%（3类）、92%（5类）；实验室机器人试验中闭环识别准确率94%，端到端分拣成功率94%。

Conclusion: VitaTouch通过融合视觉、触觉和语言模态，有效解决了智能制造中材料属性推理的挑战，在质量检测任务中展现了卓越的性能和实际应用潜力。
...
TL;DR: 该论文研究了自动驾驶车辆感知系统的安全对齐评估与优化方法，提出了安全导向的评估指标NDS-USC和安全感知损失函数EC-IoU，并在单车感知、车路协同感知和端到端驾驶框架中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习感知系统虽然性能提升，但存在统计不确定性，且标准训练目标和评估基准对所有感知错误一视同仁，而实际上只有部分错误是安全关键的。需要开发安全对齐的评估和优化方法来提升CAV安全性。

Method: 1) 使用安全导向指标NDS-USC评估单车3D目标检测模型；2) 采用安全感知损失函数EC-IoU进行微调；3) 在车路协同感知系统中进行安全导向评估；4) 将EC-IoU集成到端到端驾驶框架SparseDrive中。

Result: 1) 标准指标(mAP, NDS)的提升不一定转化为安全指标(NDS-USC)的改善；2) EC-IoU微调能提升安全关键检测性能；3) 车路协同模型优于单车模型；4) 安全感知硬化可将碰撞率降低近30%。

Conclusion: 安全对齐的感知评估和优化为提升CAV安全性提供了实用路径，适用于单车感知、协同感知和端到端自动驾驶等多种场景，有助于实现"零愿景"安全目标。
...
TL;DR: 该研究比较了九种代表性的叶面重建方法在三个公开数据集上的表现，分析了不同方法在精度、平滑度、鲁棒性和计算成本方面的权衡，为资源受限的农业机器人平台提供选择指导。


<details>
  <summary>Details</summary>
Motivation: 从3D点云准确重建叶面对于表型分析等农业应用至关重要，但真实植物数据复杂且现有多种重建方法的相对性能缺乏充分理解，需要系统比较以指导实际应用。

Method: 对九种代表性表面重建方法（参数化、三角化、隐式和基于学习的方法）在三个公开数据集（LAST-STRAW、Pheno4D、Crops3D）上进行比较评估，涵盖不同物种、传感器和采集环境。

Result: 不同方法在表面面积估计精度、平滑度、噪声鲁棒性、缺失数据容忍度和计算成本方面存在显著权衡，每种方法都有其特定优势，选择取决于具体应用场景和资源约束。

Conclusion: 研究结果为资源受限的机器人平台选择表面重建技术提供了实用指导，强调了需要根据应用需求在精度、鲁棒性和计算效率之间进行权衡。
...
TL;DR: CoLoRSMamba：一种基于VideoMamba和AudioMamba的多模态暴力检测架构，通过CLS引导的条件LoRA耦合视频和音频模态，无需token级交叉注意力，在音频过滤的数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的声音场景可能嘈杂或与视觉场景弱相关，但音频信息对暴力检测有益。需要一种能够有效整合视频和音频信息的多模态架构。

Method: 提出CoLoRSMamba架构，将VideoMamba和AudioMamba通过CLS引导的条件LoRA耦合。VideoMamba的CLS token产生通道调制向量和稳定门，调整AudioMamba的选择性状态空间参数。训练结合二元分类和对称AV-InfoNCE目标对齐音频视频嵌入。

Result: 在NTU-CCTV和DVD数据集的音频过滤子集上，分别达到88.63%准确率/86.24% F1-V和75.77%准确率/72.94% F1-V，优于音频、视频和多模态基线模型，且参数和计算量更少。

Conclusion: CoLoRSMamba通过轻量级条件调制有效整合视频和音频信息，在暴力检测任务中实现了优越的性能和效率平衡，为多模态学习提供了新思路。
...
TL;DR: 这篇综述论文系统回顾了将2D视觉模型（CNN/ViT）扩展到3D分析领域的方法，提出了一个统一分类框架，将方法分为数据中心、架构中心和混合三类，并分析了计算复杂度、预训练依赖和几何归纳偏置等关键权衡。


<details>
  <summary>Details</summary>
Motivation: CNN和ViT在2D视觉中取得了显著成功，但将其扩展到3D分析面临核心挑战：2D图像的规则密集网格与3D数据（点云、网格）的不规则稀疏特性之间存在根本差异。需要系统研究如何弥合这一差距。

Method: 提出统一分类法，将3D视觉模型适配策略分为三类：1）数据中心方法：将3D数据投影到2D格式以利用现成2D模型；2）架构中心方法：设计固有3D网络；3）混合方法：协同结合两种建模范式。

Result: 通过该框架定性分析了三类方法在计算复杂度、大规模预训练依赖和几何归纳偏置保留方面的基本权衡。讨论了关键开放挑战和未来研究方向。

Conclusion: 该综述为3D视觉领域提供了系统分类框架，指出了未来研究方向包括开发3D基础模型、推进几何数据的自监督学习以及更深度的多模态信号集成。
...
TL;DR: 本文介绍了一个名为RGxEStat的轻量级交互工具，用于分析基因型与环境互作（GxE），包括显著性分析和稳定性分析，旨在简化育种数据分析流程。


<details>
  <summary>Details</summary>
Motivation: 基因型与环境互作（GxE）会影响基因型在不同环境下的表现，降低表型在目标环境中的可预测性。深入分析GxE互作有助于识别基因优势或缺陷在特定环境条件下的表达或抑制，从而支持遗传选择和育种实践。目前缺乏用户友好的工具，育种家和农学家需要学习复杂的SAS或R编程。

Method: 提出了两种关键模型：1）基于混合效应模型的显著性分析，用于确定基因或GxE互作是否显著影响表型性状；2）稳定性分析，进一步研究基因与环境之间的交互关系以及基因型在不同环境中的相对优劣。开发了RGxEStat工具，集成了这些模型的构建、求解和可视化功能。

Result: RGxEStat是一个轻量级交互工具，提供了用户友好的界面，消除了育种家和农学家学习复杂编程的需求，能够显著加速研究周期。代码和数据集已在GitHub上公开。

Conclusion: RGxEStat工具为GxE互作研究提供了便捷的分析平台，通过简化数据分析流程，有助于促进遗传选择和育种实践的改进。
...
Result: 在NYU Depth v2、KITTI和MatterPort3D数据集上达到SOTA：NYU Depth v2上Abs Rel降至0.088(-7.4%)，RMSE降至0.316(-5.4%)；KITTI上阈值准确率接近完美(δ<1.25³≈99.8%)；仅194M参数，推理时间21ms。

Conclusion: 该方法通过创新的特征融合策略、轻量级适配器和CRF解码器，有效解决了单目深度估计的挑战，在精度和效率上均取得显著提升。
...
Method: 提出加性组合潜在动作模型(AC-LAM)，在潜在动作空间上强制短时域内的场景级加性组合结构，鼓励潜在动作空间的简单代数结构(恒等、逆、循环一致性)，抑制非加性组合的信息。

Result: AC-LAM学习到更结构化、运动特定且位移校准的潜在动作，为下游策略学习提供更强监督，在模拟和真实世界桌面任务中优于最先进的潜在动作模型。

Conclusion: 通过编码物理运动的加性组合结构先验，AC-LAM能学习更高质量的潜在动作表示，显著提升下游策略学习性能。
...
TL;DR: 该论文是关于遥感领域中混合专家模型（MoE）的首个系统性综述，涵盖了基本原理、架构设计和关键应用，并展望了未来趋势。


<details>
  <summary>Details</summary>
Motivation: 遥感数据分析面临传感器模态多样性和时空动态性的挑战，MoE模型通过动态路由输入到专门设计的专家来处理这些挑战，但该领域缺乏全面的MoE综述。

Method: 这是一篇综述性论文，通过系统性调研和总结的方式，对遥感领域的MoE应用进行全面梳理，包括基本原理、架构设计和关键应用。

Result: 提供了遥感领域MoE应用的首个系统性综述，涵盖了各种遥感任务中的应用，为研究人员提供了全面的参考框架。

Conclusion: MoE在遥感领域具有重要应用价值，该综述为未来研究提供了基础，并指出了未来发展趋势以激发更多创新研究。
...
TL;DR: YOLOv11是YOLO系列实时目标检测器的最新版本，通过引入新颖的架构模块改进了特征提取和小目标检测能力，在保持实时性的同时提升了精度。


<details>
  <summary>Details</summary>
Motivation: 开发新一代实时目标检测器，在保持YOLO系列实时处理能力的同时，改进特征提取和小目标检测性能，满足自动驾驶、监控和视频分析等应用需求。

Method: 引入C3K2块、SPPF（空间金字塔池化-快速）和C2PSA（带空间注意力的跨阶段部分连接）等新模块，优化骨干网络、颈部网络和头部组件，增强空间特征处理能力。

Result: 在标准基准测试中，YOLOv11相比先前YOLO版本在平均精度（mAP）和推理速度方面均有提升，实现了更高的准确性而不牺牲实时处理能力。

Conclusion: YOLOv11在保持实时性的同时显著提升了检测精度，特别适合自动驾驶、监控和视频分析等应用，为后续研究提供了清晰的参考框架。
...
TL;DR: ViBA提出了一种可持续学习框架，将几何优化与特征学习结合，在无约束视频流上进行连续在线训练，显著提升了视觉里程计的性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有图像关键点检测和描述方法依赖精确位姿和深度标注数据集，限制了可扩展性和泛化能力，且常导致导航和定位性能下降。需要一种能在无约束视频流上持续学习的方法。

Method: ViBA框架集成几何优化与特征学习，包含：(1)帧间对应关系的初始跟踪网络；(2)基于深度的离群点过滤；(3)通过最小化重投影误差联合优化相机位姿和特征位置的可微分全局束调整。结合BA的几何一致性和跨帧的长期时间一致性。

Result: 在EuRoC和UMA数据集上，相比SuperPoint+SuperGlue、ALIKED、LightGlue等方法，ViBA将平均绝对平移误差降低12-18%，绝对旋转误差降低5-10%，同时保持实时推理速度（36-91 FPS）。在未见序列上保持90%以上的定位精度。

Conclusion: ViBA通过几何和时间一致性支持连续在线学习，在真实场景中持续改进导航和定位性能，展示了强大的泛化能力和实际应用价值。
...
TL;DR: 多轮图像编辑会导致图像质量迭代退化，现有图像质量评估指标无法检测这种退化，威胁多模态智能系统的稳定性。


<details>
  <summary>Details</summary>
Motivation: 多模态智能系统的多步迭代图像编辑能力改变了数字内容创作，但现有模型在多轮编辑中存在图像质量迭代退化问题，而图像质量评估器无法检测这种退化，可能威胁未来模型训练和部署系统的安全性。

Method: 引入Banana100数据集，包含28,000张通过100次迭代编辑步骤生成的退化图像，涵盖多样纹理和图像内容。评估了21种流行的无参考图像质量评估（NR-IQA）指标。

Result: 图像在多轮编辑中质量严重退化，但21种NR-IQA指标都无法一致地为严重退化图像分配比干净图像更低的分数。生成器和评估器的双重失败可能威胁未来模型训练和部署系统的稳定性。

Conclusion: 多轮图像编辑存在迭代退化问题，现有质量评估指标无法检测，需要开发更鲁棒的模型来缓解多模态智能系统的脆弱性。发布完整代码和数据以促进相关研究。
...
TL;DR: KiToke是一种无需训练、查询无关的视频令牌压缩方法，通过核基冗余度量全局估计令牌多样性，在极端压缩比下保持视觉信息，显著降低视频大语言模型推理成本。


<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在视频理解任务上表现良好，但由于视觉令牌数量庞大导致推理成本高昂。现有方法依赖局部或分段启发式方法，无法有效捕捉全局冗余。

Method: 提出KiToke方法：1) 使用核基冗余度量全局估计令牌多样性，实现内容自适应选择；2) 引入轻量级时间间隔构建与间隔感知令牌合并，保持时间连贯性；3) 无需训练且查询无关。

Result: 在多个视频理解基准测试和视频LLM骨干网络上，KiToke始终优于现有的无需训练压缩方法，在低至1%的保留率下仍能取得显著性能提升。

Conclusion: KiToke通过全局冗余估计和内容自适应选择，有效减少视频令牌的时空冗余，显著降低视频LLM推理成本，在极端压缩比下仍能保持良好性能。
...
TL;DR: 量化鲁棒性可通过权重空间中的"量化向量"在任务间迁移，无需接收方量化感知训练，为极低比特部署提供零成本替代方案


<details>
  <summary>Details</summary>
Motivation: 量化感知训练(QAT)需要大量计算和训练数据，成本高昂。本文旨在寻找一种无需接收方训练数据、低成本的量化鲁棒性迁移方法

Method: 通过权重空间算术从捐赠任务中提取"量化向量"，将其应用于接收模型以提升对量化噪声的鲁棒性，无需接收方量化感知训练

Result: 在Vision Transformer模型上，该方法可将量化鲁棒性提升高达60%，为极低比特部署提供零样本、低成本的QAT替代方案

Conclusion: 量化鲁棒性不仅是任务特定训练的副产品，更是权重空间几何中可重用、可迁移的特征，可通过量化向量在不同任务间转移
...
TL;DR: 本文提出了一种基于基础模型的猪群监测工作流，通过预训练视觉-语言模型作为通用视觉骨干，结合轻量级后处理模块，实现了对群养猪的长期稳定跟踪监测。


<details>
  <summary>Details</summary>
Motivation: 精准畜牧业中大多数监测流程仍依赖需要大量标注数据的监督学习模型，存在重复训练和农场特定调优的问题。基础模型能够减少对任务特定监督学习的依赖，利用大规模学习的通用视觉表示。

Method: 采用基础模型为中心的工作流：1) 使用Grounding-DINO进行目标检测建立基线；2) 针对夜间和遮挡问题集成时序跟踪逻辑；3) 使用Grounded-SAM2进行短期视频分割；4) 开发长期跟踪管道，包含初始化、跟踪、匹配、掩码细化、重识别和后处理质量控制。

Result: 在1418张标注图像上建立检测基线，在550个一分钟视频片段中，经过后处理后80%以上的4927个活动轨迹完全正确。在132分钟连续视频上，系统保持稳定身份跟踪，在132个均匀采样的真实帧上达到：平均区域相似度(J)0.83、轮廓精度(F)0.92、J&F 0.87、MOTA 0.99、MOTP 90.7%，无身份切换。

Conclusion: 这项工作展示了如何将基础模型的先验知识与轻量级、任务特定的逻辑相结合，实现猪生产中可扩展、标注高效和长期监测的解决方案。
...
TL;DR: 在无需微调模型权重的情况下，通过推理时重复特定Transformer层的电路复制方法，显著提升了基于冻结自监督视觉基础模型的水下物种分类性能。


<details>
  <summary>Details</summary>
Motivation: 水下物种自动分类面临标注成本高和环境变化导致模型泛化能力受限的问题。现有研究表明冻结的自监督视觉基础模型嵌入已能提供良好的标签效率基准，但研究者希望探索能否在推理时（不微调模型权重）进一步提升性能。

Method: 应用电路复制方法（Circuit Duplication），在推理时对选定的Transformer层进行两次前向传播。在AQUA20基准测试中使用冻结的DINOv3嵌入，评估两种设置：全局电路选择（整个数据集使用单一重复电路）和类别特定电路选择（每个物种使用不同的最优电路）。两种设置都使用简单的半监督下游分类器。

Result: 电路复制方法持续优于标准冻结前向传播。在最大标签预算下，类别特定选择达到0.875的宏F1分数，将与全监督ConvNeXt基准（0.889）的差距缩小到1.4个百分点，且无需任何基于梯度的训练。四个物种的性能超过其全监督参考，章鱼提升了+12.1 F1点。在所有预算下，约75%的类别偏好类别特定电路，表明存在真正的类别依赖性效益。

Conclusion: 这是电路复制方法首次应用于计算机视觉领域，证明了在推理时通过简单修改前向传播路径（不改变模型权重）可以显著提升冻结自监督嵌入的分类性能，为标签稀缺的水下物种分类提供了高效解决方案。
...
TL;DR: ExpressEdit是一个开源Photoshop插件，用于快速、无伪影地编辑角色面部表情，支持检索增强生成，包含135种表情标签数据库。


<details>
  <summary>Details</summary>
Motivation: 当前AI图像编辑模型在风格化表情编辑时会产生全局噪声和像素漂移，无法集成到专业图像编辑软件和工作流程中，需要解决这些问题。

Method: 开发完全开源的Photoshop插件ExpressEdit，避免专有模型的常见伪影，与Photoshop原生操作（如液化）协同工作，并构建包含135种表情标签的数据库，支持检索增强生成。

Result: ExpressEdit可在单个消费级GPU上3秒内无缝编辑表情，比流行专有模型快得多，且无常见伪影，能与Photoshop原生操作稳健协同。
...
TL;DR: RDFace是一个包含456张儿童面部图像、涵盖103种罕见遗传病的基准数据集，用于在真实世界低数据约束下开发数据高效的AI罕见病诊断模型，通过合成图像增强可将诊断准确率提升13.7%。


<details>
  <summary>Details</summary>
Motivation: 罕见病常表现出独特的面部表型特征，但该领域发展受到两个主要限制：1) 缺乏经过伦理验证的、高质量的面部数据；2) 不同疾病间的表型特征高度相似。这些限制阻碍了AI辅助筛查系统的发展。

Method: 1) 构建RDFace基准数据集：包含456张儿童面部图像，涵盖103种罕见遗传病，每张图像都经过伦理验证并配有标准化元数据；2) 使用预训练视觉骨干网络进行交叉验证基准测试；3) 探索使用DreamBooth和FastGAN进行合成增强，通过面部标志点相似性过滤生成图像以保持表型保真度；4) 使用视觉语言模型评估合成图像的语义有效性。

Result: 1) 合成图像与真实数据融合后，在极低数据情况下诊断准确率提升高达13.7%；2) 视觉语言模型从真实和合成图像生成的表型描述报告相似度得分达到0.84，表明合成图像具有良好的语义有效性；3) 建立了透明、基准就绪的数据集和可扩展的评估框架。
...
TL;DR: SpectralSplat是一种前馈3D高斯溅射方法，能够将场景几何与外观属性（如光照、天气、时间）解耦，实现重光照、外观迁移和多时间数据的一致渲染。


<details>
  <summary>Details</summary>
Motivation: 现有的前馈3D高斯溅射方法在自动驾驶场景重建中取得了良好效果，但将场景几何与瞬态外观属性（光照、天气、时间）耦合在一起，这阻碍了重光照、外观迁移以及在不同环境条件下捕获的多遍历数据的一致渲染。

Method: 1. 将颜色预测分解为外观无关的基础流和外观条件化的适应流，两者由共享MLP生成，基于DINOv2特征提取的全局外观嵌入进行条件化；2. 使用混合重光照管道生成的配对观测数据进行训练，结合基于物理的本征分解和基于扩散的生成细化；3. 采用互补的一致性、重建、跨外观和基础颜色损失进行监督；4. 引入外观可适应的时间历史，存储外观无关特征，使累积的高斯能在任意目标外观下重新渲染。

Result: 实验表明，SpectralSplat在保持底层骨干网络重建质量的同时，实现了可控的外观迁移和跨驾驶序列的时间一致重光照。

Conclusion: SpectralSplat成功地将外观与几何在前馈高斯溅射框架中解耦，为自动驾驶场景的灵活渲染和外观控制提供了有效解决方案。
...
TL;DR: 将DeepSeek-OCR-2适配用于分子光学识别，提出两阶段渐进微调策略，在PubChem和USPTO-MOL数据上训练得到MolSeek-OCR模型，在序列生成任务上表现有竞争力但不及最先进的图结构模型。


<details>
  <summary>Details</summary>
Motivation: 光学化学结构识别（OCSR）对于将印刷文献中的2D分子图转换为机器可读格式至关重要。虽然视觉语言模型在端到端OCR任务中显示出潜力，但直接应用于OCSR仍具挑战性，且直接的全参数监督微调通常失败。

Method: 将任务形式化为图像条件SMILES生成，采用两阶段渐进监督微调策略：从参数高效的LoRA开始，过渡到使用分割学习率的选择性全参数微调。在结合PubChem合成渲染和USPTO-MOL专利图像的大规模语料库上进行训练。

Result: 微调模型MolSeek-OCR展现出有竞争力的能力，在精确匹配准确度上与最佳图像到序列模型相当，但仍不及最先进的图像到图模型。强化式后训练和数据筛选细化未能改善精确SMILES匹配所需的严格序列级保真度。

Conclusion: 通过渐进微调策略成功将视觉语言模型适配于OCSR任务，但序列生成方法在精确匹配方面仍有限制，未来可能需要探索图结构生成方法以获得更好的性能。
...
TL;DR: 提出多模态框架结合卫星影像与街景图像，在有限标注条件下实现可扩展的城市树木检测，通过混合学习策略显著提升性能


<details>
  <summary>Details</summary>
Motivation: 传统城市树木调查方法劳动密集且成本高，难以扩展到不同城市环境，需要解决标注成本高和跨场景泛化能力差的问题

Method: 1) 多模态框架：卫星影像定位候选树木，街景图像进行详细检测；2) 领域适应技术迁移知识；3) 评估三种学习策略：半监督学习、主动学习和混合方法，使用基于transformer的检测模型

Result: 混合策略获得最佳性能，F1分数0.90，比基线模型提升12%。主动学习和混合策略有效减少假阳性和假阴性，而半监督学习因伪标签确认偏差导致性能逐渐下降

Conclusion: 多模态方法和引导式标注对于实现可扩展、标注高效的城市树木测绘至关重要，能够加强可持续城市规划
...
TL;DR: 提出用户显著目标检测(UserSOD)任务，强调基于用户主动需求而非被动视觉刺激来检测显著目标，但面临缺乏训练数据集的主要挑战。


<details>
  <summary>Details</summary>
Motivation: 现有显著目标检测方法基于被动视觉刺激原理，忽略了用户主动需求的决定性作用。这既无法满足用户需求，也限制了下游任务的发展，如显著目标排序任务需要分析用户观看顺序与场景的细粒度关系。

Method: 论文提出用户显著目标检测(UserSOD)任务，专注于检测与用户主动需求一致的显著目标。主要挑战是缺乏用于模型训练和测试的数据集。

Result: 论文提出了新的UserSOD任务框架，但尚未解决数据集缺乏的问题，这是该任务面临的主要挑战。

Conclusion: 需要开发基于用户需求的显著目标检测方法，但当前首要挑战是创建相应的数据集来支持模型训练和评估。
...
TL;DR: HEDGE提出异构集成方法，通过三个互补检测路径（多样化训练数据、多尺度特征提取、骨干网络异构性）来提升AI生成图像检测的鲁棒性，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 由于生成模型的快速进化和真实世界图像的各种失真，在野外环境中鲁棒地检测AI生成图像仍然具有挑战性。单一的训练策略、分辨率或骨干网络不足以处理所有条件，需要在这些维度上引入结构化异质性。

Method: 提出HEDGE异构集成框架，包含三个互补检测路径：路径A通过分阶段数据扩展和增强升级构建DINOv3检测器；路径B加入高分辨率分支提取细粒度取证线索；路径C添加MetaCLIP2分支实现骨干网络多样性。所有输出通过logit空间加权平均融合，并由轻量级双门控机制优化处理分支级异常值和多数主导融合错误。

Result: 在NTIRE 2026鲁棒AI生成图像检测挑战赛中获得第4名，在多个AIGC图像检测基准测试中达到最先进的性能，并展现出强大的鲁棒性。

Conclusion: 通过结构化异质性集成方法，HEDGE能够有效应对AI生成图像检测中的各种挑战，证明了多样化训练策略、多尺度特征提取和骨干网络异构性对于鲁棒检测的重要性。
...
TL;DR: 提出一种轻量级推理时干预方法，通过分析视觉编码器注意力动态的三阶段结构，在焦点阶段选择性抑制低注意力token，使用DPP保留多样视觉线索，显著减少LVLM的对象幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多模态推理方面取得显著进展，但仍存在对象幻觉问题（描述图像中不存在的对象）。现有方法通常通过抑制视觉编码器中不可靠信号来缓解幻觉，但许多方法需要对每个输入进行迭代优化，导致推理延迟显著增加。

Method: 分析视觉编码器内部注意力动态，识别出视觉信息处理的三阶段结构：扩散、聚焦和再扩散。发现幻觉行为对焦点阶段接收低注意力的token特别敏感。提出轻量级推理时干预方法，在焦点阶段选择性抑制这些token。该方法无需训练，使用单次前向传播的统计信息，并采用确定性点过程（DPP）在过滤冗余token的同时保留多样视觉线索。

Result: 在多个LVLM骨干网络和解码策略上的广泛实验表明，该方法能一致地减少幻觉指标，同时保持有竞争力的字幕质量。与对抗性不确定性估计方法相比，该方法在实现可比较的幻觉缓解效果的同时，仅增加可忽略的推理延迟。

Conclusion: 通过分析视觉编码器注意力动态并识别关键的三阶段处理结构，提出了一种高效、轻量级的推理时干预方法，能有效缓解大型视觉语言模型的对象幻觉问题，为实际应用提供了实用解决方案。
...
TL;DR: LOGER提出了一种局部-全局集成框架，通过多尺度全局分支和基于多实例学习的局部分支，结合logit空间融合，实现了对多样化深度伪造技术的鲁棒检测。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测方法面临两大挑战：1）不断增长的伪造技术和现实世界退化；2）伪造线索存在于全局（语义和统计异常）和局部（伪造区域痕迹）两个互补层面，但单一骨干网络或输入尺度难以同时有效覆盖这两个层面。

Method: 提出LOcal-Global Ensemble framework (LOGER)：1）全局分支使用异构视觉基础模型在多个分辨率下捕捉整体异常；2）局部分支采用多实例学习top-k聚合策略，仅选择最可疑区域进行建模，避免正常区域稀释证据；3）双级监督（聚合图像级和单个补丁级）保持局部响应判别性；4）利用logit空间融合两个分支的预测。

Result: 在NTIRE 2026鲁棒深度伪造检测挑战赛中获得第二名，在多个公共基准测试中表现出强大的鲁棒性和泛化能力，能够应对多样化的伪造方法和现实世界退化条件。

Conclusion: LOGER通过局部-全局集成框架有效解决了深度伪造检测中的多尺度挑战，两个分支在粒度和骨干网络上的差异使它们的错误基本不相关，logit空间融合进一步提升了检测的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 单像素成像成本低但采样率极低时难以恢复高质量空间和光谱细节，现有深度学习方法需要大规模预训练数据集，这在HSI中不切实际。

Method: 三阶段物理信息框架：1) 带RGB灰度先验的正则化最小二乘法初始化；2) 无训练高光谱恢复网络通过测量一致性和混合正则化优化；3) 基于Transformer的无训练超分辨率网络通过跨模态注意力从RGB引导转移高频细节。

Result: 在基准数据集上显著超越最先进算法，在物理单像素成像系统中以仅6.25%采样率成功重建144波段高光谱数据立方体。

Conclusion: 该方法为计算高光谱成像提供了鲁棒、数据高效的无外部训练数据解决方案。
...
TL;DR: 提出SBF（尺度-身体-光流）表示法增强骨架信息，通过SFSNet网络预测，显著提升动作识别精度


<details>
  <summary>Details</summary>
Motivation: 现有基于2D骨架的动作识别方法存在局限，骨架无法捕捉关节深度、人体轮廓和人-物交互等关键信息

Method: 提出SBF表示法，包含尺度图（关节深度）、身体图（人体轮廓）和光流图（人-物交互）；设计SFSNet分割网络，利用骨架和光流监督，无需额外标注

Result: 在多个数据集上实验表明，基于SBF和SFSNet的管道相比最先进的纯骨架方法，在保持相似紧凑性和效率的同时，显著提高了动作识别准确率

Conclusion: SBF表示法有效补充了骨架信息缺失，SFSNet网络能够高效预测SBF，为视频动作识别提供了更全面的表示方法
...
TL;DR: 本文提出了一种基于分数的PnP解释，将预训练的分数扩散模型直接用于PnP框架，并引入随机生成PnP方法，通过噪声注入提升严重病态逆问题的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 尽管PnP方法和分数扩散模型都依赖于去噪器，但如何系统地将分数扩散模型作为先验用于PnP框架而不依赖反向扩散采样仍不清楚。本文旨在建立两者之间的连接。

Method: 1) 建立基于分数的PnP解释，证明可直接在PnP算法中使用预训练的分数扩散模型；2) 提出随机生成PnP框架，通过噪声注入更好地利用分数扩散模型的生成能力；3) 提供理论分析，证明噪声注入诱导高斯平滑目标优化并促进逃离严格鞍点。

Result: 在多线圈MRI重建和大掩码自然图像修复等挑战性逆任务实验中，相比传统PnP方法有持续改进，性能与基于扩散的求解器相当。

Conclusion: 本文成功建立了分数扩散模型与PnP框架的系统连接，提出的随机生成PnP方法能有效利用分数扩散模型的生成先验，在严重病态逆问题中表现出更好的鲁棒性和性能。
...
TL;DR: PortraitCraft：首个统一的肖像构图理解与生成基准，包含5万张带结构化多级标注的肖像图像，支持构图评分、属性推理、视觉问答和可控生成等任务。
...
Method: 构建包含约5万张精选真实肖像图像的数据集，提供全局构图评分、13个构图属性标注、属性级解释文本、视觉问答对和面向构图的文本描述等多级监督。在此基础上建立构图理解和构图感知生成两个互补的基准任务，并定义标准化评估协议。

Result: PortraitCraft提供了首个统一的肖像构图基准，包含全面的数据集和评估框架，为多模态模型提供了参考基线结果，支持细粒度肖像理解、可解释美学评估和可控肖像生成研究。
...
TL;DR: TABLeT使用预训练的2D自然图像自编码器将fMRI体积压缩为连续token，通过Transformer编码器实现长序列建模，在多个大规模基准测试中优于现有模型，同时显著提升计算和内存效率。


<details>
  <summary>Details</summary>
Motivation: 解决fMRI四维信号高维度建模中，现有体素模型因内存需求过大而只能捕捉有限时间窗口的问题，实现可扩展的长程时空动态建模。

Method: 提出TABLeT方法：使用预训练的2D自然图像自编码器将3D fMRI体积压缩为紧凑的连续token，然后通过简单的Transformer编码器进行长序列建模，并开发了自监督掩码token建模进行预训练。

Result: 在UK-Biobank、HCP和ADHD-200等大规模基准测试中，TABLeT在多个任务上优于现有模型，在相同输入下相比最先进的体素方法显著提升了计算和内存效率。

Conclusion: TABLeT为大脑活动的可扩展和可解释时空建模提供了一种有前景的方法，通过token化压缩实现了长序列建模，同时保持了高性能和效率。
...
TL;DR: MuPD是一个多模态病理学生成基础模型，通过扩散变换器和解耦跨模态注意力，将H&E组织学、RNA分子谱和临床文本嵌入共享潜在空间，支持多种跨模态合成任务，显著优于特定领域模型。
...
Method: 使用扩散变换器与解耦跨模态注意力，将H&E组织学、RNA分子谱和临床文本嵌入共享潜在空间。在3400万组织学图像块、160万文本-组织学对和1080万RNA-组织学对上进行预训练。

Result: 在文本条件和图像到图像生成中，FID分数降低50%，少样本分类准确率提高47%；在RNA条件组织学生成中，FID降低23%；作为虚拟染色剂，标记物相关性提高37%。

Conclusion: 单一统一的多模态生成模型在病理学中显著优于专门化替代方案，为多模态组织病理学提供了可扩展的计算框架。
...
TL;DR: 提出两阶段纳米粒子分析方案：先用自注意力U-Net从真实图像学习关键特征，再嵌入CycleGAN框架生成合成图像-掩码对，实现无需人工标注的自动数据增强和准确分割。


<details>
  <summary>Details</summary>
Motivation: 电子显微镜图像中纳米粒子的精确分析对纳米材料开发至关重要，但传统方法存在耗时、依赖大量标注数据、难以处理复杂形状和成像伪影等问题。

Method: 1. 使用自注意力驱动的U-Net架构从真实图像数据集学习纳米粒子关键特征，关注物理和形态细节，忽略背景噪声；2. 将训练好的Attention U-Net嵌入循环一致性生成对抗网络(CycleGAN)框架，生成高度真实的合成电子显微镜图像-掩码对。

Result: 模型能够准确检测各种真实世界纳米粒子图像中的特征，并自主增强训练数据集而无需人工输入。循环一致性确保合成图像与真实掩码之间的直接对应关系，为准确分割训练提供关键保障。

Conclusion: 该方法通过结合自注意力U-Net和CycleGAN，克服了传统方法对大量标注数据的依赖，实现了高效、准确的纳米粒子自动分割和分析，为纳米材料表征提供了创新解决方案。
...
TL;DR: ComPrivDet：一种高效检测压缩视频中隐私对象的方法，通过复用I帧推理结果，跳过大部分P/B帧检测，在保持高准确率的同时显著降低延迟


<details>
  <summary>Details</summary>
Motivation: 物联网视频分析中用户隐私泄露问题严重，现有逐帧保护方法延迟高，而现有对象检测器需要完全解码视频或逐帧处理压缩视频，导致解码开销大或准确率降低

Method: 提出ComPrivDet方法，利用压缩域线索识别新对象出现，复用I帧推理结果，选择性跳过P帧和B帧检测，或使用轻量级检测器进行高效精炼

Result: 在隐私人脸检测中保持99.75%准确率，隐私车牌检测中保持96.83%准确率，同时跳过超过80%的推理；相比现有压缩域检测方法，平均准确率提高9.84%，延迟降低75.95%

Conclusion: ComPrivDet通过有效利用压缩视频特性，在保持高检测准确率的同时显著降低计算开销，为大规模视频分析中的隐私保护提供了高效解决方案
...
TL;DR: CSRS提出连续软化重溯重采样方法，通过重溯推理机制、软化频率奖励和视觉语义扰动，解决MLLM自演化中多数投票导致的偏差问题，提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM自演化方法主要依赖多数投票选择伪黄金答案，但这可能源于模型内在偏见而非客观正确的推理路径，导致性能退化。需要解决反馈信号质量问题以实现稳定有效的学习。

Method: 提出CSRS框架：1) 重溯推理机制(RRM)：从锚点重新推理以探索长尾推理路径；2) 软化频率奖励(SFR)：用连续信号替代二元奖励，基于答案频率校准奖励；3) 视觉语义扰动(VSP)：确保模型优先数学逻辑而非视觉表面特征。

Result: CSRS显著提升Qwen2.5-VL-7B在MathVision等基准上的推理性能，在几何任务的无监督自演化中达到SOTA结果。

Conclusion: CSRS通过连续软化重溯重采样有效解决了MLLM自演化中的偏差问题，提升了推理路径的探索质量和模型性能，为无监督自演化提供了新方法。
...
TL;DR: ART提出自适应关系Transformer，通过时序感知关系图捕捉交互演化，自适应交互剪枝减少冗余计算，在行人轨迹预测任务上实现高精度和高效率。


<details>
  <summary>Details</summary>
Motivation: 现有图基或Transformer方法要么计算开销大，要么难以捕捉人类交互的多样性和时变特性。需要一种既能准确建模交互演化又能高效计算的方法。

Method: 提出自适应关系Transformer(ART)：1) 时序感知关系图(TARG)显式捕捉成对交互的演化；2) 自适应交互剪枝(AIP)机制高效减少冗余计算。

Result: 在ETH/UCY和NBA基准测试上，ART实现了最先进的精度和高效的计算性能。

Conclusion: ART通过显式建模交互演化和自适应剪枝，在行人轨迹预测任务上取得了精度和效率的平衡，为机器人相关应用提供了有效解决方案。
...
TL;DR: MASC-Pose：一种用于高效3D人体姿态估计的运动自适应多尺度时序建模框架，结合骨架约束空间图，在Human3.6M和MPI-INF-3DHP数据集上表现优异


<details>
  <summary>Details</summary>
Motivation: 单目视频中的3D人体姿态估计需要有效建模复杂的时空依赖关系。现有方法在建模时空依赖时，特别是在密集注意力或固定建模方案下，往往面临效率和适应性的挑战。

Method: 提出MASC-Pose框架，包含两个核心模块：1）自适应多尺度时序建模（AMTM）模块，自适应捕捉不同时间尺度的异质运动动态；2）骨架约束自适应图卷积网络（SAGCN），用于关节特定的空间交互建模。通过联合实现自适应时序推理和高效空间聚合。

Result: 在Human3.6M和MPI-INF-3DHP数据集上的大量实验证明了该方法的有效性，实现了强准确性和高计算效率。

Conclusion: MASC-Pose通过运动自适应多尺度时序建模和骨架约束空间图的结合，为3D人体姿态估计提供了一种高效且准确的解决方案，有效解决了现有方法在时空依赖建模中的效率和适应性问题。
...
Method: 采用粗到细表示学习范式：1) 生成全局上下文语义寄存器作为粗粒度高亮；2) 通过概率变分采样器和文本监督截断扩散模型迭代精炼寄存器；3) 文本语义结构学习构建良好文本潜在空间；4) 通过寄存器增强的高斯注意力块自适应融合寄存器与视频标记。

Result: 在广泛实验中，DreamPRVR超越了现有最先进方法，在部分相关视频检索任务上取得了更好的性能。

Conclusion: DreamPRVR通过粗到细表示学习有效解决了PRVR中的查询模糊性和局部噪声问题，全局语义寄存器机制显著提升了跨模态匹配的准确性。
...
TL;DR: 提出LaPR框架，通过标签感知的提示检索提升视觉上下文学习性能，强调标签一致性的重要性


<details>
  <summary>Details</summary>
Motivation: 现有视觉上下文学习(VICL)的提示检索方法主要关注图像相似性而忽略标签一致性，导致有时会检索到视觉相似但标签不一致的提示，从而降低VICL性能。研究发现查询与提示之间的标签一致性越高，VICL结果越好。

Method: 提出LaPR框架：1)设计图像-标签联合表示来显式融入标签信息；2)引入混合专家机制到双编码器中，每个专家捕获特定标签模式，路由器推断查询自适应的混合权重；3)采用交替优化策略，分别使用VICL性能引导的对比损失和标签引导的对比损失。

Result: 在上下文分割、检测和着色任务上取得显著且一致的性能提升。LaPR在不同特征提取器和交叉验证场景中泛化良好，证明了标签利用在VICL提示检索中的重要性。

Conclusion: 标签一致性对VICL性能至关重要，LaPR框架通过标签感知的提示检索有效提升了VICL性能，为视觉基础模型的提示选择提供了新思路。
...
TL;DR: 提出了一种基于视觉大语言模型（VLLMs）的第一人称视角人-物交互预测方法，通过Set-of-Mark提示增强视觉定位能力，利用用户最近注视点轨迹理解意图，并引入逆指数采样策略捕捉交互前的时序动态。


<details>
  <summary>Details</summary>
Motivation: 在智能辅助系统中预测人-物交互对于指导用户日常生活活动和理解其短期与长期目标至关重要。现有方法存在视觉定位能力不足和用户意图理解有限的问题，需要改进。

Method: 1. 使用Set-of-Mark提示增强VLLMs的视觉定位能力；2. 通过用户最近注视点形成的轨迹理解用户意图；3. 引入逆指数采样策略捕捉交互前的时序动态；4. 在HD-EPIC数据集上进行实验验证。

Result: 在HD-EPIC第一人称数据集上的实验表明，该方法超越了现有最先进方法，展示了其模型无关的特性。

Conclusion: 该方法通过改进视觉定位、理解用户意图和优化时序采样，有效提升了第一人称视角下人-物交互预测的性能，为智能辅助系统提供了更好的交互理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成方面表现出色，但多步推理机制计算成本巨大。现有层缓存或令牌缓存方法在少步扩散Transformer模型中加速效果有限，存在特征缓存策略低效、稀疏分配手动设计、部分步骤仍需完整前向计算等问题

Method: 提出可微分层间稀疏优化框架，利用令牌缓存减少令牌计算成本；通过可学习网络结合动态规划求解器端到端优化层间稀疏分配；采用两阶段训练策略，无需现有方法的完整步处理

Result: 在多种扩散Transformer模型（DiT-XL/2, PixArt-α, FLUX, Wan2.1）上实验，方法在保持样本质量的同时持续提升效率。例如在PixArt-α的20采样步中，计算成本降低54%，生成指标超过原始模型，显著优于先前方法

Conclusion: 该方法在显著提升效率的同时，往往还能改善生成质量，为扩散Transformer模型的高效推理提供了有效解决方案
...
TL;DR: DSERT-RoLL是一个多模态驾驶数据集，包含立体事件相机、RGB相机、热成像相机、4D雷达和双激光雷达，在多种天气和光照条件下采集，提供精确的2D/3D标注和基准测试。
...
Result: 创建了DSERT-RoLL数据集，提供跨传感器组合的公平比较基准，报告了单模态和多模态方法的基线结果，并提出了统一的特征空间融合框架。

Conclusion: 该数据集缓解了新型传感器数据稀缺问题，支持传感器特性比较和多模态融合研究，提出的融合框架提高了3D检测在不同天气和光照条件下的鲁棒性。
...
TL;DR: 该论文研究了科学图像领域的长尾识别问题，提出了SciLT框架，通过自适应特征融合和双重监督学习，在科学图像长尾识别任务上取得了优于现有方法的效果。


<details>
  <summary>Details</summary>
Motivation: 现有长尾识别研究和基准主要局限于自然图像领域，而科学图像具有独特的视觉特征和监督信号。在科学图像领域，微调基础模型的效果有限，需要专门的方法来解决科学图像长尾识别问题。

Method: 提出SciLT框架，通过自适应特征融合和双重监督学习，联合利用倒数第二层和最终层的特征表示，实现头尾类别的平衡性能。

Result: 在三个科学基准测试上的实验表明，SciLT框架一致优于现有方法，为科学长尾识别建立了强大且实用的基线。

Conclusion: SciLT框架为科学长尾识别提供了有效的解决方案，并为适应基础模型到具有显著领域偏移的科学数据提供了有价值的指导。
...
TL;DR: 本文揭示了深度学习图像水印中END架构对已知原始攻击(KOA)的脆弱性，并提出ResGuard插件模块增强水印鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的图像水印采用"编码器-噪声层-解码器"(END)架构来抵抗随机信道失真，但忽视了攻击者拥有原始-水印图像对时的已知原始攻击(KOA)威胁。这种攻击能几乎完全去除水印而保持视觉质量，暴露了现有方法的严重安全漏洞。

Method: 提出ResGuard插件模块，通过残差特异性增强损失强制残差与宿主图像紧密耦合，提高图像依赖性。同时引入辅助KOA噪声层，在训练中注入残差式扰动，使解码器在更强的嵌入不一致性下保持可靠。该模块可集成到现有框架中。

Result: ResGuard显著提升了KOA鲁棒性，将平均水印提取准确率从59.87%提高到99.81%，有效防御了基于残差估计的已知原始攻击。

Conclusion: END架构的水印方法存在对已知原始攻击的严重脆弱性，ResGuard通过增强残差的图像依赖性成功解决了这一问题，为深度学习水印系统提供了重要的安全增强。
...
TL;DR: FunFact是一个从RGB-D图像构建概率性开放词汇功能3D场景图的框架，通过联合概率推理解决场景级功能关系模糊性


<details>
  <summary>Details</summary>
Motivation: 现有3D场景理解方法通常孤立考虑物体对之间的功能关系，无法捕捉人类用于解决模糊性的场景级相互依赖关系

Method: 首先构建物体和部件中心的3D地图，使用基础模型提出语义上合理的功能关系候选，将其转换为因子图变量，并受LLM常识先验和几何先验约束，进行联合概率推理

Result: 在SceneFun3D、FunGraph3D和FunThor数据集上，FunFact提高了节点和关系发现召回率，显著减少了模糊关系的校准误差

Conclusion: 整体概率建模对功能场景理解有益，FunFact框架通过联合推理更好地捕捉场景级功能关系
...
TL;DR: SGTA是一个基于场景图的多模态交通智能体框架，通过结构化场景图与多模态推理结合，实现交通视频理解与可解释决策。


<details>
  <summary>Details</summary>
Motivation: 现有交通视频理解方法缺乏结构化表示和可解释性，需要结合场景图和多模态推理来提高复杂交通场景的理解能力。

Method: 从路边视频构建交通场景图（检测、跟踪、车道提取），采用ReAct框架结合大语言模型推理和工具调用，处理符号图查询和视觉输入。

Result: 在TUMTraffic VideoQA数据集样本上取得有竞争力的准确率，支持多种问题类型，并提供透明的推理步骤。

Conclusion: 结构化场景表示与多模态智能体结合在交通视频理解中具有潜力，SGTA框架实现了可解释的决策过程。
...
TL;DR: VidNum-1.4K是一个包含1,379个人工标注视频问答对的基准测试，用于评估视觉语言模型在真实世界视频中的数值推理能力，揭示当前模型在复杂多步数值逻辑推理方面存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有视频数值推理基准测试局限于狭窄领域（如重复运动），或将简单计数视为表面回归任务，无法评估真实世界多媒体内容中的多步数值逻辑。需要测试视觉语言模型是否真正"理解"现实世界动态，这需要掌握时间事件、对象持久性和组合逻辑。

Method: 构建VidNum-1.4K基准测试，包含1,379个严格人工标注的视频-问题对，涵盖高度多样化的环境（对象、动作和事件量化）。采用三层层次结构：从直接视觉感知到视频组合数值推理，要求模型执行基于时间证据的算术运算、比较和逻辑推理。

Result: 评估多个最先进的视觉语言模型显示显著的推理差距：Gemini-3.1-pro仅达到60%准确率阈值，代表性开源模型在25%-45%范围内挣扎。这表明当前模型缺乏稳定的"内部世界模型"。

Conclusion: VidNum-1.4K作为一个具有挑战性的诊断测试平台，揭示了当前视觉语言模型在真实世界视频数值推理方面的局限性，为下一代数值视频智能的发展提供了重要基准。
...
TL;DR: XSeg：最大的X射线违禁品分割数据集，包含98,644张图像和295,932个实例掩码，涵盖30个常见违禁品类别。提出APSAM（自适应点SAM）标注模型，解决SAM在跨域泛化和堆叠物体检测方面的限制。


<details>
  <summary>Details</summary>
Motivation: 当前X射线违禁品检测主要依赖边界框标注，缺乏像素级监督和真实世界数据，限制了模型的泛化能力和性能。需要构建大规模高质量分割数据集并开发高效标注工具。

Method: 1）构建XSeg数据集：整合公共数据集和合成数据，通过数据清洗管道过滤低质量样本。2）提出APSAM标注模型：基于SAM构建，引入能量感知编码器增强掩码解码器初始化，改进堆叠物体检测；设计自适应点生成器，仅需单个粗略点提示即可获得精确掩码标签。

Result: XSeg是目前最大的X射线违禁品分割数据集，包含98,644张图像和295,932个实例掩码。APSAM在XSeg上表现出优越性能，显著提高了标注效率和准确性。

Conclusion: XSeg数据集和APSAM标注模型有效解决了X射线违禁品分割中的数据稀缺和标注困难问题，为公共安全领域的违禁品检测提供了重要资源和技术支持。
...
TL;DR: 提出基于图学习的黑色素瘤检测方法，使用超像素集合图(SEG)和超像素层次图(SHG)两种图表示，通过优化学习边权重，在ISIC2017数据集上达到99%准确率


<details>
  <summary>Details</summary>
Motivation: 传统图信号处理中图结构和边权重通常通过统计和计算方法设置，缺乏灵活性。图结构学习方法能提供更可靠和灵活的数据表示，因此需要开发基于图学习的黑色素瘤检测方法

Method: 提出超像素集合图(SEG)和超像素层次图(SHG)两种图表示：SEG在多个层级生成超像素映射，SHG在相邻层级间有父子约束。探索两种边权重分配技术：手工高斯权重和基于优化的学习权重。节点信号基于纹理、几何和颜色特征。研究不同阈值(25%、50%、75%)剪枝最弱边的影响

Result: 在ISIC2017数据集上评估，使用数据增强缓解类别不平衡。结果显示，具有纹理节点信号的学习型超像素集合图性能最高，达到99.00%准确率和99.59% AUC

Conclusion: 基于图学习的黑色素瘤检测方法有效，特别是学习型超像素集合图结合纹理特征表现最佳，为皮肤病变分析提供了新的图表示方法
...
TL;DR: 提出一个紧凑的头发重建流水线，通过聚类头发丝为代表性发卡并共享纹理码本，结合3DGS渲染，显著减少重建时间和存储成本，同时保持视觉质量。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法虽然能实现逼真结果，但通常需要数百万个基元，导致高存储和渲染成本。观察到头发在发型中具有结构和视觉相似性，可以利用这种相似性进行优化。

Method: 1) 将头发丝聚类为代表性发卡；2) 将这些发卡分组到共享纹理码本中；3) 将此结构与3DGS渲染集成；4) 提出生成先验加速方法从图像集重建初始头发丝几何。

Result: 实现了4倍的重建时间减少，渲染性能相当，内存占用降低200倍以上。

Conclusion: 该方法通过利用头发结构的相似性，实现了高效、紧凑的高保真头发重建，在显著降低计算和存储成本的同时保持视觉质量。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频生成方法通常只能控制相机运动或物体动态中的一种，或者依赖模糊的2D线索，导致相机视差与真实物体运动纠缠不清。需要一种能同时精确控制相机轨迹和物体动态的统一框架。

Method: 提出SymphoMotion框架，包含：1) 相机轨迹控制机制，整合显式相机路径和几何感知线索，确保稳定的视角转换；2) 物体动态控制机制，结合2D视觉引导和3D轨迹嵌入，实现深度感知的空间一致物体操控。同时构建RealCOD-25K数据集，包含配对的相机位姿和物体级3D轨迹。

Result: 实验和用户研究表明，SymphoMotion在视觉保真度、相机可控性和物体运动准确性方面显著优于现有方法，为统一运动控制建立了新基准。

Conclusion: SymphoMotion成功实现了相机轨迹和物体动态的联合控制，解决了现有方法的局限性，并通过RealCOD-25K数据集填补了统一运动控制的关键数据空白，为视频生成中的精确运动控制提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前学术研究在多参考角色视频生成方面有限，当参考图像外观高度相似时，模型容易出现参考混淆问题，语义相似token会降低正确上下文检索能力

Method: 提出PoCo（Position Embedding as a Context Controller），将位置编码作为语义检索之外的额外上下文控制，利用token的侧信息实现精确的token级匹配，同时保持隐式语义一致性建模

Result: 大量实验表明，PoCo相比各种基线方法，在跨镜头一致性和参考保真度方面都有显著提升

Conclusion: PoCo能够可靠地控制具有极其相似视觉特征的角色，为多参考多镜头视频生成提供了有效解决方案
...
TL;DR: SA-DSVN是一个用于钢筋混凝土结构缺陷三维分割的卷积网络，通过联合处理宇宙射线μ子散射运动学和次级电磁簇射多重性，显著提升了缺陷检测精度。


<details>
  <summary>Details</summary>
Motivation: 传统μ子断层扫描方法（如POCA、MLSD）仅依赖散射角信息，忽略了次级电磁簇射多重性这一重要特征，限制了缺陷检测的准确性和鲁棒性。

Method: 提出双流3D卷积架构：一个流处理散射运动学（9通道），另一个流处理次级电磁簇射多重性（40通道），通过交叉注意力机制融合特征。使用Geant4模拟框架生成450万μ子事件训练数据。

Result: 在60个独立模拟验证体上达到96.3%体素精度，各类缺陷Dice分数0.59-0.81，体积级检测灵敏度100%，单个体积推理时间仅10毫秒。簇射多重性流贡献了主要判别能力。

Conclusion: 次级电磁簇射多重性是一个先前未被充分利用但非常有效的特征，为学习的μ子断层扫描重建开辟了新方向，显著提升了钢筋混凝土结构缺陷检测性能。
...
TL;DR: ICBench是一个新的大规模图像描述基准，包含12个内容类别，在2K图像上由10个先进MLLM生成了40K个短/长描述，并通过人类主观评估和提出的ITIScore自动指标进行综合评估。


<details>
  <summary>Details</summary>
Motivation: 现有图像描述基准存在三个主要问题：1) 描述长度多样性有限；2) 缺少最新先进MLLM的参与；3) 人类标注不足导致潜在偏见，限制了全面评估现代MLLM性能的能力。

Method: 1) 构建ICBench基准：覆盖12个内容类别，在2K图像上使用10个先进MLLM生成短/长描述，共40K个描述；2) 进行广泛人类主观研究，获得细粒度评估维度的平均意见分数；3) 提出基于图像-文本-图像框架的自动评估指标ITIScore，通过重建一致性衡量描述质量。

Result: 实验结果表明：1) ITIScore自动指标与人类判断具有强对齐性；2) 在其它公共描述数据集上展现出鲁棒的零样本泛化能力；3) 提供了包含人类评估的全面基准数据集。

Conclusion: ICBench解决了现有基准的局限性，通过多样化的描述长度、最新的MLLM参与和充分的人类标注，为全面评估现代MLLM的图像描述能力提供了新基准。提出的ITIScore指标能有效替代人类评估，数据集和模型将在发表后开源。
...
Method: 使用3D高斯泼溅作为3D表示，CLIP多模态知识作为风格参考；采用细分流进行精确特征对齐，将CLIP文本-视觉组合特征映射到VGG风格特征；引入观察损失和抑制损失优化生成过程。

Result: 实验表明M2StyleGS在视觉质量上表现更好，一致性比先前工作提升高达32.92%，能够生成风格增强的新视角序列。

Conclusion: M2StyleGS实现了支持文本和图像输入的实时3D风格迁移，通过多模态特征对齐和损失函数设计有效解决了异常变换问题，提升了风格一致性和视觉质量。
...
TL;DR: 本文首次系统比较了视觉语言模型(VLMs)和卷积神经网络(CNNs)在频谱热图理解任务中的表现，发现两者存在任务依赖的互补性：CNN在空间定位任务上表现更好，而VLM在语义推理任务上具有独特优势。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在无线网络管理中的应用正在加速，但目前缺乏对这些大型基础模型与轻量级卷积神经网络在频谱相关任务上性能差异的系统性理解。本文旨在填补这一空白，特别是在非地面网络与地面网络(NTN-TN)协同系统的频谱热图理解任务上。

Method: 提出了SpectrumQA基准测试，包含108K个视觉问答对，涵盖四个粒度级别：场景分类(L1)、区域推理(L2)、空间定位(L3)和语义推理(L4)。在三个NTN-TN场景中，比较了冻结的Qwen2-VL-7B模型和训练的ResNet-18模型。使用链式思维(CoT)提示来增强VLM推理能力，并设计了确定性任务类型路由器来分配任务。

Result: CNN在严重程度分类(L1)上达到72.9%准确率，在空间定位(L3)上达到0.552 IoU；而VLM仅用三个上下文示例就能实现语义推理(L4)，F1=0.576，这是CNN架构根本缺乏的能力。CoT提示将VLM推理性能提升12.6%。任务路由器将监督任务分配给CNN、推理任务分配给VLM，综合得分达到0.616，比单独使用CNN提升39.1%。VLM表示在跨场景鲁棒性方面更强。

Conclusion: CNN和VLM在频谱热图理解任务中存在任务依赖的互补性，不应将它们视为替代品。实际部署建议：使用CNN处理空间定位任务，使用VLM处理语义频谱推理任务。这种分工策略能显著提升整体性能。
...
TL;DR: 提出自动化精化框架，利用粗粒度CityGML模型作为几何先验，通过表面匹配和二进制整数优化选择最优面片，在保持拓扑有效性的同时提升立面几何精度。


<details>
  <summary>Details</summary>
Motivation: 传统ALS获取的CityGML建筑模型存在立面几何精度不足的问题，而MLS数据能提供高精度立面信息。现有重建方法会丢弃已有语义信息且依赖完整数据覆盖，需要一种能利用现有粗模型进行针对性精化的方法。

Method: 使用粗模型作为几何先验，通过表面匹配识别过时表面，采用二进制整数优化从候选数据中选择最优面片，并在优化中强制执行硬约束以确保拓扑有效性。

Result: 方法有效校正立面错位，将Cloud-to-Mesh RMSE降低约36%，实现厘米级对齐，同时保证输出几何的严格水密性和流形性。

Conclusion: 该框架为升级ALS衍生的城市模型提供了稳健解决方案，能在复杂城市环境中实现立面几何的针对性更新，同时保持拓扑完整性。
...
TL;DR: MoScale是一个从粗到细的多尺度自回归运动生成框架，通过分层生成运动实现了SOTA的文本到运动生成性能。


<details>
  <summary>Details</summary>
Motivation: 传统的自回归模型虽然训练稳定高效，但标准的下一token预测与文本条件运动生成所需的时间结构不对齐。需要一种更适合长程运动结构的因果层次框架。

Method: 提出MoScale框架：1）从粗到细的时间分辨率分层生成运动；2）在最粗尺度提供全局语义并逐步细化；3）引入跨尺度层次细化改进每尺度初始预测；4）尺度内时间细化进行选择性双向重新预测。

Result: MoScale实现了SOTA的文本到运动生成性能，训练效率高，模型规模可扩展，并能零样本泛化到多种运动生成和编辑任务。

Conclusion: MoScale通过建立从粗到细的因果层次结构，解决了自回归模型在文本条件运动生成中的对齐问题，在有限数据下表现出鲁棒性和泛化能力。
...
TL;DR: HistoFusionNet：一种用于夜间图像去雾的Transformer增强架构，结合了直方图引导的表征学习和频率自适应特征细化，在NTIRE 2026挑战赛中排名第一


<details>
  <summary>Details</summary>
Motivation: 夜间图像去雾面临多重挑战：雾霾、光晕、非均匀光照、颜色失真和传感器噪声，这些因素使白天去雾的常用假设失效。需要专门针对夜间复杂退化场景的解决方案。

Method: 基于多尺度编码器-解码器架构，引入直方图Transformer块，根据动态范围特征对特征进行分组以建模长距离依赖；同时加入频率感知细化分支，自适应利用低频和高频线索来恢复场景结构、抑制伪影并增强局部细节。

Result: 在NTIRE 2026夜间图像去雾挑战赛基准测试中表现出色，在22个参赛团队中排名第一，证明了该方法的有效性和鲁棒性。

Conclusion: HistoFusionNet通过结合直方图引导的表征学习和频率自适应特征细化，为夜间图像去雾提供了一个统一框架，特别适合处理真实夜间雾霾场景中的异质退化问题。
...
TL;DR: 提出基于注意力分布熵的补丁剪枝方法，通过低熵保留重要补丁、高熵剪枝冗余补丁，并使用Rényi熵增强灵活性，在细粒度图像识别中实现计算减少同时保持精度。
...
Method: 提出基于注意力分布Shannon熵的补丁重要性评估准则：低熵补丁（注意力集中）保留为重要，高熵补丁（注意力分散）视为冗余。进一步扩展到Rényi熵，强调尖锐的注意力峰值，支持适应任务需求和计算限制的剪枝策略。

Result: 在细粒度图像识别任务中，该方法在保持精度的同时减少了计算量。通过Rényi熵调整剪枝策略能获得进一步收益，改善精度与计算之间的权衡。

Conclusion: 基于注意力熵的补丁剪枝方法有效降低了Transformer的计算成本，同时保持模型性能。Rényi熵扩展提供了灵活的剪枝策略调整能力，为不同任务需求和计算限制提供了适应性解决方案。
...
TL;DR: EyeBench-V2是一个眼底图像增强模型的基准测试框架，旨在解决现有评估方法在临床相关性方面的不足，通过多维度临床对齐评估、专家指导设计和可操作见解来连接模型性能与临床效用。


<details>
  <summary>Details</summary>
Motivation: 现有眼底图像增强模型的评估存在三个主要问题：1) 传统去噪指标（如PSNR、SSIM）无法捕捉临床相关特征（如病灶保留和血管形态一致性）；2) 缺乏统一评估协议来处理配对和非配对增强方法；3) 需要提供可操作见解来指导临床对齐增强模型的未来发展。

Method: 提出EyeBench-V2基准测试，包含三个核心贡献：1) 通过下游任务进行多维度临床对齐评估，包括血管分割、糖尿病视网膜病变分级、未见噪声模式泛化和病灶分割；2) 专家指导的评估设计，构建新数据集支持配对和非配对方法的公平比较，并建立结构化的人工评估协议；3) 提供可操作见解，对现有生成模型进行严格的任务导向分析。

Result: 论文提出了一个全面的基准测试框架，但未提供具体模型性能数据。框架设计旨在为临床研究人员提供证据支持决策，并识别当前方法的局限性以指导下一代增强模型的设计。

Conclusion: EyeBench-V2基准测试填补了眼底图像增强模型评估与临床效用之间的空白，通过临床对齐的多维度评估、专家指导的设计和可操作见解，为临床研究人员提供决策依据，并推动下一代临床对齐增强模型的发展。
...
TL;DR: InCaRPose：基于Transformer的相机外参标定方法，专门针对车内监控等高度畸变环境，使用合成数据训练，能在单次推理中实现绝对度量尺度的相对位姿估计。


<details>
  <summary>Details</summary>
Motivation: 在车内监控等高度畸变、受限环境中，传统相机外参标定方法难以实现精确的相对位姿估计。需要一种能够处理鱼眼相机畸变、实现绝对度量尺度平移估计的方法，这对安全相关的感知任务至关重要。

Method: 提出InCaRPose架构，基于Transformer设计，利用冻结的DINOv3等骨干网络特征和Transformer解码器，从合成数据中学习图像对之间的几何关系。专门针对车内鱼眼相机畸变进行优化，能够在单次推理中实现物理合理的绝对度量尺度平移估计。

Result: 模型在真实车内环境中表现出良好的泛化能力，无需依赖相同的相机内参。在公开的7-Scenes数据集上达到竞争性性能。即使使用ViT-Small骨干网络，也能保持旋转和平移的高精度，实现实时推理性能。

Conclusion: InCaRPose为高度畸变环境中的相机外参标定提供了有效的解决方案，特别适用于车内监控等安全关键应用。通过合成数据训练和Transformer架构，实现了对真实环境的良好泛化，并发布了真实车内位姿测试数据集。
...
TL;DR: 论文提出了ActivityForensics，首个用于定位视频中篡改活动的大规模基准数据集，包含超过6K个伪造视频片段，并提出TADiff方法进行时序伪造定位。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注外观级伪造（如人脸交换、物体移除），但随着视频生成技术的发展，活动级伪造（修改人类动作以扭曲事件语义）日益严重，这些高度欺骗性的伪造严重损害媒体真实性和公众信任。

Method: 提出Temporal Artifact Diffuser (TADiff)，通过基于扩散的特征正则化器暴露伪影线索；创建ActivityForensics数据集，包含超过6K个无缝融入视频上下文的伪造片段。

Result: ActivityForensics数据集在视觉一致性上极高，人眼几乎无法区分；TADiff方法在时序伪造定位任务中表现有效；建立了涵盖域内、跨域和开放世界设置的全面评估协议。

Conclusion: 该工作填补了活动级视频伪造定位的研究空白，为未来研究提供了重要基准和数据集，有助于应对新兴的视频生成技术带来的媒体真实性挑战。
...
TL;DR: SPARK-IL是一个检索增强的AI生成图像检测框架，通过双路径频谱分析和增量学习，在19个生成模型上达到94.6%的平均准确率。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测器在特定生成器上训练后，往往无法泛化到未见过的模型。虽然像素级伪影在不同模型间差异很大，但频域特征表现出更强的一致性，这为跨生成器检测提供了有希望的基础。

Method: 提出SPARK-IL框架：1）双路径频谱分析：使用部分冻结的ViT-L/14编码器获取语义表示，同时并行处理原始RGB像素嵌入；2）多频带傅里叶分解：将两条路径分解为四个频带，分别由Kolmogorov-Arnold Networks（KAN）处理；3）交叉注意力融合：通过带残差连接的交叉注意力融合频谱嵌入；4）检索增强推理：使用余弦相似度从Milvus数据库中检索k个最近邻标记签名，通过多数投票进行预测；5）增量学习：扩展数据库并使用弹性权重巩固来保留先前学到的变换。

Result: 在UniversalFakeDetect基准测试中，涵盖19个生成模型（包括GANs、换脸和扩散方法），SPARK-IL达到了94.6%的平均准确率。

Conclusion: SPARK-IL通过结合双路径频谱分析和增量学习，有效解决了AI生成图像检测的泛化问题，在跨生成器检测任务上取得了优异性能，代码将公开发布。
...
Method: 提出任务引导的多标注三元组损失，通过互信息标准选择跨任务最具信息量的三元组，修改影响表示的样本而非调整损失幅度
...
TL;DR: 提出一种基于标注引导的特征增强框架，通过将标注几何信息注入目标检测主干网络，改善特征表示的质量和泛化能力


<details>
  <summary>Details</summary>
Motivation: 现代目标检测器学习的特征虽然优化了端到端任务损失，但往往捕捉到捷径相关性，未能反映底层标注结构。这种表示限制了在任务定义变化或监督稀疏时的迁移性、可解释性和鲁棒性

Method: 引入标注引导的特征增强框架，将标注引导的潜在空间嵌入注入目标检测主干网络。方法从标注引导的潜在空间构建密集空间特征网格，并将其与特征金字塔表示融合，以影响区域提议和检测头

Result: 在野生动物和遥感数据集上的实验评估了分类、定位和数据效率。结果显示在目标聚焦、背景敏感性降低以及对未见或弱监督任务的泛化能力方面都有持续改进

Conclusion: 将特征与标注几何对齐比纯粹任务优化的特征能产生更有意义的表示，证明了标注引导特征增强的有效性
...
TL;DR: 提出半监督知识蒸馏框架，将预训练视觉基础模型压缩为紧凑专家模型，在实例分割任务上使用有限标注和大量无标注数据，实现11倍压缩同时性能超越原模型和现有方法。


<details>
  <summary>Details</summary>
Motivation: 视觉基础模型感知能力强但计算量大，部署困难，且适应新任务需要昂贵的标注成本。需要一种方法在有限标注数据下压缩这些大模型，同时保持高性能。

Method: 三阶段框架：1) 通过自训练和对比校准进行域适应；2) 通过统一多目标损失进行知识转移；3) 学生模型精炼以减少伪标签偏差。核心是实例感知的像素级对比损失，融合掩码和类别分数来提取信息负样本并增强实例间边界。

Result: 在Cityscapes和ADE20K数据集上，约11倍小的学生模型相比零-shot基础模型教师提升+11.9和+8.6 AP，相比适应后的教师提升+3.4和+1.5 AP，并超越最先进的半监督知识蒸馏方法。

Conclusion: 提出的半监督知识蒸馏框架能有效压缩视觉基础模型，在有限标注数据下实现高性能的紧凑模型，通过对比学习和多阶段优化解决了域适应和知识转移的关键挑战。
...
TL;DR: 提出一个测试时框架，将先验知识（相机位姿、内参、深度）作为约束条件优化预训练的多视角Transformer，无需重新训练即可提升3D任务性能


<details>
  <summary>Details</summary>
Motivation: 现有的多视角Transformer在3D视觉任务中通常需要重新训练才能融入先验知识，这既耗时又需要大量标注数据。作者希望在不修改预训练网络架构的情况下，通过测试时优化来有效利用先验信息

Method: 提出测试时约束优化（TCO）框架：将先验知识转换为预测的约束条件，而不是直接输入网络。优化损失包括自监督目标（多视角预测一致性）和先验惩罚项。自监督目标通过光度或几何损失实现，先验知识则转换为对应输出模态的惩罚项

Result: 在ETH3D、7-Scenes和NRGBD等3D视觉基准测试中，该方法显著提升了基础多视角Transformer的性能。点云距离误差减少了一半以上，且优于重新训练的先验感知前馈方法

Conclusion: 测试时约束优化框架能够有效将先验知识融入3D视觉任务，无需重新训练预训练网络，在多个基准测试中表现出优越性能，为利用先验信息提供了灵活高效的解决方案
...
TL;DR: 论文首次系统研究视频稀疏自编码器，发现标准方法会破坏时间连贯性，提出时空对比目标和分层分组方法恢复时间连贯性，在动作分类和视频检索任务上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 标准稀疏自编码器在视频表示中会破坏时间连贯性：硬TopK选择导致跨帧特征分配不稳定，使自相关性降低36%。需要解决视频表示中时间连贯性与特征可解释性的平衡问题。

Method: 提出时空对比目标和Matryoshka分层分组方法：1）对比损失权重控制重建与时间连贯性的可调权衡；2）分层分组恢复并超越原始时间连贯性；3）在两个骨干网络和两个数据集上进行系统消融实验。

Result: 对比SAE特征在动作分类上比原始特征提升+3.9%，文本-视频检索提升达2.8倍R@1；跨骨干分析显示标准单义性指标存在骨干对齐伪影；因果消融证实对比训练将预测信号集中到少量可识别特征中。

Conclusion: 不同配置在不同目标上表现优异：重建保真度、时间连贯性、动作判别或可解释性。提出的方法有效平衡了视频表示的时间连贯性与特征可解释性，为视频理解任务提供了改进的特征表示。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像扩散模型面临生成视觉有害内容（如色情、暴力、恐怖图像）的挑战。现有安全干预方法存在两个关键限制：1）安全性与上下文保真度之间的严重权衡；2）容易受到对抗性攻击。

Method: 提出SafeCtrl框架，采用区域感知的安全控制方法，包含两个模块：1）注意力引导的检测模块精确定位特定风险区域；2）局部抑制模块通过图像级直接偏好优化（DPO）仅在中检测区域内中和有害语义，将不安全对象转换为安全替代品。

Result: 在多个风险类别上的广泛实验表明，SafeCtrl在安全性和保真度之间实现了优于最先进方法的权衡。该方法对对抗性提示攻击表现出更强的鲁棒性。

Conclusion: SafeCtrl提供了一个精确且鲁棒的解决方案，用于负责任的内容生成，通过局部化干预在保持上下文完整性的同时有效控制有害内容。
...
TL;DR: CM-GLasso：通过跨模态图套索和共同-特定结构学习，在多模态表示学习中解决模态不对齐、高维噪声和共享/特定拓扑混淆问题，在生成分类和密集语义分割任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏图估计技术（如图形套索）在多模态学习中面临高维噪声、模态不对齐以及共享拓扑与类别特定拓扑混淆的严重瓶颈，限制了可解释多模态表示的学习。

Method: 提出跨模态图形套索（CM-GLasso）：1）通过文本可视化策略和统一视觉-语言编码器将多模态特征严格对齐到共享潜在空间；2）引入跨注意力蒸馏机制，将高维补丁压缩为显式语义节点，提取空间感知的跨模态先验；3）将定制的GLasso估计和共同-特定结构学习统一为联合目标，通过ADMM优化。

Result: 在涵盖自然和医疗领域的八个基准测试上进行广泛实验，CM-GLasso在生成分类和密集语义分割任务上建立了新的最先进性能。

Conclusion: CM-GLasso通过克服多模态学习中的基本限制，能够同时解缠不变和类别特定的精度矩阵，避免了多步误差累积，为可解释多模态表示学习提供了有效解决方案。
...
TL;DR: VLA-Forget：针对视觉-语言-动作模型的混合遗忘框架，通过选择性编辑感知层和跨模态特异性，结合层选择性推理/动作遗忘，实现针对性遗忘同时保持感知和推理能力


<details>
  <summary>Details</summary>
Motivation: VLA模型在机器人操作中作为具身基础模型部署时面临新的遗忘挑战：需要移除不安全、虚假或隐私敏感行为，而不损害感知、语言理解和动作控制能力。传统遗忘方法仅针对视觉或语言模块，在具身环境中效果有限

Method: 提出VLA-Forget混合遗忘框架，结合感知层的比例感知选择性编辑与跨模态特异性，以及推理/动作层的层选择性遗忘。通过三阶段优化目标：针对性遗忘、感知保持、推理保留，对视觉编码器、投影器和上层动作生成变换器块进行分阶段更新

Result: 相比强基线方法，VLA-Forget将遗忘效果提升10%，感知特异性保持提升22%，推理和任务成功率保持提升9%，量化后恢复减少55%

Conclusion: VLA-Forget为VLA模型提供了有效的遗忘解决方案，能够针对性地移除不良行为，同时保持模型的感知和推理能力，在具身机器人应用中具有重要价值
...
TL;DR: 提出分层点-块异常评分网络，联合建模区域部件特征和局部点特征，用于3D形状异常检测，在公开和工业数据集上表现优异。
...
Method: 提出分层点-块异常评分网络，联合建模区域部件特征和局部点特征；自适应分块模块集成自监督分解以捕捉复杂结构偏差。

Result: 在公开基准（Anomaly-ShapeNet和Real3D-AD）和工业测试集上表现优异，新工业异常类型的点级改进超过40%，Real3D-AD平均对象级增益7%，Anomaly-ShapeNet增益4%。

Conclusion: 该方法在3D形状异常检测中表现出强大的鲁棒性和泛化能力，能有效处理多种异常类型和尺度。
...
<details>
  <summary>Details</summary>
Motivation: 现有参数高效的提示学习方法主要关注一阶视觉特征（空间特征图）与文本提示的对齐，虽然对细粒度语义区分有效，但这些空间纠缠特征对域偏移和局部噪声高度敏感，仅依赖一阶信息不足以实现鲁棒适应。

Method: 提出Gram-Anchored Prompt Learning (GAPL)框架，通过Gram矩阵引入二阶统计流，增强标准的一阶空间交互。将提示锚定到这些二阶先验，使语言表示能够动态适应不同域的统计分布偏移。

Result: 大量实验验证了二阶特征的有效性，GAPL在各种基准测试中表现出色。

Conclusion: 通过结合局部语义对齐与全局结构一致性，GAPL为视觉语言模型提供了一种更鲁棒的参数高效适应方法，特别适用于跨域场景。
...
TL;DR: HMAT是一种用于古代壁画修复的混合掩码感知Transformer框架，结合局部纹理建模和长距离结构推理，实现高保真修复。
...
Method: 提出混合掩码感知Transformer（HMAT）统一框架，包含：1）掩码感知动态滤波用于鲁棒的局部纹理建模；2）Transformer瓶颈用于长距离结构推理；3）掩码条件风格融合模块动态指导生成过程；4）教师强制解码器配合硬门控跳跃连接，确保有效区域保真度并聚焦缺失区域重建。

Result: 在DHMural数据集和九色鹿数据集上评估，HMAT在不同退化水平下均取得竞争性性能，相比现有方法能产生更结构一致、视觉保真的修复结果。

Conclusion: HMAT为文化遗产壁画的数字修复提供了有效解决方案，能够实现高保真修复并保持结构连贯性。
...
Method: 提出多模态排版攻击，系统研究跨模态的排版攻击如何影响MLLMs。分析音频、视觉和文本扰动之间的相互作用，展示协调的多模态攻击比单模态攻击更具威胁性。
...
TL;DR: 提出OASIC方法，通过遮挡严重程度估计和自适应模型选择，解决物体严重遮挡的计算机视觉问题


<details>
  <summary>Details</summary>
Motivation: 物体严重遮挡是计算机视觉的主要挑战，原因包括可见信息丢失和遮挡物产生的干扰模式

Method: 1) 测试时通过掩码去除遮挡物干扰模式；2) 训练时随机掩码物体部分模拟不同遮挡程度；3) 估计测试图像遮挡严重程度并选择对应优化模型

Result: OASIC方法在遮挡图像上比标准训练提升18.5 AUC_occ，比在未遮挡图像上微调提升23.7 AUC_occ

Conclusion: 结合灰度掩码和自适应模型选择的OASIC方法能有效处理严重遮挡问题，优于单一模型
...
TL;DR: HOIGS是一种新方法，通过交叉注意力机制显式建模人-物交互变形，使用HexPlane和CHS分别提取人和物的特征，在复杂交互场景中实现高质量动态重建。


<details>
  <summary>Details</summary>
Motivation: 现有高斯泼溅方法要么依赖人体姿态先验而忽略动态物体，要么将所有运动近似在单一场中，无法有效捕捉交互丰富的动态场景。

Method: 提出HOIGS方法，通过交叉注意力HOI模块显式建模人-物交互变形，使用HexPlane提取人体特征，Cubic Hermite Spline提取物体特征，整合异构特征捕捉相互依赖的运动。

Result: 在多个数据集上的实验表明，HOIGS在遮挡、接触和物体操纵等场景中显著优于现有最先进的人体中心和4D高斯方法。

Conclusion: 显式建模人-物交互对于高保真动态场景重建至关重要，HOIGS通过异构特征整合有效捕捉了交互诱导的变形。
...
TL;DR: 1.x-Distill是一个分数步蒸馏框架，打破了传统整数步限制，在1.x步内实现高质量图像生成，相比原始扩散模型实现33倍加速。
...
Method: 1. 分析教师模型CFG在DMD中的作用并引入简单修改抑制模式崩溃；2. 提出阶段化聚焦蒸馏：第一阶段通过多样性保持分布匹配学习粗结构，第二阶段通过推理一致对抗蒸馏细化细节；3. 设计轻量补偿模块实现蒸馏-缓存协同训练。

Result: 在SD3-Medium和SD3.5-Large上的实验表明，1.x-Distill在1.67和1.74有效NFE下超越了现有少步方法，实现了更好的质量和多样性，相比原始28x2 NFE采样实现了高达33倍加速。

Conclusion: 1.x-Distill首次打破了整数步限制，建立了1.x步生成作为蒸馏扩散模型的实用机制，在极少数步数下实现了高质量、多样化的图像生成。
...
TL;DR: 提出ATSS方法，利用AI生成视频中异常时间自相似性特征进行检测，通过三重相似度表示和跨模态注意力融合，显著优于现有方法。
...
Method: 提出ATSS方法，利用AI生成视频中的异常时间自相似性特征。通过构建视觉、文本和跨模态相似度矩阵来量化时间异常，使用专用Transformer编码器编码，并通过双向跨注意力融合模块整合多模态动态信息。

Result: 在GenVideo、EvalCrafter、VideoPhy和VidProM四个大规模基准测试中，ATSS在AP、AUC和ACC指标上显著优于最先进方法，展现出对不同视频生成模型的优越泛化能力。

Conclusion: ATSS通过捕捉AI生成视频中异常的时间自相似性模式，提供了一种有效的多模态检测框架，能够更好地理解视频的生成逻辑，显著提升检测性能。
...
TL;DR: TORA提出了一种拓扑优先表示对齐框架，通过从预训练的3D编码器中提取关系结构到流匹配主干中，显著提升了3D形状装配的性能和收敛速度。


<details>
  <summary>Details</summary>
Motivation: 现有流匹配方法在3D形状装配中学习点级速度场，但缺乏对跨部件交互的明确指导，导致装配过程不够高效和鲁棒。

Method: 提出TORA框架：1) 使用token-wise余弦匹配将教师模型的几何描述符注入学生模型；2) 采用中心核对齐(CKA)损失匹配师生表示间的相似性结构；3) 在transformer后期层进行对齐，因为空间结构在此自然涌现。

Result: 零推理开销下实现：收敛速度提升6.9倍，分布内精度提高，域偏移下鲁棒性增强。在5个基准测试中达到SOTA，在零样本迁移到未见数据集上表现尤为突出。

Conclusion: 几何和接触为中心的教师属性（而非语义分类能力）决定了对齐效果，在transformer后期层进行对齐最有效。TORA为3D形状装配提供了一种高效的知识蒸馏框架。
...
TL;DR: DINO-VO是一个端到端单目视觉里程计系统，通过可微分自适应补丁选择器和多任务特征提取模块，结合可微分光束法平差，实现了跨场景的强泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉里程计系统通常依赖启发式特征提取策略，在大规模室外环境中准确性和鲁棒性会下降。需要解决特征提取质量不足和跨数据集泛化能力差的问题。

Method: 1. 引入可微分自适应补丁选择器到端到端流程中；2. 集成多任务特征提取模块；3. 结合可微分光束法平差模块，利用逆深度先验；4. 有效学习外观和几何信息。

Result: 在TartanAir、KITTI、Euroc和TUM数据集上的广泛实验表明，DINO-VO在合成、室内和室外环境中都表现出强大的泛化能力，达到了最先进的跟踪精度。

Conclusion: DINO-VO通过创新的可微分自适应补丁选择器和端到端学习框架，成功解决了传统VO系统的局限性，在跨场景泛化方面取得了显著进展。
...
TL;DR: 4C4D：一种从极稀疏相机（最少4个）视频中恢复4D动态场景的新框架，通过神经衰减函数增强4D高斯的几何建模能力，实现高质量4D高斯泼溅


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要密集的多视角相机阵列（数十甚至上百个相机）来建模动态场景，限制了实际应用。本文旨在解决从极稀疏相机（最少4个）视频中恢复4D动态场景的挑战。

Method: 提出4C4D框架，核心是引入神经衰减函数作用于高斯不透明度，增强4D高斯的几何建模能力。该设计通过让4DGS梯度更关注几何学习，缓解了4DGS中几何与外观建模的不平衡问题。

Result: 在具有不同相机重叠度的稀疏视角数据集上进行广泛实验，4C4D相比现有方法取得了优越的性能表现。

Conclusion: 4C4D框架成功实现了从极稀疏相机视频中进行高保真4D高斯泼溅，为解决稀疏视角下的动态场景建模提供了有效方案。
...
TL;DR: 提出一种基于正例-无标签学习的策展人参与重复发现方法，使用轻量级克隆编码器和可解释阈值，在CIFAR-10和AtticPOT数据集上取得优异性能


<details>
  <summary>Details</summary>
Motivation: 解决AtticPOT存储库中策展人参与的重复发现问题，传统方法需要大量标注数据，而实际场景中通常只有少量正例锚点，需要一种无需显式负例样本的方法

Method: 将问题建模为正例-无标签学习问题，为每个文物使用单个锚点，通过锚点的增强视图训练轻量级克隆编码器，在潜在空间使用l_2范数阈值进行可解释的相似度评分

Result: 在CIFAR-10上获得F1=96.37(AUROC=97.97)，在AtticPOT上达到F1=90.79(AUROC=98.99)，比最佳基线SVDD提升7.70个F1点，同时保持轻量级架构

Conclusion: 该方法避免了显式负例需求，提供透明操作点，适用于去重、记录链接和策展人参与的工作流程，能够发现跨记录重复项
...
TL;DR: 提出一个基于YOLOv11和BoT-SORT/ByteTrack的离线实时交通监控系统，实现车辆检测与计数，准确率达66.67-95.83%，为智慧城市提供AI驱动的交通监控解决方案。


<details>
  <summary>Details</summary>
Motivation: 随着计算机视觉和人工智能的发展，交通监控系统需要现代化升级。现有系统往往依赖云端服务，存在延迟和隐私问题。本文旨在开发一个离线、实时的交通监控系统，减少对云端的依赖，提高系统的可访问性和实用性。

Method: 采用预训练的YOLOv11目标检测器结合BoT-SORT/ByteTrack多目标跟踪算法，使用PyTorch/OpenCV实现，并封装在基于Qt的桌面用户界面中。整个系统为CNN管道，能够从视频流中高效检测和计数车辆。

Result: 系统在不同场景下达到66.67-95.83%的计数准确率。类别检测方面：汽车精度0.97-1.00，召回率0.82-1.00，F1分数0.90-1.00；卡车精度1.00，召回率0.70-1.00，F1分数0.82-1.00。恶劣天气可能影响性能，但在正常条件下表现稳健。

Conclusion: 通过集成轻量级模型和可访问的、不依赖云端的界面，本文展示了AI驱动交通监控系统的能力，为智慧城市的现代化和发展做出了贡献。系统在典型条件下表现良好，但恶劣天气下的性能需要进一步改进。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测方法通常依赖二元分类器和隐式注意力机制，这些方法难以泛化到已知篡改方法之外，特别是面对高质量伪造时表现不佳。

Method: 提出LAA-X框架，采用基于混合数据合成的多任务学习框架，通过辅助任务引导模型关注局部伪影易发区域，兼容CNN和Transformer骨干网络（分别称为LAA-Net和LAA-Former）。

Result: 尽管仅使用真实和伪伪造样本进行训练，LAA-X在多个基准测试中与最先进方法竞争，表现出对高质量伪造的鲁棒性和良好的泛化能力。

Conclusion: LAA-X通过显式注意力策略和多任务学习，有效解决了深度伪造检测中的泛化问题，为实际应用提供了有前景的解决方案。
...
Result: 在2小时预测中达到R²=0.783，F1分数84.25%，预测区间覆盖概率92.38%，消融分析证实数字孪生特征和多模态融合显著提升性能。
...
TL;DR: 提出HGR框架，通过可修正假设节点和依赖感知图记忆，实现语义预测引导探索与错误级联修正，提升具身智能导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有图导航系统将未探索区域视为语义未知，导致探索效率低；视觉语言模型的语义预测错误会嵌入记忆并传播，造成结构错误累积，需要能利用语义预测进行定向探索并系统修正错误的框架。

Method: HGR框架包含：1) 语义假设模块，估计前沿区域的上下文条件语义分布，按目标相关性、旅行成本和不确定性排序探索目标；2) 验证驱动级联修正，比较现场观察与预测语义，不匹配时撤回被驳斥节点及其所有下游依赖节点。

Result: 在GOAT-Bench上达到72.41%成功率和56.22% SPL；在A-EQA和EM-EQA问答基准上持续改进；级联修正消除约20%结构冗余假设节点，将错误区域重访减少4.5倍，67%预测错误由镜面和透明表面引起。

Conclusion: HGR通过可修正假设节点和依赖感知图记忆，实现了语义预测引导探索与系统错误修正，相比传统图导航系统显著提升了长期导航的可靠性和效率。
...
<details>
  <summary>Details</summary>
Motivation: SAR图像舰船检测面临相干斑点噪声、复杂海岸杂波和小尺度目标的挑战。传统光学图像检测器对SAR特定退化鲁棒性有限，且空间下采样会丢失细粒度舰船特征。

Method: 提出SARES-DEIM检测框架，核心是SARESMoE模块（稀疏门控机制选择性路由特征到频率和小波专家），以及Space-to-Depth Enhancement Pyramid（SDEP）颈部结构保留浅层高分辨率空间线索。

Result: 在HRSID数据集上达到mAP50:95 76.4%和mAP50 93.8%，优于最先进的YOLO系列和专用SAR检测器。

Conclusion: SARES-DEIM通过域感知设计和稀疏激活架构，有效解决SAR图像舰船检测中的噪声、杂波和小目标问题，在多个基准数据集上表现出优越性能。
...
TL;DR: VoxelFM是一个3D CT基础模型，使用DINO框架通过自蒸馏训练，无需语言监督即可学习丰富的语义特征，在七类临床下游任务中表现优于现有CT基础模型。


<details>
  <summary>Details</summary>
Motivation: 现有CT基础模型主要关注构建能够进行问答和报告生成的通用视觉语言系统，但这需要大规模配对的图像-文本数据，而CT领域缺乏此类数据。此外，适应下游任务通常需要部分或完整的骨干网络微调，计算成本高，许多研究团队无法承担。因此，需要优先学习鲁棒的视觉表示，以便用少量标注数据且无需骨干微调即可高效迁移到新任务。

Method: 提出VoxelFM，一个3D CT基础模型，使用DINO框架通过自蒸馏进行训练，无需语言监督即可学习语义丰富的特征。在下游任务评估中，使用冻结的骨干表示和轻量级探针（probes）进行七类临床相关任务：分类、回归、生存分析、实例检索、定位、分割和报告生成。

Result: VoxelFM在七类任务中匹配或优于四个现有CT基础模型。尽管在预训练期间没有语言监督，VoxelFM在包括报告生成在内的任务中超越了明确使用语言对齐目标训练的模型。结果表明，当前CT基础模型作为轻量级探针的特征提取器表现更好，而不是作为视觉语言模型的视觉编码器。

Conclusion: VoxelFM展示了无需语言监督的自蒸馏方法在CT基础模型中的有效性，学习到的视觉表示能够高效迁移到多种临床任务，且优于现有方法。模型权重和训练代码已公开。
...
TL;DR: NTIRE 2026 3D修复与重建挑战赛综述：评估在极端低光和烟雾退化条件下的鲁棒3D重建方法
...
Method: 使用RealX3D基准数据集，组织279名参与者（33个有效提交团队）的竞赛，全面评估提交方法并与最先进基线进行比较

Result: 挑战赛显示了在恶劣条件下3D重建的显著进展，揭示了顶级方法的共享设计原则和有效处理3D场景退化的策略

Conclusion: NTIRE 2026 3DRR挑战赛推动了鲁棒3D重建技术的发展，为处理现实世界恶劣条件提供了重要见解和有效方法
...
TL;DR: 提出新的非均匀曝光校正范式，通过空间自适应调制权重和HSL补偿模块处理空间非均匀曝光误差，配合不确定性引导的非均匀损失函数优化


<details>
  <summary>Details</summary>
Motivation: 现实世界中的曝光校正面临空间非均匀退化的挑战，现有方法通常基于均匀假设，使用全局聚合的调制信号，无法处理图像中同时存在的多种曝光误差

Method: 1) 空间信号编码器预测空间自适应调制权重，指导多个查找表进行图像变换；2) HSL补偿模块提升色彩保真度；3) 不确定性引导的非均匀损失函数，根据局部恢复不确定性动态分配优化焦点

Result: 在广泛实验中，该方法在定性和定量评估上都优于现有最先进方法

Conclusion: 提出了一种专门针对空间非均匀性的曝光校正新范式，通过空间自适应调制和不确定性引导的优化策略，有效处理现实世界中的复杂曝光误差
...
TL;DR: OP-GRPO：首个面向流匹配模型的离策略GRPO框架，通过轨迹选择、重要性采样修正和轨迹截断，将训练步骤减少到34.2%，显著提升训练效率


<details>
  <summary>Details</summary>
Motivation: GRPO在提升流匹配模型生成质量方面表现出色，但其在线策略训练范式导致样本效率低下，需要大量训练步骤

Method: 1) 主动选择高质量轨迹并存入回放缓冲区供后续重用；2) 提出序列级重要性采样修正，保持GRPO裁剪机制完整性；3) 理论分析和实验表明后期去噪步骤会导致病态离策略比率，通过截断后期步骤轨迹来缓解

Result: 在图像和视频生成基准测试中，OP-GRPO仅用平均34.2%的训练步骤就达到或超越了Flow-GRPO的性能，在保持生成质量的同时大幅提升训练效率

Conclusion: OP-GRPO成功解决了GRPO样本效率低的问题，通过离策略训练框架显著减少了训练成本，为流匹配模型的高效训练提供了有效解决方案
...
TL;DR: 提出了一种不确定性感知的测试时适应框架，专门用于遥感回归任务中的时空融合，通过仅更新预训练模型的融合模块，在未见地理区域上显著提升性能。
...
Method: 提出不确定性感知的测试时适应框架：1) 仅更新预训练时空融合模型的融合模块；2) 利用认知不确定性指导适应过程；3) 结合土地利用和土地覆盖一致性约束；4) 加入偏差校正机制；5) 无需源数据或目标区域标注样本。

Result: 在四个不同气候的目标区域（意大利罗马、埃及开罗、西班牙马德里、法国蒙彼利埃）进行实验，相比在法国奥尔良预训练的模型，RMSE和MAE分别平均提升24.2%和27.9%，即使使用有限的无标注目标数据和仅10个适应轮次。

Conclusion: 该不确定性感知测试时适应框架有效解决了遥感回归任务中的域偏移问题，显著提升了时空融合模型在未见地理区域的泛化能力，为遥感回归任务的域适应提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 字体形状能唤起广泛印象，但字体与印象描述并非一一对应：有些印象与多种风格兼容，而有些则强烈限制字体选择。这种分级约束强度称为风格特异性，需要量化建模

Method: 提出双曲共嵌入框架，将字体图像和印象描述（单标签或标签集）嵌入共享双曲空间，使用两种互补的蕴含约束：印象到字体的蕴含，以及印象间从低到高风格特异性的蕴含

Result: 在MyFonts数据集上实验显示，相比强的一对一基线方法，双向检索性能有提升。学习到的空间捕获了从模糊到风格特异性印象的连贯进展，提供了有意义的数据驱动风格特异性量化
...
TL;DR: 提出一种针对双缺失场景（视图和标签都不完整）的多视图多标签学习方法，通过共享码本学习离散一致表示，并设计权重估计和融合教师自蒸馏框架提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有的多视图多标签学习方法主要关注视图缺失，但在视图和标签都缺失的双缺失场景下研究不足。现有方法主要依赖对比学习或信息瓶颈理论，但缺乏显式结构约束，难以捕获稳定且具有判别性的共享语义。

Method: 1) 通过多视图共享码本和跨视图重建学习离散一致表示，在有限的共享码本嵌入中自然对齐不同视图；2) 设计权重估计方法评估各视图保留标签相关结构的能力，据此分配权重增强融合预测质量；3) 提出融合教师自蒸馏框架，用融合预测指导视图特定分类器训练，并将全局知识反馈到单视图分支。

Result: 在五个基准数据集上与先进方法进行广泛对比实验，充分证明了所提方法的有效性。

Conclusion: 提出的方法通过共享码本学习结构化一致表示，结合权重估计和自蒸馏框架，有效解决了多视图多标签学习中的双缺失问题，提高了模型的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 科学论文中的"Figure 1"通常是核心研究思想的视觉总结，这些图表视觉简洁但概念丰富，需要人类作者大量努力和迭代才能完成，这凸显了科学视觉传播的难度。因此需要建立一个基准来评估AI模型在这方面的能力。

Method: 从顶级深度学习会议论文中筛选数据，进行严格质量控制，并引入与专家判断相关性良好的自动评估指标。评估一系列代表性模型在GENFIG1上的表现。

Result: GENFIG1任务对现有最佳性能系统也构成显著挑战，表明当前模型在科学视觉生成方面仍有很大提升空间。
...
TL;DR: 本文提出了一种针对极端远距离视频行人重识别的稳定适应方法，通过升级视觉骨干网络、引入选择性微调、轻量级时序注意力池化等技术，在DetReIDX基准上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 极端远距离视频行人重识别面临尺度压缩、分辨率下降、运动模糊和空地视角不匹配等挑战。随着相机高度和主体距离增加，基于近距离图像训练的模型性能显著下降，需要开发更鲁棒的适应方法。

Method: 1. 从CLIP基线出发，将视觉骨干从ViT-B/16升级到ViT-L/14；2. 引入骨干感知选择性微调以稳定大Transformer的适应；3. 采用轻量级时序注意力池化机制抑制退化帧并强调信息丰富的观测；4. 保留基于适配器和提示条件的跨视角学习以缓解空地领域偏移；5. 使用改进的优化和k-互惠重排序进一步优化检索。

Result: 在DetReIDX压力测试基准上，方法取得了mAP分数：A2G为46.69，G2A为41.23，A2A为22.98，总体mAP为35.73。结果表明大规模视觉语言骨干网络结合稳定性导向的适应能显著增强极端远距离视频行人重识别的鲁棒性。

Conclusion: 大规模视觉语言骨干网络结合稳定性导向的适应策略能有效提升极端远距离视频行人重识别的性能，为解决尺度压缩、分辨率下降和视角不匹配等挑战提供了有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有VideoLLM大多是离线系统，不适合需要持续观察和及时响应的实时视频流。当前的流式VideoLLM方法依赖解耦的触发-响应管道或仅限于字幕式叙述，在开放式问答和长时程交互方面效果有限。

Method: 提出AURA框架，集成上下文管理、数据构建、训练目标和部署优化，实现稳定的长时程流式交互。采用端到端设计，使统一的VideoLLM能够持续处理视频流。

Result: 在流式基准测试中达到最先进性能，支持实时演示系统（包含ASR和TTS），在2个80G加速器上以2FPS运行。发布了AURA模型和实时推理框架。

Conclusion: AURA为实时视频流交互提供了有效的端到端解决方案，解决了现有流式VideoLLM的局限性，推动了流式视觉交互研究的发展。
...
TL;DR: GraphicDesignBench (GDB) 是首个针对专业平面设计任务的综合基准套件，涵盖布局、排版、信息图表、模板与设计语义、动画五大维度，评估显示当前AI模型在空间推理、矢量代码生成、排版感知等核心设计挑战上仍有不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注自然图像理解或通用文本到图像合成，缺乏专门针对专业平面设计独特挑战的评估工具。专业设计需要将沟通意图转化为结构化布局、渲染排版准确的文本、操作分层组合、生成有效矢量图形以及动画推理等能力。

Method: 构建包含50个任务的基准套件，按布局、排版、信息图表、模板与设计语义、动画五个维度组织，每个维度在理解和生成两种设置下评估。基于LICA分层组合数据集的真实设计模板，使用标准化指标分类法评估空间准确性、感知质量、文本保真度、语义对齐和结构有效性。

Result: 评估前沿闭源模型显示，当前模型在专业设计的核心挑战上表现不足：复杂布局的空间推理、忠实矢量代码生成、细粒度排版感知、动画的时间分解等问题基本未解决。虽然高层语义理解可达，但随着任务对精度、结构和组合意识的要求提高，差距显著扩大。

Conclusion: GDB为追踪AI系统向有能力的设计协作伙伴发展提供了严格、可复现的测试平台。当前AI模型距离成为真正的设计协作工具仍有显著差距，特别是在需要精确性、结构化和组合意识的专业设计任务上。
...
TL;DR: DriveVA是一个自动驾驶世界模型，通过共享潜在生成过程联合解码未来视觉预测和动作序列，利用预训练视频生成模型的先验知识，在跨数据集和传感器配置的泛化能力上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有基于世界模型的规划方法在跨数据集和传感器配置的泛化能力有限，且规划与场景演化的松散耦合导致视频-轨迹一致性差。需要解决自动驾驶中的泛化挑战，特别是在未见场景、传感器域和环境条件下的鲁棒性能。

Method: 提出DriveVA模型，采用基于DiT的解码器联合预测未来动作序列（轨迹）和视频，实现规划与场景演化的紧密对齐。引入视频延续策略增强长时滚转一致性，并利用预训练大规模视频生成模型的运动动力学和物理合理性先验知识。

Result: 在NAVSIM挑战中获得90.9 PDM分数的闭环性能。在nuScenes上平均L2误差和碰撞率分别降低78.9%和83.3%，在基于CARLA v2的Bench2drive上分别降低52.5%和52.4%，展示了优异的零样本能力和跨域泛化性能。

Conclusion: DriveVA通过共享潜在生成过程联合解码视觉预测和动作序列，有效解决了自动驾驶世界模型的泛化挑战，在跨数据集和传感器配置上表现出卓越的零样本能力和性能提升。
...
TL;DR: 该论文提出了一个统一的设计框架3DPHDL，系统地将持久同调整合到3D点云深度学习中，通过六个注入点将拓扑作为结构性归纳偏置，并在多个任务和骨干网络上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管持久同调具有强大的理论保证和越来越多的实证应用，但在点云深度学习中的整合仍然零散且处于架构边缘。现有方法通常将持久同调作为辅助特征计算和向量化，缺乏系统性的整合框架。

Method: 提出了3DPHDL统一设计框架，形式化了复杂构造、过滤策略、持久性表示、神经骨干网络和预测任务之间的相互作用。识别了六个原则性注入点：采样、邻域图、优化动态、自监督、输出校准和内部网络正则化。在ModelNet40分类和ShapeNetPart分割任务上，系统地将持久同调图、图像和景观与PointNet、DGCNN和Point Transformer等骨干网络结合。

Result: 实验结果表明，在拓扑敏感的判别性和部件一致性方面获得了一致性改进，同时揭示了表示表达能力与组合复杂性之间的权衡。该方法提高了对噪声和采样变化的鲁棒性，并保持了计算可扩展性。

Conclusion: 通过将持久同调视为学习流程中的结构化组件而非仅仅是辅助特征，该工作为将拓扑推理整合到3D点云学习中提供了系统性框架，展示了拓扑作为结构性归纳偏置的潜力。
...
TL;DR: HighFM：首个针对高时间分辨率多光谱地球观测数据的基础模型，利用SEVIRI卫星数据通过掩码自编码学习稳健的时空表征，在云掩码和火灾检测任务上优于传统方法和现有地理空间基础模型。


<details>
  <summary>Details</summary>
Motivation: 气候相关灾害频发加剧了对实时监测、早期预警和知情决策的需求。现有基础模型主要依赖高分辨率但低重访率的卫星影像，难以应对快速演变现象和时间紧迫的应急响应。

Method: 基于SatMAE掩码自编码框架，利用超过2TB的SEVIRI气象卫星数据，学习稳健的时空表征。为支持实时监测，增强原始架构，加入细粒度时间编码以捕捉短期变化。预训练模型在云掩码和活跃火灾检测任务上进行微调。

Result: SEVIRI预训练的Vision Transformers在平衡准确率和IoU指标上均优于传统基线和近期地理空间基础模型，展示了时间密集的地球静止数据在实时地球观测中的潜力。

Conclusion: 高时间分辨率地球静止数据为实时地球观测提供了重要潜力，为灾害检测和跟踪的基础模型提供了可扩展路径。HighFM是迈向高时间分辨率多光谱地球观测数据基础模型的重要第一步。
...
Method: 1) 使用运动感知模块分割和移除动态区域；2) 使用扩散模型修复遮挡区域，提供伪地面真值监督；3) 为每个高斯基元引入可学习的真实性标量，在渲染和监督中动态调节不透明度；4) 构建Trajectory-Match数据集进行定量评估。

Result: 在DAVIS和自建数据集上的实验表明，GA-GS在静态场景重建方面达到最先进性能，特别是在大规模、持续遮挡的挑战性场景中表现优异。

Conclusion: GA-GS通过生成辅助和可学习真实性标量的创新方法，有效解决了被动态物体遮挡区域的静态场景重建问题，为相关应用提供了更完整的场景重建能力。
...
TL;DR: SW-CLIP将空间自相关引入视觉语言对比学习，通过距离感知软监督改进街景地理定位，相比传统CLIP方法显著提升精度和空间一致性。


<details>
  <summary>Details</summary>
Motivation: 传统CLIP方法在地理定位中将所有非匹配样本视为同等负样本，忽略了地理空间关系。需要将空间自相关原则融入多模态表示学习，实现从语义对齐到地理对齐的转变。

Method: 1) 引入位置作为文本表示编码地理坐标；2) 用基于测地距离的空间加权软标签替换one-hot InfoNCE目标；3) 采用邻域一致性正则化保持嵌入空间中的局部空间结构。

Result: 在多城市数据集上，SW-CLIP显著提升地理定位精度，减少长尾误差，增强空间一致性，优于标准CLIP方法。

Conclusion: 将空间原则融入多模态表示学习对于鲁棒地理定位至关重要，SW-CLIP为整合地理学第一定律到视觉语言模型提供了通用范式。
...
TL;DR: 提出了一种降低ELM网络分类器测试时计算成本的技术，通过使用整数运算代替浮点运算，在保持分类精度的同时减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 在嵌入式应用和数据中心中，功耗是重要限制因素。传统ELM分类器在测试时需要大量浮点运算，计算成本高，需要降低测试时的计算开销。

Method: 1) 使用三元集合{-1,0,1}作为输入权重，消除乘法运算；2) 证明归一化和非归一化测试信号的分类精度相同；3) 创建输出权重的整数版本，减少精度损失。

Result: 在5个常用计算机视觉数据集上测试，结果表明这些技术可以在FPGA上显著降低测试时的计算成本，同时保持分类精度。

Conclusion: 提出的整数运算技术能够有效降低ELM分类器测试时的计算成本，适用于功耗受限的嵌入式应用和大型数据中心，具有实际应用价值。
...
TL;DR: G2F-RAG提出了一种免训练的视觉空间知识融合方法，通过构建视频知识图谱并将其渲染为推理帧，在统一视觉域中进行视频推理，降低认知负荷并提高可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型（LMMs）的视频推理系统在需要外部知识时采用检索增强方法，但将文本或多片段证据附加到视频中会导致异构信号在单一注意力空间中融合，造成注意力稀释和认知负荷增加，即使对非长视频也是如此。瓶颈不仅在于检索什么，还在于如何表示外部知识并与视频主干网络融合。

Method: 提出Graph-to-Frame RAG（G2F-RAG）方法：1）离线阶段：构建问题无关的视频知识图谱，整合实体、事件、空间关系和链接的世界知识；2）在线阶段：分层多智能体控制器决定是否需要外部知识，检索最小充分子图，并将其渲染为单个推理帧附加到视频中；3）LMMs在统一视觉域中进行联合推理。

Result: G2F-RAG在多个公共基准测试中取得一致增益，在知识密集型场景中改进更大。该方法可跨主干网络和规模即插即用，消融实验证实知识表示和传递方式确实重要。

Conclusion: G2F-RAG将检索重新定义为视觉空间知识融合，为稳健且可解释的视频推理提供了新范式，降低了认知负荷并保留了明确、可检查的证据轨迹。
...
TL;DR: RLER提出了一种双范式方法，将证据生成与可靠答案获取解耦，通过强化学习训练模型生成结构化证据，在推理时通过证据加权选举提高视频推理的可靠性和可解释性。
...
Method: 采用双范式：RLER-Training使用群体相对强化学习和三个任务驱动奖励（帧敏感奖励、思维透明奖励、反重复奖励）训练模型生成结构化证据；RLER-Inference使用无训练协调器生成多样化候选，解析答案和引用帧，通过证据一致性、置信度、透明度和非冗余性评分，进行证据加权选举。

Result: 在8个代表性基准测试中，RLER实现了最先进性能，相比基础模型平均提升6.3%，每个问题平均使用3.1个候选，在计算成本和质量之间取得了良好平衡。
...
TL;DR: 提出BiTDiff框架和CM-Data数据集，解决3D指挥动作生成中的数据和算法挑战，实现高质量长序列动作合成


<details>
  <summary>Details</summary>
Motivation: 3D指挥动作生成在音乐教育、虚拟表演等领域有广泛应用，但面临两大挑战：缺乏大规模精细3D指挥数据集，以及缺乏能同时支持高质量和高效长序列生成的方法

Method: 1) 构建CM-Data数据集：约10小时SMPL-X格式的精细指挥动作数据；2) 提出BiTDiff框架：基于BiMamba-Transformer混合架构进行高效长序列建模，采用扩散生成策略和人体运动学分解实现高质量动作合成，引入物理一致性损失和手部/身体特定前向运动学设计

Result: BiTDiff在CM-Data数据集上实现了SOTA性能，支持无训练联合级动作编辑，促进人机交互设计

Conclusion: 通过构建首个大规模公开3D指挥数据集CM-Data和提出BiTDiff框架，有效解决了3D指挥动作生成的数据和方法限制，为相关应用提供了有力支持
...
TL;DR: 提出UENR-600K大规模夜间视频去雨数据集，使用Unreal Engine 3D粒子模拟真实物理雨滴特性，并基于Wan 2.2视频生成模型建立新SOTA基线


<details>
  <summary>Details</summary>
Motivation: 夜间视频去雨面临独特挑战：雨滴与人工光源交互产生彩色雨滴和局部照明效果。现有小规模合成数据集使用2D雨滴叠加，无法捕捉这些物理特性，导致模型在真实夜间雨景中泛化能力差。同时，采集真实配对夜间视频不现实，因为雨效无法与其他退化（如传感器噪声）分离。

Method: 使用Unreal Engine在虚拟环境中将雨作为3D粒子进行模拟，生成600,000个1080p帧对的大规模数据集UENR-600K。该方法保证照片级真实感和物理真实的雨滴，捕捉颜色折射、场景遮挡、雨幕等正确细节。基于此高质量数据，采用Wan 2.2视频生成模型，将去雨视为视频到视频生成任务，利用强生成先验。

Result: 建立的基线模型几乎完全弥合了模拟到真实的差距。广泛基准测试表明，使用该数据集训练的模型在真实世界视频中泛化能力显著更好。

Conclusion: UENR-600K数据集通过物理基础的3D雨滴模拟解决了夜间视频去雨的独特挑战，为夜间去雨研究提供了高质量的大规模数据资源，并建立了新的SOTA基线，显著提升了模型在真实场景中的泛化能力。
...
TL;DR: 3D-Fixer：一种新颖的原位补全范式，通过粗到细生成方案解决单视图场景生成中的边界模糊问题，利用几何碎片作为空间锚点保持布局保真度
...
Method: 提出3D-Fixer原位补全范式：1) 使用几何估计方法获得的碎片化几何作为空间锚点；2) 粗到细生成方案解决遮挡下的边界模糊；3) 双分支条件网络和遮挡鲁棒特征对齐策略；4) 构建ARSG-110K大规模场景数据集

Result: 在几何精度方面达到最先进水平，显著优于MIDI和Gen3DSR等基线方法，同时保持了扩散过程的高效性

Conclusion: 3D-Fixer通过原位补全范式有效解决了单视图3D场景生成中的布局保真度和效率问题，为复杂场景的3D重建提供了新思路
...
TL;DR: 提出了BoxComm数据集和评估框架，专注于格斗体育（拳击）解说生成，填补了现有团队体育解说基准的空白，并引入类别条件生成和节奏评估两种新评测方法。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在体育解说生成方面主要关注团队体育（如足球、篮球），而格斗体育完全未被探索。格斗体育具有独特挑战：关键动作在毫秒级内发生，视觉差异细微但语义决定性，且专业解说中战术分析比例远高于团队体育。

Method: 1) 构建BoxComm数据集：包含445场世界拳击锦标赛视频和超过52K条专业解说句子；2) 提出结构化解说分类法：将句子分为实时解说、战术分析和背景信息三类；3) 引入两种评测方法：类别条件生成（根据指定类型生成准确解说）和节奏评估（评估连续视频段中解说的时间节奏和类型分布）；4) 提出EIC-Gen基线方法：结合检测到的击打事件提供结构化动作线索。

Result: 实验表明当前最先进的MLLMs在两种评测上都表现不佳。提出的EIC-Gen基线方法通过结合击打事件检测，在所有评测中取得一致提升，突显了感知短暂细微事件对格斗体育解说的重要性。

Conclusion: BoxComm填补了格斗体育解说生成的空白，提出的分类法和评测方法为体育解说生成提供了更全面的评估框架。结果表明当前模型在格斗体育解说方面仍有不足，而结合结构化事件检测能显著提升性能，为未来研究提供了重要方向。
...
<details>
  <summary>Details</summary>
Motivation: 虚拟现实需要详细且可定制的3D手部模型，但现有方法昂贵、繁琐且自定义性差。虽然零样本文本到3D合成技术有所进展，但在手部生成上表现不佳，存在结构不自然、视角不一致和细节丢失等问题。

Method: 提出HandDreamer方法：1）使用MANO手部模型初始化提供结构先验；2）采用手部骨架引导的扩散过程确保视角和姿态一致性；3）提出新颖的校正手部形状引导损失，确保所有视角收敛到一致的模态而不产生几何畸变。

Result: 广泛评估表明，该方法在3D手部模型生成方面优于现有最先进方法，为3D手部生成开辟了新途径。

Conclusion: HandDreamer是首个从文本提示生成零样本3D手部模型的方法，通过结构先验、骨架引导和形状校正损失，有效解决了视角不一致和结构自然性问题，推动了3D手部生成领域的发展。
...
TL;DR: HSA-DINO：一个参数高效的语义增强框架，通过多尺度提示库和语义感知路由器，在保持预训练模型泛化能力的同时提升开放词汇目标检测的领域适应性。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇目标检测方法在通用场景表现良好，但在领域迁移时性能显著下降。这源于领域特定任务中类别标签稀缺且语义弱，以及现有模型无法捕获超出粗粒度类别标签的辅助语义。

Method: 提出HSA-DINO框架：1）多尺度提示库利用图像特征金字塔捕获分层语义并选择领域特定的局部语义提示，从粗到细粒度逐步丰富文本表示；2）语义感知路由器在推理时动态选择适当的语义增强策略，防止参数更新损害预训练模型的泛化能力。

Result: 在OV-COCO、多个垂直领域数据集和修改的基准设置上评估，HSA-DINO相比先前最先进方法表现优异，在领域适应性和开放词汇泛化之间实现了更好的平衡。

Conclusion: HSA-DINO通过参数高效的语义增强框架，有效解决了开放词汇目标检测在领域迁移时的性能下降问题，实现了领域适应性和泛化能力的良好权衡。
...
TL;DR: Chorus是一种跨请求缓存方法，通过利用不同视频生成请求之间的相似性来加速视频扩散模型的推理，相比仅利用单个请求内部相似性的现有方法，在工业级4步蒸馏模型上实现了高达45%的加速。


<details>
  <summary>Details</summary>
Motivation: 视频扩散Transformer模型虽然能生成高质量视频，但迭代去噪过程导致推理成本高昂。现有缓存方法主要利用单个请求扩散过程中的相似性来跳过冗余去噪步骤，但这种方法在工业级蒸馏模型上效果有限。

Method: Chorus采用三阶段缓存策略：1) 对相似请求的潜在特征进行完全复用；2) 在中间去噪步骤中对特定潜在区域进行跨请求缓存，并结合Token-Guided Attention Amplification技术改善生成视频与条件提示之间的语义对齐；3) 将完全复用的适用性扩展到后期去噪步骤。

Result: 在工业级4步蒸馏模型上实现了高达45%的加速，而先前的单请求内缓存方法在这些模型上效果不佳。

Conclusion: Chorus通过利用跨请求相似性的缓存方法，有效加速了视频扩散模型的推理，特别是在现有方法效果有限的工业级蒸馏模型上取得了显著性能提升。
...
TL;DR: 提出无需群体活动标注的Group Activity Feature学习方法，通过动态感知和群体感知的预训练任务，结合DINO提供的局部和全局特征，实现群体动态感知的特征学习。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用低层次静态局部特征学习群体活动特征，缺乏对动态和群体感知的考虑。需要一种无需群体活动标注的方法，能同时捕捉局部动态和全局群体特征。

Method: 提出两个预训练任务：1）人物流估计（捕捉局部动态），2）群体相关物体位置估计（学习全局场景上下文）。结合DINO提供的局部和全局特征，实现群体动态感知的特征学习。

Result: 在公开数据集上实现了群体活动检索和识别的state-of-the-art性能。消融研究验证了各组件（人物流估计、物体位置估计、DINO特征）的有效性。

Conclusion: 提出的方法能有效学习群体活动特征，无需群体活动标注，通过动态感知和群体感知的预训练任务，在群体活动检索和识别任务上表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在标准基准测试上表现出色，但缺乏在自然对抗场景下的全面独立评估，需要了解其鲁棒性、局限性和实际应用性。

Method: 构建系统评估框架，在多种下游任务（零样本图像分类、语义分割、视觉问答）上测试多种VLM模型（CLIP、鲁棒CLIP、BLIP2、SigLIP2），使用精心策划的对抗数据集（排版攻击、ImageNet-A、自然语言诱导对抗样本）。

Result: 鲁棒CLIP模型可能放大自然对抗脆弱性；CLIP模型在自然语言诱导的对抗样本上性能显著下降；提供了可解释分析以识别失败模式。

Conclusion: 研究揭示了视觉语言模型在自然对抗场景下的局限性，希望发现能启发未来鲁棒公平的多模态模式识别研究。
...
TL;DR: 提出MVis-Fold模型，从二维超分辨率超声图像重建三维微血管网络，解决传统方法无法三维重建的挑战


<details>
  <summary>Details</summary>
Motivation: 超分辨率超声技术虽然突破了传统超声的分辨率限制，能够实现微米级微血管成像，但由于成像原理的限制，从SRUS图像进行三维微血管重建仍然是一个未解决的挑战

Method: 开发了MVis-Fold模型，这是一种创新的三维微血管重建模型，集成了跨尺度网络架构，能够从二维SRUS图像进行高保真度的三维微血管网络推断和重建

Result: 模型能够精确计算传统二维SRUS无法轻易获得的三维空间关键参数，在实体肿瘤的三维微血管重建中验证了模型的准确性和可靠性

Conclusion: 该研究为微血管的三维定量分析奠定了基础，为各种疾病的诊断和监测提供了新的工具和方法
...
TL;DR: VicoEdit是一种无需训练和反转的视觉上下文图像编辑方法，通过直接转换源图像并采用概念对齐的后验采样，在保持编辑一致性的同时超越现有基于训练的方法。
...
Conclusion: VicoEdit通过消除反转需求并采用后验采样，提供了一种高效、一致且灵活的视觉上下文图像编辑解决方案。
...
TL;DR: 提出基于补丁增强和跨视图正则化的统一防御框架，在低中毒率下有效抑制多模态大语言模型后门攻击，同时保持正常生成能力


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在监督微调时容易植入后门，一旦特定触发模式激活就会输出有害响应。防御核心挑战在于低中毒率下抑制攻击成功率的同时保持模型正常生成能力，这两个目标存在内在冲突

Method: 提出统一防御框架：结合补丁级数据增强和跨视图输出差异正则化，利用后门响应对非语义扰动的异常不变性，主动拉开原始视图和扰动视图的输出分布，显著抑制后门触发成功率。同时通过施加输出熵约束避免过度抑制，确保正常指令生成质量

Result: 在三个模型、两个任务和六种攻击上的实验结果表明，该方法能有效降低攻击成功率，同时保持高水平的正常文本生成能力

Conclusion: 该工作使大规模多模态模型能够在现实低频率中毒和隐蔽触发场景中安全、可控地部署
...
TL;DR: 提出Indra表示假设，认为单模态基础模型学习到的表示收敛于反映现实共享关系结构的Indra表示，通过范畴理论形式化，在跨模型跨模态场景中增强鲁棒性和对齐性。


<details>
  <summary>Details</summary>
Motivation: 现有单模态基础模型学习到的表示虽然收敛，但本质上是样本的独立内部抽象，表达能力有限。需要一种能够反映样本间关系结构的表示方法。

Method: 基于Indra's Net哲学隐喻提出Indra表示假设，使用范畴理论中的V-富集Yoneda嵌入形式化定义Indra表示作为样本相对于其他样本的关系剖面，使用角距离实例化该表示。

Result: 在涉及视觉、语言和音频的跨模型和跨模态场景中，Indra表示能持续增强不同架构和模态间的鲁棒性和对齐性，为单模态基础模型提供无需训练的对齐框架。

Conclusion: Indra表示假设为单模态基础模型的表示收敛现象提供了理论解释，提出的Indra表示框架能有效提升跨模型跨模态的鲁棒性和对齐性，具有理论和实践价值。
...
TL;DR: Saliency-R1框架通过新颖的显著性映射技术提升视觉语言模型的解释性和忠实性，无需额外计算开销，并通过GRPO对齐显著区域与关键区域，改善推理质量。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型存在信任问题：过度依赖文本线索而非视觉证据，可能产生无根据或捏造的响应。需要提升模型的解释性和推理忠实性。

Method: 提出Saliency-R1框架：1) 新颖的显著性映射技术，高效突出对生成token关键的图像区域；2) 扩展用于追踪视觉信息在推理过程中的流动；3) 使用显著性映射与人工标注边界框的重叠作为奖励函数；4) 应用Group Relative Policy Optimization (GRPO)对齐显著部分与关键区域。

Result: 实验表明Saliency-R1改善了推理的忠实性、解释性，并提升了整体任务性能。

Conclusion: Saliency-R1框架有效解决了视觉语言模型的信任问题，通过显著性映射和强化学习对齐，使模型在推理时更关注相关视觉区域，提高了模型的可靠性和性能。
...
TL;DR: MedROI是一个与编解码器无关的ROI中心框架，通过裁剪背景区域来提升医学影像压缩效率，无需修改现有编解码器架构。
...
Method: 使用轻量级强度阈值提取组织边界框，丢弃背景体素，仅存储54字节元数据用于空间恢复。裁剪后的ROI可使用任何现有2D/3D编解码器压缩，无需架构修改或重新训练。

Result: 在200个ADNI脑MRI上测试6种编解码器配置，MedROI显著提升了压缩比和编码/解码时间（经多重比较校正的t检验），同时保持ROI内重建质量可比。例如JPEG2000 2D压缩比从20.35提升到27.37，压缩时间从1.701s降至1.380s。

Conclusion: MedROI是一个简单有效的插件式框架，通过ROI中心压缩显著提升医学影像压缩效率，兼容现有编解码器，为医学影像存储和传输提供了实用解决方案。
...
TL;DR: MPTF-Net：基于多视角多尺度金字塔Transformer融合网络的LiDAR地点识别方法，通过NDT-BEV编码和跨视角特征融合，在复杂环境中实现高精度实时识别。


<details>
  <summary>Details</summary>
Motivation: 现有基于BEV的LiDAR地点识别方法采用简单的统计聚合，无法捕捉细粒度几何结构，在复杂或重复环境中性能下降。需要一种能更好建模局部几何复杂性和强度分布的方法。

Method: 提出MPTF-Net，核心是多通道NDT-BEV编码，通过正态分布变换显式建模局部几何复杂性和强度分布。开发定制化金字塔Transformer模块，在多个空间尺度上捕捉Range Image Views和NDT-BEV之间的跨视角交互相关性。

Result: 在nuScenes、KITTI和NCLT数据集上实现最先进性能，特别是在nuScenes Boston分割上Recall@1达到96.31%，推理延迟仅10.02ms，适合实时自主无人系统。

Conclusion: MPTF-Net通过NDT-BEV编码和跨视角多尺度融合，有效解决了传统BEV表示在复杂环境中的局限性，实现了高精度、实时的LiDAR地点识别。
...
TL;DR: StableTTA是一种无需训练的集成方法，通过改进聚合策略提升预测稳定性，在ImageNet-1K上实现10.93-32.82%的top-1准确率提升，轻量架构能以不到5%的参数和约89.1%的计算成本超越ViT。
...
Method: 提出StableTTA，一种无需训练的方法来改进聚合稳定性和效率。该方法专注于解决集成方法中的聚合策略冲突问题。

Result: 在ImageNet-1K上实现10.93-32.82%的top-1准确率提升，33个模型达到超过95%的准确率，多个模型超过96%。轻量架构能以不到5%的参数和约89.1%的计算成本超越ViT 11.75%的top-1准确率。

Conclusion: StableTTA有效解决了集成方法中的聚合稳定性问题，显著提升了预测性能，同时大幅降低了计算成本和参数需求，使得在资源受限设备上实现高精度推理成为可能。
...
TL;DR: 该论文将视觉SLAM中的相对位姿估计重新表述为极线对应图上的关系推理问题，通过图操作估计旋转、平移和本质矩阵，在室内外基准测试中表现出更好的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖随机假设采样和迭代估计，而学习方法缺乏显式几何结构。为了解决对应点噪声问题，需要一种结合几何结构和学习能力的方法。

Method: 将相对位姿估计重新表述为极线对应图上的关系推理问题，其中匹配的关键点为节点，相邻点通过边连接。使用图操作（剪枝、消息传递、池化）估计四元数旋转、平移向量和本质矩阵。采用包含5个分量的损失函数进行优化。

Result: 在室内外基准测试中，相比传统方法和学习引导方法，该方法对密集噪声和大基线变化表现出更好的鲁棒性，验证了全局关系共识的有效性。

Conclusion: 通过将相对位姿估计重新表述为图关系推理问题，该方法结合了几何结构和学习能力，在噪声环境下表现出优越性能，为视觉SLAM提供了新的解决方案。
...
TL;DR: TILA是一个用于医学影像时序分析的新框架，通过时序反转作为监督信号，增强现有模型对影像间方向性变化的敏感性，在胸部X光片进展分类和时序嵌入对齐方面取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉-语言预训练模型大多单独分析放射影像，忽略了临床实践中比较前后影像以评估间隔变化的关键任务。对于胸部X光片，捕捉时序变化至关重要，因为放射科医生不仅要评估发现的静态外观，还要评估它们如何随时间演变。

Method: TILA框架使用时序反转（反转图像对）作为监督信号，在预训练、微调和推理阶段集成反转感知目标，将显式的时序顺序学习与传统的表观建模相结合。还提出了统一的评估协议和MS-CXR-Tretrieval检索评估集。

Result: 在公共数据集和真实医院队列上的实验表明，TILA在应用于多种现有架构时，能持续改进进展分类和时序嵌入对齐性能。

Conclusion: TILA是一个简单而有效的框架，通过时序反转增强现有模型对方向性变化的敏感性，为医学影像的时序分析提供了新的解决方案，并在实际应用中表现出色。
...
TL;DR: TAPE：一个两阶段参数高效微调框架，用于医学图像分割，通过域对齐和任务适配解决基础模型在OCT/OCTA图像分析中的领域偏移和任务不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 现有OCT/OCTA图像分析方法需要大量数据和模型规模，难以在资源有限的临床环境中部署。基于基础模型的迁移学习面临领域偏移和任务不匹配的挑战。

Method: 提出TAPE两阶段适配框架：1）域对齐阶段使用参数高效微调结合掩码图像建模进行医学图像域适配；2）任务适配阶段针对下游分割任务进行微调。应用于通用（MAE）和专用（RETFound）基础模型。

Result: TAPE在视网膜层分割任务中表现出优越的参数效率，并在多种病理条件下实现了最先进的泛化性能。

Conclusion: TAPE框架有效解决了基础模型在医学图像分析中的领域偏移问题，为资源受限的临床环境提供了高效、准确的OCT/OCTA图像分割解决方案。
...
TL;DR: 系统评估文本到图像扩散模型后处理遗忘方法，发现遗忘效果与组合生成能力之间存在权衡：强遗忘方法损害组合完整性，而保持组合结构的方法遗忘效果不足


<details>
  <summary>Details</summary>
Motivation: 现有遗忘研究主要关注遗忘成功率，但对模型整体生成能力的影响了解不足，需要系统评估遗忘方法在组合文本到图像生成中的表现

Method: 聚焦Stable Diffusion 1.4的裸露内容移除，使用T2I-CompBench++和GenEval等基准，评估多种最先进的遗忘方法，分析遗忘效果与组合完整性之间的关系

Result: 发现一致的权衡关系：实现强遗忘的方法在属性绑定、空间推理和计数方面显著退化；而保持组合结构的方法往往无法提供鲁棒的遗忘效果
...
TL;DR: 提出PR-IQA框架，用于评估扩散模型生成的稀疏视角新视图，无需真实标签，通过部分参考图像质量评估提升3D高斯溅射重建质量


<details>
  <summary>Details</summary>
Motivation: 扩散模型可用于稀疏视角新视图合成，但生成的图像存在光度与几何不一致性，直接用于监督会损害3D重建质量

Method: 提出PR-IQA框架：1) 在重叠区域计算几何一致的部分质量图；2) 通过跨注意力机制结合参考视图上下文，将部分图补全为密集全图像质量图

Result: PR-IQA优于现有IQA方法，无需真实监督即可达到全参考级准确度；集成到3DGS管道后，能有效过滤不一致性，产生更优的3D重建和新视图合成结果

Conclusion: PR-IQA框架通过质量感知的监督策略，有效解决了扩散生成视图的不一致性问题，显著提升了3D高斯溅射的重建质量
...
TL;DR: Firebolt-VL是一种高效视觉语言模型，用液态基础模型(LFM)解码器替代Transformer解码器，并引入Token-Grid相关模块实现细粒度视觉定位，在保持线性时间推理的同时提升效率


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型计算成本高，难以部署到资源受限场景；现有方法依赖Transformer交叉注意力，二次复杂度影响效率；小规模视觉语言模型难以精确捕捉细粒度视觉区域，在细粒度推理任务上性能受限

Method: 1) 用液态基础模型(LFM)解码器替代Transformer解码器；2) 提出Token-Grid相关模块，计算文本token和图像patch之间的轻量级相关性；3) 通过状态空间模型和FiLM条件进行调制，选择性强调与文本提示相关的视觉区域

Result: 在多个基准测试中，Firebolt-VL实现了准确、细粒度的理解，同时显著提升了效率，保持线性时间推理

Conclusion: Firebolt-VL通过创新的架构设计解决了MLLMs的效率瓶颈和细粒度视觉定位问题，为资源受限场景下的视觉语言理解提供了有效解决方案
...
TL;DR: 该论文提出了一种基于物理特征检测AI生成图像的新方法，通过分析15种物理特征并筛选出5个核心特征，结合CLIP模型实现跨数据集和生成架构的鲁棒检测。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测器往往过拟合特定生成模型，无法适应快速发展的AIGC技术。需要重新审视区分自然图像和AI生成图像的内在物理特征，解决检测器的适应性危机。

Method: 1) 在20多个由GAN和扩散模型生成的数据集上全面探索15种物理特征；2) 提出特征选择算法识别5个核心物理特征（拉普拉斯方差、Sobel统计量、残差噪声方差等）；3) 将这些特征转换为文本编码值，与语义描述结合指导CLIP的图像-文本表示学习。

Result: 在多个Genimage基准测试中达到最先进性能，在Wukong和SDv1.4等数据集上获得接近完美的准确率（99.8%）。物理特征显著提升了检测的鲁棒性和跨数据集泛化能力。
...
TL;DR: GLA提出了一种针对自动驾驶视觉语言模型的新型后门攻击方法，使用涂鸦式视觉触发器和跨语言文本触发器，在仅10%中毒率下达到90%攻击成功率，且不影响模型在干净任务上的性能。
...
Result: 在DriveVLM上的实验显示，GLA仅需10%的中毒率就能达到90%的攻击成功率，且0%误报率。后门不仅不削弱模型在干净任务上的性能，反而提高了BLEU-1等指标，使传统基于性能下降的检测方法难以识别攻击。
...
TL;DR: InCTRLv2是一个新颖的少样本通用异常检测与分割框架，通过双分支架构从异常-正常判别和纯正常模式学习两个互补视角进行异常感知，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前异常检测方法大多是针对特定领域的专家模型，需要大量训练数据且难以泛化到未见数据集。通用异常检测(GAD)旨在学习单一通用模型来跨域检测异常而无需重新训练。

Method: 在InCTRL基础上构建双分支框架：1)主分支使用判别式异常分数学习(DASL)，利用正常和异常数据学习语义引导的异常-正常空间；2)辅助分支使用单类异常分数学习(OASL)，仅使用正常数据学习语义空间中的广义正常模式。两个分支都由大规模视觉语言模型编码的丰富视觉-文本语义先验指导。

Result: 在十个异常检测数据集上的广泛实验表明，InCTRLv2在各种设置下的异常检测和分割任务中都达到了最先进的性能。

Conclusion: InCTRLv2通过双语义视角（正常-异常判别和正常偏离语义）为异常检测提供了互补的感知能力，显著提升了通用异常检测的性能。
...
TL;DR: 提出新的AI生成视频检测框架，通过原生尺度处理保留高频伪造痕迹，并构建大规模数据集解决现有方法预处理导致信息丢失的问题。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型快速发展，合成媒体高度逼真，引发虚假信息传播的社会担忧。现有检测方法存在关键局限：依赖固定分辨率调整和裁剪等预处理操作，丢弃细微高频伪造痕迹，造成空间扭曲和信息损失；且训练评估数据集过时，无法捕捉现代生成模型的复杂性。

Method: 1) 构建大规模数据集：包含来自15个最先进开源和商业生成器的14万+视频，以及专门评估超逼真合成内容的Magic Videos基准；2) 提出基于Qwen2.5-VL Vision Transformer的检测框架，支持原生可变空间分辨率和时间长度处理，有效保留传统预处理中丢失的高频伪影和时空不一致性。
...
TL;DR: 提出Flow Divergence Sampler (FDS)，一种无需训练的框架，通过计算边际速度场的散度来量化冲突指导的严重程度，并利用该信号引导中间状态到更明确的区域，从而提高生成质量。


<details>
  <summary>Details</summary>
Motivation: 基于流的模型通过建模边际速度场来学习目标分布，但当样本速度在相同中间状态发生冲突时，平均速度可能将样本误导到低密度区域，从而降低生成质量。

Method: 提出Flow Divergence Sampler (FDS)，这是一个无需训练的框架，在每个求解器步骤之前细化中间状态。关键发现是：这种误导的严重程度可以通过边际速度场的散度来量化，该散度在使用优化良好的模型进行推理时易于计算。FDS利用这个信号将状态引导到冲突较少的区域。

Result: FDS作为一个即插即用的框架，与标准求解器和现成的流骨干网络兼容，在各种生成任务（包括文本到图像合成和逆问题）中持续提高生成保真度。

Conclusion: 通过量化边际速度场的散度来检测和缓解样本速度冲突，FDS能够有效提高基于流模型的生成质量，且无需额外训练，具有广泛的适用性。
...
TL;DR: Synthesis4AD：利用大规模高保真合成异常数据提升3D工业异常检测性能的端到端框架


<details>
  <summary>Details</summary>
Motivation: 工业3D异常检测面临异常样本稀缺和长尾分布的根本限制，这严重制约了检测性能。

Method: 1) 开发3D-DefectStudio平台，基于MPAS可控合成引擎生成几何逼真的缺陷；2) 利用多模态大语言模型解读产品设计信息并自动生成异常合成指令；3) 提出基于空间分布归一化和几何忠实数据增强的训练流程，提升点云检测器的鲁棒性。

Result: 在Real3D-AD、MulSen-AD和真实工业零件数据集上实现了最先进的性能。

Conclusion: Synthesis4AD通过合成异常数据有效解决了工业3D异常检测的数据稀缺问题，显著提升了检测性能，相关工具将开源发布。
...
TL;DR: ZeD-MAP：通过集成增量式聚类束调整，将零样本扩散深度模型转换为度量一致、SLAM式映射框架，实现无人机图像实时深度重建


<details>
  <summary>Details</summary>
Motivation: 无人机超高分辨率图像的实时深度重建对灾害响应等时间关键地理空间任务至关重要，但面临宽基线视差、大图像尺寸、低纹理表面、遮挡和计算约束等挑战。现有零样本扩散模型虽能快速预测但缺乏度量精度和时间一致性。

Method: 提出ZeD-MAP框架：将流式无人机帧分组为重叠聚类；周期性束调整产生度量一致的位姿和稀疏3D连接点；将这些点重投影到选定帧中，作为扩散深度估计的度量引导。

Result: 在约50米高度采集的DLR MACS数据上验证，水平面误差约0.87米，垂直方向误差约0.12米，单帧处理时间1.47-4.91秒，达到亚米级精度。

Conclusion: 束调整的度量引导提供了与经典摄影测量方法相当的精度，同时显著加速处理，实现了实时3D地图生成，平衡了扩散模型的快速预测与度量一致性需求。
...
TL;DR: 该论文提出DenZa-Gaussian方法，通过3D高斯溅射技术改进稀疏视角ADF-STEM断层扫描，解决了传统方法在有限视角下的重建质量下降问题。


<details>
  <summary>Details</summary>
Motivation: ADF-STEM断层扫描需要多视角倾斜系列图像进行3D重建，但增加视角会延长电子束暴露时间，损伤剂量敏感材料并引入漂移和对齐问题。稀疏视角采集虽必要，但传统方法在有限视角下会产生伪影和结构保真度降低。

Method: 1) 将局部散射强度建模为可学习标量场denza，解决3DGS与ADF-STEM成像物理不匹配问题；2) 引入系数γ稳定不同倾斜角度的散射，通过散射视角归一化确保denza一致性；3) 加入包含2D傅里叶振幅项的损失函数，抑制稀疏视角重建中的缺失楔形伪影。

Result: 在45视角和15视角倾斜系列实验中，DenZa-Gaussian方法产生高保真重建和2D投影，与原始倾斜图像更吻合，在稀疏视角条件下表现出优越的鲁棒性。

Conclusion: DenZa-Gaussian方法成功解决了ADF-STEM稀疏视角断层扫描的挑战，通过物理感知的3D高斯溅射技术实现了高质量重建，平衡了重建保真度与样品保护的需求。
...
TL;DR: OpenWorldLib是一个标准化的先进世界模型推理框架，提出了世界模型的明确定义，并系统分类其核心能力，支持跨任务模型集成与协作推理。


<details>
  <summary>Details</summary>
Motivation: 世界模型作为人工智能的重要研究方向，目前缺乏清晰统一的定义，阻碍了该领域的发展和应用。需要建立一个标准化的框架来整合不同任务的世界模型，促进高效复用和协作推理。

Method: 基于世界模型的发展演变，提出了明确的世界模型定义：以感知为中心、具备交互和长期记忆能力的模型或框架。系统分类了世界模型的核心能力，并开发了OpenWorldLib框架，将不同任务的模型集成到统一框架中，实现高效复用和协作推理。

Result: 建立了OpenWorldLib开源框架，提供了世界模型的标准化定义和分类体系，实现了跨任务模型的集成与协作推理。代码已开源在GitHub上。

Conclusion: OpenWorldLib为世界模型研究提供了标准化的推理框架和清晰的定义，促进了该领域的统一发展。同时提出了对未来世界模型研究方向的思考和分析。
...
TL;DR: 提出自适应KV缓存量化方法，根据token重要性动态分配比特宽度，在减少内存和延迟的同时保持接近FP16的推理精度。


<details>
  <summary>Details</summary>
Motivation: LLM在移动和边缘设备部署面临KV缓存内存和带宽开销大的挑战，现有固定精度量化方案浪费比特在低重要性token上，导致精度损失。

Method: 提取token频率、质量分数、注意力方差和基于熵的不确定性等轻量级特征，通过紧凑的数据驱动控制器动态选择KV精度（2/4/8-bit或FP16）。

Result: 在多个常识推理基准测试中，相比静态KV量化减少17.75%解码延迟，提升7.60点精度，与FP16推理仅差0.30点。

Conclusion: 自适应KV缓存量化通过可变比特分配策略，有效优化了LLM在资源受限设备上的精度-延迟权衡，实现了高效的边缘部署。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在计数、空间推理等基础视觉概念上表现不佳，但人工识别这些弱点成本高、不可扩展且存在偏见，需要自动化方法发现模型的盲点

Method: 基于强化学习的框架，训练提问者智能体根据候选VLM的响应自适应生成查询，通过逐步增加问题复杂度，聚焦细粒度视觉细节和不同技能组合来引出错误答案

Result: 成功识别出36种新的失败模式，在这些模式下VLMs表现困难，框架在不同模型组合上展现出良好的泛化能力

Conclusion: 提出的RL框架能够自动、系统地发现VLMs的失败模式，为理解模型弱点提供了可扩展的解决方案，有助于改进模型性能
...
<details>
  <summary>Details</summary>
Motivation: 人类绘画是增量式的：规划全局布局、绘制粗略草图、检查并细化细节，每个步骤都基于演变的视觉状态。然而，在文本-图像交错数据集上训练的统一多模态模型能否想象中间状态链？

Method: 提出过程驱动的图像生成范式，将合成分解为思想和动作的交错推理轨迹。每个迭代包含四个阶段：文本规划（决定视觉状态如何演变）、视觉草图（生成中间图像）、文本反思（识别并修正违反提示的元素）、视觉细化（基于文本反馈改进图像）。通过密集的逐步监督，对视觉中间状态强制执行空间和语义一致性，对文本中间状态保持先验视觉知识。

Result: 在多个文本到图像生成基准测试中进行了实验验证。

Conclusion: 该方法使图像生成过程变得显式、可解释且可直接监督，模拟了人类绘画的增量式创作过程。
...
TL;DR: 通过数据工程和训练策略优化，而非架构创新，显著提升文档解析性能，在保持1.2B参数架构不变的情况下超越更大模型


<details>
  <summary>Details</summary>
Motivation: 当前文档解析方法主要竞争模型架构创新，而训练数据的系统工程研究不足。不同架构和参数规模的SOTA模型在相同困难样本上表现出高度一致的失败模式，表明性能瓶颈源于训练数据的共同缺陷而非架构本身

Method: 提出MINERUPRO方法，核心是围绕覆盖率、信息量和标注准确性共同设计的数据引擎：1)多样性和难度感知采样将训练数据从不足1000万扩展到6550万样本；2)跨模型一致性验证利用异构模型的输出一致性评估样本难度并生成可靠标注；3)Judge-and-Refine管道通过渲染-验证迭代校正改进困难样本标注质量。采用三阶段渐进训练策略：大规模预训练、困难样本微调和GRPO对齐

Result: 在修复了OmniDocBench v1.5中元素匹配偏差并引入Hard子集后，建立了更具区分度的OmniDocBench v1.6评估协议。MINERUPRO在v1.6上达到95.69分，比相同架构基线提升2.71分，超越所有现有方法，包括参数规模超过200倍的模型

Conclusion: 通过系统化的数据工程和训练策略优化，无需架构修改即可显著提升文档解析性能，表明训练数据质量是当前性能瓶颈的关键因素，为未来研究提供了新的方向
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中图像退化（模糊、噪声、压缩、光照差）严重影响多模态理解。现有统一多模态模型虽然结合了理解与生成能力，但未能利用自身生成能力处理退化输入，主要原因是训练机制从未要求模型在推理时调用生成，且标准的解码-重新编码路径不支持有效联合优化。

Method: CLEAR框架包含三个渐进步骤：1) 在退化感知数据集上进行监督微调，建立"先生成后回答"的推理模式；2) 潜在表示桥接，用直接可优化的连接替代解码-重新编码的迂回路径；3) 交错GRPO强化学习方法，在答案正确性奖励下联合优化文本推理和视觉生成。构建了MMD-Bench基准，涵盖三个退化严重级别和六个标准多模态基准。

Result: 实验表明CLEAR显著提高了模型在退化输入上的鲁棒性，同时保持了干净图像性能。分析进一步显示，移除像素级重建监督会导致具有更高感知质量的中间视觉状态，表明任务驱动优化与视觉质量自然对齐。

Conclusion: CLEAR成功连接了多模态模型的生成与推理能力，通过任务驱动的优化提升了退化图像的理解性能，同时发现视觉质量与任务优化目标自然一致，为处理现实世界图像退化问题提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 需要从单张肖像图像生成高质量、动态、可控的4D高斯化身，现有方法在精确构建和动画能力方面存在局限

Method: 使用解码器Transformer自回归生成点云用于3D高斯泼溅，动态调整点密度和总数，联合预测每点绑定信息，最后通过高斯解码器转换为可渲染的高斯属性

Result: 生成高质量、逼真、可控的化身，实验验证了方法的有效性，自回归公式为化身生成提供了新范式

Conclusion: AvatarPointillist通过自回归点云生成实现了高质量的动态4D高斯化身，为化身生成领域提供了新方法，代码将开源以促进未来研究
...
TL;DR: MMF-BEV是一个用于自动驾驶的雷达-相机BEV融合框架，通过可变形注意力机制实现跨模态特征对齐，在VoD数据集上优于单模态基线，达到竞争性融合性能。
...
Result: 在VoD数据集上，MMF-BEV一致优于单模态基线（相机或雷达单独），并在所有物体类别上达到与先前融合方法竞争的结果，包括完整标注区域和近距感兴趣区域。

Conclusion: MMF-BEV通过可变形注意力机制有效融合雷达和相机模态，传感器贡献分析提供了模态互补的可解释证据，为自动驾驶3D目标检测提供了有效的多模态融合解决方案。
...
TL;DR: E-VLA：一种事件增强的视觉-语言-动作模型，通过直接利用事件流的运动和结构线索，在极端光照、运动模糊等恶劣条件下显著提升机器人操作的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统基于帧的视觉-语言-动作模型在极端低光、运动模糊、黑色裁剪等感知退化条件下表现脆弱，需要更鲁棒的感知方案来应对现实世界中的恶劣视觉环境。

Method: 提出事件增强的VLA框架，直接利用事件流的运动和结构线索而非重建图像；构建开源遥操作平台和真实世界RGB-事件-动作数据集；设计轻量级、预训练兼容的事件集成策略，研究事件窗口化和融合方法。

Result: 在20lux低光条件下，Pick-Place任务成功率从0%（仅图像）提升到60%（叠加融合）和90%（事件适配器）；在1000ms曝光严重运动模糊下，Pick-Place从0%提升到20-25%，Sorting从5%提升到32.5%。

Conclusion: 事件驱动感知能有效集成到VLA模型中，为超越传统帧基成像的鲁棒具身智能指明方向，即使简单的参数免费融合也能显著提升恶劣条件下的操作鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在高分辨率细节处理中容易产生幻觉或推理错误，需要一种方法让模型专注于关键结构信息而非干扰性纹理细节。
...
Conclusion: 策略性降低图像保真度并引导模型关注结构信息，是提升视觉语言模型在复杂视觉任务中推理能力的有效方法。
...
Method: 1) 提出粗到细指令条件交互生成框架，与一致性模型的迭代去噪过程对齐；2) 采用动态感知策略，利用前一步精炼的轨迹更新场景上下文；3) 引入碰撞感知引导，在不依赖精细场景几何的情况下减少物理伪影；4) 设计混合训练策略，通过将体素化场景占用注入HOI数据集合成伪HOSI样本，并与高保真HSI数据联合训练。

Result: 在HOSI和HOI生成任务上均达到最先进性能，对未见场景具有强泛化能力，并能实现实时生成。

Conclusion: 该方法通过动态感知策略、碰撞感知引导和混合训练策略，有效解决了HOSI生成中的数据稀缺和物理合理性问题，实现了高质量、实时的人-物-场景交互生成。
...
TL;DR: 本文首次系统研究了自动驾驶中视觉语言模型的灾难性遗忘问题，提出了首个专门量化该问题的基准数据集，并开发了Drive Expert Adapter框架，通过提示空间适配而非权重微调来避免知识退化。


<details>
  <summary>Details</summary>
Motivation: 将视觉语言模型集成到自动驾驶中虽然能解决长尾场景，但面临灾难性遗忘的关键挑战：为适应驾驶数据而进行的微调会侵蚀模型预训练的世界知识，这与使用VLMs的核心目的相悖。

Method: 提出了Drive Expert Adapter (DEA)框架，将适配从权重空间转移到提示空间，根据场景特定线索动态路由推理到不同知识专家，避免修改基础模型参数。

Result: 实验表明，DEA在驾驶任务上达到最先进性能，同时有效缓解灾难性遗忘，保持了VLMs的泛化能力。发布了包含180K场景的大规模数据集FidelityDrivingBench。

Conclusion: 通过提示空间适配而非权重微调，DEA成功解决了自动驾驶中VLMs的灾难性遗忘问题，在提升驾驶任务性能的同时保留了预训练世界知识，为VLMs在自动驾驶中的实际应用提供了可行方案。
...
TL;DR: 提出Floorplan Markup Language (FML)通用表示法和基于transformer的FMLM模型，用于多条件住宅平面图生成，在RPLAN数据集上超越现有任务特定方法。


<details>
  <summary>Details</summary>
Motivation: 现有住宅平面图生成方法存在局限性：早期基于约束满足或组合优化的方法缺乏多样性和灵活性；近期生成模型虽然效果不错，但难以泛化到异构条件任务（如从场地边界、房间邻接图或部分布局生成），因为表示方法不够优化。

Method: 提出Floorplan Markup Language (FML)通用表示法，将平面图信息编码为结构化语法，将整个生成问题转化为下一个token预测任务。基于FML开发transformer生成模型FMLM，能够处理多样条件生成高保真功能性平面图。

Result: 在RPLAN数据集上的综合实验表明，FMLM虽然是单一模型，但超越了先前任务特定的最先进方法。

Conclusion: FML表示法和FMLM模型解决了住宅平面图生成中的泛化问题，通过统一表示法实现了跨异构条件任务的高质量生成，使空间设计更加高效和可访问。
...
TL;DR: 提出基于patch-level的幻觉检测框架，通过分析token-level的细粒度交互来检测LVLM中的幻觉，相比现有全局方法更有效


<details>
  <summary>Details</summary>
Motivation: 现有的大视觉语言模型幻觉检测方法主要依赖全局图像度量，但幻觉token可能在许多局部区域表现出微弱但广泛分布的关联性，这些关联聚合起来会形成欺骗性的高相关性，从而逃避当前全局检测器

Method: 基于关键观察（忠实对象token必须强烈锚定在特定图像区域），引入patch-level幻觉检测框架，分析模型层间的细粒度token-level交互，发现幻觉token的两个特征：扩散的非局部化注意力模式，以及缺乏与任何视觉区域的有意义语义对齐

Result: 开发了轻量级可解释的检测方法，利用patch-level统计特征结合隐藏层表示，在token-level幻觉检测中达到90%的准确率

Conclusion: 细粒度结构分析在检测幻觉方面具有优越性，patch-level方法比全局方法更有效
...
<details>
  <summary>Details</summary>
Motivation: 现有方法生成高度冗余的网格对齐高斯分布，在未观测区域存在空洞或模糊问题。需要一种能处理非网格对齐3D数据、合成合理未观测内容的方法。

Method: 1. 通过高斯参数流匹配进行生成式重建；2. 分层分块方案将空间相关高斯分组为联合transformer token，减少序列长度；3. 训练时使用时间步加权渲染损失；4. 推理时采用光度梯度引导和分类器自由引导提升保真度。

Result: 在Objaverse和Google Scanned Objects数据集上，相比像素和体素对齐方法，使用更少高斯分布获得一致改进，在输入视图未覆盖物体部分时优势更明显。

Conclusion: Free-Range Gaussians通过生成式重建和分层分块方案，实现了高质量多视角重建，能有效处理未观测区域，减少高斯冗余，提升重建质量。
...
TL;DR: DIRECT是一个分层多智能体框架，将视频混剪创建视为多模态一致性满足问题，通过编剧、导演、编辑三级级联实现专业级流畅度


<details>
  <summary>Details</summary>
Motivation: 现有自动化编辑框架往往忽视跨层级多模态协调，导致视觉过渡突兀和音乐不协调，无法达到专业级流畅度

Method: 提出DIRECT框架，模拟专业制作流程，采用分层多智能体架构：编剧负责源感知全局结构锚定，导演负责自适应编辑意图和指导，编辑负责意图引导的镜头序列编辑和细粒度优化

Result: DIRECT在客观指标和人类主观评价上均显著优于现有最先进基线方法，并引入了Mashup-Bench基准测试

Conclusion: 将视频混剪制定为多模态一致性满足问题，并通过分层多智能体框架有效解决了跨层级多模态协调挑战，实现了专业级流畅的视频混剪
...
TL;DR: HorizonWeaver是一个用于自动驾驶场景编辑的框架，通过大规模数据集、语言引导掩码和联合训练解决密集驾驶场景的多粒度编辑挑战。


<details>
  <summary>Details</summary>
Motivation: 现有基于指令的图像编辑方法主要针对物体中心或艺术数据，难以处理密集、安全关键的驾驶场景布局，需要可扩展的现实可控驾驶场景生成方法。

Method: 1) 数据：构建大规模配对真实/合成数据集(Boreas, nuScenes, Argoverse2)；2) 模型：语言引导掩码实现细粒度编辑；3) 训练：联合损失函数确保场景一致性和指令保真度。

Result: 收集255K图像覆盖13个编辑类别，在L1、CLIP、DINO指标上优于现有方法，获得+46.4%用户偏好，BEV分割IoU提升+33%。

Conclusion: HorizonWeaver为复杂驾驶场景提供了可扩展的、逼真的指令驱动编辑框架，解决了多粒度编辑、丰富语义保持和领域迁移等核心挑战。
...
TL;DR: FileGram是一个基于文件系统行为轨迹的AI代理记忆与个性化框架，包含数据引擎、评估基准和操作系统三个核心组件，旨在解决当前AI代理在文件系统操作中因数据限制导致的个性化不足问题。


<details>
  <summary>Details</summary>
Motivation: 当前协作式AI代理在文件系统中的个性化面临严重数据限制：严格的隐私壁垒和多模态真实世界轨迹的收集困难阻碍了可扩展的训练和评估。现有方法过于关注交互而忽视了文件系统操作中的密集行为轨迹。

Method: 提出FileGram框架，包含三个核心组件：1) FileGramEngine - 可扩展的人物驱动数据引擎，模拟真实工作流并生成细粒度多模态动作序列；2) FileGramBench - 基于文件系统行为轨迹的诊断基准，评估记忆系统的配置文件重建、轨迹解耦、人物漂移检测和多模态基础；3) FileGramOS - 自底向上的记忆架构，直接从原子动作和内容增量构建用户配置文件，将这些轨迹编码到程序性、语义性和情景性通道中。

Result: 广泛实验表明：FileGramBench对最先进的记忆系统仍然具有挑战性；FileGramEngine和FileGramOS是有效的。通过开源该框架，支持未来关于个性化记忆中心文件系统代理的研究。

Conclusion: FileGram通过将代理记忆和个性化建立在文件系统行为轨迹上，解决了当前AI代理在文件系统操作中的个性化限制问题，为未来研究提供了全面的框架和基准。
...
TL;DR: ClickAIXR是一个在XR设备上运行的多模态视觉语言交互框架，通过控制器点击选择真实世界物体，使用本地视觉语言模型处理图像并回答自然语言问题，实现隐私保护和低延迟交互。


<details>
  <summary>Details</summary>
Motivation: 现有XR交互系统依赖云端AI（如ChatGPT）或基于注视的选择（如GazePointAR），存在隐私泄露、延迟高和交互模糊性问题。ClickAIXR旨在通过本地化处理和精确对象选择来解决这些问题。

Method: 集成设备端视觉语言模型（VLM）与控制器对象选择范式，用户点击XR中的真实物体后，本地VLM处理图像并回答自然语言问题（支持文本和语音）。系统基于Magic Leap SDK（C API）实现，使用ONNX进行本地VLM推理。

Result: 用户研究比较ClickAIXR与Gemini 2.5 Flash和ChatGPT 5，评估可用性、信任度和用户满意度。结果显示延迟适中，用户体验可接受，证明点击式对象选择与设备端AI结合在XR交互中的潜力。

Conclusion: ClickAIXR展示了基于点击的对象选择与设备端AI结合在推进可信、隐私保护的XR交互方面的潜力，为未来XR系统设计提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 当前图像空间编辑模型在细粒度空间操作方面能力不足，缺乏专门的评估基准，需要系统性的评估框架和训练数据

Method: 1) 创建SpatialEdit-Bench基准，通过视角重建和构图分析联合评估感知合理性和几何保真度；2) 使用可控Blender流程生成SpatialEdit-500k合成数据集；3) 基于此数据开发SpatialEdit-16B基线模型

Result: 该方法在通用编辑任务上具有竞争力，在空间操作任务上显著优于现有方法

Conclusion: 提出的基准、数据集和基线模型为图像空间编辑提供了全面的评估框架和解决方案，所有资源将公开共享
...
TL;DR: DeltaWorld：基于VFM特征差异的轻量级生成式世界模型，通过DeltaTok将视频压缩为一维时序序列，实现高效多假设预测


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型面临挑战：判别式模型只能预测确定性未来（隐含平均多种可能性），而生成式模型计算成本过高。虽然已有研究使用视觉基础模型特征空间预测未来，但大多仍是判别式方法

Method: 提出DeltaTok将连续帧间的VFM特征差异编码为单个连续"delta"令牌，将视频从三维时空表示压缩为一维时序序列（如512x512帧可减少1024倍令牌）。DeltaWorld基于这些令牌构建生成式世界模型，支持并行多假设训练（生成多个未来，仅监督最佳预测）

Result: 在密集预测任务中，DeltaWorld预测的未来更接近真实世界结果，同时参数比现有生成式世界模型少35倍以上，FLOPs减少2000倍。推理时单次前向传播即可生成多样化预测

Conclusion: DeltaWorld通过特征差异编码和紧凑表示，实现了高效且多样化的未来预测，为视频世界建模提供了轻量级生成式解决方案
...
TL;DR: Vero是一个完全开放的视觉语言模型家族，通过构建600K样本的多样化数据集和任务路由奖励机制，在多个视觉推理任务上达到或超越现有开放权重模型，无需专有思考数据。


<details>
  <summary>Details</summary>
Motivation: 当前最强的视觉语言模型虽然展现出广泛的视觉推理能力，但其训练方法被专有的强化学习流程和非公开数据所锁定。研究团队希望开发完全开放的模型，探索构建跨领域视觉推理器的有效方法。

Method: 构建了Vero-600K数据集，包含来自59个数据集的60万样本，涵盖六大任务类别。设计了任务路由奖励机制来处理异构答案格式。从Qwen3-VL-8B-Instruct等基础模型出发，通过强化学习进行训练。

Result: Vero在30个挑战性基准测试套件VeroEval上取得最先进性能，平均提升3.7-5.5个百分点。在相同基础模型下，Vero-600K在所有任务类别上都优于现有RL数据集。无需额外专有思考数据，在30个基准中的23个上超越了Qwen3-VL-8B-Thinking。

Conclusion: 不同任务类别会引发性质不同的推理模式，这些模式在孤立情况下迁移效果差，表明广泛的数据覆盖是强化学习扩展的主要驱动力。所有数据、代码和模型都已开源。
...
TL;DR: 该论文提出了一种无需微调或控制模块的方法，通过直接学习文本编码器输出的提示嵌入来解锁预训练扩散模型固有的修复能力，并采用扩散桥公式解决训练不稳定性问题。
...
Method: 提出直接学习文本编码器输出的提示嵌入来解锁模型的修复能力；采用扩散桥公式对齐训练和推理动态，确保从噪声退化状态到干净图像的去噪路径一致性；在预训练的WAN视频模型和FLUX图像模型上应用轻量级学习提示。

Result: 该方法在多种退化类型上实现了竞争性性能和泛化能力，避免了模型微调和特定修复控制模块的使用，将预训练扩散模型转换为高性能修复模型。

Conclusion: 预训练扩散模型具有固有的修复行为，可以通过学习提示嵌入和扩散桥公式来解锁，这种方法提供了一种轻量级且有效的全场景修复解决方案。
...
TL;DR: SimpleProc：使用NURBS、位移和纹理模式的全程序化MVS训练数据生成器，仅需8000张图像即可超越同等规模人工数据，352k图像时性能媲美692k人工数据


<details>
  <summary>Details</summary>
Motivation: 多视图立体视觉（MVS）需要大量高质量训练数据，但人工收集和标注成本高昂。现有方法依赖游戏引擎或真实世界数据，存在规模限制和质量不一致问题。需要探索程序化生成规则的设计空间，以低成本生成有效训练数据。

Method: 提出SimpleProc：基于NURBS（非均匀有理B样条）的完全程序化生成器，使用少量规则结合基本位移和纹理模式生成训练图像。通过程序化方式创建多样化的3D场景和纹理，无需人工干预即可生成大规模MVS训练数据。

Result: 1. 仅8000张程序化图像即超越同等规模游戏和真实世界人工数据；2. 扩展到352k图像时，性能与692k人工数据相当甚至在某些基准测试中超越；3. 证明了程序化生成在数据效率和规模扩展方面的优势。

Conclusion: 程序化规则设计可以有效生成MVS训练数据，SimpleProc展示了程序化方法在数据效率和可扩展性方面的显著优势，为计算机视觉任务提供了一种低成本、高质量的数据生成解决方案。
...
TL;DR: 本文提出一个多智能体推理框架，通过让大模型重用小模型的关键推理标记来提升视觉语言模型的效率，在保持大模型短响应优势的同时获得更好的性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型通常使用大型语言模型作为解码器，通过自回归生成响应标记，输出标记数量成为端到端延迟的瓶颈。不同模型需要不同数量的输出标记才能达到可比性能，而大模型虽然需要更多计算资源，但可能用更少的输出标记达到相同性能。

Method: 提出多智能体推理框架：保持大模型的短响应优势，但在必要时从小模型转移关键推理标记。通过重用小模型的推理标记，帮助大模型接近自身推理的性能。

Result: 实验表明，大模型用更少的输出标记可以比小模型更高效；在多样化真实世界基准测试中，大模型用显著更少的输出标记能达到更好或相当的性能；多智能体框架通过重用小模型的推理标记，能接近大模型自身推理的性能。

Conclusion: 通过重用小模型的关键推理标记，多智能体推理框架能够有效提升视觉语言模型的效率，在保持大模型短响应优势的同时获得更好的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 局部特征匹配作为3D视觉系统的基础组件，其发展滞后于现代数据驱动方法。现有方法仅在少数中型数据集上训练，而现代重建模型已从大规模数据集中受益。同时，当前基准测试主要基于成功的3D重建稀疏视图，导致评估局限于相对简单的图像对。

Method: 提出LoMa方法，结合大规模多样化数据混合、现代训练方法、扩展模型容量和计算资源。同时收集1000个极具挑战性的互联网图像对构建HardMatch数据集，并通过人工标注获取真实对应关系。
...
Conclusion: 从数据驱动角度重新审视局部特征匹配，通过规模化方法（数据、模型、计算）能够显著提升性能。新构建的HardMatch数据集解决了现有基准测试饱和问题，为未来研究提供更具挑战性的评估平台。
...
TL;DR: PointTPA：一种用于场景级点云理解的测试时参数自适应框架，通过生成输入感知的网络参数来动态适应不同场景，在保持低参数开销的同时显著提升性能。
...
Result: 在PTv3结构上集成PointTPA，仅增加不到主干网络参数2%的轻量模块，在ScanNet验证集上达到78.4% mIoU，超越现有参数高效微调方法，在多个基准测试中表现优异。

Conclusion: PointTPA通过测试时动态网络参数自适应机制，有效提升了3D场景理解能力，证明了输入感知参数生成在点云理解任务中的有效性。
...
Method: 1. 构建大规模三元组监督数据：生成身份保持的替代服装人物图像、捕获完整上下服装三元组、组装多样化真实世界三元组；2. 引入双模块架构的视频扩散变换器，稳定训练并保持预训练生成质量。

Result: Vanast能够生成高保真、身份一致的动画视频，支持零样本服装插值，在服装准确性、姿态遵循和身份保持方面表现优异，适用于多种服装类型。

Conclusion: 通过统一框架、大规模三元组监督和双模块架构，Vanast成功解决了服装转移动画中的关键挑战，实现了高质量、连贯的合成效果。
