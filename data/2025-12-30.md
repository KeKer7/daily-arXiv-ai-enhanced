...
TL;DR: 该研究系统分析了视频扩散模型中时间步长对运动编码的影响，发现早期时间步主要控制运动，后期时间步主要控制外观，并基于此简化了运动定制方法。


<details>
  <summary>Details</summary>
Motivation: 尽管实践中普遍认为视频扩散模型的早期时间步主要控制运动，后期时间步主要控制外观，但这种经验法则缺乏系统性的定量验证。研究者希望深入理解运动在时间步长中的编码机制。

Method: 通过在不同时间步范围注入新条件，量化分析外观编辑与运动保持之间的权衡关系，以此作为运动编码的代理指标。在大规模定量研究中，将运动与外观在去噪轨迹上的竞争关系进行映射。

Result: 在不同架构的视频扩散模型中一致识别出早期运动主导阶段和后期外观主导阶段，确定了时间步空间中的运动-外观边界。基于此发现，将训练和推理限制在运动主导阶段，简化了现有的运动定制方法。

Conclusion: 该研究将经验性启发式方法转化为时空解耦原则，提出的时间步约束方法可直接集成到现有的运动迁移和编辑方法中，为视频生成提供了更深入的理论理解和实用工具。
...
Method: 采用3D CNN提取视频帧的时空特征，结合LSTM层建模手语手势的序列依赖性，使用WLASL数据集（2000个常用词）、ASL-LEX词汇数据库（约2700个手势）和100个专家标注的手语数据集进行训练。

Result: 系统在不同手语类别上取得F1分数0.71-0.99的范围，成功部署在AWS基础设施上，并支持OAK-D相机的边缘实时推理。

Conclusion: 混合深度学习架构有效实现了实时美式手语识别，为实际无障碍应用提供了可行的技术方案，讨论了架构设计、训练方法和部署考虑。
...
TL;DR: 该论文提出了一种结合AI与局部线性嵌入(LLE)的创新方法，用于处理高维医疗数据，旨在提高医疗账单系统和转录服务的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 人工智能在医疗领域的快速发展为改进医疗账单和转录等流程创造了机会。当前医疗数据处理存在高维度、人工错误多、效率低等问题，需要更智能的解决方案来提升准确性和操作效率。

Method: 提出AI增强的局部线性嵌入(LLE)模型，专门针对高维医疗数据进行优化。该方法通过自动化流程减少人工干预，提供完整的数学模型，并在真实医疗场景中进行实验验证。

Result: 实验结果显示，该模型在数据处理准确性和操作效率方面都有显著提升，能够支持更快、更准确的医疗文档记录和财务交易处理。

Conclusion: AI增强的LLE模型在医疗数据分析中展现出巨大潜力，为未来更广泛的医疗应用研究奠定了基础，特别是在医疗账单和转录服务的自动化方面。
...
<details>
  <summary>Details</summary>
Motivation: 端到端视觉语言模型（VLMs）在回答视觉问题时经常利用虚假相关性而非因果视觉证据，特别是在微调后更容易出现捷径学习问题。需要一种方法来提高视觉推理的鲁棒性和可靠性。

Method: VISTA框架采用模块化设计：1）冻结的VLM传感器仅进行简短、客观的感知查询；2）纯文本LLM推理器分解问题、规划查询、聚合视觉事实；3）通过强化学习（GRPO）在仅641个多步问题上训练无偏视觉推理。

Result: 在SpuriVerse数据集上显著提升鲁棒性（Qwen-2.5-VL-7B提升16.29%，Llama-3.2-Vision-11B提升6.77%），在MMVP和SeedBench子集上保持竞争力。能够跨未见VLM传感器迁移，并能识别和恢复感知失败。

Conclusion: VISTA通过感知-推理解耦和强化学习训练，有效减少了视觉语言模型对虚假相关性的依赖，提高了推理的可靠性和可解释性，为构建更鲁棒的视觉推理系统提供了新方向。
...
TL;DR: SAMM2D：一种用于颅内动脉瘤检测的双编码器框架，在RSNA数据集上AUC达到0.686，比临床基线提升32%。研究发现，在强预训练骨干网络下，数据增强反而会降低性能，颠覆了"更多增强总是更好"的传统认知。


<details>
  <summary>Details</summary>
Motivation: 颅内动脉瘤检测面临三大挑战：动脉瘤形态细微、类别严重不平衡、标注数据稀缺。传统方法依赖数据增强，但在低数据医疗场景中，这种假设可能不成立。

Method: 提出SAMM2D双编码器框架，使用ImageNet预训练骨干网络。通过六种增强策略的消融实验，发现无增强的基线模型优于所有增强变体。通过决策阈值校准实现高灵敏度。

Result: AUC达到0.686（提升32%），无增强模型比增强变体高1.75-2.23个百分点（p<0.01）。95%灵敏度超过平均放射科医生水平，每1000名患者筛查可节省1390万美元。85%真阳性关注相关血管区域（IoU 62%）。

Conclusion: ImageNet预训练特征已捕获鲁棒不变性，额外增强既冗余又破坏特征流形。未来医疗影像工作流可能从强预训练中获益更多，而非复杂增强管道。
...
Method: 提出HookMIL框架，使用紧凑、可学习的hook tokens进行结构化上下文聚合。支持三种初始化方式：关键补丁视觉特征、视觉-语言病理模型的文本嵌入、空间转录组-视觉模型的空间特征。通过双向注意力与实例交互（线性复杂度），引入Hook多样性损失促进专业化，以及hook-to-hook通信机制优化上下文交互。

Result: 在四个公共病理数据集上的广泛实验表明，HookMIL实现了最先进的性能，同时提高了计算效率和可解释性。

Conclusion: HookMIL通过创新的hook tokens机制，有效解决了传统MIL和Transformer方法在计算病理学中的局限性，为弱监督WSI分析提供了高效、可解释的解决方案。
...
TL;DR: Tiny-YOLOSAM：结合YOLO检测器和TinySAM的混合系统，通过YOLO生成框提示和稀疏点提示替代密集的"segment-everything"模式，在保持分割质量的同时大幅提升速度（4.7倍加速）。


<details>
  <summary>Details</summary>
Motivation: SAM和TinySAM虽然能实现高质量分割，但在实时性要求高的场景中计算成本过高。TinySAM的"segment-everything"模式需要数百个提示且速度仍然较慢，需要更高效的解决方案。

Method: 提出Tiny-YOLOSAM混合管道：1）使用YOLOv12检测器为显著前景对象生成框提示；2）在YOLO引导掩码未覆盖的区域补充稀疏点提示；3）结合TinySAM进行分割，避免密集的"segment-everything"提示。

Result: 在COCO val2017上，类不可知覆盖度大幅提升（AR从16.4%到77.1%，mIoU从19.2%到67.8%），端到端运行时间从49.20秒/图像减少到10.39秒/图像（4.7倍加速），在Apple M1 Pro CPU上实现。

Conclusion: 检测器引导提示结合目标稀疏采样是实际全场景分割中替代密集"segment-everything"提示的有效方法，在保持分割质量的同时显著提升效率。
...
TL;DR: 提出一种新型多模态可解释性模型，结合VLM和少样本学习，通过分析视网膜象限中的病变分布来模拟眼科医生的推理过程，为糖尿病视网膜病变诊断提供定量检测和自然语言解释。


<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是全球视力丧失的主要原因，需要早期检测。医生缺乏时间进行手动病变标注，现有模型仅突出病变位置而无法解释分类推理，且依赖单一成像模态，效果有限。需要一种能够识别个体DR病变并提供自然语言解释的定量检测系统。

Method: 采用多模态可解释性模型，结合视觉语言模型（VLM）和少样本学习，模拟眼科医生的推理过程，分析视网膜象限中的病变分布。模型生成配对的Grad-CAM热图，展示OCT和眼底图像中单个神经元的权重，可视化突出对DR严重程度分类有贡献的区域。

Result: 使用包含3,000张眼底图像和1,000张OCT图像的数据集，该方法解决了当前DR诊断中的关键限制，为改善患者预后提供了实用且全面的工具。

Conclusion: 该创新方法通过多模态可解释性模型，不仅提供病变定位，还能解释分类推理，克服了当前DR诊断模型的局限性，在筛查、治疗和研究环境中具有广泛应用潜力。
...
TL;DR: TCFormer是一个仅有500万参数的轻量级弱监督人群计数框架，通过Transformer特征提取器、可学习密度加权平均模块和密度级别分类损失，在仅使用图像级全局计数标注的情况下实现了竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 传统人群计数方法依赖劳动密集的点级标注和计算密集的主干网络，限制了其在资源受限环境中的可扩展性和部署。需要开发轻量级、弱监督的解决方案。

Method: 1) 采用高效视觉Transformer作为特征提取器；2) 设计可学习密度加权平均模块，根据预测密度动态重加权局部token；3) 引入密度级别分类损失，将人群密度离散化为不同等级进行正则化训练。

Result: 在ShanghaiTech A/B、UCF-QNRF和NWPU四个基准数据集上的实验表明，TCFormer在参数效率和计数准确性之间取得了优越的平衡，适合边缘设备部署。

Conclusion: TCFormer通过弱监督训练范式，仅使用图像级全局计数标注，通过联合优化计数和密度级别损失，实现了高精度的人群计数，为边缘设备提供了可行的解决方案。
...
TL;DR: 提出基于自定义CNN的深度学习模型，用于自动分类疟疾感染血细胞图像，准确率达96%，并利用可解释AI技术增强模型透明度。


<details>
  <summary>Details</summary>
Motivation: 传统疟疾诊断方法（如显微镜血涂片分析）灵敏度低、依赖专家判断且资源需求高，在偏远地区难以实施。需要开发快速、准确且资源需求低的自动化诊断方案。
...
Result: 自定义CNN模型达到96%的准确率，两个类别的精确率和召回率均超过0.95。模型在准确性和效率方面表现优异，特别适合资源有限的环境。

Conclusion: 深度学习能够为疟疾诊断提供快速、准确且可解释的解决方案，尤其在资源有限的地区具有重要应用价值。可解释AI技术增强了模型的透明度和可信度。
...
TL;DR: Signal-SGN++：一种拓扑感知的脉冲图框架，通过结合结构自适应与时空频脉冲动力学，在保持高能效的同时提升骨架动作识别性能。


<details>
  <summary>Details</summary>
Motivation: 传统图卷积网络（GCNs）在骨架动作识别中表现优异但能耗高，而脉冲神经网络（SNNs）能效高但难以捕捉人体运动的时空频耦合依赖和拓扑关系。需要一种能兼顾能效和性能的方法。

Method: 提出Signal-SGN++框架，包含：1）1D脉冲图卷积（1D-SGC）和频率脉冲卷积（FSC）主干网络；2）拓扑转移自注意力（TSSA）机制自适应学习骨架拓扑；3）多尺度小波变换融合（MWTF）分支和拓扑感知时频融合（TATF）单元。

Result: 在大规模基准测试中，Signal-SGN++实现了优越的精度-效率权衡，超越了现有SNN方法，并在显著降低能耗的情况下达到了与最先进GCNs竞争的结果。

Conclusion: Signal-SGN++成功地将结构自适应与时空频脉冲动力学相结合，为高效准确的骨架动作识别提供了新解决方案，证明了脉冲图神经网络在能效和性能方面的潜力。
...
TL;DR: VLM-PAR：基于冻结SigLIP 2多语言编码器的视觉语言框架，通过跨模态融合改进视觉特征，在行人属性识别任务中显著提升性能，特别是在类别不平衡的PA100K基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 行人属性识别面临严重类别不平衡、复杂属性依赖和域偏移等挑战，需要更鲁棒的解决方案来克服这些限制。

Method: 基于冻结的SigLIP 2多语言编码器构建模块化视觉语言框架，通过紧凑的交叉注意力融合对齐图像和提示嵌入，精炼视觉特征。

Result: 在高度不平衡的PA100K基准上实现显著精度提升并达到新的SOTA性能，同时在PETA和Market-1501基准上获得显著的均值精度提升。
...
TL;DR: ViSignVQA是首个大规模越南语招牌视觉问答数据集，包含10,762张图像和25,573个问答对，通过集成越南语OCR和语言模型提升VQA性能，并提出多智能体框架达到75.98%准确率。


<details>
  <summary>Details</summary>
Motivation: 自然场景中的招牌文本理解对VQA实际应用至关重要，但在低资源语言（如越南语）中研究不足。现有VQA数据集缺乏针对招牌文本的专门资源，特别是考虑越南语特有的双语文本、非正式表达和视觉特征。

Method: 1) 创建ViSignVQA数据集，捕捉越南招牌的语言、文化和视觉特征；2) 集成越南语OCR模型(SwinTextSpotter)和预训练语言模型(ViT5)到SOTA VQA模型(BLIP-2, LaTr等)；3) 提出多智能体VQA框架，结合感知和推理智能体与GPT-4，采用多数投票决策。

Result: OCR增强上下文显著提升性能，将OCR文本附加到问题时F1分数提升高达209%。多智能体框架通过多数投票达到75.98%准确率。实验表明领域特定资源对低资源语言文本VQA的重要性。

Conclusion: ViSignVQA是首个面向越南语招牌理解的大规模多模态数据集，为低资源语言的OCR集成VQA模型提供了基准。研究强调了领域特定资源在增强基于文本的VQA中的重要性，特别是对于低资源语言。
...
TL;DR: 利用2D视觉语言模型的关联激活作为"抽象物体"表示，学习被遮挡物体的3D定位与补全，通过历史放置数据提高搜索效率


<details>
  <summary>Details</summary>
Motivation: 为家庭机器人提供寻找丢失物体的技能，特别是那些被部分遮挡、无法直接被视觉语言模型识别的"隐藏物体"，以节省时间和精力

Method: 将2D视觉语言模型的关联激活图视为抽象物体表示，利用历史放置数据作为先验知识，进行无结构化搜索来定位隐藏物体的完整3D位置

Result: 模型能够显著比随机搜索更快地在第一次尝试中准确识别隐藏物体的完整3D位置

Conclusion: 语义抽象框架的扩展有望为家庭机器人提供高效寻找丢失物体的能力，特别是在处理被遮挡物体时
...
<details>
  <summary>Details</summary>
Motivation: 现有静态策略（如稀疏采样、帧压缩和聚类）针对离线场景优化，应用于连续视频流时会产生碎片化或过度压缩的输出，难以实现时序连贯的视频理解。

Method: 提出VideoScaffold框架，包含两个核心组件：弹性尺度事件分割（EES）进行预测引导的动态边界细化，以及分层事件整合（HEC）将语义相关片段逐步聚合成多层次抽象表示。

Result: 在离线和流式视频理解基准测试中达到最先进性能，框架模块化且即插即用，能无缝扩展现有基于图像的MLLMs到连续视频理解。

Conclusion: VideoScaffold通过动态调整事件粒度并保留细粒度视觉语义，有效解决了长视频理解中的冗余性和时序连贯性问题，为流式视频理解提供了高效解决方案。
...
TL;DR: 本文提出KAN-FPN-Stem架构，通过用KAN卷积层替换FPN末端的标准3x3卷积，显著提升ViT在姿态估计任务上的性能，在COCO数据集上达到+2.0 AP的提升。


<details>
  <summary>Details</summary>
Motivation: 现有ViT模型（如ViTPose）的前端设计过于简单，其朴素的patch化机制难以有效处理多尺度变化，并在初始特征提取阶段造成不可逆的信息损失，限制了模型性能。

Method: 保留FPN经典的"上采样-相加"融合流，但将其末端的标准线性3x3平滑卷积替换为基于KAN的卷积层。该KAN层利用其强大的非线性建模能力，自适应学习和校正多尺度融合过程中产生的"伪影"。
...
TL;DR: 提出MiG-DM模型，通过元信息引导的跨域协同扩散模型，整合多模态先验知识来提升低剂量PET成像质量。
...
Method: 提出元信息引导的跨域协同扩散模型(MiG-DM)：1) 元信息编码模块将临床参数转化为语义提示，考虑患者特征、剂量相关信息和半定量参数，实现文本元信息与图像重建的跨模态对齐；2) 跨域架构结合投影域和图像域处理，投影域中专门的正弦图适配器通过卷积操作捕获全局物理结构。

Result: 在UDPET公共数据集和不同剂量水平的临床数据集上的实验表明，MiG-DM在增强PET图像质量和保留生理细节方面优于现有最先进方法。

Conclusion: MiG-DM通过整合全面的跨模态先验知识，有效解决了低剂量PET成像中的质量挑战，为临床提供了高质量的PET图像重建方案。
...
TL;DR: 提出一种混合知识蒸馏框架，用于开发轻量高性能CNN模型，在保持高精度的同时大幅降低计算成本和模型大小
...
Method: 设计结合倒置残差块和密集连接的自定义学生模型，使用ResNet18作为教师网络，采用多目标策略集成硬标签监督、特征级蒸馏、响应级蒸馏和自蒸馏

Result: 在稻种品种识别任务上达到98.56%准确率（教师模型98.65%），仅需0.68 GFLOPs和约107万参数，计算成本减少约2.7倍，模型大小减少超10倍；在多个植物叶片病害数据集上表现一致

Conclusion: 该框架在保持高分类精度的同时显著降低计算和存储需求，展示了在硬件受限的智能农业系统中的鲁棒性、高效性和强大部署潜力
...
Method: 训练33个YOLO模型，使用超过22,000张标注图像，涵盖三种光照水平。通过11种融合比例（从100/0到0/100）对齐融合RGB和LWIR帧，动态选择最优检测模型

Result: 最佳全光模型（80/20 RGB-LWIR）和暗光模型（90/10融合）分别达到92.8%和92.0%的平均置信度，显著优于YOLOv5n和YOLOv11n基线。无光条件下40/60融合达到71.0%，优于基线但统计不显著
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像扩散模型虽然能生成高度详细的纹理，但主要依赖表面外观，难以遵循严格的几何约束，尤其是在几何约束与文本提示暗示的风格相冲突时。这反映了人类感知与当前生成模型之间的语义鸿沟。
...
Result: 实验表明，流模型在没有持续引导时倾向于回到默认轨迹。该方法实现了复杂三维形状（如Eames椅子）到冲突材质（如粉色金属）的零样本迁移。与无引导基线相比，引导生成将语义对齐提高了约80%。

Conclusion: 小型教师模型能够可靠地指导大型生成系统，增强几何控制能力，拓宽文本到图像合成的创意范围，无需专门训练即可引入几何理解。
...
Method: GeCo通过融合残差运动和深度先验，生成密集的一致性图来检测几何变形和遮挡不一致。该方法基于几何基础，能够提供可解释的伪影检测结果。

Result: GeCo成功检测出视频生成模型中的常见失败模式，并作为无需训练的指导损失函数，在视频生成过程中有效减少了变形伪影。

Conclusion: GeCo是一个有效的几何一致性度量工具，既能用于系统评估视频生成模型的质量，又能作为指导损失来改善生成结果，减少几何变形和遮挡不一致问题。
...
TL;DR: 该研究开发了B&J骨科基准测试，发现AI模型在结构化选择题上表现优异（>90%），但在需要多模态整合的开放式任务中表现显著下降（仅约60%），表明当前AI模型尚不具备临床多模态推理能力。
...
Method: 开发了B&J基准测试框架，包含1,245个来自真实骨科和运动医学病例的问题，评估模型在7个临床推理任务上的表现，包括知识回忆、文本和图像解释、诊断生成、治疗计划和理由提供。评估了11个视觉语言模型和6个大语言模型，与专家提供的真实答案进行比较。

Result: 结果显示任务类型间存在显著性能差距：最先进模型在结构化多选题上准确率超过90%，但在需要多模态整合的开放式任务中准确率仅约60%。VLM在解释医学图像方面存在严重限制，经常出现严重的文本驱动幻觉，忽略矛盾的视觉证据。医学专用微调模型相比通用模型没有明显优势。
...
TL;DR: 提出了首个专门用于评估视觉语言模型在胎儿超声成像中性能的标准化基准Fetal-Gauge，包含超过42,000张图像和93,000个问答对，覆盖多个临床任务，评估显示当前最佳模型准确率仅55%，远低于临床要求。


<details>
  <summary>Details</summary>
Motivation: 全球训练有素的超声技师短缺，阻碍了胎儿健康监测；深度学习有潜力提高超声技师效率并支持新从业者培训；视觉语言模型特别适合超声解释，但缺乏标准化基准来评估其在胎儿超声成像中的性能。

Method: 创建Fetal-Gauge基准，包含超过42,000张胎儿超声图像和93,000个问答对，涵盖解剖平面识别、解剖结构视觉定位、胎儿方向评估、临床视图符合性和临床诊断等任务；系统评估多个最先进的视觉语言模型，包括通用和医学专用模型。

Result: 最佳表现模型仅达到55%准确率，远低于临床要求；分析揭示了当前视觉语言模型在胎儿超声解释中的关键局限性；建立了推进产前护理中多模态深度学习的严格基础。

Conclusion: 迫切需要领域适应的架构和专门训练方法；Fetal-Gauge为推进产前护理中的多模态深度学习建立了严格基础，为解决全球医疗可及性挑战提供了途径；基准将在论文接受后公开可用。
...
Method: 基于Align3D 130数据集，采用3D文本多头注意力搜索模型，通过三个组件增强跨模态对齐：精确文本到3D检索、多视角3D到图像对齐、图像到文本对齐以生成时间一致的4D资产。

Result: 实验结果表明Uni4D实现了高质量的3D检索和可控4D生成。

Conclusion: Uni4D框架推进了动态多模态理解和实际应用的发展。
...
TL;DR: SirenPose是一种结合正弦表示网络周期激活特性和关键点结构几何先验的新型损失函数，用于提升动态3D场景重建的精度，特别在快速运动和复杂场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法在快速运动和多人场景中难以保持运动建模精度和时空一致性，需要一种能够增强时空连贯性的新方法。

Method: 结合正弦表示网络的周期激活特性与关键点结构的几何先验，引入物理启发的约束机制，强制空间和时间维度上的关键点预测一致性，并扩展训练数据集至60万标注实例。

Result: 使用SirenPose训练的模型在时空一致性指标上相比先前方法有显著提升，在处理快速运动和复杂场景变化方面表现更优。

Conclusion: SirenPose通过结合周期性激活和几何先验，有效提升了动态3D场景重建的时空一致性，特别是在挑战性场景中表现出色。
...
TL;DR: 提出一个用于低成本边缘设备的单摄像头驾驶员行为识别系统，能在树莓派5和Coral Edge TPU上实现实时监控，覆盖17种行为类别


<details>
  <summary>Details</summary>
Motivation: 车内驾驶员监控系统需要在计算、功耗和成本严格限制下，低延迟识别分心和困倦相关行为

Method: 结合紧凑的逐帧视觉模型、减少视觉相似误报的混淆感知标签设计，以及仅在预测既自信又持续时才触发警报的时序决策头

Result: 在树莓派5上达到约16 FPS（每帧延迟低于60毫秒），在Coral Edge TPU上达到约25 FPS，能够在廉价硬件上实现实时监控和稳定警报生成

Conclusion: 可靠的车内人类状态感知可以作为以人为本的车辆智能的上游输入，包括新兴的自主车辆概念
...
TL;DR: 提出一种攻击感知的深度伪造和图像取证检测器，结合红队训练和随机化测试时防御，通过双流架构实现鲁棒性、校准良好的概率和透明证据


<details>
  <summary>Details</summary>
Motivation: 在现实部署条件下，需要能够抵抗对抗攻击、提供校准良好的概率估计和透明证据的深度伪造检测器

Method: 双流架构：语义流使用预训练骨干网络，取证流提取取证残差，通过轻量残差适配器融合；红队训练应用最差K种反取证技术；测试时防御注入低成本抖动；使用浅层特征金字塔网络生成篡改热图

Result: 在现有基准测试中表现出近乎完美的攻击排名、低校准误差、最小弃权风险，在重压缩等攻击下受控退化

Conclusion: 建立了一个模块化、数据高效、可实际部署的攻击感知检测基线，具有校准概率和可操作热图
...
TL;DR: PortionNet：通过跨模态知识蒸馏从单张RGB图像进行食物营养估计，无需深度传感器，在训练时学习点云几何特征，推理时仅需RGB图像


<details>
  <summary>Details</summary>
Motivation: 从单张图像准确估计食物营养具有挑战性，因为丢失了3D信息。基于深度的方法虽然可靠，但需要深度传感器，而大多数智能手机不具备此功能

Method: 提出PortionNet跨模态知识蒸馏框架：1）双模式训练策略，轻量级适配器网络模仿点云表示；2）训练时学习点云几何特征，推理时仅需RGB图像；3）实现伪3D推理，无需专用硬件

Result: 在MetaFood3D数据集上达到最先进性能，在体积和能量估计方面优于所有先前方法。在SimpleFood45上的跨数据集评估显示能量估计具有强泛化能力

Conclusion: PortionNet通过知识蒸馏实现了无需深度传感器的食物营养估计，在保持高性能的同时具有更好的实用性和可访问性
...
TL;DR: MoFu是一个多主体视频生成框架，通过尺度感知调制和傅里叶融合解决尺度不一致和排列敏感性问题，显著提升了生成视频的质量。


<details>
  <summary>Details</summary>
Motivation: 当前多主体视频生成方法存在两个主要问题：尺度不一致（不同主体尺寸变化导致生成不自然）和排列敏感性（参考图像输入顺序变化导致主体变形）。这些问题影响了生成视频的视觉质量和主体保真度。

Method: 1. 尺度感知调制（SMO）：利用LLM从文本提示中提取隐式尺度线索，调制特征以确保主体尺寸一致
2. 傅里叶融合：通过快速傅里叶变换处理参考特征的频率信息，生成统一表示以解决排列敏感性问题
3. 尺度-排列稳定性损失：联合优化尺度一致性和排列不变性
4. 建立专门基准测试集：包含受控的主体尺度和参考排列变化

Result: 大量实验表明，MoFu在保持自然尺度、主体保真度和整体视觉质量方面显著优于现有方法。

Conclusion: MoFu通过统一的框架有效解决了多主体视频生成中的尺度不一致和排列敏感性问题，为高质量多主体视频生成提供了有效的解决方案。
...
TL;DR: VideoZoomer：一种新颖的代理框架，使多模态大语言模型能够通过动态控制视觉焦点来理解长视频，采用两阶段训练策略，在多个长视频理解基准上表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在长视频理解方面存在局限，主要受限于上下文窗口长度。现有方法通常依赖均匀帧采样或静态预选择，可能忽略关键证据且无法在推理过程中纠正初始选择错误。

Method: 提出VideoZoomer框架，让MLLMs在推理过程中动态控制视觉焦点。从低帧率概览开始，调用时间缩放工具在自主选择的时刻获取高帧率片段，以多轮交互方式逐步收集细粒度证据。采用两阶段训练策略：首先在精心策划的蒸馏示例和反思轨迹数据集上进行监督微调，然后通过强化学习进一步优化代理策略。

Result: 7B模型展现出多样且复杂的推理模式，在广泛的长视频理解和推理基准测试中表现强劲。能够持续超越现有开源模型，甚至在具有挑战性的任务上与专有系统相媲美，同时在减少帧预算的情况下实现更优效率。

Conclusion: VideoZoomer通过动态视觉焦点控制和多轮交互推理，有效解决了长视频理解中的关键挑战，为MLLMs在长视频任务中的应用提供了新的有效框架。
...
TL;DR: SpotEdit：一种无需训练的选择性区域更新扩散编辑框架，通过识别稳定区域并跳过计算来提升效率


<details>
  <summary>Details</summary>
Motivation: 当前扩散Transformer模型在图像编辑时均匀处理所有token，即使只修改小区域也进行全图去噪，导致计算冗余且可能破坏未修改区域的质量

Method: SpotEdit包含两个核心组件：SpotSelector通过感知相似性识别稳定区域并重用条件图像特征跳过计算；SpotFusion通过动态融合机制自适应混合特征，保持上下文连贯性和编辑质量

Result: 通过减少不必要的计算并保持未修改区域的高保真度，SpotEdit实现了高效精确的图像编辑

Conclusion: SpotEdit证明了在图像编辑中无需重新生成每个区域，选择性更新修改区域即可实现高质量编辑，同时显著提升计算效率
...
TL;DR: DeMoGen提出了一种基于能量扩散模型的组合训练范式，用于将整体运动分解为语义上有意义的子组件，无需单个概念的ground-truth运动数据。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注正向建模（如从文本到运动的整体映射），而本文考虑逆向视角：将整体运动分解为语义上有意义的子组件，以发现可重用的运动基元。

Method: 提出DeMoGen组合训练范式，采用基于能量的扩散模型直接捕捉多个运动概念的组合分布。包含三种训练变体：1) DeMoGen-Exp使用分解的文本提示显式训练；2) DeMoGen-OSS进行正交自监督分解；3) DeMoGen-SC强制原始和分解文本嵌入之间的语义一致性。

Result: 方法能够从复杂运动序列中解耦出可重用的运动基元，分解后的运动概念可以灵活重组生成多样新颖的运动，超越训练分布。还构建了文本分解数据集支持组合训练。

Conclusion: DeMoGen提供了一种有效的运动分解学习范式，能够发现和重组运动概念，为文本到运动生成和运动组合提供了扩展资源。
...
<details>
  <summary>Details</summary>
Motivation: 传统单模态和早期融合方法存在特征冗余高、模态特定信息建模不完整的问题，需要更有效的多模态整合方法来非侵入性推断GBM的分子肿瘤特征，特别是对预后和治疗重要的MGMT启动子甲基化状态。

Method: 基于变分自编码器的多视图潜在表示学习框架：每个模态通过独立的概率编码器编码，在紧凑的潜在空间中进行融合，保留模态特定结构的同时实现有效的多模态整合，生成的潜在嵌入用于MGMT启动子甲基化分类。

Result: 该方法能够整合T1Gd和FLAIR MRI的互补放射组学特征，在紧凑潜在空间中实现多模态融合，为MGMT启动子甲基化分类提供有效的特征表示。

Conclusion: 提出的多视图潜在表示学习框架能够克服传统方法的局限性，通过独立编码和潜在空间融合，更好地整合多模态信息，为GBM的MGMT启动子甲基化状态的非侵入性推断提供有效解决方案。
...
TL;DR: 提出基于视觉Transformer的端到端管道，用于联合分析H&E和IHC染色图像，实现像素级HER2评分定位
...
Method: 使用视觉Transformer系统，通过分块处理H&E WSI进行肿瘤定位，提出新颖映射函数关联IHC区域，嵌入临床启发的4级HER2评分机制

Result: 肿瘤定位分类准确率高，HER2状态预测准确率达0.94，特异性0.933，4级评分方法表现良好，与病理学家结果可比
...
TL;DR: 研究提出一种感知启发的伪着色技术，通过将临床相关特征（如QT间期）编码为结构化颜色表示，提升深度神经网络在ECG分析中的可解释性和少样本学习能力。


<details>
  <summary>Details</summary>
Motivation: 当前机器视觉模型（特别是深度神经网络）在生理信号解释（如心电图）中需要大量训练数据且缺乏可解释性，限制了临床可靠性和与人类推理的一致性。需要提高数据效率和可解释性。

Method: 采用感知启发的伪着色技术，将临床显著的时序特征（如QT间期持续时间）编码为结构化颜色表示。使用原型网络和ResNet-18架构，在单次心跳和完整10秒节律的ECG图像上评估单样本和少样本学习。

Result: 模型仅需1-5个训练样本就能学习到区分性和可解释的特征。可解释性分析显示伪着色引导注意力朝向临床有意义的ECG特征，同时抑制无关信号成分。聚合多个心跳周期能进一步提升性能，模拟人类跨心跳的感知平均。

Conclusion: 人类感知编码可以弥合医学机器智能中的数据效率、可解释性和因果推理之间的差距，为临床可靠的AI系统提供新途径。
...
TL;DR: 提出基于MLLMs的3D场景操作框架，通过MCP API、视觉工具套件和多智能体协作解决3D物体布置任务中的三大挑战


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在2D视觉语言任务上取得显著进展，但在复杂3D场景操作方面的应用仍未被充分探索。现有MLLMs在3D物体布置任务中存在三大挑战：视觉基础薄弱、3D场景理解不足、迭代更新容易出错

Method: 1) 引入MCP-based API，将交互从脆弱的原始代码操作转向更稳健的函数级更新；2) 增强MLLM的3D场景理解能力，配备专门视觉工具套件分析场景状态、收集空间信息、验证动作结果；3) 提出协作多智能体框架，分配规划、执行、验证角色，处理多步指令并从中间错误中恢复

Result: 在25个复杂物体布置任务上验证了方法的有效性，显著优于现有基线方法

Conclusion: 通过解决MLLMs在3D场景操作中的三大关键挑战，成功将MLLMs应用于复杂3D物体布置任务，为3D场景理解与操作开辟了新方向
...
TL;DR: Self-E 是一种新颖的文本到图像生成模型，通过自评估机制实现从零开始训练，支持任意步数推理，在少步生成和多步生成中均表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统扩散或流匹配模型依赖局部监督需要多步推理，蒸馏方法需要预训练教师模型。Self-E 旨在填补这两种范式之间的差距，实现从零开始训练的高质量文本到图像生成，支持任意步数推理。

Method: Self-E 结合了流匹配模型的数据学习方式，同时引入新颖的自评估机制：使用当前分数估计评估自身生成的样本，作为动态自教师。它不依赖局部监督（需要多步推理），也不需要预训练教师模型。

Result: 在大规模文本到图像基准测试中，Self-E 在少步生成方面表现出色，同时在50步时与最先进的流匹配模型竞争。性能随着推理步数增加单调提升，支持超快速少步生成和高质量长轨迹采样。

Conclusion: Self-E 是第一个从零开始、支持任意步数的文本到图像模型，为高效可扩展生成提供了统一框架，填补了局部学习和全局匹配之间的范式差距。
...
TL;DR: iOSPointMapper是一个移动应用，利用iPhone/iPad的传感器和AI技术实时、隐私保护地收集人行道数据，通过用户验证确保质量，数据匿名传输到交通数据交换平台。


<details>
  <summary>Details</summary>
Motivation: 当前人行道数据收集方法成本高、分散且难以扩展，需要准确、及时的数据来建设无障碍和包容性的行人基础设施。

Method: 开发iOSPointMapper应用，利用设备端语义分割、LiDAR深度估计和GPS/IMU融合数据检测和定位人行道相关特征（交通标志、信号灯、杆柱），包含用户引导的标注界面验证系统输出。

Result: 系统特征检测和空间映射性能的详细评估显示该应用在增强行人地图绘制方面具有潜力，能够无缝集成到多模式交通数据集中。

Conclusion: iOSPointMapper提供了一个可扩展、以用户为中心的方法来填补行人基础设施中的关键数据缺口，支持更智能的城市规划和无障碍设计。
...
TL;DR: DeFloMat提出了一种基于条件流匹配的生成式目标检测框架，通过确定性流场替代扩散模型的随机去噪过程，在仅需3步推理的情况下实现SOTA精度，解决了扩散检测器在临床应用中延迟过高的问题。


<details>
  <summary>Details</summary>
Motivation: 扩散模型检测器（如DiffusionDet）虽然精度高，但需要大量采样步骤（T>60），在时间敏感的临床应用中（如克罗恩病的磁共振肠造影检测）存在严重的延迟瓶颈，无法满足实际需求。

Method: DeFloMat采用条件流匹配（CFM）框架，基于条件最优传输理论构建确定性流场，近似Rectified Flow，通过简单的常微分方程求解器实现快速推理，替代了扩散模型的多步随机去噪过程。

Result: 在MRE临床数据集上，DeFloMat仅用3步推理就达到了43.32% AP_{10:50}的SOTA精度，比DiffusionDet在4步时的最佳性能（31.03%）提升了1.4倍，同时显著改善了定位特性，在少步推理下具有更好的召回率和稳定性。
...
TL;DR: Bright-4B是一个40亿参数的基础模型，直接从3D明场体积图像分割亚细胞结构，无需荧光标记或复杂后处理


<details>
  <summary>Details</summary>
Motivation: 无标记3D明场显微镜能快速、无创地可视化细胞形态，但现有的稳健体积分割通常依赖荧光标记或复杂的后处理，需要填补这一技术空白

Method: 结合硬件对齐的原生稀疏注意力机制（捕捉局部、粗粒度和选择的全局上下文）、深度-宽度残差超连接稳定表示流、软专家混合实现自适应容量，以及各向异性补丁嵌入尊重共聚焦点扩散和轴向变薄

Result: 模型能够仅从明场图像堆栈中产生形态准确的细胞核、线粒体和其他细胞器分割，在多个共聚焦数据集上保持跨深度和细胞类型的精细结构细节，优于当代CNN和Transformer基线
...
TL;DR: FluenceFormer：基于Transformer的放疗通量图预测框架，通过两阶段设计和物理感知回归损失，解决传统卷积方法难以捕捉长程依赖的问题，在IMRT计划中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 放疗通量图预测是自动化放疗计划的核心，但由于解剖结构与束流强度调制之间的复杂关系，这仍然是一个不适定逆问题。传统卷积方法难以捕捉长程依赖，可能导致结构不一致或物理不可行的计划。

Method: 提出FluenceFormer框架：1）两阶段设计：第一阶段从解剖输入预测全局剂量先验，第二阶段将该先验与显式束流几何条件结合，回归物理校准的通量图；2）引入Fluence-Aware Regression (FAR)损失函数，整合体素级保真度、梯度平滑性、结构一致性和束流能量守恒；3）框架与多种Transformer骨干网络兼容。

Result: 在多个Transformer骨干网络（Swin UNETR、UNETR、nnFormer、MedFormer）上评估，使用前列腺IMRT数据集。FluenceFormer与Swin UNETR组合表现最佳，相比现有CNN和单阶段方法，将能量误差降低至4.5%，并在结构保真度上获得统计学显著提升（p < 0.05）。

Conclusion: FluenceFormer通过两阶段Transformer框架和物理感知回归损失，有效解决了放疗通量图预测中的长程依赖问题，实现了更准确、物理一致的自动化放疗计划。
...
TL;DR: 提出EmoCtrl模型，实现可控情感图像内容生成，在保持内容描述准确性的同时表达目标情感，解决了现有模型在情感表达和内容保真度之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 图像通过视觉内容和情感色调共同传达意义，但现有文本到图像模型缺乏情感意识，而情感驱动模型则以内容失真为代价。需要一种既能保持内容一致性又能表达目标情感的图像生成方法。

Method: 提出EmoCtrl模型，基于内容、情感和情感提示标注的数据集，通过文本和视觉情感增强模块，利用描述性语义和感知线索丰富情感表达，学习具有互补效果的情感标记。

Result: 定量和定性实验表明EmoCtrl在忠实内容和表达情感控制方面优于现有方法，用户研究确认与人类偏好高度一致，且学习的情绪标记具有良好的泛化能力和适应性。

Conclusion: EmoCtrl成功实现了可控情感图像内容生成，在保持内容准确性的同时表达目标情感，学习的情绪标记具有互补效果和良好泛化能力，为创意应用提供了强大工具。
...
TL;DR: SuperiorGAT是一个基于图注意力的框架，用于重建稀疏LiDAR点云中缺失的高程信息，通过建模光束感知图和使用门控残差融合，在不增加网络深度的情况下提高LiDAR分辨率。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统中的LiDAR感知受到固定垂直光束分辨率的限制，并且由于环境遮挡导致光束丢失问题进一步恶化，需要在不增加额外传感器硬件的情况下提高分辨率。

Method: 将LiDAR扫描建模为光束感知图，采用图注意力机制，结合门控残差融合和前馈细化，在不增加网络深度的情况下重建缺失的高程信息。

Result: 在KITTI数据集的不同环境（Person、Road、Campus、City）中，通过模拟移除每第四个垂直扫描光束的结构化光束丢失，SuperiorGAT相比基于PointNet的模型和更深的GAT基线，始终实现更低的重建误差和更好的几何一致性。

Conclusion: 架构细化提供了一种计算高效的方法来改善LiDAR分辨率，无需额外的传感器硬件，SuperiorGAT能够保持结构完整性并最小化垂直失真。
...
TL;DR: 提出基于直线的异步事件相机标定框架，利用人造环境中常见物体的几何直线特征，无需专用标定板，适用于单目和双目事件相机。
...
Method: 直接从事件流中检测直线，利用事件-直线标定模型生成相机参数初始估计，适用于平面和非平面直线，然后采用非线性优化精化相机参数。

Result: 仿真和真实世界实验验证了方法的可行性和准确性，在单目和双目事件相机上均进行了验证，源代码已开源。

Conclusion: 提出的基于直线的标定框架能够利用环境中常见物体的几何直线特征，实现快速、准确的事件相机标定，无需专用标定板，适应性强。
...
<details>
  <summary>Details</summary>
Motivation: 视障人士定位物体是一项重大挑战，影响他们的独立性并可能导致危险情况。为了增强视障人士的自给自足能力，需要开发辅助工具减少对他人的依赖

Method: 使用Flutter开发平台构建应用，后端采用Efficientdet-D2模型进行物体检测。通过耳机提供声音提示：物体在右侧时右耳播放正弦声音，在左侧时左耳播放，正前方时双耳同时播放

Result: 开发了SonoVision应用，能够帮助视障人士通过智能手机定位物体，减少对周围人的依赖，增强独立性。应用完全离线工作，安全且用户友好

Conclusion: SonoVision应用将显著帮助视障人士以安全、用户友好的方式定位日常物品，提高他们的独立性和自给自足能力
...
TL;DR: SAM 3D在单目遥感图像建筑重建中表现优于TRELLIS，能生成更连贯的屋顶几何和清晰边界，并通过分段-重建-组合流程扩展到城市场景重建。


<details>
  <summary>Details</summary>
Motivation: 现有单目3D建筑重建方法通常需要特定任务架构和密集监督，限制了可扩展性。本文旨在评估通用图像到3D基础模型SAM 3D在遥感建筑重建中的性能，探索基础模型在城市3D重建中的应用潜力。

Method: 在NYC Urban Dataset上对SAM 3D与TRELLIS进行基准测试，使用FID和CMMD作为评估指标。进一步通过分段-重建-组合流程将SAM 3D扩展到城市场景重建。

Result: 实验结果显示，SAM 3D相比TRELLIS能生成更连贯的屋顶几何和更清晰的边界。通过扩展流程成功实现了城市场景重建，证明了其在城市建模中的潜力。

Conclusion: SAM 3D在单目遥感建筑重建中表现优异，为城市3D重建中基础模型的部署提供了实用指导，并激励未来整合场景级结构先验的进一步研究。
...
TL;DR: 本文比较了YOLOv8、YOLOv11和RF-DETR三种计算机视觉模型在变电站组件检测中的性能，旨在实现高效、自动化的变电站基础设施映射。


<details>
  <summary>Details</summary>
Motivation: 变电站作为电网关键基础设施，其资产易受多种灾害影响，传统人工映射方法耗时耗力，需要自动化解决方案来提高效率和便利性。

Method: 使用手动标注的美国变电站图像数据集，训练并比较YOLOv8、YOLOv11和RF-DETR三种模型，评估它们的检测精度、精确度和效率。

Result: 分析了各模型的关键优势和局限性，识别出能够提供可靠且大规模变电站组件映射的最佳模型，并展示了这些模型在美国变电站组件映射中的实际应用案例。

Conclusion: 通过比较三种计算机视觉模型，为变电站组件映射提供了有效的机器学习解决方案，展示了机器学习在变电站基础设施映射中的应用潜力。
...
TL;DR: PGR²M提出了一种混合表示方法，通过残差向量量化增强可解释的姿态编码，以改进文本驱动的3D动作生成和编辑的质量与可控性。


<details>
  <summary>Details</summary>
Motivation: 现有基于姿态编码的方法（如CoMo）虽然支持可解释的动作控制，但其逐帧表示难以捕捉细微的时间动态和高频细节，导致重建保真度和局部可控性下降。

Method: 提出PGR²M混合表示：1）姿态引导的RVQ分词器将动作分解为编码全局粗结构的姿态潜在变量和建模细粒度时间变化的残差潜在变量；2）残差丢弃防止过度依赖残差；3）基础Transformer从文本自回归预测姿态编码，精炼Transformer基于文本、姿态编码和量化阶段预测残差编码。

Result: 在HumanML3D和KIT-ML数据集上，PGR²M在Fréchet inception距离和重建指标上优于CoMo及最近的扩散和分词基线方法，用户研究证实其支持直观、结构保持的动作编辑。

Conclusion: PGR²M通过混合表示有效平衡了动作生成的可解释性和细节保真度，为文本驱动的3D动作合成和编辑提供了更高质量的解决方案。
...
TL;DR: 提出基于多事件相机的高时空分辨率冲击波运动参数测量框架，通过极坐标编码、自适应ROI提取和迭代斜率分析，实现冲击波不对称性估计、运动场重建和爆炸当量反演。


<details>
  <summary>Details</summary>
Motivation: 冲击波快速不均匀传播和不稳定测试条件给高时空分辨率测量带来挑战，需要利用事件相机的高速高动态范围特性来准确测量冲击波运动参数。

Method: 建立极坐标系编码事件揭示传播模式，通过事件偏移计算自适应ROI提取，利用迭代斜率分析提取冲击波前事件，基于事件光学成像模型推导几何模型和3D重建模型。

Result: 与压力传感器和经验公式对比，速度测量最大误差5.20%，最小误差0.06%，实现了高时空分辨率的高精度冲击波运动场测量。

Conclusion: 该方法利用多事件相机成功实现了冲击波不对称性估计、多角度测量、运动场重建和爆炸当量反演，在冲击波测量领域取得显著进展。
...
TL;DR: 提出一种用于红外小目标检测的半监督范式，通过分层MoE适配器和两阶段知识蒸馏，使用10%标注数据训练专家教师模型，再生成伪标签训练轻量下游模型。
...
Method: 设计分层MoE适配器（四个白盒神经算子），采用两阶段范式：1) 先验引导知识蒸馏，用MoE适配器和10%标注数据将SAM蒸馏为专家教师模型Scalpel-SAM；2) 部署导向知识转移，用Scalpel-SAM生成伪标签训练轻量下游模型。

Result: 实验表明，在最小标注量下，该范式使下游模型达到甚至超越全监督模型的性能，是首个系统解决IR-SOT数据稀缺问题的半监督范式。

Conclusion: 提出的分层MoE适配器和两阶段知识蒸馏范式有效解决了红外小目标检测中的标注成本问题，实现了高效的知识迁移，为半监督红外目标检测提供了新思路。
...
TL;DR: Video-GMAE是一种自监督视频表示学习方法，将视频表示为随时间移动的高斯溅射集合，通过这种3D场景表示学习实现零样本跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 视频通常是动态3D场景的2D投影，因此将视频表示为移动的高斯集合能够提供合理的归纳偏置，促进跟踪等时空理解能力的出现。

Method: 提出Video Gaussian Masked Autoencoders，将图像序列编码为随时间移动的高斯溅射集合，通过掩码自编码器架构进行自监督预训练。

Result: 学习到的高斯轨迹映射到图像平面可实现零样本跟踪，性能媲美SOTA；微调后在Kinetics和Kubric数据集上分别提升34.6%和13.1%，超越现有自监督方法。

Conclusion: Video-GMAE通过高斯表示提供有效的3D场景归纳偏置，在自监督视频表示学习中实现了出色的跟踪性能和下游任务表现。
...
TL;DR: SCAFusion是一个针对月球机器人任务的多模态3D目标检测模型，通过认知适配器、对比对齐模块、相机辅助训练分支和分段感知坐标注意力机制，显著提升了小尺寸不规则目标的检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有用于地面自动驾驶的多模态3D感知方法在月球等外星环境中表现不佳，主要原因是特征对齐差、多模态协同有限、小目标检测能力弱。月球表面小尺寸不规则物体（如陨石碎片和岩石）的可靠精确检测对于自主导航和操作至关重要。

Method: 基于BEVFusion框架构建，集成了四个关键组件：1）认知适配器用于高效调整相机骨干网络；2）对比对齐模块增强相机-LiDAR特征一致性；3）相机辅助训练分支强化视觉表示；4）分段感知坐标注意力机制专门提升小尺寸不规则目标的检测性能。

Result: 在nuScenes验证集上达到69.7% mAP和72.1% NDS，比基线分别提升5.0%和2.7%。在Isaac Sim构建的模拟月球环境中达到90.93% mAP，比基线提升11.5%，在检测陨石状小障碍物方面表现尤为突出。

Conclusion: SCAFusion通过创新的多模态融合架构，在参数和计算量几乎不增加的情况下，显著提升了月球环境中小尺寸不规则目标的检测性能，为月球机器人任务提供了有效的3D感知解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有统一生成和编辑模型主要依赖文本提示，但语言难以准确捕捉用户意图的编辑位置和视觉细节，需要更直观的交互方式

Method: 1) 数据合成管道：涂鸦编辑（4个任务）和涂鸦生成（3个任务）；2) 框架设计：联合输入方案，将原始图像和涂鸦源图像同时输入，使用不同颜色区分区域，应用相同索引和位置编码

Result: 实验结果表明DreamOmni3取得出色性能，模型和代码将公开发布，并建立了综合基准促进进一步研究

Conclusion: DreamOmni3通过涂鸦交互解决了文本提示的局限性，实现了更精确的编辑定位和灵活的创作，为多模态GUI创作提供了新方向
...
TL;DR: CoAgent：一个用于连贯视频生成的协作闭环框架，通过计划-合成-验证流程解决现有模型中的身份漂移、场景不一致和时间结构不稳定问题。
...
Method: 提出CoAgent框架，采用计划-合成-验证流程：故事板规划器分解输入为镜头级计划；全局上下文管理器维护实体级记忆；合成模块在视觉一致性控制器指导下生成镜头；验证代理评估中间结果并触发选择性重新生成；节奏感知编辑器优化时间节奏和过渡。

Result: 大量实验表明，CoAgent在长视频生成中显著提高了连贯性、视觉一致性和叙事质量。

Conclusion: CoAgent通过协作闭环框架有效解决了开放域视频生成中的叙事连贯性和视觉一致性问题，为长视频生成提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态LLM常产生流畅但不可靠的推理，表现出弱的步骤间一致性和不足的视觉基础性，因为现有对齐方法只监督最终答案而忽略了中间推理过程的可靠性。

Method: 引入SR-MCR框架，利用五个自参考线索（语义对齐、词汇保真度、非冗余性、视觉基础性、步骤一致性）构建归一化的可靠性加权奖励，提供细粒度的过程级指导。采用无批评者的GRPO目标，结合置信度感知冷却机制稳定训练。

Result: 基于Qwen2.5-VL，SR-MCR在广泛的视觉基准上提高了答案准确性和推理一致性；在可比规模的开源模型中，SR-MCR-7B以81.4%的平均准确率实现了最先进性能。

Conclusion: SR-MCR通过利用内在过程信号有效对齐多模态LLM的推理过程，消融研究证实了每个奖励项和冷却模块的独立贡献，为可靠的多模态推理提供了轻量级解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 胶质瘤是最具侵袭性的癌症之一，死亡率高且诊断过程复杂。现有研究存在成像数据变异性高、计算资源优化不足、分割和分类效率低等问题，需要改进。

Method: 提出首个放射组学增强的融合残差多参数3D网络(ReFRM3D)，基于3D U-Net架构，包含多尺度特征融合、混合上采样和扩展残差跳跃机制。同时提出基于多特征肿瘤标记物的分类器，利用分割区域提取的放射组学特征。

Result: 在BraTS2019、BraTS2020和BraTS2021数据集上分割性能显著提升：BraTS2019的WT、ET、TC Dice系数分别为94.04%、92.68%、93.64%；BraTS2020为94.09%、92.91%、93.84%；BraTS2021为93.70%、90.36%、92.13%。

Conclusion: 提出的ReFRM3D网络和多特征分类器能有效解决胶质瘤分割和分类中的挑战，为脑肿瘤表征提供了创新解决方案，在多个数据集上验证了其优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 多视图3D几何网络提供了强大的先验知识，但推理速度过慢，无法满足实时应用需求。需要一种方法将这些网络适配到在线使用场景，实现实时的6-DoF姿态跟踪和在线重建。

Method: 1. 快速选择和管理关键帧集合，通过π³网络进行场景或物体映射；2. 缓存全局自注意力块的键值对作为唯一的场景表示；3. 使用缓存的KV对进行在线跟踪，无需重新计算；4. 该缓存策略是模型无关的，可直接应用于现成的多视图网络而无需重新训练。

Result: 在TUM RGB-D、7-Scenes、Arctic和OnePose数据集上表现出色，推理速度提升高达15倍，同时保持高达27 FPS的高帧率，避免了漂移和灾难性遗忘问题。

Conclusion: KV-Tracker成功将多视图3D几何网络适配到实时应用，通过缓存注意力机制的关键值对实现了高效的在线跟踪和重建，为实时SLAM和AR/VR应用提供了实用解决方案。
...
TL;DR: PTalker是一个用于个性化3D说话头动画的新框架，通过风格解耦和三层次对齐机制，在保持说话风格的同时提高唇部同步精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法虽然在高唇部同步精度方面取得了进展，但大多忽略了个人说话风格的细微差别，这限制了个性化和真实感。需要一种能够同时保持说话风格和提高唇部同步精度的方法。

Method: 1. 风格解耦：通过解耦约束将音频和面部运动序列编码到不同的风格和内容空间，增强说话风格表示。2. 三层次对齐机制：空间对齐（使用图注意力网络捕捉3D网格顶点连接）、时间对齐（使用交叉注意力捕捉时间依赖）、特征对齐（使用top-k双向对比损失和KL散度约束确保语音和网格模态一致性）。

Result: 在公共数据集上的大量定性和定量实验表明，PTalker能够有效生成逼真、风格化的3D说话头，准确匹配特定身份的说话风格，优于现有最先进方法。

Conclusion: PTalker通过风格解耦和三层次对齐机制，成功实现了在保持个性化说话风格的同时提高唇部同步精度，为3D说话头生成提供了更真实和个性化的解决方案。
...
TL;DR: 提出基于预测驱动的Top-K Jaccard相似度系数和稀疏差分Transformer，提升人脸聚类中嵌入关系测量的准确性和抗噪能力
...
Method: 1) 预测驱动的Top-K Jaccard相似度系数，提高邻居节点纯度；2) Transformer基预测模型分析中心节点与Top-K附近邻居的关系；3) 稀疏差分Transformer(SDT)替代标准Transformer，消除噪声并增强抗噪能力

Result: 在MS-Celeb-1M等多个数据集上达到最先进(SOTA)性能，优于现有方法，为人脸聚类提供更鲁棒的解决方案

Conclusion: 提出的预测驱动Top-K Jaccard相似度系数和稀疏差分Transformer能有效提升人脸聚类中关系测量的可靠性，解决了现有方法中无关节点过多和噪声干扰的问题
...
Method: 构建基于扩散大语言模型的视觉语言模型Dream-VL，并通过在开放机器人数据集上持续预训练扩展为Dream-VLA视觉语言动作模型。
...
Conclusion: 扩散模型的双向特性使其成为视觉语言动作任务的优越基础，支持动作分块和并行生成，在下游微调中收敛更快，在机器人控制任务中表现卓越。
...
TL;DR: 本文系统研究了基于SAM的视觉目标跟踪中的记忆机制，提出了统一的混合记忆框架，将记忆分解为短期外观记忆和长期干扰解决记忆，在SAM2和SAM3骨干网络上均提升了跟踪鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前基于SAM2的跟踪方法虽然性能强劲，但记忆机制设计缺乏系统性理解，且不清楚这些机制如何迁移到更强的SAM3等下一代基础模型。现有方法以特定方式处理记忆限制，缺乏统一的记忆设计原则。

Method: 首先分析代表性SAM2跟踪器，发现它们主要在短期记忆帧选择上不同，但共享对象中心表示。然后在SAM3框架中忠实重新实现这些记忆机制，并进行大规模评估。基于实证发现，提出统一的混合记忆框架，将记忆明确分解为短期外观记忆和长期干扰解决记忆。

Result: 在十个多样化基准上进行大规模评估，提出的框架在SAM2和SAM3骨干网络上均能一致提升鲁棒性，特别是在长期遮挡、复杂运动和干扰密集场景下表现更优。

Conclusion: 记忆机制的系统性研究揭示了基于SAM跟踪的关键设计原则，提出的统一混合记忆框架为未来跟踪系统提供了模块化和原则性的记忆设计方法，代码已开源。
...
TL;DR: Envision是一个基于扩散的视觉规划框架，通过两阶段方法生成目标对齐的轨迹：首先合成目标图像，然后插值生成物理合理的视频轨迹，用于具身智能体的操作任务。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型主要进行前向预测，仅基于初始观察生成轨迹，缺乏明确的目标建模，导致空间漂移和目标不对齐问题。需要一种能够生成物理合理且目标一致的视觉规划方法。
...
Conclusion: Envision通过显式约束生成过程，解决了现有视觉规划方法中的目标不对齐问题，为具身智能体提供了可靠的视觉规划指导。
...
<details>
  <summary>Details</summary>
Motivation: 传统IQA模型通常只输出全局质量分数，对局部和细粒度失真异常不敏感，导致ISR模型可能产生感知上不理想的伪影却获得虚假高分，造成奖励黑客问题，使优化目标与感知质量不一致。

Method: 1) 提出基于编码器-解码器架构的FinPercep-RM细粒度感知奖励模型，不仅提供全局质量分数，还生成感知退化图来空间定位和量化局部缺陷；2) 构建FGR-30k数据集训练该模型；3) 提出CCL协同进化课程学习机制，奖励模型和ISR模型同步进行课程学习，从简单全局奖励逐步过渡到复杂模型输出。

Result: 实验验证了该方法在ISR模型上的有效性，在RLHF方法中同时提升了全局质量和局部真实感，解决了训练不稳定性问题并抑制了奖励黑客现象。

Conclusion: 通过细粒度感知奖励模型和协同进化课程学习的结合，有效解决了图像超分辨率任务中传统IQA模型的局限性，实现了更稳定、更符合人类感知的强化学习优化。
...
<details>
  <summary>Details</summary>
Motivation: 为深度估计提供扩散模型之外的替代方法，探索自回归先验在几何感知生成模型中的潜力，强调数据可扩展性和对3D视觉任务的适应性

Method: 采用大规模文本到图像VAR模型，引入尺度条件上采样机制和分类器自由引导，通过10个固定自回归阶段进行推理，仅需74K合成样本微调

Result: 在室内基准测试中取得最先进性能（训练条件受限），在室外数据集上表现强劲，自回归先验成为深度估计中几何感知生成模型的补充家族

Conclusion: 自回归先验是深度估计中几何感知生成模型的可行替代方案，具有数据可扩展性和3D视觉任务适应性优势，代码已开源
...
TL;DR: 该研究评估了多种深度学习架构（3D Inception、双流、CNN-RNN）用于从超声心动图视频自动估计左心室射血分数（LVEF），发现改进的3D Inception架构在EchoNet-Dynamic数据集上表现最佳（RMSE 6.79%）。


<details>
  <summary>Details</summary>
Motivation: 超声心动图是评估LVEF的常用非侵入性方法，但人工评估耗时且存在观察者间差异。深度学习有望提供与人类专家相当的性能，实现自动化、标准化的心脏功能评估。
...
Conclusion: 深度学习架构在超声心动图LVEF估计中表现良好，改进的3D Inception架构达到最佳性能。研究发现的架构设计和训练策略见解可广泛应用于其他医学和非医学视频分析任务。
...
<details>
  <summary>Details</summary>
Motivation: 虽然计算机视觉领域受益于大规模数据集和预训练模型，但许多专业科学领域和资源受限的下游任务仍然面临数据稀缺的挑战，需要有效的方法来迁移预训练知识。

Method: 提出Cluster Attention Adapter (CLAdapter)，通过注意力机制和聚类中心，利用分布相关性和变换矩阵来个性化增强转换特征，使模型能够学习针对不同特征集的特定表示。

Result: 在10个涵盖通用、多媒体、生物、医学、工业、农业、环境、地理、材料科学、OOD和3D分析等领域的数据集上，CLAdapter在数据有限的科学领域实现了最先进的性能。

Conclusion: CLAdapter通过自适应迁移有效释放了基础视觉模型的潜力，其统一接口设计支持与CNN和Transformer等多种架构在2D和3D场景中的无缝集成。
...
Result: 在486个专家标注的结膜活检切片数据集上，INTERACT-CMIL相比CNN和基础模型基线取得一致改进，相对宏观F1分数提升最高达55.1%（WHO4）和25.0%（垂直扩散）。

Conclusion: 该框架提供与专家分级一致的可解释多标准预测，为CMIL诊断提供了可重复的计算基准，向标准化数字眼科病理学迈进一步。
...
TL;DR: CritiFusion：一种无需训练的多模态语义批判与频域融合框架，通过语义反馈和频谱对齐提升文本到图像生成的语义一致性和细节质量
...
Method: 1. CritiCore模块：结合视觉语言模型和多个大语言模型，丰富提示上下文并产生高层语义反馈，指导扩散过程；2. SpecFusion：在频谱域融合中间生成状态，注入粗粒度结构信息同时保留高频细节；无需额外模型训练，可作为插件兼容现有扩散模型

Result: 在标准基准测试中显著提升文本到图像对应性的人类对齐指标和视觉质量，在人类偏好分数和美学评估上持续提升性能，达到与最先进奖励优化方法相当的结果

Conclusion: CritiFusion通过语义批判和频谱对齐策略有效提升了生成图像的细节、真实性和提示保真度，证明了该框架在提升文本到图像生成语义一致性方面的有效性
...
TL;DR: 提出ARFM方法用于序列连续数据的概率建模，在多样化视频数据集上训练以生成长时程的未来点轨迹位置，并开发了评估运动预测模型能力的基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有运动预测模型通常在狭窄分布上训练，而大规模视频预测模型虽然视觉真实感强但难以准确建模复杂运动。受视频生成规模扩展的启发，需要开发能够准确预测复杂运动的新方法。

Method: 开发了自回归流匹配（ARFM）方法，这是一种用于序列连续数据概率建模的新技术。在多样化视频数据集上训练模型，以生成未来点轨迹位置，支持长时程预测。

Result: 模型能够预测复杂运动，实验表明将机器人动作预测和人体运动预测建立在预测的未来轨迹上，可以显著提升下游任务性能。

Conclusion: ARFM方法在运动预测方面表现出色，通过条件化预测能够有效提升下游任务性能，代码和模型已公开。
...
<details>
  <summary>Details</summary>
Motivation: 传统非刚性配准方法存在精度、计算复杂度和正则化之间的权衡，且通常假设图像对在解剖同源区域具有强度相关性，限制了其在多模态场景的应用。学习型模型需要大量训练数据且对未见模态性能下降。

Method: 提出基于神经ODE的实例特定框架，利用连续深度网络结合结构描述符（模态无关的度量模型，利用参数化邻域几何的自相似性）。提出三种变体：整合基于图像或特征的结构描述符，以及由局部互信息计算的非结构图像相似性。

Result: 在多种扫描数据集组合的实验中进行广泛评估，相比适用于大/小变形和多模态配准的SOTA基线，获得超越的定性和定量结果。方法对显式正则化水平具有鲁棒性，保持低误差，适合不同尺度的配准，且在大变形配准中效率更高。

Conclusion: 提出的神经ODE多模态微分同胚配准框架解决了传统方法的局限性，无需大量训练数据，对未见模态保持性能，在精度、鲁棒性和效率方面表现优异。
...
TL;DR: SCPainter统一框架结合3D高斯泼溅资产表示与扩散模型，实现自动驾驶场景中3D资产插入和新视角合成的联合处理。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶模拟需要多样化的训练数据，但现有方法将3D资产插入和新视角合成分开处理，缺乏统一的框架来创建交互式、真实的驾驶场景。

Method: 提出SCPainter框架：集成3D高斯泼溅（GS）汽车资产表示和3D场景点云，通过扩散模型生成高质量图像。将3D GS资产和场景点云投影到新视角，用这些投影条件化扩散模型。

Result: 在Waymo Open Dataset上的评估表明，该框架能够实现3D资产插入和新视角合成，促进创建多样化和真实的驾驶数据。

Conclusion: SCPainter提供了一个统一的框架，能够联合处理3D资产插入和新视角合成，为自动驾驶模拟生成更丰富、更真实的训练数据。
...
<details>
  <summary>Details</summary>
Motivation: 囊性水囊瘤是高风险产前超声发现，与染色体异常、结构畸形和不良妊娠结局相关。传统监督深度学习受限于小规模标注数据集，需要更有效的自动化检测方法以提高可重复性和支持大规模早期筛查。

Method: 使用基于掩码自编码(MAE)的超声自监督基础模型(USF-MAE)，该模型在超过37万张未标注超声图像上预训练，然后在囊性水囊瘤病例和正常对照的二元分类任务上进行微调。采用与DenseNet-169基线相同的预处理流程和4折交叉验证协议进行评估。
...
Conclusion: 超声特异性自监督预训练能够实现准确、稳健的囊性水囊瘤深度学习检测，为大规模早期筛查项目提供了有前景的技术支持。
...
TL;DR: 提出Freetime FeatureGS方法，通过可学习特征的Gaussian primitives和流式特征学习策略，从单帧分割图实现分解的4D场景重建，无需视频分割
...
Method: 使用Freetime FeatureGS建模动态场景为具有可学习特征和线性运动能力的Gaussian primitives集合，应用对比损失使特征根据2D分割图中的实例关系接近或远离，通过流式训练策略在时间维度传播特征

Result: 在多个数据集上的实验结果显示，该方法的重建质量大幅优于现有方法

Conclusion: 提出的方法成功实现了不依赖视频分割的4D场景分解重建，通过Gaussian primitives的时空特征学习和流式训练策略，显著提升了重建质量
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在处理视觉输入时，将图像编码为大量token并与文本token拼接处理，导致推理成本高昂。现有视觉token剪枝方法往往忽视长上下文多图像场景的挑战。
...
Conclusion: 该方法有效解决了长上下文多图像场景中的视觉token剪枝挑战，通过自适应预算分配和多样性平衡，在降低推理成本的同时保持了模型性能。
...
Method: 提出基于希尔伯特曲线重排序的邻居感知token缩减方法，将2D空间中的邻居结构用1D序列表示来显式保留。包含两个关键策略：邻居感知剪枝（NAP）用于选择性保留token，相邻token相似性合并（MAT）用于局部token聚合
...
TL;DR: 提出一种基于Fisher信息的主动学习算法，用于从多相机设置中选择信息量最大的帧，以同时优化语义理解和动态场景建模。


<details>
  <summary>Details</summary>
Motivation: 语义理解和动态场景建模对于具身智能体至关重要，但这些任务相比静态场景理解具有更高的数据冗余。需要一种原则性的方法来优先选择对模型训练最有信息增益的帧，而不是依赖启发式或随机策略。

Method: 将视角选择问题形式化为主动学习问题，提出基于Fisher信息的主动学习算法，量化候选视角对语义高斯参数和变形网络的信息量，从而联合处理语义推理和动态场景建模。

Result: 在大规模静态图像和动态视频数据集上评估，从多相机设置中选择信息帧。实验结果表明，该方法一致地提高了渲染质量和语义分割性能，优于基于随机选择和不确定性启发式的基线方法。

Conclusion: 提出的基于Fisher信息的主动学习算法为视角选择提供了原则性方法，能够有效处理语义理解和动态场景建模任务，在渲染质量和语义分割性能上优于现有方法。
...
TL;DR: 提出基于心理测量学改进的分级响应模型(GRM)用于AGIQA，通过算术方式建模难度级别，解决文本-图像语义漂移问题，提升质量评估性能。


<details>
  <summary>Details</summary>
Motivation: 现有AGIQA模型通过测量图像嵌入与多级质量描述文本嵌入的相似性来评估质量，但发现相似性分布常呈现多模态模式，即图像嵌入可能同时与"优秀"和"差"等级描述高度相似，而与"良好"等级偏离。这种文本嵌入与其预期描述之间的语义不一致性（称为"语义漂移"）削弱了文本-图像共享空间学习的可靠性。
...
Conclusion: 提出的算术GRM质量分级模块通过解决语义漂移问题，提供了一种更可靠、可解释的图像质量评估方法，在多种AGIQA框架中表现出优异的性能和泛化能力。
...
TL;DR: EPD-Solver：一种新颖的ODE求解器，通过并行梯度评估减少截断误差，实现扩散模型的高质量快速采样


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然生成质量优秀，但顺序去噪导致采样延迟高。现有加速方法在低延迟预算下图像质量显著下降，主要原因是无法捕捉高曲率轨迹段导致的累积截断误差。

Method: 提出EPD-Solver：1）基于几何洞察（采样轨迹主要位于低维流形），利用向量值函数的均值定理更准确逼近积分解；2）引入两阶段优化框架：先通过蒸馏方法优化可学习参数，再提出参数高效的RL微调方案，将求解器重构为随机Dirichlet策略；3）可作为插件（EPD-Plugin）改进现有ODE采样器。

Result: EPD-Solver通过并行梯度评估有效减少截断误差，同时保持低延迟采样特性。RL微调在复杂文本到图像生成任务中有效缓解奖励攻击并提升性能。

Conclusion: EPD-Solver是一种高效、灵活的ODE求解器，通过并行梯度评估和两阶段优化显著提升扩散模型采样速度和质量，可作为插件兼容现有采样器。
...
TL;DR: VPTracker：首个基于多模态大语言模型的全局视觉语言跟踪框架，通过位置感知视觉提示机制结合空间先验，在保持全局搜索优势的同时有效抑制干扰


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言跟踪方法通常局限于局部搜索，在视角变化、遮挡和目标快速移动等挑战场景下容易失败，需要更鲁棒的全局跟踪框架
...
Conclusion: VPTracker通过结合多模态大语言模型的语义推理能力和空间先验，实现了更鲁棒的全局视觉语言跟踪，有效解决了现有方法的局限性
...
Result: 在超声(US)和磁共振(MRI)等多模态医学数据集上的实验表明，该方法能在稀疏数据条件下生成高质量、解剖结构连贯、语义稳定的医学图像，同时显著提升重建效率
...
TL;DR: 评估开放词汇目标检测模型在低质量图像下的性能，发现高级图像退化会显著降低所有模型性能，OWLv2表现相对稳定
...
Method: 创建模拟真实世界低质量图像的新数据集，对多种开放词汇目标检测模型（OWLv2、OWL-ViT、GroundingDINO、Detic等）在不同级别图像退化条件下进行性能评估

Result: 低级别图像退化下mAP分数无明显下降，但高级别图像退化导致所有模型性能急剧下降；OWLv2在不同类型退化中表现相对稳定，其他模型性能显著下降

Conclusion: 开放词汇目标检测模型对高质量图像退化具有鲁棒性，但对高级别退化敏感；OWLv2表现最佳；将发布数据集和代码以促进未来研究
...
TL;DR: EgoReAct：首个自回归框架，从第一人称视角视频流实时生成3D对齐的人类反应动作，通过HRD数据集解决现有数据空间不一致问题，使用VQ-VAE和GPT实现高效生成。


<details>
  <summary>Details</summary>
Motivation: 人类对第一人称视觉输入具有自适应、上下文敏感的响应，但现有方法难以忠实建模这种反应，因为需要同时满足严格因果生成和精确3D空间对齐的双重要求。现有数据集（如ViMo）存在第一人称视频与反应动作之间的显著空间不一致问题。

Method: 首先构建Human Reaction Dataset (HRD)解决数据稀缺和错位问题；然后提出EgoReAct自回归框架：1) 使用VQ-VAE将反应动作压缩到紧凑表达空间；2) 训练GPT从视觉输入生成反应；3) 在生成过程中融入3D动态特征（度量深度和头部动态）增强空间基础。

Result: 实验表明EgoReAct相比先前方法在真实性、空间一致性和生成效率方面显著提升，同时保持生成过程中的严格因果性。能够实时生成3D对齐的人类反应动作。

Conclusion: EgoReAct是首个能够从第一人称视频流实时生成3D对齐人类反应动作的自回归框架，通过HRD数据集和3D动态特征集成有效解决了空间对齐和因果生成问题，为第一人称视觉反应建模提供了新解决方案。
...
TL;DR: DA360将Depth Anything V2适配到全景深度估计，通过ViT学习偏移参数实现尺度不变输出，结合环形填充消除接缝，在室内外数据集上显著提升零样本泛化性能。
...
Method: 提出DA360，即全景适配的Depth Anything V2。核心创新包括：1）从ViT骨干网络学习偏移参数，将模型的尺度-偏移不变输出转换为尺度不变估计，直接生成良好的3D点云；2）在DPT解码器中集成环形填充，消除接缝伪影，确保空间连贯的深度图并保持球形连续性。

Result: 在标准室内基准和新构建的室外数据集Metropolis上评估，DA360相比基础模型在室内外基准上分别实现超过50%和10%的相对深度误差降低。相比现有全景深度估计方法，DA360在所有三个测试数据集上相比PanDA实现约30%的相对误差改进，为零样本全景深度估计建立了新的最先进性能。

Conclusion: DA360通过将透视域深度估计能力迁移到全景域，显著提升了全景深度估计的零样本泛化性能，为机器人和AR/VR应用提供了更全面的环境结构信息捕捉方案。
...
TL;DR: 提出基于Kolmogorov-Arnold定理的KANO算子，用于图像超分辨率的可解释建模，通过B样条函数逼近光谱曲线，实现透明可控的退化过程表征。


<details>
  <summary>Details</summary>
Motivation: 现有可解释超分辨率方法依赖黑盒深度网络建模潜在变量，导致退化过程不透明且不可控。为解决单图像超分辨率中高度非线性退化、复杂物理交互和多源不确定性的挑战，需要更透明可解释的方法。

Method: 基于Kolmogorov-Arnold定理提出Kolmogorov-Arnold Neural Operator (KANO)，采用有限个B样条函数的加性结构分段逼近连续光谱曲线。通过学习优化样条函数在定义区间内的形状参数，捕捉局部线性趋势和非线性拐点处的峰谷结构。

Result: KANO能够准确捕捉关键光谱特征，赋予超分辨率结果物理可解释性。通过理论建模和在自然图像、航拍照片、卫星遥感数据上的实验评估，系统比较了MLP和KAN在处理复杂序列拟合任务中的表现。

Conclusion: 该比较研究阐明了这些模型在表征复杂退化机制方面的各自优势和局限性，为可解释超分辨率技术的发展提供了有价值的见解。KANO为图像超分辨率提供了透明结构化的潜在退化拟合过程表示。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D变化检测方法存在空间不一致问题，且无法明确分离变化前后的状态，需要更鲁棒的物体级变化检测方法

Method: 1) 基于有符号距离的2D差分模块；2) 多视图聚合（投票和剪枝）；3) 利用3DGS一致性分离变化前后状态；4) 持续场景重建策略选择性更新动态区域

Result: 方法在精度和效率上都优于现有方法，并贡献了CCS3D合成数据集支持可控评估

Conclusion: SCaR-3D能够有效检测物体级变化并分离变化前后状态，支持持续场景重建，在3D变化检测任务中表现优异
...
TL;DR: 提出一种基于两个仿射对应和已知垂直方向（来自IMU）来估计相对姿态（3自由度）和焦距的新求解器，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 在自动驾驶、智能手机、无人机等应用中，相机常与IMU结合使用。IMU可提供相机垂直方向，将相对姿态从5自由度降至3自由度。现有方法在已知垂直方向下利用两个仿射对应估计相对姿态和焦距的问题有待改进。

Method: 首先基于已知垂直方向和两个仿射对应建立约束方程；然后利用方程组非平凡解的性质推导出仅包含焦距和相对旋转角的四个方程；最后采用多项式特征值方法求解焦距和相对旋转角。

Result: 在合成和真实数据集上的评估表明，所提出的求解器性能优于现有的最先进求解器。

Conclusion: 该方法有效利用IMU提供的垂直方向信息，仅需两个仿射对应即可估计3自由度相对姿态和焦距，为相机-IMU组合系统提供了一种高效准确的相对位姿估计方案。
...
Method: 提出了ByteLoom框架，采用扩散Transformer架构，使用简化的人体条件和3D对象输入。核心创新包括：1) RCM-cache机制，利用相对坐标图作为通用表示来保持对象几何一致性并精确控制6自由度对象变换；2) 渐进式训练课程，利用现有数据集增强模型能力并减少对手部网格的需求。

Result: 实验表明，该方法能够忠实保持人体身份和对象的多视角几何特性，同时维持平滑的运动和对象操作，在生成质量上优于现有方法。
...
TL;DR: MUSON是一个用于短时程社交导航的多模态数据集，通过五步思维链标注提供结构化推理监督，解决了现有数据集缺乏推理监督和动作分布长尾的问题。


<details>
  <summary>Details</summary>
Motivation: 现有社交导航数据集缺乏明确的推理监督，且动作分布高度长尾，限制了模型学习安全关键行为的能力。

Method: 创建MUSON多模态数据集，采用五步思维链标注（感知、预测、推理、动作、解释），明确建模静态物理约束和合理平衡的离散动作空间。

Result: 在MUSON上评估多个SOTA小视觉语言模型，Qwen2.5-VL-3B达到最高决策准确率0.8625，证明MUSON作为社交合规导航基准的有效性。

Conclusion: MUSON数据集通过结构化推理监督和平衡的动作空间，为社交合规导航提供了有效的可重用基准，有助于模型学习安全关键行为。
...
<details>
  <summary>Details</summary>
Motivation: 医学影像的关键基础是人体解剖结构，但现有自监督学习方法往往忽视解剖学视角，限制了学习解剖特征的能力
...
TL;DR: 提出NSF去偏方法，通过识别、中和、消除和更新四个步骤缓解深度学习中的虚假相关性，在图像和NLP基准上比ERM提升20%以上最差组准确率。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型经常学习与类别标签虚假相关但与预测任务无关的特征。现有方法通常需要标注潜在虚假属性或基于经验假设（如偏见的简单性）过滤虚假特征，但由于现实数据中虚假相关性的复杂性和隐蔽性，这些方法可能效果不佳。

Method: 提出数据导向的去偏方法NSF：1）识别：观察到受虚假特征影响的样本在特征空间中呈现分散分布，据此识别虚假特征；2）中和：基于简单分组策略获得偏置不变表示；3）消除：学习特征变换以对齐偏置不变表示来消除虚假特征；4）更新：结合学习的特征变换更新分类器得到无偏模型。

Result: 在图像和NLP去偏基准测试中，相比标准经验风险最小化（ERM），最差组准确率提升超过20%。

Conclusion: 提出的NSF方法通过识别、中和、消除和更新的集成管道有效缓解深度学习中的虚假相关性，无需标注虚假属性，在多个基准上显著提升模型鲁棒性。
...
TL;DR: 本文提出了M-ErasureBench多模态评估框架，用于系统评估概念擦除方法在文本提示、学习嵌入和反演潜变量三种输入模态下的性能，并提出了IRECE插件模块来增强推理时的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法主要针对文本提示，忽略了图像编辑和个性化生成等实际应用中日益重要的其他输入模态（如学习嵌入和反演潜变量），这些模态可能成为攻击面，导致已擦除的概念重新出现。

Method: 1) 提出M-ErasureBench多模态评估框架，在三种输入模态（文本提示、学习嵌入、反演潜变量）和两种访问设置（白盒、黑盒）下进行系统评估；2) 提出IRECE（推理时概念擦除鲁棒性增强）插件模块，通过交叉注意力定位目标概念并在去噪过程中扰动相关潜变量。

Result: 现有方法在文本提示下表现良好，但在学习嵌入和反演潜变量下失败严重（白盒设置下概念重现率超过90%）。IRECE能显著恢复鲁棒性，在最具挑战性的白盒潜变量反演场景下将概念重现率降低达40%，同时保持视觉质量。

Conclusion: M-ErasureBench是首个超越文本提示的全面概念擦除基准，结合IRECE为构建更可靠保护性生成模型提供了实用保障，揭示了多模态输入下的安全漏洞并提出了有效解决方案。
...
Method: 采用基于Transformer的视觉编码器提取体素特征，通过紧凑的文本编码器和高效融合机制将视觉特征与语言表示对齐，实现自然语言提示与医学体积空间结构的语义对应。

Result: 在BTCV数据集上，SwinTF3D在多个器官上取得了有竞争力的Dice和IoU分数，对未见数据泛化能力强，相比传统Transformer分割网络显著提升了计算效率。
...
TL;DR: 论文提出GPS方法解决CFG在迭代细化中导致采样路径偏离数据流形的问题，通过流形约束插值确保稳定性，在SDXL等模型上实现质量和语义对齐的提升。


<details>
  <summary>Details</summary>
Motivation: 基于去噪-反转循环的迭代细化方法能提升扩散模型质量和控制，但结合标准CFG时效果受限。研究发现CFG的外推性质会系统性推动采样路径偏离数据流形，导致近似误差发散，破坏细化过程。

Method: 提出GPS新范式，用原则性的流形约束插值替代不稳定的外推，确保采样路径保持在数据流形上。理论证明该方法能将误差从无界放大转变为严格有界。还设计了最优调度策略，动态调整引导强度，使语义注入与模型自然从粗到细的生成过程对齐。
...
Conclusion: 路径稳定性是有效迭代细化的前提条件，GPS为实现这一目标提供了稳健框架。该方法通过流形约束插值解决了CFG导致的不稳定性问题，显著提升了生成质量和语义控制能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前哈希网格在用于高斯溅射的隐式神经场学习时，由于高斯溅射在3D空间中的不规则和非均匀分布，存在大量稀疏区域，导致哈希网格中许多特征无效，造成冗余存储和传输开销。

Method: 提出哈希网格特征剪枝方法，基于输入高斯溅射的坐标识别并剪除无效特征，只对有效特征进行编码，从而减少哈希网格的存储大小。

Result: 该方法在不影响模型性能的前提下，相比基线方法平均实现了8%的比特率降低，提升了率失真性能。

Conclusion: 通过剪除哈希网格中的无效特征，可以有效减少存储和传输开销，提升高斯溅射编码的效率，为相关应用提供更优的解决方案。
...
TL;DR: JavisGPT是首个用于联合音视频理解与生成的多模态大语言模型，采用编码器-LLM-解码器架构，通过三阶段训练和高质量数据集实现卓越性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型主要关注视觉-语言任务，缺乏对音视频联合理解与生成的支持，特别是在时间同步性要求高的复杂场景中。

Method: 采用编码器-LLM-解码器架构，包含SyncFusion模块进行时空音视频融合，使用同步感知可学习查询连接预训练的JAV-DiT生成器。通过三阶段训练流程：多模态预训练、音视频微调和大规模指令调优。

Result: 在音视频理解和生成基准测试中表现优异，特别是在复杂和时间同步设置中超越现有MLLMs。构建了包含20万+GPT-4o策划对话的JavisInst-Omni数据集。

Conclusion: JavisGPT成功实现了音视频联合理解与生成，为多模态AI系统提供了统一框架，在时间同步性要求高的复杂场景中展现出显著优势。
...
Conclusion: ColaVLA成功地将VLM的推理能力转移到统一潜在空间，结合分层并行解码器，在保持VLM泛化能力和可解释性的同时，实现了高效、准确和安全的轨迹生成，为自动驾驶规划提供了新的解决方案。
...
TL;DR: DRMNet利用密度图作为空间先验，通过密度生成分支、密集区域聚焦模块和双滤波器融合模块，提升高密度小目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 高分辨率遥感影像中密集小目标检测面临严重遮挡和像素有限的问题，现有方法均匀分配计算资源，无法自适应聚焦密集区域，限制了特征学习效果。

Method: 提出DRMNet：1) 密度生成分支建模目标分布模式；2) 密集区域聚焦模块利用密度图识别密集区域，实现高效局部-全局特征交互；3) 双滤波器融合模块通过离散余弦变换将多尺度特征解耦为高低频分量，进行密度引导的交叉注意力增强互补性。

Result: 在AI-TOD和DTOD数据集上的实验表明，DRMNet超越了现有最优方法，特别是在高目标密度和严重遮挡的复杂场景中表现优异。

Conclusion: DRMNet通过密度图引导的自适应特征学习，有效解决了遥感影像中密集小目标检测的挑战，为高密度目标检测提供了新思路。
...
TL;DR: CLIP-Joint-Detect：一种简单且与检测器无关的框架，通过端到端联合训练集成CLIP风格的对比视觉-语言监督，显著提升目标检测性能
...
Conclusion: 通过可学习文本嵌入的联合优化显著增强了不同架构和数据集上的闭集检测性能，该方法可无缝应用于两阶段和一阶段检测器
...
TL;DR: WRCFormer：一种新颖的3D目标检测框架，通过多视角表示融合原始4D雷达立方体与相机数据，利用小波注意力模块增强稀疏雷达信号表示，采用几何引导渐进融合机制，在恶劣天气条件下实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 4D毫米波雷达在自动驾驶中广泛应用，但其固有的稀疏性和语义信息有限限制了感知能力。虽然融合相机数据是有效的解决方案，但基于点云的雷达存在信息损失问题，而直接使用原始雷达数据计算成本过高。

Method: 提出WRCFormer框架：1）通过解耦雷达立方体的多视角表示融合原始雷达立方体与相机输入；2）设计小波注意力模块作为基于小波的特征金字塔网络基础模块，增强稀疏雷达信号和图像数据表示；3）引入两阶段查询式、模态无关的几何引导渐进融合机制，高效整合多模态多视角特征。

Result: 在K-Radar基准测试中实现最先进性能：在所有场景下超越最佳模型约2.4%，在雨雪场景下超越1.6%，展示了在恶劣天气条件下的鲁棒性。

Conclusion: WRCFormer通过有效融合原始雷达立方体与相机数据，解决了4D雷达感知中的稀疏性和信息损失问题，在保持计算效率的同时显著提升了3D目标检测性能，特别是在恶劣天气条件下表现出色。
...
TL;DR: YOLO-IOD：首个基于YOLO的实时增量目标检测框架，解决YOLO在增量学习中的灾难性遗忘问题


<details>
  <summary>Details</summary>
Motivation: 现有增量目标检测方法主要基于Faster R-CNN或DETR，无法支持实时YOLO检测框架。YOLO在增量学习中存在三种知识冲突导致灾难性遗忘

Method: 基于预训练YOLO-World构建，采用三阶段参数高效微调：1）冲突感知伪标签细化（CPR）缓解前景-背景混淆；2）基于重要性的核选择（IKS）识别更新关键卷积核；3）跨阶段非对称知识蒸馏（CAKD）解决知识蒸馏不对齐问题

Result: 在传统和新的LoCo COCO基准测试中，YOLO-IOD实现了优异性能且遗忘最小，同时提出了更现实的LoCo COCO基准消除阶段间数据泄漏
...
TL;DR: ReamCamo：基于外绘的统一框架，通过布局控制和多模态文本-视觉条件生成逼真的伪装图像，解决现有方法视觉相似性不足和语义不一致问题


<details>
  <summary>Details</summary>
Motivation: 现有伪装图像生成方法存在明显缺陷：要么因视觉相似性不足导致伪装效果差，要么背景杂乱且与前景目标语义不一致，与真实伪装图像存在显著差距
...
Result: 大量实验和可视化结果表明，所提框架在生成逼真伪装图像方面表现出色，有效解决了现有方法的局限性

Conclusion: ReamCamo通过布局控制和多模态条件生成机制，显著提升了伪装图像生成的视觉逼真度和语义一致性，为伪装目标检测提供了高质量的训练数据生成方案
...
TL;DR: PoseStreamer：针对高速移动场景的鲁棒多模态6DoF姿态估计框架，通过自适应姿态记忆队列、物体中心2D跟踪器和光线姿态滤波器提升性能，并在新数据集MoCapCube6D上验证有效性。


<details>
  <summary>Details</summary>
Motivation: 在高速和低光场景中，传统RGB相机因运动模糊导致6DoF姿态估计性能下降，而事件相机虽有高时间分辨率，但现有方法在高速物体移动场景中表现不佳。

Method: 提出PoseStreamer框架，包含三个核心组件：1）自适应姿态记忆队列，利用历史方向线索保持时间一致性；2）物体中心2D跟踪器，提供强2D先验以提升3D中心召回；3）光线姿态滤波器，沿相机光线进行几何细化。

Result: 在新建的多模态数据集MoCapCube6D上进行广泛实验，证明PoseStreamer在高速移动场景中不仅达到更高的精度，而且作为无模板框架对未见过的移动物体表现出强大的泛化能力。

Conclusion: PoseStreamer为高速移动场景下的6DoF姿态估计提供了一个有效的解决方案，通过多模态融合和时间一致性处理显著提升了性能，并为该领域提供了新的基准数据集。
...
TL;DR: 提出SSA框架解决医学图像分割中混合文本（位置、描述、诊断）处理问题，通过对称最优传输对齐和复合方向引导策略实现空间感知分割


<details>
  <summary>Details</summary>
Motivation: 当前文本引导医学图像分割方法存在两个关键瓶颈：1）难以同时处理诊断和描述文本，无法有效识别病灶并建立与图像区域的关联；2）现有方法关注病灶描述但忽略位置约束，导致关键偏差（如"左下肺"可能错误覆盖两侧）

Method: 提出空间感知对称对齐（SSA）框架：1）对称最优传输对齐机制，加强图像区域与多个相关表达之间的关联，建立双向细粒度多模态对应；2）复合方向引导策略，通过构建区域级引导掩码在文本中显式引入空间约束

Result: 在公开基准测试中，SSA实现了最先进的性能，特别是在准确分割具有空间关系约束特征的病灶方面表现优异

Conclusion: SSA框架有效解决了混合医学文本（位置、描述、诊断信息）的处理难题，通过对称对齐和空间约束显著提升了医学图像分割的准确性和可靠性
...
<details>
  <summary>Details</summary>
Motivation: 现有基于提示的身份特征移除方法要么依赖预训练模型对主体的良好表示，要么需要针对特定身份进行模型微调，缺乏对非训练数据中主体的泛化能力，且现有匿名化方法缺乏对面部属性的控制。

Method: 提出反向个性化框架，利用条件扩散反演直接操作图像，无需文本提示；引入身份引导条件分支以泛化到模型训练数据之外的主体；支持属性可控的匿名化。
...
TL;DR: 提出一个基于低成本RGB无人机的统一果园智能管道，集成ResNet50进行叶片病害检测、VGG16进行苹果新鲜度判断、YOLOv8进行实时苹果检测定位，实现完全离线的现场推理。


<details>
  <summary>Details</summary>
Motivation: 现有无人机系统通常孤立处理果园任务且依赖昂贵的多光谱传感器，需要一种低成本、统一的解决方案来同时处理病害检测、质量评估和产量估算。

Method: 使用RGB-only无人机采集图像，集成ResNet50进行叶片病害分类，VGG16进行苹果新鲜度分类，YOLOv8进行苹果检测和定位，系统运行在ESP32-CAM和树莓派上实现完全离线推理。

Result: 叶片病害分类准确率达98.9%，新鲜度分类准确率达97.4%，苹果检测F1分数为0.857，证明系统在低成本硬件上的有效性。
...
TL;DR: OpenGround：基于主动认知推理的零样本开放世界3D视觉定位框架，无需预定义物体查找表，通过动态更新认知范围实现开放类别定位


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉定位方法依赖预定义物体查找表（OLT）查询视觉语言模型，限制了在未定义或未知目标场景中的应用。需要解决开放世界场景下的3D视觉定位问题。

Method: 提出OpenGround框架，核心是主动认知推理（ACR）模块，通过认知任务链模拟人类感知过程，主动推理上下文相关物体，动态更新OLT扩展VLM认知范围，支持预定义和开放世界类别。

Result: 在Nr3D上达到竞争性性能，在ScanRefer上达到最先进水平，在提出的OpenTarget数据集上实现17.6%的显著提升。创建了包含7000+物体-描述对的OpenTarget数据集用于开放世界评估。

Conclusion: OpenGround通过主动认知推理有效解决了开放世界3D视觉定位问题，突破了预定义OLT的限制，为实际应用提供了更灵活的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 人类识别物体时能充分利用场景上下文信息（空间关系、材料属性、物体共现），而现有计算系统通常在孤立图像区域上操作，忽略了这些关键上下文信息，导致识别性能受限。

Method: 1) 从单目图像构建Geo-Semantic Contextual Graph (GSCG)：结合度量深度估计器和统一的全景与材料分割模型，将物体编码为节点（包含几何、色彩、材料属性），空间关系编码为边；2) 提出专门的基于图的分类器，聚合目标物体、邻近物体和全局场景特征进行分类预测。

Result: 在COCO 2017数据集上，上下文感知模型达到73.4%的分类准确率，远超无上下文版本（最低38.4%），也显著优于微调ResNet模型（最高53.5%）和最先进的多模态大语言模型Llama 4 Scout（最高42.3%）。

Conclusion: 显式结构化和可解释的上下文表示对于物体识别任务具有显著优势，GSCG框架通过充分利用场景上下文信息，大幅提升了物体分类性能，同时保持了推理过程的可解释性。
...
TL;DR: 该报告分析了两种现代视觉语言模型（Qwen2.5-VL-7B-Instruct和Llama-4-Scout-17B-16E-Instruct）的架构特性，并将其映射到视频到人工产物流水线的实际实现中，重点讨论了结构化输出验证和系统约束。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于理解现代视觉语言模型的架构特性如何影响实际视频处理系统的设计和实现，特别是结构化输出生成、验证机制以及系统约束对工程决策的影响。

Method: 采用架构导向的分析方法，首先总结共享的多模态基础（视觉标记化、Transformer注意力、指令跟随），然后详细描述每个模型的架构特性，最后将模型行为映射到系统约束，包括结构化输出验证、语义正确性评估和框架本地标识符处理。

Result: 分析揭示了结构化输出可能语法正确但语义错误，验证机制仅限于结构而非几何正确性，人物标识符在当前提示合约中是框架本地的，交互式单帧分析返回自由格式文本而非模式强制JSON。这些发现对系统设计至关重要。

Conclusion: 理解模型架构特性与实际系统约束之间的映射关系对于编写可靠声明、设计鲁棒接口和规划评估至关重要，特别是在处理结构化输出验证、语义正确性和系统交互模式方面。
...
Method: 提出异构双学生架构，包含基于CLIP和DINOv3预训练的ViT模型；引入显式-隐式语义协同引导机制，利用文本嵌入和可学习查询提供类级指导；开发全局-局部特征协作融合策略，融合CLIP的全局上下文信息和DINOv3的局部细节。
...
TL;DR: LAM3C：从无标签视频中学习3D表示的自监督框架，无需真实3D传感器数据，在室内语义和实例分割上超越现有自监督方法。


<details>
  <summary>Details</summary>
Motivation: 大规模3D场景扫描成本高且费时，研究是否可以从无真实3D传感器的无标签视频中学习3D表示。
...
TL;DR: Video-BrowseComp是首个针对开放网络视频研究的基准测试，包含210个需要主动浏览视频时间线来验证外部声明的问题，现有最先进模型准确率仅15.24%，揭示了视频推理中的关键瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前自主代理正在从被动检索转向主动的开放网络研究，但视频作为网络中最动态的模态存在显著差距。现有视频基准主要关注被动感知，无法评估需要主动浏览视频时间线、交叉引用分散证据和验证声明的代理视频研究能力。

Method: 提出了Video-BrowseComp基准，包含210个专门为开放网络代理视频推理设计的问题。该基准强制依赖时间视觉证据，确保答案不能仅通过文本搜索获得，而需要导航视频时间线来验证外部声明。

Result: 评估最先进模型显示关键瓶颈：即使是GPT-5.1（带搜索）等先进搜索增强模型也仅达到15.24%准确率。分析表明这些模型主要依赖文本代理，在元数据丰富的领域表现良好，但在元数据稀疏的动态环境中（如体育、游戏）表现崩溃。

Conclusion: Video-BrowseComp作为首个开放网络视频研究基准，将领域从被动感知推向主动视频推理，揭示了当前模型在需要视觉基础的动态视频环境中的严重局限性。
...
TL;DR: 提出基于MedSAM的肺部分割引导的胸部X光分类流程，通过肺部分割掩码作为空间先验来改善多标签异常分类，发现掩码效果与任务和架构相关。


<details>
  <summary>Details</summary>
Motivation: 胸部X光自动解读面临疾病信号弱、数据集偏差和空间监督有限等挑战。医学图像分割基础模型MedSAM提供了引入解剖学先验的机会，可能提高CXR分析的鲁棒性和可解释性。

Method: 提出分割引导的CXR分类流程：1) 使用Airlangga大学医院公开数据集微调MedSAM作为肺区域提取模块；2) 在NIH CXR数据集子集上训练和评估用于5种异常（肿块、结节、肺炎、水肿、纤维化）多标签预测的深度卷积神经网络；3) 比较原始图像、宽松肺掩码和紧密肺掩码三种条件下的分类性能。

Result: MedSAM在不同成像条件下产生解剖学合理的肺掩码。掩码效果具有任务和架构依赖性：ResNet50在原始图像上实现最佳异常判别；宽松肺掩码获得可比的宏观AUROC但显著改善正常病例筛查；紧密掩码降低异常级别性能但提高训练效率；宽松掩码通过保留肺门和周围上下文部分缓解性能下降。

Conclusion: 肺掩码应被视为可控制的空间先验，根据主干网络和临床目标进行选择，而不是统一应用。需要在异常特异性分类和正常病例筛查之间权衡。
...
<details>
  <summary>Details</summary>
Motivation: 当前生成模型通常在全局像素域操作或依赖二值掩码，存在特征纠缠问题，导致解剖结构损坏或结构不连续。需要一种能保持解剖完整性同时生成高质量病理变化的方法。

Method: 将合成任务分解为确定性解剖重建和随机偏差建模。核心是偏差空间扩散模型，学习病理残差的条件分布，结合接缝感知融合策略和推理时稳定模块，确保空间一致性。

Result: 在肿瘤成像基准测试中，PathoSyn在感知真实性和解剖保真度方面显著优于整体扩散和掩码条件基线方法，能生成高质量患者特异性合成数据集。

Conclusion: PathoSyn提供了一个数学原理驱动的管道，用于生成高保真合成数据，支持可解释的反事实疾病进展建模，有助于在低数据环境下开发稳健的诊断算法和临床决策支持系统。
...
TL;DR: 深度学习变形配准具有固有的跨域鲁棒性，源于其依赖局部特征而非全局外观，UniReg框架验证了这一机制


<details>
  <summary>Details</summary>
Motivation: 传统认为学习型配准模型对域偏移敏感，需要大量多样化训练数据，但缺乏对其内在机制的解释。本文旨在揭示深度学习变形配准模型是否具有固有的域偏移免疫性

Method: 提出UniReg通用配准框架，将特征提取与形变估计解耦：使用固定的预训练特征提取器和UNet形变网络。通过单数据集训练验证跨域鲁棒性

Result: UniReg在单数据集训练下展现出与优化方法相当的跨域和多模态配准性能。分析发现传统CNN模型在模态偏移下的失败源于早期卷积层的数据集偏差

Conclusion: 局部特征一致性是学习型变形配准鲁棒性的关键驱动因素，这为设计保持域不变局部特征的主干网络提供了理论依据
...
TL;DR: GeoTeacher：一种半监督3D目标检测方法，通过几何关系监督和体素级数据增强提升模型对物体几何结构的理解能力


<details>
  <summary>Details</summary>
Motivation: 现有半监督3D目标检测方法主要关注伪标签质量和特征一致性，但忽略了在有限标注数据下模型对物体几何结构的敏感性不足问题，这限制了模型对物体感知和定位的能力

Method: 1. 基于关键点的几何关系监督模块：将教师模型对物体几何的知识迁移给学生模型；2. 体素级数据增强策略：增加物体几何多样性，并引入距离衰减机制保护远距离物体的完整性；3. 可与不同SS3D方法结合

Result: 在ONCE和Waymo数据集上的大量实验表明方法有效且具有泛化能力，取得了新的最先进结果

Conclusion: GeoTeacher通过几何关系监督和数据增强有效提升了学生模型对物体几何结构的理解能力，特别是在有限标注数据情况下，能够显著改善半监督3D目标检测性能
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型评估方法大多依赖粗粒度指标或静态QA流程，缺乏细粒度可解释性且难以反映人类偏好，需要更精细的对齐度评估框架。

Method: 采用"定位-推理-结论"结构化范式，通过多模态大语言模型显式定位语义元素并得出可解释的对齐判断，使用GRPO优化复合奖励函数（包含结构格式、定位准确性和对齐保真度）。

Result: 在四个基准测试（EvalMuse-40K、RichHF、MHaluBench、GenAI-Bench）上达到最先进性能，优于强大的专有模型和监督基线，推理效率优于现有迭代视觉推理方法。

Conclusion: REVEALER提供了一个统一且可解释的框架，用于细粒度评估文本-图像对齐度，在准确性和效率方面均表现出色，为T2I模型评估提供了新方向。
...
TL;DR: GVSynergy-Det：一种基于图像的三维物体检测新框架，通过高斯-体素协同表示学习，无需深度传感器或密集三维监督，在室内基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于图像的3D物体检测方法面临两难：高精度方法需要密集3D监督，而无监督方法难以从图像中提取准确几何信息。需要一种无需深度传感器或密集3D监督的高性能解决方案。
...
Conclusion: 高斯和体素表示捕获互补几何信息，通过协同学习策略可以有效提升基于图像的3D检测性能，无需昂贵传感器或密集监督，为实际应用提供了可行方案。
...
TL;DR: 提出基于3D高斯场景表示的驾驶世界模型框架，实现3D场景理解和多模态场景生成，通过语言特征嵌入和任务感知采样提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶世界模型缺乏3D场景理解能力，只能基于输入数据生成内容，无法解释或推理驾驶环境。同时，当前方法使用点云或BEV特征表示3D空间信息，无法准确对齐文本信息与底层3D场景。

Method: 1) 基于3D高斯场景表示的统一框架；2) 将丰富语言特征嵌入每个高斯基元，实现早期模态对齐；3) 设计任务感知的语言引导采样策略，去除冗余3D高斯并注入准确紧凑的3D令牌到LLM；4) 设计双条件多模态生成模型，结合高层语言条件和低层图像条件共同指导生成过程。

Result: 在nuScenes和NuInteract数据集上进行全面研究，验证了框架的有效性。方法实现了最先进的性能。

Conclusion: 提出的基于3D高斯场景表示的驾驶世界模型框架能够同时实现3D场景理解和多模态场景生成，通过语言特征嵌入和任务感知采样策略有效解决了现有方法的局限性。
...
TL;DR: 提出ForCM方法，结合面向对象图像分析(OBIA)与深度学习(DL)，使用Sentinel-2影像进行森林覆盖制图，在亚马逊雨林区域验证，相比传统OBIA方法精度提升显著。


<details>
  <summary>Details</summary>
Motivation: 传统森林覆盖制图方法精度有限，需要探索结合深度学习与面向对象分析的新方法，利用免费卫星影像和开源工具提高制图精度，支持全球环境监测与保护。

Method: 提出ForCM方法：1) 使用Sentinel-2 Level 2A高分辨率卫星影像；2) 评估多种深度学习模型(UNet、UNet++、ResUNet、AttentionUNet、ResNet50-Segnet)；3) 将最佳DL模型与OBIA技术集成；4) 使用三组数据集(两组三波段，一组四波段)；5) 在亚马逊雨林区域验证。

Result: ForCM方法显著提升森林覆盖制图精度：ResUNet-OBIA达到94.54%总体精度，AttentionUNet-OBIA达到95.64%总体精度，相比传统OBIA方法的92.91%有明显提升。证明了QGIS等免费工具在制图应用中的潜力。

Conclusion: ForCM方法成功结合深度学习与面向对象分析，显著提高森林覆盖制图精度，为利用免费卫星影像和开源工具进行环境监测提供了有效方案，支持全球森林保护与监测工作。
...
TL;DR: 提出使用虚幻引擎生成RGB合成数据用于军事目标检测的跨域适应方法，通过合成到真实数据迁移实验评估现有域适应方法，发现使用图像最小提示的方法优于无监督或半监督方法。


<details>
  <summary>Details</summary>
Motivation: 军事目标检测在军民应用中至关重要，但现有域适应方法主要针对自然或自动驾驶场景，而军事领域环境复杂多变。SAR数据成本高且处理时间长，RGB相机成本低但缺乏军事目标检测数据集。

Method: 使用虚幻引擎生成RGB合成数据，通过合成到真实数据迁移实验，在合成数据集上训练并在网络收集的真实军事目标数据集上验证，评估不同监督程度的先进域适应方法。

Result: 使用图像最小提示（如目标类别）的方法相比无监督或半监督域适应方法有显著改进，但仍面临挑战需要克服。

Conclusion: 提出了一种利用虚幻引擎生成合成数据解决军事目标检测数据集缺乏问题的方法，通过跨域适应实验揭示了当前域适应方法的局限性，为未来研究指明了方向。
...
TL;DR: 提出TTS和TFC模块，通过自适应选择最优扩散时间步特征来提升少样本密集预测任务性能
...
Method: 提出两个模块：1) TTS基于时间步损失和相似度得分选择理想扩散时间步；2) TFC整合选定时间步特征以提升少样本密集预测性能。配合参数高效的微调适配器

Result: 在Taskonomy数据集上实证验证了方法的有效性，在少样本密集预测任务中实现了优越性能

Conclusion: 提出的可学习时间步整合方法能够有效提升扩散模型在少样本密集预测任务中的性能，适用于通用和少样本学习场景
...
TL;DR: 作者提出了AVOID数据集，这是一个在模拟环境中收集的用于实时障碍物检测的对抗性视觉条件数据集，包含各种天气和时间条件下的道路障碍物，并提供了语义分割、深度图、LiDAR数据和路径点等多模态标注。


<details>
  <summary>Details</summary>
Motivation: 现有道路驾驶数据集要么只包含正常场景，要么只包含对抗性场景，且往往缺乏与其他类别在同一视觉域中捕获的道路障碍物。为了可靠地实时检测各种恶劣条件下的小型道路危险，需要一个新的数据集。

Method: 在模拟环境中收集大型数据集，包含各种天气和时间条件下的意外道路障碍物。每个图像都配有相应的语义和深度图、原始和语义LiDAR数据以及路径点，支持大多数视觉感知任务。

Result: 提出了AVOID数据集，并在高性能实时网络上对障碍物检测任务进行了基准测试，同时提出并进行了使用综合多任务网络进行语义分割、深度和路径点预测任务的消融研究。

Conclusion: AVOID数据集填补了现有数据集的空白，为在对抗性视觉条件下进行实时障碍物检测提供了全面的多模态数据支持，有助于推动自动驾驶视觉感知技术的发展。
...
TL;DR: MM-UAVBench：首个针对低空无人机场景的多模态大语言模型综合评测基准，涵盖感知、认知、规划三大能力维度，包含19个子任务和5700+人工标注问题，揭示当前模型在低空场景的适应性不足问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在低空无人机应用领域的研究不足，现有基准缺乏对低空场景独特挑战的覆盖，而无人机相关评测主要关注特定任务如定位或导航，缺乏对MLLMs通用智能的统一评估。

Method: 构建MM-UAVBench基准，系统评估MLLMs在低空无人机场景下的三大核心能力维度：感知、认知和规划。基准包含19个子任务，超过5700个人工标注问题，所有数据均来自公开数据集的真实无人机采集数据。

Result: 对16个开源和专有MLLMs的广泛实验表明，当前模型难以适应低空场景的复杂视觉和认知需求。分析进一步揭示了空间偏差和多视角理解等关键瓶颈，这些因素阻碍了MLLMs在无人机场景中的有效部署。

Conclusion: MM-UAVBench填补了低空无人机场景下MLLMs评估的空白，揭示了当前模型的局限性，有望推动面向真实世界无人机智能的稳健可靠MLLMs的未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 时尚物品检测面临挑战，因为时尚物品外观高度多样化且子类别相似性高，导致检测歧义。传统检测器独立检测每个物品，无法利用时尚物品之间的上下文关系。

Method: 提出Holi-DETR，在DETR基础上整合三种上下文信息：1）时尚物品共现关系；2）基于物品间空间布局的相对位置和大小；3）物品与人体关键点的空间关系。通过整体检测减少歧义。

Result: 实验表明，该方法将原始DETR性能提升3.6个百分点，将Co-DETR性能提升1.1个百分点（基于平均精度AP指标）。

Conclusion: 通过整合多种上下文信息进行整体检测，能有效减少时尚物品检测中的歧义，显著提升检测性能。
...
TL;DR: UniMAGE是一个统一的导演模型，将剧本创作和关键镜头设计整合到单一框架中，使用户能通过提示生成结构化的剧本和关键帧图像。
...
Method: 采用Mixture-of-Transformers架构统一文本和图像生成；引入"先交错后解耦"训练范式：1）交错概念学习，使用交错文本图像数据培养对剧本的深度理解和想象力；2）解耦专家学习，将剧本写作与关键帧生成解耦，提升故事讲述的灵活性和创造力。

Result: UniMAGE在开源模型中达到最先进性能，能生成逻辑连贯的视频剧本和视觉一致的关键帧图像。

Conclusion: UniMAGE成功统一了剧本创作和关键镜头设计，为非专业人士创作长上下文、多镜头电影提供了有效工具，验证了统一导演模型的可行性。
...
Method: 1. 使用预训练文本引导图像到图像翻译模型生成合成缺陷图像；2. 利用图像检索模型评估生成图像与真实正常图像的相似性，过滤不相关输出；3. 提出两阶段训练策略：先在大量基于规则合成的图像上进行预训练，然后在少量高质量图像上进行微调。

Result: 在MVTec AD数据集上的实验证明了该方法的有效性，显著降低了数据收集成本，同时提高了异常检测性能。

Conclusion: 提出的框架通过结合预训练翻译模型和图像检索模型，能够高效生成高质量的合成缺陷图像，配合两阶段训练策略，在降低数据收集成本的同时有效提升了异常检测性能。
...
TL;DR: 提出SURE引导的后验采样(SGPS)方法，通过SURE梯度更新和PCA噪声估计校正扩散采样轨迹偏差，在少于100次神经网络评估下实现高质量逆问题重建


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的逆问题求解方法需要数百或数千步迭代才能获得高质量重建，存在误差累积问题，需要开发更高效的采样方法

Method: 使用Stein's Unbiased Risk Estimate (SURE)梯度更新和PCA噪声估计来校正采样轨迹偏差，减少噪声引起的误差，特别是在早期和中期采样阶段

Result: SGPS在少于100次神经网络评估(NFEs)下保持高质量重建，在多种逆问题上优于现有方法

Conclusion: SGPS通过校正采样轨迹偏差有效减少误差累积，显著提高了扩散模型在逆问题求解中的效率和性能
...
<details>
  <summary>Details</summary>
Motivation: 红外气体泄漏检测在环境监测和工业安全中至关重要，但由于气体羽流微弱、小尺寸、半透明且边界模糊，检测难度很大。

Method: 1) Gas Block扩散-对流单元建模气体传输：局部分支捕捉短程变化，大核分支捕捉长程传播；2) AGPEO自适应梯度和相位边缘算子从多方向梯度和相位一致响应计算可靠边缘先验；3) CASR-PAN内容自适应稀疏路由路径聚合网络基于边缘和内容线索选择性传播跨尺度特征。

Result: 在IIG数据集上，PEG-DRNet达到整体AP 29.8%、AP50 84.3%、小物体AP 25.3%，相比RT-DETR-R18基线分别提升3.0%、6.5%、5.3%，仅需43.7 Gflops和14.9 M参数。在IIG和LangGas数据集上超越现有CNN和Transformer检测器。

Conclusion: PEG-DRNet通过物理边缘混合设计实现了红外气体泄漏检测的优异性能，在精度和计算效率方面达到最佳平衡，为微弱气体羽流检测提供了有效解决方案。
...
TL;DR: 提出一种免训练的两阶段数据剪枝方法，通过信息熵和场景感知聚类，在高剪枝率下选择高质量子集，提升遥感扩散基础模型的训练效率和生成质量。
...
Result: 即使剪枝85%的训练数据，该方法仍能显著改善收敛和生成质量。基于该方法训练的扩散基础模型在下游任务（如超分辨率和语义图像合成）中持续达到最先进的性能。

Conclusion: 该数据剪枝范式为开发遥感生成基础模型提供了实用指导，通过选择高质量子集使基础模型能够快速收敛，并作为生成、下游微调和其他应用的多功能骨干。
...
Conclusion: 该技术框架为构建高效鲁棒的多模态遥感系统提供了新方法，为智能遥感解译的工程应用奠定了理论基础和技术指导
...
TL;DR: ViLaCD-R1是一个两阶段遥感变化检测框架，通过多图像推理器和掩码引导解码器，结合视觉语言模型和强化学习，显著提升语义变化识别和定位精度。


<details>
  <summary>Details</summary>
Motivation: 传统遥感变化检测方法（像素级操作或编码器-解码器网络）无法充分捕捉高层语义，且对非语义扰动敏感。现有的多模态和视觉语言模型方法虽然增强了语义理解，但仍存在空间定位不准确、像素级边界划分不精确和可解释性有限等问题。

Method: 提出两阶段框架ViLaCD-R1：1）多图像推理器（MIR）：通过监督微调和强化学习训练视觉语言模型，输入双时相图像块，输出粗略变化掩码；2）掩码引导解码器（MGD）：整合双时相图像特征和粗略掩码，预测精确的二进制变化图。

Result: 在多个遥感变化检测基准测试中，ViLaCD-R1显著提高了真实语义变化的识别和定位能力，有效抑制了非语义变化，在复杂真实场景中达到了最先进的准确率。

Conclusion: ViLaCD-R1通过结合视觉语言模型和强化学习的两阶段框架，成功解决了遥感变化检测中的语义理解、空间定位和边界精度问题，为复杂场景下的变化检测提供了有效解决方案。
...
TL;DR: ASemconsist是一个解决文本到图像生成中角色身份一致性问题的新框架，通过选择性文本嵌入修改、语义容器策略和自适应特征共享，在保持身份一致性的同时确保每张图像与文本提示的对齐。
...
Conclusion: ASemconsist通过创新的语义控制策略和自适应特征共享，成功解决了文本到图像生成中的角色身份一致性问题，同时保持了与文本提示的良好对齐。
...
Method: 提出轮廓信息感知的2D高斯泼溅框架，将物体分割先验融入高斯表示中。通过在光栅化过程中将每个高斯约束到特定的分割区域，防止跨边界混合，并引入预热方案来稳定训练和提高收敛性。

Result: 在合成色卡和DAVIS数据集上的实验表明，该方法在物体边缘区域实现了比现有2DGS方法更高的重建质量，特别是在高斯数量很少的情况下效果更明显，同时保持了快速渲染和低内存使用。

Conclusion: 通过融入分割先验来增强轮廓感知能力，可以显著提高2D高斯泼溅在压缩场景下的图像表示质量，特别是在保持边缘结构方面表现优异。
...
TL;DR: CEM是一种通过累积误差最小化来优化现有误差校正方法的插件，能够动态调整缓存策略以最小化生成误差，显著提升扩散变换器加速后的生成保真度。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器（DiT）的迭代去噪过程导致推理速度慢，现有基于缓存的加速方法存在较大计算误差，且固定的缓存策略无法适应去噪过程中复杂的误差变化，限制了误差校正的潜力。

Method: 提出CEM插件，通过预定义误差来表征模型对加速的敏感性（受时间步和缓存间隔共同影响），基于此先验设计动态规划算法进行累积误差近似优化，实现缓存误差最小化。

Result: 在9个生成模型和量化方法上的实验表明，CEM显著提升了现有加速模型的生成保真度，在FLUX.1-dev、PixArt-α、StableDiffusion1.5和Hunyuan等模型上甚至超越了原始生成性能。

Conclusion: CEM是一种模型无关、泛化性强的插件，可无缝集成到现有误差校正框架和量化模型中，无需额外计算开销，能显著提升扩散变换器加速后的生成质量。
...
Method: 提出YOLO-Master框架，引入实例条件自适应计算，通过高效稀疏专家混合（ES-MoE）块根据场景复杂度动态分配计算资源。使用轻量级动态路由网络引导专家专业化训练，通过多样性增强目标鼓励专家互补，推理时仅激活最相关专家。

Result: 在五个大规模基准测试中表现优异。在MS COCO上达到42.4% AP和1.62ms延迟，优于YOLOv13-N（+0.8% mAP，推理速度快17.8%）。在密集挑战性场景中提升最明显，同时保持典型输入效率和实时推理速度。

Conclusion: YOLO-Master通过实例条件自适应计算有效解决了现有实时目标检测方法的计算资源分配问题，在保持实时性的同时显著提升了检测性能，特别是在复杂场景中表现突出。
...
TL;DR: 本文提出了两个多模态框架，分别用于微手势识别和行为情感预测，在iMiGUE数据集上取得了良好性能，其中行为情感预测任务在MiGA 2025挑战赛中获得了第二名。


<details>
  <summary>Details</summary>
Motivation: 微手势识别和行为情感预测都是极具挑战性的任务，需要建模人类细微、细粒度的行为模式。这些任务主要利用视频和骨骼姿态数据，但现有方法在捕捉这些微妙时空模式方面存在局限。
...
Conclusion: 提出的多模态框架能够有效处理微手势识别和行为情感预测任务，通过融合不同模态的信息（视频、骨骼、面部）来捕捉人类行为的细微模式，在挑战性任务中取得了竞争性结果。
...
TL;DR: 该研究比较了医学影像诊断中两种AI架构：经过LoRA微调的MedGemma-4b-it模型在六种疾病诊断中表现优于GPT-4，平均测试准确率达到80.37%，特别在癌症和肺炎等高风险临床任务中敏感性更高。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型为医学影像分析提供了新范式，但不同AI架构在临床诊断中的性能差异需要系统评估。本研究旨在比较专用开源模型MedGemma与专有大模型GPT-4在疾病分类中的表现，为临床实施提供依据。

Method: 使用低秩适应（LoRA）对MedGemma-4b-it模型进行微调，与未经调优的GPT-4进行对比。评估六种不同疾病的诊断能力，通过混淆矩阵和分类报告进行定量分析，重点关注模型在癌症和肺炎等高风险临床任务中的敏感性。

Result: 微调后的MedGemma-4b-it平均测试准确率达到80.37%，显著高于GPT-4的69.58%。MedGemma在高风险临床任务（如癌症和肺炎检测）中表现出更高的敏感性。定量分析显示MedGemma在所有疾病类别中均有更优表现。

Conclusion: 领域特定的微调对于最小化临床实施中的幻觉至关重要。MedGemma通过精细调优展现出复杂的循证医学推理能力，成为医学影像诊断的先进工具，而通用大模型在未经领域适应的情况下表现较差。
...
<details>
  <summary>Details</summary>
Motivation: 传统CAD建模复杂，现有方法从草图重建的3D模型通常不可编辑且精度不足，依赖文本/图像输入需要大量人工标注，限制了工业设计的可扩展性和适用性

Method: 提出异构协同多专家强化学习（CME-CAD）范式，包含两阶段训练：多专家微调（MEFT）和多专家强化学习（MERL），整合不同模型的互补优势
...
TL;DR: 该论文从拓扑结构视角研究视觉表示学习，提出视觉语义空间具有纤维丛结构，语义对应商空间而非子流形，需要非连续映射和特定架构才能实现语义抽象。


<details>
  <summary>Details</summary>
Motivation: 从结构和拓扑角度理解视觉表示学习，基于一个核心假设：视觉理解需要语义语言，其中大量感知观测对应少量离散语义状态。结合表示学习中可迁移性和抽象性的普遍假设，探索视觉空间的拓扑结构。

Method: 采用拓扑和几何分析方法，将视觉观测空间建模为纤维丛结构：nuisance变化填充纤维，语义对应商基空间。从该结构推导出两个理论结果：1) 语义商空间不是子流形，需要非连续映射；2) 近似商空间对模型架构有结构要求。

Result: 理论推导表明：1) 语义不变性需要非同胚的判别性目标（如标签监督、跨实例识别或多模态对齐）；2) 语义抽象需要支持拓扑变化的表示机制（"扩展-捕捉"过程：先几何扩展分离结构，再坍缩形成离散语义区域）。

Conclusion: 该框架提供了拓扑视角，与大规模判别性和多模态模型的经验规律以及统计学习理论的经典原理相一致。这些结果是解释性的而非规定性的，为理解视觉表示学习提供了结构性洞察。
...
<details>
  <summary>Details</summary>
Motivation: 现有计数方法在指定目标对象方面存在限制：视觉示例需要手动标注，无法指定不计数对象，且只能使用自然图像示例。这些限制影响了计数方法的灵活性、准确性和用户友好性。

Method: 1. 扩展提示方式：支持用文本和/或视觉示例描述不计数对象；2. 引入伪示例概念，在推理时自动标注视觉示例；3. 扩展模型以接受自然和合成外部图像的视觉示例；4. 将CountGD++作为LLM的视觉专家代理。

Result: 在多个数据集上实现了准确性、效率和泛化能力的显著提升。代码已开源。

Conclusion: 通过扩展目标对象指定方式，CountGD++显著提升了多模态开放世界计数的提示灵活性，为更智能、更通用的计数系统奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖预构建的3D表示或现成重建流程，限制了可扩展性和实际应用。真实环境中的部分可见性、遮挡和低重叠条件等挑战未得到充分探索

Method: 提出可扩展的多视角数据生成和标注流程，构建SpatialMosaic数据集（200万QA对）和SpatialMosaic-Bench基准（100万QA对，6个任务）。提出SpatialMosaicVLM混合框架，将3D重建模型作为几何编码器集成到视觉语言模型中

Result: 实验表明提出的数据集和VQA任务能有效增强在挑战性多视角条件下的空间推理能力，验证了数据生成流程在构建真实多样QA对方面的有效性

Conclusion: 提出的SpatialMosaic数据集、基准和混合框架为解决真实世界多视角空间推理挑战提供了有效解决方案，代码和数据集即将发布
...
Method: 提出MGCA-Net网络，包含两个核心模块：1) 上下文几何注意力模块(CGA)，通过自适应注意力机制动态整合空间位置和特征信息，增强局部和全局几何关系捕捉能力；2) 跨阶段多图共识模块(CSMGC)，通过跨阶段稀疏图网络建立几何共识，确保不同阶段间几何信息的一致性。

Result: 在YFCC100M和SUN3D两个代表性数据集上的实验结果表明，MGCA-Net在异常点剔除和相机姿态估计任务上显著优于现有的SOTA方法。

Conclusion: MGCA-Net通过创新的上下文几何注意力和跨阶段多图共识机制，有效解决了现有方法在几何建模和跨阶段优化方面的局限，提升了对应关系学习的准确性和鲁棒性。
...
TL;DR: NeXT-IMDL是一个大规模诊断基准，通过四个基本轴（编辑模型、操作类型、内容语义、伪造粒度）和五个跨维度评估协议，系统性地揭示现有图像篡改检测模型的泛化脆弱性。


<details>
  <summary>Details</summary>
Motivation: 用户友好图像编辑模型的普及和滥用风险，迫切需要通用且最新的图像篡改检测与定位方法。当前研究采用跨数据集评估，但这种简化方法掩盖了现有方法处理多样化AI生成内容时的脆弱性，导致对进展的误导性印象。

Method: 提出NeXT-IMDL基准，从四个基本轴（编辑模型、操作类型、内容语义、伪造粒度）对AIGC篡改进行分类，并设计五个严格的跨维度评估协议，系统性地探测当前检测器的泛化边界。

Result: 对11个代表性模型的广泛实验表明：这些模型在原始设置下表现良好，但在模拟真实世界各种泛化场景的协议下，表现出系统性失败和显著的性能下降。
...
TL;DR: SoulX-LiveTalk是一个14B参数的实时音频驱动虚拟人生成框架，通过双向注意力蒸馏和多步自校正机制，在保持高视觉质量的同时实现0.87秒启动延迟和32FPS实时吞吐。


<details>
  <summary>Details</summary>
Motivation: 大规模扩散模型在实时、无限时长、音频驱动的虚拟人生成中存在计算负载与严格延迟约束的矛盾，现有方法通常通过强制单向注意力或降低模型容量来牺牲视觉质量。

Method: 1) 使用自校正双向蒸馏策略，在视频块内保留双向注意力以保持时空相关性；2) 多步回顾自校正机制，使模型能从累积错误中自主恢复；3) 全栈推理加速套件，包括混合序列并行、并行VAE和内核级优化。

Result: SoulX-LiveTalk是首个14B规模系统，实现0.87秒启动延迟和32FPS实时吞吐，在保持高视觉保真度的同时满足实时交互要求。

Conclusion: 该框架通过创新的双向注意力蒸馏和自校正机制，解决了实时虚拟人生成中计算效率与视觉质量的权衡问题，为高保真交互式数字人合成设立了新标准。
...
TL;DR: SOFTooth：一种语义增强、顺序感知的2D-3D融合框架，利用冻结的2D语义进行3D牙齿实例分割，无需显式2D掩码监督，在包含第三磨牙的复杂病例上表现优异。
...
Method: 提出SOFTooth框架：1）点级残差门控模块将咬合视图SAM嵌入注入3D点特征以细化边界；2）中心引导掩码细化正则化实例掩码与几何质心的一致性；3）顺序感知匈牙利匹配策略整合解剖牙齿顺序和中心距离，确保连贯标注。

Result: 在3DTeethSeg'22数据集上达到最先进的整体准确率和平均IoU，在涉及第三磨牙的病例上表现尤其突出，证明丰富的2D语义可以在无需2D微调的情况下有效转移到3D牙齿实例分割。

Conclusion: SOFTooth通过融合冻结2D语义和3D几何特征，解决了3D牙齿分割的关键挑战，为临床工作流程提供了实用解决方案，特别在复杂解剖结构和少数类别上表现优异。
...
TL;DR: 本文提出ArtQuant框架和RAD数据集，通过多维度结构化美学描述和LLM解码器，解决AIGC美学质量评估中的数据稀缺、不平衡和模型碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 美学质量评估对AIGC的人类对齐定量评估系统至关重要，但其复杂性（涵盖视觉感知、认知和情感）带来根本挑战。现有数据集过度关注视觉感知而忽略更深维度，且模型存在碎片化问题。

Method: 1) 提出RAD数据集：大规模（70k）、多维度结构化数据集，通过迭代流水线生成，无需昂贵人工标注；2) 提出ArtQuant框架：通过联合描述生成耦合孤立美学维度，利用LLM解码器更好建模长文本语义。

Result: 在多个数据集上达到最先进性能，仅需传统训练轮次的33%，缩小了艺术图像与美学判断之间的认知差距。理论分析证实数据语义充分性和生成范式共同最小化预测熵。

Conclusion: 通过RAD数据集和ArtQuant框架，有效解决了美学评估中的数据稀缺和模型碎片化问题，为AIGC的人类对齐美学评估提供了有效解决方案，将开源代码和数据集支持未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶中的世界模型方法存在局限性：虽然在统一架构中运行，但仍将世界预测和运动规划作为解耦过程处理。这导致预测与规划之间缺乏内在一致性，无法充分发挥世界模型的潜力。

Method: DriveLaW包含两个核心组件：DriveLaW-Video（生成高保真预测的强世界模型）和DriveLaW-Act（从DriveLaW-Video潜在表示生成一致可靠轨迹的扩散规划器）。采用三阶段渐进训练策略优化这两个组件。

Result: DriveLaW在两个任务上都取得了新的最先进结果：在视频预测方面，FID指标超越最佳工作33.3%，FVD提升1.8%；在NAVSIM规划基准测试中创造了新纪录。

Conclusion: DriveLaW通过统一视频生成与运动规划，实现了预测与规划的内在一致性，为自动驾驶世界模型提供了更有效的范式，显著提升了性能表现。
...
TL;DR: DDSPO提出了一种新的扩散模型偏好优化方法，通过直接从可用策略中获取时间步级监督，无需依赖昂贵的人工标注数据，提高了文本-图像对齐和视觉质量。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在文本到图像生成中难以完全对齐用户意图并保持一致的审美质量。现有的偏好训练方法如Diffusion DPO依赖昂贵且可能有噪声的人工标注数据集。

Method: DDSPO直接从获胜和失败策略中推导出每个时间步的监督信号，在去噪轨迹上提供密集的转换级信号。使用预训练参考模型自动生成偏好信号：对比原始提示和语义降级变体条件下的输出。

Result: DDSPO在文本-图像对齐和视觉质量方面都有改进，优于或匹配现有的基于偏好的方法，同时需要显著更少的监督。

Conclusion: DDSPO提供了一种有效的分数空间偏好监督方法，无需显式的奖励建模或人工标注，为扩散模型偏好优化提供了实用解决方案。
...
TL;DR: 该研究评估了不确定性量化能否提升SAM在挑战性分割场景下的泛化能力，创建了UncertSAM基准并测试了多种后验不确定性估计方法，发现拉普拉斯近似能有效关联分割误差。


<details>
  <summary>Details</summary>
Motivation: 尽管SAM等基础分割模型在零样本任务上表现良好，但在领域偏移或知识受限的场景下仍然脆弱。本研究旨在探索不确定性量化能否以领域无关的方式缓解这些挑战并增强模型泛化能力。

Method: 1) 创建UncertSAM基准，包含8个设计用于压力测试的数据集，涵盖阴影、透明度和伪装等挑战性分割条件；2) 评估一系列轻量级后验不确定性估计方法；3) 评估初步的不确定性引导预测细化步骤。

Result: 在评估的方法中，最后一层的拉普拉斯近似产生的不确定性估计与分割误差有良好相关性，表明存在有意义的信号。细化步骤的益处尚属初步，但结果显示了将不确定性纳入分割模型以支持鲁棒、领域无关性能的潜力。

Conclusion: 不确定性量化有潜力增强分割模型在挑战性场景下的鲁棒性和泛化能力。研究提供了公开的基准和代码，为后续研究奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 传统道路监测方法昂贵且不系统，需要大量测量时间。需要一种实时、系统的道路状态监测方法，为车辆规划和主动控制系统提供有价值信息。

Method: 1. 使用手机摄像头在卡尔斯鲁厄理工学院校园周围道路采集数据；2. 测试多种基于图像的深度学习算法（AlexNet、LeNet、VGG、ResNet）进行道路分类；3. 将道路加速度数据与图像数据结合训练；4. 比较加速度基和摄像头图像基方法的性能；5. 使用模糊逻辑根据天气和时间对路面进行分类。

Result: 在5类道路条件分类（沥青、损坏沥青、碎石路、损坏碎石路、铺面路）中，实现了超过95%的准确率。比较了不同深度学习算法的性能表现。

Conclusion: 提出的实时系统能够有效监测道路表面状态，结合图像和加速度数据以及模糊逻辑方法，为车辆控制和规划提供了实用的解决方案。
...
TL;DR: RealX3D是一个真实采集的多视图视觉恢复和3D重建基准数据集，包含多种物理退化类型，用于评估方法在真实世界挑战性环境下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前多视图视觉恢复和3D重建方法在理想条件下表现良好，但在真实世界中的物理退化（如光照变化、散射、遮挡、模糊等）下性能显著下降，缺乏系统评估这些物理退化影响的基准数据集。

Method: 通过统一的采集协议捕获像素对齐的低质量/高质量视图，将退化分为四类（光照、散射、遮挡、模糊），每个类别包含多个严重程度级别。每个场景包含高分辨率捕获、RAW图像和密集激光扫描，从中导出世界尺度网格和度量深度。

Result: 对基于优化的和前馈方法进行广泛基准测试显示，在物理退化下重建质量显著下降，突显了当前多视图管道在真实世界挑战性环境中的脆弱性。

Conclusion: RealX3D基准数据集揭示了当前多视图视觉恢复和3D重建方法在真实世界物理退化下的局限性，为开发更鲁棒的算法提供了重要的评估工具。
...
TL;DR: CoFi-Dec是一个无需训练的解码框架，通过生成自反馈和粗到细视觉条件来减少大视觉语言模型中的幻觉问题。
...
Method: CoFi-Dec采用训练自由的解码框架，受人类从全局场景感知到详细检查的视觉过程启发，首先生成基于粗粒度和细粒度视图的中间文本响应，然后使用文本到图像模型将其转换为合成图像，形成多级视觉假设。引入基于Wasserstein的融合机制，将多个视觉条件的预测分布对齐到几何一致的解码轨迹中。

Result: 在六个专注于幻觉的基准测试中，CoFi-Dec显著减少了实体级和语义级幻觉，优于现有的解码策略。

Conclusion: 该框架是模型无关的，无需额外训练，可以无缝应用于广泛的大视觉语言模型，通过统一高层语义一致性和细粒度视觉基础，产生更稳健和忠实的输出。
...
TL;DR: 提出结合视觉水线检测、YOLOv8姿态尺度提取和大规模多模态语言模型的混合框架，用于自动读取河流水位标尺，实现高精度水文监测。


<details>
  <summary>Details</summary>
Motivation: 传统水文观测方法受限于人工测量误差和环境限制，需要准确连续的水位监测用于洪水预报、水资源管理和生态保护。

Method: 采用序列化方法：图像预处理、标注、水线检测、尺度间隙估计和数值读取提取，结合视觉检测与多模态语言模型（GPT-4o和Gemini 2.0 Flash）。

Result: 水线检测精度达94.24%，F1分数83.64%；尺度间隙检测为后续读取提供准确几何校准；结合尺度间隙元数据显著提升LLM性能，Gemini Stage 2在最优图像条件下MAE为5.43cm，RMSE为8.58cm，R²为0.84。

Conclusion: 该方法为自动化水文监测提供了可扩展、高效可靠的解决方案，展示了实时河流标尺数字化和改进水资源管理的潜力，强调结合几何元数据与多模态AI对鲁棒水位估计的重要性。
...
TL;DR: 提出Dual-approx Bridge模型，利用布朗桥动态和双近似器实现高保真、低方差的确定性图像到图像转换


<details>
  <summary>Details</summary>
Motivation: 在图像到图像转换任务中，确定性方法需要保证输入到输出的稳定一致映射，同时保持高保真度和图像质量。现有方法在保持输出一致性和图像质量方面存在挑战。

Method: 提出基于布朗桥动态的去噪模型，使用两个神经网络近似器（前向和反向过程）来生成高保真输出，实现低方差的确定性图像转换。

Result: 在图像生成和超分辨率等基准数据集上的实验表明，Dual-approx Bridge在图像质量和保真度方面优于随机性和确定性基线方法。

Conclusion: Dual-approx Bridge模型通过布朗桥动态和双近似器设计，实现了高质量、低方差的确定性图像到图像转换，在多个任务中表现优异。
...
TL;DR: HY-Motion 1.0是一个基于扩散变换器的大规模3D人体运动生成模型，通过亿级参数规模和全阶段训练范式，实现了从文本描述生成高质量3D人体运动，性能超越当前开源基准。


<details>
  <summary>Details</summary>
Motivation: 当前3D人体运动生成领域缺乏大规模、高性能的开源模型，需要开发能够精确遵循文本指令并生成高质量运动的模型，推动该领域向商业化成熟发展。

Method: 采用基于扩散变换器的流匹配模型，扩展到亿级参数规模；实施全阶段训练范式：包括3000+小时运动数据的大规模预训练、400小时精选数据的高质量微调，以及基于人类反馈和奖励模型的强化学习；配合严格的数据处理流程进行运动清洗和标注。

Result: 模型实现了最广泛的动作覆盖，涵盖6大类超过200个动作类别；在指令跟随能力上显著超越当前开源基准；模型已开源发布以促进未来研究。

Conclusion: HY-Motion 1.0是运动生成领域首个成功扩展到亿级参数的扩散变换器模型，通过创新的全阶段训练范式实现了高质量的文本到运动生成，有望加速3D人体运动生成模型的商业化进程。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的点云配准方法通常依赖欧几里得邻域策略进行特征提取，难以有效捕捉点云中的隐式语义和结构一致性，需要更强大的特征学习方法
...
Conclusion: MCI-Net通过多域上下文集成有效提升了点云特征表示和配准性能，为点云配准任务提供了更鲁棒和判别性的特征学习方法
...
TL;DR: SC-Net通过空间和通道双重视角整合双边上下文，提出自适应聚焦正则化、双边场调整和位置感知恢复模块，在相对姿态估计和异常值去除任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有CNN骨干网络在双视角对应学习中虽优于多层感知机方法，但未针对特定任务优化，难以有效聚合全局上下文，且在视差大的场景中会过度平滑密集运动场。

Method: 提出SC-Net网络，包含三个核心模块：1) 自适应聚焦正则化模块(AFR)增强位置感知和对虚假运动样本的鲁棒性；2) 双边场调整模块(BFA)通过建模长程关系和跨空间-通道维度交互来优化运动场；3) 位置感知恢复模块(PAR)从优化场中恢复运动向量，确保一致性和精度。

Result: 在YFCC100M和SUN3D数据集上的大量实验表明，SC-Net在相对姿态估计和异常值去除任务上优于最先进方法。

Conclusion: SC-Net通过空间和通道双重视角有效整合双边上下文，解决了CNN骨干网络在双视角对应学习中的局限性，在多个任务上取得了优异性能。
...
TL;DR: TV-RAG：一种无需训练的架构，通过时间对齐和熵引导语义增强长视频推理能力，结合时间衰减检索和熵加权关键帧采样，提升现有大型视频语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有大型视频语言模型在处理长视频时存在局限性：时间窗口狭窄，无法捕捉长时间跨度的语义变化；主流文本检索方法仅依赖表层词汇重叠，忽略了视觉、音频和字幕通道之间的时间相互依赖性。

Method: 提出TV-RAG架构，包含两个核心机制：1) 时间衰减检索模块，将显式时间偏移注入相似度计算，根据真实多媒体上下文对文本查询进行排序；2) 熵加权关键帧采样器，选择均匀间隔、信息密集的帧，减少冗余同时保持代表性。

Result: TV-RAG在Video-MME、MLVU和LongVideoBench等长视频基准测试中持续超越大多数领先基线，验证了模型的有效性。该框架无需重新训练或微调即可集成到任何LVLM中，提供轻量级、经济高效的升级路径。

Conclusion: TV-RAG通过结合时间和语义信号，实现了双级推理机制，有效解决了长视频处理中的时间窗口狭窄和语义变化捕捉问题，为现有大型视频语言模型提供了实用的性能增强方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注基本几何关系或局部特征，忽略了对象间的跨尺度上下文交互，限制了复杂场景的理解能力

Method: 提出深度全景上下文聚合网络(PanCAN)，在高维希尔伯特空间中通过跨尺度特征聚合分层整合多阶几何上下文，结合随机游走和注意力机制学习多阶邻域关系，跨尺度模块级联实现动态特征融合

Result: 在NUS-WIDE、PASCAL VOC2007和MS-COCO基准测试中取得竞争性结果，在定量和定性评估中均优于现有技术，显著提升多标签分类性能

Conclusion: PanCAN通过有效的跨尺度建模和多阶上下文特征融合，显著增强了复杂场景理解能力，为视觉识别中的上下文建模提供了新思路
...
TL;DR: IdentityStory是一个用于以人物为中心的图像故事生成框架，通过迭代身份发现和重去噪身份注入技术，确保多张序列图像中角色身份的一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉生成模型虽然能从文本生成故事，但在以人物为中心的故事生成中面临额外挑战：难以保持详细多样的人脸一致性，以及在不同图像中协调多个角色。
...
Conclusion: IdentityStory通过创新的身份保持生成技术，有效解决了以人物为中心的故事生成中的身份一致性问题，为视觉故事创作提供了新的解决方案。
...
TL;DR: IAFS：一种无需训练的推理时缩放框架，通过迭代细化和频率感知粒子融合，解决扩散模型在图像超分辨率中感知质量与结构保真度的平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在图像超分辨率中难以同时保证高频感知质量和低频结构保真度。现有的推理时缩放策略存在不足：奖励驱动的粒子优化会导致感知过度平滑，而最优路径搜索则容易失去结构一致性。

Method: 提出IAFS框架，结合迭代细化和频率感知粒子融合。通过迭代修正结构偏差逐步细化生成图像，同时自适应地融合高频感知线索和低频结构信息，实现跨不同图像细节的准确平衡重建。

Result: 在多个基于扩散的超分辨率模型上的广泛实验表明，IAFS有效解决了感知-保真度冲突，持续改善了感知细节和结构准确性，优于现有的推理时缩放方法。

Conclusion: IAFS是一个无需训练的推理时缩放框架，通过迭代频率引导有效平衡了图像超分辨率中的感知质量和结构保真度，为扩散模型提供了更好的性能表现。
...
Method: 提出AnyMS框架，采用无需训练的方法，利用文本提示、主体图像和布局约束三个输入条件，引入自底向上的双级注意力解耦机制：全局解耦分离文本和视觉条件的交叉注意力以确保文本对齐；局部解耦将每个主体的注意力限制在其指定区域以防止冲突。

Result: AnyMS在实验中达到最先进性能，支持复杂构图并扩展到更多主体，无需主体学习或适配器调优。

Conclusion: AnyMS通过创新的注意力解耦机制解决了多主体定制中的关键挑战，实现了无需训练的高效布局控制，具有良好的可扩展性。
...
TL;DR: PathFound：一种用于病理诊断的代理多模态模型，通过主动信息获取和诊断细化的证据寻求工作流程，显著提升诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 当前病理基础模型大多采用静态推理范式，一次性处理全切片图像产生预测，缺乏在模糊诊断下的重新评估或针对性证据获取。这与临床诊断工作流程形成对比，后者通过重复观察切片和进一步检查请求来完善假设。

Method: PathFound整合了病理视觉基础模型、视觉语言模型和强化学习训练推理模型，通过初始诊断、证据寻求和最终决策三个阶段，执行主动信息获取和诊断细化。

Result: 采用证据寻求策略一致提高了多个大型多模态模型的诊断准确性。PathFound在多样化临床场景中实现了最先进的诊断性能，并展现出发现细微细节（如核特征和局部浸润）的强大潜力。

Conclusion: 证据寻求工作流程在计算病理学中具有显著有效性，PathFound通过模拟临床诊断的迭代推理过程，为病理诊断提供了更准确和可靠的解决方案。
...
TL;DR: PurifyGen是一种无需训练的双阶段提示净化方法，通过语义距离评估token安全性，将有害语义投影到有毒概念零空间，同时对齐到清洁概念范围空间，实现安全文本到图像生成。
...
Method: 提出双阶段提示净化策略：1) 通过计算互补语义距离评估每个token的安全性，测量提示token与预定义有毒和清洁列表概念嵌入的语义接近度；2) 对风险提示应用双空间变换：将有毒性对齐的嵌入投影到有毒概念矩阵的零空间以移除有害语义成分，同时将其对齐到清洁概念的范围空间，选择性替换风险token嵌入。

Result: 在五个数据集上的广泛测试表明，PurifyGen在减少不安全内容方面超越了现有方法，并且与依赖训练的方法竞争良好。该方法具有理论依据，对未见过的提示和模型具有强泛化能力。

Conclusion: PurifyGen提供了一种即插即用的解决方案，无需修改模型原始权重，通过语义净化实现了安全文本到图像生成，在保留原始意图和连贯性的同时有效减少不安全内容生成。
...
Method: 开发了RxnBench多层级基准，包含两个任务：1) 单图问答：基于305个精选反应方案生成的1525个问题，测试细粒度视觉感知和机理推理；2) 全文问答：基于108篇文章，要求跨模态整合文本、反应方案和表格信息。

Result: 评估发现关键能力差距：模型擅长提取显式文本，但在深层化学逻辑和精确结构识别方面表现不佳。具有推理时推理能力的模型显著优于标准架构，但所有模型在全文问答任务上的准确率均未超过50%。

Conclusion: 需要开发领域特定的视觉编码器和更强的推理引擎，以推进自主AI化学家的发展，当前模型在化学文献理解方面仍有显著局限性。
...
TL;DR: ThinkGen：首个基于思维链推理的视觉生成框架，通过解耦的MLLM和DiT架构实现多场景生成，采用SepGRPO训练范式在多个基准上达到SOTA性能。
...
Conclusion: ThinkGen是首个将MLLM的思维链推理明确应用于多种生成场景的视觉生成框架，其灵活设计支持跨数据集的联合训练，为广泛的生成场景提供了有效的思维链推理解决方案。
...
TL;DR: 提出Haar-tSVD方法，结合Haar变换和张量奇异值分解进行图像去噪，平衡速度与性能，并引入自适应噪声估计和深度神经网络增强


<details>
  <summary>Details</summary>
Motivation: 随着成像设备普及和图像数据激增，对高效有效的图像去噪需求日益增长，需要平衡去噪速度与性能的方法

Method: 建立PCA与Haar变换在循环表示下的理论联系，提出Haar-tSVD方法：结合张量奇异值分解投影和Haar变换捕捉全局和局部相关性，采用自适应噪声估计，并集成深度神经网络增强性能

Result: 在多个去噪数据集上的实验结果表明，该方法在噪声去除方面具有高效性和有效性
...
<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展导致多模态安全风险不断涌现，现有防御方法多为被动反应式，需要模型调整，无法有效应对分布外安全风险。

Method: 1) 构建包含87K样本的模态平衡数据集，采用分层多模态安全分类法标注；2) 通过强化学习训练视觉语言基础模型；3) 引入分布外安全类别推理任务，并添加基于同义词库的相似性奖励机制。

Result: ProGuard在二元安全分类上达到与闭源大模型相当的性能，在非安全内容分类上显著优于现有开源防护模型，分布外风险检测提升52.6%，风险描述提升64.8%。

Conclusion: ProGuard通过主动防护方法有效解决了多模态安全风险问题，特别是在分布外风险识别和描述方面表现出色，为生成模型安全防护提供了新思路。
...
TL;DR: 提出LiveTalk系统，通过改进蒸馏方法实现实时多模态交互视频生成，将响应延迟从1-2分钟降低到实时生成


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型通过双向注意力迭代去噪所有视频帧，无法实现实时交互。现有蒸馏方法主要关注文本到视频生成，在多模态条件下存在视觉伪影和质量下降问题

Method: 改进蒸馏配方，重点关注条件输入质量、初始化策略和在线优化调度。结合音频语言模型和长视频推理技术Anchor-Heavy Identity Sinks构建LiveTalk系统

Result: 蒸馏模型在HDTF、AVSpeech和CelebV-HQ基准测试中，以20倍更低的推理成本和延迟匹配全步双向基线的视觉质量。LiveTalk在多轮交互基准测试中优于Sora2和Veo3
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在粗粒度视觉理解上表现良好，但缺乏细粒度感知能力，容易忽略细微视觉细节。现有训练数据强调一般识别而非细粒度感知，限制了模型性能。

Method: 构建TWIN数据集，包含56.1万张图像对查询，要求模型判断两个视觉相似的图像是否描绘同一物体。引入FGVQA基准测试套件，包含1.2万查询，用于评估细粒度识别性能。

Result: 在TWIN上微调的视觉语言模型在FGVQA基准上提升高达19.3%，在艺术、动物、植物、地标等未见领域也表现良好，且不影响通用VQA基准性能。

Conclusion: TWIN数据集能有效提升视觉语言模型的细粒度感知能力，数据集规模是关键因素，可作为开源VLM训练数据的补充，推动未来模型的感知精度发展。
...
TL;DR: 该研究提出三方面贡献：额外NIR数据集、两阶段检测模型和Patched-YOLO，旨在提升红外夜视摄像头的火灾检测精度，特别是减少人工光源误报。


<details>
  <summary>Details</summary>
Motivation: 红外夜视摄像头火灾检测精度提升仍具挑战性，现有模型（如YOLOv7、RT-DETR、YOLOv9）虽有一定性能，但数据集构建限制导致人工亮光常被误判为火灾，需要改进。

Method: 1) 构建额外NIR数据集并应用多种数据增强策略；2) 提出两阶段检测管道，结合YOLOv11和EfficientNetV2-B0，先检测再分类以减少误报；3) 提出Patched-YOLO，通过基于补丁的处理增强RGB图像中小型和远距离目标的检测能力。

Result: 相比先前方法（YOLOv7 mAP50-95 0.51、RT-DETR 0.65、YOLOv9 0.598），所提方法在夜间火灾检测中达到更高检测精度，特别是减少了人工光源导致的误报。

Conclusion: 通过数据集增强、两阶段检测架构和补丁处理策略，有效提升了红外夜视摄像头的火灾检测性能，解决了人工光源误报问题，并改善了RGB图像中小目标的检测能力。
...
TL;DR: 提出SRFA框架用于胰腺肿瘤早期检测，通过多阶段处理达到96.23%准确率，显著优于传统CNN和transformer模型。


<details>
  <summary>Details</summary>
Motivation: 胰腺肿瘤早期检测面临临床困境，主要因为CT扫描中肿瘤对比度低、患者间解剖变异大，需要能增强细微视觉线索并具有良好泛化能力的系统。
...
Conclusion: SRFA框架在胰腺肿瘤早期检测中表现出优异性能，可作为临床有用的工具，其多阶段处理和优化策略有效解决了胰腺肿瘤检测的复杂性。
...
TL;DR: 本文提出了一个评估框架来量化3D生成模型中的记忆化现象，并通过实验发现数据模态、多样性和建模设计对记忆化的影响，同时提出了降低记忆化的有效策略。
...
Conclusion: 该框架和分析提供了对3D生成模型记忆化的实证理解，并提出了简单有效的策略来降低记忆化而不影响生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常使用注意力机制对齐跨帧物体，依赖统一的显式物理模型（如恒定速度），但不同类别和帧间的运动状态变化使这种对齐不够理想。

Method: HAT首先利用多个显式运动模型生成空间锚点和运动感知特征提案，然后结合缓存物体查询中的语义和运动线索进行多假设解码，为目标帧提供最优对齐方案。

Result: 在nuScenes数据集上，HAT显著提升了3D时序检测器和跟踪器性能，与DETR3D检测器结合达到46.0% AMOTA的SOTA跟踪结果；在端到端AD方法中提升感知精度并降低32%碰撞率。
...
TL;DR: OmniAgent：一种完全音频引导的主动感知智能体，通过动态编排专用工具实现细粒度音视频推理，在三个基准测试中达到SOTA性能，超越现有模型10%-20%


<details>
  <summary>Details</summary>
Motivation: 现有全模态大语言模型在统一音视频模态方面取得进展，但缺乏细粒度跨模态理解能力，难以实现多模态对齐。传统方法依赖静态工作流程和密集帧标注，存在局限性。

Method: 提出从被动响应生成到主动多模态查询的范式转变。采用动态规划自主编排工具调用，战略性地聚焦任务相关线索。核心是新颖的粗到细音频引导感知范式，利用音频线索定位时间事件并指导后续推理。

Result: 在三个音视频理解基准测试上进行广泛实证评估，OmniAgent实现了最先进的性能，显著超越领先的开源和专有模型，准确率提升10%-20%。

Conclusion: OmniAgent通过动态工具编排和音频引导的主动感知，解决了现有全模态模型在细粒度跨模态理解和多模态对齐方面的局限性，为音视频推理任务提供了更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: RGB图像将材质属性、光照和视角相关效应纠缠在一起，而现有基于扩散的方法在单视角本征分解上表现良好，但扩展到多视角设置时面临挑战，常导致严重的视角不一致问题。

Method: IDT采用基于物理的图像形成模型，将图像明确分解为漫反射率、漫反射着色和镜面反射着色。通过Transformer注意力机制联合推理多个输入图像，分离Lambertian和非Lambertian光传输。

Result: 在合成和真实世界数据集上的实验表明，IDT实现了更清晰的漫反射率、更一致的漫反射着色和更好分离的镜面反射分量，相比先前方法显著提高了多视角一致性。

Conclusion: IDT通过前馈Transformer框架成功解决了多视角本征图像分解中的视角一致性问题，实现了可解释和可控的材质与光照效应分解，为视觉理解提供了更好的基础。
...
<details>
  <summary>Details</summary>
Motivation: 透明/反射物体对传统感知系统极具挑战性，因为折射、反射和透射破坏了立体视觉、ToF和单目深度估计的基本假设，导致深度估计存在空洞和时间不稳定问题。作者观察到现代视频扩散模型已经能够合成逼真的透明现象，表明它们已经内化了光学规则。

Method: 1. 构建TransPhy3D合成视频数据集：11k个序列，使用Blender/Cycles渲染，包含RGB+深度+法线；2. 从大型视频扩散模型出发，通过轻量级LoRA适配器学习视频到视频的深度（和法线）转换器；3. 在训练时将RGB和（含噪声的）深度潜在表示在DiT骨干网络中拼接，并在TransPhy3D和现有帧级合成数据集上联合训练；4. 得到DKT模型，能够对任意长度输入视频产生时间一致的预测。

Result: DKT在涉及透明度的真实和合成视频基准测试中实现零样本SOTA：ClearPose、DREDS（CatKnown/CatNovel）和TransPhy3D-Test。相比强图像/视频基线，提高了准确性和时间一致性。法线变体在ClearPose上创下最佳视频法线估计结果。紧凑的1.3B版本运行速度约0.17秒/帧。集成到抓取系统中，DKT的深度估计提高了在透明、反射和漫反射表面上的成功率，优于先前估计器。

Conclusion: 结果支持更广泛的主张："扩散模型理解透明度"。生成式视频先验可以高效、无标签地重新用于实现鲁棒、时间一致性的感知，应对具有挑战性的现实世界操作任务。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的视频超分辨率方法虽然感知质量好，但依赖未来帧和昂贵的多步去噪，无法满足延迟敏感场景的需求。需要开发适合在线部署的低延迟扩散VSR方法。
