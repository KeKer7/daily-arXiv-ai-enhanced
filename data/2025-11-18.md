...
Result: 与Gemini-2.5-Pro配对，DocLens在MMLongBench-Doc和FinRAGBench-V上实现了最先进的性能，甚至超越了人类专家，在视觉中心和不可回答查询上表现尤为突出。
...
<details>
  <summary>Details</summary>
Motivation: 现有优化方法需要密集多视图输入和昂贵的逐实例优化，限制了可扩展性；而现有feedforward方法通常产生粗糙几何、缺乏纹理重建，且依赖脆弱的多阶段流水线。

Method: 扩展LVSM静态3D物体新视角合成方法到关节设置，使用基于transformer的架构联合推理相机姿态和关节变化，生成深度图和部件掩码以支持显式3D网格提取和关节估计。

Result: 在多个物体类别上实现高保真重建，在新视角和状态合成以及3D关节物体重建方面优于最先进方法，生成与输入图像紧密贴合的高质量网格。

Conclusion: LARM提供了一个可扩展且准确的解决方案，无需密集监督即可实现高质量的3D关节物体重建，在多个任务上表现出色。
